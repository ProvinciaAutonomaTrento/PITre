--------------------------------------------------------
--  DDL for Procedure COMPUTEATIPICITA
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ITCOLL_6GIU12"."COMPUTEATIPICITA" (
      ...
      THEN
         BEGIN
            ...
            ELSE
               ...
               IF    newlevel < oldlevel
                  OR (iduo != idvecchiauo AND calcolasuisottoposti = 1)
               THEN
                  BEGIN
                     querycustomdoc :=
                           ' select /*+ index(s)*/ distinct(s.thing)
                                    from security s
                                    inner join profile p
                                    on p.system_id = s.thing
                                    where personorgroup in (

...
namespace DocsPaDB.Query_DocsPAWS
{
    ...
    public class Amministrazione : DBProvider
    {
        ...
        public string FattElCtrlDupl(string idTibco)
        {
            string retval = "";
            try
            {
                DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("FATT_S_INTEGR_FATTURA");
                q.setParam("idTibco", idTibco);
                string queryString = q.getSQL();
                logger.Debug(queryString);
                DataSet dataset = new DataSet();
                ...
                {
                    retval = dataset.Tables["CTRLFATT"].Rows[0]["ID_PITRE"].ToString();
                }
            }
            catch (Exception e)
            {
        ...
        public bool FattElInsTabellaIntegrazione(string idDoc, string idTibco, string oggFattura)
        {
            bool retval = false;
            try
            {
                DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("FATT_I_INTEGR_FATTURA");
                q.setParam("idDoc", idDoc);
                q.setParam("idTibco", idTibco);
                q.setParam("oggettoFattura", oggFattura);
                string querystring = q.getSQL();
                logger.Debug(querystring);
                retval = this.ExecuteNonQuery(querystring);

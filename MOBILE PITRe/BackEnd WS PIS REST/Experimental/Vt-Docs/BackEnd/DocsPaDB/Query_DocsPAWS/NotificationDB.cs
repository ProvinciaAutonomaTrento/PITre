...
namespace DocsPaDB.Query_DocsPAWS
{
    public class NotificationDB : DBProvider
    {
                ...
                if (ds.Tables["notifications"] != null && ds.Tables["notifications"].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables["notifications"].Rows)
                    {
                        Notification notification = new Notification()
                        {
                            ...
                            ACCESSRIGHTS = !string.IsNullOrEmpty(row["ACCESSRIGHTS"].ToString()) ? row["ACCESSRIGHTS"].ToString() : string.Empty,
                        };
        ...
        /// <returns></returns>
        public bool RemoveNotifications(List<Notification> listNotification)
        {
            bool result = false;
            try
            {

...
{
    public class HSMConnector_arsss : ArubaConnector.I_HSMConnector 
    {
        private static ILog logger = LogManager.GetLogger(typeof(HSMConnector_arsss));

        public  object createClient(string endPoindAddress)
        {
            ArubaSignServiceService ss = new ArubaSignServiceService();
        ...
        */
        ...
        private string componiErrore(Exception e, string metodo)
        {
        ...
        public byte[] FirmaFilePADES(byte[] fileDafirmare, string aliasCertificato, string dominioCertificato, string pinCertificato, string otpFirma, bool marcaTemporale, object client)
        {
            ...
            ArubaSignServiceService wsclient = client as ArubaSignServiceService;
            if (wsclient == null)
                return null;

            ArubaConnector.ArubaSignServices.auth authData = new ArubaConnector.ArubaSignServices.auth
            {
                otpPwd = otpFirma,
            ...
            ArubaConnector.ArubaSignServices.signRequestV2 sr = new ArubaConnector.ArubaSignServices.signRequestV2 { binaryinput = fileDafirmare, transport = ArubaConnector.ArubaSignServices.typeTransport.BYNARYNET, transportSpecified = true, requiredmark = marcaTemporale, identity = authData, certID = "AS0", profile = null };
            ArubaConnector.ArubaSignServices.pdfSignApparence pdfsignApp = new ArubaConnector.ArubaSignServices.pdfSignApparence { page = 1 };
            ArubaConnector.ArubaSignServices.signReturnV2 retval = wsclient.pdfsignatureV2(sr, pdfsignApp, ArubaSignServices.pdfProfile.PADESBES, false, "");
        ...
        public byte[] FirmaFileCADES(byte[] fileDafirmare, string aliasCertificato, string dominioCertificato, string pinCertificato, string otpFirma, bool marcaTemporale, bool cofirma, object client)
        {
            ...
            ArubaConnector.ArubaSignServices.signRequestV2 sr = new ArubaConnector.ArubaSignServices.signRequestV2 { binaryinput = fileDafirmare, transport = ArubaConnector.ArubaSignServices.typeTransport.BYNARYNET, transportSpecified = true, requiredmark = marcaTemporale, identity = authData, certID = "AS0", profile = null };
            ArubaConnector.ArubaSignServices.signReturnV2 retval = null;

            if (cofirma) //aggiunta firma parallela.
                retval = wsclient.addpkcs7sign(sr, false);
            else
                retval = wsclient.pkcs7signV2(sr, false, false);
            ...
            if (Directory.Exists(sessionDir))
            {
                ...
                if (File.Exists(manifestFile))
                {
                    ...
                    foreach (Manifest.MainfestFileInformation FileInformation in mft.FileInformation)
                    {
                        ...
                        ArubaConnector.ArubaSignServices.signReturnV2 retval = null;
                        ArubaConnector.ArubaSignServices.signRequestV2 sr = new ArubaConnector.ArubaSignServices.signRequestV2 { binaryinput = content, transport = ArubaConnector.ArubaSignServices.typeTransport.BYNARYNET, transportSpecified = true, requiredmark = mft.timestamp, identity = authData, certID = "AS0", profile = null };
                        ...
                        {
                            retval = wsclient.pdfsignatureV2(sr, pdfsignApp, ArubaSignServices.pdfProfile.PADESBES, false, "");
                            if (retval.status.Equals("OK"))
                            {
                                filesRead.Add(retval.binaryoutput);
                            }

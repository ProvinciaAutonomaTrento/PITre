...
{
    internal class ShortSectorAllocation
    {
        CompoundDocument Document;

        List<int> ShortSectorAllocationTable;
        ...
        {
            int newSectorID = AllocateSector();
            LinkSectorID(sectorID, newSectorID);
            return newSectorID;
        }
        ...
        public void Save()
        {
            if (ShortSectorAllocationTable.Count > 0)
            {
                if (Document.Header.FirstSectorIDofShortSectorAllocationTable == SID.EOC)
                {
                    ...
                    int[] sids = new int[SecIDCapacity];

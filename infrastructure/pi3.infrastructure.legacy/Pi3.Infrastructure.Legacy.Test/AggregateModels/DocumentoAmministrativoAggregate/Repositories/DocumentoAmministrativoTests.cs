...
namespace Pi3.Infrastructure.Legacy.EF.AggregateModels.DocumentoAmministrativoAggregate.Repositories.Tests
{
    public class DocumentoAmministrativoTests
    {
        ...
        IDocumentoAmministrativoRepository _documentoAmministrativoRepository;
        IDocumentBlobRepository _documentBlobRepository;

        [OneTimeSetUp]
        public void Setup()
        {
            var variables = JsonConvert.DeserializeObject<Dictionary<string, string>>(Pi3.Infrastructure.Legacy.Test.Resources.DocumentoAmministrativoTestsVariables);
            ...
            this._documentoAmministrativoRepository = this._serviceProvider.GetRequiredService<IDocumentoAmministrativoRepository>();
            this._documentBlobRepository = this._serviceProvider.GetRequiredService<IDocumentBlobRepository>();
        }
        ...
        public async Task PredisponiEProtocollaDocumentoTest()
        {
            await TestRunner.Run(async () =>
            {
                ...
                Assert.That(!string.IsNullOrWhiteSpace(aggregate.Id)
                    && aggregate.DatiRegistrazione! != null!
                    && aggregate.DatiRegistrazione!.IsRegistrato == true
                    && aggregate.DatiRegistrazione is DatiRegistrazioneProtocollo
                    && ((DatiRegistrazioneProtocollo)aggregate.DatiRegistrazione).NumeroProtocollo.HasValue
                    && ((DatiRegistrazioneProtocollo)aggregate.DatiRegistrazione).TipologiaFlusso == TipologiaFlussoEnum.E
                    && ((DatiRegistrazioneProtocollo)aggregate.DatiRegistrazione).CodiceRegistro != null
                    && ((DatiRegistrazioneProtocollo)aggregate.DatiRegistrazione).DataProtocollazione.HasValue);
            });
        }
        ...
        public async Task AssignDocumentBlobDocumentoProtocollatoInEntrataTest()
        {
            await TestRunner.Run(async () =>
            {
                ...
                Assert.That(!string.IsNullOrWhiteSpace(aggregate.Id)
                    && aggregate.CurrentVersion.DocumentBlobRef! != null!);
            });
        }
        ...
        public async Task CreateEmptyVersionDocumentoProtocollatoInEntrataTest()
        {
            await TestRunner.Run(async () =>
            {
                ...
                Assert.That(!string.IsNullOrWhiteSpace(aggregate.Id)
                    && aggregate.CurrentVersion.Name.Value == "Nuova versione vuota");
            });
        }
        ...
        public async Task RemoveEmptyVersionDocumentoProtocollatoInEntrataTest()
        {
            await TestRunner.Run(async () =>
            {
                ...
                aggregate.RemoveVersion(aggregate.CurrentVersion.Id);

                await _documentoAmministrativoRepository.Update(aggregate);

                Assert.That(!string.IsNullOrWhiteSpace(aggregate.Id));
                // TODO: qui dovrebbe rimuovere la versione dall'aggregato
            });
        }
        ...
        public async Task AddProtocolloUscitaTest()
        {
            await TestRunner.Run(async () =>
            {
                ...
                Assert.That(!string.IsNullOrWhiteSpace(aggregate.Id)
                    ...
                    && aggregate.DatiRegistrazione is DatiRegistrazioneProtocollo
                    && ((DatiRegistrazioneProtocollo)aggregate.DatiRegistrazione).NumeroProtocollo.HasValue
                    && ((DatiRegistrazioneProtocollo)aggregate.DatiRegistrazione).TipologiaFlusso == TipologiaFlussoEnum.U
                    && ((DatiRegistrazioneProtocollo)aggregate.DatiRegistrazione).CodiceRegistro != null
                    && ((DatiRegistrazioneProtocollo)aggregate.DatiRegistrazione).DataProtocollazione.HasValue);
            });
        }
        ...
        public async Task GetDocumentoStampaRegistroTest()
        {
            await TestRunner.Run(async () =>
            {
                var aggregate = await GetDocumentoAmministrativo(Environment.GetEnvironmentVariable("IdDocumentoStampaRegistro")!, true);
        ...
        public async Task GetDocumentoStampaRepertorioTest()
        {
            await TestRunner.Run(async () =>
            {
                var aggregate = await GetDocumentoAmministrativo(Environment.GetEnvironmentVariable("IdDocumentoStampaRepertorio")!, true);
        ...
        private async Task<DocumentBlob> CreateDocumentBlobAggregate()
        {
            ...
            using var ms = new MemoryStream(Pi3.Infrastructure.Legacy.Test.Resources.documentoTest1, 0, Pi3.Infrastructure.Legacy.Test.Resources.documentoTest1.Length);

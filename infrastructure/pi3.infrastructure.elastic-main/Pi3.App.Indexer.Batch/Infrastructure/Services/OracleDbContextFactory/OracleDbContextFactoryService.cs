...
namespace Pi3.App.Indexer.Infrastructure.Services.OracleDbContextFactory
{
    ...
    public class OracleDbContextFactoryService : IOracleDbContextFactoryService
    {
        ...
        {
            var connectionString = this._configuration.GetConnectionString(this._options.Value.Instance);

            this._logger.LogInformation($"Instance: {this._options.Value.Instance} - ConnectionString: {connectionString}");

            if (string.IsNullOrWhiteSpace(connectionString))
                throw new ConnectionStringNotFoundPi3Exception(this._options.Value.Instance);

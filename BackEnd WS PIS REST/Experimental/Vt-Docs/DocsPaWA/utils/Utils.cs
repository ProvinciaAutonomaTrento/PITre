...
namespace DocsPAWA
{
    ...
    public class Utils
    {
            ...
            try
            {
                for (int i = 0; i <= page.Session.Keys.Count - 1; i++)
                {
                    switch (page.Session.Keys[i])
                    {
                        ...
                        case DocsPAWA.ricercaTrasm.DialogFiltriRicercaTrasmissioni.CURRENT_FILTERS_SESSION_KEY:
                        case DocsPAWA.ricercaTrasm.FiltriRicercaTrasmissioni.CURRENT_UI_FILTERS_SESSION_KEY:
        ...
        public static void CaricaComboRegistri(DropDownList ddl, DocsPAWA.DocsPaWR.Registro[] userReg)
        {
            if (userReg != null)
            {
        ...
        public static ArrayList eseguiRicercaCampiComuni(DocsPAWA.DocsPaWR.InfoUtente infoUtente, DocsPAWA.DocsPaWR.FiltroRicerca[][] listaFiltri, int numPage, int pageSize, out int nRec, Page page)
         {
        ...
        public static bool GetAbilitazioneAtipicita()
        {
            ...
            string valoreChiaveAtipicita = DocsPAWA.utils.InitConfigurationKeys.GetValue(infoUtente.idAmministrazione, "ATIPICITA_DOC_FASC");
            if (string.IsNullOrEmpty(valoreChiaveAtipicita))
                valoreChiaveAtipicita = DocsPAWA.utils.InitConfigurationKeys.GetValue("0", "ATIPICITA_DOC_FASC");
        ...
        internal static bool GetAbilitazioneCopiaVisibilita()
        {
            ...
            string valoreChiaveAtipicita = DocsPAWA.utils.InitConfigurationKeys.GetValue(infoUtente.idAmministrazione, "FE_COPIA_VISIBILITA");
            if (string.IsNullOrEmpty(valoreChiaveAtipicita))
                valoreChiaveAtipicita = DocsPAWA.utils.InitConfigurationKeys.GetValue("0", "FE_COPIA_VISIBILITA");
        ...
        internal static bool GetAbilitazioneGestioneRuoliAvanzata()
        {
            ...
            string valoreChiaveAtipicita = DocsPAWA.utils.InitConfigurationKeys.GetValue(infoUtente.idAmministrazione, "FE_GEST_RUOLI_AVANZATA");
            if (string.IsNullOrEmpty(valoreChiaveAtipicita))
                valoreChiaveAtipicita = DocsPAWA.utils.InitConfigurationKeys.GetValue("0", "FE_GEST_RUOLI_AVANZATA");

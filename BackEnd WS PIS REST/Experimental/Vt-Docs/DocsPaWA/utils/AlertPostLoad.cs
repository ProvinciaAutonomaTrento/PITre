...
namespace DocsPAWA.utils
{
    ...
    public class AlertPostLoad
    {
        ...
        {
            string valoreChiaveDB = DocsPAWA.utils.InitConfigurationKeys.GetValue(UserManager.getInfoUtente().idAmministrazione, "MAX_ROW_SEARCHABLE");
            if (valoreChiaveDB.Length==0)
                valoreChiaveDB = DocsPAWA.utils.InitConfigurationKeys.GetValue("0", "MAX_ROW_SEARCHABLE");

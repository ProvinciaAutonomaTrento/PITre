<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PolicyStampe.aspx.cs" Inherits="DocsPAWA.AdminTool.Gestione_Conservazione.PolicyStampe" %>
...
<head id="Head1" runat="server">
    <title>Conservazione - Policy Stampe</title>
    ...
    <script language="JavaScript" type="text/javascript">
        ...
        function apriPopup() {
            hlp = window.open('../help.aspx?from=CON', '', 'width=450,height=200,scrollbars=YES');
        }
        function cambiaPwd() {
            cambiapass = window.open('../CambiaPwd.aspx', '', 'width=410,height=200,scrollbars=NO');
        }
        function ClosePopUp() {
        ...
        function NewPolicyStampe() {
            var newLeft = (screen.availWidth / 2 - 225);
            var newTop = (screen.availHeight - 704);

            var retval = window.showModalDialog(_url_new_policy, 'NewPolicyStampe', 'dialogWidth:700px;dialogHeight:500px;status:no;resizable:no;scroll:yes;dialogLeft:' + newLeft + ';dialogTop:' + newTop + ';center:no;help:no;', '');
            if (retval != null) {
        ...
        function OpenPolicyDetails(id) {
            var newLeft = (screen.availWidth / 2 - 225);
            var newTop = (screen.availHeight - 704);
            var retval = window.showModalDialog(_url_view_policy + "?id=" + id, 'OpenPolicyDetails', 'dialogWidth:700px;dialogHeight:500px;status:no;resizable:no;scroll:yes;dialogLeft:' + newLeft + ';dialogTop:' + newTop + ';center:no;help:no;', '');
            if (retval != null) {
                window.document.getElementById('<%= Page.Form.ClientID %>').submit();
            }
        }
...
<body onunload="ClosePopUp()" style="margin: 0px; padding: 0px;">
   ...
   <asp:ScriptManager ID="ScriptManager" runat="server" AsyncPostBackTimeout="360000">
    ...
    <uct:AppTitleProvider ID="appTitleProvider" runat="server" PageName="AMMINISTRAZIONE > Conservazione > Policy Stampe" />
                                    ...
                                    <asp:DataGrid ID="grvPolicy" runat="server" AllowSorting="false" AutoGenerateColumns="false"
                                        ...
                                        <Columns>
                                            <asp:TemplateColumn Visible="false">
                                                <ItemTemplate>
                                                    <asp:Label ID="SYSTEM_ID" runat="server" Text='<%# this.GetPolicyID((DocsPAWA.DocsPaWR.Policy)Container.DataItem) %>'></asp:Label>
                                                </ItemTemplate>
                                            </asp:TemplateColumn>
                                            <asp:TemplateColumn Visible="false">
                                                <ItemTemplate>
                                                    <asp:Label ID="AUTOMATICO" runat="server" Text='<%# this.GetAutomaticPeriod((DocsPAWA.DocsPaWR.Policy)Container.DataItem) %>'></asp:Label>
                                                </ItemTemplate>
                                            </asp:TemplateColumn>
                                            <asp:TemplateColumn ItemStyle-Width="10%" HeaderStyle-HorizontalAlign="center" HeaderStyle-CssClass="head_tab"
                                                HeaderText="TIPO">
                                                <ItemTemplate>
                                                    <asp:Label ID="TIPOPOLICY" runat="server" Text='<%# this.GetPolicyType((DocsPAWA.DocsPaWR.Policy)Container.DataItem) %>'></asp:Label>
                                                </ItemTemplate>
                                            </asp:TemplateColumn>
                                            <asp:TemplateColumn ItemStyle-Width="64%" HeaderStyle-HorizontalAlign="center" HeaderStyle-CssClass="head_tab"
                                                HeaderText="NOME">
                                                <ItemTemplate>
                                                    <asp:Label ID="NAMEPOLICY" runat="server" Text='<%# this.GetPolicyName((DocsPAWA.DocsPaWR.Policy)Container.DataItem) %>'></asp:Label>
                                                </ItemTemplate>
                                            </asp:TemplateColumn>
                                            <asp:TemplateColumn ItemStyle-Width="8%" HeaderText="PERIODICITA'" ItemStyle-CssClass="tab_sx2"
                                                ...
                                                <ItemTemplate>
                                                 <asp:ImageButton ID="btn_periodo" runat="server" CssClass="testo_grigio" CommandName="Automatica"
                                                    AlternateText="Vai al dettaglio del periodo" ImageUrl='<%# this.GetImagePeriod((DocsPAWA.DocsPaWR.Policy)Container.DataItem) %>'

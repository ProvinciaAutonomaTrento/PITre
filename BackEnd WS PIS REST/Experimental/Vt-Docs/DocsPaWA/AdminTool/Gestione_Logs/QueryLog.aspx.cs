...
namespace Amministrazione.Gestione_Logs
{
	...
	/// </summary>
	public class QueryLog : System.Web.UI.Page
	{
        ...
        protected DocsPAWA.AdminTool.Getione_Logs.LogCalendar txt_data_da;
        protected DocsPAWA.AdminTool.Getione_Logs.LogCalendar txt_data_a;
        ...
        /// <returns></returns>
        private DocsPAWA.AdminTool.Getione_Logs.LogCalendar GetCalendarControl(string controlId)
        {
            return (DocsPAWA.AdminTool.Getione_Logs.LogCalendar)this.FindControl(controlId);
        }
		...
		{			
            ...
            // I log conservazione devono essere nascosti se è attiva la conservazione PARER
            if (this.DisableAmmGestCons() || this.IsConservazionePARER())
                query_table += 10;
			...
			try
			{
				// chiama WS -----------------------------------------------------------------------------------
				AmmUtils.WebServiceLink ws = new AmmUtils.WebServiceLink();
				string xmlStream = ws.GetXmlLogFiltrato(query_data_da,query_data_a,query_user,query_oggetto,query_azione,this.codAmm,query_esito, "", query_table);

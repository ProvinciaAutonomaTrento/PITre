...
namespace DocsPAWA.AdminTool.Gestione_Organigramma.Gestione_Ruolo_Titolario
{
    public partial class GestRuoloTitolario : System.Web.UI.Page
    {
        ...
        private DataSet ConvertToDataSet(DocsPAWA.DocsPaWR.OrgNodoTitolario[] nodi)
        {
            DataSet ds = this.CreateGridDataSet();
            DataTable dt = ds.Tables["VisibAttuale"];

            foreach (DocsPAWA.DocsPaWR.OrgNodoTitolario nodo in nodi)
            {
        ...
        private bool ExtendToNode(string idNodoTitolario, string idRuolo, bool isInsert, bool showMsgSuccessed, bool AllTitolario)
        {
            ...
            DocsPAWA.DocsPaWR.EsitoOperazione[] esiti = this.ApplyChanges(idNodoTitolario, isInsert, AllTitolario);

            if (esiti != null)
            {
                foreach (DocsPAWA.DocsPaWR.EsitoOperazione es in esiti)
                {
                    if (es.Codice != 0)
                {
                    //    if (showMsgSuccessed)
                    ...
                    // visualizzazione messaggio di errore
                        ...
                        //foreach (DocsPAWA.DocsPaWR.EsitoOperazione esito in esiti)
                        //{

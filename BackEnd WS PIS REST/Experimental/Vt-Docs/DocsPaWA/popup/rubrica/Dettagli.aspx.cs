...
namespace DocsPAWA.popup.RubricaDocsPA
{
    ...
    /// </summary>
    public class Dettagli : DocsPAWA.CssPage
    {
        ...
        protected DocsPAWA.DocsPaWR.DatiModificaCorr datiModifica = new DocsPAWA.DocsPaWR.DatiModificaCorr();
        ...
        protected DocsPAWA.DocsPaWR.Registro r;
        ...
        protected DocsPAWA.DocsPaWR.Corrispondente c;
        ...
        private void apriDettaglio()
        {
            ...
            else
            {
                ...
                if (ie.ToUpper() == "I")
                    tipoIE = DocsPAWA.DocsPaWR.AddressbookTipoUtente.INTERNO;
                else
                {
                    if (ie.ToUpper() == "E")
                    {
                        tipoIE = DocsPAWA.DocsPaWR.AddressbookTipoUtente.ESTERNO;
                        pnl_canalePref.Visible = true;
                    }
                    else
                        tipoIE = DocsPAWA.DocsPaWR.AddressbookTipoUtente.GLOBALE;
                }
    ...
    bool.Parse(ConfigSettings.getKey(ConfigSettings.KeysENUM.ATTIVA_CODICE_DESCRIZIONE_AMMINISTRAZIONE_MITTENTE)) &&
     !string.IsNullOrEmpty(c.codDescAmministrizazione))
                ...
                if (!(c is DocsPAWA.DocsPaWR.Ruolo))
                {
                    ...
                    if (c is DocsPAWA.DocsPaWR.Utente)
                    {
                        ...
                        DocsPaWR.Utente u = (DocsPAWA.DocsPaWR.Utente)c;
                        //dati utente
                ...
                else
                {
                    ...
                    DocsPaWR.Ruolo u = (DocsPAWA.DocsPaWR.Ruolo)c;
        ...
        private void setcampiReadOnly(DocsPAWA.DocsPaWR.Corrispondente corr)
        {
        ...
        private string get_codice_amm(string id_amm)
        {
            ...
            foreach (DocsPAWA.DocsPaWR.Amministrazione amm in amms)
        ...
        private string get_codice_reg(string id_reg)
        {
            ...
            foreach (DocsPAWA.DocsPaWR.Registro reg in regs)
        ...
        private bool verificaSelezione(string testoDelPulsante)
        {
            ...
            if (updPanelMail.Visible)
            {
                ...
                if (gvCaselle.Rows.Count < 1 && (dd_canpref.SelectedIndex == indxMail || dd_canpref.SelectedIndex == indxInterop))
                {
                    ...
                    if (resultCheck)
                    {
                        (ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>).Add(new DocsPAWA.DocsPaWR.MailCorrispondente()
                        {
                ...
                else
                {
                    ...
                    if (resultCheck)
                    {
                        ...
                        foreach (DocsPAWA.DocsPaWR.MailCorrispondente c in (List<DocsPAWA.DocsPaWR.MailCorrispondente>)ViewState["gvCaselle"])
                        {
            ...
            if (pnlStandard.Visible)//caso utenti e Uo
            {
                ...
                if (this.txt_dataNascita.Text != string.Empty && !DocsPAWA.Utils.isDate(this.txt_dataNascita.Text))
                {
                    messaggioErrore = "Attenzione, il formato della data non è valido. \\nIl formato richiesto è gg/mm/aaaa";
                    resultCheck = false;
                }

                if (this.txt_CAP != null && !this.txt_CAP.Text.Equals("") && !DocsPAWA.Utils.isNumeric(this.txt_CAP.Text))
                {
                    messaggioErrore = "Attenzione, il campo CAP deve essere numerico!";
                    resultCheck = false;
                }

                if (this.txt_Prov != null && !this.txt_Prov.Text.Equals("") && !DocsPAWA.Utils.isCorrectProv(this.txt_Prov.Text))
                {
                ...
                {
                    if (this.txt_CodFis != null && !this.txt_CodFis.Text.Equals("") && (DocsPAWA.Utils.CheckTaxCode(this.txt_CodFis.Text.TrimStart(" ".ToCharArray()).TrimEnd(" ".ToCharArray())) != 0))
                    {
                    ...
                    {
                        if ((this.txt_CodFis != null && !this.txt_CodFis.Text.Trim().Equals("")) && ((this.txt_CodFis.Text.Trim().Length == 11 && DocsPAWA.Utils.CheckVatNumber(this.txt_CodFis.Text.Trim()) != 0) || (this.txt_CodFis.Text.Trim().Length == 16 && DocsPAWA.Utils.CheckTaxCode(this.txt_CodFis.Text.Trim()) != 0) || (this.txt_CodFis.Text.Trim().Length != 11 && this.txt_CodFis.Text.Trim().Length != 16)))
                        {
                            messaggioErrore = "Attenzione, Codice Fiscale non corretto!";
                            resultCheck = false;
                        }
                ...
                if (this.txt_PI != null && !this.txt_PI.Text.Equals("") && (DocsPAWA.Utils.CheckVatNumber(this.txt_PI.Text.TrimStart(" ".ToCharArray()).TrimEnd(" ".ToCharArray())) != 0))
                {
                                    ...
                                    else if (Session["Interop"].ToString().Equals("P"))
                                    {
                                        switch (corr_type)
                                        {
                                            ...
                                            case "R":
                                                ...
                                                Corrispondente res = new DocsPAWA.DocsPaWR.Corrispondente();
                                                DocsPaWR.Ruolo ruolo = new DocsPAWA.DocsPaWR.Ruolo();
                                            ...
                                            case "P":
                                                res = new DocsPAWA.DocsPaWR.Corrispondente();
                                                DocsPaWR.Utente utente = new DocsPAWA.DocsPaWR.Utente();
                            ...
                            switch (corr_type)
                            {
                                ...
                                case "R":
                                    ...
                                    if (!avviso.Equals("2"))
                                    {
                                        ...
                                        {
                                            case "OK":
                                                DocsPAWA.utils.MultiCasellaManager.DeleteMailCorrispondenteEsterno(idCorr);
                                                message = "Modifica del corrispondente avvenuta con successo";
                                                break;
                                            default:
                                                message =
        ...
        private void modifyUO(ref DocsPAWA.DocsPaWR.DatiModificaCorr datiModifica)
        {
        ...
        private void modifyRuolo(ref DocsPAWA.DocsPaWR.DatiModificaCorr datiModifica)
        {
            datiModifica.idCorrGlobali = this.hd_systemId.Value;
            datiModifica.descCorr = this.txt_descrizione.Text.TrimStart(" ".ToCharArray()).TrimEnd(" ".ToCharArray());
            datiModifica.codiceAoo = this.txt_CodAOO.Text;
        ...
        private void modifyUtente(ref DocsPAWA.DocsPaWR.DatiModificaCorr datiModifica)
        {
            datiModifica.idCorrGlobali = this.hd_systemId.Value;
            datiModifica.codiceAoo = this.txt_CodAOO.Text;
            datiModifica.codiceAmm = this.txt_CodAmm.Text;
        ...
        private bool checkFields()
        {
            ...
            else if (this.txt_CAP != null && !this.txt_CAP.Text.Equals("") && !DocsPAWA.Utils.isNumeric(this.txt_CAP.Text))
            {
            ...
            else if (this.txt_Prov != null && !this.txt_Prov.Text.Equals("") && !DocsPAWA.Utils.isCorrectProv(this.txt_Prov.Text))
            {
                Response.Write("<script language='javascript'>window.alert(\"Attenzione, il campo provincia non è corretto\");</script>");
                return false;
            }
            else if (this.txt_CodFis != null && !this.txt_CodFis.Text.Equals("") && DocsPAWA.Utils.CheckTaxCode(this.txt_CodFis.Text.TrimStart(" ".ToCharArray()).TrimEnd(" ".ToCharArray())) != 0)
            {
                Response.Write("<script language='javascript'>window.alert(\"Attenzione, Codice Fiscale non corretto!\");</script>");
                return false;
            }
            else if (this.txt_PI != null && !this.txt_PI.Text.Equals("") && DocsPAWA.Utils.CheckVatNumber(this.txt_PI.Text.TrimStart(" ".ToCharArray()).TrimEnd(" ".ToCharArray())) != 0)
            {
        ...
        protected void BindGridViewCaselle(Corrispondente corr)
        {
            if (ViewState["gvCaselle"] == null)
                ViewState.Add("gvCaselle", DocsPAWA.utils.MultiCasellaManager.GetMailCorrispondenteEsterno(corr.systemId));
            gvCaselle.DataSource = (List<DocsPAWA.DocsPaWR.MailCorrispondente>)ViewState["gvCaselle"];
            gvCaselle.DataBind();
            //se è disabilitato il multicasella, dopo l'immissione di una casella disabilito il pulsante aggiungi.
            if (((List<DocsPAWA.DocsPaWR.MailCorrispondente>)ViewState["gvCaselle"]).Count > 0 && !utils.MultiCasellaManager.IsEnabledMultiMail(UserManager.getRuolo().idAmministrazione))
            {
        ...
        protected void imgEliminaCasella_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (isComboMain && (ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>).Count > 1)
            {
            ...
            (ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>).RemoveAt(indexRowDelete);
            gvCaselle.DataSource = (List<DocsPAWA.DocsPaWR.MailCorrispondente>)ViewState["gvCaselle"];
            gvCaselle.DataBind();
            if (((List<DocsPAWA.DocsPaWR.MailCorrispondente>)ViewState["gvCaselle"]).Count < 1 && !utils.MultiCasellaManager.IsEnabledMultiMail(UserManager.getRuolo().idAmministrazione))
            {
                txtCasella.Enabled = true;
                txtNote.Enabled = true;
                imgAggiungiCasella.Enabled = true;
        ...
        protected void imgAggiungiCasella_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if ((ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>).Count > 0)
            {
                txtCasella.Enabled = false;
                txtNote.Enabled = false;
                imgAggiungiCasella.Enabled = false;
        ...
        protected void rdbPrincipale_ChecekdChanged(object sender, EventArgs e)
        {
            ...
            List<DocsPAWA.DocsPaWR.MailCorrispondente> listCaselle = ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>;
            foreach (DocsPAWA.DocsPaWR.MailCorrispondente c in listCaselle)
            {
            ...
            ViewState["gvCaselle"] = listCaselle as List<DocsPAWA.DocsPaWR.MailCorrispondente>;
        ...
        protected void txtEmailCorr_TextChanged(object sender, EventArgs e)
        {
            ...
            (ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>)[rowModify].Email = newMail;
        }
        ...
        protected void txtNoteMailCorr_TextChanged(object sender, EventArgs e)
        {
            ...
            int rowModify = ((sender as System.Web.UI.WebControls.TextBox).Parent.Parent as System.Web.UI.WebControls.GridViewRow).RowIndex;
            (ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>)[rowModify].Note = newNote;
        }
        ...
        protected bool InsertComboMailsCorr(string idCorrispondente)
        {
            ...
            if (!string.IsNullOrEmpty(idCorrispondente))
            {
                ...
                if ((ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>).Count > 0)
                {
                    res = DocsPAWA.utils.MultiCasellaManager.InsertMailCorrispondenteEsterno(
                        (ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>), idCorrispondente);
                    if (!res)
                    {
                ...
                if ((ViewState["gvCaselle"] as List<DocsPAWA.DocsPaWR.MailCorrispondente>).Count == 0)
                {
                    res = DocsPAWA.utils.MultiCasellaManager.DeleteMailCorrispondenteEsterno(idCorrispondente);
                    if (!res)
                    {
                        message = "Attenzione: si è verificato un errore durante l'aggiornamento delle caselle mail associate al corrispondente";
                    }

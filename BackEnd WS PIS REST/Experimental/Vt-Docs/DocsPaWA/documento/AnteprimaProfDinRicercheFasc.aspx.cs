...
{
    public class AnteprimaProfDinRicercheFasc : System.Web.UI.Page
    {
        ...
        private void Page_Load(object sender, System.EventArgs e)
        {
            ...
            fRic = FascicoliManager.getFiltroRicFasc(this);
            if (!IsPostBack)
            {
        ...
        public void ricercaFiltro()
        {
            if (fRic != null)
            {
                ...
                for (int i = 0; i < fRic[0].Length; i++)
                {
                    aux = fRic[0][i];
                    if (aux != null && aux.argomento == DocsPAWA.DocsPaWR.FiltriFascicolazione.PROFILAZIONE_DINAMICA.ToString())
        ...
        public void inserisciMenuATendina(DocsPAWA.DocsPaWR.OggettoCustom oggettoCustom)
        {
            ...
            int valoreDiDefault = -1;
            // quando salviamo la ricerca fascicoli, viene salvato anche l'elenco dei valori possibili nel menu a tendina 
            ...
            DocsPAWA.DocsPaWR.OggettoCustom[] oggCus = ws.GetValuesDropDownList(oggettoCustom.SYSTEM_ID.ToString(), "F");
        ...
        public void inserisciData(DocsPAWA.DocsPaWR.OggettoCustom oggettoCustom)
        {
            ...
            dataDa.VisibleTimeMode = ProfilazioneFascManager.getVisibleTimeMode(oggettoCustom);
            ...
            dataA.VisibleTimeMode = ProfilazioneFascManager.getVisibleTimeMode(oggettoCustom);
        ...
        public void inserisciCorrispondente(DocsPAWA.DocsPaWR.OggettoCustom oggettoCustom)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //    {
                    //        DocsPAWA.DocsPaWR.Corrispondente corr_4 = UserManager.getCorrispondenteSelezionato(this.Page);
                    //        if (corr_4 != null)
                    //        {
                    //            //Verifico che l'id del campo sia quello che mi interessa.
                    //            //Questo id viene messo in sessione dallo UserControl e serve a 
        ...
        private void btn_ConfermaProfilazione_Click(object sender, System.EventArgs e)
        {
            ...
            //Devo verificare se il modello selezionato è un iperfascicolo.
            //In caso affermativo va controllato che sia compilato un unico campo di ricerca.
            //if (template.IPERFASCICOLO == "1")
            //{
            //    if (verificaValorizzazioneRicercaCampiComuni())
            //    {
            //        Label_Avviso.Text = "Per le ricerche su campi comuni, va impostato un solo criterio di ricerca !";
            //        Label_Avviso.Visible = true;
            //        return;
            //    }
            //}
            ...
            Session.Add("filtroProfDinamica", filtro);
        ...
        private bool controllaCampi(DocsPAWA.DocsPaWR.OggettoCustom oggettoCustom, string idOggetto)
        {
            ...
            switch (oggettoCustom.TIPO.DESCRIZIONE_TIPO)
            {
                ...
                case "SelezioneEsclusiva":
                    ...
                    if (oggettoCustom.VALORE_DATABASE == "-1" || radioButtonList.SelectedIndex == -1)
                    {
                ...
                case "Data":
                    ...
                    //if (dataDa.txt_Data.Text.Equals("") && dataA.txt_Data.Text.Equals("") && oggettoCustom.CAMPO_OBBLIGATORIO.Equals("SI"))
                    if (dataDa.Text.Equals("") && dataA.Text.Equals("") && oggettoCustom.CAMPO_OBBLIGATORIO.Equals("SI"))
                    {
                    ...
                    /*
                    TextBox giorno	= (TextBox) panel_Contenuto.FindControl("giorno"+idOggetto);
                    TextBox mese	= (TextBox) panel_Contenuto.FindControl("mese"+idOggetto);
                    TextBox anno	= (TextBox) panel_Contenuto.FindControl("anno"+idOggetto);
                    if( giorno.Text.Equals("") || mese.Text.Equals("") || anno.Text.Equals("") )
                    {
                        SetFocus(giorno);
                        oggettoCustom.VALORE_DATABASE = "";
                        return true;
                    }
                    oggettoCustom.VALORE_DATABASE = giorno.Text +"-"+ mese.Text +"-"+ anno.Text;
                    */
                    break;
                case "Contatore":
        ...
        private bool verificaValorizzazioneRicercaCampiComuni()
        {
            int numeroCriteriDiRicerca = 0;
            for (int i = 0; i < template.ELENCO_OGGETTI.Length; i++)
            {
                DocsPAWA.DocsPaWR.OggettoCustom ogg = (DocsPAWA.DocsPaWR.OggettoCustom)template.ELENCO_OGGETTI[i];
                if (ogg.VALORE_DATABASE != null && ogg.VALORE_DATABASE != "")
                    numeroCriteriDiRicerca++;
            }

            if (numeroCriteriDiRicerca > 1)
                return true;
            else
                return false;
        ...
        {
            if (objCustom.VALORI_SELEZIONATI != null && objCustom.VALORI_SELEZIONATI.Length > 0)
            {
                ...
                switch (tipoOggetto)
                {
                    case "CasellaDiSelezione":
                        for (int i = 0; i < oggettoCustom.VALORI_SELEZIONATI.Length; i++)
                        {
                            ...
                            {
                                select += " (SELECT Id_Project, Valore_Oggetto_Db,ID_OGGETTO,ID_AOO_RF FROM DPA_ASS_TEMPLATES_FASC) DPA" + indiceDpa + ",";
        ...
        public string creaCorpoCentrale(ArrayList indiciDpa)
        {
            ...
            corpoCentrale += " AND DPA_OGGETTI_CUSTOM_FASC.Campo_Di_Ricerca = 'SI'";

            if (indiciDpa.Count - 1 == 0)
            {
                //corpoCentrale += " AND DPA0.ID_OGGETTO = DPA_OGGETTI_CUSTOM_FASC.SYSTEM_ID ";
                corpoCentrale += " AND A.SYSTEM_ID = DPA0.Id_Project ";
            ...
            for (int i = 0; i < indiciDpa.Count - 1; i++)
            {
                if (i == 0)
                {
                    //corpoCentrale += " AND DPA"+indiciDpa[i]+".ID_OGGETTO = DPA_OGGETTI_CUSTOM_FASC.SYSTEM_ID ";
                    corpoCentrale += " AND A.SYSTEM_ID = DPA" + indiciDpa[i] + ".Id_Project ";
                    corpoCentrale += " AND DPA" + indiciDpa[i] + ".Id_Project=DPA" + indiciDpa[i + 1] + ".Id_Project";
                }
                else
                {
                    corpoCentrale += " AND DPA" + indiciDpa[i] + ".Id_Project=DPA" + indiciDpa[i + 1] + ".Id_Project";

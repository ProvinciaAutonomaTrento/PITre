begin 
	utl_backup_sp ('INS_OCC','3.24');
end;
/

create or replace procedure INS_OCC
(p_ID_REG in integer,
p_IDAMM in integer,
  ...
  ELSE
    BEGIN
      ...
      new_var_cod_rubrica1 := new_var_cod_rubrica1 || '_' || TO_CHAR (p_ID_Corr_Globali);
        ...
        DPA_CORR_GLOBALI (system_id,id_registro,ID_AMM,VAR_COD_RUBRICA,VAR_DESC_CORR,ID_OLD,DTA_INIZIO,ID_PARENT,
        VAR_CODICE,CHA_TIPO_CORR,CHA_DETTAGLI)
        VALUES (sysid,id_reg,idamm,p_Prefix_cod_rub||to_char(sysid),p_desc_corr,p_ID_Corr_Globali,sysdate,0,p_Prefix_cod_rub||to_char(sysid),'O',0)
        returning system_id into p_RESULT;
     END;
    END IF;

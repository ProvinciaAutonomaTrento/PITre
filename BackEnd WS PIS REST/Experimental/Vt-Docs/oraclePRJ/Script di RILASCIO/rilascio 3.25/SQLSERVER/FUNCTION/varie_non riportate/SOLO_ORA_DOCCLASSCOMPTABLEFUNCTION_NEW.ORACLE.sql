---- DOCCLASSCOMPTABLEFUNCTION_NEW.ORA.sql  marcatore per ricerca ----
begin 
Utl_Backup_Plsql_code ('FUNCTION','DOCCLASSCOMPTABLEFUNCTION_NEW'); 
end;
/
...
begin
	...
	where Object_Type= 'TYPE' and Object_Name= 'DOCCLASSCOMPTABLETYPE';

	if cnt = 0 then 
	execute immediate 
		'create or replace TYPE DOCCLASSCOMPTABLETYPE                                                                          
			IS OBJECT (TOT_DOC_CLASS number
...
begin
	select count(*) into cnt from user_objects 
	where Object_Type= 'TYPE' and Object_Name= 'DOCCLASSCOMPTABLETYPE_P';

	if cnt = 0 then 
	execute immediate 
		'create or replace TYPE docclasscomptabletype_p 
			AS OBJECT (
				id_amm              INTEGER,
				id_registr          INTEGER,
				id_anno                INTEGER,
				...
				cod_class           VARCHAR2 (256),
				desc_class          VARCHAR2 (3000),
				tot_doc_class_vt    FLOAT,
				perc_doc_class_vt   FLOAT,
				num_livello         VARCHAR2 (256)
							) '		; 
	end if;
end;
end;
...
begin
	select count(*) into cnt from user_objects 
	where Object_Type= 'TYPE' and Object_Name= 'DOCCLASSCOMPTABLEROW';

	if cnt = 0 then 
	execute immediate 
		'create or replace 
		TYPE DOCCLASSCOMPTABLEROW IS TABLE OF DocClassCompTableType'; 
	end if;
end;
end;
/
...
begin
	select count(*) into cnt from user_objects 
	where Object_Type= 'TYPE' and Object_Name= 'DOCCLASSCOMPTABLEROW_P';

	if cnt = 0 then 
	execute immediate 
		'create or replace TYPE docclasscomptablerow_p 
			IS TABLE OF docclasscomptabletype_p '; 
	end if;
end;
end;
/
...
create or replace
FUNCTION DOCCLASSCOMPTABLEFUNCTION_NEW (
   id_amm       NUMBER,
   id_registr   NUMBER,
   id_anno      NUMBER,
   sede         VARCHAR,
   ...
   RETURN DOCCLASSCOMPTABLEROW_P PIPELINED
/*
...
condizione_titolario varchar2(200);
  ...
  outrec                  DOCCLASSCOMPTABLETYPE_p
                := Docclasscomptabletype_P (
   ...
   Is
      Select   Pr.System_Id As Sysid, Pr.Description As Descrizione,
               tit.Description||' - '||Pr.Var_Codice As Var_Codice, Pr.Num_Livello
          From Project Pr, (Select System_Id, Var_Codice, Description From Project Where Id_Titolario=0) Tit
         WHERE pr.var_codice IS NOT NULL and tit.system_id=pr.id_titolario
...
EXCEPTION
   ...
   THEN
      ...
      --RAISE;
      RETURN;
END docclasscomptablefunction_new;
/

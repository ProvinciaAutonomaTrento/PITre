...
begin
	...
	where Object_Type= 'TYPE' and Object_Name= 'INTEROPTABLETYPE';

	if cnt = 0 then 
	execute immediate 
		'create or replace TYPE INTEROPTABLETYPE 
		IS OBJECT (var_cod_amm varchar (100),
				var_cod_aoo varchar(100)
				, gennaio varchar(20)
				, febbraio varchar(20)
				, marzo varchar(20)
				, aprile varchar(20)
				...
				, luglio varchar(20)
				, agosto varchar(20)
				, settembre varchar(20)
				, ottobre varchar(20)
				, novembre varchar(20)
				, dicembre varchar(20)
				, tot_m_sped varchar(20)
				, tot_sped varchar(20)
				) '		; 
	end if;
end;
...
begin
	select count(*) into cnt from user_objects 
	where Object_Type= 'TYPE' and Object_Name= 'INTEROPTABLEROW';

	if cnt = 0 then 
	execute immediate 
		'create or replace TYPE INTEROPTABLEROW 
				IS TABLE OF InteropTableType '; 
	end if;
end;
end;
/
...
Distinct  Count (*) As Conteggio, To_Number (To_Char (Dta_Spedizione, 'MM')) --As Mese_Spedizione --,D.Description
    , A.Var_Desc_Amm As Var_Codice_Amm, Var_Codice_Aoo
          ...
          WHERE cha_tipo_proto = 'P'            AND tc.id_documenttype = d.system_id

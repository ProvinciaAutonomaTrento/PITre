--------------------------------------------------------
--  DDL for Table DPA_CORR_GLOBALI
--------------------------------------------------------

  CREATE TABLE "ITCOLL_6GIU12"."DPA_CORR_GLOBALI" 
   (	"SYSTEM_ID" NUMBER(10,0), 
	"ID_REGISTRO" NUMBER(10,0), 
	"ID_AMM" NUMBER(10,0), 
	"VAR_COD_RUBRICA" VARCHAR2(128 BYTE), 
	"VAR_DESC_CORR" VARCHAR2(256 BYTE), 
	"ID_OLD" NUMBER(10,0), 
	"DTA_INIZIO" DATE, 
	"DTA_FINE" DATE, 
	"ID_PARENT" NUMBER(10,0), 
	"NUM_LIVELLO" NUMBER(10,0), 
	"VAR_CODICE" VARCHAR2(128 BYTE), 
	"ID_GRUPPO" NUMBER(10,0), 
	"ID_TIPO_RUOLO" NUMBER(10,0), 
	"CHA_DEFAULT_TRASM" VARCHAR2(1 BYTE), 
	"ID_UO" NUMBER(10,0), 
	"VAR_COGNOME" VARCHAR2(50 BYTE), 
	"VAR_NOME" VARCHAR2(50 BYTE), 
	"ID_PEOPLE" NUMBER(10,0), 
	"CHA_TIPO_CORR" VARCHAR2(1 BYTE), 
	"CHA_TIPO_IE" VARCHAR2(1 BYTE), 
	"CHA_TIPO_URP" VARCHAR2(1 BYTE), 
	"CHA_PA" VARCHAR2(1 BYTE), 
	"VAR_CODICE_AOO" VARCHAR2(16 BYTE), 
	"VAR_CODICE_AMM" VARCHAR2(32 BYTE), 
	"VAR_CODICE_ISTAT" VARCHAR2(32 BYTE), 
	"ID_PESO" NUMBER(10,0), 
	"VAR_EMAIL" VARCHAR2(128 BYTE), 
	"CHA_DETTAGLI" VARCHAR2(1 BYTE), 
	"NUM_FIGLI" NUMBER(10,0), 
	"VAR_SMTP" VARCHAR2(128 BYTE), 
	"NUM_PORTA_SMTP" NUMBER(10,0), 
	"VAR_FAX_USER_LOGIN" VARCHAR2(8 BYTE), 
	"CHA_RIFERIMENTO" VARCHAR2(1 BYTE), 
	"ID_PEOPLE_LISTE" NUMBER(*,0), 
	"ID_GRUPPO_LISTE" NUMBER(10,0), 
	"CHA_RESPONSABILE" VARCHAR2(1 BYTE), 
	"ID_PESO_ORG" NUMBER(10,0), 
	"AOO_SOSPESO" VARCHAR2(1 BYTE), 
	"CHA_SEGRETARIO" VARCHAR2(1 BYTE), 
	"ID_RF" NUMBER, 
	"COD_DESC_INTEROP" VARCHAR2(516 BYTE), 
	"CHA_DISABLED_TRASM" VARCHAR2(1 BYTE), 
	"VAR_CHIAVE_AE" VARCHAR2(12 BYTE) DEFAULT '0', 
	"VAR_INSERT_BY_INTEROP" VARCHAR2(1 BYTE) DEFAULT NULL, 
	"VAR_ORIGINAL_CODE" VARCHAR2(128 BYTE), 
	"ORIGINAL_ID" NUMBER(10,0), 
	"VAR_DESC_CORR_OLD" VARCHAR2(256 BYTE), 
	"INTEROPURL" VARCHAR2(4000 BYTE), 
	"INTEROPREGISTRYID" NUMBER, 
	"CLASSIFICA_UO" VARCHAR2(50 BYTE), 
	"INTEROPRFID" NUMBER
   ) ;

...
set define off;
  ...
  CREATE OR REPLACE PROCEDURE "ITCOLL_6GIU12"."SPSETDATAVISTA" (
...
AND dpa_trasm_utente.id_people != p_idpeople
and exists (select 'x' from dpa_trasm_utente a  where a.id_trasm_singola=dpa_trasm_utente.ID_TRASM_SINGOLA and
a.id_people = p_idpeople ); --ovvero solo se io sono tra i notificati!!!
EXCEPTION
WHEN OTHERS
THEN
p_resultvalue := 1;

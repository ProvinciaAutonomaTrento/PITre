...
where profile.author = people.system_id and people.id_amm = id_ammi and num_proto is null AND cha_tipo_proto = 'G'
AND TO_NUMBER(TO_CHAR(CREATION_DATE,'YYYY')) = anno and (profile.var_sede = v_VAR_SEDE);
...
where profile.author = people.system_id and people.id_amm = id_ammi
and profile.cha_img = '0' and TO_NUMBER(TO_CHAR(profile.CREATION_DATE,'YYYY')) = anno and profile.cha_tipo_proto = 'G' AND (profile.var_sede = v_VAR_SEDE);

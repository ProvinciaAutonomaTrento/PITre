...
namespace BusinessLogic.Mobile
{
    public class RubricaManager
    {
        ...
        public static List<RicercaSmistamentoElement> GetListaCorrispondentiVeloce(string descrizione, InfoUtente infoUtente, string idRegistro, string idRuolo, string ragione, int numMaxResults)
        {
            ...
            qco.descrizioneIndicizzata = descrizione;

            //lista dei preferiti
            ArrayList listaIdPref= new ArrayList();
            List<InfoPreferito> listaPref = BusinessLogic.Mobile.RubricaManager.PrefMobile_getList(infoUtente.idPeople, false, null);
            if (listaPref != null && listaPref.Count > 0)
            {
                foreach (InfoPreferito infPref in listaPref)
                {
                    listaIdPref.Add(infPref.IdCorrespondent);
                }
            }
            ...
            if (string.IsNullOrEmpty(ragione) || ragione != "comp") ragione = "con";
            switch (ragione)
            {
                case "comp":
            ...
            if (res != null && res.Count() > 0)
            {
                foreach (string s in res)
                {
                    ...
                    //if(listaIdPref!=null && listaIdPref.Count>0 && listaIdPref.Contains(sysId)) 
                    // UTENTE
                    if (tipo.Equals("P"))
                    {
                    ...
                    if (i < numMaxResults)
                    {
                        if (el != null)
                        {
                            el.CodiceCorrispondente = s.Split('§')[4];
                            el.idCorrispondente = sysId;
                            if (listaIdPref != null && listaIdPref.Count > 0 && listaIdPref.Contains(sysId)) el.Preferito = true;
                            else el.Preferito = false;
                            elements.Add(el);
                            i++;
                        }
                    }
        ...
        private static string[] GetElementiRubricaVeloce(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.rubrica.ParametriRicercaRubrica qco)
        {
            ...
            for (int i = 0; i < ers.Count; i++)
            {
                ...
                tempStringElemento = tempElement.descrizione + "§" + tempElement.systemId + "§" + tempElement.tipo + "§" + tempElement.idPeople + "§" + tempElement.codice;
        ...
        public static bool PrefMobile_Insert(DocsPaVO.Mobile.InfoPreferito infoPref)
        {
            DocsPaDB.Query_DocsPAWS.Mobile.RubricaMobile mdb = new RubricaMobile();
            return mdb.PrefMobile_Insert(infoPref);

        }

        public static bool PrefMobile_Delete(DocsPaVO.Mobile.InfoPreferito infoPref)
        {
            DocsPaDB.Query_DocsPAWS.Mobile.RubricaMobile mdb = new RubricaMobile();
            return mdb.PrefMobile_Delete(infoPref);
        }

        public static List<InfoPreferito> PrefMobile_getList(string idpeopleowner, bool soloP, string tipoPref)
        {
            DocsPaDB.Query_DocsPAWS.Mobile.RubricaMobile mdb = new RubricaMobile();
            return mdb.PrefMobile_getList(idpeopleowner, soloP, tipoPref);
        }

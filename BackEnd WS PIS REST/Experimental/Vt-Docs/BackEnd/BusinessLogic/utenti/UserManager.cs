    ...
    public class UserManager
    {
        ...
        public static UserLogin.ResetPasswordResult ResetPasswordInviaOTP(string userID, out string email)
        {
            ...
            try
            {
                ...
                string body = "Gentile utente,<br /><br />hai ricevuto questa mail a seguito della richiesta di reset della password.<br />Inserire all'interno della maschera di Login nel campo di testo 'OTP' la seguente password:<br /><br />" + otp;
                res = BusinessLogic.Interoperabilità.Notifica.notificaByMail(utente.email, fromEmailAmministra, oggetto, body, string.Empty, utente.idAmministrazione, null);
                if (!res)
                {

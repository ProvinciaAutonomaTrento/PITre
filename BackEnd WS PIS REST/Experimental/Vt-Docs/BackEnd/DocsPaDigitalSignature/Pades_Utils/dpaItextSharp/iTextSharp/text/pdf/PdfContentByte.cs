...
using dpaItextSharp.text.exceptions;
using dpaItextSharp.text.pdf;
using dpaItextSharp.text.pdf.intern;
        ...
        */
        ...
        public void Rectangle(Rectangle rectangle) {
            ...
            if (rectangle.UseVariableBorders) {
                ...
                if (rectangle.BorderWidth != dpaItextSharp.text.Rectangle.UNDEFINED) {
                ...
                if (rectangle.HasBorder(dpaItextSharp.text.Rectangle.BOX)) {
                    this.Rectangle(x1, y1, x2 - x1, y2 - y1);
                }
                    // if the border isn't a rectangle, the different sides are added apart
                else {
                    if (rectangle.HasBorder(dpaItextSharp.text.Rectangle.RIGHT_BORDER)) {
                        MoveTo(x2, y1);
                        LineTo(x2, y2);
                    }
                    if (rectangle.HasBorder(dpaItextSharp.text.Rectangle.LEFT_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x1, y2);
                    }
                    if (rectangle.HasBorder(dpaItextSharp.text.Rectangle.BOTTOM_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x2, y1);
                    }
                    if (rectangle.HasBorder(dpaItextSharp.text.Rectangle.TOP_BORDER)) {
                        MoveTo(x1, y2);
                        LineTo(x2, y2);
                    }
                }

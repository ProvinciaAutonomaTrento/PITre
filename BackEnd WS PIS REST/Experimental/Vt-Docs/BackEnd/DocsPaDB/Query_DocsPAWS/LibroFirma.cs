    ...
    public class LibroFirma : DBProvider
    {
        ...
        public List<IstanzaProcessoDiFirma> GetIstanzaProcessiDiFirmaByUtenteTitolare(string idUtenteCoinvolto, string idRuoloCoinvolto)
        {
            ...
            try
            {
                ...
                q.setParam("idRuoloCoinvolto", string.IsNullOrEmpty(idRuoloCoinvolto) ? string.Empty : " AND " + idRuoloCoinvolto);
                query = q.getSQL();
                ...
                if (this.ExecuteQuery(out ds, "passiProcessoDiFirma", query))
                {
                    if (ds.Tables["passiProcessoDiFirma"] != null && ds.Tables["passiProcessoDiFirma"].Rows.Count > 0)
                    {
                        foreach (DataRow row in ds.Tables["passiProcessoDiFirma"].Rows)
                        {
                            ...
                            //E' un modello se non è definito il ruolo e il tipo ruolo
                            passo.IsModello = (passo.ruoloCoinvolto == null || string.IsNullOrEmpty(passo.ruoloCoinvolto.idGruppo)
                                && (passo.TpoRuoloCoinvolto == null || string.IsNullOrEmpty(passo.TpoRuoloCoinvolto.systemId)) && !passo.Evento.TipoEvento.Equals("W"));
        ...
        private string BindConditionFilters(List<DocsPaVO.LibroFirma.FiltroIstanzeProcessoFirma> filtri)
        {
            ...
            if(!string.IsNullOrWhiteSpace(idProcesso))
            ...
            if (!string.IsNullOrWhiteSpace(condition) && condition.Trim().ToUpper().StartsWith("AND"))
        ...
        public IstanzaProcessoDiFirma CreateIstanzaFromProcesso(ProcessoFirma processoDiFirma, DocsPaVO.documento.FileRequest file, DocsPaVO.utente.InfoUtente infoUtente, string note, bool notificaInterruzione, bool notificaConclusione, bool daCambioStato)
        {

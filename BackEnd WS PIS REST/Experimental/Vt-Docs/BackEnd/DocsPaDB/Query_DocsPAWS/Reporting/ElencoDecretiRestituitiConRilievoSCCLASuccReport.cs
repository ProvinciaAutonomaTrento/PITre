...
namespace DocsPaDB.Query_DocsPAWS.Reporting
{
    ...
    public class ElencoDecretiRestituitiConRilievoSCCLASuccReport
    {

        [ReportDataExtractorMethod(ContextName = "ElencoDecretiRestituitiConRilievoSCCLASucc")]
        public DataSet GetDescretiInEsameData(InfoUtente userInfo, List<FiltroRicerca> searchFilters)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                Query query = InitQuery.getInstance().getQuery("CDC_REPORT_ELENCO_DECRETI_RESTITUITI_CON_RILIEVO_SCCLA_SUCC");
                    ...
                    case "CDCNumElenco":   // Codice del revisore
                        if (!String.IsNullOrEmpty(filter.valore))
                            ...
                            filterCondition.AppendFormat(" and " +
                                                            "dpa_tipo_atto.id_amm = (select id_amm from dpa_tipo_atto where UPPER(dpa_tipo_atto.var_desc_atto) = UPPER('CONTROLLO SUCCESSIVO SCCLA')) " +
                                                            "and " +

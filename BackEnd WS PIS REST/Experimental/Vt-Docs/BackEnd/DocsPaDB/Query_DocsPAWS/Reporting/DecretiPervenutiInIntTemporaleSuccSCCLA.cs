...
namespace DocsPaDB.Query_DocsPAWS.Reporting
{
    ...
    /// decreti pervenuti in un dato intervallo temporale per i report successivi SCCLA (reportistica sviluppata per Corte Dei Conti)
    /// : Iacozzilli Giordano
    /// </summary>
    [ReportDataExtractorClass()]
    public class DecretiPervenutiInIntTemporaleSuccSCCLA
    {
        /// <summary>
        ...
        [ReportDataExtractorMethod(ContextName = "DecretiPervenutiInIntTemporaleSuccSCCLA")]
        public DataSet GetDescretiInEsamePervenutiInRangeDateData(InfoUtente userInfo, List<FiltroRicerca> searchFilters)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                Query query = InitQuery.getInstance().getQuery("CDC_REPORT_DECRETI_PERVENUTI_IN_INT_TEMPORALE_SUCC_SCCLA");

...
namespace DocsPaDB.Query_DocsPAWS.Reporting
{
    ...
    public class ElencoDecretiRestituitiSRCReport
    {
        [ReportDataExtractorMethod(ContextName = "ElencoDecretiRestituitiSRC")]
        public DataSet GetDescretiInEsameData(InfoUtente userInfo, List<FiltroRicerca> searchFilters)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                Query query = InitQuery.getInstance().getQuery("CDC_REPORT_ELENCO_DECRETI_RESTITUITI_SRC");
        ...
        public DataSet GetElencoDescreti(InfoUtente userInfo, List<FiltroRicerca> searchFilters, out int numRows)
        {
            ...
            {
                // Reperimento query da eseguire ed esecuzione
                Query query = InitQuery.getInstance().getQuery("CDC_REPORT_ELENCO_DECRETI_SRC_2");
                query.setParam("dbuser", DocsPaDbManagement.Functions.Functions.GetDbUserSession());
                ...
                switch (filter.argomento)
                {
                    case "CDCNumElenco":   // Codice del revisore
                        if (!String.IsNullOrEmpty(filter.valore))
                            ...
                            //filterCondition.AppendFormat(" AND (getvaloreoggettodb(dpa_tipo_atto.id_amm,p.docnumber,dpa_tipo_atto.var_desc_atto,'Num. restituz. Amministrazione') = '{0}')", filter.valore);
                            ...
                            filterCondition.AppendFormat(   " and " +
                                                            ...
                                                            "and " +
                                                            "UPPER(dpa_oggetti_custom.descrizione) = UPPER('Num. restituz. Amministrazione') " +
                                                            "and " +

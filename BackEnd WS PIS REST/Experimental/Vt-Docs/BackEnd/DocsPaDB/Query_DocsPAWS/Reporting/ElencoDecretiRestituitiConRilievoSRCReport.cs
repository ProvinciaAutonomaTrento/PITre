...
{
    /// <summary>
    /// Questa classe viene utilizzata per l'estrazione dei dati relativi al report per l'estrazione dell'Elenco decreti restituiti
    /// (reportistica sviluppata per Corte Dei Conti)
    /// </summary>
    [ReportDataExtractorClass()]
    public class ElencoDecretiRestituitiConRilievoSRCReport
    {
        [ReportDataExtractorMethod(ContextName = "ElencoDecretiRestituitiConRilievoSRC")]
        public DataSet GetDescretiInEsameData(InfoUtente userInfo, List<FiltroRicerca> searchFilters)
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                Query query = InitQuery.getInstance().getQuery("CDC_REPORT_ELENCO_DECRETI_RESTITUITI_CON_RILIEVO_SRC");
        ...
        public DataSet GetElencoDescreti(InfoUtente userInfo, List<FiltroRicerca> searchFilters, out int numRows)
        {
            DataSet dataSet = new DataSet();
            numRows = 0;
            using (DBProvider dbProvider = new DBProvider())
            {
                // Reperimento query da eseguire ed esecuzione
                Query query = InitQuery.getInstance().getQuery("CDC_REPORT_ELENCO_DECRETI_RESTITUITI_CON_RILIEVO_SRC_2");
                query.setParam("dbuser", DocsPaDbManagement.Functions.Functions.GetDbUserSession());
                ...
                switch (filter.argomento)
                {
                    case "CDCNumElenco":   // Codice del revisore
                        if (!String.IsNullOrEmpty(filter.valore))
                            ...
                            //filterCondition.AppendFormat(" AND (getvaloreoggettodb(dpa_tipo_atto.id_amm,p.docnumber,dpa_tipo_atto.var_desc_atto,'numero elenco rilievi') = '{0}')", filter.valore);
                            ...
                            filterCondition.AppendFormat(   " and " +
                                                            ...
                                                            "and " +
                                                            "UPPER(dpa_oggetti_custom.descrizione) = UPPER('numero elenco rilievi') " +
                                                            "and " +

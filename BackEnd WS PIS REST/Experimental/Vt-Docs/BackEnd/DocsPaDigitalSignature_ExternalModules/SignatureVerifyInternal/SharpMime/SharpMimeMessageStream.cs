...
namespace anmar.SharpMimeTools
{
	...
	internal class SharpMimeMessageStream {
        ...
        public System.Byte[] ReadAllBytes(long start)
        {
            return this.ReadBytes(start, this.stream.Length);
        }
        ...
        public System.Byte[] ReadBytes(long start, long end)
        {
            ...
            do
            {
                ...
                byt = this.sr.BaseStream.ReadByte();
                finalpos = this.sr.BaseStream.Position;
                if (byt != -1)
                {
                    retval.Add((byte)byt);
                }
            } while (byt != -1 && this.Position != -1 && this.Position < end);

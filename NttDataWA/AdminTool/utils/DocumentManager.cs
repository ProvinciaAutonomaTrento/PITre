...
namespace SAAdminTool
{
    ...
    public class DocumentManager : Page
    {
        ...
        public static SchedaDocumento protocolla(Page page,
                                                SchedaDocumento schedaDoc, out SAAdminTool.DocsPaWR.ResultProtocollazione risultatoProtocollazione)
        {
            logger.Info("BEGIN");
            SchedaDocumento result = null;
        ...
        public static SchedaDocumento creaProtocollo(Page page, SchedaDocumento schedaDoc, DocsPaWR.Fascicolo fasc, out SAAdminTool.DocsPaWR.ResultProtocollazione risultatoProtocollazione)
        {
            string message = null;
            ...
            risultatoProtocollazione = SAAdminTool.DocsPaWR.ResultProtocollazione.OK;
            try
            {
                string valoreChiaveConsentiClass = string.Empty;
                valoreChiaveConsentiClass = SAAdminTool.utils.InitConfigurationKeys.GetValue("0", "FE_BLOCCA_CLASS");
                ...
                {
                    risultatoProtocollazione = SAAdminTool.DocsPaWR.ResultProtocollazione.ERRORE_DURANTE_LA_FASCICOLAZIONE;
                ...
                if (result == null || risultatoProtocollazione != SAAdminTool.DocsPaWR.ResultProtocollazione.OK)
                {
                    ...
                    switch (risultatoProtocollazione)
                    {
                        ...
                        case SAAdminTool.DocsPaWR.ResultProtocollazione.APPLICATION_ERROR:
                            ...
                            break;
                        case SAAdminTool.DocsPaWR.ResultProtocollazione.DOCUMENTO_GIA_PROTOCOLLATO: //TODO: Se predisposto già protocollato
                            //message = "Errore imprevisto dell'applicazione. Ritentare l'operazione.";
                        ...
                        case SAAdminTool.DocsPaWR.ResultProtocollazione.FORMATO_SEGNATURA_MANCANTE:
                            ...
                            break;
                        case SAAdminTool.DocsPaWR.ResultProtocollazione.ERRORE_WSPIA_PROTOCOLLO_ENTRATA_MITTENTE:
                            message = " WSPIA - Non è possibile utilizzare il mittente selezionato.";
                            break;

                        case SAAdminTool.DocsPaWR.ResultProtocollazione.ERRORE_WSPIA_CLASSIFICA_NODO_FOGLIA:
                            message = " WSPIA - Selezionare la voce di titolario di dettaglio";
                            break;
                        case SAAdminTool.DocsPaWR.ResultProtocollazione.ERRORE_WSPIA_PROTOCOLLAZIONE_SEMPLICE:
                ...
                if (result != null && risultatoProtocollazione == SAAdminTool.DocsPaWR.ResultProtocollazione.OK)
                {
                    if (schedaDoc.protocollo.invioConferma != null && schedaDoc.protocollo.invioConferma.Equals("1"))
                    {
                        ...
                        {
                            SAAdminTool.DocsPaWR.DocsPaWebService ws = new DocsPaWebService();
        ...
        public static bool DocumentoInvioRicevuta(Page page, DocsPaWR.SchedaDocumento schedaDocumento, SAAdminTool.DocsPaWR.Registro registro)
        {
            bool result = false;
            string errormessage = string.Empty;
            try
            {
                ...
                DataSet ds = SAAdminTool.utils.MultiCasellaManager.GetAssDocAddress(DocumentManager.getDocumentoSelezionato().docNumber);
                if (ds != null && ds.Tables["ass_doc_rf"].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables["ass_doc_rf"].Rows)
                    {
                        if (!row["registro"].ToString().Equals(registro.systemId) && (!string.IsNullOrEmpty(registro.email)) && 
                            ...
                            SAAdminTool.utils.MultiCasellaManager.UpdateAssDocAddress(DocumentManager.getDocumentoSelezionato().docNumber, registro.systemId, registro.email);
                    }
                }
        ...
        public static bool DocumentoInvioNotificaAnnulla(Page page, string idProfile, SAAdminTool.DocsPaWR.Registro registro)
        {
            bool result = false;
        ...
        public static SchedaDocumento protocolla(Page page, SchedaDocumento schedaDoc, DocsPaWR.Fascicolo fascicolo, SAAdminTool.DocsPaWR.TemplateTrasmissione template, out SAAdminTool.DocsPaWR.ResultProtocollazione risultatoProtocollazione, ref string returnMsg)
        {
            string segnatura = "";
            risultatoProtocollazione = SAAdminTool.DocsPaWR.ResultProtocollazione.OK;
            try
        ...
        public static SAAdminTool.DocsPaWR.Fascicolo GetFascicoloPrimarioDoc(Page page, SAAdminTool.DocsPaWR.SchedaDocumento schedaDoc)
        {
        ...
        /*        public static void addDocumentoInFolder(Page page, string idProfile, string idFolder, bool fascRapida, out bool isInFolder)
                ...
                protected static SAAdminTool.DocsPaWR.AreaLavoro addInListaAreaLav(SAAdminTool.DocsPaWR.AreaLavoro areaLav, object oggetto)
                {
                    object[] nuovaLista;
                ...
                //		public static SAAdminTool.DocsPaWR.AreaLavoro rimuoviDaListaAreaLav(SAAdminTool.DocsPaWR.AreaLavoro areaLav, int index) 
                //		{
                ...
                //				return null;
                //
                //			DocsPaWR.AreaLavoro nuovaAreaLav = new SAAdminTool.DocsPaWR.AreaLavoro();
        ...
        */
        ...
        public static bool addDocumentoInFolder(Page page, string idProfile, string idFolder, bool fascRapida, out bool isInFolder, out String message)
        {
            ...
            try
            {
                ...
                if (!isInFolder)
                {
                    ...
                    bool outValue = false;
                    string msg = string.Empty;
                    Fascicolo fasc = SAAdminTool.FascicoliManager.getFascicoloById(page, fol.idFascicolo);
        ...
        public static SAAdminTool.DocsPaWR.ProtocolloDestinatario[] getDestinatariInteropAggConferma(Page page, string idProfile, Corrispondente corr)
        {
            try
            {
        ...
        public static InfoDocumento[] getDocAll(SAAdminTool.DocsPaWR.InfoUtente Safe, Page page, DocsPaWR.FiltroRicerca[][] query)
        {
        ...
        public static bool editingACL(SAAdminTool.DocsPaWR.DocumentoDiritto docDiritto, string personOrGroup, DocsPaWR.InfoUtente infoUtente)
        {
            bool result = false;
            try
            {
        ...
        public static bool ripristinaACL(SAAdminTool.DocsPaWR.DocumentoDiritto docDiritto, string personOrGroup, DocsPaWR.InfoUtente infoUtente)
        {
            bool result = false;
            try
            {
        ...
        public static SchedaDocumento annullaProtocollo(Page page, string autoriz)
        {
            try
            {
                ...
                DocsPaWR.ProtocolloAnnullato protAnnullato = new SAAdminTool.DocsPaWR.ProtocolloAnnullato();
        ...
        public static SAAdminTool.DocsPaWR.SendDocumentResponse spedisciDocMail(Page page, DocsPaWR.SchedaDocumento schedaDocumento, string mailAddress)
        {
        ...
        public static SAAdminTool.DocsPaWR.SendDocumentResponse spedisciDoc(Page page, DocsPaWR.SchedaDocumento schedaDocumento)
        {
            DocsPaWR.SendDocumentResponse retValue = null;
        ...
        */
        ...
        public static SAAdminTool.DocsPaWR.DocumentoParolaChiave[] getListaParoleChiaveSel(Page page)
        {
            return (SAAdminTool.DocsPaWR.DocumentoParolaChiave[])GetSessionValue("ricDoc.listaParoleChiave");
        }
        ...
        public static SchedaDocumento riproponiDati(Page page, SchedaDocumento schedaDoc, bool eUffRef)
        {
            try
            {
                ...
                {
                    case "A":
                        schedaNewDoc.protocollo = new SAAdminTool.DocsPaWR.ProtocolloEntrata();
                        ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaNewDoc.protocollo).mittente = ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittente;
                        ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaNewDoc.protocollo).mittenteIntermedio = ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenteIntermedio;
                        if (eUffRef)
                        {
                            ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaNewDoc.protocollo).ufficioReferente = ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).ufficioReferente;
                        }
                        if (DocumentManager.isEnableMittentiMultipli())
                        {
                            ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaNewDoc.protocollo).mittenti = ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenti;
                        }
                        break;
                    case "P":
                        schedaNewDoc.protocollo = new SAAdminTool.DocsPaWR.ProtocolloUscita();
                        ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaNewDoc.protocollo).mittente = ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaDoc.protocollo).mittente;
                        ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaNewDoc.protocollo).destinatari = ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaDoc.protocollo).destinatari;
                        ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaNewDoc.protocollo).destinatariConoscenza = ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaDoc.protocollo).destinatariConoscenza;
                        if (eUffRef)
                        {
                            ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaNewDoc.protocollo).ufficioReferente = ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaDoc.protocollo).ufficioReferente;
                        }
                        ...
                        break;
                    case "I":
                        schedaNewDoc.protocollo = new SAAdminTool.DocsPaWR.ProtocolloInterno();
                        ((SAAdminTool.DocsPaWR.ProtocolloInterno)schedaNewDoc.protocollo).mittente = ((SAAdminTool.DocsPaWR.ProtocolloInterno)schedaDoc.protocollo).mittente;
                        ((SAAdminTool.DocsPaWR.ProtocolloInterno)schedaNewDoc.protocollo).destinatari = ((SAAdminTool.DocsPaWR.ProtocolloInterno)schedaDoc.protocollo).destinatari;
                        ((SAAdminTool.DocsPaWR.ProtocolloInterno)schedaNewDoc.protocollo).destinatariConoscenza = ((SAAdminTool.DocsPaWR.ProtocolloInterno)schedaDoc.protocollo).destinatariConoscenza;
                        if (eUffRef)
                        {
                            ((SAAdminTool.DocsPaWR.ProtocolloInterno)schedaNewDoc.protocollo).ufficioReferente = ((SAAdminTool.DocsPaWR.ProtocolloInterno)schedaDoc.protocollo).ufficioReferente;
                        }
                        break;
                }
                return schedaNewDoc;
        ...
        public static SchedaDocumento riproponiDatiRispIngresso(Page page, SchedaDocumento schedaDoc, SAAdminTool.DocsPaWR.Corrispondente destSelezionato)
        {
            try
            {
                ...
                {
                    schedaNewDoc.protocollo = new SAAdminTool.DocsPaWR.ProtocolloEntrata();
                    ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaNewDoc.protocollo).mittente = new SAAdminTool.DocsPaWR.Corrispondente();
                    ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaNewDoc.protocollo).mittente = destSelezionato;
                }
        ...
        public static SchedaDocumento riproponiDatiRisp(Page page, SchedaDocumento schedaDoc)
        {
            try
            {
                ...
                {
                    schedaNewDoc.protocollo = new SAAdminTool.DocsPaWR.ProtocolloUscita();
                    ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaNewDoc.protocollo).destinatari = new SAAdminTool.DocsPaWR.Corrispondente[1];
                    ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaNewDoc.protocollo).destinatari[0] = new SAAdminTool.DocsPaWR.Corrispondente();
                    ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaNewDoc.protocollo).destinatari[0] = ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittente;
                }
        ...
        public static SAAdminTool.DocsPaWR.AreaLavoro getListaAreaLavoro(Page page, string tipoDoc, string chaDaProto, int nPage, out int numRec, out int nRec, string idRegistro)
        {
            ...
            try
            {
                DocsPaWR.AreaLavoroTipoOggetto tipoObj = SAAdminTool.DocsPaWR.AreaLavoroTipoOggetto.DOCUMENTO;
                DocsPaWR.AreaLavoroTipoFascicolo tipoFasc = new SAAdminTool.DocsPaWR.AreaLavoroTipoFascicolo();
                DocsPaWR.AreaLavoroTipoDocumento tipoDocumento = new SAAdminTool.DocsPaWR.AreaLavoroTipoDocumento();

                Utente utente = (Utente)GetSessionValue("userData");
                Ruolo ruolo = (Ruolo)GetSessionValue("userRuolo");

                if (tipoDoc.Equals("A"))
                    tipoDocumento = SAAdminTool.DocsPaWR.AreaLavoroTipoDocumento.ARRIVO;
                else if (tipoDoc.Equals("P"))
                    tipoDocumento = SAAdminTool.DocsPaWR.AreaLavoroTipoDocumento.PARTENZA;
                else if (tipoDoc.Equals("I"))
                    tipoDocumento = SAAdminTool.DocsPaWR.AreaLavoroTipoDocumento.INTERNO;
                else if (tipoDoc.Equals("T"))
                    tipoDocumento = SAAdminTool.DocsPaWR.AreaLavoroTipoDocumento.TUTTI;
                else
                    tipoDocumento = SAAdminTool.DocsPaWR.AreaLavoroTipoDocumento.GRIGIO;
        ...
        public static SAAdminTool.DocsPaWR.DocumentoParolaChiave[] getListaParoleChiave(Page page)
        {
            try
            {
        ...
        public static SAAdminTool.DocsPaWR.TipologiaAtto[] getListaTipologiaAtto(Page page)
        {
            try
            {
        ...
        public static SAAdminTool.DocsPaWR.TipologiaAtto[] getTipoAttoPDInsRic(Page page, string idAmministrazione, string idGruppo, string diritti)
        {
            try
            {
                DocsPaWR.TipologiaAtto[] listaTipologiaAtto;
        ...
        public static SAAdminTool.DocsPaWR.TipologiaAtto[] getListaTipologiaAtto(Page page, string idAmministrazione)
        {
            try
            {
                DocsPaWR.TipologiaAtto[] listaTipologiaAtto;
        ...
        public static string verificaDirittiCestinaDocumento(Page page, SAAdminTool.DocsPaWR.SchedaDocumento schedaDocumento)
        {
            string result = "";
            try
            {
        ...
        public static bool CestinaDocumento(Page page, SAAdminTool.DocsPaWR.SchedaDocumento schedaDocumento, string tipoDoc, string note, out string errorMsg)
        {
            bool result = true;
        ...
        public static InfoDocumento[] getDocInCestino(Page page, SAAdminTool.DocsPaWR.InfoUtente infoUtente, DocsPaWR.FiltroRicerca[][] filtriRicerca)
        {
        ...
        public static bool riattivaDocumento(Page page, SAAdminTool.DocsPaWR.InfoUtente infoUtente, SAAdminTool.DocsPaWR.InfoDocumento infoDoc)
        {
            bool result = false;
            try
            {
        ...
        public static string rimuoviSchedadocumento(Page page, SAAdminTool.DocsPaWR.SchedaDocumento schedaDocumento)
        {
            string result = "";
            try
            {
        ...
        public static bool svuotaCestino(out bool docInCestino, Page page, SAAdminTool.DocsPaWR.InfoUtente infoUtente, DocsPaWR.InfoDocumento[] ListaDoc)
        {
            bool result = false;
            docInCestino = false;
            try
        ...
        public static bool EliminaDoc(Page page, SAAdminTool.DocsPaWR.InfoUtente infoUtente, DocsPaWR.InfoDocumento infoDoc)
        {
            bool result = false;
            try
            {
        ...
        public static SAAdminTool.DocsPaWR.DocumentoStoricoOggetto[] getStoriaModifiche(Page page, SAAdminTool.DocsPaWR.SchedaDocumento schedaDocumento, string tipo)
        {
            try
            {
        ...
        public static SAAdminTool.DocsPaWR.StoricoProfilati[] getStoriaProfilatiAtto(Page page, SAAdminTool.DocsPaWR.Templates template, string doc_number, string idGroup)
        {
            try
            {
        ...
        public static SAAdminTool.DocsPaWR.DocumentoLogDocumento[] getStoriaLog(Page page, string idOggetto, string varOggetto)
        {
            try
            {
                SAAdminTool.DocsPaWR.DocumentoLogDocumento[] result = docsPaWS.DocumentoGetListaLog(idOggetto, varOggetto);
        ...
        /// <returns></returns>
        public static SAAdminTool.DocsPaWR.Allegato[] getAllegati(DocsPaWR.SchedaDocumento schedaDocumento, string filterTipologiaAllegato, string simplifiedInteroperabilityId="")
        {
        ...
        /// <returns></returns>
        public static SAAdminTool.DocsPaWR.Allegato aggiungiAllegato(DocsPaWR.Allegato allegato)
        {
            try
            {
        ...
        /// <param name="schedaDocumento"></param>
        /// <returns></returns>
        public static bool rimuoviAllegato(SAAdminTool.DocsPaWR.Allegato allegato, DocsPaWR.SchedaDocumento schedaDocumento)
        {
            //inserisco i dati relativi all'utente
        ...
        public static void scambiaAllegato(Page page, SAAdminTool.DocsPaWR.Documento documento, SAAdminTool.DocsPaWR.Allegato allegato)
        {
            try
            {
        ...
        public static SchedaDocumento scambiaAllegatoinLista(SAAdminTool.DocsPaWR.SchedaDocumento schedaDocumento, int index)
        {
            ...
            DocsPaWR.Documento newDoc = new SAAdminTool.DocsPaWR.Documento();
            DocsPaWR.Allegato newAllegato = new SAAdminTool.DocsPaWR.Allegato();
        ...
        public static SAAdminTool.DocsPaWR.Allegato[] getAllegatiPerRimozione(string docNumber)
        {
            DocsPaWR.Allegato[] allegati = null;
        ...
        public static bool rimuoviVersione(Page page, SAAdminTool.DocsPaWR.Documento documento, DocsPaWR.SchedaDocumento schedaDocumento)
        {
            bool result = false;
        ...
        public static SAAdminTool.DocsPaWR.AnelloDocumentale getCatenaDoc(string idGruppo, string idPeople, string idProfile, Page page)
        {
            try
            {
        ...
        public static SAAdminTool.DocsPaWR.Fascicolo[] GetFascicoliDaDoc(Page page, string idProfile)
        {
            try
            {
                InfoUtente infoUtente = UserManager.getInfoUtente(page);
        ...
        public static string getFascicoloDoc(Page page, SAAdminTool.DocsPaWR.InfoDocumento infoDocumento)
        {
        ...
        public static SAAdminTool.DocsPaWR.EsitoRicercaDuplicatiEnum cercaDuplicati(Page page, SchedaDocumento schedaDoc, string cercaDuplicati2, out InfoProtocolloDuplicato[] datiProtDupl)
        {
            SAAdminTool.DocsPaWR.EsitoRicercaDuplicatiEnum res = SAAdminTool.DocsPaWR.EsitoRicercaDuplicatiEnum.NessunDuplicato;
            ...
            {
                res = SAAdminTool.DocsPaWR.EsitoRicercaDuplicatiEnum.ErroreGenerico;
                ErrorManager.redirect(page, es);
            }
        ...
        public static SAAdminTool.DocsPaWR.DocumentoParolaChiave[] removeParoleChiave(SAAdminTool.DocsPaWR.DocumentoParolaChiave[] lista, int index)
        {
            // Lista da restituire
        ...
        public static SAAdminTool.DocsPaWR.DocumentoParolaChiave[] addParolaChiave(SAAdminTool.DocsPaWR.DocumentoParolaChiave[] lista, SAAdminTool.DocsPaWR.DocumentoParolaChiave parolaC)
        {
            DocsPaWR.DocumentoParolaChiave[] nuovaLista;
            if (lista != null)
            {
                int len = lista.Length;
                nuovaLista = new SAAdminTool.DocsPaWR.DocumentoParolaChiave[len + 1];
                lista.CopyTo(nuovaLista, 0);
                nuovaLista[len] = parolaC;
            }
            else
            {
                nuovaLista = new SAAdminTool.DocsPaWR.DocumentoParolaChiave[1];
        ...
        public static SAAdminTool.DocsPaWR.FascicolazioneClassificazione getClassificazioneSelezionata(Page page)
        {
            return (SAAdminTool.DocsPaWR.FascicolazioneClassificazione)GetSessionValue("classifSelezionata");
        }

        public static void setClassificazioneSelezionata(Page page, SAAdminTool.DocsPaWR.FascicolazioneClassificazione classificazione)
        {
        ...
        public static void SetSignedDocument(SAAdminTool.DocsPaWR.FileDocumento signedDocument)
        {
            SetSessionValue("SignedDocument", signedDocument);
        }

        public static SAAdminTool.DocsPaWR.FileDocumento GetSignedDocument()
        {
            return (SAAdminTool.DocsPaWR.FileDocumento)GetSessionValue("SignedDocument");
        }
        ...
        public static SAAdminTool.DocsPaWR.FiltroRicerca[][] getMemoriaFiltriRicDoc(Page page)
        {
            return (SAAdminTool.DocsPaWR.FiltroRicerca[][])GetSessionValue("MemoriaFiltriRicDoc");
        }

        public static void setMemoriaFiltriRicDoc(Page page, SAAdminTool.DocsPaWR.FiltroRicerca[][] listaFiltriDoc)
        {
        ...
        public static SAAdminTool.DocsPaWR.SchedaDocumento DO_AddDestinatrioModificato(SAAdminTool.DocsPaWR.SchedaDocumento scheda, string systemId)
        {
            return docsPaWS.DO_AddDestinatrioModificato(scheda, systemId);
        }

        public static SAAdminTool.DocsPaWR.SchedaDocumento DO_AddDestinatrioCCModificato(SAAdminTool.DocsPaWR.SchedaDocumento scheda, string systemId)
        {
            return docsPaWS.DO_AddDestinatrioCCModificato(scheda, systemId);
        }

        public static SAAdminTool.DocsPaWR.SchedaDocumento DO_ClearDestinatariModificati(SAAdminTool.DocsPaWR.SchedaDocumento scheda)
        {
            return docsPaWS.DO_ClearDestinatariModificati(scheda);
        }

        public static SAAdminTool.DocsPaWR.SchedaDocumento DO_ClearDestinatariCCModificati(SAAdminTool.DocsPaWR.SchedaDocumento scheda)
        {
            return docsPaWS.DO_ClearDestinatariCCModificati(scheda);
        }

        public static SAAdminTool.DocsPaWR.SchedaDocumento DO_RemoveDestinatarioModificati(SAAdminTool.DocsPaWR.SchedaDocumento scheda, string systemId)
        {
            return docsPaWS.DO_RemoveDestinatrioModificato(scheda, systemId);
        }

        public static SAAdminTool.DocsPaWR.SchedaDocumento DO_RemoveDestinatarioCCModificati(SAAdminTool.DocsPaWR.SchedaDocumento scheda, string systemId)
        {
            return docsPaWS.DO_RemoveDestinatrioCCModificato(scheda, systemId);
        }

        public static bool DO_UpdateVisibilita(Page page, DocsPaWR.SchedaDocumento scheda, SAAdminTool.DocsPaWR.Ruolo ruolo)
        {
        ...
        //public static SAAdminTool.DocsPaWR.DocumentoStoricoMittente[] getStoriaModifiche(SAAdminTool.DocsPaWR.SchedaDocumento schedaDocumento, string tipo)
        //{
        ...
        public static bool isDocInConversionePdf(SAAdminTool.DocsPaWR.InfoUtente infoUtente, string idProfile)
        {
            try
            {
        ...
        public static void delDocRichiestaConversionePdf(SAAdminTool.DocsPaWR.InfoUtente infoUtente, string idProfile)
        {
            try
            {
        ...
        public static int getItemSize(Page page, SAAdminTool.DocsPaWR.SchedaDocumento schedaDoc, string idItem)
        {
            int size = 0;
            try
            {
        ...
        public static bool insertAllInCons(Page page, SAAdminTool.DocsPaWR.FiltroRicerca[][] objFiltri, SAAdminTool.DocsPaWR.InfoUtente Safe)
        {
            bool res = true;
            int result = 1;
            try
        ...
        public static SAAdminTool.DocsPaWR.Templates[] getTipoAttoTrasfDeposito(Page page, string idAmministrazione, bool seRepertorio)
        {
            try
            {
        ...
        public static InfoDocumento[] getDocInSerie(string idGruppo, string idPeople, Page page, SAAdminTool.DocsPaWR.Templates template, int numPage, out int numTotPage, out int nRec, string valOggetto, string anno, string rfAOO)
        //public static InfoDocumento[] getDocInSerie(string idGruppo, string idPeople, Page page, SAAdminTool.DocsPaWR.Templates template, out int nRec)
        {
            nRec = 0;
            numTotPage = 0;
            InfoDocumento[] DocS = null;
        ...
        public static int trasfInDepositoSerie(InfoUtente utente, Page page, SAAdminTool.DocsPaWR.Templates template, string anno, string valOggetto, string tipoOp, string rfAOO)
        {
            int result = 0;
            try
            {
        ...
        public static String GetProtocolloTitolario(SAAdminTool.DocsPaWR.SchedaDocumento schedaDocumento)
        {
        ...
        public static SAAdminTool.DocsPaWR.InfoDocumento GetInfoDocumento(string idProfile, string docNumber, Page page)
        {
            try
            {
        ...
        public static SAAdminTool.DocsPaWR.DocumentoStoricoDataArrivo[] getStoriaModifiche(string docnumber)
        {
        ...
        public static SAAdminTool.DocsPaWR.Fascicolo[] GetFascicoliDaDocNoSecurity(Page page, string idProfile)
        {
            try
            {
                InfoUtente infoUtente = UserManager.getInfoUtente(page);
        ...
        /// <returns></returns>
        public static void executeAndSaveTSR(SAAdminTool.DocsPaWR.InfoUtente infoUtente, SAAdminTool.DocsPaWR.InputMarca richiesta, SAAdminTool.DocsPaWR.FileRequest fileRequest, out string message)
        {
        ...
        /// <param name="utente"></param>
        /// <returns></returns>
        public static ArrayList getTimestampsDoc(SAAdminTool.DocsPaWR.InfoUtente infoUtente, SAAdminTool.DocsPaWR.FileRequest fileRequest)
        {
            docsPaWS.Timeout = System.Threading.Timeout.Infinite;
        ...
        /// <param name="utente"></param>
        /// <returns></returns>
        public static FileRequest CreateTSDVersion(SAAdminTool.DocsPaWR.InfoUtente infoUtente, SAAdminTool.DocsPaWR.FileRequest fileRequest)
        {
            docsPaWS.Timeout = System.Threading.Timeout.Infinite;
        ...
        public static SAAdminTool.DocsPaWR.SearchObject[] getQueryInfoDocumentoPagingCustom(InfoUtente infoUtente, Page page, DocsPaWR.FiltroRicerca[][] query, int numPage, out int numTotPage, out int nRec, bool security, bool getIdProfilesList, bool gridPersonalization, int pageSize, bool export, Field[] visibleFieldsTemplate, String[] documentsSystemId, out SearchResultInfo[] idProfilesList)
        {
            // La lista dei system id dei documenti restituiti dalla ricerca
            SearchResultInfo[] idProfiles = null;
            ...
            try
            {
                SAAdminTool.DocsPaWR.SearchObject[] DocS = null;
                string textToSearch = string.Empty;

...
namespace SAAdminTool
{
    public class ProfilazioneDocManager
    {
        ...
        public static string verificaOkContatore(SAAdminTool.DocsPaWR.Templates template)
        {
        ...
        public static void verificaCampiPersonalizzati(Page page, SAAdminTool.DocsPaWR.SchedaDocumento schedaDocumento)
        {
            ...
            if (currentContext != null && currentContext.ContextName == SiteNavigation.NavigationKeys.DOCUMENTO)
            {
                ...
                {
                    SAAdminTool.DocsPaWR.Templates template = schedaDocumento.template;
            ...
            if (schedaDocumento != null && !string.IsNullOrEmpty(schedaDocumento.docNumber) && schedaDocumento.template != null)
            {
                ...
                if (templateDelDoc != null && templateDelDoc.SYSTEM_ID == 0)
                {
                    page.Session.Add("template", schedaDocumento.template);
                    impostaOpzioniTipologia(page, schedaDocumento.template);
                    if (SAAdminTool.DiagrammiManager.getStatoDoc(schedaDocumento.docNumber, page) != null)
                    {
                        ddl_tipoAtto.Enabled = false;
                        if (isAutorizedDelTipology)
                            btn_delTipologyDoc.Visible = true;
            ...
            //            {
            //                templateInSess = (SAAdminTool.DocsPaWR.Templates)page.Session["template"];
        ...
        public static void estendiDirittiRuoloACampiDoc(ArrayList listaDirittiRuoli, ArrayList listaCampi)
        {
            try
            {
                DocsPaWR.OggettoCustom[] listaCampi_1 = new SAAdminTool.DocsPaWR.OggettoCustom[listaCampi.Count];
                listaCampi.CopyTo(listaCampi_1);

                DocsPaWR.AssDocFascRuoli[] listaDirittiRuoli_1 = new SAAdminTool.DocsPaWR.AssDocFascRuoli[listaDirittiRuoli.Count];
        ...
        public static SAAdminTool.DocsPaWR.Contatore[] GetValuesContatoriDoc(Page page, OggettoCustom oggettoCustom)
        {
            try
            {
                SAAdminTool.DocsPaWR.Contatore[] result = docsPaWS.GetValuesContatoriDoc(oggettoCustom);
                return result;
            }
            catch (Exception ex)
            {
        ...
        public static void AnnullaContatoreDiRepertorio(string idOggetto, string docNumber, Page page)
        {
            try
            {
                docsPaWS.AnnullaContatoreDiRepertorio(idOggetto, docNumber);
            }
            catch (Exception ex)
            {
                ErrorManager.redirect(page, ex);                

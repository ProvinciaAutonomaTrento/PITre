...
namespace SAAdminTool.utils
{
    ...
    public class MultiCasellaManager
    {
        ...
        public static List<CasellaRegistro> GetComboRegisterConsult(string idRegistro)
        {
            ...
            string casellaPrincipale = SAAdminTool.utils.MultiCasellaManager.GetMailPrincipaleRegistro(idRegistro);
            DataSet ds = SAAdminTool.utils.MultiCasellaManager.GetRightMailRegistro(idRegistro, UserManager.getRuolo().systemId);
        ...
        /// <returns></returns>
        public static bool RoleIsAuthorizedConsult(SAAdminTool.DocsPaWR.Registro registro, string idRuolo)
        {
            bool isEnable = true;
            ...
            DataSet ds = SAAdminTool.utils.MultiCasellaManager.GetRightMailRegistro(registro.systemId, idRuolo);
            if (ds.Tables.Count > 0)
            {
                if (ds.Tables["RIGHT_RUOLO_MAIL_REGISTRI"].Rows.Count > 0)
                {
        ...
        {
            List<DocsPaWR.Registro> listRegistriRF = new List<SAAdminTool.DocsPaWR.Registro>();
            //prendo gli rf associati al ruolo 
            SAAdminTool.DocsPaWR.Registro[] rf = UserManager.getListaRegistriWithRF(UserManager.getRuolo().systemId, "1", UserManager.getRegistroSelezionato(page).systemId);
            foreach (SAAdminTool.DocsPaWR.Registro registro in rf)
            {
                DataSet ds = utils.MultiCasellaManager.GetRightMailRegistro(registro.systemId, UserManager.getRuolo().systemId);
                if (ds.Tables["RIGHT_RUOLO_MAIL_REGISTRI"].Rows.Count > 0)
        ...
        public static bool InsertMailCorrispondenteEsterno(System.Collections.Generic.List<SAAdminTool.DocsPaWR.MailCorrispondente> listCaselle, string idCorrispondente)
        {
        ...
        public static List<SAAdminTool.DocsPaWR.MailCorrispondente> GetMailsAllCorrProto(int idDoc)
        {

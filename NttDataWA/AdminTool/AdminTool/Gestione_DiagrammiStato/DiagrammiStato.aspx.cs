...
namespace SAAdminTool.AdminTool.Gestione_DiagrammiStato
{
	...
	public class DiagrammiStato : System.Web.UI.Page
	{
		...
		public void settaStatiIniziali(ArrayList statiIniziali)
		{
			...
			{
				lbl_statiIniziali.Text += ((SAAdminTool.DocsPaWR.Stato) statiIniziali[i]).DESCRIZIONE + " - ";
			}
		...
		public void settaStatiFinali(ArrayList statiFinali)
		{
			...
			{
				lbl_statiFinali.Text += ((SAAdminTool.DocsPaWR.Stato) statiFinali[i]).DESCRIZIONE + " - ";
			}
		...
		private void setLbxStati2(SAAdminTool.DocsPaWR.Stato[] successivi /*ArrayList successivi*/)
		{
			...
			if(successivi != null)
			{
				...
				for(int i=0; i<successivi.Length; i++)
				{				
					lbox_stati2.Items.Add( ((SAAdminTool.DocsPaWR.Stato) successivi[i]).DESCRIZIONE);
					ddl_statiAutomatici.Items.Add( ((SAAdminTool.DocsPaWR.Stato) successivi[i]).DESCRIZIONE);
				}
			}
		}
		...
		private void rimuoviPasso(int stepDaRimuovere)
		{
			if( ((SAAdminTool.DocsPaWR.DiagrammaStato) Session["DiagrammaStato"]).PASSI != null)
			{
				ArrayList passi = new ArrayList( ((SAAdminTool.DocsPaWR.DiagrammaStato) Session["DiagrammaStato"]).PASSI);
		...
		private void btn_nuovoDiagr_Click(object sender, System.EventArgs e)
		{
			...
			//DocsPaWR.DiagrammaStato diagrammaStato = new SAAdminTool.DocsPaWR.DiagrammaStato();
			DocsPaWR.DiagrammaStato diagrammaStato = new SAAdminTool.DocsPaWR.DiagrammaStato();

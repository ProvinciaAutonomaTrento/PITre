...
namespace Amministrazione.Gestione_Funzioni
{
    ...
    public class TipiFunzione : System.Web.UI.Page
    {
        ...
        /// </summary>
        /// <returns></returns>
        private SAAdminTool.DocsPaWR.OrgTipoFunzione[] GetTipiFunzione(string idAmm)
        {
            AmmUtils.WebServiceLink ws = new AmmUtils.WebServiceLink();
        ...
        /// <returns></returns>
        private SAAdminTool.DocsPaWR.OrgTipoFunzione GetTipoFunzione(string idTipoFunzione)
        {
            AmmUtils.WebServiceLink ws = new AmmUtils.WebServiceLink();
            return ws.GetTipoFunzione(idTipoFunzione, true);
        }

        private SAAdminTool.DocsPaWR.OrgFunzione[] GetFunzioni(string idTipoFunzione)
        {
            AmmUtils.WebServiceLink ws = new AmmUtils.WebServiceLink();
        ...
        /// <returns></returns>
        private SAAdminTool.DocsPaWR.ValidationResultInfo InsertTipoFunzione(ref SAAdminTool.DocsPaWR.OrgTipoFunzione tipoFunzione)
        {
            SAAdminTool.DocsPaWR.ValidationResultInfo canInsert = this.CanSaveFunction(ref tipoFunzione);
        ...
        private SAAdminTool.DocsPaWR.ValidationResultInfo CanSaveFunction(ref SAAdminTool.DocsPaWR.OrgTipoFunzione tipoFunzione)
        {
            ...
            if (!retVal.Value)
                retVal.BrokenRules = new SAAdminTool.DocsPaWR.BrokenRule[] { new SAAdminTool.DocsPaWR.BrokenRule() { Description = "Associare almeno una funzione elementare al tipo funzione", Level = SAAdminTool.DocsPaWR.BrokenRuleLevelEnum.Error, ID = "FUNZIONI_MANCANTI" } };
        ...
        /// </summary>
        /// <param name="tipoFunzione"></param>
        private void FillListFunzioni(SAAdminTool.DocsPaWR.OrgFunzione[] funzioni)
        {
             ...
             */
            //this.btn_aggiungi.Text = "Modifica";
            SAAdminTool.Utils.DefaultButton(this, ref txt_ricerca, ref btn_find);
        ...
        {
            string idAmm = AmmUtils.UtilsXml.GetAmmDataSession((string)Session["AMMDATASET"], "3");
            SAAdminTool.DocsPaWR.OrgTipoFunzione[] tipiFunzione = this.GetTipiFunzione(idAmm);
        ...
        /// <returns></returns>
        private DataSet ConvertToDataSetTipiFunzione(SAAdminTool.DocsPaWR.OrgTipoFunzione[] tipiFunzione)
        {
            DataSet ds = this.CreateGridDataSetTipiFunzione();
            DataTable dt = ds.Tables["TipiFunzione"];

            foreach (SAAdminTool.DocsPaWR.OrgTipoFunzione tipoFunzione in tipiFunzione)
            {
        ...
        private DataSet ConvertToDataSetFunzioni(SAAdminTool.DocsPaWR.OrgFunzione[] funzioni)
        {
            DataSet ds = this.CreateGridDataSetFunzioni();
            DataTable dt = ds.Tables["Funzioni"];

            foreach (SAAdminTool.DocsPaWR.OrgFunzione funzione in funzioni)
            {
                DataRow row = dt.NewRow();

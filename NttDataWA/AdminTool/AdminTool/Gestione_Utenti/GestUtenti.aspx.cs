    ...
    public class GestUtenti : System.Web.UI.Page
    {
        ...
        private void LoadUtenti(ArrayList listaUtenti)
        {
            try
            {
                ...
                DataRow row;
                foreach (SAAdminTool.DocsPaWR.OrgUtente utente in listaUtenti)
                {
        ...
        /// </summary>
        private void AggiornaDGDopoMod(SAAdminTool.DocsPaWR.OrgUtente utente)
        {
            try
            {
                ...
                if (this.VerificaCampiObbligatori())
                {
                    if (this.VerificaCampiInseriti())
                    {
                        ...
                        if (continua)
                        {
                            ...
                            utente.SmartClientConfigurations.ComponentsType = (this.rdbIsEnabledJavaApplet.Checked ? "3" : (this.rdbIsEnabledSmartClient.Checked ? "2" : (this.rdbIsEnabledActiveX.Checked ? "1" : "0")));
        ...
        private bool VerificaUtenteMultiAmministrazione(string userId)
        {
            ...
            SAAdminTool.DocsPaWR.Amministrazione[] listaAmm = UserManager.getListaAmministrazioniByUser(this, userId, false, out returnMsg);
        ...
        {
            string returnMsg = string.Empty;
            SAAdminTool.DocsPaWR.Amministrazione[] listaAmm = UserManager.getListaAmministrazioniByUser(this, userId, true, out returnMsg);
        ...
        private void ModificaUtente()
        {
            ...
            try
            {
                ...
                if (this.VerificaCampiObbligatori())
                {
                    ...
                    {
                        Manager.UtentiManager theManager = new Amministrazione.Manager.UtentiManager();
                        SAAdminTool.DocsPaWR.OrgUtente utente = null;
                        isUserMultiAmm = (VerificaUtenteMultiAmministrazioneMod(this.txt_userid.Text.Trim().ToUpper()));

                        if (Session["ok_multiAmm"] != null)
                        {
                            utente = (SAAdminTool.DocsPaWR.OrgUtente)Session["ok_multiAmm"];
                            Session.Remove("ok_multiAmm");
                        }
        ...
        /// ELIMINA
        /// </summary>
        private void EliminaUtente(SAAdminTool.DocsPaWR.OrgUtente utente)
        {
            try
            {
        ...
        private void EliminaUtenteDefin(SAAdminTool.DocsPaWR.OrgUtente utente)
        {
            try
            {
                Manager.UtentiManager utManager = new Amministrazione.Manager.UtentiManager();
        ...
        private bool GestAbilitaDisabilitaUtente(SAAdminTool.DocsPaWR.OrgUtente utente)
        {
            bool retValue = true;
        ...
        /// Disabilita Utente
        /// </summary>
        private bool DisabilitaUtente(SAAdminTool.DocsPaWR.OrgUtente utente)
        {
            bool retValue = false;
        ...
        /// Abilita Utente
        /// </summary>
        private bool AbilitaUtente(SAAdminTool.DocsPaWR.OrgUtente utente)
        {
            bool retValue = false;
        ...
        /// </summary>
        private void DisabilitaUtDaEliminare(SAAdminTool.DocsPaWR.OrgUtente utente)
        {
            try
            {
                Manager.UtentiManager theManager = new Amministrazione.Manager.UtentiManager();
        ...
        protected bool LdapIntegrationActivated
        {
            get
            {
                if (this.ViewState["LdapIntegrationActivated"] == null)
                {
                    ...
                    try
                    {
                        SAAdminTool.DocsPaWR.LdapConfig config = ws.GetLdapConfig(sessionManager.getUserAmmSession(), this.GetIdAmministrazione().ToString());
        ...
        {
            string configValue = SAAdminTool.utils.InitConfigurationKeys.GetValue("0", "FE_GESTIONE_MATRICOLE");

...
namespace SAAdminTool.popup
{
    ...
    {
        protected SAAdminTool.DocsPaWR.DocumentoDiritto[] list;
        
        protected SAAdminTool.dataSet.DS_Visibilit dS_Visibilit1;
        ...
        protected string GetTipoCorr(SAAdminTool.DocsPaWR.DocumentoDiritto docDirit)
        {
            try
            {
                string rtn = "";
                if (docDirit.soggetto.GetType() == typeof(SAAdminTool.DocsPaWR.Utente))
                    rtn = "UTENTE";
                else if (docDirit.soggetto.GetType() == typeof(SAAdminTool.DocsPaWR.Ruolo))
                    rtn = "RUOLO";
                else if (docDirit.soggetto.GetType() == typeof(SAAdminTool.DocsPaWR.UnitaOrganizzativa))
                    rtn = "U.O.";
                return rtn;
            }
            catch (Exception ex)

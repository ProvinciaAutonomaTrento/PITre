...
namespace SAAdminTool.AdminTool.Gestione_ModelliTrasm
{
	/// <summary>
	...
	public class ModelliTrasm : System.Web.UI.Page
	{
		...
		protected SAAdminTool.DocsPaWR.RagioneTrasmissione[] listaRagioni;
		...
		protected SAAdminTool.DocsPaWR.ModelloTrasmissione ModelloTrasmissione = new SAAdminTool.DocsPaWR.ModelloTrasmissione();
        ...
        protected SAAdminTool.UserControls.PannelloRicercaModelliTrasmissione prmtPannelloRicerca;
        ...
        private void ContinueWithPageLoad()
        {
            ...
            if (!IsPostBack)
            {
                ...
                ModelloTrasmissione = new SAAdminTool.DocsPaWR.ModelloTrasmissione();
            ...
            {
                addMittSelDaRubrica((SAAdminTool.DocsPaWR.ElementoRubrica[])Session["selMittDaRubrica"]);
            }
            //
            //carico i destinatari che mi ritornano da rubrica
            if (Session["selDestDaRubrica"] != null)
            {
                addDestSelDaRubrica((SAAdminTool.DocsPaWR.ElementoRubrica[])Session["selDestDaRubrica"]);
            }
        ...
        private void ibtnMoveToA_Click(object sender, System.Web.UI.ImageClickEventArgs e)
        {
            try
            {
                    ...
                    if (this.txt_codDest.Text.Trim() != "" || this.txt_codDest.Text.Trim() != string.Empty)
                {
                    ...
                    SAAdminTool.UserManager.setQueryRubricaCaller(ref qco);
                    ...
                    {
                        case "T":
                            qco.calltype = SAAdminTool.DocsPaWR.RubricaCallType.CALLTYPE_MODELLI_TRASM_ALL;
                            break;
                        case "I":
                            qco.calltype = SAAdminTool.DocsPaWR.RubricaCallType.CALLTYPE_MODELLI_TRASM_INF;
                            break;
                        case "S":
                            qco.calltype = SAAdminTool.DocsPaWR.RubricaCallType.CALLTYPE_MODELLI_TRASM_SUP;
                            break;
                        case "P":
                            qco.calltype = SAAdminTool.DocsPaWR.RubricaCallType.CALLTYPE_MODELLI_TRASM_PARILIVELLO;
                            break;
                    }
        ...
        private void ImpostaDestinatario(SAAdminTool.DocsPaWR.ElementoRubrica[] corrSearch, SAAdminTool.DocsPaWR.ParametriRicercaRubrica prr)
        {
            ...
            if (corrSearch[0].tipo.Equals("L"))
            {
                if (System.Configuration.ConfigurationManager.AppSettings["LISTE_DISTRIBUZIONE"] != null && System.Configuration.ConfigurationManager.AppSettings["LISTE_DISTRIBUZIONE"] == "1")
                {
                    ...
                    if (listaCorr != null && listaCorr.Count > 0)
                    {
                        ...
                        {
                            DocsPaWR.ElementoRubrica er_1 = new SAAdminTool.DocsPaWR.ElementoRubrica();
                            DocsPaWR.Corrispondente c = (SAAdminTool.DocsPaWR.Corrispondente)listaCorr[i];
                        ...
                        SAAdminTool.DocsPaWR.ElementoRubrica[] elRub = new DocsPaWR.ElementoRubrica[nuovaListaCorr.Count];
                        for (int a = 0; a < nuovaListaCorr.Count; a++)
                        {
                            DocsPaWR.ElementoRubrica er_1 = new SAAdminTool.DocsPaWR.ElementoRubrica();
                            DocsPaWR.Corrispondente c = (SAAdminTool.DocsPaWR.Corrispondente)nuovaListaCorr[a];
                            er_1 = UserManager.getElementoRubrica(this, c.codiceRubrica);
                            elRub[a] = er_1;
                        }
        ...
        private void addDestSelDaRubrica(SAAdminTool.DocsPaWR.ElementoRubrica[] selDestDaRubrica)
		{
			if(selDestDaRubrica!=null)
			{
				...
				{
					DocsPaWR.ElementoRubrica el		= (SAAdminTool.DocsPaWR.ElementoRubrica) selDestDaRubrica[i];
                    if (el.tipo.Equals("L"))
                    {
                        string idAmm = UserManager.getInfoUtente().idAmministrazione;                        
                        ...
                        {
                            DocsPaWR.MittDest destinatario = new SAAdminTool.DocsPaWR.MittDest();
                    ...
                    {
                        DocsPaWR.Corrispondente corr = UserManager.getCorrispondenteByCodRubricaIE(this, el.codice, el.interno ? SAAdminTool.DocsPaWR.AddressbookTipoUtente.INTERNO : SAAdminTool.DocsPaWR.AddressbookTipoUtente.ESTERNO);
		...
		public void CaricaRagioni(string idAmm,bool all)
		{
			try
			{
				...
				if (listaRagioni!=null && listaRagioni.Length>0)
				{
                    ...
                    DocsPaWR.RagioneTrasmissione ragioneSel = new SAAdminTool.DocsPaWR.RagioneTrasmissione();
                ...
                for (int i = 0; i < ddl_registri.Items.Count; i++)
                {
                    if (ddl_registri.Items[i].Value == modello.ID_REGISTRO)
                    {
                        ...
                        if (Session["userRegistro"] != null)
                            reg = (SAAdminTool.DocsPaWR.Registro)Session["userRegistro"];
                        reg.systemId = this.ddl_registri.SelectedValue.ToString();
                        reg.codRegistro = this.ddl_registri.SelectedItem.Text.ToString();
                        Session.Add("userRegistro", reg);
                    }
		...
		public void caricaDataGridMitt()
		{
			...
			if(modello.MITTENTE != null)
			{
                for (int j = 0; j < modello.MITTENTE.Length; j++)
                {
				    ...
				    //row[0] = corr.systemId;
                        row[1] = ((SAAdminTool.DocsPaWR.MittDest)modello.MITTENTE[j]).DESCRIZIONE;
                        row[2] = ((SAAdminTool.DocsPaWR.MittDest)modello.MITTENTE[j]).VAR_COD_RUBRICA;
			...
			if(dt_dest!=null && dt_dest.Items!=null && dt_dest.Items.Count>0)
			{
                if (modello != null && modello.RAGIONI_DESTINATARI != null)
                {
                    ...
                    for (int i = 0; i < modello.RAGIONI_DESTINATARI.Length; i++)
                    {
                        for (int j = 0; j < modello.RAGIONI_DESTINATARI[i].DESTINATARI.Length; j++)
                        {
                            ((SAAdminTool.DocsPaWR.MittDest)modello.RAGIONI_DESTINATARI[i].DESTINATARI[j]).CHA_TIPO_TRASM = ((DropDownList)dt_dest.Items[n].Cells[5].Controls[1]).SelectedValue;
                            ((SAAdminTool.DocsPaWR.MittDest)modello.RAGIONI_DESTINATARI[i].DESTINATARI[j]).VAR_NOTE_SING = ((TextBox)dt_dest.Items[n].Cells[6].Controls[1]).Text;
                            if (((TextBox)dt_dest.Items[n].Cells[7].Controls[3]).Text != "")
                                ((SAAdminTool.DocsPaWR.MittDest)modello.RAGIONI_DESTINATARI[i].DESTINATARI[j]).SCADENZA = Convert.ToInt32(((TextBox)dt_dest.Items[n].Cells[7].Controls[3]).Text);
                            else
                                ((SAAdminTool.DocsPaWR.MittDest)modello.RAGIONI_DESTINATARI[i].DESTINATARI[j]).SCADENZA = 0;
                            ...
                            if (chkNascondiVersioniPrecedentiDocumento != null)
                                ((SAAdminTool.DocsPaWR.MittDest)modello.RAGIONI_DESTINATARI[i].DESTINATARI[j]).NASCONDI_VERSIONI_PRECEDENTI = chkNascondiVersioniPrecedentiDocumento.Checked;
                            else
                                ((SAAdminTool.DocsPaWR.MittDest)modello.RAGIONI_DESTINATARI[i].DESTINATARI[j]).NASCONDI_VERSIONI_PRECEDENTI = false;
		...
		private void AggiornaModello()
		{
			...
			DocsPaWR.Registro reg =(SAAdminTool.DocsPaWR.Registro) Session["userRegistro"];
			reg.systemId = this.ddl_registri.SelectedValue.ToString();
			reg.codRegistro = this.ddl_registri.SelectedItem.Text.ToString();
			Session.Add("userRegistro",reg);
		}

		private void addMittSelDaRubrica(SAAdminTool.DocsPaWR.ElementoRubrica[] selMittDaRubrica)
		{
			if(selMittDaRubrica!= null)
			{
                //((SAAdminTool.DocsPaWR.ModelloTrasmissione)Session["Modello"]).MITTENTE = new SAAdminTool.DocsPaWR.MittDest[selMittDaRubrica.Length];
                ArrayList mittenti = new ArrayList();
                for (int i = 0; i < selMittDaRubrica.Length; i++)
                {
                    DocsPaWR.ElementoRubrica el = (SAAdminTool.DocsPaWR.ElementoRubrica)selMittDaRubrica[i];
				
				    DocsPaWR.Corrispondente corr	= UserManager.getCorrispondenteByCodRubricaIE(this,el.codice,el.interno ? SAAdminTool.DocsPaWR.AddressbookTipoUtente.INTERNO : SAAdminTool.DocsPaWR.AddressbookTipoUtente.ESTERNO);
				
				    DocsPaWR.Ruolo rl = (SAAdminTool.DocsPaWR.Ruolo)Session["userRuolo"];
				    rl.systemId = ((SAAdminTool.DocsPaWR.Ruolo)corr).systemId;
				    rl.uo = new SAAdminTool.DocsPaWR.UnitaOrganizzativa();
				    rl.uo.codiceRubrica = ((SAAdminTool.DocsPaWR.Ruolo)corr).uo.codiceRubrica;
				    Session.Add("userRuolo",rl);

				    //inserisco le informazione del mittente 
				    DocsPaWR.MittDest mittente = new SAAdminTool.DocsPaWR.MittDest();
                    ...
                    //((SAAdminTool.DocsPaWR.ModelloTrasmissione)Session["Modello"]).MITTENTE[i] = mittente;
				    ((SAAdminTool.DocsPaWR.ModelloTrasmissione) Session["Modello"]).SINGLE ="0";
		...
		private void AddMittDest(string ragione,ArrayList destinatari,string cha_tipo_ragione)
		{
			...
			if(((SAAdminTool.DocsPaWR.ModelloTrasmissione) Session["Modello"]).RAGIONI_DESTINATARI!=null)
			{
				//controllo se esiste per la ragione
				DocsPaWR.RagioneDest rd = null;
				for(int i=0;i<modello.RAGIONI_DESTINATARI.Length;i++)
				{
					if(((SAAdminTool.DocsPaWR.RagioneDest)modello.RAGIONI_DESTINATARI[i]).RAGIONE.Equals(ragione))
					{
						rd = (SAAdminTool.DocsPaWR.RagioneDest)modello.RAGIONI_DESTINATARI[i];
						break;
					}
				}
					...
					{						
						...
						rd.DESTINATARI = new SAAdminTool.DocsPaWR.MittDest[array_1.Count];
						array_1.CopyTo(rd.DESTINATARI); 
					}
					else
					{
						DocsPaWR.RagioneDest rd1 = new SAAdminTool.DocsPaWR.RagioneDest();
						rd1.RAGIONE = ragione;
						rd1.CHA_TIPO_RAGIONE = cha_tipo_ragione;
						array_1 = new ArrayList();
						array_1.AddRange(destinatari);
						rd1.DESTINATARI = new SAAdminTool.DocsPaWR.MittDest[array_1.Count];
						array_1.CopyTo(rd1.DESTINATARI);
						
						ArrayList array_2;
						if(((SAAdminTool.DocsPaWR.ModelloTrasmissione) Session["Modello"]).RAGIONI_DESTINATARI != null)
							array_2 = new ArrayList( ((SAAdminTool.DocsPaWR.ModelloTrasmissione) Session["Modello"]).RAGIONI_DESTINATARI );
						else
							array_2 = new ArrayList();

						array_2.Add(rd1);
						((SAAdminTool.DocsPaWR.ModelloTrasmissione) Session["Modello"]).RAGIONI_DESTINATARI = new SAAdminTool.DocsPaWR.RagioneDest[array_2.Count];
						array_2.CopyTo( ((SAAdminTool.DocsPaWR.ModelloTrasmissione) Session["Modello"]).RAGIONI_DESTINATARI );
					}				
		...
		private void verificaEsistenzaDest(ArrayList destVecchi, ref ArrayList destNuovi)
		{
			...
			for(int i=0; i<destNuovi.Count; i++)
			{
				...
				{
                    if (((SAAdminTool.DocsPaWR.MittDest)destNuovi[i]).VAR_COD_RUBRICA == ((SAAdminTool.DocsPaWR.MittDest)destVecchi[j]).VAR_COD_RUBRICA && ((SAAdminTool.DocsPaWR.MittDest)destNuovi[i]).CHA_TIPO_MITT_DEST == "D")
                    {
                        result = false;
                    }
                    else 
                    {
                        if (((SAAdminTool.DocsPaWR.MittDest)destNuovi[i]).CHA_TIPO_MITT_DEST != "D" && ((SAAdminTool.DocsPaWR.MittDest)destNuovi[i]).CHA_TIPO_MITT_DEST == ((SAAdminTool.DocsPaWR.MittDest)destVecchi[j]).CHA_TIPO_MITT_DEST)
                        {
                            result = false;
                        }
                    }
		...
		private void rb_tipo_mittente_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			{
				this.btn_Rubrica_mitt.Visible=true;
				CaricaRagioni(((SAAdminTool.DocsPaWR.Registro) Session["userRegistro"]).idAmministrazione.ToString(),false);
			}
			else
			{
				this.btn_Rubrica_mitt.Visible=false;
				CaricaRagioni(((SAAdminTool.DocsPaWR.Registro) Session["userRegistro"]).idAmministrazione.ToString(),true);					
			}
		}
		...
		private void ddl_registri_SelectedIndexChanged(object sender, System.EventArgs e)
		{
            ...
            {
                SAAdminTool.DocsPaWR.Registro registro = wws.GetRegistroBySistemId(this.ddl_registri.SelectedValue);
                if (registro.Sospeso)
                {
		...
		private void btn_salvaModello_Click(object sender, System.EventArgs e)
		{
			...
			else
			{
                ...
                //bool isModelloUnico = wws.isUniqueCodModelloTrasm((SAAdminTool.DocsPaWR.ModelloTrasmissione)Session["Modello"]);
                //if (isModelloUnico)
                //{
                    AggiornaRagioneDest();
                    ...
                    else
                    {
                        ...
                        //wws.salvaModello((SAAdminTool.DocsPaWR.ModelloTrasmissione)Session["Modello"], UserManager.getInfoUtente());
        ...
        private void PerformSaveModel()
        {
            ...
            if (continua)
                {
                    string codModello = wws.salvaModello((SAAdminTool.DocsPaWR.ModelloTrasmissione)Session["Modello"], UserManager.getInfoUtente());
		...
		private void dt_listaModelli_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			if(modelloTrasmSel.SINGLE.Equals("0"))
			{
                try
                {
                    ...
                    //DocsPaWR.Ruolo rl = (SAAdminTool.DocsPaWR.Ruolo)Session["userRuolo"];
                    //if (rl != null && !string.IsNullOrEmpty(((SAAdminTool.DocsPaWR.Ruolo)corr).systemId))
                    //{
                    //    rl.systemId = ((SAAdminTool.DocsPaWR.Ruolo)corr).systemId;
                    //    rl.uo = new SAAdminTool.DocsPaWR.UnitaOrganizzativa();
                    //    rl.uo.codiceRubrica = ((SAAdminTool.DocsPaWR.Ruolo)corr).uo.codiceRubrica;
		...
		private void CancellaDestinatario(string idragione,string var_cod_rubrica)
		{
            ...
            if (modello != null && modello.RAGIONI_DESTINATARI != null)
            {
                for (int i = 0; i < modello.RAGIONI_DESTINATARI.Length; i++)
                {
                    ...
                    for (int j = 0; j < modello.RAGIONI_DESTINATARI[i].DESTINATARI.Length; j++)
                    {
                        DocsPaWR.MittDest dest = ((SAAdminTool.DocsPaWR.MittDest)modello.RAGIONI_DESTINATARI[i].DESTINATARI[j]);
                        ...
                        if (dest.ID_RAGIONE.ToString().Equals(idragione) && dest.VAR_COD_RUBRICA.Equals(var_cod_rubrica.Trim()))
                        {
                            if (rd.DESTINATARI.Length > 1)
                            {
                                ...
                                appoggio.RemoveAt(j);
                                rd.DESTINATARI = new SAAdminTool.DocsPaWR.MittDest[appoggio.Count];
                            ...
                            else
                            {
                                ...
                                appoggio.RemoveAt(i);
                                modello.RAGIONI_DESTINATARI = new SAAdminTool.DocsPaWR.RagioneDest[appoggio.Count];
        ...
        private string checkRagTrasmConCessioneDuplicate()
        {
            ...
            if (modello != null && modello.RAGIONI_DESTINATARI != null)
            {
                foreach (DocsPaWR.RagioneDest ragDest in modello.RAGIONI_DESTINATARI)
                {
                    ...
                    if (ragione != null)
                    {
                        if (!ragione.PrevedeCessione.Equals(SAAdminTool.DocsPaWR.CedeDiritiEnum.No))
                        {
        ...
        private bool unicaUOinModello()
        {
            ...
            if (modello != null && modello.RAGIONI_DESTINATARI != null)
            {
                foreach (SAAdminTool.DocsPaWR.RagioneDest ragioneDest in modello.RAGIONI_DESTINATARI)
                    foreach (SAAdminTool.DocsPaWR.MittDest mDest in ragioneDest.DESTINATARI)
        ...
        private void addMitt(ArrayList mittenti)
        {
            ...
            //se già ci sono dei destinatari li mantengo aggiungendo quelli nuovi selezionati da rubrica
                ...
                if (modello.MITTENTE!=null && modello.MITTENTE.Length>0)
                {
                    ...
                    modello.MITTENTE = new SAAdminTool.DocsPaWR.MittDest[array_1.Count];
                    array_1.CopyTo(modello.MITTENTE);
                }
                else
                {
                    modello.MITTENTE = new SAAdminTool.DocsPaWR.MittDest[mittenti.Count];
        ...
        private void verificaEsistenzaMitt(ArrayList mittVecchi, ref ArrayList mittNuovi)
        {
            ...
            for (int i = 0; i < mittNuovi.Count; i++)
            {
                ...
                {
                    if (((SAAdminTool.DocsPaWR.MittDest)mittNuovi[i]).VAR_COD_RUBRICA == ((SAAdminTool.DocsPaWR.MittDest)mittVecchi[j]).VAR_COD_RUBRICA && ((SAAdminTool.DocsPaWR.MittDest)mittNuovi[i]).CHA_TIPO_MITT_DEST == "M")
                    {
                        result = false;
                    }                    
                }

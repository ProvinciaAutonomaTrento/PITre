...
using SAAdminTool.AdminTool.Gestione_Organigramma;
using SAAdminTool;
...
namespace Amministrazione.Gestione_Organigramma
{
    ...
    public class Organigramma : System.Web.UI.Page
    {
        ...
        protected SAAdminTool.AdminTool.UserControl.CalculateAtipicitaOptions calculateAtipicitaOptions;
        ...
        private void txtCodUo_TextChanged(object sender, System.EventArgs e)
        {
            try
            {
                ...
                {
                    foreach (SAAdminTool.DocsPaWR.OrgRisultatoRicerca risultato in theManager.getRisultatoRicerca())
                    {
            ...
            try
            {
                ...
                if (lista != null && lista.Count > 0)
                {
                    foreach (SAAdminTool.DocsPaWR.OrgRuolo ruolo in lista)
                    {
                        ...
                        SAAdminTool.DocsPaWR.OrgTipoRuolo[] elencoTipiRuolo = (SAAdminTool.DocsPaWR.OrgTipoRuolo[])theManager.getListaTipiRuolo().ToArray(typeof(SAAdminTool.DocsPaWR.OrgTipoRuolo));
                        SAAdminTool.DocsPaWR.OrgTipoRuolo tipoRuolo = elencoTipiRuolo.Where(oggetto => oggetto.IDTipoRuolo.ToUpper().Equals(ruolo.IDTipoRuolo.ToUpper())).FirstOrDefault();
            ...
            try
            {
                ...
                switch (TreeNodo.getTipoNodo())
                {
                    ...
                    case "R": // Ruolo
                        ...
                        SAAdminTool.DocsPaWR.OrgTipoRuolo[] elencoTipiRuolo = (SAAdminTool.DocsPaWR.OrgTipoRuolo[])theManager.getListaTipiRuolo().ToArray(typeof(SAAdminTool.DocsPaWR.OrgTipoRuolo));
                        SAAdminTool.DocsPaWR.OrgTipoRuolo tipoRuolo = elencoTipiRuolo.Where(oggetto => oggetto.IDTipoRuolo.ToUpper().Equals(TreeNodo.getIDTipoRuolo().ToUpper())).FirstOrDefault();
        ...
        private void DettagliUOStampaBuste(string idCorrGlob)
        {
            ...
            {
                SAAdminTool.DocsPaWR.OrgDettagliGlobali dettagli = theManager.getDatiUOStampaBuste();
        ...
        private void DettagliRuolo(myTreeNode TreeNode)
        {
            try
            {
                ...
                this.btnShowRoleHistory.Attributes["onclick"] = SAAdminTool.popup.RoleHistory.GetScriptToOpenWindow(TreeNode.getIDCorrGlobale(), String.Empty);
        ...
        private void DettagliFunzioni(myTreeNode TreeNode)
        {
            try
            {
                ...
                if (theManager.getListaFunzioni().Count > 0)
                {
                    ...
                    DataRow row;
                    foreach (SAAdminTool.DocsPaWR.OrgTipoFunzione funzione in theManager.getListaFunzioni())
                    {
        ...
        private void DettagliUtente(myTreeNode TreeNode)
        {
            try
            {
                ...
                SAAdminTool.DocsPaWR.OrgUtente utente = theManager.getDatiUtente();
                ...
                //gestione qualifiche
                string chiaveQualifiche = SAAdminTool.utils.InitConfigurationKeys.GetValue("0", "GESTIONE_QUALIFICHE");
                ...
                string chiaveApplicazioni = SAAdminTool.utils.InitConfigurationKeys.GetValue("0", "ENABLE_GEST_EXT_APPS");
        ...
        private void CaricaComboBox_TipoRuolo(string idTipoRuolo, string idAmm)
        {
            try
            {
                ...
                {
                    foreach (SAAdminTool.DocsPaWR.OrgTipoRuolo tipoRuolo in theManager.getListaTipiRuolo())
                    {
        ...
        private void btn_sposta_ruolo_Click(object sender, System.EventArgs e)
        {
            ...
            if (theManager.getRisultatoRicerca() != null && theManager.getRisultatoRicerca().Count.Equals(1))
            {
                foreach (SAAdminTool.DocsPaWR.OrgRisultatoRicerca risultato in theManager.getRisultatoRicerca())
                {
                    ...
                    SAAdminTool.Utils.DefaultButton(this, ref txt_codNewRuolo, ref btn_sposta);
                    SAAdminTool.Utils.DefaultButton(this, ref txt_descNewRuolo, ref btn_sposta);
                }
            }*/
        ...
        private void InserimentoUO()
        {
            try
            {
                if (this.txt_rubricaUO.Text.Trim() != "" && this.txt_descrizioneUO.Text.Trim() != "")
                {
                    ...
                    // dettagli UO
                    SAAdminTool.DocsPaWR.OrgDettagliGlobali dettagli = new SAAdminTool.DocsPaWR.OrgDettagliGlobali();
            ...
            try
            {
                ...
                if (Session["interopUOChanged"] != null && (bool)Session["interopUOChanged"])
                {
                    if (Session["UOinSessione"] != null)
                    {
                        ...
                        SAAdminTool.DocsPaWR.OrgDettagliGlobali dettagli = (SAAdminTool.DocsPaWR.OrgDettagliGlobali)Session["UOinSessione"];
                        if (
                            (this.txt_uo_indirizzo.Text != dettagli.Indirizzo) ||
                            (this.txt_uo_citta.Text != dettagli.Citta) ||
                            (this.txt_uo_cap.Text != dettagli.Cap) ||
                ...
                if (!bloccaesecuzione)
                {
                    if (this.txt_rubricaUO.Text.Trim() != "" && this.txt_descrizioneUO.Text.Trim() != "")
                    {
                        ...
                        TreeNodo = (myTreeNode)treeViewUO.GetNodeFromIndex(treeViewUO.SelectedNodeIndex);
                        Manager.OrganigrammaManager theManager = new Amministrazione.Manager.OrganigrammaManager();
                        SAAdminTool.DocsPaWR.OrgUO theUO = new SAAdminTool.DocsPaWR.OrgUO();
                        ...
                        {   
                            ...
                            if (this.GestioneRubricaComuneAbilitata)
                            {
                                ...
                                RubricaComune.Proxy.Elementi.ElementoRubrica elemento = SAAdminTool.RubricaComune.RubricaServices.GetElementoRubricaUO(sessionMng.getUserAmmSession(), theUO.IDCorrGlobale);
                                if (elemento != null)
                                {
                        ...
                        switch (tipoAzione)
                        {
                            ...
                            case "cancellazione":
                                ...
                                else
                                {
                                    ...
                                    SAAdminTool.DocsPaWR.EsitoOperazione res = new SAAdminTool.DocsPaWR.EsitoOperazione();
                                    ...
                                    else theManager.EliminaRuolo(ruolo);
                                }
                                break;
                        }
                        ...
                        switch (tipoAzione)
                        {
                            ...
                            case "modifica":
                                ...
                                else
                                {
                                    ...
                                    if (esito.Codice.Equals(4))
                                    {
                                        Ruolo r = SAAdminTool.UserManager.getRuoloById(ruolo.IDCorrGlobale, this);
                                        if (r.disabledTrasm)
                                        {
                                            TreeNodo.setDisabledTrasm("1");
                            ...
                            case "cancellazione":
                                if (esito.Codice.Equals(0) || esito.Codice.Equals(1) || esito.Codice.Equals(2) || esito.Codice.Equals(9))
                                {
                                    ...
                                    if (dsListe != null)
                                    {
                                        ...
                                        {
                                            messaggio += "\\nAttenzione, ruolo coinvolto nei seguenti processi di firma\\n";
                                            foreach (ProcessoFirma processo in listProcessoFirma)
                                            {
                                                messaggio += processo.nome.ToUpper() + " creato dal ruolo " + processo.idRuoloAutore.ToUpper() + "\\n";
                                            }
                                        }
        ...
        private void InserimentoTipoFunzioni()
        {
            try
            {
                if (this.dg_funzioni.Items.Count > 0)
                {
                    ...
                    SAAdminTool.DocsPaWR.OrgTipoFunzione funzione = null;
                    ...
                    for (int i = 0; i < this.dg_funzioni.Items.Count; i++)
                    {
                        ...
                        if (spunta.Checked)
                        {
                            funzione = new SAAdminTool.DocsPaWR.OrgTipoFunzione();
                    ...
                    {
                        SAAdminTool.DocsPaWR.OrgTipoFunzione[] funzioni = new SAAdminTool.DocsPaWR.OrgTipoFunzione[listaFunzioniSelezionate.Count];
        ...
        protected void FetchDatiRubricaComune(string idUO)
        {
            if (this.pnlRubricaComune.Visible)
            {
                ...
                RubricaComune.Proxy.Elementi.ElementoRubrica elemento = SAAdminTool.RubricaComune.RubricaServices.GetElementoRubricaUO(sessionMng.getUserAmmSession(), idUO);
        ...
        private SAAdminTool.DocsPaWR.ValidationResultInfo SetVisCaselleRegistri()
        {
            ...
            SAAdminTool.DocsPaWR.ValidationResultInfo result = MultiCasellaManager.DeletelRightMailRegistro(string.Empty, treeNode.getIDCorrGlobale(), string.Empty);
            if (result.Value)
            {

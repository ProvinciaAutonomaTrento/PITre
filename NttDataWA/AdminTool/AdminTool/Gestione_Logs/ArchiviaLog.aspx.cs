...
namespace Amministrazione.Gestione_Logs
{
	...
	/// </summary>
	public class ArchiviaLog : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{				
				...
				if(this.VerificaArchivioLogPath())
				{
				...
				{
					//this.lbl_tit.Text = "";
					this.lbl_archivio.Text = "ATTENZIONE!<br>non esiste una directory dove archiviare i file .pdf<br><br>Verificare la chiave ARCHIVIO_LOG_PATH sul web.config del WS.<br><br>";
        ...
        private void ArchiviaLog_PreRender(object sender, System.EventArgs e)
        {
		...
		private bool VerificaArchivioLogPath()
		{
			bool retValue = false;
			AmmUtils.WebServiceLink ws = new AmmUtils.WebServiceLink();
			string result = ws.VerificaArchivioLogPath();
        ...
        private void modificaNum_Click(object sender, System.EventArgs e)
        {
            if (this.permesso.Value == "ok")
            {
                if (this.chk_avviso.Checked)
                {
                    ...
                    {
                        if (!SAAdminTool.FileManager.modifyRecordNumber(idAmm, '1', Convert.ToInt16(this.txt_num_rec.Text)))
                        {
                ...
                {
                    if (!SAAdminTool.FileManager.modifyRecordNumber(idAmm, '0', 0))
                    {
                        ClientScript.RegisterStartupScript(this.GetType(), "modificaErrata", "alert('Modifica non effettuata correttamente.');", true);
                    }
                }
		...
		{    		
            ...
            this.PreRender += new EventHandler(ArchiviaLog_PreRender);
		...
		{
			AmmUtils.WebServiceLink ws = new AmmUtils.WebServiceLink();
            bool esito = ws.stampaLog(AmmUtils.UtilsXml.GetAmmDataSession((string)Session["AMMDATASET"], "0"), "");
			if(esito)
			{			

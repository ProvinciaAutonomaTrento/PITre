<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="DigitalSignWrapper.ascx.cs" Inherits="SAAdminTool.SmartClient.DigitalSignWrapper" %>

<form name="frm">
        <OBJECT id="digitalSignServicesControl" height="0" width="0" 
                data="data:application/x-oleobject;base64,"
                classid="http:../SmartClient/librerie/DPA.Web.dll#DPA.Web.DigitalSignature.DigitalSignServicesControl"
                VIEWASTEXT>			
        </OBJECT>    
</form>

<script type="text/javascript">
    function CapicomWrapper_GetCertificateList(storeLocation, storeName) {
        try 
        {
            if (storeLocation == 2) {
                storeLocation = 1;
            }
            var retValue = new Array();

            var ctrl = document.getElementById("digitalSignServicesControl");
            ctrl.CheckCertificate = false;

            var list = ctrl.GetCertificateListAsJsonFormat(storeLocation, storeName);
            var jsonList = eval(list);

            for (var i = 0; i < jsonList.length; i++) {
                retValue[i] = jsonList[i];
            }
        
            return retValue;
	    }
	    catch(e)
	    {
	        alert("Errore nel reperimento della lista dei certificati residenti nello store '" + storeName + "'\n" + e);
	    }
    }

    function CapicomWrapper_SignData(content, certIndex, applyCoSign, storeLocation, storeName) {
        var signedData = null;
        certIndex = certIndex - 1;
        try
        {
            var ctrl = document.getElementById("digitalSignServicesControl");
            if (storeLocation == 2) {
                storeLocation = 1;
            }
            signedData = ctrl.SignData(content, certIndex, applyCoSign, storeLocation, storeName);
	    }
	    catch(e)
	    {
	        alert("Errore: " + e.message.toString());
	    } 
	
	    return signedData;
	}

	function CapicomWrapper_SignHash(sha256Hash, certIndex, applyCoSign, storeLocation, storeName) {
	    var signedData = null;

	    certIndex = certIndex - 1;
	    try {
	        var ctrl = document.getElementById("digitalSignServicesControl");
	        if (storeLocation == 2) {
	            storeLocation = 1;
	        }
	        signedData = ctrl.SignHash(sha256Hash, certIndex, applyCoSign, storeLocation, storeName);
	    }
	    catch (e) {
	        alert("Errore: " + e.message.toString());
	    }

	    return signedData;
	}


</script>

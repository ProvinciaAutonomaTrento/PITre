...
namespace SAAdminTool.popup.RubricaDocsPA 
{
	...
	public class CorrDataGrid : DataGrid, IPostBackDataHandler
	{
		...
		private SAAdminTool.DocsPaWR.InfoUtente _infoUtente;
		...
		/// InfoUtente 
		/// </summary>
		public SAAdminTool.DocsPaWR.InfoUtente InfoUtente
		{
		...
		public override object DataSource
		{
			...
			set
			{
				if (!(value is SAAdminTool.DocsPaWR.ElementoRubrica[])) 
				{
		...
		private string build_pager()
		{
			...
			int pstart = 0, pend = 0;
			if (this.DataSource is SAAdminTool.DocsPaWR.ElementoRubrica[] && 
				((SAAdminTool.DocsPaWR.ElementoRubrica[]) this.DataSource).Length > 0) 
			{
		...
		private void CorrDataGrid_ItemCreated(object sender, DataGridItemEventArgs e)
		{
			...
			if (e.Item.ItemType == ListItemType.Footer) 
			{
				...
				_l_show_remove_all = _l_show_remove_all && 
					(DataSource is SAAdminTool.DocsPaWR.ElementoRubrica[]) &&
					(((SAAdminTool.DocsPaWR.ElementoRubrica[])DataSource).Length > 0);
		...
		public void RaisePostDataChangedEvent()
		{
			...
			switch ((string) event_data.Key) 
			{
				case CorrDataGrid.EVT_OPENSUBITEMS:
					if (event_data.Value is SAAdminTool.DocsPaWR.ElementoRubrica && OpenSubItems != null) 
					{
						DocsPaWR.ElementoRubrica er = (SAAdminTool.DocsPaWR.ElementoRubrica) event_data.Value;
		...
		public bool IsVisibleCheckOrRadio(SAAdminTool.DocsPaWR.ElementoRubrica er) 
		{
			if (er != null)
		...
		private void handle_paging()
		{
			if (this.DataSource is SAAdminTool.DocsPaWR.ElementoRubrica[])
			{
				...
				DocsPaWR.ElementoRubrica[] viewable_items = new SAAdminTool.DocsPaWR.ElementoRubrica[this.PageSize];
				DocsPaWR.ElementoRubrica[] all_items = (SAAdminTool.DocsPaWR.ElementoRubrica[]) this.DataSource;
		...
		private void check_children_existence (ref SAAdminTool.DocsPaWR.ElementoRubrica[] items)
		{
		...
		public void AddHierarchyElement (SAAdminTool.DocsPaWR.ElementoRubrica[] elements, string ParentId, string ParentText, string ParentType, bool ParentIsInterno)
		{
		...
		public SAAdminTool.DocsPaWR.ElementoRubrica[] GetSelected()
		{
            SAAdminTool.DocsPaWR.ElementoRubrica[] elementi;
            SAAdminTool.DocsPaWR.ElementoRubrica el = null;
			...
			if (s_codici != "") 
			{
                ...
                if (codici.Length == 1)
                {
                    ...
                    if (el == null)
                    {
                        appoggioElemSingolo = UserManager.getElementiRubricaRange(codici, SAAdminTool.DocsPaWR.AddressbookTipoUtente.GLOBALE, this.Page);
                    ...
                    elementi = new SAAdminTool.DocsPaWR.ElementoRubrica[1];
                    elementi[0] = el;
                    return elementi;
                }
                else if (codici.Length > 0)
                {
                    elementi = new SAAdminTool.DocsPaWR.ElementoRubrica[codici.Length];
                    ...
                    for (int i = 0; i < codici.Length; i++)
                    {
                        ...
                        if (el == null)
                        {
                            ...
                            appoggioElem = UserManager.getElementiRubricaRange(codiciRR, SAAdminTool.DocsPaWR.AddressbookTipoUtente.GLOBALE, this.Page);
                ...
                //return UserManager.getElementiRubricaRange (codici, SAAdminTool.DocsPaWR.AddressbookTipoUtente.GLOBALE, this.Page);
	...
	public class CorrDescTemplate : ITemplate
	{
		...
		public void OnDataBinding (object sender, EventArgs e)
		{
			...
			DocsPaWR.ElementoRubrica er = (SAAdminTool.DocsPaWR.ElementoRubrica) container.DataItem;
            if (er != null)
            {
...
#if USE_HIERARCHY_CACHE
					string key = (er.interno ? @"E\" : @"I\") + er.codice;
					DocsPaWR.ElementoRubrica[] ers = (SAAdminTool.DocsPaWR.ElementoRubrica[]) dg.hierarchy_cache[key];
#else
                    DocsPaWR.ElementoRubrica[] ers = UserManager.getGerarchiaElemento(er.codice,
                        er.interno ? SAAdminTool.DocsPaWR.AddressbookTipoUtente.INTERNO : SAAdminTool.DocsPaWR.AddressbookTipoUtente.ESTERNO,
                        dg.Page);
#endif

                    foreach (SAAdminTool.DocsPaWR.ElementoRubrica ee in ers)
	...
	public class CorrSelectTemplate : ITemplate
	{
		...
		public void OnDataBinding (object sender, EventArgs e)
		{
			...
			//dell'utente loggato, devono essere comunque selezionabili - PER ANAS
                ...
                //if(dg.ShowSelectors && SAAdminTool.Utils.getAbilitazioneSmistamento().Equals("1") &&

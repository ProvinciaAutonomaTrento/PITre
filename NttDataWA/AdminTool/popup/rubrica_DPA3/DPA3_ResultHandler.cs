...
namespace SAAdminTool.popup.RubricaDocsPA
{
	public class DPA3_ResultHandler : IResultHandler
	{
		...
		public void execute(ElementoRubrica[] _A_recipients, ElementoRubrica[] _CC_recipients, SAAdminTool.DocsPaWR.RubricaCallType calltype)
		{
            ...
            if (CallContextStack.CurrentContext == null || CallContextStack.CurrentContext.ContextState["TransmissionReasons"] != null)
            {
                ...
                trasm.tipoOggetto = SAAdminTool.utils.MassiveOperationUtils.ObjectType;
          ...
          //  }
			...
			switch (calltype) 
			{
				case RubricaCallType.CALLTYPE_PROTO_IN:
					pe = (SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo;
				...
				case RubricaCallType.CALLTYPE_PROTO_OUT:
					pu = (SAAdminTool.DocsPaWR.ProtocolloUscita)schedaDoc.protocollo;
                    SAAdminTool.DocsPaWR.Corrispondente[] dest = pu.destinatari;
                    SAAdminTool.DocsPaWR.Corrispondente[] destCC = pu.destinatariConoscenza;
				...
				case RubricaCallType.CALLTYPE_TRASM_PARILIVELLO:
					...
					foreach (SAAdminTool.DocsPaWR.ElementoRubrica e in _A_recipients) 
					{
						...
						if(e.tipo == "L")
						{
							...
							DocsPaWR.ParametriRicercaRubrica qr = new SAAdminTool.DocsPaWR.ParametriRicercaRubrica();
							...
							{
								case "T":
									qr.calltype = SAAdminTool.DocsPaWR.RubricaCallType.CALLTYPE_TRASM_ALL;							
									break;					
								case "I":
									qr.calltype = SAAdminTool.DocsPaWR.RubricaCallType.CALLTYPE_TRASM_INF;							
									break;					
								case "S":
									qr.calltype = SAAdminTool.DocsPaWR.RubricaCallType.CALLTYPE_TRASM_SUP;							
									break;							
								case "P":
									qr.calltype = SAAdminTool.DocsPaWR.RubricaCallType.CALLTYPE_TRASM_PARILIVELLO;
									break;		
							}
				...
				case RubricaCallType.CALLTYPE_PROTO_OUT_MITT:
                    ...
                    else if (er.isRubricaComune)
                    {
					    ...
					    corr = UserManager.getCorrispondenteByCodRubricaIE (page, er.codice, er.interno ? SAAdminTool.DocsPaWR.AddressbookTipoUtente.INTERNO : SAAdminTool.DocsPaWR.AddressbookTipoUtente.ESTERNO);
					pu.mittente = corr;
					...
					{				
						...
						{
							pu.ufficioReferente = ((SAAdminTool.DocsPaWR.Ruolo)corr).uo;
							gestisci_scheda_protocollo(schedaDoc);
						}
						if(corr.GetType() == typeof(SAAdminTool.DocsPaWR.UnitaOrganizzativa))
						{
							pu.ufficioReferente = corr;
							gestisci_scheda_protocollo(schedaDoc);
						}
						if(corr.GetType() == typeof(SAAdminTool.DocsPaWR.Utente))
						{
							((SAAdminTool.DocsPaWR.Utente)corr).ruoli = UserManager.GetRuoliUtente(page,((SAAdminTool.DocsPaWR.Utente)corr).idPeople);
							((SAAdminTool.DocsPaWR.Utente)corr).ruoli = UserManager.getRuoliFiltrati(((SAAdminTool.DocsPaWR.Utente)corr).ruoli);
							if (((SAAdminTool.DocsPaWR.Utente)corr).ruoli.Length == 0)
							{
							...
							else
							{
								...
								if (((SAAdminTool.DocsPaWR.Utente)corr).ruoli.Length == 1)
								{
									pu.ufficioReferente = ((SAAdminTool.DocsPaWR.Utente)corr).ruoli[0].uo;	
									gestisci_scheda_protocollo(schedaDoc);
								}
								else
								{
				...
				case RubricaCallType.CALLTYPE_PROTO_INT_MITT:
					pi = (SAAdminTool.DocsPaWR.ProtocolloInterno) schedaDoc.protocollo;
						...
						&& ConfigSettings.getKey(ConfigSettings.KeysENUM.ENABLE_UFFICIO_REF).Equals("1"))
					{
						...
						if(corr.GetType() == typeof(SAAdminTool.DocsPaWR.Ruolo))
						{
							pi.ufficioReferente = ((SAAdminTool.DocsPaWR.Ruolo)corr).uo;
							gestisci_scheda_protocollo(schedaDoc);
						}
						if(corr.GetType() == typeof(SAAdminTool.DocsPaWR.UnitaOrganizzativa))
						{
						...
						if(corr.GetType() == typeof(SAAdminTool.DocsPaWR.Utente))
						{
							...
							if (((SAAdminTool.DocsPaWR.Utente)corr).ruoli.Length == 1)
							{
								pi.ufficioReferente = ((SAAdminTool.DocsPaWR.Utente)corr).ruoli[0].uo;	
								gestisci_scheda_protocollo(schedaDoc);
							}
							else
							{
				...
				case RubricaCallType.CALLTYPE_PROTO_INGRESSO:
                    ...
                    if (schedaDoc != null)
					{
                        if ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo != null && !UserManager.esisteCorrispondente(((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenti, corr))
                        {
                ...
                case RubricaCallType.CALLTYPE_PROTO_OUT_MITT_SEMPLIFICATO:
                    ...
                    if (schedaDoc != null)
                    {
                        ...
                        ((SAAdminTool.DocsPaWR.ProtocolloUscita)schedaDoc.protocollo).mittente = corr;
                    }
                ...
                case RubricaCallType.CALLTYPE_PROTO_USCITA_SEMPLIFICATO:
                    ...
                    if (schedaDoc != null && schedaDoc.protocollo!=null)
                    {
                        ...
                        SAAdminTool.DocsPaWR.Corrispondente[] destinatari = ((DocsPaWR.ProtocolloUscita)(schedaDoc.protocollo)).destinatari;
                        SAAdminTool.DocsPaWR.Corrispondente[] destinatariCC = ((DocsPaWR.ProtocolloUscita)(schedaDoc.protocollo)).destinatariConoscenza;
				...
				case RubricaCallType.CALLTYPE_UFFREF_PROTO:
					...
					((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).ufficioReferente = corr;
					gestisci_scheda_protocollo(schedaDoc);
				...
				case RubricaCallType.CALLTYPE_RICERCA_UFFREF:
                    ...
                    else if (er.isRubricaComune)
                    {
                        ...
                        corr = UserManager.getCorrispondenteByCodRubricaIE(page, er.codice, SAAdminTool.DocsPaWR.AddressbookTipoUtente.INTERNO);
				...
				case RubricaCallType.CALLTYPE_PROTO_INT_DEST:
					pi = (SAAdminTool.DocsPaWR.ProtocolloInterno)schedaDoc.protocollo;
                    SAAdminTool.DocsPaWR.Corrispondente[] desti = pi.destinatari;
                    SAAdminTool.DocsPaWR.Corrispondente[] destiCC = pi.destinatariConoscenza;
                ...
                case RubricaCallType.CALLTYPE_LISTE_DISTRIBUZIONE:
                    ...
                    for (int i = 0; i < _A_recipients.Length; i++)
                    {
                        ...
                        {
                            //DocsPaWR.ElementoRubrica es = new SAAdminTool.DocsPaWR.ElementoRubrica();
                ...
                case RubricaCallType.CALLTYPE_RICERCA_CORR_NON_STORICIZZATO:
                    ...
                    else if (er.isRubricaComune)
                    {
                        ...
                        corr = UserManager.getCorrispondenteByCodRubricaIE(page, er.codice, SAAdminTool.DocsPaWR.AddressbookTipoUtente.GLOBALE);
                ...
                case RubricaCallType.CALLTYPE_CORR_EST:
                    ...
                    else if (er.isRubricaComune)
                    {
                        ...
                        corr = UserManager.getCorrispondenteByCodRubricaIE(page, er.codice, SAAdminTool.DocsPaWR.AddressbookTipoUtente.ESTERNO);
                    HttpContext.Current.Session.Add("rubrica.campoCorrispondente", corr);
                    ((RubricaDocsPA)page).closeRubrica();
                    break;
                ...
                case RubricaCallType.CALLTYPE_MITT_MULTIPLI:
                    ...
                    foreach (ElementoRubrica erMittMultiplo in _A_recipients)
                    {
                        if (erMittMultiplo.tipo != "L")
                        {
                            ...
                            if (!UserManager.esisteCorrispondente(((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenti, corrMittMultiplo) &&
                                !UserManager.esisteCorrispondente(new Corrispondente[1] { ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittente }, corrMittMultiplo)
                                )
                            {
                                ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenti = UserManager.addCorrispondente(((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenti, corrMittMultiplo);
                                ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).daAggiornareMittentiMultipli = true;
                            }
                        }
                        if (erMittMultiplo.tipo == "L")
                        {
                            ...
                            {
                                if (!UserManager.esisteCorrispondente(((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenti, co) &&
                                    !UserManager.esisteCorrispondente(new Corrispondente[1] { ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittente }, co)
                                )
                                {
                                    ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenti = UserManager.addCorrispondente(((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).mittenti, co);
                                    ((SAAdminTool.DocsPaWR.ProtocolloEntrata)schedaDoc.protocollo).daAggiornareMittentiMultipli = true;
                                }
                            }
                        }
		...
		private void addDestinatari ( ref SAAdminTool.DocsPaWR.Corrispondente[] corrs, SAAdminTool.DocsPaWR.Corrispondente[] corrsCC, SAAdminTool.DocsPaWR.ElementoRubrica[] ers, SAAdminTool.DocsPaWR.SchedaDocumento doc, bool is_cc)
		{
			if (ers == null || ers.Length == 0)
				return;

			if (corrs == null)
				corrs = new SAAdminTool.DocsPaWR.Corrispondente[0];
			...
			for (int n = 0; n < ers.Length; n++) 
			{
                if(ers[n].tipo == "L")
                {
                    ...
                    corr = UserManager.getCorrispondenteByCodRubricaIE(page, ers[n].codice, SAAdminTool.DocsPaWR.AddressbookTipoUtente.GLOBALE);
                    ...
                    {
                        DocsPaWR.Corrispondente c = (SAAdminTool.DocsPaWR.Corrispondente) corrLista[i];
                            ...
                            if(doc.tipoProto == "I")
                            {
                                if(c.tipoIE == "I")
                                {
                                    ...
                                    {									
                                        ...
                                        {
                                            corrTotali=SAAdminTool.Utils.addToArray(corrTotali, c);	
                                            corrListaInterni++;
                                        }
                                    }
                                }
                ...
                if (ers[n].tipo != "F" && ers[n].tipo != "L" || (ers[n].tipo == "F" && ers[n].isRubricaComune))
                {
                    ...
                    {
                        if (corr.tipoIE == "I")
                            corrTotali = SAAdminTool.Utils.addToArray(corrTotali, corr);
                        else
			...
			DocsPaWR.Corrispondente[] dest = new SAAdminTool.DocsPaWR.Corrispondente[corrTotali.Length + corrs.Length];		
			corrs.CopyTo(dest,0);
		...
		private void gestisci_scheda_protocollo(SAAdminTool.DocsPaWR.SchedaDocumento sd)
		{
		...
		private void gestisci_ric_fasc_lf (SAAdminTool.DocsPaWR.Corrispondente corr)
		{
		...
		private void gestisci_ric_fasc_uffref(SAAdminTool.DocsPaWR.Corrispondente corr, bool fasc_modifica)
		{
        ...
        private bool esisteRagTrasmCessioneInTrasm(SAAdminTool.DocsPaWR.Trasmissione trasmissione)
        {
            bool retValue = false;

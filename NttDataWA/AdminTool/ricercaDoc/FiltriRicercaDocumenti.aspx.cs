...
namespace SAAdminTool.ricercaDoc
{
    ...
    public class FiltriRicercaDocumenti : System.Web.UI.Page
    {
        ...
        protected SAAdminTool.UserControls.Calendar txtInitDataCreazione;
        protected SAAdminTool.UserControls.Calendar txtEndDataCreazione;
        ...
        protected SAAdminTool.UserControls.Calendar txtInitDataProtocollo;
        protected SAAdminTool.UserControls.Calendar txtEndDataProtocollo;
        ...
        SAAdminTool.ricercaDoc.SchedaRicerca sRic = null;
        ...
        private void AddControlsClientAttribute()
        {
            ...
            string use_new_rubrica = SAAdminTool.ConfigSettings.getKey(ConfigSettings.KeysENUM.RUBRICA_V2);
        ...
        private void Page_Load(object sender, System.EventArgs e)
        {
            ...
            sRic = (SAAdminTool.ricercaDoc.SchedaRicerca)Session[SAAdminTool.ricercaDoc.SchedaRicerca.SESSION_KEY];
            ...
            if (sRic == null)
            {
                ...
                //ricerche salvate
                DocsPaWR.Utente utente = (SAAdminTool.DocsPaWR.Utente)Session["userData"];
                DocsPaWR.Ruolo ruolo = (SAAdminTool.DocsPaWR.Ruolo)Session["userRuolo"];
        ...
        {
            DocsPaWR.Templates template = new SAAdminTool.DocsPaWR.Templates();
            if (!ddl_tipoDoc.SelectedValue.Equals(""))
            {
                template = (SAAdminTool.DocsPaWR.Templates)Session["templateRicerca"];
        ...
        private void ddl_tipoDoc_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            sRic.RimuoviFiltro(SAAdminTool.DocsPaWR.FiltriDocumento.PROFILAZIONE_DINAMICA.ToString());
            verificaCampiPersonalizzati();
        }
        #endregion
        ...
        private bool IsValidDate(DocsPaWebCtrlLibrary.DateMask dateMask)
        {
            ...
            if (dateMask.Text.Length > 0)
                retValue = SAAdminTool.Utils.isDate(dateMask.Text);
        ...
        /// </summary>
        /// <returns></returns>
        private SAAdminTool.DocsPaWR.FiltroRicerca[][] GetFilters()
        {
            ...
            if (!GridManager.IsRoleEnabledToUseGrids())
            {
                ...
                {
                    SAAdminTool.DocsPaWR.Templates template = ProfilazioneDocManager.getTemplatePerRicerca((UserManager.getInfoUtente(this)).idAmministrazione, ddl_tipoDoc.SelectedItem.Text, this);
                    if (template != null)
                    {
                        ...
                        FirstOrDefault();
                        SAAdminTool.DocsPaWR.FiltroRicerca fV1;
                        fV1 = new SAAdminTool.DocsPaWR.FiltroRicerca();
            ...
            DocsPaWR.FiltroRicerca[] initArray = new SAAdminTool.DocsPaWR.FiltroRicerca[filterItems.Count];
            filterItems.CopyTo(initArray);
            filterItems = null;

            DocsPaWR.FiltroRicerca[][] retValue = new SAAdminTool.DocsPaWR.FiltroRicerca[1][];
        ...
        private void AddProfilazioneDinamica(ArrayList filterItems)
        {
            if (sRic != null && sRic.FiltriRicerca != null)
            {
                ...
                for (int i = 0; !found && i < sRic.FiltriRicerca[0].Length; i++)
                {
                    ...
                    if (aux != null && aux.argomento == SAAdminTool.DocsPaWR.FiltriDocumento.PROFILAZIONE_DINAMICA.ToString())
                        found = true;
                    else aux = null;
                }
        ...
        public sealed class CurrentFilterSessionStorage
        {
            ...
            public static SAAdminTool.DocsPaWR.FiltroRicerca[][] GetCurrentFilter()
            {
                return (SAAdminTool.DocsPaWR.FiltroRicerca[][])HttpContext.Current.Session[SESSION_KEY];
            }

            public static void SetCurrentFilter(SAAdminTool.DocsPaWR.FiltroRicerca[][] filter)
            {
        ...
        public sealed class RubricaWrapper
        {
            ...
            /// <returns></returns>
            public static SAAdminTool.DocsPaWR.Corrispondente GetCorrispondenteDaCodice(Page page, string codCorrispondente)
            {
                DocsPaWR.Corrispondente retValue = null;

                if (codCorrispondente != null)
                {
                    retValue = SAAdminTool.UserManager.getCorrispondente(page, codCorrispondente, true);
                }
            ...
            /// </summary>
            /// <returns></returns>
            public static SAAdminTool.DocsPaWR.Corrispondente GetCorrispondenteDaRubrica()
            {
                return (SAAdminTool.DocsPaWR.Corrispondente)HttpContext.Current.Session[SESSION_KEY];
            }
            ...
            public static void SetCorrispondenteDaRubrica(SAAdminTool.DocsPaWR.Corrispondente corr)
            {

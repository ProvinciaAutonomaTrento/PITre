...
namespace NttDataWA.Handler
{
    ...
    /// </summary>
    public class UploadFileFormazioneHandler : FileTransferHandler, IHttpHandler
    {
        ...
        protected override bool Upload(HttpPostedFile file, string fileName, InfoUtente infoUtente)
        {
            ...
            try
            {
                ...
                result = UIManager.ImportDocumentManager.UploadFileFormazione(file.InputStream, fileName, idUO, infoUtente);
            }
            catch(Exception)
            {
                result = false;

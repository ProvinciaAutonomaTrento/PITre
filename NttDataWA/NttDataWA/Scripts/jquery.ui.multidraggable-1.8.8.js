 ...
 *
 * Modified by Filippo Di Blasi filippo.diblasi@gmail.com on 2013-feb
 *
...
(function ($) {
    ...
    $.widget("ui.multidraggable", $.extend({}, $.ui.draggable.prototype, {
        ...
        _mouseCapture: function (event) {
            var actIndex = -1;
            for (var i = 0; i < this.element.attr('class').split(' ').length; i++)
                if (this.element.attr('class').split(' ')[i].indexOf('rowindex') > -1)
                    actIndex = this.element.attr('class').split(' ')[i].replace('rowindex', '');

            if (event.ctrlKey) {
                // CLICK MOUSE WITH CTRL KEY
                this.element.toggleClass('ui-multidraggable');
                multidraggableLastIndex = actIndex;
            }
            else if (this.element.attr('class').indexOf('rowindex')>-1 && event.shiftKey) {
                // CLICK MOUSE WITH SHIFT KEY
                for (var i = 0; i <9; i++)
                    $('.rowindex' + i).removeClass('ui-multidraggable');

                if (multidraggableLastIndex < 0) {
                    this.element.addClass('ui-multidraggable');
                }
                else {
                    if (actIndex<multidraggableLastIndex) {
                        for (var i = actIndex; i <= multidraggableLastIndex; i++)
                            $('.rowindex' + i).addClass('ui-multidraggable');
                    }
                    else {
                        for (var i = multidraggableLastIndex; i <= actIndex; i++)
                            $('.rowindex' + i).addClass('ui-multidraggable');
                    }
                }

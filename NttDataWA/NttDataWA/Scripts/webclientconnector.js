...
// INIZIO MODIFICHE ALESSANDRO

/*
Per consentire l'utilizzo del connettore sulla stezza macchina in contesti  multiutenti (login di altri utenti
senza logout) è stato aggiunto al connettore una sentinella, ossia un altro webserver in ascolto sulle porte
da 54321 a 54330 in base a quanti utenti abbiano un login attivo sulla stessa macchina.

*/

var BASE_URL_SENTINELLA = 'ws://localhost'; //non usa certificati
var PORTA_SENTINELLA = 0;

function CercaSentinella() {
    var port_temp = 54321;
    
    while (port_temp < 54330) {
        var connessioneSentinella = new WebSocket(BASE_URL_SENTINELLA + ":" + port_temp);
        try {
            conneconnessioneSentinellaction.onopen = function () {
                conneconnessioneSentinellaction.send(message);
            };
            ...
            conneconnessioneSentinellaction.onmessage = function (event) {
                try {
                    ...
                    switch (msg.STATUS) {
                        case 'SUCCESS': onSuccess(msg.MESSAGE, conneconnessioneSentinellaction);
                            break;
                        case 'ERROR': throw msg.MESSAGE;
                    }
                } catch (ex) {
                    onError(ex, conneconnessioneSentinellaction);
                }
            };
        } catch (Err) {
            throw Err;
...
function projectToFS(path, xmlProject, urlDoc, userInfo, onSuccess, onError) {

    var message =  JSON.stringify({ GUID: guid(), CLASS: TIPO_APPLET.FSO, METHOD: FSO_METHOD.PROJECT_TO_FS, PARAMETERS: [path, xmlProject, urlDoc, userInfo] });
    connectionManager(message, onSuccess, onError);
}
...
function initScanningDialog(applyImageCompression, pdfCompressionLevel, fileType, onSuccess, onError) {

    try{
        onSuccess = onSuccess || null;
        onError = onError || null;
        var message = JSON.stringify({ GUID: guid(), CLASS: TIPO_APPLET.SCANNER, METHOD: SCANN_METHOD.INIT_SCANNING_DIALOG, PARAMETERS: [applyImageCompression, pdfCompressionLevel, fileType]
        });
        connectionManager(message, onSuccess, onError);
    } catch (ex) {
        console.log(ex);
...
// test Promise
function signHashPromise(hashEnc, varcertSerial, applyCosign, storeLocation, storeName, onSuccess, onError) {
    console.log(">>>> INIT - signHashPromise");
    onSuccess = onSuccess || null;
    onError = onError || null;
    var message = JSON.stringify({ GUID: guid(), CLASS: TIPO_APPLET.SIGNER, METHOD: SIGNER_METHOD.SIGN_HASH, PARAMETERS: [hashEnc, varcertSerial, applyCosign, storeLocation, storeName] });
    var promiseSign = new Promise(function (resolve, reject) {
        console.log(">>>> START - signHashPromise");
        connectionManagerPromise(message, onSuccess, onError).then(function () { console.log(">>>> RESOLVE - signHashPromise <<<<<"); resolve(); });
        console.log(">>>> END - signHashPromise");
    });
    console.log(">>>> RETURN - signHashPromise");
    return promiseSign;
}
// test PROMISE
function connectionManagerPromise(message, onSuccess, onError) {
    console.log(">>>> INIT - connectionManagerPromise");
    var promiseConnection = new Promise(function (resolve, reject) {
        console.log(">>>> START - connectionManagerPromise");
        var connection = {};
        var maxConnections = MAX_CONNECTIONS;
        console.log('connectionManager - init connectionCount: ' + connectionCount + ' ISMSIE: ' + ISMSIE);
        if (ISMSIE)
        ...
        if (connectionCount > maxConnections) {
            ...
            invokeMethodPromise(message, onSuccess, onError, connection).then(
                function () {
                    console.log(">>>> RESOLVE - connectionManagerPromise");
                    resolve();
                }
            );
        }
        console.log('connectionManager - end connectionCount: ' + connectionCount + ' ISMSIE: ' + ISMSIE);
        console.log(">>>> END - connectionManagerPromise");
    });
    console.log(">>>> RETURN - connectionManagerPromise");
    return promiseConnection;
}

// test promise
function invokeMethodPromise(message, onSuccess, onError, connection) {
    console.log(">>>> INIT - invokeMethodPromise");
    var promiseInvoke = new Promise(function (resolve, reject) {
        console.log(">>>> START - invokeMethodPromise");
        try {
            connection.onopen = function () {
                connection.send(message);
            };
                ...
                try {
                    ...
                    switch (msg.STATUS) {
                        case 'SUCCESS': onSuccess(msg.MESSAGE, connection);
                            console.log(">>>> RESOLVE - invokeMethodPromise");
                            resolve();
                            break;
                        case 'ERROR': throw msg.MESSAGE;
                    }
    ...
    console.log(">>>> RETURN - invokeMethodPromise");
    return promiseInvoke;
}

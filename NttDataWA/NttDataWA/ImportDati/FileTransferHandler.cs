    ...
    public abstract class FileTransferHandler : IRequiresSessionState
    {
        ...
        protected void UploadWholeFile(HttpContext context, List<FilesStatus> statuses)
        {
            ...
            for (int i = 0; i < context.Request.Files.Count; i++)
            {
                ...
                if (String.IsNullOrWhiteSpace(userPostedFile.FileName) || userPostedFile.FileName.Equals("formatPdfExport.xml")) { continue; }  // adattare


                bool result = this.Upload(userPostedFile, _tempFileName, infoUtente);
                FilesStatus fs = new FilesStatus(_tempFileName, userPostedFile.ContentLength);
                fs.error = result ? null : "File non caricato";
                statuses.Add(fs);
            }
        }

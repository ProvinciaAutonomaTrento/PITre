...
function invia(caller) {
    try {
        if (Validate() == false) return;
        else {
            document.getElementById('btnInvia').style.display = 'none';
            ShowWaitingPage("L'importazione può richiedere alcuni minuti...", caller);
            listadir();
...
function ShowWaitingPage(msg, caller) {
    //disallowOp();
    var page = 'tempPageExport.aspx';
    if (caller && caller == 'import')
        page = 'tempPageImport.aspx';
    wndAttendi = window.open(page, '_blank', "location=0,toolbar=0,scrollbars=0,resizable=0,width=350,height=10,left=350,top=350");
}
...
function pushContentAndMetadata(contentFile, metaData, name, type) {
    ...
    http.Open("POST", "ImportDocumentActiveX.aspx?Absolutepath=" + Url.encode(metaData) + "&codFasc=" + Url.encode(document.getElementById('codFasc').value) + "&foldName=" + Url.encode(name) + "&type=" + type, false);
    http.send(content);

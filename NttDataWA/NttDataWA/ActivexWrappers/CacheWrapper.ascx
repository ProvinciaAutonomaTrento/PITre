<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CacheWrapper.ascx.cs" Inherits="NttDataWA.ActivexWrappers.CacheWrapper" %>
<% if (NttDataWA.ActivexWrappers.Configurations.CacheControl && !this.Page.ClientScript.IsClientScriptBlockRegistered(this.Page.GetType(), "cacheObject")) { %> 
...
<% this.Page.ClientScript.RegisterClientScriptBlock(this.Page.GetType(), "cacheObject", string.Empty); }%>
   <script>
    ...
    {
        try{
                if ("<%=NttDataWA.ActivexWrappers.Configurations.CacheControl%>" == "True" 
                    &&  "<%=isActiveCache%>" == "TRUE") 
                {

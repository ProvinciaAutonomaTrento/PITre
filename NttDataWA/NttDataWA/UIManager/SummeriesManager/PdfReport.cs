...
namespace NttDataWA.UIManager.SummariesManager
{
    ...
    {
        public static DocsPaWR.FileDocumento do_MakePdfReport(ReportDisponibili tipoReport, string templateFilePath, DataSet ds, ArrayList parametriPDF)
        {
            return do_MakePdfReport(tipoReport, templateFilePath, null, ds, parametriPDF);
        }


        public static DocsPaWR.FileDocumento do_MakePdfReport(ReportDisponibili tipoReport, string templateFilePath, string titolo, DataSet ds, ArrayList parametriPDF)
        {
            ...
            //string RootPathTemplate = AppDomain.CurrentDomain.BaseDirectory + "ProspettiRiepilogativi/Frontend/TemplateXML/";//OLD
            //ADP 9 maggio 2013
            string RootPathTemplate = AppDomain.CurrentDomain.BaseDirectory + "Summaries/TemplateXML/";
            ...
            DocsPaWR.FileDocumento doc = new DocsPaWR.FileDocumento();
            ...
            //ADP 13 maggio 2013
            HttpContext.Current.Session["fileDoc"] = doc;
                ...
                case ReportDisponibili.Documenti_Classificati:
                    switch (region)
                    {
                        case RegioniDocumento.Generale:
                            ...
                            objReport.appendParagraph("PAR_TOT_DIST_DOC", parametriPDF, false);
                ...
                case ReportDisponibili.Documenti_Trasmessi_Altre_AOO:
                    switch (region)
                    {
                        case RegioniDocumento.Generale:
                            ...
                            //
                            //objReport.appendParagraph("TOT_SPED_PR", parametriPDF, false);                         
                            objReport.appendParagraph("SPED_INTEROP", parametriPDF, false);
                            objReport.appendParagraph("SPED_IS", parametriPDF, false);
                            objReport.appendParagraph("SPED_MAIL", parametriPDF, false);
                            objReport.appendParagraph("TOT_SPED_AMM", parametriPDF, false);
                            break;
                        case RegioniDocumento.Mensile:
                            objReport.appendParagraph("TOT_SPED_PR", parametriPDF, false);
                            break;
        ...
        private static void DO_AddTableToReport(StampaPDF.Report objReport, ReportDisponibili tipoReport, DataTable dt, string TableName)
        {
            switch (tipoReport)
            {
                ...
                case ReportDisponibili.Annuale_By_Registro:
                    ...
                    switch (TableName)
                    {
                        ...
                        case "TABLE_CLASS":
                            ...
                            objReport.appendParagraph("PAR_NOTE_2", "", false);
                            break;
                ...
                case ReportDisponibili.Documenti_Trasmessi_Altre_AOO:
                    ...
                    //				}
                    #endregion
                    // divido il datatable nelle due parti relative ad amministrazioni e persone/ruoli
                    ...
                    switch(TableName)
                    {
                        case "TABLE_AMM":
                            ...
                            {
                                DataColumn _dc = new DataColumn(dc.ColumnName, dc.DataType);
                                dtRep.Columns.Add(_dc);
                            }
                            foreach (DataRow row in dt.Rows)
                            {
                                if (row["CORR"].ToString().Equals("AMM"))
                                    dtRep.ImportRow(row);
                            }
                            break;
                        ...
                        case "TABLE_PR":
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                if (row["CORR"].ToString().Equals("PR"))
                                    dtRep.ImportRow(row);
                            }
                            break;
                    }

                    objReport.appendTable(TableName, dtRep, false);
        ...
        private static void DO_AppendObject(StampaPDF.Report objReport, ReportDisponibili tipoReport, ArrayList parametriPDF, DataTable dt)
        {
            switch (tipoReport)
            {
                ...
                case ReportDisponibili.Documenti_Trasmessi_Altre_AOO:
                    DO_AddParagraphToReport(objReport, tipoReport, parametriPDF, RegioniDocumento.Generale);
                    DO_AddTableToReport(objReport, tipoReport, dt, "TABLE_AMM");
                    DO_AddParagraphToReport(objReport, tipoReport, parametriPDF, RegioniDocumento.Mensile);
                    DO_AddTableToReport(objReport, tipoReport, dt, "TABLE_PR");

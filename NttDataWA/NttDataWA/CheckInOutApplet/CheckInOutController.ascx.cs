namespace NttDataWA.CheckInOutApplet
{
	...
	{
        protected System.Web.UI.WebControls.HiddenField OuFilePath;
        ...
        {
            string componentType = UserManager.getComponentType(Request.UserAgent);
        }

        private void SetCheckInOutStatus() {
            if (CheckOutAppletContext.Current == null)
                CheckOutAppletContext.Current = new CheckOutAppletContext(UIManager.DocumentManager.getSelectedRecord());
        }

        public void UpdateCheckOutPath()
        {
            this.OuFilePath.Value = this.CheckOutFilePath;
        ...
        protected virtual void SetCheckInOutFolderUrl()
        {
            ...
            {
                this.ViewState["CheckInOutFolderUrl"] = NttDataWA.Utils.utils.getHttpFullPath() + "/CheckInOutApplet/";
            }
        }
        ...
        protected String MTextShowDocumentUrl
        {
            get
            {
                ...
                if(CheckOutAppletContext.Current != null && 
                    CheckOutAppletContext.Current.Status != null &&
                    !String.IsNullOrEmpty(CheckOutAppletContext.Current.Status.DocumentLocation) &&
                    CheckOutAppletContext.Current.Status.DocumentLocation.StartsWith("mtext://"))
                    retVal = String.Format("{0}/Models/MText/Show.aspx?fqn={1}",
                        Utils.utils.getHttpFullPath(), 
                        HttpUtility.UrlEncode(CheckOutAppletContext.Current.Status.DocumentLocation.Remove(0, 8))); 

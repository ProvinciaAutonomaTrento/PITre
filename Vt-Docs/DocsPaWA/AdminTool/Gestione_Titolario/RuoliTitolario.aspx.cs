...
namespace Amministrazione.Gestione_Titolario
{
	...
	public class RuoliTitolario : System.Web.UI.Page
	{
		...
		private void btnOK_Click(object sender, System.EventArgs e)
		{
			...
			DocsPAWA.DocsPaWR.EsitoOperazione[] retValue=this.ApplyChanges();
            ...
            if (retValue != null)
            {
                if (retValue.Length > 0)
                {
                    ...
                    DocsPAWA.AdminTool.EsitoOperazioneViewerSessionManager.Items = retValue;
		...
		{
			DocsPAWA.DocsPaWR.OrgRuoloTitolario[] ruoliTitolario=this.GetRuoliTitolario();
		...
		private DocsPAWA.DocsPaWR.OrgRuoloTitolario[] GetRuoliTitolario()
		{
			DocsPAWA.DocsPaWR.OrgRuoloTitolario[] retValue=null;
		...
		private void FillGridDataTableRuoliTitolario(DocsPAWA.DocsPaWR.OrgRuoloTitolario[] ruoliTitolario)
		{
			DataTable dt=this.CreateGridDataTableRuoliTitolario();

            if (ruoliTitolario != null)
            {
                foreach (DocsPAWA.DocsPaWR.OrgRuoloTitolario ruoloTitolario in ruoliTitolario)
                {
...
//				Session.Remove(SESSION_KEY_RUOLI_TITOLARIO);
                ...
                if (checkBox != null && !checkBox.Checked)
                {
                    ...
                    if (retValue != null)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (DocsPAWA.DocsPaWR.EsitoOperazione esito in retValue)
                            {
		...
		public sealed class RuoliTitolarioSessionManager
		{
			...
			public static void SetRuoliTitolario(DocsPAWA.DocsPaWR.OrgRuoloTitolario[] ruoliTitolario)
			{
				HttpContext.Current.Session[SESSION_KEY_RUOLI_TITOLARIO]=ruoliTitolario;
			}

			public static DocsPAWA.DocsPaWR.OrgRuoloTitolario[] GetRuoliTitolario()
			{
				return HttpContext.Current.Session[SESSION_KEY_RUOLI_TITOLARIO] as DocsPAWA.DocsPaWR.OrgRuoloTitolario[];
			}

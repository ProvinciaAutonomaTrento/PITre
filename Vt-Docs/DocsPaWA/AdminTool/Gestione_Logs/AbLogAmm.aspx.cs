            ...
            try
            {
                ...
                if (xmlStream != null && xmlStream != "")
                {
                    ...
                    if (listaAzioni.ChildNodes.Count > 0)
                    {
                        ...
                        bool isPARER = this.IsConservazionePARER();
                        ...
                        for (int i = 0; i < dg_AbilitaLogAmm.Items.Count; i++)
                        {
                            ...
                            // Se è attiva la conservazione classica visualizzo i log CONSERVAZIONE
                            // Se è attiva la conservazione PARER visualizzo i log CONSERVAZIONE_PARER
                            if (isPARER)
                            {
                                if(dg_AbilitaLogAmm.Items[i].Cells[0].Text.Equals("CONSERVAZIONE"))
                                    dg_AbilitaLogAmm.Items[i].Visible = false;
                            }
                            else
                            {
                                if (dg_AbilitaLogAmm.Items[i].Cells[0].Text.Equals("CONSERVAZIONE_PARER"))
                                    dg_AbilitaLogAmm.Items[i].Visible = false;
                            }
                        }

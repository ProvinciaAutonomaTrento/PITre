...
namespace Amministrazione.Gestione_Utenti
{
    ...
    public class GestUtenti : System.Web.UI.Page
    {
        ...
        protected System.Web.UI.WebControls.CheckBox chkApplicativoVerticale;
        ...
        protected System.Web.UI.UpdatePanel UpPnlAggiungiCasella;
        ...
        private void RipulisceTutto()
        {
            ...
            //utente verticale
            this.chkApplicativoVerticale.Checked = false;
            this.gvCaselle.DataSource = null;
            this.gvCaselle.Attributes["style"] = "display:none";
            ViewState["gvCaselle"] = null;
        }
                ...
                if (this.VerificaCampiObbligatori())
                {
                    if (this.VerificaCampiInseriti())
                    {
                        ...
                        if (continua)
                        {
                            ...
                            if (this.chkApplicativoVerticale.Checked)
                            {
                                List<string> emails = ViewState["gvCaselle"] as List<string>;
                                if (emails != null && emails.Count > 0)
                                {
                                    foreach (string e in emails)
                                        utente.Email += string.IsNullOrEmpty(utente.Email) ? e : ";" + e;
                                }
                            }
                            ...
                            if (esito.Codice.Equals(0))
                            {
                                ...
                                if(this.chkApplicativoVerticale.Checked)
                                    this.SetUtenteVerticale(utente.UserId, utente.IDAmministrazione, true);
        ...
        private bool SetUtenteVerticale(string userId, string idAmm, bool verticale)
        {
            bool resultValue = false;
            resultValue = UserManager.SetUtenteVerticale(this, userId, idAmm, verticale);
            return resultValue;
        }
        ...
        private bool IsUtenteVerticale(string idPeople)
        {
            bool resultValue = false;
            resultValue = UserManager.IsUtenteVerticale(this, idPeople);
            return resultValue;
        }
                    ...
                    if (this.VerificaCampiInseriti())
                    {
                        ...
                        if (continua)
                        {
                            ...
                            if (esito.Codice.Equals(0))
                            {
                                ...
                                this.SetUtenteVerticale(utente.UserId, utente.IDAmministrazione, this.chkApplicativoVerticale.Checked);
        ...
        private bool VerificaCampiInseriti()
        {
            ...
            if (this.txt_email.Text.Trim() != "" && !IsValidEmail(this.txt_email.Text.Trim(), this.chkApplicativoVerticale.Checked))
            {
        ...
        private  bool IsValidEmail(string strToCheck, bool isVerticale)
        {
            bool result = true;
            if (!isVerticale)
            {
                result = Regex.IsMatch(strToCheck, @"^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$");
            }
            else
            {
                string[] emails = strToCheck.Split(';');
                if(emails != null && emails.Count() > 0)
                {
        ...
        private void LoadDatiUtente(DataGridItem item)
        {
            ...
            //Check utente verticate
            this.chkApplicativoVerticale.Checked = IsUtenteVerticale(item.Cells[1].Text);
            ...
            if (this.chkApplicativoVerticale.Checked)
            {
                this.imgAggiungiCasella.Visible = true;
                string email = this.txt_email.Text;
                if (!string.IsNullOrEmpty(email))
                {
                    this.txt_email.Text = string.Empty;
                    string[] emails = email.Split(';');
                    this.gvCaselle.Attributes["style"] = "display:block";
                    ViewState["gvCaselle"] = emails.ToList() as List<string>;
        ...
        private void LoadDatiUtente(DataGridCommandEventArgs e)
        {
            ...
            this.chkApplicativoVerticale.Checked = IsUtenteVerticale(e.Item.Cells[1].Text);
        ...
        #region APPLICATIVO VERTICALE

        protected void chkApplicativoVerticale_OnCheckedChanged(object sender, EventArgs e)
        {
            if(this.chkApplicativoVerticale.Checked)
            {
                this.imgAggiungiCasella.Visible = true;
            ...
            else
            {
                this.imgAggiungiCasella.Visible = false;
                this.gvCaselle.Attributes["style"] = "display:none";
                if ((ViewState["gvCaselle"] as List<string>) != null && (ViewState["gvCaselle"] as List<string>).Count > 0)
                {
                    this.txt_email.Text = (ViewState["gvCaselle"] as List<string>)[0];
                    ViewState["gvCaselle"] = null;
                }
            }
            //this.UpPnlAggiungiCasella.Update();
        }
        ...
        protected void imgAggiungiCasella_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (!System.Text.RegularExpressions.Regex.Match(this.txt_email.Text, pattern).Success)
            {
            ...
            if ((List<string>)ViewState["gvCaselle"] != null)
            {
                foreach (string c in (List<string>)ViewState["gvCaselle"])
                {
                    if (c.Trim().Equals(this.txt_email.Text.Trim()))
                    {
            ...
            {
                ViewState["gvCaselle"] = new List<string>();
            }
            (ViewState["gvCaselle"] as List<string>).Add(this.txt_email.Text);
            gvCaselle.DataSource = (List<string>)ViewState["gvCaselle"];
            gvCaselle.DataBind();

            this.gvCaselle.Attributes["style"] = "display:block";

            //PULISCO I CAMPI EMAIL
            this.txt_email.Text = string.Empty;
        }
        ...
        protected void imgEliminaCasella_Click(object sender, ImageClickEventArgs e)
        {
            ...
            (ViewState["gvCaselle"] as List<string>).RemoveAt(indexRowDelete);
            if ((ViewState["gvCaselle"] as List<string>) != null && (ViewState["gvCaselle"] as List<string>).Count() > 0)
            {
                gvCaselle.DataSource = (List<string>)ViewState["gvCaselle"];
                gvCaselle.DataBind();

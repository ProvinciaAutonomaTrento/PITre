<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PolicyStampePARER.aspx.cs" Inherits="DocsPAWA.AdminTool.Gestione_Conservazione.PolicyStampePARER" %>
...
<head id="Head1" runat="server">
    ...
    <script language="JavaScript" type="text/javascript">
        ...
        function NewPolicyStampe() {
            ...
            var retval = window.showModalDialog(_url_new_policy, 'NewPolicyStampe', 'dialogWidth:750px;dialogHeight:680px;status:no;resizable:no;scroll:yes;dialogLeft:' + newLeft + ';dialogTop:' + newTop + ';center:no;help:no;', '');
            if (retval != null) {
        ...
        function OpenPolicyDetails(id) {
            var newLeft = (screen.availWidth / 2 - 225);
            var newTop = (screen.availHeight - 704);
            var retval = window.showModalDialog(_url_view_policy + "?id=" + id, 'OpenPolicyDetails', 'dialogWidth:700px;dialogHeight:680px;status:no;resizable:no;scroll:yes;dialogLeft:' + newLeft + ';dialogTop:' + newTop + ';center:no;help:no;', '');
            if (retval != null) {
                window.document.getElementById('<%= Page.Form.ClientID %>').submit();
            }
        }
        ...
        function Export() {
            var newLeft = (screen.availWidth / 2 - 225);
            var newTop = (screen.availHeight - 704);
            var retval = window.open(_url_export + "?tipo=ST", 'ExportPolicy', 'width=500,height=300,scrollbars=NO,top=' + newTop + ',left=' + newLeft);
        }
    </script>
    <style type="text/css">
        .tab_sx
                                                ...
                                                <asp:DataGrid ID="grvPolicy" runat="server" AllowSorting="false" AutoGenerateColumns="false"
                                                    ...
                                                    <Columns>
                                                        ...
                                                        <asp:TemplateColumn ItemStyle-Width="8%" HeaderText="ATTIVA" ItemStyle-CssClass="tab_sx2" HeaderStyle-HorizontalAlign="left" HeaderStyle-CssClass="head_tab">
                                                            <ItemTemplate>
                                                                <asp:CheckBox runat="server" ID="chk_attiva_policy" Checked="<%# this.GetStatoAttivazionePolicy((DocsPAWA.DocsPaWR.PolicyPARER)Container.DataItem) %>" OnCheckedChanged="chkPolicy_CheckedChanged" />
                                                            </ItemTemplate>
                                                        </asp:TemplateColumn>

...
namespace ProtocollazioneIngresso.Scansione
{
	...
	public class UploadFile : System.Web.UI.Page
	{
			...
			{	
                if (Request.QueryString.Count>1)
				{
					...
					if (filePath!=null)
					{
                        ...
                        if (Convert.ToBoolean(DocsPAWA.Utils.isEnableConversionePdfLatoServer()) && Convert.ToBoolean(convertPdf))
                        {
		...
		private bool IsEnabledPdfAcrobatIntegration()
		{
			...
			try
			{
				retValue=Convert.ToBoolean(DocsPAWA.ConfigSettings.getKey(DocsPAWA.ConfigSettings.KeysENUM.ADOBE_ACROBAT_INTEGRATION));
			}
			catch
			{
			}

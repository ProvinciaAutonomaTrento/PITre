...
{
    public partial class TrasmissioneMassiva : MassivePage
    {
        ...
        private List<InfoDocumento> InfoDocumentList
        {
            get
            {
                return CallContextStack.CurrentContext.ContextState["InfoDocumentList"] as List<InfoDocumento>;
            }
            set
            {
        ...
        private List<InfoFascicolo> InfoProjectList
        {
            get
            {
                return CallContextStack.CurrentContext.ContextState["InfoProjectList"] as List<InfoFascicolo>;
            }
            set
            {
                CallContextStack.CurrentContext.ContextState["InfoProjectList"] = value;
            }
        }

        #region strutture per la trasmissione fascicoli extra AOO
        /// <summary>
        ...
        private List<InfoFascicolo> InfoProjectListExtraAOO
        {
            get
            {
                return CallContextStack.CurrentContext.ContextState["InfoProjectListExtraAOO"] as List<InfoFascicolo>;
            }
            set
            {
                CallContextStack.CurrentContext.ContextState["InfoProjectListExtraAOO"] = value;
            }
        }
        /// <summary>
        ...
        private List<string> ListaCorrProjectOnlyExtraAOO
        {
            get
            {
                return CallContextStack.CurrentContext.ContextState["ListaCorrProjectOnlyExtraAOO"] as List<string>;
            }
            set
            {
                CallContextStack.CurrentContext.ContextState["ListaCorrProjectOnlyExtraAOO"] = value;
            }
        }
        #endregion
        ...
        private RagioneTrasmissione[] TransmissionReasons
        {
            get
            {
                return CallContextStack.CurrentContext.ContextState["TransmissionReasons"] as RagioneTrasmissione[];
            }
            set
            {
                CallContextStack.CurrentContext.ContextState["TransmissionReasons"] = value;
            }
        }
        ...
        private MassiveOperationTrasmissionDetailsCollection TransmissionCollection
        {
            get
            {
                return CallContextStack.CurrentContext.ContextState["TransmissionCollection"] as MassiveOperationTrasmissionDetailsCollection;
            }

            set
            {
                CallContextStack.CurrentContext.ContextState["TransmissionCollection"] = value;
            }
        ...
        private void Initialize(string objType)
        {
            ...
            // Al click sul pulsante della rubrica, bisogna aprire la rubrica
            this.btnReciviersAddressBook.OnClientClick = String.Format(
                "openTransmissionAddressBook({0}.value);",
                this.ddlReasons.ClientID);
            ...
            if (ws.isFiltroAooEnabled() && this.InfoProjectList != null && this.InfoProjectList.Count > 0)
            {
                ...
                {
                    string idRegFasc = FascicoliManager.getFascicoloById(this.Page, f.idFascicolo).idRegistroNodoTit;
            ...
            this.txtSenderUser.Text = UserManager.getUtente(this).descrizione;

            // Compilazione della casella di testo Ruolo
            this.txtSenderRole.Text = UserManager.getRuolo(this).descrizione;
        ...
        private void FillTransmissionModelDDLWithSharedModel(string objType)
        {
            ...
            transmissionModelTemplates = (ModelloTrasmissione[])ModelliTrasmManager.getModelliPerTrasmLite(
                userInfo.idAmministrazione,
                ((Ruolo)UserManager.getRuolo(this)).registri,
                userInfo.idPeople,
        ...
        private List<InfoFascicolo> LoadProjectInformation(List<MassiveOperationTarget> selectedItemList)
        {
            ...
            foreach (MassiveOperationTarget temp in selectedItemList)
                toReturn.Add(FascicoliManager.getInfoFascicoloDaFascicolo(
                    FascicoliManager.getFascicoloById(
                        this,
                        temp.Id),
                        this));
        ...
        private void FillDocumentTransmissionReasonDDL(MassiveOperationTrasmissionDetailsCollection transmission)
        {
            ...
            documentScheda = DocumentManager.getDettaglioDocumento(
                this,
                this.InfoDocumentList[0].idProfile,
                this.InfoDocumentList[0].docNumber);

            DocumentManager.setDocumentoInLavorazione(this, documentScheda);
        ...
        private void FillProjectTransmissionReasonDDL(MassiveOperationTrasmissionDetailsCollection transmission)
        {
            ...
            project = FascicoliManager.getFascicolo(
                this,
        ...
        private bool CheckIsRoleEnabledToRightLease()
        {
            ...
            if (this.InfoDocumentList != null && this.InfoDocumentList.Count > 0)
                isAuth = UserManager.ruoloIsAutorized(this, "ABILITA_CEDI_DIRITTI_DOC");

            if (this.InfoProjectList != null && this.InfoProjectList.Count > 0)
                isAuth = UserManager.ruoloIsAutorized(this, "ABILITA_CEDI_DIRITTI_FASC");
        ...
        private void SearchCorrAndAddASetRecivier(string corrCode, RubricaCallType callType, String transmissionType, String[] userToNotify, bool simpleTransmission, string singleNote, string expirationDate, bool hidePreviousVersions)
        {
            ...
            // Valore di configurazione che indica la possibilità di ricercare
            // corrispondente all'interno delle liste
            int canSearchInLists = 0;
            ...
            // Vengono ricercati tutti i corrispondenti
            // Lettura dell'impostazione relativa alla possibilità di ricercare corrispondenti
            // all'interno delle liste
            Int32.TryParse(
                ConfigurationManager.AppSettings["LISTE_DISTRIBUZIONE"],
                out canSearchInLists);

            searchParameters.doListe = canSearchInLists == 1 ? true : false;
            ...
            searchParameters.ObjectType = Request["objType"].ToString();
            ...
            if (result != null && result.Length == 1)
            {
                ...
                else
                {
                    ...
                    if (ws.isFiltroAooEnabled() && InfoProjectList != null && InfoProjectList.Count > 0)
                    {
                        Registro reg = null;
                        if (result[0].tipo.Equals("P"))// se il corrispondente è di tipo utente
                        {
                            Ruolo[] ruolo = UserManager.GetRuoliUtenteByIdCorr(this.Page, result[0].systemId);
                        ...
                        if (reg != null && (!reg.systemId.Equals(UserManager.getRegistroSelezionato(this.Page).systemId))) //il corrispondente non ha visibilità sul registro del mittente
                        {
                            if (InfoProjectListExtraAOO.Count < 1)
                            {
                                setReciviers = false;
                                this.showMessage("Il corrispondente selezionato non ha visibilità sui fascicoli selezionati");
                            }
        ...
        /// <summary>
        /// Funzione per l'impostazione del calltype da adottare per la ricerca del corrispondente
        /// </summary>
        /// <param name="reason">La ragione selezionata</param>
        /// <returns>Il call type</returns>
        private RubricaCallType GetCallType(RagioneTrasmissione reason)
        {
            // Il valore da restituire
            RubricaCallType toReturn = RubricaCallType.CALLTYPE_TRASM_ALL;

            switch (reason.tipoDestinatario.ToString()[0])
            {
                case 'T':
                    toReturn = RubricaCallType.CALLTYPE_TRASM_ALL;
                    break;
                case 'I':
                    toReturn = RubricaCallType.CALLTYPE_TRASM_INF;
                    break;
                case 'S':
                    toReturn = RubricaCallType.CALLTYPE_TRASM_SUP;
                    break;
                case 'P':
                    toReturn = RubricaCallType.CALLTYPE_TRASM_PARILIVELLO;
                    break;
            }

            // Restituzione del call type
            return toReturn;
        ...
        private void SetReciviers(ElementoRubrica[] searchResult, ParametriRicercaRubrica searchParameters, String transmissionType, String[] userToNotify, bool simpleTransmission, string singleNote, string expirationDate, bool hidePreviousVersions)
        {
            ...
            switch (searchResult[0].tipo)
            {
                ...
                default:
                    ...
                    corrQuery = new DocsPAWA.DocsPaWR.AddressbookQueryCorrispondente();
                    corrQuery.codiceRubrica = searchParameters.codice;
                    corrQuery.getChildren = false;
                    corrQuery.idAmministrazione = UserManager.getInfoUtente(this.Page).idAmministrazione;
                    corrQuery.tipoUtente = DocsPAWA.DocsPaWR.AddressbookTipoUtente.INTERNO;
            ...
            bool notifyAll)
        {
            ...
            if (transmission.GetSingleTransmissionNumber() > 0)
            {
                ...
                if (!canContinue && ws.isFiltroAooEnabled() && trasmRestrictFasc)
            ...
            {
                this.showMessage("Per poter utilizzare la trasmissione con cessione dei diritti è necessario selezionare un ruolo");
                canContinue = false;
            }
            ...
            if (canContinue)
            {
                ...
                //se abilitato il controllo di visibilità per la trasmissione dei fascicoli e il corrispondente corrente ha visibilità limitata sui fascicoli
                if (ws.isFiltroAooEnabled() && trasmRestrictFasc)
                    ListaCorrProjectOnlyExtraAOO.Add(singleTransmission.corrispondenteInterno.codiceCorrispondente);
                ...
                if (corr is Utente)
                {
                    ...
                    userTransmission.utente = (DocsPAWA.DocsPaWR.Utente)corr;
                ...
                if (corr is UnitaOrganizzativa)
                {
                    ...
                    corrAuthoryzed.ruolo = UserManager.getRuolo();
        ...
        private Corrispondente[] GetUsers(Corrispondente corr)
        {
            ...
            query.idAmministrazione = UserManager.getInfoUtente(this.Page).idAmministrazione;
            ...
            MassiveOperationTrasmissionDetailsCollection transmission)
        {
            ...
            {
                this.showMessage("Attenzione! Poichè la trasmissione creata prevede la cessione dei diritti, non è possibile inserire altro.");
                retValue = true;
            }
            else
                ...
                {
                    this.showMessage("Attenzione! non è possibile inserire ragioni di trasmissione con cessione insieme a ragioni senza cessione.");
                    retValue = true;
                }
        ...
        private void ExecuteTransmission(string objectType)
        {
            ...
            foreach (InfoFascicolo prjInfo in projects)
            {
                ...
                try
                {
                    if (result != MassiveOperationReport.MassiveOperationResultEnum.KO)
                        TrasmManager.saveExecuteTrasmAM(this, TrasmManager.getGestioneTrasmissione(this), UserManager.getInfoUtente(this));
                }
                catch (Exception ex)
                {
        ...
        private void TransmitProjectsExtraAoo(MassiveOperationReport report)
        {
            ...
            if (trasmSingola.Count > 0)
            {
                ...
                foreach (InfoFascicolo prjInfo in InfoProjectList)
                {
                    ...
                    {
                        if (result != MassiveOperationReport.MassiveOperationResultEnum.KO)
                            TrasmManager.saveExecuteTrasmAM(this, trasm, UserManager.getInfoUtente(this));
                    }
                    catch (Exception ex)
                    {
                        result = MassiveOperationReport.MassiveOperationResultEnum.KO;
            ...
            out string details)
        {
            ...
            string accessRight = ws.getAccessRightFascBySystemID(prjInfo.idFascicolo, UserManager.getInfoUtente(this));
            ...
            try
            {
                bool isGenerale = FascicoliManager.IsFascicoloGenerale(UserManager.getInfoUtente(), prjInfo.idFascicolo);
        ...
        private String SetRightLeaseOnTransmissionPrj(Trasmissione trasmissione)
        {
            ...
            if (this.TransmissionCollection.GetNumberOfTransmissionsWithCessionReason() > 0)
                ...
                else
                {
                    ...
                    trasmissione.cessione.idPeople = UserManager.getInfoUtente().idPeople;
                    trasmissione.cessione.idRuolo = UserManager.getRuolo().idGruppo;
                    trasmissione.cessione.userId = UserManager.getInfoUtente().userId;
                    ...
                    trasmissione.cessione.idRuoloNewPropr = String.IsNullOrEmpty(this.TransmissionCollection.DocumentLeasing.idRuoloNewPropr) ?
                        UserManager.getRuoloById(trasmissione.trasmissioniSingole[0].trasmissioneUtente[0].utente.ruoli[0].systemId, this).idGruppo :
        ...
        private bool IsUserOwnerOfPrj(InfoFascicolo prjInfo)
        {
            ...
            try
            {
                ...
                ws.SelectSecurity(prjInfo.idFascicolo, UserManager.getInfoUtente(this).idPeople, "= 0", out accessRights, out idGruppoTrasm, out tipoDiritto);
                isPersonOwner = (accessRights.Equals("0"));

                // verifica se è proprietario come RUOLO...
                ws.SelectSecurity(prjInfo.idFascicolo, UserManager.getInfoUtente(this).idGruppo, "= 255", out accessRights, out idGruppoTrasm, out tipoDiritto);
            ...
            out string details)
        {
            ...
            // Verifica accessRight
            string accessRight = ws.getAccessRightDocBySystemID(docInfo.idProfile, UserManager.getInfoUtente(this));
            if (accessRight.Equals("20"))
            {
                retValue = MassiveOperationReport.MassiveOperationResultEnum.KO;
            ...
            if (ws.IsDocAnnullatoByIdProfile(docInfo.idProfile))
            {
                retValue = MassiveOperationReport.MassiveOperationResultEnum.KO;
        ...
        private bool IsUserOwnerOfDocument(InfoDocumento infoDocumento)
        {
            ...
            {
                // L'utente potrebbe essere proprietario a livello di utente
                ws.SelectSecurity(infoDocumento.idProfile, UserManager.getInfoUtente(this).idPeople, "= 0", out accessRights, out idGruppoTrasm, out tipoDiritto);
                isPersonOwner = (accessRights.Equals("0"));
                ...
                {
                    // verifica se è proprietario come RUOLO...
                    ws.SelectSecurity(infoDocumento.idProfile, UserManager.getInfoUtente(this).idGruppo, "= 255", out accessRights, out idGruppoTrasm, out tipoDiritto);
                    isGroupOwner = (accessRights.Equals("255"));
                }
        ...
        private void LoadTransmissionModelDataAndCreateTransmission(ModelloTrasmissione transmissionModel)
        {
            ...
            try
            {
                    ...
                    transmissionModel.CHA_TIPO_OGGETTO == "D")
                {
                    ...
                    documentLeasing.idPeople = UserManager.getInfoUtente(this).idPeople;
                    documentLeasing.idRuolo = UserManager.getInfoUtente(this).idGruppo;
                    documentLeasing.userId = UserManager.getInfoUtente(this).userId;
                ...
                foreach (MittDest senderRecivier in recivierReason.DESTINATARI)
                {
                    ...
                    if (corr != null)
                        this.SearchCorrAndAddASetRecivier(
                            corr.codiceRubrica,
                            this.GetCallType(transmissionReason),
        ...
        private void ResetReportAndDisplayTransmissionButton()
        {
            ...
            this.txtRecivierCode.Text = String.Empty;
        }
        ...
        /// <summary>
        /// Funzione per la visualizzazione della finestra per l'importazione del nuovo proprietario del
        /// documento / fascicolo
        /// </summary>
        /// <param name="dataGridItem">L'iteme con i dati sulla trasmissione</param>
        private void ChooseNewOwner(DataGridItem dataGridItem)
        {
            ...
            System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowChooseOwner", "window.showModalDialog('../popup/sceltaNuovoProprietario.aspx?tipo=" + type + "', '', 'dialogWidth:700px;dialogHeight:600px;status:no;resizable:yes;scroll:no;center:yes;help:no;', '');", true);
            //System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowChooseOwner", "window.open('../popup/sceltaNuovoProprietario.aspx', '', 'dialogWidth:700px;dialogHeight:600px;status:no;resizable:no;scroll:auto;center:yes;help:no;close:no;', '');", true);
        }

        /// <summary>
        /// Funzione per la cancellazione di un item dalla lista dei destinatari
        /// </summary>
        /// <param name="dataGridItem">L'item con i dati sull'elemento da eliminare</param>
        private void DeleteItem(DataGridItem dataGridItem)
        {
        ...
        /// </summary>
        /// <param name="dataGridItem">L'item con i dati aggiornati</param>
        private void UpdateData(DataGridItem dataGridItem)
        {
        ...
        /// <summary>
        /// Evento generato al caricamento della pagina. Si occupa dell'inizializzazione della pagina.
        /// </summary>
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                this.Initialize(Request["objType"]);
                // se stiamo trasmettendo massivamente fascicoli, dobbiamo rendere invisibile la colonna "Nascondi vers. prec."
                DataGridColumnCollection columns = this.dgDestinatari.Columns;
                foreach (DataGridColumn c in columns)
                {
                    if (c.HeaderText.Equals("Nascondi vers. prec.") && IsFasc)
                    {
                        c.Visible = false;
                        break;
                    }
                }
                // Il dettaglio della trasmissione deve essere visibile solo se non si tratta di
                // trasmissione da modello
                this.dgDestinatari.DataSource = this.ddlTemplates.SelectedIndex > 0 ?
                    null :
                    this.TransmissionCollection.GetTransmissionElementArray();

                this.dgDestinatari.DataBind();
            }

            // Se il contesto attuale contiene dati sui destinatari selezionati da rubrica,
            // si procede prima con l'aggiunta delle trasmissioni singole alla lista delle trasmissioni
            // e si cancella la trasmissione in sessione
            if (CallContextStack.CurrentContext.ContextState["RecivierFromAddressBook"] != null)
            {
                // Prelevamento dell'array delle trasmissioni singole create
                singleTransmissionArray = TrasmManager.getGestioneTrasmissione(this).trasmissioniSingole as TrasmissioneSingola[];

                foreach (TrasmissioneSingola singleTransmission in singleTransmissionArray)
                    this.SearchCorrAndAddASetRecivier(
                        singleTransmission.corrispondenteInterno.codiceRubrica,
                        this.GetCallType(TrasmManager.getRagioneSel(this)),
                        singleTransmission.tipoTrasm,
                        new String[0],
                        true,
                        singleTransmission.noteSingole,
                        singleTransmission.dataScadenza,
                        singleTransmission.hideDocumentPreviousVersions);

                CallContextStack.CurrentContext.ContextState.Remove("RecivierFromAddressBook");

                TrasmManager.setGestioneTrasmissione(this, null);
        ...
        protected void ddlReasons_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddl.SelectedIndex != 0)
            {
                ...
                if (this.ddlTemplates.SelectedIndex != 0)
                {
                    ...
                    this.dgDestinatari.EditItemIndex = -1;
                ...
                this.txtRecivierCode.Enabled = true;
                this.ibSearch.Enabled = true;
                this.btnReciviersAddressBook.Enabled = true;
                //this.btnAnswareTo.Enabled = true;
            ...
            else
            {
                ...
                this.txtRecivierCode.Enabled = false;
                this.ibSearch.Enabled = false;
                this.btnReciviersAddressBook.Enabled = false;
                //this.btnAnswareTo.Enabled = false;
        ...
        {
            foreach (DataGridColumn column in this.dgDestinatari.Columns)
        ...
        /// e viene eseguita
        /// </summary>
        protected void dgDestinatari_OnItemCommand(object sender, DataGridCommandEventArgs e)
        {
            // Se la trasmissione viene creata da modello, non è possibile effettuare
            // alcuna operazione
            if (this.TransmissionCollection.FromModel)
            {
                this.showMessage("Non è possibile compiere l'azione richiesta in quanto la trasmissione è stata creata da modello.");
                return;
            }
            ...
            {
                case "Edit":
                    this.dgDestinatari.EditItemIndex = e.Item.ItemIndex;
                ...
                case "NewOwner":
                    this.ChooseNewOwner(e.Item);
                    this.dgDestinatari.EditItemIndex = -1;
                    break;
            }

            // Aggiornamento dei dati del daagrid
            this.dgDestinatari.DataSource = this.TransmissionCollection.GetTransmissionElementArray();
            this.dgDestinatari.DataBind();
        ...
        /// <summary>
        /// Al verificarsi di questo evento bisogna ricercare nella rubrica un
        /// corrispondente interno con il codice uguale a quello riportato nella
        /// casella di testo del codice
        /// </summary>
        public void ibSearch_Click(object sender, EventArgs e)
        {
            // Se la casella di testo del codice è valorizzata,
            // si procede con la ricerca del corrispodente
            if (!String.IsNullOrEmpty(this.txtRecivierCode.Text))
            {
                ...
                {
                    this.SearchCorrAndAddASetRecivier(
                        this.txtRecivierCode.Text.Trim(),
                        this.GetCallType(TrasmManager.getRagioneSel(this)), "S", new String[0], true, String.Empty, String.Empty, false);
        ...
        protected void cblUsers_PreRender(object sender, EventArgs e)
        {
            ...
            transmissionId = ((HiddenField)this.dgDestinatari.Items[this.dgDestinatari.EditItemIndex].FindControl("hfId")).Value;
            ...
            {
                list.Enabled = false;
                ((CheckBox)this.dgDestinatari.Items[this.dgDestinatari.EditItemIndex].FindControl("chkSelectDeselectAll")).Visible = false;
            }
            else
                ...
                if (transmissionDetails.UserTransmission.Where(
                    ...
                    ((CheckBox)this.dgDestinatari.Items[this.dgDestinatari.EditItemIndex].FindControl("chkSelectDeselectAll")).Checked = true;

            ((Literal)this.dgDestinatari.Items[this.dgDestinatari.EditItemIndex].FindControl("ltlNoEditable")).Visible =
                !list.Enabled && TransmissionCollection.NoNotify;

        }

        /// <summary>
        /// Alla pressione del pulsante di conferma, vengono inviate le trasmissioni
        /// </summary>
        protected override bool btnConferma_Click(object sender, EventArgs e)
        {
            ...
            if (this.dgDestinatari.EditItemIndex != -1)
            {
                canContinue = false;
                message = new StringBuilder("Prima di poter effetture la trasmissione, è necessario uscire dalla modalità di modifica cliccando sul pulsante Annulla o sul pulsante Salva.");
            }
            ...
            {
                canContinue = false;
                message = new StringBuilder("Prima di poter effettuare la trasmissione è necessario selezionare il nuovo proprietario.");
            }

            if (canContinue)
            {
                message = new StringBuilder(
                    "Selezionare almeno un utente da notificare per le seguenti trasmissioni:");
            ...
            if (canContinue)
            {
                this.ExecuteTransmission(Request["objType"]);
                return true;
            }
            else
            {
        ...
        protected void ddlTemplates_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlModels.SelectedIndex > 0)
            {
                ...
                this.txtNotes.Text = String.Empty;
                this.txtDescription.Text = String.Empty;
                this.upDescriptionSection.Update();
                this.Update_panel_note.Update();
                this.InitializeTransmission(Request["objType"]);
                this.dgDestinatari.DataSource = null;
                this.dgDestinatari.DataBind();
                ...
                transmissionModel = ModelliTrasmManager.getModelloByID(
                    UserManager.getInfoUtente().idAmministrazione,
                ...
                {
                    this.showMessage("Non è stato possibile caricare le informazioni sul modello.");
                }
                else
                    try
                    {
        ...
        protected void ddlType_PreRender(object sender, EventArgs e)
        {
            ...
            idTransmission = ((HiddenField)this.dgDestinatari.Items[this.dgDestinatari.EditItemIndex].FindControl("hfId")).Value;

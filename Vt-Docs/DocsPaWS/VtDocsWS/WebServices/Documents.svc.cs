...
namespace VtDocsWS.WebServices
{
    ...
    public class Documents : IDocuments
    {
        ...
        /// <summary>
        /// Servizio per protocollare un predisposto
        /// </summary>
        /// <param name="request"></param>
        /// <returns>Response </returns>
        public Services.Documents.EditDocument.EditDocumentResponse ProtocolPredisposed(Services.Documents.ProtocolPredisposed.ProtocolPredisposedRequest request)
        {
            logger.Info("BEGIN");

            Services.Documents.EditDocument.EditDocumentResponse response = Manager.DocumentsManager.ProtocolPredisposed(request);

            // Log
            //
            DocsPaVO.Logger.CodAzione.Esito esito = (response != null && response.Success ? DocsPaVO.Logger.CodAzione.Esito.OK : DocsPaVO.Logger.CodAzione.Esito.KO);
            DocsPaVO.utente.InfoUtente infoUtente = Utils.CheckAuthentication(request, "");
            BusinessLogic.UserLog.UserLog.WriteLog(infoUtente, "DOCUMENTOSAVEDOCUMENTO", request.IdDocument, "Modificati i dati del documento " + request.IdDocument, esito);
        ...
        public Services.Documents.SendDocument.SendDocumentResponse SendDocument(Services.Documents.SendDocument.SendDocumentRequest request)
        {
            ...
            // Il log deve essere spostato nel metodo perché non stampa i log dei documenti indicati con segnatura.
            //
            //DocsPaVO.Logger.CodAzione.Esito esito = (response != null && response.Success ? DocsPaVO.Logger.CodAzione.Esito.OK : DocsPaVO.Logger.CodAzione.Esito.KO);
            //DocsPaVO.utente.InfoUtente infoUtente = Utils.CheckAuthentication(request, "");

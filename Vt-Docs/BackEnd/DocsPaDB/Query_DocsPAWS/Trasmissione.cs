            ...
            try
            {
                using (DocsPaDB.DBProvider dbProvider = new DBProvider())
                {
                    ...
                    if (dbProvider.ExecuteNonQuery(queryString))
                    {
                        ...
                        q.setParam("param1", thing + "," + idPersonOrGroup + "," + accessRights + "," + idGruppoTrasm + ",'" + tipoDiritto + "','" + note + "'," + DocsPaDbManagement.Functions.Functions.ToDate(DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss")) + ",'" + idPerson + "','" + idRuolo + "', NULL");
                        queryString = q.getSQL();
                        logger.Debug(queryString);
                        retValue = dbProvider.ExecuteNonQuery(queryString);
                    }

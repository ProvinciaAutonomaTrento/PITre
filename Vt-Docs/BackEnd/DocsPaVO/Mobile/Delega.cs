...
namespace DocsPaVO.Mobile
{
    public class Delega : DelegaInfo
    {
        ...
        public static Delega buildInstance(InfoDelega input, Ruolo ruoloDelegato)
        {
            ...
            if (res.DataDecorrenza.CompareTo(DateTime.Now) > 0) res.Stato = StatoDelega.IMPOSTATA;

            if (!string.IsNullOrEmpty(input.dataScadenza))
            {
                res.DataScadenza = toDate(input.dataScadenza);
                if (res.DataScadenza.CompareTo(DateTime.Now) < 0) res.Stato = StatoDelega.SCADUTA;
            }

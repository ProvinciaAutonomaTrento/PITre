...
namespace BusinessLogic.Fascicoli
{
    ...
    public class FolderManager
    {
        ...
        public static bool addDocFolder(DocsPaVO.utente.InfoUtente infoUtente, string idProfile, string idFolder, bool fascRapida, out string msg)
        {
            ...
            using (DocsPaDB.TransactionContext transactionContext = new DocsPaDB.TransactionContext())
            {
                ...
                */
                ...
                else if (!string.IsNullOrEmpty(BusinessLogic.Documenti.AllegatiManager.getIdDocumentoPrincipale(new DocsPaVO.documento.Allegato() { docNumber = idProfile })))
                {
                    msg = string.Format("Il documento con ID {0} è un allegato per cui non può essere inserito nel folder con ID {1}", idProfile, idFolder);
                    result = false;
                }
                else
                {
        ...
        public static bool getCountFullSottofascDoc(string idProject, out string nDoc)
        {
            bool result = false;
            nDoc = "";
            try
            {
                logger.Debug("getCountFullSottofascDoc");
                DocsPaDB.Query_DocsPAWS.Fascicoli fascicoli = new DocsPaDB.Query_DocsPAWS.Fascicoli();
                result = fascicoli.GetCountfascAndFolderDoc(idProject, out nDoc);
            }
            catch (Exception e)
            {
                logger.Debug("Errore nel conteggio dei documenti nel fascicolo e suoi figli. (getCountFullSottofascDoc)", e);
                result = false;
            }
            return result;
        }

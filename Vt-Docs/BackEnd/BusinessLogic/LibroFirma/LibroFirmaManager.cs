...
namespace BusinessLogic.LibroFirma
{
    public class LibroFirmaManager
    {
        ...
        /// <param name="infoUtente"></param>
        /// <returns></returns>
        public static List<ProcessoFirma> GetProcessesSignatureVisibleRole(bool asProponente, bool asMonitoratore, bool includiVisibilitaRimosse, DocsPaVO.utente.InfoUtente infoUtente)
        {
            List<ProcessoFirma> listProcessiDiFirma = new List<ProcessoFirma>();
            try
            {
                DocsPaDB.Query_DocsPAWS.LibroFirma libroFirma = new DocsPaDB.Query_DocsPAWS.LibroFirma();
                listProcessiDiFirma = libroFirma.GetProcessesSignatureVisibleRole(asProponente, asMonitoratore, includiVisibilitaRimosse, infoUtente);
            }
            catch (Exception e)
            {
        ...
        /// <param name="infoUtente"></param>
        /// <returns></returns>
        public static bool UpdateTipoVisibilitaProcesso(VisibilitaProcessoRuolo visibilita, DocsPaVO.utente.InfoUtente infoUtente)
        {
            bool retValue = true;
            try
            {
                DocsPaDB.Query_DocsPAWS.LibroFirma libroFirma = new DocsPaDB.Query_DocsPAWS.LibroFirma();
                retValue = libroFirma.UpdateTipoVisibilitaProcesso(visibilita, infoUtente);
            }
            catch (Exception e)
            {
        ...
        public static bool SalvaStatoAutomaticoLF(DocsPaVO.DiagrammaStato.Stato statoDoc, DocsPaVO.DiagrammaStato.DiagrammaStato diagramma, string docnumber, DocsPaVO.utente.InfoUtente infoUtente)
        {
            bool retVal = true;
            try
            {
                if (statoDoc != null)
                {
                    SalvaStatoDiagrammaDoc(statoDoc, diagramma, docnumber, infoUtente);
       ...
       /// <summary>
                ...
                using (DocsPaDB.TransactionContext transactionContext = new DocsPaDB.TransactionContext())
                {
                    ...
                    if (libroFirma.EliminaElementoInLibroFirma(idPassoInAttesa))
                    {
                        ...
                        if (result)
                        {
                            ...
                            ModificaStatoDocumentoPerInterruzione(istanza, infoUtente);
        ...
        public static void ModificaStatoDocumentoPerInterruzione(IstanzaProcessoDiFirma istanza, DocsPaVO.utente.InfoUtente infoUtente)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(istanza.IdStatoInterruzione))
                {
                    ...
                    if (!istanza.IdStatoInterruzione.Equals(statoDocumento.SYSTEM_ID.ToString()))
                    {
                        ...
                        SalvaStatoDiagrammaDoc(statoInterruzione, diagramma, istanza.docNumber, infoUtente);
                    }
                }
                ...
                using (DocsPaDB.TransactionContext transactionContext = new DocsPaDB.TransactionContext())
                {
                    ...
                    if (result && elemento.Modalita.Equals("A"))
                    {
                        ...
                        if (result)
                        {
                            ...
                            IstanzaProcessoDiFirma istanza = libroFirma.GetIstanzaProcessoDiFirmaByIdIstanzaProcessoLite(elemento.IdIstanzaProcesso, infoUtente);
                            ModificaStatoDocumentoPerInterruzione(istanza, infoUtente);
        ...
        private static bool IsRoleAuthorizedFunctions(string funzione, DocsPaVO.utente.Ruolo ruolo)
        {
            try
            {
                if (ruolo == null)
                    return false;
                String tipologiaUpper = funzione.ToUpper();
                return (from function in ruolo.funzioni.Cast<DocsPaVO.utente.Funzione>() where function.codice.ToUpper().Equals(tipologiaUpper) select function.systemId).Count() > 0 ? true : false;
            }
            catch (System.Exception ex)
            {
                return false;
        ...
        private static bool CheckAuthorizationPasso(ProcessoFirma processo, DocsPaVO.utente.Ruolo ruolo, out ResultProcessoFirma esito)
        {
            bool retVal = true;
            esito = ResultProcessoFirma.OK;
            ...
            try
            {
                ...
                {
                    if(!IsRoleAuthorizedFunctions("DO_CREATE_MODEL_PROCESS", ruolo))
                    {
                        esito = ResultProcessoFirma.RUOLO_NON_ABILITATO_A_CREAZIONE_MODELLI;
                        return false;
                    }
                }
                ...
                foreach(PassoFirma passo in processo.passi)
                {
                    if(passo.IsAutomatico)
                    {
                        ...
                        {
                            case Azione.RECORD_PREDISPOSED:
                                if (!IsRoleAuthorizedFunctions("CREA_PASSO_PROTO_AUTO", ruolo))
                                    esito = ResultProcessoFirma.RUOLO_NON_ABILITATO_A_CREAZIONE_PASSO_PROTO_AUTO;
                                break;
                            case Azione.DOCUMENTOSPEDISCI:
                                if (!IsRoleAuthorizedFunctions("CREA_PASSO_SPEDIZIONE_AUTO", ruolo))
                                    esito = ResultProcessoFirma.RUOLO_NON_ABILITATO_A_CREAZIONE_PASSO_SPEDIZIONE_AUTO;
                                break;
                            case Azione.DOCUMENTO_REPERTORIATO:
                                if (!IsRoleAuthorizedFunctions("CREA_PASSO_REPERTORIAZIONE_AUTO", ruolo))
            ...
            {
                logger.Error("Errore in CheckAuthorizationPasso " + e.Message);
        ...
        public static List<CopiaProcessiFirmaResult> CopiaProcessiFirma(List<ProcessoFirma> processiFirma, bool copiaVisibilita, bool mantieniInRuoloOrigine, string idRuoloDest, string idPeopleDest, DocsPaVO.utente.InfoUtente utenteOrigine)
        {
            DocsPaDB.Query_DocsPAWS.LibroFirma libroFirma = new DocsPaDB.Query_DocsPAWS.LibroFirma();
            List<CopiaProcessiFirmaResult> esitoCopiaProcessi = new List<CopiaProcessiFirmaResult>();
            ResultProcessoFirma resultCreazioneProcesso;
            ProcessoFirma newProcesso;
            string idProcessoNew = string.Empty;
            string idProcessoOld = string.Empty;
            DocsPaVO.utente.Ruolo ruolo = BusinessLogic.Utenti.UserManager.getRuoloByIdGruppo(idRuoloDest);              
            if (!mantieniInRuoloOrigine)
            {
                //Modifico l'idGruppo del ruolo disegnatore
                foreach(ProcessoFirma processo in processiFirma)
                {
                    resultCreazioneProcesso = ResultProcessoFirma.OK;
                    try
                    {
                        bool checkAutorization = CheckAuthorizationPasso(processo, ruolo, out resultCreazioneProcesso);
                        if (checkAutorization)
                            if (!libroFirma.SpostaProcessoFirma(processo.idProcesso, idRuoloDest, idPeopleDest))
                                resultCreazioneProcesso = ResultProcessoFirma.EXISTING_PROCESS_NAME;
                    }
                    catch(Exception e)
                    {
                        logger.Error("Errore in CopiaProcessiFirma nella copia del processo di firma " + processo.idProcesso + ". " + e.Message);
                        resultCreazioneProcesso = ResultProcessoFirma.KO;
                    }
                    esitoCopiaProcessi.Add(new CopiaProcessiFirmaResult() { idProcesso = processo.idProcesso, nomeProcesso = processo.nome, esito = resultCreazioneProcesso });
                }
            }
            else //Devo creare i nuovi processi per il ruolo selezionato
            {
                DocsPaVO.utente.InfoUtente utenteDest = new DocsPaVO.utente.InfoUtente()
                {
                    idGruppo = idRuoloDest,
                    idPeople = idPeopleDest,
                    idAmministrazione = utenteOrigine.idAmministrazione
                };
                foreach(ProcessoFirma processo in  processiFirma)
                {
                    resultCreazioneProcesso = ResultProcessoFirma.OK;
                    using (DocsPaDB.TransactionContext transactionContext = new DocsPaDB.TransactionContext())
                    {
                        try
                        {
                            ...
                            if (checkAutorization)
                            {
                                newProcesso = new ProcessoFirma()
                                {
                                    nome = processo.nome,
                                    IsProcessModel = processo.IsProcessModel,
                                    isInvalidated = processo.isInvalidated
                                };
                                idProcessoOld = processo.idProcesso;
                                idProcessoNew = InsertProcessoDiFirma(newProcesso, utenteDest, out resultCreazioneProcesso).idProcesso;
                                if (!string.IsNullOrEmpty(idProcessoNew) && resultCreazioneProcesso.Equals(ResultProcessoFirma.OK))
                                {
                                    if (!DuplicaPassiFirmaByIdProcesso(idProcessoNew, idProcessoOld, utenteDest))
                                        resultCreazioneProcesso = ResultProcessoFirma.KO;
                                    else
                                    {
                                        if (copiaVisibilita && !CopiaVisibilitaByIdProcesso(idProcessoNew, idProcessoOld, utenteDest))
                                            resultCreazioneProcesso = ResultProcessoFirma.KO;
                                    }
                                }
                            }
            ...
            return esitoCopiaProcessi;
        }

        public static ProcessoFirma DuplicaProcessoFirma(ProcessoFirma processoOld, string nomeNuovoProcesso, bool copiaVisibilita, DocsPaVO.utente.InfoUtente utente, out DocsPaVO.LibroFirma.ResultProcessoFirma resultCreazioneProcesso)
        {
            resultCreazioneProcesso = ResultProcessoFirma.OK;
            ...
            using (DocsPaDB.TransactionContext transactionContext = new DocsPaDB.TransactionContext())
            {
                try
                {
                    ...
                    newProcesso.IsProcessModel = processoOld.IsProcessModel;
                    newProcesso.isInvalidated = processoOld.isInvalidated;
                    ...
                    {
                        if (!DuplicaPassiFirmaByIdProcesso(newProcesso.idProcesso, processoOld.idProcesso, utente))
                        {
                            resultCreazioneProcesso = ResultProcessoFirma.KO;
                        }
                        else
                        {
                            if (copiaVisibilita && !CopiaVisibilitaByIdProcesso(newProcesso.idProcesso, processoOld.idProcesso, utente))
                                resultCreazioneProcesso = ResultProcessoFirma.KO;
                        }
                    }
                    if (resultCreazioneProcesso.Equals(ResultProcessoFirma.OK))
        ...
        public static AnagraficaEventi GetAnagraficaEventoByCodice(string codiceAzione)
        {
            try
            {
                DocsPaDB.Query_DocsPAWS.LibroFirma libroFirma = new DocsPaDB.Query_DocsPAWS.LibroFirma();
                return libroFirma.GetAnagraficaEventoByCodice(codiceAzione);
            }
            catch (Exception e)
            {
                logger.Error("Errore in BusinessLogic.LibroFirma.LibroFirmaManager  - metodo: GetAnagraficaEventoByCodice ", e);
                return null;
            }
        }
            ...
            try
            {
                ...
                switch (codiceEvento)
                {
                    ...
                    case Azione.DOCUMENTOSPEDISCI:
                        ...
                        break;
                    case Azione.DOC_CAMBIO_STATO:
                        result = CambioStato(docnumber, passo, infoUtente, ruolo, out error);
                        if (result)
                        {
                            //Se il processo era in errore e si sta procedendo con un ritentativo di esecuzione, aggiorno lo stato
                            if (istanzaProcesso.statoProcesso == TipoStatoProcesso.IN_ERROR)
        ...
        private static bool CambioStato(string docnumber, IstanzaPassoDiFirma passo, DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.utente.Ruolo ruolo, out string error)
        {
            bool result = false;
            error = string.Empty;
            DocsPaDB.Query_DocsPAWS.DiagrammiStato diag = new DocsPaDB.Query_DocsPAWS.DiagrammiStato();
            ...
            if(!SalvaStatoAutomaticoLF(stato, diagramma, docnumber, infoUtente))
            {
        ...
        public static bool IsProcessoFirmaConCambioStato(string idProcesso)
        {
            try
            {
                DocsPaDB.Query_DocsPAWS.LibroFirma libroFirma = new DocsPaDB.Query_DocsPAWS.LibroFirma();
                return libroFirma.IsProcessoFirmaConCambioStato(idProcesso);
            }
            catch (Exception e)
            {
                logger.Debug("Errore in BusinessLogic.LibroFirma.LibroFirmaManager  - metodo: IsProcessoFirmaConCambioStato ", e);
                return false;
            }
        }

        public static bool IsIstanzaProcessoFirmaConCambioStato(string idIstanzaProcesso)
        {
            try
            {
                DocsPaDB.Query_DocsPAWS.LibroFirma libroFirma = new DocsPaDB.Query_DocsPAWS.LibroFirma();
                return libroFirma.IsIstanzaProcessoFirmaConCambioStato(idIstanzaProcesso);
            }
            catch (Exception e)
            {
                logger.Debug("Errore in BusinessLogic.LibroFirma.LibroFirmaManager  - metodo: IsModelloDiFirma ", e);
        ...
        public static bool IsDocumentoInLibroFirmaConCambioSato(string idDocumento)
        {
            bool retValue = false;
            DocsPaDB.Query_DocsPAWS.LibroFirma libroFirma = new DocsPaDB.Query_DocsPAWS.LibroFirma();
            try
            {
                string idIstanzaProcesso = libroFirma.GetIdIstanzaProcessoInExec(idDocumento);
                retValue = IsIstanzaProcessoFirmaConCambioStato(idIstanzaProcesso);
            }
            catch (Exception e)
            {
                logger.Debug("Errore in BusinessLogic.LibroFirma.LibroFirmaManager  - metodo: IsDocumentoInLibroFirmaConCambioSato ", e);
                return false;
            }

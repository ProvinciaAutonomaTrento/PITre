...
namespace BusinessLogic.Conservazione
{
    public class ConservazioneManager
    {
            ...
            if (docToSend != null && docToSend.Count > 0)
            {
                ...
                else
                {
                    ...
                    foreach (ItemsVersamento item in docToSend)
                    {
                        ...
                        {
                            logger.Info("Versamento non attivo per l'amministrazione ID=" + item.idAmm);
                            counter++;
                        }
                    }
                }
        ...
        public void ExecuteVersamentoSingolo(string idProfile, string idAmm, bool forzaInvio)
        {
            try
            {
                logger.Debug("BEGIN");
                logger.DebugFormat("Versamento doc ID={0}", idProfile);

                DocsPaDB.Query_DocsPAWS.Conservazione cons = new DocsPaDB.Query_DocsPAWS.Conservazione();
                string statoCons = cons.getStatoConservazione(idProfile);
                if (statoCons == "C" && !forzaInvio)
                {
                    logger.Debug("Il documento è già preso in carico");
                }
                else
                {
                    // Responsabile conservazione
                    Utente u = Utenti.UserManager.getUtenteById(this.GetIdUtenteRespConservazione(idAmm));
                    Ruolo r = Utenti.UserManager.getRuoloByIdGruppo(this.GetIdRuoloRespConservazione(idAmm));
                    if (u == null || r == null)
                    {
                        throw new Exception("Responsabile conservazione non definito");
                    }
                    InfoUtente respCons = Utenti.UserManager.GetInfoUtente(u, r);
                    ...
                    string uri = this.getConfigKey("0", "BE_VERSAMENTO_URL");
                    string versamentoUserName = this.getConfigKey(idAmm, "BE_VERSAMENTO_USER");
                    string versamentoPwd = this.getConfigKey(idAmm, "BE_VERSAMENTO_PWD");
                    string versamentoVer = this.getConfigKey(idAmm, "BE_VERSAMENTO_CURR_VER");
                    string maxTentativi = this.getConfigKey(idAmm, "BE_VERSAMENTO_MAX_TENTATIVI");
                    string maxTentativiStampe = this.getConfigKey(idAmm, "BE_VERSAMENTO_MAX_T_STAMPE");

                    // Costruzione oggetto ItemVersamento
                    ItemsVersamento item = new ItemsVersamento();
                    item.idProfile = idProfile;
                    item.idPeople = respCons.idPeople;
                    item.idGruppo = respCons.idGruppo;
                    item.idAmm = respCons.idAmministrazione;
                    item.tentativiInvio = "0";
                    item.stato = "V";
                    ...
                    string esito = this.getStatoConservazione(idProfile);
                    logger.DebugFormat("STATO CONSERVAZIONE: {0}", esito);
                    //if (esito.Equals("E") || esito.Equals("F"))
                    //{
                    //    logger.Debug("Invio mail");
                    //    this.SendMailFailure(idAmm, esito, idProfile, string.Empty, string.Empty);
                    //}                    
                }
            ...
            {
                logger.Debug("Errore nel versamento: " + ex.Message);
            }
        }
        ...
        public bool VersamentoDoc(ItemsVersamento item, InfoUtente utente, string userName, string pwd, string version, string uri, string maxTentativi, string maxTentativiStampe, SchedaDocumento sch)
        {
            ...
            try
            {
                ...
                if (sch != null)
                {
                    ...
                    string res = DocsPaConsRequest.DocsPaConsRequest.sendRequest(formFields, filesToSend, uri);
        ...
        public string RecuperoDoc(string idDoc, InfoUtente utente)
        {
            ...
            try
            {
                ...
                result = DocsPaConsRequest.DocsPaConsRequest.sendRequest(formFields, null, uri);
        ...
        private XmlDocument createXMLDoc(SchedaDocumento doc, InfoUtente infoUt, string userVersamento, out Dictionary<string, FileDocumento> filesToSend)
        {
            ...
            {
                logger.Debug("File firmato - ricerca info firma");
                List<DocsPaVO.LibroFirma.FirmaElettronica> listaFirme = BusinessLogic.Documenti.FileManager.GetElectronicSignatureDocument(docPrincipale.docNumber, docPrincipale.versionId, infoUt);
                if (listaFirme != null && listaFirme.Count > 0)
                {
                    logger.DebugFormat("{0} firme trovate", listaFirme.Count);
                    foreach (DocsPaVO.LibroFirma.FirmaElettronica firma in listaFirme)
                    {
                        if (!string.IsNullOrEmpty(firma.Xml))
                        {
                            ...
                            sc.SelectSingleNode("NomeComponente").InnerText = sc.SelectSingleNode("NomeComponente").InnerText.Replace("@@", sottocomponenti.ToString());
                            sc.SelectSingleNode("IDComponenteVersato").InnerText = sc.SelectSingleNode("IDComponenteVersato").InnerText.Replace("@@", sottocomponenti.ToString());

                            if (sc.SelectSingleNode("ContentFirma") != null)
                            {
                                sc.RemoveChild(sc.SelectSingleNode("ContentFirma"));
                            }

                            // costruisco il filedocumento da inserire nel dictionary
                            FileDocumento fdFirma = new FileDocumento();
                            fdFirma.estensioneFile = "XML";
                            fdFirma.nomeOriginale = docPrincipale.versionId + "_firma_elettronica_" + sottocomponenti.ToString() + ".xml";
                            fdFirma.fullName = docPrincipale.versionId + "_firma_elettronica_" + sottocomponenti.ToString() + ".xml";
                            fdFirma.content = Encoding.UTF8.GetBytes(firma.Xml);
                            fdFirma.length = fdFirma.content.Length;
                            filesToSend.Add(idSottoComponente, fdFirma);
            ...
            if (!(BusinessLogic.Documenti.FileManager.getExtFileFromPath(fileDocPrincipale.name).ToUpper().Contains("TSD")) && !isTSD)
            {
                ...
                if (timestampArray != null && timestampArray.Count > 0)
                {
                    ...
                    foreach (DocsPaVO.documento.TimestampDoc ts in timestampArray)
                    {
                        ...
                        if (sc.SelectSingleNode("ContentMarca") != null)
                        {
                            sc.RemoveChild(sc.SelectSingleNode("ContentMarca"));
                        }
                            ...
                            if (!(doc.tipoProto.Equals("A") && !string.IsNullOrEmpty(doc.interop) && doc.interop.Equals("S") && all.descrizione.ToUpper().Contains("ALLEGATO SEGNATURA")))
                            {
                                ...
                                if (element.SelectSingleNode("StrutturaOriginale/Componenti/Componente/SottoComponenti") != null)
                                {
                                    ...
                                    foreach (XmlNode n in sc)
                                    {
                                        ...
                                        if (n.SelectSingleNode("ContentFirma") != null && !string.IsNullOrEmpty(n.SelectSingleNode("ContentFirma").InnerText))
                                        {
                                            string xmlFirma = n.SelectSingleNode("ContentFirma").InnerText;
                                            logger.Debug("XML FIRMA ALLEGATO - " + xmlFirma);

                                            // costruisco il filedocumento da inserire nel dictionary
                                            FileDocumento fdFirma = new FileDocumento();
                                            fdFirma.estensioneFile = "XML";
                                            fdFirma.nomeOriginale = all.versionId + "_firma_elettronica_" + sottocomponentiAll.ToString() + ".xml";
                                            fdFirma.fullName = all.versionId + "_firma_elettronica_" + sottocomponentiAll.ToString() + ".xml";
                                            fdFirma.content = Encoding.UTF8.GetBytes(xmlFirma);
                                            fdFirma.length = fdFirma.content.Length;
                                            filesToSend.Add(idSc, fdFirma);

                                            n.RemoveChild(n.SelectSingleNode("ContentFirma"));
                                        }

                                        if (n.SelectSingleNode("ContentMarca") != null && !string.IsNullOrEmpty(n.SelectSingleNode("ContentMarca").InnerText))
                                        {
                                            string stringMarca = n.SelectSingleNode("ContentMarca").InnerText;
                                            string nomeFileMarca = n.SelectSingleNode("NomeComponente").InnerText;
                                            logger.Debug("MARCA TEMPORALE ALLEGATO - " + stringMarca);

                                            // costruisco il filedocumento da inserire nel dictionary
                                            FileDocumento fdMarca = new FileDocumento();
                                            fdMarca.estensioneFile = "TSR";
                                            fdMarca.nomeOriginale = nomeFileMarca;
                                            fdMarca.fullName = nomeFileMarca;
                                            fdMarca.content = Encoding.UTF8.GetBytes(stringMarca);
                                            fdMarca.length = fdMarca.content.Length;
                                            filesToSend.Add(idSc, fdMarca);

                                            n.RemoveChild(n.SelectSingleNode("ContentMarca"));
            ...
            if (doc.firmato.Equals("1"))
            {
                ...
                if (listaFirme != null && listaFirme.Count > 0)
                {
                    ...
                    foreach (DocsPaVO.LibroFirma.FirmaElettronica firma in listaFirme)
                    {
                        if (!string.IsNullOrEmpty(firma.Xml))
                        {
                            XmlElement sc = this.AddSottoComponenteFirma(ref xml, doc);
                            if (sc.SelectSingleNode("ContentFirma") != null)
                            {
                                sc.SelectSingleNode("ContentFirma").InnerText = firma.Xml;
                            }
        ...
        private XmlElement AddSottoComponente(ref XmlDocument xml, DocsPaVO.documento.FileRequest doc, DocsPaVO.documento.TimestampDoc ts)
        {
            ...
            XmlElement formato = xml.CreateElement("FormatoFileVersato");
            XmlElement idVersione = xml.CreateElement("IDComponenteVersato");
            XmlElement contentMarca = xml.CreateElement("ContentMarca"); // valorizzato fuori con stringa xml
            ...
            contentMarca.InnerText = ts.TSR_FILE;
            ...
            element.AppendChild(formato);
            element.AppendChild(idVersione);
            element.AppendChild(contentMarca);
        ...
        private XmlElement AddSottoComponenteFirma(ref XmlDocument xml, DocsPaVO.documento.FileRequest doc)
        {
            ...
            element.AppendChild(formato);
            element.AppendChild(idVersione);
            element.AppendChild(contentFirma);
            ...
            */
            ...
            switch (tipo)
            {
                ...
                case TipologiaUnitaDocumentaria.DocRepertoriato:
                    ...
                    logger.Debug("ID AOO/RF: " + oggContatore.ID_AOO_RF);
                    ...
                    else if (oggContatore.TIPO_CONTATORE.Equals("A"))
                    {
                    ...
                    else if (oggContatore.TIPO_CONTATORE.Equals("R"))
                    {
        ...
        public TipologiaUnitaDocumentaria getTipoDocumento(SchedaDocumento doc, string codiceAmm)
        {
            ...
            if (doc.tipoProto.Equals("A") || doc.tipoProto.Equals("P") || doc.tipoProto.Equals("I"))
            {
                ...
                else
                {
                    ...
                    if (contatore != null && contatore.CONS_REPERTORIO != null && contatore.CONS_REPERTORIO.Equals("1") && string.IsNullOrEmpty(contatore.DATA_ANNULLAMENTO))
        ...
        public bool CtrlXMLFattura(DocsPaVO.documento.FileRequest fr, InfoUtente infoUt, SchedaDocumento doc)
        {
            ...
            try
            {
                ...
                {
                    FileDocumento file1 = null;
                    if (fr.fileName.ToUpper().EndsWith("XML.P7M"))
                    {
                        file1 = BusinessLogic.Documenti.FileManager.getFile(fr, infoUt);
                    }
                    else
                    {
                    ...
                    {
                        logger.Debug("Eccezione gestita - Errore bomUTF8");
                        string byteOrderMarkUtf8 = Encoding.UTF8.GetString(Encoding.UTF8.GetPreamble());
                        if (stringaXml.StartsWith(byteOrderMarkUtf8))
                        {
                            stringaXml = stringaXml.Remove(0, byteOrderMarkUtf8.Length);
                        ...
                        catch(Exception exUTF16)
                        {
                            logger.Debug("Eccezione gestita - conversione UTF-16");
                            stringaXml = System.Text.Encoding.Unicode.GetString(file1.content);
                            stringaXml = stringaXml.Trim();
                            logger.Debug("Stringa XML (UTF16): " + stringaXml);
                            try
                            {
                                xmlDoc.LoadXml(stringaXml);
                            }
                            catch (Exception bomUTF16)
                            {
                                logger.Debug("Eccezione gestita - controllo presenza byteOrderMark UTF16");
                                string byteOrderMarkUtf16 = Encoding.Unicode.GetString(Encoding.Unicode.GetPreamble());
                                if (stringaXml.StartsWith(byteOrderMarkUtf16))
                                {
                                    logger.Debug("bomUTF16 presente");
                                    stringaXml = stringaXml.Remove(0, byteOrderMarkUtf16.Length);
                                }
                                logger.Debug("Ultimo tentativo di caricamento caricamento");
                                xmlDoc.LoadXml(stringaXml);
                            }
                        }
                    }
        ...
        public void SendMailFailure(string idAmm, string stato, string idDoc)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(config.Subject))
                {
                    ...
                    {
                        case "E":
                            subject = string.Format("Errore nell'invio documento ID={0}", idDoc);
                            break;
                        case "F":
                            subject = string.Format("Versamento fallito ID={0}", idDoc);
                            break;
                        case "R":
                            subject = string.Format("Rifiuto versamento ID={0}", idDoc);
                            break;
                    }
                }
                else
                ...
                {
                    string nomeRepertorio = string.Empty;
                    string dataStampa = string.Empty;


                    if (this.IsRepertorio(idDoc, out nomeRepertorio))
                    {
                        body = body.Replace("#TIPO#", string.Format("repertorio {0}", nomeRepertorio));
                        body = body.Replace("#TIPO2#", "repertoriati");
                        subject = subject.Replace("#TIPO#", "repertorio");
                        DocsPaVO.areaConservazione.StampaRegistro info = this.getInfoStampaRepertorio(idDoc);
                        if (info != null)
                            dataStampa = info.dtaStampaTruncString;
                    }
                    else
                    {
                        subject = subject.Replace("#TIPO#", "protocollo");
                        body = body.Replace("#TIPO#", "protocollo");
                        body = body.Replace("#TIPO2#", "protocollati");
                        DocsPaVO.areaConservazione.StampaRegistro info = this.getInfoStampaRegistro(idDoc);
                        if (info != null)
                            dataStampa = info.dtaStampaTruncString;
                    }
                    if (string.IsNullOrEmpty(dataStampa))
                        dataStampa = DateTime.Now.ToString("dd/MM/yyyy");
                ...
                {
                    for (int i = 0; i < config.FixedRecipients.Count(); i++)
                    {
                        if (!string.IsNullOrEmpty(msgBCC))
                            msgBCC = msgBCC + ",";

                        msgBCC = msgBCC + config.FixedRecipients[i];
                    }
                }

                // Destinatari di struttura
                if (stato != "E")
                {
                    if (config.Recipients != null && config.Recipients.Count() > 0)
                    {
                        for (int i = 0; i < config.Recipients.Count(); i++)
                        {
                            ...
                            msgTo = msgTo + config.Recipients[i];
                        }
                    }
                }
                ...
                if (!string.IsNullOrEmpty(msgTo))
                {
                    ...
                    svr.sendMail(config.MailBoxConfiguration.From, msgTo, "", msgBCC, subject, body, BusinessLogic.Interoperabilità.CMMailFormat.HTML, null, out error);
                }
                else
                {
                    svr.sendMail(config.MailBoxConfiguration.From, msgBCC, subject, body);
                }
                logger.Debug("END");
            }
            catch (Exception ex)
        ...
        private ArrayList GetMailRecipientsReg(string idAmm, string idDoc)
        {
            ...
            try
            {
                ...
                string idUtenteRespCons = this.GetIdUtenteRespConservazione(idAmm);
                string mailResp = utente.GetEmailUtente(idUtenteRespCons);
                ...
                StampaRegistro info = this.getInfoStampaRegistro(idDoc);

                if (info != null && !string.IsNullOrEmpty(info.docNumber) && !string.IsNullOrEmpty(info.idRegistro))
                {
                    // Registro di protocollo
                    DocsPaVO.amministrazione.OrgRegistro reg = BusinessLogic.Amministrazione.RegistroManager.GetRegistro(info.idRegistro);
                    if (reg != null && !string.IsNullOrEmpty(reg.idUtenteResp))
                    {
                        string mailRespReg = utente.GetEmailUtente(reg.idUtenteResp);
                        if (!string.IsNullOrEmpty(mailRespReg))
                        {
                            list.Add(mailRespReg);
                        }
                    }
                }
                else
                {
                    info = this.getInfoStampaRepertorio(idDoc);
                    if (info != null && !string.IsNullOrEmpty(info.docNumber) && !string.IsNullOrEmpty(info.idRepertorio))
                    {
                        // Registro di repertorio
                        DocsPaVO.ProfilazioneDinamica.OggettoCustom contatore = BusinessLogic.ProfilazioneDinamica.ProfilazioneDocumenti.getOggettoById(info.idRepertorio);
                        DocsPaVO.utente.Repertori.RegistroRepertorioSingleSettings settings = new DocsPaVO.utente.Repertori.RegistroRepertorioSingleSettings();

                        if (contatore.TIPO_CONTATORE.Equals("T"))
                        {
                            settings = BusinessLogic.utenti.RegistriRepertorioPrintManager.GetRegisterSettings(info.idRepertorio, string.Empty, string.Empty, DocsPaVO.utente.Repertori.RegistroRepertorio.TipologyKind.D, DocsPaVO.utente.Repertori.RegistroRepertorio.SettingsType.S);
                        }
                        else
                        {
                            DocsPaVO.utente.Registro reg = BusinessLogic.Utenti.RegistriManager.getRegistro(info.idRegistro);
                            if (reg.chaRF.Equals("1"))
                            {
                                settings = BusinessLogic.utenti.RegistriRepertorioPrintManager.GetRegisterSettings(info.idRepertorio, string.Empty, reg.systemId, DocsPaVO.utente.Repertori.RegistroRepertorio.TipologyKind.D, DocsPaVO.utente.Repertori.RegistroRepertorio.SettingsType.S);
                            }
                            else
                            {
                                settings = BusinessLogic.utenti.RegistriRepertorioPrintManager.GetRegisterSettings(info.idRepertorio, reg.systemId, string.Empty, DocsPaVO.utente.Repertori.RegistroRepertorio.TipologyKind.D, DocsPaVO.utente.Repertori.RegistroRepertorio.SettingsType.S);
                            }
                        }
                        if (settings != null && !string.IsNullOrEmpty(settings.PrinterUserRespId))
                        {
                            idUtenteRespReg = settings.PrinterUserRespId;
                        }
                    }
                }
                if (!string.IsNullOrEmpty(idUtenteRespReg))
                {
                    string mailRespReg = utente.GetEmailUtente(idUtenteRespReg);
                    if (!string.IsNullOrEmpty(mailRespReg))
                    {
                        list.Add(mailRespReg);
                    }
            ...
            {
                logger.Debug("Errore in GetMailRecipientsReg - ", ex);
            }
        ...
        private bool IsRepertorio(string idDoc, out string repName)
        {
            ...
            try
            {
                StampaRegistro info = this.getInfoStampaRepertorio(idDoc);
                if (info != null && !string.IsNullOrEmpty(info.docNumber) && !string.IsNullOrEmpty(info.idRepertorio))
                {
                    RegistriRepertorioPrintManager manager = new RegistriRepertorioPrintManager();
                    repName = manager.GetNomeTipologia(info.idRepertorio);
                    result = true;
                }
                else
                {
        ...
        private bool ftpUploadSIP(string ftpHost, string ftpUser, string ftpPass, string zipFilePath, string zipCode)
        {
            ...
            try
            {
                ...
                if(!dirExists)
                {
                    ...
                    {
                        logger.Debug("Eccezione gestita - creazione directory non riuscita");
                        logger.Debug("Ricerca status code FTP 550");
                        if(wex.Response != null &&(FtpWebResponse)wex.Response != null)
                        {
                            ftpResponse = (FtpWebResponse)wex.Response;
                            logger.DebugFormat("Risposta: {0} - {1}", ftpResponse.StatusCode, ftpResponse.StatusDescription);

                            if(ftpResponse.StatusCode != FtpStatusCode.ActionNotTakenFileUnavailable)
                            {
                                logger.Debug("Verifica status code fallita");
                                throw wex;
                            }
                        }
                    }                  

...
namespace BusinessLogic.Documenti
{
    ...
    public class DocManager
    {
        ...
        {
            bool result = false;
            SchedaDocumento schedaDoc = getDettaglioNoSecurity(infoUt, schedaDocumento.docNumber);
            using (DocsPaDB.TransactionContext transactionContext = new DocsPaDB.TransactionContext())
            {
                DocsPaDB.Query_DocsPAWS.Documenti doc = new DocsPaDB.Query_DocsPAWS.Documenti();
        ...
        public static ArrayList getTipoAttoPDInsRicWithStateDiagram(string idAmministrazione, string idGruppo, string diritti)
        {
            DocsPaDB.Query_DocsPAWS.Documenti doc = new DocsPaDB.Query_DocsPAWS.Documenti();
            return doc.getTipoAttoPDInsRicWithStateDiagram(idAmministrazione, idGruppo, diritti);
        }
        ...
        public static bool CestinaDocumento(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.documento.SchedaDocumento schedaDoc, string tipoDoc, string note, out string errorMsg)
        {
            ...
            using (DocsPaDB.TransactionContext transactionContext = new DocsPaDB.TransactionContext())
            {
                try
                {
                    ...
                    //Se il documento è conoslidato non posso cestinare
                    DocumentConsolidation.CanExecuteAction(infoUtente, schedaDoc.systemId, DocumentConsolidation.ConsolidationActionsDeniedEnum.DeleteDocument, true);
        ...
        public static string GetDescrizioneTipoDocumento(string typeId)
        {
            DocsPaDB.Query_DocsPAWS.Documenti doc = new DocsPaDB.Query_DocsPAWS.Documenti();
        ...
        public static bool IsDocumentoConforme(string docnumber, InfoUtente infoUtente)
        {
            DocsPaDB.Query_DocsPAWS.Documenti doc = new DocsPaDB.Query_DocsPAWS.Documenti();
            return doc.IsDocumentoConforme(docnumber);
        }

        public static List<InfoFile> GetListInfoFileDocument(string docnumber, InfoUtente infoUtente)
        {
            DocsPaDB.Query_DocsPAWS.Documenti doc = new DocsPaDB.Query_DocsPAWS.Documenti();
            return doc.GetListInfoFileDocument(docnumber);
        }

        public static List<DocsPaVO.documento.FileRequest> GetDocumentiNonConformiDaNotificare()
        {
            DocsPaDB.Query_DocsPAWS.Documenti doc = new DocsPaDB.Query_DocsPAWS.Documenti();
            return doc.GetDocumentiNonConformiDaNotificare();
        }

        public static void NotificaDocumentoNonConforme(DocsPaVO.documento.FileRequest fileRequest, DocsPaVO.utente.InfoUtente infoUtente)
        {
            //Se il documento è un NP oppure è un protocollo in Partenza, Interno o Arrivo ma non trasmite IS, la trasmissione
            //viene inviata al creatore del documento ed il mittente è lo stesso creatore
            //Se il documento è un protocollo in arrivo ricevuto per IS 
            try
            {
                SchedaDocumento schedaDoc = getDettaglioNoSecurity(infoUtente, fileRequest.docNumber);
                InfoDocumento infoDoc = getInfoDocumento(schedaDoc);
                bool result = true;
                DocsPaDB.Query_DocsPAWS.Utenti u = new DocsPaDB.Query_DocsPAWS.Utenti();
                if (schedaDoc != null)
                {
                    Ruolo ruolo = u.GetRuoloByIdGruppo(infoUtente.idGruppo);
                    Utente utente = u.getUtenteById(infoUtente.idPeople);
                    bool verticale = u.IsUtenteVerticale(utente.idPeople);
                    if (!(schedaDoc.tipoProto.Equals("A") && schedaDoc.interop != null && schedaDoc.interop.Equals("S")))
                    {
                        //Se il documento è stato creato da un utente vericale invio solo la mail altrimenti invio una trasmissione al creatore
                        if (verticale)
                            result = InvioEmailDocNonConforme(utente, schedaDoc);
                        else
                            result = EseguiTrasmissioneDocNonConforme(infoDoc, ruolo, utente, ruolo, utente);
                    }
                    else
                    {
                        //Se il documento è stato protocollato trasmetto al solo utente/ruolo protocollatore altrimenti a tutti i ruoli destinatari della trasmissione IS
                        if (schedaDoc.protocollo != null && !string.IsNullOrEmpty(schedaDoc.protocollo.segnatura))
                        {
                            //trasmetto al protocollatore
                            Ruolo ruoloDest = u.GetRuolo(schedaDoc.protocollatore.ruolo_idCorrGlobali);
                            Utente utenteDest = u.getUtenteById(schedaDoc.protocollatore.utente_idPeople);
                            result = EseguiTrasmissioneDocNonConforme(infoDoc, ruolo, utente, ruoloDest, utenteDest);
                        }
                        else
                        {
                            //Se non è stato ancora protocollato devo notificare a tutti i ruoli destinatari della trasmissione IS
                            DocsPaDB.Query_DocsPAWS.Trasmissione trasm = new DocsPaDB.Query_DocsPAWS.Trasmissione();
                            List<string> destinatari = trasm.GetDestTrasmissioneInteroperabilitaSemplificata(schedaDoc.docNumber);
                            if(destinatari != null && destinatari.Count > 0)
                                result = EseguiTrasmissioneDocNonConforme(infoDoc, ruolo, utente, destinatari);
                        }
                    }
                    if (result)
                    {
                        //Aggiorno il cha_notifica a false(non notificare)
                        DocsPaDB.Query_DocsPAWS.Documenti doc = new DocsPaDB.Query_DocsPAWS.Documenti();
                        doc.UpdateChaNotificaInfoFile(schedaDoc.docNumber, false);
                    }
                }
            }
            catch(Exception e)
            {
                logger.Error("Errore in NotificaDocumentoNonConforme " + e.Message);
            }
        }

        private static bool InvioEmailDocNonConforme(Utente utente, SchedaDocumento schedaDoc)
        {
            bool retValue = false;
            string oggetto = "Notifica documento non conforme";
            string body = "Gentile utente, <br/>La presente mail per comunicarle che il documento da lei creato è non conforme.";
            body += "<br/>ID DOCUMENTO: " + schedaDoc.docNumber;
            if (schedaDoc.protocollo != null && !string.IsNullOrEmpty(schedaDoc.protocollo.segnatura))
                body += "<br/>SEGNATURA :" + schedaDoc.protocollo.segnatura;
            body += "<br/>OGGETTO: " + schedaDoc.oggetto.descrizione;
            try
            {
                DocsPaDB.Query_DocsPAWS.Amministrazione amm = new DocsPaDB.Query_DocsPAWS.Amministrazione();
                string fromEmailAmministra = amm.GetEmailAddress(utente.idAmministrazione);
                retValue = BusinessLogic.Interoperabilità.Notifica.notificaByMail(utente.email, fromEmailAmministra, oggetto, body, string.Empty, utente.idAmministrazione, null);
            }
            catch (Exception e)
            {
                logger.Error("Errore in InvioEmailDocNonConforme" + e.Message);
                retValue = false;
            }
        ...
        private static bool EseguiTrasmissioneDocNonConforme(DocsPaVO.documento.InfoDocumento infoDoc, DocsPaVO.utente.Ruolo ruoloMitt, DocsPaVO.utente.Utente utenteMitt, DocsPaVO.utente.Ruolo ruoloDest, DocsPaVO.utente.Utente utenteDest)
        {
            bool retValue = false;
            try
            {
                DocsPaVO.trasmissione.Trasmissione trasm = new DocsPaVO.trasmissione.Trasmissione();
                trasm.ruolo = ruoloMitt;
                trasm.utente = utenteMitt;
                trasm.infoDocumento = infoDoc;
                trasm.noteGenerali = "Documento non conforme";

                //INSERISCO LA RAGIONE DI TRASMISSIONE DI SISTEMA PER LIBRO FIRMA
                DocsPaDB.Query_DocsPAWS.Utenti utenti = new DocsPaDB.Query_DocsPAWS.Utenti();
                DocsPaVO.trasmissione.RagioneTrasmissione ragione = Trasmissioni.RagioniManager.GetRagioneByTipoOperazione("NON_CONFORME", utenteMitt.idAmministrazione);
                ...
                trasmSing.corrispondenteInterno = ruoloDest;
                ...
                qc.codiceRubrica = ruoloDest.codiceRubrica;
                System.Collections.ArrayList registri = ruoloDest.registri;
                qc.tipoUtente = DocsPaVO.addressbook.TipoUtente.INTERNO;
                qc.idAmministrazione = ruoloDest.idAmministrazione;
                qc.getChildren = true;
                ...
                //Se l'id utente non è specificato la trasmissione andrà notificata a tutti gli utenti, altrimenti al solo utente specificato
                if (utenteDest == null)
                {
                    for (int k = 0; k < listaUtenti.Count; k++)
                    {
                        ...
                        trUt.daNotificare = (listaUtenti[k] as DocsPaVO.utente.Utente).idPeople.Equals(utenteDest.idPeople);
                ...
                DocsPaVO.trasmissione.Trasmissione result = BusinessLogic.Trasmissioni.ExecTrasmManager.saveExecuteTrasmMethod(urlfrontend, trasm);
                if (result != null && !string.IsNullOrEmpty(result.systemId))
                {
                    retValue = true;
                    //Inserimento Log
                    ...
                    if (result != null)
                    {
                        BusinessLogic.UserLog.UserLog.WriteLog(result.utente.userId, result.utente.idPeople, result.ruolo.idGruppo, result.utente.idAmministrazione, method, result.infoDocumento.docNumber, desc, DocsPaVO.Logger.CodAzione.Esito.OK,
                            null, "1", (result.trasmissioniSingole[0] as DocsPaVO.trasmissione.TrasmissioneSingola).systemId);
                    }
                }
            }
            catch (Exception e)
            {
                logger.Error("Errore in EseguiTrasmissione", e);
                retValue = false;
            }
            return retValue;
        }
        ...
        private static bool EseguiTrasmissioneDocNonConforme(DocsPaVO.documento.InfoDocumento infoDoc, DocsPaVO.utente.Ruolo ruoloMitt, DocsPaVO.utente.Utente utenteMitt, List<string> idCorrGlobaliDest)
        {
            bool retValue = false;
            try
            {
                ...
                //CREO LA TRASMISSIONE SINGOLA
                trasm.trasmissioniSingole = new ArrayList();
                Ruolo ruoloDest;
                DocsPaVO.trasmissione.TrasmissioneSingola trasmSing;
                foreach (string idCorrGlobali in idCorrGlobaliDest)
                {
                    ruoloDest = utenti.GetRuolo(idCorrGlobali);
                    trasmSing = new DocsPaVO.trasmissione.TrasmissioneSingola();
                    trasmSing.ragione = ragione;
                    trasmSing.tipoTrasm = "S";
                    ...
                    trasmSing.trasmissioneUtente = trasmissioniUt;
                    trasm.trasmissioniSingole.Add(trasmSing);
                }
                ...
                DocsPaVO.trasmissione.Trasmissione resultTrasm = BusinessLogic.Trasmissioni.ExecTrasmManager.saveExecuteTrasmMethod(urlfrontend, trasm);
                if (resultTrasm != null && !string.IsNullOrEmpty(resultTrasm.systemId))
                {
                    retValue = true;
                    //Inserimento Log
                    ...
                    foreach (DocsPaVO.trasmissione.TrasmissioneSingola single in resultTrasm.trasmissioniSingole)
                    {
                        ...
                        {
                            if (!BusinessLogic.UserLog.UserLog.WriteLog(resultTrasm.utente.userId, resultTrasm.utente.idPeople,
                            ...
                            {
                                logger.Error("Errore durante la scrittura in DPA_LOG");

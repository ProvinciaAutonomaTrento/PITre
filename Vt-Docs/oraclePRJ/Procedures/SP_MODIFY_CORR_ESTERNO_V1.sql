--------------------------------------------------------
--  DDL for Procedure SP_MODIFY_CORR_ESTERNO_V1
--------------------------------------------------------
set define off;

  CREATE OR REPLACE PROCEDURE "ITCOLL_6GIU12"."SP_MODIFY_CORR_ESTERNO_V1" (
...
BEGIN
   ...
   BEGIN
      ...
      <<reperimento_dati>>
      BEGIN
         SELECT var_cod_rubrica, cha_tipo_urp, id_registro, id_amm,cha_pa, cha_tipo_ie, num_livello, id_parent, id_peso_org, id_uo, id_tipo_ruolo, id_gruppo, InteropUrl
           INTO cod_rubrica, cha_tipourp, id_reg, idamm, chapa, chaTipoIE, numLivello, idParent, idPesoOrg, IdUO, idTipoRuolo, idGruppo, url
...
--VERIFICO se il corrisp e stato utilizzato come dest/mitt di protocolli
         SELECT COUNT (id_profile)
           INTO myprofile
           FROM dpa_doc_arrivo_par
          WHERE id_mitt_dest = idcorrglobale;
          
-- 1) non e stato mai utilizzato come corrisp in un protocollo
         IF /* 2 */ (myprofile = 0)
         THEN
            BEGIN
...
/* SE L'UPDATE SU DPA_CORR_GLOBALI E' ANDTATA A BUON FINE
            ...
            BEGIN
               ...
               INSERT INTO dpa_corr_globali
                           (system_id, num_livello, cha_tipo_ie, id_registro,
                            ...
                            id_gruppo, id_tipo_ruolo, id_uo, InteropUrl
                           )
                    VALUES (newid, numLivello, chaTipoIE, id_reg,
                            ...
                            idGruppo, idTipoRuolo, idUO, SimpInteropUrl
                           );
            EXCEPTION
               WHEN OTHERS
               THEN

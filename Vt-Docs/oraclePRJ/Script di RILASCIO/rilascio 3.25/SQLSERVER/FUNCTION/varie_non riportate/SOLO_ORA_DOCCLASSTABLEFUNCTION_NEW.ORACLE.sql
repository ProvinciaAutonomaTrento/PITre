...
---- DOCCLASSTABLEFUNCTION_NEW.ORA.sql  marcatore per ricerca ----
begin 
Utl_Backup_Plsql_code ('FUNCTION','DOCCLASSTABLEFUNCTION_NEW'); 
end;
/
...
begin
	...
	where Object_Type= 'TYPE' and Object_Name= 'DOCCLASSTABLETYPE_P';

	if cnt = 0 then 
	execute immediate 
		'create or replace TYPE DOCCLASSTABLETYPE_P 
		IS OBJECT (
			id_amm       INTEGER,
			id_registr   INTEGER,
			id_anno      INTEGER,
			sede         VARCHAR(256),
			titolario    INTEGER,
			tot_doc_class       INTEGER,
			cod_class           VARCHAR (255),
			desc_class          VARCHAR (3000),
...
begin
	select count(*) into cnt from user_objects 
	where Object_Type= 'TYPE' and Object_Name= 'DOCCLASSTABLEROW_P';

	if cnt = 0 then 
	execute immediate 
		'create or replace TYPE DOCCLASSTABLEROW_P 
				IS TABLE OF docclasstabletype_P '; 
	end if;
end;
end;
/
...
create or replace
FUNCTION DOCCLASSTABLEFUNCTION_NEW (
   id_amm       NUMBER,
   id_registr   NUMBER,
 --  id_anno      NUMBER,
...
IS
   ...
   IS
      Select   Pr.System_Id As Sysid, Pr.Description As Descrizione,
               tit.Description||' - '||Pr.Var_Codice As Var_Codice, Pr.Num_Livello As Livello
...
-- rispetto alla versione compatta si puo togliere il filtro su num_livello
      ...
      ORDER BY tit.Description, pr.var_cod_liv1;

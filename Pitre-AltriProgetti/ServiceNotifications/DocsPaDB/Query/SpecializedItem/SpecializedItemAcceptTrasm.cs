...
{
    public class SpecializedItemAcceptTrasm : DBProvider, ISpecializedItem
    {
        private static ILog logger = LogManager.GetLogger(typeof(SpecializedItemAcceptTrasm));
        /// <summary>
        /// 
        /// </summary>
        /// <param name="e"></param>
            ...
            try
            {
                ...
                switch (e.DOMAIN_OBJECT)
                {
                    case SupportStructures.ListDomainObject.DOCUMENT:
                        q = DocsPaUtils.InitQuery.getInstance().getQuery("S_ITEMS_SPECIALIZED_ACCEPT_TRASM_DOCUMENT");
                    ...
                    case SupportStructures.ListDomainObject.FASCICOLO:
                        q = DocsPaUtils.InitQuery.getInstance().getQuery("S_ITEMS_SPECIALIZED_ACCEPT_TRASM_FOLDER");
                        q.setParam("idTrasmSing", e.ID_SPECIALIZED_OBJECT.ToString());
                        q.setParam("idUser", e.ACTORS.PRODUCER.IDUSER.ToString());
                        queryString = q.getSQL();
        ...
        public void CreateSpecializedItemObject(DataSet ds, ref string specializedItem)
        {
            ...
            try
            {
                ...
                if(dr != null)
                {
                    ...
                    {
                        strbuilder.Append(SupportStructures.TagItem.LINE + SupportStructures.TagItem.LABEL + 
                            "lblAcceptNote" + SupportStructures.TagItem.CLOSE_LABEL + dr["NOTE"].ToString() + SupportStructures.TagItem.CLOSE_LINE);
                    }
                    if (!isFolder)
                    {

...
{
    public class SpecializedItemChangeState : DBProvider, ISpecializedItem
    {
        private static ILog logger = LogManager.GetLogger(typeof(SpecializedItemChangeState));
        /// <summary>
        /// 
        /// </summary>
        /// <param name="e"></param>
            ...
            try
            {
                ...
                switch (e.DOMAIN_OBJECT)
                {
                    case SupportStructures.ListDomainObject.DOCUMENT:
                        q = DocsPaUtils.InitQuery.getInstance().getQuery("S_ITEMS_SPECIALIZED_CHANGE_STATE_DOCUMENT");
                    ...
                    case SupportStructures.ListDomainObject.FASCICOLO:
                        q = DocsPaUtils.InitQuery.getInstance().getQuery("S_ITEMS_SPECIALIZED_CHANGE_STATE_FOLDER");
                        queryString = q.getSQL();
                        this.ExecuteQuery(out ds, "SpecializedItem", q.getSQL());
                        break;
                }
        ...
        public void CreateSpecializedItemObject(DataSet ds, ref string specializedItem)
        {
            try
            {
                ...
                if (ds.Tables["SpecializedItem"] != null && ds.Tables["SpecializedItem"].Rows.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(dr["DESC_STATE"].ToString()))
                    {
                        strbuilder.Append(SupportStructures.TagItem.LINE + SupportStructures.TagItem.LABEL +
                            "lblChangeStateDoc" + SupportStructures.TagItem.CLOSE_LABEL + dr["DESC_STATE"].ToString() + SupportStructures.TagItem.CLOSE_LINE);
                    }
                }

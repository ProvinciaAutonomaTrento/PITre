...
{
    class SpecializedItemAbortRecord : DBProvider, ISpecializedItem
    {

        private static ILog logger = LogManager.GetLogger(typeof(SpecializedItemAbortRecord));
        /// <summary>
        /// 
        /// </summary>
        /// <param name="e"></param>
        ...
        public string CreateSpecializedItem(Event e)
        {
            ...
            try
            {
                ...
                DocsPaUtils.Query q;
                string queryString;
                q = DocsPaUtils.InitQuery.getInstance().getQuery("S_ITEMS_SPECIALIZED_ABORT_RECORD");
        ...
        public void CreateSpecializedItemObject(DataSet ds, ref string specializedItem)
        {
            try
            {
                ...
                if (ds.Tables["SpecializedItem"] != null && ds.Tables["SpecializedItem"].Rows.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(dr["DTA_ABORT_RECORD"].ToString()))
                    {
                        strbuilder.Append(SupportStructures.TagItem.LINE + SupportStructures.TagItem.LABEL +
                            "lblDtaAbortRecord" + SupportStructures.TagItem.CLOSE_LABEL + dr["DTA_ABORT_RECORD"].ToString() + SupportStructures.TagItem.CLOSE_LINE);
                    }
                    if (!string.IsNullOrEmpty(dr["DESC_ABORT_RECORD"].ToString()))
                    {
                        strbuilder.Append(SupportStructures.TagItem.LINE + SupportStructures.TagItem.LABEL +
                            "lblDescAbortRecord" + SupportStructures.TagItem.CLOSE_LABEL + dr["DESC_ABORT_RECORD"].ToString() + SupportStructures.TagItem.CLOSE_LINE);
                    }

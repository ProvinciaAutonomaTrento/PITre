...
{
    public class SpecializedItemInstanceAccess : DBProvider, ISpecializedItem
    {
        private static ILog logger = LogManager.GetLogger(typeof(SpecializedItemInstanceAccess));
        ...
        public string CreateSpecializedItem(Event e)
        {
            ...
            try
            {
                ...
                DocsPaUtils.Query q;
                string queryString;
                q = DocsPaUtils.InitQuery.getInstance().getQuery("S_ITEMS_SPECIALIZED_INSTANCE_ACCESS");
        ...
        public void CreateSpecializedItemObject(DataSet ds, ref string specializedItem)
        {
            try
            {
                ...
                if (ds.Tables["SpecializedItem"] != null && ds.Tables["SpecializedItem"].Rows.Count > 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(dr["ESITO_CREAZIONE_FILE_ZIP"].ToString()))
                    {
                        strbuilder.Append(SupportStructures.TagItem.LINE + SupportStructures.TagItem.LABEL +
                            "lblResultCreationFileInstance" + SupportStructures.TagItem.CLOSE_LABEL + dr["ESITO_CREAZIONE_FILE_ZIP"].ToString() + SupportStructures.TagItem.CLOSE_LINE);
                    }
                }

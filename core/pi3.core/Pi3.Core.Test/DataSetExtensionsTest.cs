...
namespace Pi3.Core.Test
{
    ...
    {
        [Test]
        public async Task AsDataSetTest()
        {
            var list = new List<CorrGlobaliEntity>();
            list.Add(new CorrGlobaliEntity() { SYSTEM_ID = 1 });
            list.Add(new CorrGlobaliEntity() { SYSTEM_ID = 2 });
            list.Add(new CorrGlobaliEntity() { SYSTEM_ID = 3 });

            var dataSetName = "DS";
            var tableName = "CorrGlob";
            var dataSet = list.AsDataSet(dataSetName, tableName);

            Assert.True(dataSet != null);
            Assert.True(dataSet.DataSetName == dataSetName);
            Assert.True(dataSet.Tables.Contains(tableName));
            Assert.True(dataSet.Tables[tableName].Rows.Count == list.Count);
        }

        [Test]
        public async Task AsDataTableTest()
        {
            var list = new List<ProfileEntity>();
            list.Add(new ProfileEntity() { SYSTEM_ID = 1 });
            list.Add(new ProfileEntity() { SYSTEM_ID = 2 });
            list.Add(new ProfileEntity() { SYSTEM_ID = 3 });

            var tableName = "Profile";
            var dataTable = list.AsDataTable(tableName);

            Assert.True(dataTable != null);
            Assert.True(dataTable.TableName == tableName);
            Assert.True(dataTable.Rows.Count == list.Count);
        }
    }
}

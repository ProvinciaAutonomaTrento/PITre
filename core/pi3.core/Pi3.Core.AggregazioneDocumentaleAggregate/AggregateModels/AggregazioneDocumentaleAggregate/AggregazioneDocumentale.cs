    ...
    public class AggregazioneDocumentale : ContentElement
    {
            ...
            TipologieVisibilitaEnum? tipologiaVisibilita = null)
        {
            this.ApplyChange(new AggregazioneDocumentaleCreatedEvent()
            {
                Id = id,
                IdTenant = idTenant,
                TypeName = nameof(AggregazioneDocumentale),
        ...
        public virtual void AddClassification(string id, TextValue? name = null, string? idSerieDocumentale = null, TextValue? nameSerieDocumentale = null)
        {
            ...
            if (!String.IsNullOrEmpty(idSerieDocumentale))
        ...
        public virtual void RemoveIdDoc(IdDoc idDoc, string? idFolder = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(idFolder))
            {
                ...
                if(!folder.IdDocs.Any(d => d.Identiticativo == idDoc.Identiticativo))
        ...
        public virtual void AddFolder(string id, TextValue name, IReadOnlyList<IdDoc>? idDocs = null, string? idParent = null)
        {
            ...
            this.ApplyChange(new FolderAddedEvent()
            {
                IdFolder = id,
                Name = name, 
                IdDocs = idDocs,
        ...
        public virtual void RemoveFolder(string id)
        {
            this.AssertAggregazioneDocumentaleInStatoChiuso();
            ...
            if (folder == null)
                throw new FolderNotFoundPi3Exception(id);

            this.ApplyChange(new FolderRemovedEvent()
            {
                IdFolder = id
            });
        }
        ...
        public CollocazioneFisica? CollocazioneFisica
        {
            get
            {
                return this._collocazioneFisica;
            }
        }
        ...
        protected virtual void Handle(FolderAddedEvent @event)
        {
            if (!string.IsNullOrWhiteSpace(@event.IdParent))
            {
                var parentFolder = this.FindFolderById(@event.IdParent);

                parentFolder?.AddFolder(new Folder(@event.IdFolder, @event.Name, @event.IdDocs, parentFolder));
            }
            else
                this._folders.Add(new Folder(@event.IdFolder, @event.Name, @event.IdDocs));
        }

        protected virtual void Handle(FolderRemovedEvent @event)
        {
            var folder = this.FindFolderById(@event.IdFolder);

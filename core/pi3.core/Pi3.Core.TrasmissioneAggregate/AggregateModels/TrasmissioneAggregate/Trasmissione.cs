    ...
    public class Trasmissione : Element
    {
                  ...
                  TextValue? noteGenerali = null)
        {
            ...
            this.ApplyChange(new TrasmissioneCreataEvent()
            {
                ...
                Name = new TextValue(string.Format(Descriptions.Nome, tipoOggettoTrasmesso, idOggettoTrasmesso)),
                Autore = autore,
        ...
        public virtual void Invia(DateTime? dataInvio = null, InviaBehavior? inviaBehavior = null)
        {
            this.AssertTrasmissioneInviata();
        ...
        public virtual void Accetta(string idGruppo, string idUtente, Accetta accetta)
        {
            idGruppo = idGruppo ?? throw new ArgumentNullException(nameof(idGruppo));
            ...
            foreach (var ts in this._trasmissioniSingole.Where(ts => (ts.RagioneTrasmissione.ConWorkflow ?? false)
                    && ((ts.Tipo == TipiTrasmissioneSingolaEnum.Tutti
                    && ts.TrasmissioniUtente.Count(tu => tu.DataAccettazione.HasValue) < ts.TrasmissioniUtente.Count())
                                            || (ts.Tipo == TipiTrasmissioneSingolaEnum.Uno
                    && !ts.TrasmissioniUtente.Any(tu => tu.DataAccettazione.HasValue)))))
            {
                bool doAccetta = false;

                if (ts is TrasmissioneSingolaGruppo
                        && ((TrasmissioneSingolaGruppo)ts).GruppoDestinatario.Id == idGruppo)
                {
                    doAccetta = true;
                }
                else if (ts is TrasmissioneSingolaUtente
                        && ((TrasmissioneSingolaUtente)ts).UtenteDestinatario.Id == idUtente)
                {
                    doAccetta = true;
                }
        ...
        public virtual void Rifiuta(string idGruppo, string idUtente, Rifiuta rifiuta)
        {
            idGruppo = idGruppo ?? throw new ArgumentNullException(nameof(idGruppo));
            idUtente = idUtente ?? throw new ArgumentNullException(nameof(idUtente));
            ...
            foreach (var ts in this._trasmissioniSingole.Where(ts => (ts.RagioneTrasmissione.ConWorkflow ?? false)
                    && ((ts.Tipo == TipiTrasmissioneSingolaEnum.Tutti
                    && ts.TrasmissioniUtente.Count(tu => tu.DataRifiuto.HasValue) < ts.TrasmissioniUtente.Count())
                                            || (ts.Tipo == TipiTrasmissioneSingolaEnum.Uno
                    && !ts.TrasmissioniUtente.Any(tu => tu.DataRifiuto.HasValue)))))
            {
                bool doRifiuta = false;
        ...
        public virtual void Visto(string idGruppo, string idUtente, Visto visto)
        {
            idGruppo = idGruppo ?? throw new ArgumentNullException(nameof(idGruppo));
            idUtente = idUtente ?? throw new ArgumentNullException(nameof(idUtente));

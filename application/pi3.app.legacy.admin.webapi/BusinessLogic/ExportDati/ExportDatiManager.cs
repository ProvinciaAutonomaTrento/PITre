...
namespace BusinessLogic.ExportDati
{
    public class ExportDatiManager
    {
        ...
        public DocsPaVO.documento.FileDocumento ExportTitolarioInExcel(string serverPath, DocsPaVO.amministrazione.OrgTitolario titolario, string idRegistro, ISpreadsheetService spreadsheetService)
        {
            ...
            try
            {
                ...
                var columns = Resources.ColumnsExportTitolario.Split(",").ToList();
                bool isEnableContatoreTitolario = !string.IsNullOrEmpty(DocsPaUtils.Configuration.CustomConfigurationBaseManager.isEnableContatoreTitolario());

                if (isEnableContatoreTitolario)
                    columns.AddRange(new string[] { "CHA_CONTA_PROT_TIT", "NUM_PROT_TIT" });

                int column = 0;
                foreach (string campoSelezionato in columns)
                {
                    sheet.AddCell(new CellModel()
                    {
                        Row = 0,
                        Column = column,
                        ValueAsString = campoSelezionato.Replace("\r\n", ""),
                ...
                if ((ds_nodiDiTitolario.Tables[0] != null) && ds_nodiDiTitolario.Tables[0].Rows.Count > 0)
                {

                    DocsPaDB.Query_DocsPAWS.Amministrazione amm = new DocsPaDB.Query_DocsPAWS.Amministrazione();
                    foreach (DataRow dataRow in ds_nodiDiTitolario.Tables[0].Rows)
                    {
                        ...
                        if (dataRow["ID_AMM"].ToString() != null && dataRow["ID_AMM"].ToString() != "")
                            codiceAmm = amm.GetVarCodiceAmm(dataRow["ID_AMM"].ToString());
                        if (dataRow["ID_REGISTRO"].ToString() != null && dataRow["ID_REGISTRO"].ToString() != "")
                            codiceReg = amm.GetCodiceRegistro(dataRow["ID_REGISTRO"].ToString());
                        if (dataRow["VAR_CODICE"].ToString() != null && dataRow["VAR_CODICE"].ToString() != "")
                        {
                            var codiceClassifica = dataRow["VAR_CODICE"].ToString().Split('.');
                            if (codiceClassifica != null)
                            {
                                for (int i = 0; i < codiceClassifica.Length; i++)
                                {
                                    levels[i] = codiceClassifica[i];
                                }
                            }
                        }
                        ...
                        {
                            { columns[0], codiceAmm ?? string.Empty },
                            { columns[1], codiceReg ?? string.Empty },
                            { columns[2], levels[0] ?? string.Empty },
                            { columns[3], levels[1] ?? string.Empty },
                            { columns[4], levels[2] ?? string.Empty },
                            { columns[5], levels[3] ?? string.Empty },
                            { columns[6], levels[4] ?? string.Empty },
                            { columns[7], levels[5] ?? string.Empty },
                            { columns[8], dataRow["DESCRIPTION"].ToString() ?? string.Empty },
                            { columns[9], dataRow["CHA_RW"].ToString()?.ToUpper() == "W" ? "SI" : "NO" },
                            { columns[10], dataRow["NUM_MESI_CONSERVAZIONE"].ToString() ?? string.Empty },
                            { columns[11], string.Empty },
                            { columns[12], dataRow["CHA_BLOCCA_FIGLI"].ToString() ?? string.Empty },
                            { columns[13], dataRow["CHA_CONSENTI_CLASS"].ToString() ?? string.Empty },
                            { columns[14], dataRow["CHA_CONSENTI_FASC"].ToString() ?? string.Empty }
                        };

                        if (isEnableContatoreTitolario)
                        {
                            values.Add(columns[16], dataRow["CHA_CONTA_PROT_TIT"].ToString() ?? string.Empty);
                            values.Add(columns[17], dataRow["NUM_PROT_TIT"].ToString() ?? string.Empty);
                        };

                        var value = string.Empty;
                        foreach (string campoSelezionato in columns)
                        {
                            if (values.TryGetValue(campoSelezionato, out value))
                            {
                                sheet.AddCell(new CellModel()
                                {
                                    Row = row,
                ...
                var reportGenerated = spreadsheetService.Write(model, stream).Result;

                file = new FileDocumento()
                {
        ...
        public DocsPaVO.documento.FileDocumento ExportPianoCons_IntegrPIS(string idAmministrazione, ISpreadsheetService spreadsheetService )
        {
            DocsPaVO.documento.FileDocumento file = new DocsPaVO.documento.FileDocumento();
            ...
            try
            {
                ...
                var sheet = new SheetModel()
                {
                    Name = "Tabella di confine"
                };

                var columns = Resources.ColumsExportPCPis.Split(",").ToList();
                ...
                {
                    foreach (var dataRow in pianoConservazione)
                    {
                        var values = new Dictionary<string, string>
                        {
                        ...
                        //Colonna Amministrazione
                            ...
                            { columns[2], dataRow.IdAmministrazione},
                        //Colonna Codice Registro
                        
                            { columns[3], (!string.IsNullOrEmpty(dataRow.CodRegistro) ? dataRow.CodRegistro : "") },
                        //Descrizione integrazione
                        
                            { columns[4],  (!string.IsNullOrEmpty(dataRow.DescIntegrazione) ? dataRow.DescIntegrazione : "") },
                        //Colonna CODE APPLICATION
                        
                            { columns[5],  (!string.IsNullOrEmpty(dataRow.CodeApplication) ? dataRow.CodeApplication : "") },
                        //Colonna CLASSIFICAZIONE
                        
                            { columns[6],  (!string.IsNullOrEmpty(dataRow.CodiceClassifica) ? dataRow.CodiceClassifica : "") },
                        //Colonna Tipologia Fascicolo
                        
                            { columns[7],  (!string.IsNullOrEmpty(dataRow.DescPianoConservazione) ? dataRow.DescPianoConservazione : "") },
                        //Colonna Tempi di conservazione
                        
                            { columns[8],  (!string.IsNullOrEmpty(dataRow.TempoConservazione) ? dataRow.TempoConservazione : "") },
                        //Colonna Utente
                        
                            { columns[9],  (!string.IsNullOrEmpty(dataRow.UserIdIntegrazione) ? dataRow.UserIdIntegrazione : "") },
                        //Colonna Ruolo
                        
                            { columns[10],  (!string.IsNullOrEmpty(dataRow.CodRuoloIntegrazione) ? dataRow.CodRuoloIntegrazione : "") },
                        //Colonna Tipo documento
                        
                            { columns[11],  (!string.IsNullOrEmpty(dataRow.DescTipoDoc) ? dataRow.DescTipoDoc : "") },
                        //Colonna Campo profilato fascicolo
                        
                            { columns[12],  (!string.IsNullOrEmpty(dataRow.DescTipoFasc) ? dataRow.DescTipoFasc : "") },
                        };
                    ...
                    file = new FileDocumento()
                {
                    ...
                    contentType = "application/vnd.ms-excel",
                    estensioneFile = "xlsx",
                    name = "ExportPianoCons_IntegrPIS.xlsx"
                };
                ...
                //    file.estensioneFile = "xlsx";
                //    file.name = "ExportPianoCons_IntegrPIS.xlsx";
        ...
        #region Esporta Piano conservazione 

        public DocsPaVO.documento.FileDocumento ExportPianoConservazione(DocsPaVO.amministrazione.OrgTitolario titolario, string idRegistro, ISpreadsheetService spreadsheetService, string serverPath)
        {
            DocsPaVO.documento.FileDocumento file = new DocsPaVO.documento.FileDocumento();
            ...
            try
            {
                ...
                var columns = Resources.ColumnsExportPC.Split(",").ToList();
                ...
                List<DocsPaVO.PianoConservazione> pianoConservazione = BusinessLogic.PianoConservazione.PianoConservazioneManager.GetPianoConservazione(titolario.ID);


                column = 0;
                int row = 1;
                pianoConservazione.ForEach(n =>
                {
                    var values = new Dictionary<string, string>
                    {
                        { columns[0], n.CodiceClassificazione ?? string.Empty },
                        { columns[1], n.VoceProcedimento ?? string.Empty },
                        { columns[2], n.NumeroProcedimento ?? string.Empty },
                        { columns[3], n.TipologiaFascicolo ?? string.Empty },
                        { columns[4], n.TempoConservazione ?? string.Empty },
                        { columns[5], n.NoteChiusuraFascicolo.Replace("\n", " ") ?? string.Empty},
                        { columns[6], n.NoteScartabilitaDocumenti.Replace("\n", " ") ?? string.Empty },
                        { columns[7], n.NoteDocumenti.Replace("\n", " ") ?? string.Empty }
                    };
        ...
        private StringBuilder creaXMLPianoConservazione(List<DocsPaVO.PianoConservazione> pianoConservazione)
        {
            StringBuilder sb = new StringBuilder();
            string strXML = string.Empty;
            //Intestazione XML
            ...
            //Fogli Excel
            strXML += sheetPianoConservazione(pianoConservazione);
        ...
        private string sheetPianoConservazione(List<DocsPaVO.PianoConservazione> pianoConservazione)
        {
            string strXML = string.Empty;

            strXML = "<Worksheet ss:Name=\"Piano conservazione\">";
            strXML += "<Table>";
            strXML += creaTabellaPianoConservazione();
            strXML += datiPianoConservazioneXML(pianoConservazione);
            strXML += "</Table>";
            strXML += workSheetOptionsXML();
            strXML += "</Worksheet>";
            return strXML;
        ...
        private string creaTabellaPianoConservazione()
        {
            string strXML = string.Empty;
        ...
        private string datiPianoConservazioneXML(List<DocsPaVO.PianoConservazione> pianoConservazione)
        {
            string strXML = string.Empty;
            strXML = creaColonnePianoConservazione();
            strXML += inserisciDatiPianoConservazione(pianoConservazione);
            return strXML;
        }

        private string inserisciDatiPianoConservazione(List<DocsPaVO.PianoConservazione> pianoConservazione)
        {
            string righe = string.Empty;
            if (pianoConservazione != null && pianoConservazione.Count > 0)
            {
                foreach (DocsPaVO.PianoConservazione piano in pianoConservazione)
                    righe += inserisciRigaPianoConservazione(piano);
            }
            return righe;
        }

        private static string inserisciRigaPianoConservazione(DocsPaVO.PianoConservazione row)
        {
            string riga = string.Empty;
            riga = "<Row>";
            ...
            //Colonna Classificazione
            riga += "<Cell>";
            riga += "<Data ss:Type=\"String\">" + row.CodiceClassificazione; ;
            riga += "</Data>";
            riga += "</Cell>";

            //Colonna Voce procedimento amministrativo
            if (!string.IsNullOrEmpty(row.VoceProcedimento))
            {
                riga += "<Cell>";
                riga += "<Data ss:Type=\"String\">" + row.VoceProcedimento;
                riga += "</Data>";
                riga += "</Cell>";
            }
            else
            ...
            //Colonna Numero procedimento
            if (!string.IsNullOrEmpty(row.NumeroProcedimento))
            {
                riga += "<Cell>";
                riga += "<Data ss:Type=\"String\">" + row.NumeroProcedimento;
                riga += "</Data>";
                riga += "</Cell>";
            }
            else
            ...
            //Colonna Tipologia Fascicolo
            riga += "<Cell>";
            riga += "<Data ss:Type=\"String\">" + row.TipologiaFascicolo;
            riga += "</Data>";
            riga += "</Cell>";
            ...
            //Colonna Tempi di conservazione
            riga += "<Cell>";
            riga += "<Data ss:Type=\"String\">" + row.TempoConservazione;
            riga += "</Data>";
            riga += "</Cell>";

            //Colonna Chiusura del fascicolo corrispondente a
            if (!string.IsNullOrEmpty(row.NoteChiusuraFascicolo))
            {
                riga += "<Cell>";
                riga += "<Data ss:Type=\"String\">" + row.NoteChiusuraFascicolo;
                riga += "</Data>";
                riga += "</Cell>";
            }
            else
            ...
            //Note su scartabilità documenti
            if (!string.IsNullOrEmpty(row.NoteScartabilitaDocumenti))
            {
                riga += "<Cell>";
                riga += "<Data ss:Type=\"String\">" + row.NoteScartabilitaDocumenti;
                riga += "</Data>";
                riga += "</Cell>";
            }
            else
            ...
            //Colonna Note su documenti
            if (!string.IsNullOrEmpty(row.NoteDocumenti))
            {
                riga += "<Cell>";
                riga += "<Data ss:Type=\"String\">" + row.NoteDocumenti;
                riga += "</Data>";
                riga += "</Cell>";
            }
            else
        ...
        private string creaColonnePianoConservazione()
        {
            string strXML = string.Empty;
            strXML += "<Row>";
            ...
            //Colonna Classificazione
            strXML += "<Cell ss:StyleID=\"s70\">";
            strXML += "<Data ss:Type=\"String\">Classificazione";
            strXML += "</Data>";
            strXML += "</Cell>";
            //Voce procedimento amministrativo
            strXML += "<Cell ss:StyleID=\"s70\">";
            strXML += "<Data ss:Type=\"String\">Voce procedimento amministrativo";
            strXML += "</Data>";
            strXML += "</Cell>";
            //Colonna Numero procedimento
            strXML += "<Cell ss:StyleID=\"s70\">";
            strXML += "<Data ss:Type=\"String\">Numero procedimento";
            strXML += "</Data>";
            strXML += "</Cell>";
            //Colonna Tipologia fascicolo
            strXML += "<Cell ss:StyleID=\"s70\">";
            ...
            strXML += "</Data>";
            strXML += "</Cell>";
            //Colonna Tempo di conservazione
            strXML += "<Cell ss:StyleID=\"s70\">";
            strXML += "<Data ss:Type=\"String\">Tempi di conservazione";
            strXML += "</Data>";
            strXML += "</Cell>";
            //Chiusura del fascicolo corrispondente a
            strXML += "<Cell ss:StyleID=\"s70\">";
            strXML += "<Data ss:Type=\"String\">Chiusura del fascicolo corrispondente a";
            strXML += "</Data>";
            strXML += "</Cell>";
            //Colonna Note su scartabilità documenti (alert Pitre)
            strXML += "<Cell ss:StyleID=\"s70\">";
            strXML += "<Data ss:Type=\"String\">Note su scartabilità documenti";
            strXML += "</Data>";
            strXML += "</Cell>";
            //Colonna Note su documenti
            strXML += "<Cell ss:StyleID=\"s70\">";
            strXML += "<Data ss:Type=\"String\">Note su documenti";
            strXML += "</Data>";
            strXML += "</Cell>";
            strXML += "</Row>";
            return strXML;

...
namespace BusinessLogic.Reporting
{
    ...
    public abstract class ReportGeneratorCommand
    {
        ...
        public static PrintReportResponse GetReport(PrintReportRequest request, ISpreadsheetService spreadsheetService,
                                                              IFileConverterFactory fileConverterPDF,
                                                              IReportGeneratorService reportGeneratorService)
        {
            ...
            foreach (Type t in types)
            {
                ...
                if (reportType != null)
                {
                    ...
                    response = ((ReportGeneratorCommand)Activator.CreateInstance(t)).GenerateReport(request, spreadsheetService, fileConverterPDF, reportGeneratorService);
                    break;
                }
        ...
        protected virtual PrintReportResponse GenerateReport(PrintReportRequest request, ISpreadsheetService spreadsheetService,
                                                              IFileConverterFactory fileConverterPDF,
                                                              IReportGeneratorService reportGeneratorService)
        {
            ...
            return this.GenerateReport(request, reportDataExtractor, formatGenerator, spreadsheetService, fileConverterPDF, reportGeneratorService);
        }
        ...
        protected virtual IReportGeneratorBehavior CreateReportGenerator(PrintReportRequest request)
        {
            switch (request.ReportType)
            {
                ...
                case ReportTypeEnum.PDF:
                    return new PdfReportGeneratorBehavior();  // usa classi commentate
        ...
        protected virtual List<DocsPaVO.Report.Report> CreateReport(DataSet dataSet, PrintReportRequest request)
        {
            ...
            if (dataSet != null && dataSet.Tables.Count > 0 && dataSet.Tables[0].Rows != null)
            {
                ...
                {
                    // Generazione dell'header del report
                    report.ReportHeader = this.GenerateReportHeader(dataSet, request.ColumnsToExport, idCounter);
                    report.ReportMapRow.Rows.AddRange(this.GenerateReportRows(dataSet, report.ReportHeader, idCounter));
                }
            }
        ...
        protected virtual HeaderColumnCollection GenerateReportHeader(DataSet dataSet, HeaderColumnCollection fieldsToExport, string counterId)
        {
            HeaderColumnCollection header = new HeaderColumnCollection();
        ...
        protected virtual IEnumerable<ReportMapRowProperty> GenerateReportRows(DataSet dataSet, HeaderColumnCollection reportHeader, string counterId)
        {
            List<ReportMapRowProperty> rows = new List<ReportMapRowProperty>();

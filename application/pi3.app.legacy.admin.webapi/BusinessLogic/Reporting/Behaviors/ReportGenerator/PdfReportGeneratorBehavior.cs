...
namespace BusinessLogic.Reporting.Behaviors.ReportGenerator
{
    ...
    public class PdfReportGeneratorBehavior : IReportGeneratorBehavior
    {
        ...
        protected readonly ILogger<PdfReportGeneratorBehavior> _logger;
        /// <summary>
        ...
        public FileDocumento GenerateReport(PrintReportRequest request, List<DocsPaVO.Report.Report> reports, ISpreadsheetService spreadsheetService, IFileConverterFactory fileConverterPDF, IReportGeneratorService reportGeneratorService)
        {
            ...
            try
            {
                ...
                {
                    report.Size = PageSizes.A3;
                }
                else
                {
                    report.Size = PageSizes.A4;
                }
                ...
                report.AddSection(new TextSectionModel
                {
                    ...
                    Content = new TextContentModel
                    {
                        Value = r.AdditionalInformation ?? string.Empty,
                        Style = new TextStyleModel
                        {
                            FontName = "Helvetica",
                            FontSize = 10,
                ...
                GridRowModel header = AddReportHeader(r.ReportHeader);

                model.AddRow(header);

                foreach (var row in r.ReportMapRow.Rows)
                {

                    GridRowModel rowToAdd = new GridRowModel();
                    foreach (var cell in row.Columns)
                    {
                        rowToAdd.AddCell(Cell(cell.Value, VerticalAlignments.Center, Justifications.Center));
                    }
                ...
                var reportGenerated = reportGeneratorService.Generate(report, stream).Result;

                var creation = fileConverterPDF.TryCreate(Path.GetExtension(reportGenerated.FileName)).Result;
                if (creation.Success && creation.Service != null)
                {
                    var converted = creation.Service.Convert(reportGenerated.FileName, stream.ToArray(), FileConverterOutputFormatsEnum.ToPdf).Result;
                    document = new FileDocumento()
                    {
                        content = converted.Content,
                        length = converted.Content.Length,
                        contentType = converted.ContentType,
                        fullName = String.Format("Report_{0}.pdf", DateTime.Now.ToString("dd-MM-yyyy")),
                        name = String.Format("Report_{0}.pdf", DateTime.Now.ToString("dd-MM-yyyy"))
                    };
                }
        ...
        private GridRowModel AddReportHeader(HeaderColumnCollection reportHeader)
        { // Aggiunta delle colonne dell'header

            GridRowModel header = new GridRowModel();
            foreach (HeaderProperty col in reportHeader)
            {
                header.AddCell(HeaderCell((100 / reportHeader.Count), col.ColumnName));
            }
            return header;

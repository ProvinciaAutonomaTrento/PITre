...
namespace BusinessLogic.Reporting.Behaviors.ReportGenerator
{
    class ExcelReportGeneratorBehavior : IReportGeneratorBehavior
    {
        ...
        protected readonly ILogger<ExcelReportGeneratorBehavior> _logger;
        /// <summary>
        ...
        public FileDocumento GenerateReport(PrintReportRequest request, List<DocsPaVO.Report.Report> reports, ISpreadsheetService spreadsheetService, IFileConverterFactory fileConverterPDF, IReportGeneratorService reportGeneratorService)
        {
            ...
            try
            {
                ...
                foreach (var report in reports)
                {
                    ...
                    {
                        sheet.Name = report.SectionName;
                    }
                    else
                    {
                        sheet.Name = String.Format("Foglio {0}", idxReports);
                    }

                    addSheetContent(sheet, report);
                    idxReports++;
                ...
                output = new FileDocumento()
                {
                    ...
                    estensioneFile = "xlsx",
                    name = String.Format("Report_{0}.xlsx", DateTime.Now.ToString("dd-MM-yyyy"))
                };
            }
            catch (Exception ex)
            {
        ...
        private void addSheetContent(SheetModel sheet, DocsPaVO.Report.Report report)
        {
            #region titles
            if (report.ShowHeaderRow)
            {
                ...
                sheet.AddCell(new CellModel()
                {
                    Row = 3,
                    Column = 0,
                    ValueAsString = report.AdditionalInformation,
                    CellStyle = new CellStyleModel()
                    {
                        FontName = "Arial",
                        FontSize = 12,
            ...
            int column = 0;
            foreach (var col in report.ReportHeader)
            {
                sheet.AddCell(new CellModel()
                {
                    Row = 6,
                    Column = column,
                    ValueAsString = col.ColumnName,
                    CellStyle = new CellStyleModel()
                    {
                        FontName = "Arial",
                        FontSize = 8,
            ...
            int row = 7;
            foreach (var rowReport in report.ReportMapRow.Rows)
            {
                column = 0;
                foreach (var rowData in rowReport.Columns)
                {
                    sheet.AddCell(new CellModel()
                    {
                        Row = row,
                        Column = column,
                        ValueAsString = rowData.Value,
                        CellStyle = new CellStyleModel()
                        {
                            FontName = "Arial",
                            FontSize = 8,
                            Width = rowData.Value.Length,
                        }

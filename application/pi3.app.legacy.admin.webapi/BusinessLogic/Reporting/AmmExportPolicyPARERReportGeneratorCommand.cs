...
namespace BusinessLogic.Reporting
{
    ...
    {
        private static ILogger logger = Serilog.Log.ForContext(typeof(AmmExportPolicyPARERReportGeneratorCommand));
        ...
        protected override PrintReportResponse GenerateReport(PrintReportRequest request, IReportDataExtractionBehavior dataExtractor, IReportGeneratorBehavior reportGeneration, ISpreadsheetService spreadsheetService, IFileConverterFactory fileConverterPDF, IReportGeneratorService reportGeneratorService)
        {
            ...
            try
            {
                ...
                DataSet dataSet = this.GetReportData(dataExtractor, request, tipo);

                List<DocsPaVO.Report.Report> report = this.CreateReport(dataSet, request);

                response.Document = reportGeneration.GenerateReport(request, report, spreadsheetService, fileConverterPDF, reportGeneratorService);
        ...
        protected override List<DocsPaVO.Report.Report> CreateReport(DataSet dataSet, PrintReportRequest request)
        {
            ...
            if (tipo.Equals("FASC"))
            {
                report.Title = "Dettaglio policy fascicoli";
            }
        ...
        protected HeaderColumnCollection GenerateReportHeader(DataSet dataSet, HeaderColumnCollection fieldsToExport, string type)
        {
            ...
            if (type.Equals("FASC"))
            {
                ...
                header.Add(new HeaderProperty()
                {
                    ColumnName = "CLASSIFICAZIONE",
                    ColumnSize = 200,
                    OriginalName = "CLASSIFICA",
                    Export = true,
                    DataType = HeaderProperty.ContentDataType.String
                });
                header.Add(new HeaderProperty()
                {
                    ColumnName = "DATA APERTURA",
                    ColumnSize = 150,
                    OriginalName = "DATA_APERTURA",
                    Export = true,
                    DataType = HeaderProperty.ContentDataType.String
                });
                header.Add(new HeaderProperty()
                {
                    ColumnName = "DATA CHIUSURA",
                    ColumnSize = 150,
                    OriginalName = "DATA_CHIUSURA",
                    Export = true,
                    DataType = HeaderProperty.ContentDataType.String
                });
                header.Add(new HeaderProperty()
        ...
        protected IEnumerable<ReportMapRowProperty> GenerateReportRows(DataSet dataSet, HeaderColumnCollection reportHeader, string type)
        {
            ...
            foreach (DataRow row in dataSet.Tables[0].Rows)
            {
                ...
                if (type.Equals("FASC"))
                {
                    ...
                    reportRow.Columns.Add(this.GenerateHeaderColumn(row["REGISTRO"].ToString(), "REGISTRO/AOO", "REGISTRO"));
                    reportRow.Columns.Add(this.GenerateHeaderColumn(this.GetTitolario(row["ID_TITOLARIO"].ToString()), "TITOLARIO", "TITOLARIO"));
                    reportRow.Columns.Add(this.GenerateHeaderColumn(this.GetClassificazioneFascicolo(row["ID_CLASSIFICA"].ToString()), "CLASSIFICAZIONE", "CLASSIFICA"));
                    reportRow.Columns.Add(this.GenerateHeaderColumn(this.GetData(row["CHA_DATA_APERTURA_TIPO"].ToString(), row["DATA_AP_FROM"].ToString(), row["DATA_AP_TO"].ToString(), string.Empty), "DATA APERTURA", "DATA_APERTURA"));
                    reportRow.Columns.Add(this.GenerateHeaderColumn(this.GetData(row["CHA_DATA_CHIUSURA_TIPO"].ToString(), row["DATA_CH_FROM"].ToString(), row["DATA_CH_TO"].ToString(), string.Empty), "DATA CHIUSURA", "DATA_CHIUSURA"));
                    reportRow.Columns.Add(this.GenerateHeaderColumn(row["TEMPLATE"].ToString(), "CAMPI PROFILATI", "CAMPI_PROFILATI"));
                }
                #endregion
                rows.Add(reportRow);
            }
        ...
        private DataSet GetReportData(IReportDataExtractionBehavior dataExtractor, PrintReportRequest request, string tipo)
        {
            if (tipo == "FASC")
            {
                request.ReportKey = "AmmExportPolicyFascPARER";
            }
        ...
        private string GetClassificazioneFascicolo(string idFascicolo)
        {
            var fascicolo = Fascicoli.FascicoloManager.getFascicoloByIdNoSecurity(idFascicolo);
            if (fascicolo != null)
            {
                return string.Format("{0} - {1}", fascicolo.codice, fascicolo.descrizione);
            }

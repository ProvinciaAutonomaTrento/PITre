...
namespace BusinessLogic.Amministrazione
{
    ...
    {
        private static ILogger logger = Serilog.Log.ForContext(typeof(RegistroManager));
        ...
        private static CasellaRegistro GetMail(DataRow rowMailRegistro)
        {
            ...
            mail.Provider = rowMailRegistro["PROVIDER_ID"].ToString()?? String.Empty;
        ...
        private static OrgRegistro GetRegistro(DataRow rowRegistro)
        {
            ...
            //Inserita gestione per MS Exchange
            registro.Mail.Provider = rowRegistro["PROVIDER_ID"].ToString();
            registro.Mail.ClientId = rowRegistro["MS_CLIENT_ID"].ToString();
            registro.Mail.ClientSecret = rowRegistro["MS_CLIENT_SEC"].ToString();
            registro.Mail.TenantId = rowRegistro["MS_TENANT_ID"].ToString();
            ...
            if (rowRegistro.Table.Columns.Contains("VAR_CODICE_IPA") && rowRegistro["VAR_CODICE_IPA"] != DBNull.Value)
                registro.codiceIpa = rowRegistro["VAR_CODICE_IPA"].ToString();
        ...
        public static bool DeleteModelloStampaRicevuta(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.amministrazione.OrgRegistro registro)
        {
            ...
            {
                logger.Error(ex, "Errore in DeleteModelloStampaRicevuta");
                retValue = false;
            }
        ...
        private static string ModelsRootPath
        {
            get
            {
                return DocsPaVO.Settings.AppSettings.Instance.MODELS_ROOT_PATH;
            }
        }
        ...
        public static bool SaveModelloStampaRicevuta(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.amministrazione.OrgRegistro registro, byte[] modelContent, bool modelPdf)
        {
            ...
            {
                logger.Error(ex, "Errore in SaveModelloStampaRicevuta");
                retValue = false;
            }
        ...
        public static DocsPaVO.amministrazione.MailProvider[] GetProviderList()
        {
            DocsPaVO.amministrazione.MailProvider[] mail_provider_list = null;

            DocsPaDB.Query_DocsPAWS.Amministrazione dbAmministrazione = new DocsPaDB.Query_DocsPAWS.Amministrazione();
            DataSet ds = dbAmministrazione.GetDsProviderList();
            if (ds.Tables.Count > 0)
            {
                int index = 0;
                mail_provider_list = new DocsPaVO.amministrazione.MailProvider[ds.Tables["MAIL_PROVIDERS"].Rows.Count];
                foreach (DataRow row in ds.Tables["MAIL_PROVIDERS"].Rows)
                {
                    mail_provider_list[index] = new DocsPaVO.amministrazione.MailProvider
                    {
                        ProviderId = row["PROVIDER_ID"].ToString(),
                        Description = row["PROVIDER_DESC"].ToString(),
                        Lib = row["LIB"].ToString()
                    };
                    ++index;
                }
            }

            return mail_provider_list;
        }
        ...
        public static string GetLibByProviderId(string providerId)
        {
            DocsPaDB.Query_DocsPAWS.Amministrazione dbAmministrazione = new DocsPaDB.Query_DocsPAWS.Amministrazione();
            return dbAmministrazione.GetLibByProviderId(providerId);
        }

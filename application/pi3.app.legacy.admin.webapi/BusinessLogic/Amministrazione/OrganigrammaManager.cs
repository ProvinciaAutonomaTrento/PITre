...
using DocsPaDB.Query_DocsPAWS.Formazione;
    ...
    public class OrganigrammaManager
    {
        ...
        public static EsitoOperazione CopyVisibility(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.Security.CopyVisibility copyVisibility)
        {
            try
            {
                ...
                if (eo.numDocCopiati == 0 && eo.numeroFascCopiati == 0 && eo.numeroDocinFascCopiati == 0 && eo.Codice !=  0)
                ...
                else if(eo.numDocCopiati == 0 && eo.numeroFascCopiati == 0 && eo.numeroDocinFascCopiati == 0 && eo.Codice == 0)
                    UserLog.UserLog.WriteLog(infoUtente, "AMMCOPY", infoUtente.idPeople, "COPIA VISIBILITA' dal ruolo [ " + copyVisibility.codRuoloOrigine + " ] " + copyVisibility.descRuoloOrigine + " al ruolo  [ " + copyVisibility.codRuoloDestinazione + " ] " + copyVisibility.descRuoloDestinazione + " non risultano documenti, fascicoli o documenti in fascicoli/sottofascicoli da copiare. ", DocsPaVO.Logger.CodAzione.Esito.OK, copyVisibility.idAmm);
                else
        ...
        public static bool importOrganigramma(byte[] dati, string nomeFile, string serverPath, DocsPaVO.utente.InfoUtente infoUtente, ISpreadsheetService spreadsheetService, IBaseRubricaComuneService rubricaComuneService)
        {
            bool warning = false;
            ...
            try
            {
                ...
                logger.Debug("Metodo \"importOrganigramma\" classe \"OrganigrammaManager\" : inizio lettura file \"importOrganigramma.xlsx\"");
                ...
                foreach (var sheet in model.Sheets)
                {
                    ...
                    int totalColumns = sheet.Cells.Max(itm => itm.Column);
                    ...
                    switch (sheet.Name)
                    {
                        ...
                        case "UO":
                            ...
                            while (sheet.Cells.Where(itm => itm.Row == row && !string.IsNullOrEmpty(itm.ValueAsString)).Any())
                            {

                                var rowCells = sheet.Cells.Where(itm => itm.Row == row).ToList();
                                ...
                                if (rowCells.Any(itm => string.IsNullOrEmpty(itm.ValueAsString)))
                                {
                                ...
                                if (string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("C")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("E")).FirstOrDefault().ValueAsString))
                                ...
                                else
                                {
                                    ...
                                    //string idAmm = BusinessLogic.Amministrazione.OrganigrammaManager.GetIDAmm(rowCells[0].ValueAsString.ToUpper());

                                    string idAmm = BusinessLogic.Amministrazione.OrganigrammaManager.GetIDAmm(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString.ToUpper());
                                    ...
                                    //Valorizzo i campi
                                    if (string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("B")).FirstOrDefault().ValueAsString))
                                    {
                                        newUo.IDParent = idAmmCorrGlobali;
                                    }
                                    else
                                    {
                                        DocsPaVO.utente.UnitaOrganizzativa uoPadre = amministrazione.getUOByCod(rowCells.Where(itm => itm.Name.StartsWith("B")).FirstOrDefault().ValueAsString);

                                        newUo.IDParent = uoPadre.systemId;
                                        livelloPadre = uoPadre.livello;
                                    }

                                    newUo.Livello = rowCells.Where(itm => itm.Name.StartsWith("C")).FirstOrDefault().ValueAsString;
                                    ...
                                    if (!livelloPadre.Equals("0"))
                                    {
                                        ...
                                        {
                                            string codice = rowCells.Where(itm => itm.Name.StartsWith("D")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.Codice = codice.Replace("\n", "");

                                            newUo.Codice = codice.Replace("\n", "");
                                            string codiceRubrica = rowCells.Where(itm => itm.Name.StartsWith("D")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.Codice = codice.Replace("\n", "");
                                            newUo.CodiceRubrica = codiceRubrica.Replace("\n", "");
                                            string descrizione = rowCells.Where(itm => itm.Name.StartsWith("E")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("E")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.Codice = codice.Replace("\n", "");
                                            newUo.Descrizione = descrizione.Replace("'", "''");
                                            newUo.IDAmministrazione = idAmm;
                                            newUo.CodiceRegistroInterop = rowCells.Where(itm => itm.Name.StartsWith("F")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("F")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.Codice = codice.Replace("\n", "");
                                            newUo.DettagliUo = new OrgDettagliGlobali();
                                            newUo.DettagliUo.Indirizzo = rowCells.Where(itm => itm.Name.StartsWith("G")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("G")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.Codice = codice.Replace("\n", "");
                                            newUo.DettagliUo.Citta = rowCells.Where(itm => itm.Name.StartsWith("H")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("H")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.Codice = codice.Replace("\n", "");
                                            newUo.DettagliUo.Cap = rowCells.Where(itm => itm.Name.StartsWith("I")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("I")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.Codice = codice.Replace("\n", "");
                                            if (rowCells.Where(itm => itm.Name.StartsWith("J")).Any() && rowCells.Where(itm => itm.Name.StartsWith("J")).FirstOrDefault().ValueAsString.Length >= 2)

                                                newUo.DettagliUo.Provincia = rowCells.Where(itm => itm.Name.StartsWith("J")).FirstOrDefault().ValueAsString.Substring(0, 2);
                                            newUo.DettagliUo.Nazione = rowCells.Where(itm => itm.Name.StartsWith("K")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("K")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.DettagliUo.Telefono1 = rowCells.Where(itm => itm.Name.StartsWith("L")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("L")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.DettagliUo.Telefono2 = rowCells.Where(itm => itm.Name.StartsWith("M")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("M")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                            newUo.DettagliUo.Fax = rowCells.Where(itm => itm.Name.StartsWith("N")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("N")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    ...
                                    {
                                        string codice = rowCells.Where(itm => itm.Name.StartsWith("D")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString : null;
                                        newUo.Codice = codice.Replace("\n", "");
                                        string codiceRubrica = rowCells.Where(itm => itm.Name.StartsWith("D")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString : null;
                                        newUo.CodiceRubrica = codiceRubrica.Replace("\n", "");
                                        string descrizione = rowCells.Where(itm => itm.Name.StartsWith("E")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("E")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                        newUo.Descrizione = descrizione.Replace("'", "''");
                                        newUo.IDAmministrazione = idAmm;
                                        newUo.CodiceRegistroInterop = rowCells.Where(itm => itm.Name.StartsWith("F")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("F")).FirstOrDefault().ValueAsString.ToUpper() : null;

                                        newUo.DettagliUo = new OrgDettagliGlobali();
                                        newUo.DettagliUo.Indirizzo = rowCells.Where(itm => itm.Name.StartsWith("G")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("G")).FirstOrDefault().ValueAsString : null;
                                        newUo.DettagliUo.Citta = rowCells.Where(itm => itm.Name.StartsWith("H")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("H")).FirstOrDefault().ValueAsString : null;
                                        newUo.DettagliUo.Cap = rowCells.Where(itm => itm.Name.StartsWith("I")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("I")).FirstOrDefault().ValueAsString : null;

                                        if (rowCells.Where(itm => itm.Name.StartsWith("J")).Any() && rowCells.Where(itm => itm.Name.StartsWith("J")).FirstOrDefault().ValueAsString.Length >= 2)
                                            newUo.DettagliUo.Provincia = rowCells.Where(itm => itm.Name.StartsWith("J")).FirstOrDefault().ValueAsString.Substring(0, 2);

                                        newUo.DettagliUo.Nazione = rowCells.Where(itm => itm.Name.StartsWith("K")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("K")).FirstOrDefault().ValueAsString : null;
                                        newUo.DettagliUo.Telefono1 = rowCells.Where(itm => itm.Name.StartsWith("L")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("L")).FirstOrDefault().ValueAsString : null;
                                        newUo.DettagliUo.Telefono2 = rowCells.Where(itm => itm.Name.StartsWith("M")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("M")).FirstOrDefault().ValueAsString : null;
                                        newUo.DettagliUo.Fax = rowCells.Where(itm => itm.Name.StartsWith("N")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("N")).FirstOrDefault().ValueAsString : null;
                        ...
                        case "RUOLI":
                            ...
                            while (sheet.Cells.Where(itm => itm.Row == row && !string.IsNullOrEmpty(itm.ValueAsString)).Any())
                            {
                                ...
                                //controllo campi obbligatori
                                if (string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("B")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("C")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("E")).FirstOrDefault().ValueAsString))
                                {
                                    sl.Log("");
                                ...
                                else
                                {
                                    ...
                                    //valorizzo i campi
                                    DocsPaVO.utente.UnitaOrganizzativa uopadre = (DocsPaVO.utente.UnitaOrganizzativa)Utenti.UserManager.getCorrispondenteByCodRubrica(rowCells.Where(itm => itm.Name.StartsWith("B")).FirstOrDefault().ValueAsString.ToUpper(), infoUtente, rubricaComuneService);
                                    ...
                                    {
                                        //if (tiporuolo.Codice.ToUpper() == rowCells[2].ValueAsString.ToUpper())
                                        if (tiporuolo.Codice.ToUpper() == rowCells.Where(itm => itm.Name.StartsWith("C")).FirstOrDefault().ValueAsString.ToUpper())
                                            newruolo.IDTipoRuolo = tiporuolo.IDTipoRuolo;
                                    }

                                    //newruolo.Codice = rowCells[3].ValueAsString.ToUpper();
                                    newruolo.Codice = rowCells.Where(itm => itm.Name.StartsWith("D")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;

                                    //newruolo.CodiceRubrica = rowCells[3].ValueAsString.ToUpper();
                                    newruolo.CodiceRubrica = rowCells.Where(itm => itm.Name.StartsWith("D")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;


                                    //string descrizione = rowCells[4].ValueAsString;
                                    string descrizione = rowCells.Where(itm => itm.Name.StartsWith("E")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("E")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;

                                    newruolo.Descrizione = descrizione.Replace("'", "''");
                                    newruolo.IDAmministrazione = idamm;

                                    //newruolo.DiRiferimento = rowCells[5].ValueAsString;
                                    newruolo.DiRiferimento = rowCells.Where(itm => itm.Name.StartsWith("F")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("F")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;

                                    //newruolo.Responsabile = rowCells[6].ValueAsString;
                                    newruolo.Responsabile = rowCells.Where(itm => itm.Name.StartsWith("G")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("G")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                            ...
                            while (sheet.Cells.Where(itm => itm.Row == row && !string.IsNullOrEmpty(itm.ValueAsString)).Any())
                            {
                                ...
                                if (string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("B")).FirstOrDefault().ValueAsString) ||
                                    string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString) || string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("I")).FirstOrDefault().ValueAsString))

                                {
                                    ...
                                    Ruolo ruolo = (Ruolo)Utenti.UserManager.getCorrispondenteByCodRubrica(rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString.ToUpper(), infoUtente, rubricaComuneService);
                                    string idcorrglobruolo = string.Empty;
                                    if (ruolo != null)
                                        idcorrglobruolo = ruolo.systemId;

                                    //recupero la lista registri dell'amministrazione
                                    ArrayList registriamm = Amministrazione.RegistroManager.GetRegistri(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString, "");

                                    //if (!string.IsNullOrEmpty(rowCells[8].ValueAsString))
                                    if (!string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("I")).FirstOrDefault().ValueAsString))
                                    {
                                        //recupero la lista dei codici registro da associare
                                        //string[] codicireg = rowCells[8].ValueAsString.Split(';');
                                        string[] codicireg = rowCells.Where(itm => itm.Name.StartsWith("I")).FirstOrDefault().ValueAsString.Split(';');
                                        ...
                                        {
                                            sl.Log("");
                                            //sl.Log("registri associati al ruolo con codice " + rowCells[3].ValueAsString.ToUpper());
                                            sl.Log("registri associati al ruolo con codice " + rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString.ToUpper());
                                        }
                                    }
                                }
                                row++;
                            ...
                            while (sheet.Cells.Where(itm => itm.Row == row && !string.IsNullOrEmpty(itm.ValueAsString)).Any())
                            {
                                ...
                                //controllo che siano presenti dei registri da associare
                                if (!string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString) || !string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString) || !string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("J")).FirstOrDefault().ValueAsString))
                                {
                                    //recupero l'idamministrazione e lo imposto all'infoutente
                                    //perchè essendo un infoutente che viene dall'amministrazione non ha questo parametro impostato
                                    string idamm = BusinessLogic.Amministrazione.OrganigrammaManager.GetIDAmm(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString.ToUpper());
                                    ...
                                    if (!string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString))
                                    {
                                        //recupero la lista dei codici funzione da associare
                                        string[] codicifuzioni = rowCells.Where(itm => itm.Name.StartsWith("J")).FirstOrDefault().ValueAsString.Split(';');
                                        ...
                                        {
                                            sl.Log("");
                                            //sl.Log("funzioni associati al ruolo con codice " + rowCells[3].ValueAsString.ToUpper());
                                            sl.Log("funzioni associati al ruolo con codice " + rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString.ToUpper());
                                        }
                                    }
                                }
                                row++;
                        ...
                        case "UTENTI":
                            ...
                            while (sheet.Cells.Where(itm => itm.Row == row && !string.IsNullOrEmpty(itm.ValueAsString)).Any())
                            {
                                ...
                                else
                                {
                                    //recupero l'idamministrazione e lo imposto all'infoutente
                                    //perchè essendo un infoutente che viene dall'amministrazione non ha questo parametro impostato
                                    string idamm = BusinessLogic.Amministrazione.OrganigrammaManager.GetIDAmm(rowCells.Where(itm => itm.Name.StartsWith("B")).FirstOrDefault().ValueAsString.ToUpper());
                                    infoUtente.idAmministrazione = idamm;

                                    //creo e valorirzzo l'utente da inserire
                                    DocsPaVO.amministrazione.OrgUtente utente = new DocsPaVO.amministrazione.OrgUtente();

                                    utente.UserId = rowCells.Where(itm => itm.Name.StartsWith("A")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    utente.CodiceRubrica = rowCells.Where(itm => itm.Name.StartsWith("A")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    utente.Codice = utente.CodiceRubrica;
                                    utente.IDAmministrazione = idamm;
                                    utente.Nome = rowCells.Where(itm => itm.Name.StartsWith("C")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("C")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    utente.Cognome = rowCells.Where(itm => itm.Name.StartsWith("D")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    utente.Dominio = rowCells.Where(itm => itm.Name.StartsWith("E")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("E")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    utente.Password = rowCells.Where(itm => itm.Name.StartsWith("F")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("F")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    utente.Sede = rowCells.Where(itm => itm.Name.StartsWith("H")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("H")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    utente.Email = rowCells.Where(itm => itm.Name.StartsWith("I")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("I")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : null;
                                    utente.NotificaTrasm = rowCells.Where(itm => itm.Name.StartsWith("J")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("J")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : "null";
                                    utente.Abilitato = rowCells.Where(itm => itm.Name.StartsWith("K")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("K")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : "0";
                                    utente.Amministratore = rowCells.Where(itm => itm.Name.StartsWith("L")).Any() ? rowCells.Where(itm => itm.Name.StartsWith("L")).FirstOrDefault().ValueAsString.ToUpper().TrimStart().TrimEnd() : "0";
                            ...
                            var sheet_ruoli = model.Sheets.FirstOrDefault(s => s.Name == "RUOLI");

                            row = 1;
                            while (sheet_ruoli.Cells.Where(itm => itm.Row == row && !string.IsNullOrEmpty(itm.ValueAsString)).Any())
                            {

                                var rowCells = sheet_ruoli.Cells.Where(itm => itm.Row == row).ToList();
                                contatore++;
                                ...
                                if (!string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString) || !string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString) || !string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("K")).FirstOrDefault().ValueAsString))
                                {
                                    //recupero l'idamministrazione e lo imposto all'infoutente
                                    //perchè essendo un infoutente che viene dall'amministrazione non ha questo parametro impostato
                                    string idamm = BusinessLogic.Amministrazione.OrganigrammaManager.GetIDAmm(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString.ToUpper());
                                    ...
                                    if (!string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("K")).FirstOrDefault().ValueAsString))
                                    {
                                        //recupero i codici degli utenti da inserire
                                        string[] codiciutente = rowCells.Where(itm => itm.Name.StartsWith("K")).FirstOrDefault().ValueAsString.Split(';');

                                        //associo l'utente al ruolo
                                        for (int i = 0; i < codiciutente.Length; i++)
                                        {
                                            //recupero l'utente da associare
                                            Utente utente = (Utente)Utenti.UserManager.getCorrispondenteByCodRubrica(codiciutente[i].ToUpper().TrimStart().TrimEnd(), infoUtente, rubricaComuneService);
                                            ...
                                            {
                                                sl.Log("");
                                                sl.Log("utente - con codice " + codiciutente[i] + " associato al ruolo " + rowCells.Where(itm => itm.Name.StartsWith("D")).FirstOrDefault().ValueAsString);
                                            }
                                        }
                                    }
                            ...
                            sl.Log("");
                            sl.Log("****FINE  associazione utenti a ruolo - " + System.DateTime.Now.ToString());
                            ...
                            while (sheet.Cells.Where(itm => itm.Row == row && !string.IsNullOrEmpty(itm.ValueAsString)).Any())
                            {
                                ...
                                if (!string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString) || !string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("B")).FirstOrDefault().ValueAsString) || !string.IsNullOrEmpty(rowCells.Where(itm => itm.Name.StartsWith("M")).FirstOrDefault().ValueAsString))
                                {
                                    //recupero l'idamministrazione e lo imposto all'infoutente
                                    //perchè essendo un infoutente che viene dall'amministrazione non ha questo parametro impostato
                                    string idamm = BusinessLogic.Amministrazione.OrganigrammaManager.GetIDAmm(rowCells.Where(itm => itm.Name.StartsWith("B")).FirstOrDefault().ValueAsString.ToUpper());
                                    ...
                                    //recupero l'utente
                                    Utente utente = (Utente)Utenti.UserManager.getCorrispondenteByCodRubrica(rowCells.Where(itm => itm.Name.StartsWith("A")).FirstOrDefault().ValueAsString.ToUpper(), infoUtente, rubricaComuneService);
                                    string idpeople = string.Empty;
                                    if (utente != null)
                                        idpeople = utente.idPeople;

                                    //recupero il ruolo predefinito da associare all'utente
                                    Ruolo ruolo = (Ruolo)Utenti.UserManager.getCorrispondenteByCodRubrica(rowCells.Where(itm => itm.Name.StartsWith("M")).FirstOrDefault().ValueAsString.ToUpper(), infoUtente, rubricaComuneService);
                                    string idgruppo = string.Empty;
                                    if (ruolo != null)
                                        idgruppo = ruolo.idGruppo;
                                    ...
                                    if (!string.IsNullOrEmpty(idpeople) && !string.IsNullOrEmpty(idgruppo))
                                    {
                                        ...
                                        {
                                            sl.Log("");
                                            sl.Log("error - " + rowCells.Where(itm => itm.Name.StartsWith("M")).FirstOrDefault().ValueAsString + " - " + esito.Descrizione);
                                            warning = true;
                                            //return false;
                                        }
                                        else
                                        {
                                            sl.Log("");
                                            sl.Log("ruolo - con codice " + rowCells.Where(itm => itm.Name.StartsWith("M")).FirstOrDefault().ValueAsString + " associato come predefinito all'utente " + rowCells[0].ValueAsString);
                                        }
                                    }
                                }
                                row++;
                            ...
                            sl.Log("");
                            sl.Log("**** FINE impostazione ruolo predefinito per utente - " + System.DateTime.Now.ToString());
            ...
            if (ds.Tables.Count > 0)
            {
                foreach (DataRow row in ds.Tables["AMM_REGISTRI_LIST"].Rows)
                {
                    ...
                    if (idRuolo != null && idRuolo != string.Empty)
                    {
                        ...
                        if (row.Table.Columns.Contains("CHA_PROTOCOLLO_ABILITATO"))
                            registro.ProtocolloAbilitato = row["CHA_PROTOCOLLO_ABILITATO"].ToString() == "1";
                    }
                    else
                    {
        ...
        public static SaveChangesToRoleResponse SaveChangesToRole(SaveChangesToRoleRequest request, IBaseRubricaComuneService rubricaComuneService)
        {
                ...
                case SaveChangesToRoleRequest.SaveChangesToRolePhase.CleanToDoList:
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(cleanToDoList_SP) && cleanToDoList_SP.Equals("1"))
                            ...
                            response = CleanToDoList(request, rubricaComuneService);
                    }
                    catch (Exception e)
                    {
                        response.Result = GetErrorDescription(request.Phase, e);
        ...
        private static SaveChangesToRoleResponse CleanToDoList(SaveChangesToRoleRequest request, IBaseRubricaComuneService rubricaComuneService)
        {
            // Costruzione della response a partire dalla request
            SaveChangesToRoleResponse response = new SaveChangesToRoleResponse(request);
            ...
            */
            ...
            try
            {
                ...
                if (retValue != null && retValue.Equals("0"))
                {
                    // verifica punto 2
                    retValue = null;
                    retValue = dbAmm.AmmVerificaMittDestDoc(utente.IDCorrGlobale);
                    if (retValue != null && retValue.Equals("0"))
                    {
                        // verifica punto 3
                        retValue = null;
        ...
        public static DocsPaVO.amministrazione.EsitoOperazione AmmEliminaUO(DocsPaVO.utente.InfoUtente infoUtente, string idCorrGlob, IBaseRubricaComuneService rubricaCommuneService)
        {
            ...
            if (config.GestioneAbilitata)
            {
                ...
                codiceRubricaComune = RubricaComune.RubricaServices.GetElementoRubricaUO(infoUtente, idCorrGlob).CodiceRubrica;
            }
        ...
        new CriterioRicerca { Campo = CampiRicercaEnum.Codice, Valore = string.IsNullOrEmpty(codiceRubricaComune) ? codiceRubricaComune : codiceRubricaComune.TrimEnd(), TipoRicercaParola = BusinessLogic.RubricaComune.TipiRicercaParolaEnum.ParolaIntera }

    };

                string authToken = HeaderValue.AddressBookAuthToken.Token.Value;
                var correspondentsRC = rubricaCommuneService.Search(authToken, new BusinessLogic.RubricaComune.SearchRequest
                {
                ...
                var idElementoRubrica = correspondentsRC?.Count>0 ? correspondentsRC[0].Id : null;

                if (idElementoRubrica != null)
                {
                    try
                    {
                        rubricaCommuneService.Delete(authToken, (int)idElementoRubrica.AsLong());
                    }
                    catch (Exception ex)
                    {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (orgUser != null)
                        {
                            ...
                            EsitoOperazione esitoDocu = orgManager.ModificaUtente(orgUser);
                            if (esitoDocu.Codice != 0)
                                logger.Warning("Errore nell'abilitazione dell'utente sul documentale: {0}", esitoDocu.Descrizione);
                        }
                        else
                            logger.Warning("Errore nel reperimento dell'utente da modificare");
                    }
                    ...
                    {
                        logger.Warning("Errore nell'abilitazione dell'utente sul documentale", e);
                    }
        ...
        public static DocsPaVO.amministrazione.EsitoOperazione AmmAccettaTrasmConWFRuolo(string idCorrGlobaliRuolo, IBaseRubricaComuneService rubricaComuneService)
        {
                    ...
                    if (singleTrasm.ragione.tipo.ToUpper().Equals("W"))
                    {
                        switch (singleTrasm.tipoTrasm.ToUpper())
                        {
                            case "S":   // Singola
                                ...
                                if (!BusinessLogic.Trasmissioni.ExecTrasmManager.executeAccRifMethod(trasmUtente, trasm.systemId, role, infoUtente, rubricaComuneService, out msg, out mode, out idObj) &&
                                    msg != "Non è possibile accettare/rifiutare il Documento in quanto rimosso dal sistema.")
                                {
                                    esito.Codice = 1;
                            ...
                            case "T":   // Tutti
                                ...
                                foreach (DocsPaVO.trasmissione.TrasmissioneUtente tu in singleTrasm.trasmissioneUtente)
                                {
                                    ...
                                    if (!BusinessLogic.Trasmissioni.ExecTrasmManager.executeAccRifMethod(tu, trasm.systemId, role, infoUtente, rubricaComuneService, out msg, out mode, out idObj) &&
                                        msg != "Non è possibile accettare/rifiutare il Documento in quanto rimosso dal sistema.")
                                    {
                                        esito.Codice = 1;
                                        esito.Descrizione = "si è verificato un errore durante l'accettazione delle trasmissioni a ruolo che prevedono accettazione";

...
namespace BusinessLogic.Amministrazione
{
    ...
    {
        private static ILogger logger = Log.ForContext(typeof(ImportPregressiManager));
        ...
        {
            ILogger logger = Log.ForContext(typeof(ImportPregressiManager));
        ...
        public static void CreaDocumenti(DocsPaVO.utente.InfoUtente infoUtente, ReportPregressi report, IBaseRubricaComuneService rubricaComuneService)
        {
                        ...
                        if (!string.IsNullOrEmpty(item.tipoOperazione) && (item.tipoOperazione.ToUpper().Equals("I") || item.tipoOperazione.ToUpper().Equals("M") || item.tipoOperazione.ToUpper().Equals("C")))
                        {
                            ...
                            if (item.tipoProtocollo.ToUpper().Equals("NP"))
                            {
                                ...
                                if (item.tipoOperazione.ToUpper().Equals("I"))
                                {
                                    ...
                                    template = GetTemplate(item, infoUtenteProprietario, user, role, null, null, ref errori, ref inserisci, ref tipiDocumento, DictionaryIdRuoli, rubricaComuneService);
                                    ...
                                    if (inserisci)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            ControlloTrasmissioni(item, infoUtenteProprietario, infoUtente, role, schedaDoc, importazione, ref errori, ref esitoTrasmissioni, ref DictionaryIdRuoli, rubricaComuneService);
                                ...
                                if (item.tipoOperazione.ToUpper().Equals("M"))
                                {
                                    if (!string.IsNullOrEmpty(item.idNumProtocolloExcel))
                                    {
                                        ...
                                        if (schedaDocModificato != null)
                                        {
                                            ...
                                            schedaDocModificato.template = GetTemplate(item, infoUtenteProprietario, user, role, null, null, ref errori, ref inserisci, ref tipiDocumento, DictionaryIdRuoli, rubricaComuneService);
                            ...
                            else
                            {
                                ...
                                if (item.tipoOperazione.ToUpper().Equals("I"))
                                {
                                    ...
                                    template = GetTemplate(item, infoUtenteProprietario, user, role, register, rf, ref errori, ref inserisci, ref tipiDocumento, DictionaryIdRuoli, rubricaComuneService);
                                ...
                                if (item.tipoOperazione.ToUpper().Equals("M"))
                                {
                                    if (!string.IsNullOrEmpty(item.idNumProtocolloExcel) && !string.IsNullOrEmpty(item.idRegistro))
                                    {
                                        ...
                                        if (schedaDocModificato != null)
                                        {
                                            ...
                                            {
                                                schedaDocModificato.template = GetTemplate(item, infoUtenteProprietario, user, role, register, rf, ref errori, ref inserisci, ref tipiDocumento, DictionaryIdRuoli, rubricaComuneService);
                                            }
        ...
        private static DocsPaVO.ProfilazioneDinamica.Templates GetTemplate(ItemReportPregressi item, DocsPaVO.utente.InfoUtente infoUtenteProprietario, DocsPaVO.utente.Utente user, DocsPaVO.utente.Ruolo role, DocsPaVO.utente.Registro register, DocsPaVO.utente.Registro rf, ref List<string> errori, ref bool inserisci, ref Dictionary<string, DocsPaVO.ProfilazioneDinamica.Templates> tipiDocumento, Dictionary<string, ArrayList> DictionaryIdRuoli, IBaseRubricaComuneService rubricaComuneService)
        {
            ...
            if (!string.IsNullOrEmpty(item.tipo_documento))
            {
                ...
                if (template != null)
                {
                    template = GetTemplateValorizzato(infoUtenteProprietario, template, item.valoriProfilati, user, role, register, rf, ref errori, ref inserisci, DictionaryIdRuoli, rubricaComuneService);
        ...
        private static void ControlloTrasmissioni(ItemReportPregressi item, DocsPaVO.utente.InfoUtente infoUtenteProprietario, InfoUtente infoUtente, DocsPaVO.utente.Ruolo role, DocsPaVO.documento.SchedaDocumento schedaDoc, DocsPaDB.Query_DocsPAWS.ImportPregressi importazione, ref List<string> errori, ref bool esitoTrasmissioni, ref Dictionary<string, ArrayList> DictionaryIdRuoli, IBaseRubricaComuneService rubricaComuneService)
        {
            ...
            if (!string.IsNullOrEmpty(item.cod_modello_trasm))
            {
                ...
                if (!string.IsNullOrEmpty(idModel))
                {
                    ...
                    {
                        result = BusinessLogic.Trasmissioni.TrasmManager.TransmissionExecuteDocTransmFromModelCodeSoloConNotifica(infoUtenteProprietario, string.Empty, schedaDoc, item.cod_modello_trasm, role, out model, rubricaComuneService);
                        if (!result)
                        {
                            item.esito = "W";
        ...
        {
            // TODO: usato per fissare proprietà che finisce nel log
            if (!string.IsNullOrEmpty(userId)) LogContext.PushProperty("userId",userId.ToUpper());
        }
        ...
        {
            string ftpAddress = DocsPaVO.Settings.AppSettings.Instance.FTP_ADDRESS;
            string ftpUsername = DocsPaVO.Settings.AppSettings.Instance.FTP_USERNAME;
            string ftpPassword = DocsPaVO.Settings.AppSettings.Instance.FTP_PASSWORD;
        ...
        private static DocsPaVO.ProfilazioneDinamica.Templates GetTemplateValorizzato(InfoUtente infoUtenteProprietario, DocsPaVO.ProfilazioneDinamica.Templates template, List<string> listaValori, Utente user, Ruolo role, Registro register, Registro rf, ref List<string> errori, ref bool inserisci, Dictionary<string, ArrayList> DictionaryIdRuoli, IBaseRubricaComuneService rubricaComuneService)
        {
            ...
            if (valoriTemplate != null && result != null && result.ELENCO_OGGETTI != null && result.ELENCO_OGGETTI.Count > 0)
            {
                ...
                foreach (DocsPaVO.ProfilazioneDinamica.OggettoCustom oggettoCustom in result.ELENCO_OGGETTI)
                {
                    ...
                    {
                        if (!CompileProfilationField(tempObjCustom, valoriTemplate[oggettoCustom.DESCRIZIONE], infoUtenteProprietario, role, rf, infoUtenteProprietario.idAmministrazione, register, ref errori, DictionaryIdRuoli, rubricaComuneService))
                        {
                            inserisci = false;
        ...
        public static bool CompileProfilationField(DocsPaVO.ProfilazioneDinamica.OggettoCustom customObject, String values, InfoUtente userInfo, Ruolo role, Registro rf, string administrationId, Registro regist, ref List<string> errori, Dictionary<string, ArrayList> DictionaryIdRuoli, IBaseRubricaComuneService rubricaComuneService)
        {
            ...
            switch (customObject.TIPO.DESCRIZIONE_TIPO.ToUpper())
            {
                ...
                case "CORRISPONDENTE":
                    ...
                    Corrispondente corr = FindCorrispondente(values, customObject, userInfo, role, rf, regist, administrationId, false, DictionaryIdRuoli, rubricaComuneService);
        ...
        private static Corrispondente FindCorrispondente(string codice, DocsPaVO.ProfilazioneDinamica.OggettoCustom customObject, InfoUtente userInfo, Ruolo role, Registro RF, Registro registry, string administrationId, bool isEnabledSmistamento, Dictionary<string, ArrayList> DictionaryIdRuoli, IBaseRubricaComuneService rubricaComuneService)
        {
            ...
            if (RF != null && registry != null)
            {
                ...
                corrispondenteResult = GetCorrispondenteByCode(
                    ...
                    rfSyd,
                    isEnabledSmistamento,
                    userType, DictionaryIdRuoli, rubricaComuneService);
        ...
        public static Corrispondente GetCorrispondenteByCode(
            ...
            TipoUtente userTypeForProject, Dictionary<string, ArrayList> DictionaryIdRuoli, IBaseRubricaComuneService rubricaComuneService)
        {
            #region Dichiarazione Variabili
            ...
            corrList = corrSearcher.Search(searchParameters, smistamentoRubrica, rubricaComuneService);
            ...
            try
            {
                ...
                {
                    Corrispondente tempCorr = BusinessLogic.RubricaComune.RubricaServices.UpdateCorrispondente(userInfo, temp.codice, rubricaComuneService);
                    toReturn = tempCorr;
                }
                else
                {

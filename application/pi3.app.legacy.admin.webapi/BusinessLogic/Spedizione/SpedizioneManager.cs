...
public class SpedizioneManager
{
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(creaAllegatoSegnaturaXml) && creaAllegatoSegnaturaXml.Equals("1") && existsIterop)
            {
                ...
                if (allegatoSegnatura != null)
                {
                    ...
                    allegatoSegnatura = CreaSegnaturaProtocollo(allegatoSegnatura, documento, registroRfMittente, infoUtente, out resultSigillo) as DocsPaVO.documento.Allegato;
    ...
    private static DocsPaVO.documento.FileRequest CreaSegnaturaProtocollo(DocsPaVO.documento.Allegato allegatoSegnatura, DocsPaVO.documento.SchedaDocumento schedaDocumento, DocsPaVO.utente.Registro registroRfMittente, DocsPaVO.utente.InfoUtente infoUtente, out DocsPaVO.documento.ResultSigilloElettronico resultSigillo)
    {
        ...
        if (fileReq != null)
        {
            ...
            byte[] segnaturaProtocollo = BusinessLogic.Interoperabilita.InteroperabilitaInvioSegnatura.CreaSegnaturaProtocollo(mittente, infoUtente, schedaDocumento.registro, registroRfMittente, schedaDocumento.docNumber, true, out resultSigillo);
            if (resultSigillo != DocsPaVO.documento.ResultSigilloElettronico.OK)
            {
                return null;
            }

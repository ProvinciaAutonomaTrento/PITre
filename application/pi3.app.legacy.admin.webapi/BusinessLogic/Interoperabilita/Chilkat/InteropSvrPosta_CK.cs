...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(SvrPosta_CK));
    ...
    private void mmInitialize()
    {
        ...
        if (DocsPaVO.Settings.AppSettings.Instance.POP3_CONN_TIMEOUT != null &&
            DocsPaVO.Settings.AppSettings.Instance.POP3_CONN_TIMEOUT.ToString() != "")
            mm.ConnectTimeout = Int32.Parse(DocsPaVO.Settings.AppSettings.Instance.POP3_CONN_TIMEOUT.ToString());

        if (DocsPaVO.Settings.AppSettings.Instance.POP3_READ_TIMEOUT != null &&
            DocsPaVO.Settings.AppSettings.Instance.POP3_READ_TIMEOUT.ToString() != "")
            mm.ReadTimeout = Int32.Parse(DocsPaVO.Settings.AppSettings.Instance.POP3_READ_TIMEOUT.ToString());
    ...
    private void mmInitializeImap()
    {
        ...
        if (DocsPaVO.Settings.AppSettings.Instance.IMAP_CONN_TIMEOUT != null &&
          DocsPaVO.Settings.AppSettings.Instance.IMAP_CONN_TIMEOUT.ToString() != "")
            mm.ConnectTimeout = Int32.Parse(DocsPaVO.Settings.AppSettings.Instance.IMAP_CONN_TIMEOUT.ToString());

        if (DocsPaVO.Settings.AppSettings.Instance.IMAP_READ_TIMEOUT != null &&
            DocsPaVO.Settings.AppSettings.Instance.IMAP_READ_TIMEOUT.ToString() != "")
            mm.ReadTimeout = Int32.Parse(DocsPaVO.Settings.AppSettings.Instance.IMAP_READ_TIMEOUT.ToString());
    ...
    public bool cancellaMailImap()
    {
        ...
        {
            logger.Error("cancellaMailImap - errore: {0}", e.Message);
            return false;
        }
    }
    ...
    public bool moveImap(int index, bool elaborata)
    {
        ...
        catch (Exception e)
        {
            ...
            logger.Error("{0} ; errore:{1}", imap.SessionLog, e.Message);
            return false;
        }
    ...
    public bool salvaMailInLocale(int indexMail, string pathFile, string NomeDellaMail)
    {
        ...
        try
        {
            ...
            if (eml != null)
            {
                ...
                {
                    logger.Error("Errore durante il salvataggio della mail su disco: {0}", e.Message);
                    retval = false;
        ...
        {
            logger.Error("Errore durante reperimento per il salvataggio della mail: {0}", e.Message);
    ...
    public bool salvaMailInLocale(string uidl, string pathFile, string NomeDellaMail)
    {
        ...
        {
            logger.Error("Errore durante il salvataggio della mail: {0}", e.Message);
    ...
    private CMMsg extractMessage(Email msgComponent)
    {
        ...
        if (msg != null)
        {
            try { ReceiveDate = Convert.ToDateTime(msgComponent.LocalDateStr); } catch { }
    ...
    private CMMsg extractDSNInfo(Email msgComponent, int dsnRootLeaf)
    {
        ...
        var dstat = mimemail.GetPart(dsnRootLeaf);
        ...
        if (dstat.ContentType.Equals("message/delivery-status"))
        {
            ...
            //else
            {
                ...
                var part3 = mime.GetPart(dsnRootLeaf + 1);
    ...
    public CMMsg getMessage(byte[] email)
    {
        ...
        if (msgComponent.SetFromMimeBytes(email))
        {
            if ((msgComponent.ReceivedSigned) && (!msgComponent.SignaturesValid))
            {
                ...
                try
                {
                    logger.Error("Errore nella firma del file .eml: Segnatura non valida. Soggetto:{0}. Firmato da {1}. Indirizzo Mittente: {2} ", msgComponent.Subject, msgComponent.SignedBy, msgComponent.FromAddress);
                }
                catch (Exception e) { }
            }
        }
    ...
    */
    ...
    private CMMsg extractMail(Email msgComponent)
    {
        ...
        try
        {
            if (msgComponent != null)
            {
                    ...
                    !BusinessLogic.Interoperabilita.InteroperabilitaManager.isRicevutaPec(msg))
                {
                    if (msgComponent.NumAttachedMessages != 0)
                    {
                        var mailAtt = msgComponent.GetAttachedMessage(0);
        ...
        {
            throw new Exception(String.Format("Errore durante l'estrazione della mail. {0} ", exc.Message));
        }
    }
    ...
    private string ExtractEmail(Email msgComponent, CMMsg msg)
    {
        ...
        try
        {
            msg.date = Convert.ToDateTime(msgComponent.LocalDateStr);
        }
        catch
        { }

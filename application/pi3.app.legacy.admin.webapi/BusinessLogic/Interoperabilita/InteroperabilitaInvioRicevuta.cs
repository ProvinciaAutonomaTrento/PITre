...
{
    private static ILogger logger = Log.ForContext(typeof(InteroperabilitaInvioRicevuta));
    /// <summary>
    /// </summary>
        ...
        try
        {
                ...
                && !dsMitt.Tables["INFO_MITT"].Rows[0]["CHA_TIPO_IE"].Equals(System.DBNull.Value))
            {
                ...
                if (protoMitt != null && dataProtoMitt != null && protoMitt[0] != null && protoMitt[1] != null)
                {
                    ...
                    BusinessLogic.Interoperabilita.InteroperabilitaControlloRicevute.processaRicevutaConferma(ric, out err);
    ...
    private static void sendRicRitorno(string idProfile, DocsPaVO.utente.Registro reg, DocsPaVO.documento.SchedaDocumento schedaDoc, out string messaggioErrore)
    {
        ...
        try
        {
            ...
            if ((schedaDoc.typeId.ToUpper().Trim() == "INTEROPERABILITA") || (schedaDoc.interop.Equals("E") && InteroperabilitaUtils.GetLivelloAnomaliaSegnatura(mailMitt, "INVIO_CONFERMA").Equals("0")))
            {
                string creaconfermaXmlAllegato6 = DocsPaUtils.Configuration.InitConfigurationKeys.GetValue("0", "BE_SEGNATURA_PROTO_ALLEGATO6");
                if (!string.IsNullOrEmpty(creaconfermaXmlAllegato6) && creaconfermaXmlAllegato6.Equals("1"))
                {
                    CostruisciConfermaXml(idProfile, reg, ref numRegMitt, ref qco, separatore, pathFiles);
                }
                else
                {
        ...
        if (mittArr.Count > 0)
        {
            ...
            foreach (DocsPaVO.utente.Corrispondente c in mittArr)
            {
                ...
                if (c.GetType() == typeof(DocsPaVO.utente.UnitaOrganizzativa))
                {
                    ...
                    else if (interop.Equals("E") && !string.IsNullOrEmpty(c.email) && InteroperabilitaUtils.GetLivelloAnomaliaSegnatura(c.email, "INVIO_CONFERMA").Equals("0"))
                    {
                        mailMitt = c.email;
                        break;
                    }
    ...
    private static void CostruisciConfermaXml(string idProfile, DocsPaVO.utente.Registro reg, ref string numRegMitt, ref DocsPaVO.addressbook.QueryCorrispondente qco, string separatore, string filePath)
    {
        System.Data.DataSet ds;
        ...
        try
        {
            ...
            DocsPaVO.Interoperabilita.Segnatura.ConfermaType conferma = new DocsPaVO.Interoperabilita.Segnatura.ConfermaType();
            conferma.Identificatore = new DocsPaVO.Interoperabilita.Segnatura.IdentificatoreType()
            {
                CodiceAmministrazione = new DocsPaVO.Interoperabilita.Segnatura.CodiceIPA() { Value = reg.codAmministrazione },
                CodiceAOO = new DocsPaVO.Interoperabilita.Segnatura.CodiceIPA() { Value = reg.codRegistro },
                CodiceRegistro = reg.codRegistro,
                NumeroRegistrazione = numeroRegString,
                DataRegistrazione = ConvertToDate(dataRegString)
            };

            DocsPaVO.Interoperabilita.Segnatura.IdentificatoreType indentificatore = new DocsPaVO.Interoperabilita.Segnatura.IdentificatoreType()
            {
                CodiceAmministrazione = new DocsPaVO.Interoperabilita.Segnatura.CodiceIPA() { Value = codiceAmmMittString },
                CodiceAOO = new DocsPaVO.Interoperabilita.Segnatura.CodiceIPA() { Value = codiceAOOMittString },
                CodiceRegistro = codiceAOOMittString,
                NumeroRegistrazione = numeroRegMittString,
                DataRegistrazione = ConvertToDate(dataRegMittString)
            };
            ...
            conferma.MessaggioRicevuto = new DocsPaVO.Interoperabilita.Segnatura.MessaggioRicevutoType();
            conferma.MessaggioRicevuto.ItemsElementName = new DocsPaVO.Interoperabilita.Segnatura.ItemsChoiceType3[] { item };
            conferma.MessaggioRicevuto.Items = new object[] { indentificatore = indentificatore };

            string confermaXml = BusinessLogic.XmlParsing.StringWriterWithEncoding.ToXmlString(conferma);

            byte[] content = System.Text.Encoding.UTF8.GetBytes(confermaXml);

            //salvataggio file
            File.WriteAllBytes(filePath + "\\conferma.xml", content);
        }
        catch (Exception e)
        {
            logger.Error("Errore nella gestione dell'interoperabilità. (CostruisciConfermaXml)", e);
            throw e;
        }
    }

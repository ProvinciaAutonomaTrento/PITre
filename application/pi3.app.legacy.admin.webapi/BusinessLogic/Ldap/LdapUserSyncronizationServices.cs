...
{
    public class LdapUserSyncronizationServices
    {
        private static ILogger logger = Serilog.Log.ForContext(typeof(LdapUserSyncronizationServices));

        public static LdapSyncronizationResponse GetLdapSyncResponse(InfoUtente infoUtente, int idHistoryItem, string serverPath)
        {
            try
            {
                // Verifica che il servizio di integrazione con ldap è attivo o meno
                DocsPaLdapServices.LdapConfigurations.CheckForLdapIntegrationActivated(infoUtente.idAmministrazione);

                return RestoreResponseHistory(idHistoryItem, serverPath);
            }
            catch (Exception ex)
            {
        ...
        private static LdapSyncronizationResponse RestoreResponseHistory(int idHistoryItem, string serverPath)
        {
            string filePath = GetSyncResponseFilePath(idHistoryItem, serverPath);
        ...
        private static string GetSyncResponseFilePath(int idHistoryItem, string serverPath)
        {
            string folder = Path.Combine( serverPath, "LdapSyncHistory");
            ...
            return Path.Combine(folder, (idHistoryItem.ToString() + ".xml")).PathAsUnixPath();
        }

        private static LdapSyncronizationResponse Deserialize(string filePath)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(LdapSyncronizationResponse));
            using (FileStream stream = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                return (LdapSyncronizationResponse)serializer.Deserialize(stream);
            }
        }
        ...
        public static LdapSyncronizationResponse SyncronizeLdapUsers(LdapSyncronizationRequest request, string serverPath)
        {
            ...
            try
            {
                ...
                // Verifica che il servizio di integrazione con ldap è attivo o meno
                DocsPaLdapServices.LdapConfigurations.CheckForLdapIntegrationActivated(request.IdAmministrazione);
                ...
                if (ldapInfo != null)
                {
                    ...
                    if (ldapUsers == null)
                        throw new Exception("Errore connessione Ldap");
            ...
            finally
            {
                ...
                PersistResponseHistory(response, historyItem.Id, serverPath);
            }
        ...
        private static Dictionary<string, OrgUtente> GetUsers()
        {
            ...
            catch (Exception ex)
            {
                if(userError != null)
                    ...
                    logger.Debug("Errore in GetUsers \r Messaggio errore: " + ex.Message);
        ...
        private static Dictionary<string, LdapUser> GetLdapUsers(LdapConfig ldapInfo)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.Debug("Errore in GetLdapUsers \r Messaggio errore: " + ex.Message);
        ...
        private static void PersistResponseHistory(LdapSyncronizationResponse instance, int idHistoryItem, string serverPath)
        {
            Serialize(GetSyncResponseFilePath(idHistoryItem, serverPath), instance );
        }
        ...
        private static void Serialize(string filePath, LdapSyncronizationResponse graph)
        {
            XmlSerializer serializer = new XmlSerializer(typeof(LdapSyncronizationResponse));
        ...
        public static LdapSyncronizationHistoryItem[] GetLdapSyncHistory(InfoUtente infoUtente, string idAmministrazione)
        {
            ...
            {
                // Verifica che il servizio di integrazione con ldap è attivo o meno
                DocsPaLdapServices.LdapConfigurations.CheckForLdapIntegrationActivated(idAmministrazione);
        ...
        public static void DeleteLdapSyncHistoryItem(InfoUtente infoUtente, int idHistoryItem, string serverPath)
        {
            try
            {
                // Verifica che il servizio di integrazione con ldap è attivo o meno

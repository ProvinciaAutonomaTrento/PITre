...
namespace DocsPaUtils
{
	...
	{
        private static ILogger logger = Log.ForContext(typeof(InitImportExportQuery));
        private Hashtable queryHashTable = null;
		private static InitImportExportQuery itsInstanceIE = null;

		/// <summary>
		/// Costruttore privato poich� la classe implementa il design pattern singleton
		/// </summary>
		...
		public static InitImportExportQuery getInstance()
		{
			if (itsInstanceIE == null)
			{
                ...
                itsInstanceIE.initializeQuery(AppSettings.Instance.ImportExportQuery);
		...
		public void initializeQuery(String xmlPathName)
		{
			...
			{
				logger.Debug(e, "Errore nel caricamento della hashtable");
			}
		}
  ...
  //          logger.Debug($"getKey for {keyName} in hastable size: " + itsInstanceIE.Len);
  ...
  //          {
		...
		//	{
		//		logger.Debug(exception, "Errore nella lettura dei dati dalla chiave di configurazione '" + keyName);
		//	}

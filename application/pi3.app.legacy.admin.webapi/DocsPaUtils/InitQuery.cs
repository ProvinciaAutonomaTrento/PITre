...
{
        private static ILogger logger = Log.ForContext(typeof(InitQuery));
	...
	public static InitQuery getInstance()
	{
		if (itsInstance == null)
		{
			...
			//itsInstance.initializeQuery(getKey("QueryFilePath"));
                itsInstance.initializeQuery(getKey());
            }
...
//				dateTimeFormat = xmlNodeListDateTime.Item(0).InnerText;
	...
	public string switchQueryList(string xmlPathName)
	{
		try
		{
			...
			if( DocsPaVO.Settings.AppSettings.Instance.DBType.ToUpper() == "ORACLE" )
			{
	...
	{
		return System.IO.Path.Combine(DocsPaVO.Settings.AppSettings.Instance.WebRoot, DocsPaVO.Settings.AppSettings.Instance.QueryFilePath);
        }
        /// <summary>

...
namespace DocsPaDB
{
    public class DBProvider : IDatabase
	{
		...
		public DBProvider()
		{
			dbType = DocsPaVO.Settings.AppSettings.Instance.DBType.ToUpper();
			connectionString = HeaderValue.Instance.ConnectionString.Value;
		...
		{
			return databaseProvider.ExecuteQuery(out dataSet, tableName, command, parameters);
		}
		...
		{
			return databaseProvider.ExecuteNonQuery(command, parameters);
		}
		...
		{
			return databaseProvider.ExecuteReader(qry, parameters);
		}

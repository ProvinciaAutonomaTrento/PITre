...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Integrazioni));
    ...
    public ArrayList MIBACT_BACHECA_getDocsDaNotificare(string statoInvia, string statoAggiorna, string campoNCirc)
    {
        ...
        {
            logger.Error(ex, "errore in MIBACT_BACHECA_getDocsDaNotificare");
            retVal = null;
        }
        return retVal;
    }
    ...
    public ArrayList MIBACT_BACHECA_GetFileInfoDoc(string idDoc)
    {
        ...
        {
            logger.Error(ex, "errore in MIBACT_BACHECA_GetFileInfoDoc");
            retVal = null;
        }
        return retVal;
    }
    ...
    public bool BigFilesFTP_InsertIntoTable(DocsPaVO.ExternalServices.FileFtpUpInfo infoFile)
    {
        ...
        {
            logger.Error(ex, "errore in BigFilesFTP_InsertIntoTable");
            retval = false;
        }
        return retval;
    }
        ...
        if (dataset.Tables["BFFTPINFO"] != null && dataset.Tables["BFFTPINFO"].Rows.Count > 0)
        {
            foreach (DataRow r in dataset.Tables["BFFTPINFO"].Rows)
            {
                ...
                if (string.IsNullOrEmpty(infoFile.FSPath))
                    infoFile.FSPath = DocsPaVO.Settings.AppSettings.Instance.UPLOAD_BIGFILE_REPOSITORY;

                
                if(string.IsNullOrEmpty(infoFile.FSPath))
                    infoFile.FSPath = DocsPaVO.Settings.AppSettings.Instance.DOC_ROOT;
    ...
    public bool BigFilesFTP_updateTable(DocsPaVO.ExternalServices.FileFtpUpInfo infoFile)
    {
        ...
        {
            logger.Error(ex, "errore in BigFilesFTP_updateTable");
            retval = false;
        }
        return retval;
    }
    ...
    public DocsPaVO.ExternalServices.FileFtpUpInfo BigFilesFTP_GetInfoFile(string idQueue, string idDoc)
    {
        ...
        try
        {
            ...
            logger.Error(ex, "errore in BigFilesFTP_GetInfoFile");
            retVal = null;
        }
        return retVal;
    }

...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Fascicoli));
    ...
    /// <summary>
    /// return, valore dal web.config dei WS, se gli ARCHIVIO DI DEPOSITO � abilitato o no.
    /// </summary>
    ...
    public static string GetQueryFascicoli(string idAmministrazione, string idGruppo, string idPeople, DocsPaVO.utente.Registro registro, bool enableUfficioRef, bool enableProfilazione, string codFascicolo, string insRic)
    {
        ...
        {
            //SAB 20/05/2013 corretto da sabrina xch� mancava la gestione sql server
    ...
    public void UpdateFolderLevels(string idpeople, string idgruppo, string idamm, Folder folder)
    {
        ...
        {
            logger.Error(ex, "errore in UpdateFolderLevels");
        }
    }
    ...
    public static string GetQueryListaFascicoli(string idAmministrazione, string idGruppo, string idPeople, DocsPaVO.utente.Registro registro, bool enableUfficioRef, bool enableProfilazione, string varcodliv1, bool child)
    {
        ...
        if (enableUfficioRef)//se � abilitato l'ufficio referente
        {
    ...
    /// <returns>ArrayList o 'null' se si � verificato un errore</returns>
    ...
    public bool SetFascicolo(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.fascicolazione.Fascicolo fascicolo)
    {
        ...
        try
        {
            ...
            //
            // Mev Ospedale Maggiore Policlinico
            // Se la funzionalit� Riclassificazione non � attiva, i valori NodoRiclassificazione_Codice e NodoRiclassificazione_SystemID saranno string.Empty
            ...
            if (!string.IsNullOrEmpty(fascicolo.NodoRiclassificazione_Codice) && !string.IsNullOrEmpty(fascicolo.NodoRiclassificazione_SystemID))
            {
                ...
                // Il campo "var_codice" � nel formato identificato dal campo VAR_FORMATO_FASCICOLATURA della tabella DPA_AMMINISTRA
                if (infoUtente != null && !string.IsNullOrEmpty(infoUtente.idAmministrazione))
                {
                    ...
                    var numFascicolo = fascicolo.numFascicolo;
                    ...
                    newCodeFasc = this.calcolaCodiceFascicolo(infoUtente.idAmministrazione, fascicolo.NodoRiclassificazione_Codice, fascicolo.dataCreazione, fascicolo.NodoRiclassificazione_SystemID, ref numFascicolo, true);
                    fascicolo.numFascicolo = numFascicolo;
    ...
    /// <summary>
    /// Verifica se la chiave di configurazione da amministrazione � attiva o meno
    /// </summary>
    /// <returns></returns>
    ...
    public bool FolderHasDocument(Folder item)
    {
        ...
        {
            logger.Error(ex, "errore in FolderHasDocument");
            result = false;
        }
    ...
    public bool SospendiRiattivaUtente(string idPeople, DocsPaVO.utente.Corrispondente corr, DocsPaVO.fascicolazione.Fascicolo fasc)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                //l'utente non � proprietario del fascicolo: eccezione
                //throw new Exception("F_System");
                logger.Debug("F_System");
    ...
    private static DocsPaVO.fascicolazione.Fascicolo GetFascicolo2(System.Data.DataSet dataSet, System.Data.DataRow dataRow, bool enableProfilazione)
    {
        ...
        //			//nuovo per popolare il campo descrizione del registro a cui il fascicolo � associato
    ...
    private static DocsPaVO.fascicolazione.Fascicolo GetFascicolo(DocsPaVO.utente.InfoUtente infoUtente, System.Data.DataSet dataSet, System.Data.DataRow dataRow, bool enableProfilazione)
    {
        ...
        if (dataRow.Table.Columns.Contains("ID_PIANO_CONSERVAZIONE"))
        {
            DocsPaDB.Query_DocsPAWS.PianoConservazione piano = new DocsPaDB.Query_DocsPAWS.PianoConservazione();
            string idPianoConservazione = dataRow["ID_PIANO_CONSERVAZIONE"].ToString();
            objFascicolo.pianoConservazione = piano.GetPianoConservazioneById(idPianoConservazione);
        }


        // Reperimento dell'ultima nota di visibilit� generale inserita nel documento
        ...
        // Impostazione del flag per indicare se il fascicolo � in ADL
    ...
    /// <returns>1 se il fascicolo � in ADL, 0 altrimenti</returns>
    ...
    /// <summary>
    /// Metodo per la ricrca dell'ultima nota di visibilit� generale inserita
    /// </summary>
    ...
    {
        // Creo un nuovo oggetto Note che si occuper� di cercare le ultime note
    ...
    private static DocsPaVO.fascicolazione.Fascicolo GetFascicolo(DocsPaVO.utente.InfoUtente infoUtente, System.Data.DataSet dataSet, System.Data.DataRow dataRow)
    {
        ...
        //Aggiunta campo visilbit� dell'utente. Se 1 l'utente pu� vedere il fascicolo
        ...
        //Reperimento atipicit� del documento
    ...
    /// <param name="infoUtente"></param>
    /// <param name="debug"></param>
    /// <returns>ArrayList di fascicoli o 'null' se si � verificato un errore.</returns>
    ...
    /// <param name="infoUtente"></param>
    /// <param name="debug"></param>
    /// <returns>Fascicolo o 'null' se si � verificato un errore.</returns>
    ...
    public string filtroExcel(byte[] datiExcel, string nomeFile, string nomeAttributo, string serverPath)
    {
        ...
        try
        {
            ...
            xlsConn.ConnectionString = "Provider=" + DocsPaVO.Settings.AppSettings.Instance.DS_PROVIDER + "Data Source=" + serverPath + "\\" + nomeFile + ";Extended Properties='" + DocsPaVO.Settings.AppSettings.Instance.DS_EXTENDED_PROPERTIES + "IMEX=1'";
            xlsConn.Open();
            OleDbCommand xlsCmd;
            ...
            if (xlsReader.HasRows)
            {
                ...
                {
                    //Controllo se si � arrivati all'ultima riga
            ...
            if (getSystemIdList)
            {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        // Se � richiesta la lista dei system id dei fascicoli,
                    ...
                    // Se � richiesta la lista dei system id dei fascicoli, viene
    ...
    /// <param name="requestedPage"></param>
    /// <param name="pageSize"></param>
    /// <param name="getSystemIdList">True se � richiesta anche la lista dei system id dei fascicoli restituiti dalla ricerca</param>
    /// <param name="idProjectList">Lista dei system id dei fascicoli restituiti dalla ricerca. Verr� valorizzata solo se getSystemIdList � true</param>
    /// <returns></returns>
    public ArrayList GetListaFascicoliPaging(DocsPaVO.utente.InfoUtente infoUtente,
        DocsPaVO.fascicolazione.Classificazione objClassificazione,
                                       ...
                                       bool getSystemIdList, byte[] datiExcel, string serverPath)
    {
        ...
        try
        {
            ...
            // Se getSystemIdList � true e queryType � DATACOUNT...
            if (getSystemIdList &&
            ...
            if (this.dbType == "SQL")
            {
                ...
                {
                    // Altrimenti se � valorizzato il campo da utilizzare per l'ordinamento...
                    if (sqlField != null)
                    {
            ...
            if (objClassificazione != null && !string.IsNullOrEmpty(objClassificazione.varcodliv1))
            {
                ...
                // esiste solo se objClassificazione � valido
        ...
        for (int i = 0; i < objListaFiltri.Length; i++)
        {
            ...
            if (f.valore != null && !f.valore.Equals(""))
            {
                switch (f.argomento)
                {
                    ...
                    //il campo cha_stato della tabella dpa_items_conservazione non � mai impostato a C
                    ...
                    case "CODICE_FASCICOLO":
                        ...
                        break;
                    case "TIPOLOGIA_FASCICOLO_PIANO_CONSERVAZIONE":
                        if(!string.IsNullOrEmpty(f.valore))
                            queryStr += " AND A.ID_PIANO_CONSERVAZIONE = " + f.valore + " ";
                        break;
                    case "VISIBILITA_T_A":
                    ...
                    case "STATO_CONSERVAZIONE":
                        if(!f.valore.Equals("NVWCRETF"))
                        {
                            ...
                            {
                                condNoQueue = " NOT EXISTS (SELECT 'x' FROM DPA_VERSAMENTO_FASCICOLI V WHERE V.ID_PROJECT=A.SYSTEM_ID) ";
                            }
                            ...
                            if (!string.IsNullOrEmpty(value))
                            {
                                ...
                                {
                                    queryStr += string.Format(" AND EXISTS (SELECT 'x' FROM DPA_VERSAMENTO_FASCICOLI V WHERE V.ID_PROJECT=A.SYSTEM_ID AND V.CHA_STATO IN ({0}))", value);
                                }
                                else
                                {
                                    condQueue = string.Format(" EXISTS (SELECT 'x' FROM DPA_VERSAMENTO_FASCICOLI V WHERE V.ID_PROJECT=A.SYSTEM_ID AND V.CHA_STATO IN ({0}))", value);
                                    queryStr += string.Format("AND ( ({0}) OR ({1}) )", condNoQueue, condQueue);
                                }
                            }
                        }
                        break;
                    case "DATA_VERSAMENTO_DA":
                        if(!string.IsNullOrEmpty(f.valore))
                        {
                            queryStr += string.Format(" AND EXISTS ( SELECT 'X' FROM DPA_VERSAMENTO_FASCICOLI V WHERE V.ID_PROJECT=A.SYSTEM_ID AND V.DTA_INVIO>{0})", DocsPaDbManagement.Functions.Functions.ToDate(f.valore + " 00:00:00", true)) ;
                        };
                        break;
                    case "DATA_VERSAMENTO_A":
                        if (!string.IsNullOrEmpty(f.valore))
                        {
                            queryStr += string.Format(" AND EXISTS ( SELECT 'X' FROM DPA_VERSAMENTO_FASCICOLI V WHERE V.ID_PROJECT=A.SYSTEM_ID AND V.DTA_INVIO<{0})", DocsPaDbManagement.Functions.Functions.ToDate(f.valore + " 23:59:59", true));
                        };
                        break;
                    case "DATA_VERSAMENTO_IL":
                        if (!string.IsNullOrEmpty(f.valore))
                        {
                            queryStr += string.Format(" AND EXISTS ( SELECT 'X' FROM DPA_VERSAMENTO_FASCICOLI V WHERE V.ID_PROJECT=A.SYSTEM_ID AND V.DTA_INVIO BETWEEN {0} AND {1})", DocsPaDbManagement.Functions.Functions.ToDateBetween(f.valore, true), DocsPaDbManagement.Functions.Functions.ToDateBetween(f.valore, false));
                        };
                        break;
                }
            }
        ...
        DocsPaVO.fascicolazione.Classifica[] gerarchia, bool enableUffRef, string chiaveFascicolo)
    {
        ...
        using (DocsPaDB.DBProvider dbProvider = new DBProvider())
        {
            ...
            q.setParam("idPianoConservazione", (fascicolo.pianoConservazione != null && !string.IsNullOrEmpty(fascicolo.pianoConservazione.SystemId)) ? fascicolo.pianoConservazione.SystemId : "null");
            q.setParam("param10", " WHERE SYSTEM_ID = " + fascicolo.systemID);
    ...
    public static string Cfg_SET_DATA_VISTA_GRD
    {
        get
        {
            string eme = DocsPaVO.Settings.AppSettings.Instance.SET_DATA_VISTA_GRD;
    ...
    {
        //Veronica
        //controllo se il folderParent � privato, in tal caso anche questo folder viene creato come privato
        string privato = null;
    ...
    public bool RiproponiStrutturaFascicolo(string idFascicoloOrigine, DocsPaVO.fascicolazione.Fascicolo fascicolo, DocsPaVO.utente.InfoUtente infoUtente)
    {
        ...
        try
        {
            logger.Debug("INIZIO SP_RIPROPONI_STRUTTURA_FASC");

            // Creazione parametri per la Store Procedure
            ArrayList parameters = new ArrayList();
            //DocsPaUtils.Data.ParameterSP outParam;
            //outParam = new DocsPaUtils.Data.ParameterSP("outvalue", new Int32(), 10, DocsPaUtils.Data.DirectionParameter.ParamOutput, System.Data.DbType.Int32);
            parameters.Add(this.CreateParameter("idFascicoloOrigine", Convert.ToInt32(idFascicoloOrigine)));
            parameters.Add(this.CreateParameter("idFascicoloNew", Convert.ToInt32(fascicolo.systemID)));
            parameters.Add(this.CreateParameter("idParentOrigine", 0));
            parameters.Add(this.CreateParameter("idParentNew", 0));
            ...
            if(infoUtente.delegato != null && !string.IsNullOrEmpty(infoUtente.delegato.idPeople))
                parameters.Add(this.CreateParameter("idPeopleDelegato", Convert.ToInt32(infoUtente.delegato.idPeople)));
            else
                parameters.Add(this.CreateParameter("idPeopleDelegato", 0));

            //parameters.Add(outParam);
            int outvalue;
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
                outvalue = dbProvider.ExecuteStoredProcedure("SP_RIPROPONI_STRUTTURA_FASC", parameters, null);

            logger.Debug("SP_RIPROPONI_STRUTTURA_FASC RESULT:" + outvalue.ToString());
            if (outvalue != 1)
            {
                retProc = false;
            }
        ...
        {
            logger.Debug("Errore durante il metodo RiproponiStrutturaFascicolo" + e.Message);
            retProc = false;
        }
        logger.Information("END SP_RIPROPONI_STRUTTURA_FASC");
        return retProc;
    ...
    /// <param name="infoUtente"></param>
    /// <param name="debug"></param>
    /// <returns>Folder o 'null' se si � verificato un errore.</returns>
    public DocsPaVO.fascicolazione.Folder GetFolderById(string idPeople, string idGruppo, string idFolder)
    {
        ...
        catch (Exception)
        {
            ...
            //NB: L'istruzione seguente � solo per evitare il warning
    ...
    public int DelFolder(DocsPaVO.fascicolazione.Folder folder, DocsPaVO.utente.InfoUtente infoUtente)
    {
        ...
        try
        {
            // verifico se � un root folder
            if (folder.idParent.Equals(folder.idFascicolo))
            {
                logger.Debug("Errore nella gestione dei Fascicoli (Query - DelFolder). Non si pu� cancellare un root folder!");
                throw new Exception("Non si pu� cancellare un root folder");
            }
    ...
    /// </summary>
    /// <remarks>
    /// La successiva implementazione consister� nello spostare la logica del metodo
                ...
                if (f.valore != null && !f.valore.Equals(""))
                {
                    switch (f.argomento)
                    {
                        ...
                        case "OGGETTO":
                            ...
                            // La stringa di testo contenuta nel campo oggetto � messa in
            ...
            out List<SearchResultInfo> idProfiles)
    {
        ...
        try
        {
            ...
            {
                listaDocumenti = null;
                throw new Exception("ID folder in GetDocumentiPaging fascicolo � vuoto.");
            }
            if (filtriRicerca != null)
            {
    ...
    private string GetMaxRisultatiQuery()
    {
        ...
        string maxRowCount = DocsPaVO.Settings.AppSettings.Instance.numeroMaxRisultatiQuery;
    ...
    public bool AddDocFolder(DocsPaVO.utente.InfoUtente infoUtente, string idGruppo, string idProfile, string idFolder)
    {
        ...
        try
        {
            ...
            if (resOut.Equals("0"))//caso in cui il documento non � stato fasciolato nella folder corrente
            {
                string insertString = "";
                ...
                //Prima di inserire il doc in fascicolo, si verifica che non sia gi� stato inserito in un altro
                //fascicolo, se non lo � e se � attiva la chiave di configurazione "CHA_FASC_PRIMARIA" allora
                ...
                //REQUISITO: visibilit� dei documenti slegata dai fascicoli in cui devono essere inseriti
                //Se il fascicolo � controllato i documenti in esso inseriti non ne ereditano le ACL
                string queryControllato = string.Empty;
                ...
                string isControllato;
                this.ExecuteScalar(out isControllato, queryControllato);
                //Se il fascicolo � controllato allora il documento non eredita le ACL
            ...
            {
                // se il doc � gi� fascicolato ritorno true
                //questa modifica � stata introdotta a seguito di un bug dovuto al checkin del
                //12aprile2007 quando � stato cambiato il valore di ritorno di dafault del metodo da true a false,
                result = false;
            }
        }
        catch (Exception e)
    ...
    public bool DeleteDoc(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.fascicolazione.Folder folder, string idProfile)
    {
        ...
        try
        {
            ...
            string resOut;
            this.ExecuteScalar(out resOut, queryString);
            //SE IL DOC NON � PIU' contenuto in nessun fascicolo allora rimetto a '0' il
    ...
    public void updateDocTrustees(string idGruppo, string idProfile, string idFolder)
    {
        ...
        // leggo il registro cui � associato il documento
        queryString = "";
        ...
        //Emanuela: poich� se si rimuove le visibilita del fascicolo ad un ruolo non viene rimossa anche per i sottofascicoli, nel caso di inserimento di un doc
        ...
        if (idReg != null && idReg.ToString() != null && !idReg.Equals(""))
        {
            ...
            /*    tabRegistro = ", DPA_L_RUOLO_REG B, DPA_CORR_GLOBALI C";
                ...
                */
            if (!string.IsNullOrEmpty(DocsPaVO.Settings.AppSettings.Instance.NO_FILTRO_AOO) &&
                DocsPaVO.Settings.AppSettings.Instance.NO_FILTRO_AOO == "1")
            {
    ...
    public void GetVisibilita(out System.Data.DataSet ds, string idProject)
    {
        ...
        logger.Debug("Query visibilit� di un fascicolo - RUOLI: " + queryStringRuoli);
    ...
    public void GetVisibilita_rimossi(out System.Data.DataSet ds, string idProject)
    {
        ...
        q.setParam("param1", idProject);
        string sql = q.getSQL();
        logger.Debug("Query visibilit� di un fascicolo - RUOLI RIMOSSI: " + sql);
    ...
    public void GetVisibilita1(out System.Data.DataSet ds, string idProject)
    {
        ...
        logger.Debug("Query visibilit� fascicolo - UTENTI: " + queryStringUtenti);
    ...
    public void GetVisibilita_UtentiRimossi(out System.Data.DataSet ds, string idProject)
    {
        ...
        q.setParam("param1", idProject);
        string queryStringUtenti = q.getSQL();
        logger.Debug("Query visibilit� fascicolo - UTENTI RIMOSSI: " + queryStringUtenti);
    ...
    public bool SetProjectTrustees(string idPeople, string thing, DocsPaVO.utente.Ruolo objRuolo, string idClassificazione, string idRegistro, bool isPrivato, out System.Collections.ArrayList listaRuoliSup, DocsPaVO.utente.InfoUtente delegato, bool pubblico = false)
    {
        ...
        using (DocsPaDB.DBProvider dbProvider = new DBProvider())
        {
            ...
            if (retValue)
            {
                ...
                {
                    //se il registro � null, la visibilit� deve essere estesaa tutti i ruoli che vedono il nodo
                ...
                {
                    //se il registro � diverso da NULL allora la visibilit� deve essere estesa solamente ai superiori che vedono sia il nodo e il registro
                ...
                //Se il fascicolo � privato si da la visibilit� solo all'utente creatore e al ruolo di appartenenza,
                //non ai superiori
                if (!isPrivato)
                {
                    for (int i = 0; i < listaRuoliSup.Count; i++)
                ...
                //Nel caso di fascicolo pubblico d� diritti in lettura al ruolo pubblico definito in amministrazione
                if (pubblico)
                {
    ...
    public void SetProjectTrustees(string idPeople, string thing, DocsPaVO.utente.Ruolo objRuolo, string idClassificazione, bool isFolder, out System.Collections.ArrayList listaRuoliSup, DocsPaVO.utente.InfoUtente delegato)
    {
        ...
        //si estende la visibilit� ai superiori
        DocsPaDB.Utils.Gerarchia gerarchia = new DocsPaDB.Utils.Gerarchia();
        listaRuoliSup = gerarchia.getGerarchiaSup(objRuolo, null, idClassificazione, DocsPaVO.trasmissione.TipoOggetto.FASCICOLO);
        if (listaRuoliSup == null)
        {
        ...
        /* se si sta creando una Folder, allora si da la visibilit� di essa a
        tutti coloro che vedono il fascicolo in cui essa � contenuta
        (il tipo diritto � F ovvero TRASMESSO IN FASCICOLO)*/
        if (isFolder)
        {
    ...
    /// <summary>
    /// Setta la visibilit� nella security relativamente alla Folder/Root, nella transazione corrente
    /// </summary>
    /// <param name="idPeople"></param>
    /// <param name="thing"></param>
    /// <param name="objRuolo"></param>
    ...
    /// <param name="registro"></param>
    /// <param name="debug"></param>
    /// <returns>Array di oggetti Classifica o 'null' se si � verificato un errore.</returns>
    ...
    /// <param name="debug"></param>
    /// <returns>ArrayList o 'null' se si e � verificato un errore.</returns>
    ...
    public bool ExistingCode(DocsPaVO.fascicolazione.Classificazione nodoTitolario, string idAmministrazione)
    {
        ...
        //	throw new Exception("Il codice scelto � gi� presente");
    ...
    public bool GetCountSottofascDoc(string project_Id, out string nFasc)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    /* A questo punto poich� il fascicolo selezionato non contiene n� sottofascicoli
                     * n� documenti � possibile rimuoverlo*/
                    result = true;
                }
    ...
    /// <param name="descrizione"></param>
    /// <param name="systemID"></param>
    /// <returns>Classificazione o 'null' se si � verificato un errore.</returns>
    ...
    public string GetIdTitolarioAttivo(string idAmministrazione)
    {
        string idTitolario = string.Empty;

        try
        {
            DocsPaUtils.Query queryMng = DocsPaUtils.InitQuery.getInstance().getQuery("S_GET_TITOLARIO_ATTIVO");
            queryMng.setParam("idAmm", idAmministrazione);
            string commandText = queryMng.getSQL();
            logger.Debug("Get GetIdTitolarioAttivo per amministrazione QUERY : " + commandText);
            DataSet ds = new DataSet();
            using (DBProvider dbProvider = new DBProvider())
            {
                dbProvider.ExecuteQuery(ds, "ID_TITOLARIO", commandText);
                if (ds.Tables["ID_TITOLARIO"] != null && ds.Tables["ID_TITOLARIO"].Rows.Count > 0)
                {
                    idTitolario = ds.Tables["ID_TITOLARIO"].Rows[0]["ID_TITOLARIO"].ToString();
                }
            }
        }
        catch(Exception e)
        {
            ...
            logger.Error("Errore in GetIdTitolarioAttivo: " + e.Message);
        }
    ...
    //				{
    //					// 3 - Si verifica se il codice � gi� presente viene calcolato il nuovo codice
    ...
    //
    //				// 5 - Si aggiorna il codUltimo (solo per ora perch� andr� tolta questa gestione)
    //				newFascicolo(fascicolo.codUltimo,fascicolo.idClassificazione);
    //
    //				// 6 - Si estende la visibilit� sul fascicolo creato
    ...
    public bool IsDocumentoClassificatoInFolder(string idProfile, string idFolder)
    {
        ...
        try
        {
            ...
            if (retValue > 0)
            {
                result = true; // il doc � gi� classificato nella folder corrente
            }
        ...
        try
        {
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                ...
                if (idGruppoTrasm.Equals(""))
                    q.setParam("param1", dirittoOggetto.idObj + "," + personOrGroup + "," + dirittoOggetto.accessRights + ",'','" + tipoDiritto + "','" + note + "'," + DocsPaDbManagement.Functions.Functions.GetDate() + ",'" + infoUtente.idPeople + "','" + infoUtente.idGruppo + "', NULL");
                else
                    q.setParam("param1", dirittoOggetto.idObj + "," + personOrGroup + "," + dirittoOggetto.accessRights + "," + idGruppoTrasm + ",'" + tipoDiritto + "','" + note + "'," + DocsPaDbManagement.Functions.Functions.GetDate() + ",'" + infoUtente.idPeople + "','" + infoUtente.idGruppo + "', NULL");
                queryString = q.getSQL();
                logger.Debug(queryString);
                if (!dbProvider.ExecuteNonQuery(queryString))
                {
                ...
                q = DocsPaUtils.InitQuery.getInstance().getQuery("I_DELETED_SECURITY");
                if (idGruppoTrasm.Equals(""))
                    q.setParam("param1", dirittoOggetto.rootFolder + "," + personOrGroup + "," + dirittoOggetto.accessRights + ",'','" + tipoDiritto + "','" + note + "'," + DocsPaDbManagement.Functions.Functions.GetDate() + ",'" + infoUtente.idPeople + "','" + infoUtente.idGruppo + "', NULL");
                else
                    q.setParam("param1", dirittoOggetto.rootFolder + "," + personOrGroup + "," + dirittoOggetto.accessRights + "," + idGruppoTrasm + ",'" + tipoDiritto + "','" + note + "'," + DocsPaDbManagement.Functions.Functions.GetDate() + ",'" + infoUtente.idPeople + "','" + infoUtente.idGruppo + "', NULL");
                queryString = q.getSQL();
                logger.Debug(queryString);
                if (!dbProvider.ExecuteNonQuery(queryString))
                {
                ...
                if (tipoDiritto == "P")
                {
                    ...
                    if (idGruppoTrasmRU.Equals("") || idGruppoTrasmRU.Equals(null))
                        q.setParam("param1", dirittoOggetto.idObj + "," + personOrGroupRU + "," + accessRight + ",'','P','" + note + "'," + DocsPaDbManagement.Functions.Functions.GetDate() + ",'" + infoUtente.idPeople + "','" + infoUtente.idGruppo + "', NULL");
                    else
                        q.setParam("param1", dirittoOggetto.idObj + "," + personOrGroupRU + "," + accessRight + "," + idGruppoTrasmRU + ",'P','" + note + "'," + DocsPaDbManagement.Functions.Functions.GetDate() + ",'" + infoUtente.idPeople + "','" + infoUtente.idGruppo + "', NULL");
                    queryString = q.getSQL();
                    logger.Debug(queryString);
                    if (!dbProvider.ExecuteNonQuery(queryString))
                    {
                    ...
                    q = DocsPaUtils.InitQuery.getInstance().getQuery("I_DELETED_SECURITY");
                    if (idGruppoTrasmRU.Equals("") || idGruppoTrasmRU.Equals(null))
                        q.setParam("param1", dirittoOggetto.rootFolder + "," + personOrGroupRU + "," + accessRight + ",'','P','" + note + "'," + DocsPaDbManagement.Functions.Functions.GetDate() + ",'" + infoUtente.idPeople + "','" + infoUtente.idGruppo + "', NULL");
                    else
                        q.setParam("param1", dirittoOggetto.rootFolder + "," + personOrGroupRU + "," + accessRight + "," + idGruppoTrasmRU + ",'P','" + note + "'," + DocsPaDbManagement.Functions.Functions.GetDate() + ",'" + infoUtente.idPeople + "','" + infoUtente.idGruppo + "', NULL");
                    queryString = q.getSQL();
                    logger.Debug(queryString);
                    if (!dbProvider.ExecuteNonQuery(queryString))
                    {
                ...
                //Calcolo l'eventuale atipici� del fascicolo
    ...
    //perch� � classificato in altri fascicoli oltre quello in questione
    ...
    public bool AddFascInDeposito(string idProject, string valore, string tipoOp)
    {
        ...
        try
        {
            ...
            //mentre i fascicoli generali non si cancellano perch� rimangono sempre a 2
            if (result && valore == "1")
            {
    ...
    public void creaProtTitRifMitt(DocsPaVO.utente.InfoUtente infoUtente, string idGruppo, string idProfile, string idFolder)
    {
        ...
        try
        {
            if (!string.IsNullOrEmpty(protTit) || rifMit)
            {
                ...
                //Creo il Protocollo Titolario e il Riferimento Mittente che sar� un protocollo di titolario senza sottonumero progressivo
                if (!string.IsNullOrEmpty(protTit))
                {
                    //Controllo se non esiste gi� un protocollo titolario per il documento
                    if (ds.Tables[0].Rows.Count > 0 && string.IsNullOrEmpty(ds.Tables[0].Rows[0]["PROT_TIT"].ToString()))
                    {
                            ...
                            )
                        {
                            ...
                            */
                            ...
                            //Solo se il protocollo � in uscita
                ...
                //Creo il riferimento mittente solo se il protocollo di titolario non � attivo perch� altrimenti l'ho creato sopra
                //e sarebbe il protocollo di titolario senza sottonumero ed il tipo di protocollo � in uscita
    ...
    /// <summary>
    /// Verifico se � possibile rimuovere un fascicolo senza rompere la catena dei sottonumeri
    /// </summary>
    /// <param name="codiceFascicolo"></param>
    /// <returns></returns>
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(schedaDoc.protocolloTitolario) && fascicolo != null && fascicolo.systemID == schedaDoc.idFascProtoTit)
            {
                ...
                if (ds.Tables[0].Rows.Count != 0 && !string.IsNullOrEmpty(ds.Tables[0].Rows[0]["NUM_DOC_IN_FASC"].ToString()))
                {
                    ...
                    {
                        //Non posso declassificare il documento perch� nel fascicolo esistono documenti con sottonumeri successivi
    ...
    public int GetCountTrasmissioniFascicolo(int idProject)
    {
        ...
        // Il valore in cui verr� salvato il risultato della query
    ...
    /// <summary>
    /// Prende la lista dei fascicoli in sui � stato fascicolato un documento senza security
    /// </summary>
    /// <param name="infoDocumento"></param>
    /// <param name="sicurezza"></param>
    /// <returns>ArrayList o 'null' se si � verificato un errore</returns>
    ...
    /// <summary>
    /// Verifica se il titolario di appartenenza del folder in cui inserire il documento � chiuso o meno
    /// </summary>
    /// <param name="infoUtente"></param>
    /// <param name="idFolder">
    /// Id folder
    ...
    public void GetVisibilitaSemplificata(out System.Data.DataSet ds, string idProject)
    {
        ...
        q.setParam("param1", idProject);
        string queryStringRuoli = q.getSQL();
        logger.Debug("Query visibilit� di un fascicolo semplificata - RUOLI: " + queryStringRuoli);
        this.ExecuteQuery(out ds, "DIRITTI_RUOLI", queryStringRuoli);
    ...
    public System.Collections.ArrayList getQueryExportFasc(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.fascicolazione.Classificazione objClassificazione, DocsPaVO.filtri.FiltroRicerca[] objListaFiltri, bool enableUffRef, bool enableProfilazione, bool childs, DocsPaVO.utente.Registro registro, byte[] datiExcel, string serverPath, string[] idProjectsList)
    {
        ...
        try
        {
            ...
            // Se la lista dei system id � valorizzata, viene aggiunta un filtro dui system id
    ...
    public List<FascOrFolderBaseInfo> GetBaseInfoFascOrFolderByIdDoc(String idProfile)
    {
        ...
        q.setParam("idProfile", idProfile);
        string query = q.getSQL();
        logger.Debug("Query recupero informazioni di basei su fascicoli in cui � contenuto un documento:  " + query);
        DataSet dataSet = new DataSet();
                        ...
                        out List<SearchResultInfo> idProjectList, byte[] datiExcel, string serverPath, bool showGridPersonalization, bool export, Field[] visibleFieldsTemplate, String[] documentsSystemId, bool security)
    {
        ...
        try
        {
            ...
            {
                /* ABBATANGELI GIANLUIGI
                * Non carico i documenti perch� raggiunto il numero massimo
        ...
        {
            logger.Error(e, "errore in GetListaFascicoliPagingCustom");
            listaFascicoli = null;
        }
                                      ...
                                      bool getSystemIdList, byte[] datiExcel, string serverPath, bool showGridPersonalization, bool export, Field[] visibleFieldsTemplate, String[] documentsSystemId, bool conSecurity)
    {
        ...
        try
        {
            ...
            foreach (DocsPaVO.filtri.FiltroRicerca fil in objListaFiltri)
            {
                ...
                if(fil.argomento.Equals("STATO_CONSERVAZIONE"))
                {
                    from_versamento = true;
                }
            ...
            if (this.isConservazionePARER())
            {
                if (from_versamento)
                {
                    if (this.dbType == "SQL")
                    {
                        q.setParam("conservazione", " ," + getUserDB() + ".GetStatoConservazioneFasc(a.system_id) as StatoConservazione ");
                    }
                    else
                    {
                        q.setParam("conservazione", " ,GetStatoConservazioneFasc(a.system_id) as StatoConservazione ");
                    }
                }
                else
                {
        ...
        {
            logger.Error(e, "errore in setCommonParameterCustom");
            return null;
        }
    }
    ...
    public DocsPaVO.Grids.SearchObject GetDatiFascicoloCustom(DataSet dataSet, DataRow dataRow, Field[] visibleFieldsTemplate)
    {
        ...
        if (dataRow.Table.Columns.Contains("tipologia_fascicolo_piano_cons"))
        {
            objField = new DocsPaVO.Grids.SearchObjectField();
            objField.SearchObjectFieldValue = dataRow["tipologia_fascicolo_piano_cons"].ToString();
            objField.SearchObjectFieldID = "TIPOLOGIA_FASCICOLO";
            objDoc.SearchObjectField.Add(objField);
        }
                    ...
                    String[] documentsSystemId, int pageSize, DocsPaVO.filtri.FiltroRicerca[][] orderRicerca)
    {
        ...
        try
        {
            ...
            {
                listaDocumenti = null;
                throw new Exception("ID folder in GetDocumentiPagingCustom fascicolo � vuoto.");
            }
            if (filtriRicerca != null)
            {
                queryFrom = string.Empty;
...
documentsSystemId.Length > 0)
    ...
    public DocsPaVO.fascicolazione.Fascicolo getDettaglioFascicoloByIdNoSecurity(string idFascicolo, DocsPaVO.utente.InfoUtente infoUtente)
    {
        logger.Debug("GetFascicoloById");
        DocsPaVO.fascicolazione.Fascicolo fascicolo = null;
        ...
        try
        {
            ...
            this.ExecuteQuery(out dataSet, "PROJECT", query);
            if (dataSet.Tables["PROJECT"].Rows.Count > 0)
                fascicolo = GetFascicolo(infoUtente, dataSet, dataSet.Tables["PROJECT"].Rows[0], false);
            dataSet.Dispose();
        }
        catch (Exception e)
        {
           ...
           DocsPaVO.fascicolazione.Folder objFolder)
    {
        ...
        try
        {
            ...
            {
                result = false;
                throw new Exception("ID folder in GetIfDocumentiCountVisibleIsEgualNotVisible fascicolo � vuoto.");
            }
            ...
            if (this.ExecuteQuery(out dataSet, "DOCUMENTI", queryString))
            {
                ...
                {
                    // anche se non ci sono documenti il match � true: 0 � sempre uguale a 0
                    result = true;
                }
            }
            else
    ...
    private bool isConservazionePARER()
    {
        ...
        {
            logger.Debug("Errore nel reperimento della chiave di conservazione! {0}\r\n{1}", ex.Message, ex.StackTrace);
            result = false;
        }
        return result;

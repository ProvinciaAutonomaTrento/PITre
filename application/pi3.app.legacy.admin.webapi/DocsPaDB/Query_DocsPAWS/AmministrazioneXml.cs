...
{
        private static ILogger logger = Serilog.Log.ForContext(typeof(AmministrazioneXml));
        /// <summary>
        ///
        /// </summary>
	...
	public bool GetUserByName(out DocsPaVO.utente.InfoUtente infoUtente, string nomeUtente, string nomeRuolo)
	{
		...
		try
		{
			...
			var idPeople = infoUtente.idPeople;
            //if (!this.ExecuteScalar(out infoUtente.idPeople, command))
            if (!this.ExecuteScalar(out idPeople, command))
            {
                throw new Exception();
			}
			...
			var idGruppo = infoUtente.idGruppo;
            //this.ExecuteScalar(out infoUtente.idGruppo, command);
            this.ExecuteScalar(out idGruppo, command);
	...
	public string GetAdminByName(string nomeAmm)
	{
		...
		{
			logger.Debug(exception, "Errore durante l'acquisizione dell'amministrazione.");
	...
	/// <summary>
	/// Imposta la visibilit� di un dato nodo titolario/fascicolo/folder
	/// </summary>
	...
	public bool UpdateSecurity(string idPeople, string nodeId)
	{
		...
		{
			logger.Debug(exception, "Errore durante l'aggiornamento della visibilit� del nodo Titolario/Fascicolo/Folder.");
	...
	/// <summary>
	/// Inserisce un record di visibilit� per ogni utente di un dato ruolo. Se idPeople � NULL
	...
	public bool InsertSecurity(string idPeople, string idGruppo, string nodeId)
	{
		...
		{
			logger.Debug(exception, "Errore durante l'inserimento della visibilit�.");
	...
	public string NewAmministrazione(string codice, string descrizione, string libreria, string segnatura, string fascicolatura,string dominio,string serversmtp,string portasmtp,string protoInt,string ragioneTO,string ragioneCC,string usersmtp,string pwdsmtp)
	{
		...
		{
			logger.Debug(exception, "Errore durante l'inserimento dell'amministrazione.");
	...
	public bool UpdateAmministrazione(string codice, string descrizione, string libreria, string segnatura, string fascicolatura,string dominio,string serversmtp,string portasmtp,string protoInt,string ragioneTO,string ragioneCC,string usersmtp,string pwdsmtp)
	{
		...
		{
			logger.Debug(exception, "Errore durante l'aggiornamento dell'amministrazione.");
	...
	public string NewRegistro(string codice, string descrizione, string automatico, string indirizzo, string utente, string password, string smtp,	string pop,	string portaSmtp, string portaPop, string usersmtp, string pwdsmtp, string idAmm)
	{
		...
		{
			logger.Debug(exception, "Errore durante l'inserimento del registro.");
	...
	public bool NewServerPosta(string codice,string serverPop, string portaPop, string serverSmtp, string portaSmtp, string dominio, string descrizione)
	{
		...
		{
			logger.Debug(exception, "Errore durante l'inserimento del server di posta.");
	...
	public bool UpdateServerPosta(string codice,string serverPop, string portaPop, string serverSmtp, string portaSmtp, string dominio, string descrizione)
	{
		...
		{
			logger.Debug(exception, "Errore durante la modifica del server di posta.");
	...
	public bool DeleteServerPosta(string codice)
	{
		...
		{
			logger.Debug(exception, "Errore durante la cancellazione del server di posta.");
	...
	public string NewVisibilita(string thing,string personOrGroup)
	{
		...
		{
			logger.Debug("Errore durante l'inserimento della regola di visibilit�.", exception);
...
//				}
				...
				foreach(System.Data.DataRow trasmissione in trasmissioni.Tables[0].Rows)
				{
					...
					switch (trasmissione[1].ToString())
					{
						case "R": // ruolo
							...
							//verifica se il nuovo utente ha gi� questa trasmissione
							...
							if(this.ExecuteQuery(out preTrasm,command))
							{
								if(preTrasm!=null)
								{
									if(preTrasm.Tables[0].Rows.Count>0)
									{
										...
										if(valore>0)
										{
											logger.Debug("Trovate gi� trasmissioni per il nuovo utente: " + idNewUser + ". Esegue cancellazione delle trasmissioni del vecchio utente: " + idOldUser);
	...
	public bool DeleteVisibilita(string thing)
	{
		...
		{
			logger.Debug("Errore durante la cancellazione delle regole di visibilit�.", exception);
	...
	public bool CopyVisibilita(string source,string destination)
	{
		...
		{
			logger.Debug("Errore durante la copia delle regole di visibilit�.", exception);
	...
	/// <summary>
	/// funzione che torna true se il codice 'code' � univoco nel campo 'column' della
	/// tabella 'table'
	/// </summary>
	...
	/// <summary>
	/// Verifica dell'amministratore corrente. Se l'amministratore non � gi� collegato
	/// oppure � lo stesso utente che si vuole collegare, restituisce il nome dell'utente
	...
	public bool CheckVisibilitaUtente(string peopleId)
	{
		...
		try
		{
			...
			{
				//letti i documenti
				//verifica che i documenti abbiano visibilit� anche da altri utenti
				...
				//per il quale c'� la visibilit� del solo utente passato
				if(documenti!=null)
				{
		...
		{
			logger.Debug("Errore durante la lettura delle visibilit� private dell'utente.", exception);
...
//				}
								...
								string R_W)
	{
		...
		try
		{
			...
			// inserisce la visibilit� dei ruoli in UO ----------------------------------------------------

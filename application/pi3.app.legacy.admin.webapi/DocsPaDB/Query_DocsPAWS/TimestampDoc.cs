...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(TimestampDoc));
    public void saveTSR(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.areaConservazione.OutputResponseMarca resultMarca, DocsPaVO.documento.FileRequest fileRequest)
    {
        ...
        try
        {
            ...
            if (fileRequest != null && !string.IsNullOrEmpty(fileRequest.versionId))
            ...
            if (resultMarca != null && !string.IsNullOrEmpty(resultMarca.docm_date))
            ...
            if (resultMarca != null && !string.IsNullOrEmpty(resultMarca.dsm))

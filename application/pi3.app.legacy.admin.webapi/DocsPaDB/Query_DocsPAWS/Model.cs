...
public class Model
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Model));
        ...
        try
        {
            ...
            if (dbProvider.ExecuteNonQuery(commandText, out rowsAffected))
            {
                ...
                if (retValue)
                {
                    ...
                    GetIdLastInserted(dbProvider, out system_id_templates, "DPA_TIPO_ATTO");
                }
                else
                {
                    dbProvider.RollbackTransaction();
            ...
            for (int i = 0; i < template.ELENCO_OGGETTI.Count; i++)
            {
                ...
                if (dbProvider.ExecuteNonQuery(commandText, out rowsAffected))
                {
                    ...
                    if (retValue)
                    {
                        ...
                        GetIdLastInserted(dbProvider, out system_id_oggettoCustom, "DPA_OGGETTI_CUSTOM");
                ...
                /*
                //Poich� sto inserendo un nuovo oggetto della tipologia devo dare la visibilit� a tutti i ruoli dell'amministrazione
                ...
                */
                ...
                // Se l'oggetto inserito � un contatore di repertorio, viene inserito nell'anagrafica
    ...
    {
        //Il seguente metodo elimina l'oggettoCustom dal Database, non � stato possibile realizzare
        //l'eliminazione nel metodo "aggiornaTemplate" impostando il tipo di operazione, perch� si
        ...
        try
        {
            ...
            if (template.IN_ESERCIZIO == "NO")
            {
                ...
                //Cancellazione visibilit� oggetto
            ...
            {
                //controllo se il contatore � di custom,ossia se esiste un record all'interno della
    ...
    public bool aggiornaTemplate(DocsPaVO.ProfilazioneDinamica.Templates template)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < template.ELENCO_OGGETTI.Count; i++)
            {
                ...
                if (oggettoCustom.TIPO_OPERAZIONE.Equals("DaAggiungere"))
                {
                    ...
                    if (dbProvider.ExecuteNonQuery(commandText, out rowsAffected))
                    {
                        ...
                        if (retValue)
                        {
                            ...
                            //if (system_id_oggettoCustom == null) {
                            //    Thread.Sleep(100);
                            //    dbProvider.ExecuteScalar(out system_id_oggettoCustom, commandText);
                            //}
                ...
                {
                    //controllo se il contatore � di tipo custom, in caso positivo aggiorno la tabella
    ...
    private void GetIdLastInserted(DBProvider dbProvider, out string system_id, string tableName) {
        // Reperimento systemid

        var commandText = DocsPaDbManagement.Functions.Functions.GetQueryLastSystemIdInserted(tableName);
        dbProvider.ExecuteScalar(out system_id, commandText);

        if (system_id == null)
        {
            logger.Error("Errore in GetIdLastInserted  - system_id == null first attempt");
            Thread.Sleep(5000);
            dbProvider.ExecuteScalar(out system_id, commandText);
        }
        if (system_id == null) {
            logger.Error("Errore in GetIdLastInserted  - system_id == null second attempt");
        }
        else
            logger.Debug($"GetIdLastInserted valore letto - {tableName} = {system_id}");
    ...
    public DocsPaVO.ProfilazioneDinamica.Templates getTemplateById(string idTemplate)
    {
        ...
        try
        {
            ...
            //Se il template non ha oggetti custom vengono restituite solo le propriet� del template
    ...
    public void salvaInserimentoUtenteProfDim(DocsPaVO.ProfilazioneDinamica.Templates template, string docNumber)
    {
        ...
        try
        {
                ...
                for (int i = 0; i < template.ELENCO_OGGETTI.Count; i++)
                {
                    ...
                    switch (oggettoCustom.TIPO.DESCRIZIONE_TIPO)
                    {
                        ...
                        case "ContatoreSottocontatore":
                            ...
                            //true = se l'oggetto � da inserire - come in questo caso
                            //false= se l'oggetto � da aggiornare
                            ...
                            //Perch� � stata messa questo inserimento che dipende dal culture info del pc?
                            ...
                            //Se il contatore � da far scattare aggiorno la nuova colonna VAR_SEGNATURA
                            DocsPaDB.Query_Utils.Utils date = new DocsPaDB.Query_Utils.Utils();
                            oggettoCustom.DATA_INSERIMENTO = date.GetDBDate(true);
                            if (oggettoCustom.CONTATORE_DA_FAR_SCATTARE)
                                queryMng.setParam("var_segnatura", CostruzioneSegnaturaRepertorio(oggettoCustom, template.ID_AMMINISTRAZIONE, docNumber));
                            else
                                queryMng.setParam("var_segnatura", "");
            ...
            else
            {
                ...
                List<StoricoProfilatiOldValue> oldOggCustom = template.OLD_OGG_CUSTOM;
                ...
                for (int i = 0; i < template.ELENCO_OGGETTI.Count; i++)
                {
                    ...
                    if (modifica)
                    {
                        ...
                        {
                            case "CasellaDiSelezione":
                                int modif_cas_selez = 0; //booleano che mi informa se � cambiato lo stato della casella di selezione
                                oggettoCustom.gestisciCaratteriSpeciali();
                                for (int j = 0; j < oggettoCustom.ELENCO_VALORI.Count; j++)
                                {
                            ...
                            case "ContatoreSottocontatore":
                                ...
                                {
                                    //verifico se nel contatore ci sia gi� impostato un valore numerico: in questo caso non faccio update
                                    ...
                                    {
                                        logger.Debug("Il contatore non presenta un valore numerico e quindi si pu� effettuare l'aggiornamento");
                                        continua = true;
                                    }
                                    ...
                                    //{
                                    //Il parametro booleano � :
                                    //true = se l'oggetto � da inserire
                                    //false= se l'oggetto � da aggiornare - come in questo caso
                                    if (oggettoCustom.CAMPO_COMUNE != "1")
                                    ...
                                    if (oggettoCustom.CONTATORE_DA_FAR_SCATTARE)
                                    {
                                        DocsPaDB.Query_Utils.Utils date = new DocsPaDB.Query_Utils.Utils();
                                        oggettoCustom.DATA_INSERIMENTO = date.GetDBDate(true);
                                        queryMng.setParam("var_segnatura", ", VAR_SEGNATURA = '" + CostruzioneSegnaturaRepertorio(oggettoCustom, template.ID_AMMINISTRAZIONE, docNumber) + "'");
                                    }
                                    else
                                    {
                                        queryMng.setParam("var_segnatura", "");
    ...
    private string CostruzioneSegnaturaRepertorio(OggettoCustom oggetto, string idAmm, string docnumber)
    {
        string segnaturaRepertorio = string.Empty;
        try
        {
            string formato_contatore = oggetto.FORMATO_CONTATORE;
            string valore_database = oggetto.VALORE_DATABASE;
            string anno = oggetto.ANNO;
            string codice_db = oggetto.CODICE_DB;
            string data_inserimento = oggetto.DATA_INSERIMENTO;
            string id_aoo_rf = oggetto.ID_AOO_RF;

            DocsPaDB.Query_DocsPAWS.Amministrazione amm = new DocsPaDB.Query_DocsPAWS.Amministrazione();
            string codiceAmm = amm.AmmGetInfoAmmCorrente(idAmm).Codice;
            ...
            if (!string.IsNullOrEmpty(valore_database))
            {
                ...
                if (!string.IsNullOrEmpty(id_aoo_rf) && id_aoo_rf != "0")
                {
...

                    {
                        ...
                        else //se contatore di AOO non ho i dati per ricavare RF perch� non mi viene passato in input.
                        {
        ...
        {
            logger.Error("Errore in CostruzioneSegnaturaRepertorio " + e.Message);
        }
    ...
    public bool disabilitaTemplate(DocsPaVO.ProfilazioneDinamica.Templates template, string idAmministrazione, string serverPath, string codiceAmministrazione)
    {
        ...
        try
        {
            ...
            #region Codice Commetato - Non viene pi� eliminato il template ma solo disabilitato
            /*
            if (template.IN_ESERCIZIO.Equals("NO"))
            {
                ...
                for (int i = 0; i < template.ELENCO_OGGETTI.Count; i++)
                {
                    ...
                    //Se � un oggetto comune non devo cancellarlo correttamente ho precedentemente
                ...
                //Elimino la visibilit� sul template
            ...
            */
            #endregion Codice Commetato - Non viene pi� eliminato il template ma solo disabilitato
    ...
    public void messaInEsercizioTemplate(DocsPaVO.ProfilazioneDinamica.Templates template, string idAmministrazione)
    {
        ...
        try
        {
            ...
            OggettoCustom obj = ((OggettoCustom[])template.ELENCO_OGGETTI.ToArray()).Where(o => o.TIPO.DESCRIZIONE_TIPO.ToLower() == "contatore" && o.REPERTORIO == "1").FirstOrDefault();
            if (obj != null)
            {
            ...
            OggettoCustom cc =((OggettoCustom[])template.ELENCO_OGGETTI.ToArray()).Where(c => c.TIPO.DESCRIZIONE_TIPO.ToLower() == "contatore" && !string.IsNullOrEmpty(c.DATA_INIZIO) && !string.IsNullOrEmpty(c.DATA_FINE) ).FirstOrDefault();
            if (cc != null)
            {
    ...
    public void salvaModelli(byte[] dati, string nomeProfilo, string codiceAmministrazione, string nomeFile, string estensione, string serverPath, DocsPaVO.ProfilazioneDinamica.Templates template)
    {
        ...
        //TODO: tolto temporaneame
            ...
            {
                if (template.PATH_MODELLO_1.IndexOf('/') == 0 || estensione.Equals("MTXT"))
                    queryMng.setParam("pathModUno", template.PATH_MODELLO_1.Replace("\\", "/"));
                else
                    queryMng.setParam("pathModUno", serverPath + "/" + template.PATH_MODELLO_1.Replace("\\", "/"));
            ...
            {
                if (template.PATH_MODELLO_2.IndexOf('/') == 0 || estensione.Equals("MTXT"))
                    queryMng.setParam("pathModDue", template.PATH_MODELLO_2.Replace("\\", "/"));
                else
                    queryMng.setParam("pathModDue", serverPath + "/" + template.PATH_MODELLO_2.Replace("\\", "/"));
            ...
            {
                if (template.PATH_MODELLO_STAMPA_UNIONE.IndexOf('/') == 0)
                    queryMng.setParam("pathModSU", template.PATH_MODELLO_STAMPA_UNIONE.Replace("\\", "/"));
                else
                    queryMng.setParam("pathModSU", serverPath + "/" + template.PATH_MODELLO_STAMPA_UNIONE.Replace("\\", "/"));
            }
            else
            {
            ...
            {
                if (template.PATH_ALLEGATO_1.IndexOf('/') == 0)
                    queryMng.setParam("pathAllUno", template.PATH_ALLEGATO_1.Replace("\\", "/"));
                else
                    queryMng.setParam("pathAllUno", serverPath + "/" + template.PATH_ALLEGATO_1.Replace("\\", "/"));
            ...
            {
                if (template.PATH_MODELLO_EXCEL.IndexOf('/') == 0)
                    queryMng.setParam("pathmodexc", template.PATH_MODELLO_EXCEL.Replace("\\", "/"));
                else
                    queryMng.setParam("pathmodexc", serverPath + "/" + template.PATH_MODELLO_EXCEL.Replace("\\", "/"));
            }
            else
            {
    ...
    public void eliminaModelli(string nomeProfilo, string codiceAmministrazione, string nomeFile, string estensione, string serverPath, DocsPaVO.ProfilazioneDinamica.Templates template)
    {
        ...
        try
        {
            ...
            if (template.PATH_MODELLO_1 != "")
            {
                if (template.PATH_MODELLO_1.IndexOf('/') == 0)
            ...
            if (template.PATH_MODELLO_2 != "")
            {
                if (template.PATH_MODELLO_2.IndexOf('/') == 0)
    ...
    public void UpdateConsolidaCampo(int systemId, string consolida, string systemId_template)
    {
        ...
        try
        {
            ...
            // se si disattiva il flag consolidamento � necessario rimuovere anche i flag conservazione e conservazione repertorio
            ...
            // se il parametro systemId_template non � null devo attivare il campo invio conservazione per il template
            /*
    ...
    public void UpdateConservaCampo(int systemId, string conserva, string systemId_template)
    {
        ...
        try
        {
            ...
            // se si attiva il flag consolidamento � necessario attivare anche il flag conservazione
            // se invece si disattiva, � necessario disattivare il flag conserva repertorio
    ...
    //questa funzione controlla se � associato un record nella DPA_CONT_CUSTOM_DOC
    ...
    private void calcolaContatore(DocsPaVO.ProfilazioneDinamica.Templates template, ref DocsPaVO.ProfilazioneDinamica.OggettoCustom oggettoCustom, bool inserimentoAggiornamento)
    {
            ...
            {
                {
                    ...
                    if (ds_datiContatore.Tables[0].Rows.Count == 0)
                    {
                        ...
                        GetIdLastInserted(dbProvider, out idContatoreInserito, "DPA_CONTATORI_DOC");
                    ...
                    if (ds_datiContatore.Tables[0].Rows.Count != 0)
                    {
                        ...
                        //Caso in cui non ci troviamo nella situazione di inserire un nuovo oggettoCustom perch� gi� esiste
                        //e inoltre non dovendo far scattere il conta dopo, non si deve incementare nulla  a meno che il contatore non � stato inserito ora
        ...
        else
        {
            ...
            {   //Verifico se deve scattare il contatore
                ...
                //perch� posso decidere di far scattare il contatore in momenti diversi
                    ...
                    switch (oggetto.TIPO.DESCRIZIONE_TIPO)
                    {
                        ...
                        case "Contatore":
                            ...
                            if(!string.IsNullOrEmpty(oggetto.DATA_INSERIMENTO))
                            {
                                if (oggetto.DATA_INSERIMENTO.IndexOf('@') != -1)
                                {
                                    ...
                                    filterDataRepertorio += " AND dpa0.valore_oggetto_db IS NOT NULL AND dpa0.DTA_INS >=" + DocsPaDbManagement.Functions.Functions.ToDateBetween(dataInserimento[0], true) + " AND dpa0.DTA_INS <=" + DocsPaDbManagement.Functions.Functions.ToDateBetween(dataInserimento[1], false);
                                }
                                else
                                {
                                    filterDataRepertorio += " AND dpa0.valore_oggetto_db IS NOT NULL AND dpa0.DTA_INS >=" + DocsPaDbManagement.Functions.Functions.ToDateBetween(oggetto.DATA_INSERIMENTO, true) + " AND dpa0.DTA_INS <=" + DocsPaDbManagement.Functions.Functions.ToDateBetween(oggetto.DATA_INSERIMENTO, false);
                                }
                            }
                        ...
                        case "Data":
                            if (oggetto.VALORE_DATABASE != null && oggetto.VALORE_DATABASE != "")
                            {
                                ...
                                {
                                    // Ricerca per data con la possibilit� di passare l'operatore di ricerca
                        ...
                        case "Corrispondente":
                            if (oggetto.VALORE_DATABASE != null && oggetto.VALORE_DATABASE != "")
                            {
                                ...
                                if (Int32.TryParse(oggetto.VALORE_DATABASE, out codiceCorrispondente))
                                {
                                    ...
                                    {
                                        if (DocsPaVO.Settings.AppSettings.Instance.DBType.ToUpper() == "SQL")
                                        {
    ...
    public void estendiDirittiRuoloACampiDoc(ArrayList listaDirittiRuoli, ArrayList listaCampi)
    {
        ...
        try
        {
            ...
            {
                //Se il diritto sulla tipologia � 0, allora ai campi non viene dato nessun diritto
                ...
                //Se il diritto sulla tipologia � 1, allora ai campi viene data solo la visibilit�
                ...
                //Se il diritto sulla tipologia � 2, allora ai campi vengono dati pieni diritti
    ...
    private void setTemplate(ref DocsPaVO.ProfilazioneDinamica.Templates template, DataSet dataSet, int rowNumber)
    {
        ...
        {
            logger.Debug("SQL - setTemplate - ProfilazioneDinamica/Database/model.cs - Exception : {0} \r\n{1}", ex.Message, ex.StackTrace);
        }
    }
    ...
    private void setOggettoCustom(ref DocsPaVO.ProfilazioneDinamica.OggettoCustom oggettoCustom, DataSet dataSet, int rowNumber)
    {
        try
        {
            ...
            if (dataSet.Tables[0].Columns.Contains("VAR_SEGNATURA"))
            {
                if (!string.IsNullOrEmpty(dataSet.Tables[0].Rows[rowNumber]["VAR_SEGNATURA"].ToString()))
                    oggettoCustom.VAR_SEGNATURA = dataSet.Tables[0].Rows[rowNumber]["VAR_SEGNATURA"].ToString();
            }
        }
        catch (Exception ex)
        {
    ...
    public DocsPaVO.ProfilazioneDinamica.Templates impostaCampiComuniDoc(DocsPaVO.ProfilazioneDinamica.Templates modello, ArrayList campiComuni)
    {
        ...
        //Quando vengono associati uno o pi� campi comuni, vengono ricalcolte le posizioni dei campi non comuni
        ...
        try
        {
            ...
            for (int i = 0; i < campiComuni.Count; i++)
            {
                ...
                //Verifico se il campo comune � gi� presente nella DPA_OGG_CUSTOM_COMP per il template interessato,
    ...
    public void insertInStorico(DocsPaVO.ProfilazioneDinamica.OggettoCustom oggettoCustom, List<StoricoProfilatiOldValue> listOldObj, int rowsEffected, ref int indexListOldObj)
    {
        logger.Information("BEGIN");
        DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider();
        try
    ...
    public ArrayList getListaStoricoAtto(string id_tipo_atto, string doc_number, string idGroup)
    {
        ...
        using (DBProvider dbProvider = new DBProvider())
        {
            ...
            //impostazione dell'id del documento al quale � associato il modello
    ...
    public bool UpdateIsTypeInstance(DocsPaVO.ProfilazioneDinamica.Templates template, string idAmministrazione)
    {
        ...
        {
            logger.Information("Errore in DocsPaDb.Query_DocsPAWS.Models - Metodo UpdateIsTypeInstance", exc);
            return false;
        }
        return result;
    }
    ...
    public ArrayList getIdDocByAssTemplates(string idOggetto, string valoreDB, string ordine)
    {
        ...
        {
            retVal = null;
            logger.Error(ex, "errore in getIdDocByAssTemplates");
        }

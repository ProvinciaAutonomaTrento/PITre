...
public class RF : DBProvider
	{
    private static ILogger logger = Serilog.Log.ForContext(typeof(RF));
    ...
    public ArrayList getCorrispondentiByCodRF(string codiceRF, string idAmm)
    {
        ...
        {
            //semaforo.WaitOne();
            DocsPaUtils.Query queryMng = DocsPaUtils.InitQuery.getInstance().getQuery("GET_CORRISPONDENTE_BY_RF_IDAMM");
            queryMng.setParam("param1", codiceRF.ToUpper().Replace("'", "''"));
    ...
    public RaggruppamentoFunzionale GetRaggruppamentoFunzionaleRC(String idRf)
    {
        ...
        using (DBProvider dbProvider = new DBProvider())
        {
            ...
            // Sostituzione del dbUser se il db � sql server

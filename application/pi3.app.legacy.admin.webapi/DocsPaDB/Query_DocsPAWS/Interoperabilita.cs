...
public partial class Interoperabilita : DBProvider
	{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Interoperabilita));
    ...
    public void findIdProf(out DataSet dataSet,string codiceAOO,string numeroRegistrazione,int anno)
        ...
        {
            //rifaccio il controllo con anno precedente perch� potrebbe essere che ricevo una conferma di un protocollo dell'anno
            //precedente, protocollato dal destinatario nell'anno successivo.. pi� di un anno di differenza sembra molto imporbabile.
    ...
    public bool updStatoInvioEccezione(string SystemID,  string motivo_annulla)
    {
        ...
        if (!string.IsNullOrEmpty(statusmask))
        {
            ...
            if (sm[5] == 'A' && sm[2]=='V')
            {
                ...
                sm[4] = 'N';
                sm[5] = 'X';
                if (motivo_annulla.Contains("Il documento � stato ricevuto dall�Amministrazione destinataria e pertanto non occorre effettuare rispedizioni."))
                {
                    sm[0] = 'A';
                    sm[3] = 'A';
                    sm[4] = 'A';
    ...
    public string addNewCorrAllegato6(/*DocsPaWS.Utils.Database db,*/string tipoIE, DocsPaVO.utente.Registro reg, string mailMitt, string descrizioneAmm, string codiceAmm, string tipoCorr, string codiceAOO, string descrInterAmm, string codEdesc)
    {
        DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("I_DPACorrGlobali4");
        string dataInizio = DocsPaDbManagement.Functions.Functions.ToDate(System.DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss tt", new CultureInfo("en-US")));
    ...
    /// <summary>
    /// Ricava i destinatari del protocollo in partenza dal quale � stato originato il predisposto
    /// in arrivo durante il processo di interoperabilit� interna (interop_int_no_mail = "1")
    /// </summary>
    /// <param name="ds"></param>
    ...
    public void getIdProtocolloUscitaOriginario(out DataSet ds, string numProto, string codRegistro, int anno)
    {
        ...
        if (string.IsNullOrEmpty(mailAddress) &&
            DocsPaVO.Settings.AppSettings.Instance.INTEROP_INT_NO_MAIL != null &&
        ...
        */
        ...
        //Si verifica se � gi� associato un tipo canale al corrispondente
    ...
    public String GetLivelloAnomaliaSegnatura(string email, string anomalia)
    {
        logger.Debug("START : DocsPaDB > Query_DocsPAWS > Interoperabilita > GetLivelloAnomaliaSegnatura");
        DataSet ds = new DataSet();
        DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_DPA_LIVELLO_ANOMALIA_SEGNATURA");
        string emailQuery = "'" + email + "'";
        string[] s = email.Split('@');
        if (s != null && s.Count() > 1)
            emailQuery += ", '*" + s[1] + "'";
        q.setParam("email", emailQuery.ToUpper());
        q.setParam("anomalia", anomalia.ToUpper());
        string queryString = q.getSQL();
        logger.Debug(queryString);
        ExecuteQuery(out ds, "ANOMALIA", queryString);
        string livello = "2"; //se non presente eccezione bloccante
        if (ds != null && ds.Tables["ANOMALIA"].Rows.Count > 0)
        {
            DataRow regRow = ds.Tables["ANOMALIA"].Rows[0];
            livello = regRow["CHA_LIVELLO_ECCEZIONE"].ToString();
        }
        logger.Debug("END : DocsPaDB > Query_DocsPAWS > Interoperabilita > GetLivelloAnomaliaSegnatura");
        return livello;
    }
    ...
    public DocsPaVO.Spedizione.StatiSpedizioneDocumentoEnum GetStatoInvio(DocsPaVO.utente.InfoUtente infoUtente, string idProfile, string idDest)
    {
        string stato = string.Empty;
        StatiSpedizioneDocumentoEnum statoSpedizione = StatiSpedizioneDocumentoEnum.Spedito;
        try
        {
            DocsPaUtils.Query queryDef = DocsPaUtils.InitQuery.getInstance().getQuery("S_DPA_STATO_INVIO_STATUSMASK");
            queryDef.setParam("idProfile", idProfile);
            queryDef.setParam("destinatario", idDest);
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                dbProvider.ExecuteScalar(out stato, commandText);
            }

            if (!string.IsNullOrEmpty(stato))
            {
                InfoSpedizione.TipologiaAzione azione = GetStatoAzione(stato.Substring(0, 1));
                if (azione.Equals(InfoSpedizione.TipologiaAzione.Rispedire))
                    statoSpedizione = StatiSpedizioneDocumentoEnum.DaRispedire;
            }
        }
        catch(Exception e)
        {
            logger.Error("Errore in GetStatoInvio " + e.Message);
            statoSpedizione = StatiSpedizioneDocumentoEnum.Spedito;
        }
    ...
    public bool InsertInStoricoSpedizioni(string idDocumento, string Corrispondente, string esito, string mailAddress, string idGruppo, DocsPaVO.utente.Canale canale, string mail_mittente, string idRegistroMailMittente)
    {
        try
        {
            string descCanPref = (canale != null ? canale.descrizione : "MAIL");
            bool retval = false;
            logger.Debug("Insert in storico spedizioni, documento {0}, idCorrispondente {1}, esito {2}", idDocumento, Corrispondente, esito);
            // Il mezzo � da ricavare nella DPA_DOC_ARRIVO_PAR con corrispondente e iddocumento.
            ...
            // Modifica per l'inserimento nello storico. Utilizzo la stessa condizione della spedizione se un destinatario � IS
            if (string.IsNullOrEmpty(mailAddress) || (canale != null && canale.typeId != null && canale.typeId.Equals(InteroperabilitaSemplificata.SimplifiedInteroperabilityId)))
            {
        ...
        {
            logger.Error("Errore durante nell'inserimento dello storico. Messaggio: {0}", e.Message);
    ...
    {
        logger.Debug("Query in storico spedizioni per il documento {0}", idDocument);
        ArrayList lista = new ArrayList();
        try
        {
        ...
        {
            logger.Error("Errore durante nella query dello storico. Messaggio: {0}", e.Message);
            logger.Error("Stacktrace: {0}", e.StackTrace);
        }
        return lista;
    ...
    public bool UpdateStoricoSpedizione(string idDoc, string Corrispondente, string idRuolo, string idRegMittente, string esito, bool errore)
    {
        try
        {
            ...
            if (!errore)
            {
                logger.Debug("Update storico spedizioni, documento {0}, idCorrispondente {1}, esito {2}", idDoc, Corrispondente, esito);
            ...
            {
                logger.Debug("Update storico spedizioni, errore generale, documento {0}, esito {1}", idDoc, esito);
        ...
        {
            logger.Error("Errore durante nell'update dello storico. Messaggio: {0}", e.Message);
            logger.Error("Stacktrace: {0}", e.StackTrace);
            return false;
        }
    }
    ...
    private static bool isLastUO(XmlElement elUO)
		...
		private static string getPA(XmlElement elUO)
		{
			...
			if(elUoInt==null)
			{
				//non ci sono ruoli interni, non ci sono unit� interne.
				return "1";
			}
			else
			{
    ...
    public List<InfoDocumentoSpedito> GetReportSpedizioni(FiltriReportSpedizioni filters, string idPeople, string idGruppo)
    {
        try
        {
            ...
            // Controllo della visibilit� nella query
    ...
    private DocsPaVO.Spedizione.InfoSpedizione.TipologiaStatoRicevuta GetStatoRicevuta(string charMask)
    {
         ...
         * Nella tabella dpa_stato_invio � presente un campo di 7 caratteri denominato status_c_mask. I caratteri indicano
         * Essi possono assumere i valori A (Attendere), V (OK), X (KO), N(Non prevista)
         * 1 - Esito generale
         * Indica se una spedizione ha raggiunto l'esito finale, se � in stato di attesa o se ha avuto un errore. Per le spedizioni via mail
        ...
        try
        {
            ...
            if (dsSpedizioni.Tables["Spedizioni"].Rows.Count > 0)
            {
                ...
                foreach (DataRow rowSpedizioni in dsSpedizioni.Tables["Spedizioni"].Rows)
                {
                    ...
                    //Modifica per IS utilizzo il TYPE_ID e non la DESCRIPTION
                    spedizione.MezzoSpedizione = rowSpedizioni["MEZZO"].ToString();
                    spedizione.IdMezzoSpedizione = rowSpedizioni["TYPE_ID_MEZZO"].ToString();

...
public class Note
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Note));
            ...
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                if (nota.DaRimuovere == false)
                {
                    if (nota.Testo.Contains("°"))
                        nota.Testo = nota.Testo.Replace("°", "&ordm;");
    ...
    {
        string retValue = VISIBILITA_TUTTI;

        if (tipoVisibilita == TipiVisibilitaNotaEnum.Tutti)
        ...
        if (filtroRicerca != null)
        {
            if (!string.IsNullOrEmpty(filtroRicerca.Testo))
            {
                ...
                if (!string.IsNullOrEmpty(filtro))
                    filtro = string.Format(" AND {0}", filtroTesto);
                else
                    filtro = filtroTesto;
            }
        }

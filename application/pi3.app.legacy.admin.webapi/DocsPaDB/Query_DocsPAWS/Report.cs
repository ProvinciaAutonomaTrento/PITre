...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Report));
    ...
    private string GetProfOggFilterString(DocsPaVO.filtri.FiltroRicerca[][] filterList)
    {
        ...
        if (filters.ContainsKey("ID_UO_PROT_GERARCHIA"))
        {
            ...
            for (int i = 0; l != null && i < l.Count; i++)
            {
                ...
                if (i == 0)
                    queryString += ","; // c'� id della uo padre.
    ...
    public void UpdProf(DocsPaVO.documento.SchedaDocumento sd, string idReg, string docNumber)
    {
        ...
        q.setParam("param4", "'" + sd.oggetto.descrizione.Replace("'", "''") + "'");
        ...
        // preventivamente se il tipo proto della scheda � null ed in questo caso
        // viene impostato a R
    ...
    public ArrayList GetQueryReportAvanzatiXLS(DocsPaVO.ExportData.ExportExcelClass objReport)
    {
        ...
        switch (objReport.filtro.tipologiaReport)
        {
            ...
            case "PR_REG_R":
                ...
                break;
            /********************************************************************************/
            //Giordano Iacozzilli: Gestire in report con una sola query � complesso, per celerit� ho diviso
            //la query in 4 sotto query che andr� a sistemare in una datatable.
            case "PFCNC_REG_R":
                protoIntEnabled = this.isProtoIntEnabled(objReport.filtro.idAmministrazione);
                /********************************************************************************/
                //Il primo set di dati sono i protocolli in Uscita ed Entrata pi� gli altri campi utili valorizzati a ZERO
                /********************************************************************************/

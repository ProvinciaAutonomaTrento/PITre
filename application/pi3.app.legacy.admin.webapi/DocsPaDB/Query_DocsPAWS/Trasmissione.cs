...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Trasmissione));
    ...
    /// <summary>
    /// Questo metodo, a seconda della modalit� (accettazione o rifiuto) ritorna la query che verr� eseguita 
    ...
    public string GetUpdateTrasmissioneUtente(DocsPaVO.trasmissione.TrasmissioneUtente objTrasmUtente, string modo, string idTrasmissione, DocsPaVO.utente.InfoUtente infoUtente)
    {
        ...
        if (modo == "ACCETTATA")
        {
            ...
            //Nuovo sistema: se si accetta una trasmissione dalla todolist, ma non si � 
            //ancora visto il documento o fascicolo, questo non scompare dalla todolist, ma vi rimane finch� non
            //viene visto
        ...
        retValue = q != null ? q.getSQL() : string.Empty;
    ...
    /// <summary>
    /// Questo metodo, aggiorna la dpa_trasm_utente solamente se la trasmissione � di tipo UNO 
    /// (questo poich� se la trasmissione � di tipo UNO se un componente del ruolo l'ha accettata
                    ...
                    )
                {
                    ...
                    */
                    ...
                    switch (retProc)
                    {
                        ...
                        case 3:
                            this.RollbackTransaction();
                            logger.Debug("ERRORE - Eseguita Rollback alla Stored Procedure: SP_TRASM_RIF_TORNA_MITT - CAUSA : Query del reperimento della system_id della profile se il tipo oggetto � 'documento'");
                            break;
                        case 4:
                            this.RollbackTransaction();
                            logger.Debug("ERRORE - Eseguita Rollback alla Stored Procedure: SP_TRASM_RIF_TORNA_MITT - CAUSA : Query del reperimento della system_id della project se il tipo oggetto � 'fascicolo'");
                            break;
                        case 5:
                            this.RollbackTransaction();
    ...
    /*public void execSenderRigths(DocsPaVO.trasmissione.Trasmissione objTrasm)
    {
        if (objTrasm.cessione != null)
        {
            if (objTrasm.cessione.docCeduto)
            {
                ...
                //l'utente cede il suo diritto di propriet�
                ...
                { 
                    //l'utente cede il suo diritto non di propriet�
                    string tipoDiritto = "";
                    string accessRight = "";
                    string cod_rubrica = "";
            ...
            if (objTrasm.cessione.docCeduto)
            {
                try
                {
                    ...
                    using (DocsPaDB.DBProvider dbProvider = new DBProvider())
                    {
                        ...
                        // verifica se � proprietario come UTENTE...
                        ...
                        logger.Debug("Verifica la propriet� dell'oggeto come UTENTE. isPersonOwner: {0}", isPersonOwner.ToString());

                        // gestione del doc. personale
                        if (tipoOggettoTrasm.Equals("D") && isPersonOwner && docPersonale)
                        {
                            isGroupOwner = true; // imposta a true perch� per i doc personali non esiste il ruolo impostato                                
                        }
                        else
                        {
                            // verifica se � proprietario come RUOLO...
                        ...
                        logger.Debug("Verifica la propriet� dell'oggeto come RUOLO. isGroupOwner: {0}; tipoOggettoTrasm: {1}", isGroupOwner.ToString(), tipoOggettoTrasm);
                        ...
                        try
                        {
                            ...
                            logger.Debug("Valore chiave FE_CEDI_DIRITTI_IN_RUOLO: {0}", valoreChiaveCediDiritti);
                            if (!string.IsNullOrEmpty(valoreChiaveCediDiritti) && valoreChiaveCediDiritti.Equals("1"))
                            {
                                ...
                                //che l'utente che sta facendo la cessione dei diritti � il proprietario, anche se non lo �.
                                //Deve per� almeno far parte del Ruolo del proprietario (255).
                                if (isGroupOwner)
                                    isPersonOwner = true;
                                //FINE
                                //**************************************************************************************************************
                                ...
                                //di un utente che non � l'Owner del doc.
                                //LOG APPLICATIVO
                        ...
                        // se non � il proprietario come utente e ruolo, ceder� i diritti acquisiti...
                        if (isPersonOwner && isGroupOwner)
                        {
                            tipoDirittoLog = "di propriet�";

                            logger.Debug("Cessione Diritti. Tipo Diritto: {0}, isPersonOwner: {1}, isGroupOwner: {2}", tipoDirittoLog, isPersonOwner, isGroupOwner);
                            logger.Debug("Valore idPeopleNewPropr: {0}, idRuoloNewPropr: {1}", objTrasm.cessione.idPeopleNewPropr, objTrasm.cessione.idRuoloNewPropr);
                            //l'utente cede il suo diritto di propriet�, prima come utente poi come ruolo
                            if ((objTrasm.cessione.idPeopleNewPropr != null && objTrasm.cessione.idRuoloNewPropr != null) ||
                                (docPrivate && string.IsNullOrEmpty(objTrasm.cessione.idRuoloNewPropr)))
                            {
                                logger.Debug("Cessione Diritti Di Proriet�. L'utente cede il suo diritto di propriet�, prima come utente poi come ruolo");
                                logger.Debug("Inizio cessione diritti di propriet� documento/fascicolo come Utente");
                                ...
                                logger.Debug("Cessione Diritti di Propriet� Utente con Mantieni Diritti per Documento/fascicolo: Scrittura: {0}, Lettura: {1}", objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString());

                                //Se l'opzione di mantenere i diritti di scrittura � attiva
                                ...
                                if (objTrasm.mantieniScrittura == true)
                                {
                                    ...
                                    // � stato commentato il seguente pezzo di codice:
                                    /*
                                ...
                                {
                                    //Se l'opzione di mantenere i diritti di lettura � attiva
                                ...
                                */
                                ...
                                if (tipoOggettoTrasm.Equals("F"))
                                {
                                    ...
                                    logger.Debug("Cessione Diritti di Propriet� Utente per Fascicolo; IDObjectFolder: {0}", IDObjectFolder);
                                ...
                                // ##################################################
                                logger.Debug("End cessione diritti di propriet� documento/fascicolo come Utente");

                                //Se il ruolo dell'utente cedente � diverso dal ruolo del nuovo proprietario
                                ...
                                if (objTrasm.cessione.idRuolo != objTrasm.cessione.idRuoloNewPropr)
                                {
                                    if (!docPersonale)
                                    {
                                        logger.Debug("Cessione Diritti di Propriet� Ruolo con Mantieni Diritti per Documento/fascicolo: Scrittura: {0}, Lettura: {1}", objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString());
                                        logger.Debug("Inizio cessione diritti di propriet� documento/fascicolo come Ruolo");
                                        ...
                                        */
                                        ...
                                        {
                                            dbProvider.RollbackTransaction();
                                            throw new Exception("Non � presente il nuovo ruolo proprietario");
                                        }
                                        ...
                                        if (tipoOggettoTrasm.Equals("F"))
                                        {
                                            ...
                                            logger.Debug("Cessione Diritti di Propriet� Ruolo per Fascicolo; IDObjectFolder: {0}", IDObjectFolder);
                                        ...
                                        // ##################################################
                                        logger.Debug("End cessione diritti di propriet� documento come Ruolo");
                                    }
                                    else
                                    {
                                        // se � doc personale, allora elimino altri record preesistenti in security (di tipo ACQUISITO) del nuovo ruolo proprietario se esistente
                        ...
                        else
                        {
                                ...
                                objTrasm.cessione.idRuolo != string.Empty))
                            {
                                ...
                                logger.Debug("Cessione Diritti Acquisiti come Utente. Valore cedeAcqUtente: {0}", cedeAcqUtente.ToString());
                                ...
                                if (cedeAcqUtente)
                                {
                                    ...
                                    // CASI POSSIBILI:
                                    // 1. Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di Scrittura e si possiedono solo quelli
                                    //    la cessione del nuovo utente nn viene eseguita.
                                    // 2. Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di lettura e si possiedono solo quelli 
                                    //    la cessione del nuovo utente nn viene eseguita
                                    // 3. Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di lettura e si possiedono quelli di Scrittura
                                    ...
                                    {
                                        logger.Debug("Inizio caso 1. Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di Scrittura ma si possiedono solo quelli la cessione del nuovo utente nn viene eseguita");
                                        logger.Debug("Cessione Diritti Acquisiti come Utente per il Documento/fascicolo. Mantieni Scrittura: {0}, Mantieni Lettura: {1}, AccessRight: {2}", objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString(), accessRights);
                                        ...
                                        if (tipoOggettoTrasm.Equals("F"))
                                        {
                                            ...
                                            logger.Debug("Cessione Diritti Acquisiti come Utente per Fascicolo. IDObjectFolder: {0}, Mantieni Scrittura: {1}, Mantieni Lettura: {2}, AccessRight: {3}", IDObjectFolder, objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString(), accessRights);
                                    ...
                                    {
                                        logger.Debug("Inizio caso 2. Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di Lettura ma si possiedono solo quelli la cessione del nuovo utente nn viene eseguita");
                                        logger.Debug("Cessione Diritti Acquisiti come Utente per il Documento/fascicolo. Mantieni Scrittura: {0}, Mantieni Lettura: {1}, AccessRight: {2}", objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString(), accessRights);
                                    ...
                                    {
                                        logger.Debug("Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di Lettura e si possiedono quelli di Scrittura deve avvenire la cessione del nuovo utente");
                                        logger.Debug("Cessione Diritti Acquisiti come Utente per il Documento/fascicolo. Mantieni Scrittura: {0}, Mantieni Lettura: {1}, AccessRight: {2}", objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString(), accessRights);
                                        logger.Debug("Se l'utente a cui cedere il diritto � presente, avviene la cessione. Utente a cui cedere il diritto: {0}", objTrasm.cessione.idPeopleNewPropr);
                                ...
                                logger.Debug("Cessione Diritti Acquisiti come Ruolo. Valore cedeAcqRuolo: {0}", cedeAcqRuolo.ToString());
                                ...
                                if (cedeAcqRuolo)
                                {
                                    ...
                                    if ((objTrasm.mantieniScrittura && !accessRights.Equals("63")) || (!objTrasm.mantieniScrittura && !objTrasm.mantieniLettura))
                                    {
                                        logger.Debug("Inizio caso 1. Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di Scrittura ma si possiedono solo quelli la cessione del nuovo ruolo nn viene eseguita");
                                        logger.Debug("Cessione Diritti Acquisiti come Ruolo per il Documento/fascicolo. Mantieni Scrittura: {0}, Mantieni Lettura: {1}, AccessRight: {2}", objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString(), accessRights);
                                        ...
                                        if (tipoOggettoTrasm.Equals("F"))
                                        {
                                            ...
                                            logger.Debug("Cessione Diritti Acquisiti come Ruolo per Fascicolo. IDObjectFolder: {0}, Mantieni Scrittura: {1}, Mantieni Lettura: {2}, AccessRight: {3}", IDObjectFolder, objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString(), accessRights);
                                    ...
                                    if ((objTrasm.mantieniLettura && !objTrasm.mantieniScrittura && !accessRights.Equals("45") && !accessRights.Equals("63")) || (!objTrasm.mantieniLettura && !objTrasm.mantieniScrittura))
                                    {
                                        logger.Debug("Inizio caso 2. Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di Lettura ma si possiedono solo quelli la cessione del nuovo ruolo nn viene eseguita");
                                        logger.Debug("Cessione Diritti Acquisiti come Ruolo per il documento/fascicolo. Mantieni Scrittura: {0}, Mantieni Lettura: {1}, AccessRight: {2}", objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString(), accessRights);
                                    ...
                                    {
                                        logger.Debug("Se Cedo i Diritti ed � attiva l'opzione di mantenere i diritti di Lettura e si possiedono quelli di Scrittura deve avvenire la cessione del nuovo ruolo");
                                        logger.Debug("Cessione Diritti Acquisiti come Ruolo per il Documento/fascicolo. Mantieni Scrittura: {0}, Mantieni Lettura: {1}, AccessRight: {2}", objTrasm.mantieniScrittura.ToString(), objTrasm.mantieniLettura.ToString(), accessRights);
                                        logger.Debug("Se l'utente a cui cedere il diritto � presente, avviene la cessione. Utente a cui cedere il diritto: {0}", objTrasm.cessione.idPeopleNewPropr);
    ...
    //				return q;
    //			}*/
    //			//nella versione 2 c'� questo controllo invece di quello sopra	
    ...
    {
        // ret � false se i filtri coninvolgono le tabelle B e C e quindi la query va ripetuta
        bool ret = false;
            ...
            if (f.valore != null && !f.valore.Equals(""))
            {
                switch (f.argomento)
                {
                    ...
                    case "MITTENTE_UTENTE":
                        ...
                        //dove il VAR_COD_RUBRICA = f.valore. Ma questo in questo caso non centra nulla poich�
                        //in questo CASE ci entra quando si � digitato qualcosa nel campo DESCRIZIONE DEL corrispondente.
                        //in pi� la CLAUSOLA IN non deve contenere una sola system_id poich� IL CAMPO VAR_DESC_CORR non � univoco.
                        //							if (peopleID.Equals(string.Empty))
                        //								peopleID=GetPeopleID(f.valore);
                        //	
                        //							queryWhere += " AND A.ID_PEOPLE IN (" + peopleID + ")";
    ...
    /// <summary>
    /// Reperimento condizioni di visibilit� per le trasmissioni ricevute
    /// </summary>
    /// <param name="queryWhere"></param>
    /// <param name="objOggettoTrasmesso"></param>
    /// <param name="objUtente"></param>
    ...
    /// <summary>
    /// Reperimento condizioni di visibilit� per le trasmissioni effettuate
    /// </summary>
    /// <param name="queryWhere"></param>
    /// <param name="objOggettoTrasmesso"></param>
    /// <param name="objUtente"></param>
                                            ...
                                            DocsPaVO.filtri.FiltroRicerca[] objListaFiltri, DocsPaVO.trasmissione.OggettoTrasm objOggettoTrasmesso)
    {
        ...
        // Se non � valorizzata la direzione di ordinamento, viene creato un filtro di tipo ORDER_DIRECTION
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    // Altrimenti viene creato il filtro standard
                    extractFieldValue = String.Empty;
                    orderCondition = String.Format("TO_DATE (a.dta_invio, 'dd/MM/yyyy HH24:mi:ss') {0}, B.ID_TRASMISSIONE {0}", orderDirection.valore);
                }
            }
        }
    ...
    public ArrayList GetListRag(DocsPaVO.trasmissione.Diritti objDiritti, bool flgDaRicercaTrasm, bool sysExt=false)
    {
        ...
        try
        {
            ...
            // prima era cos� ora i PM richiedono order per var_NOTE
    ...
    /// <param name="isEnableRef"> booleano che identifica se � stata abilitata la gestione
    ...
    public bool VerificaRagProtIntBis(string idAmm, bool isEnableRef, string tipoProto, bool destTO, bool destCC, out string message)
    {
        ...
        if (tipoProto.Equals("A"))
        {
            if (dataset != null && dataset.Tables[0].Rows.Count > 0)
            {
                if (isEnableRef == true)	//se � abilitato l'ufficio referente
                {
        ...
        if (tipoProto.Equals("P") || tipoProto.Equals("I"))
        {
            ...
            if (dataset != null && dataset.Tables[0].Rows.Count > 0)
            {
                if (destTO)//se sono specificati i dest in TO
                {
                    ...
                    if (ragioneTO == null || ragioneTO == "") //Se la ragione in TO � assente
                    {
                        ...
                        if (destCC)//se sono specificati i dest in CC
                        {
                            ...
                            if (ragioneCC == null || ragioneCC == "") //Se la ragione in CC � assente
                            {
                                if (message != "")
                                    message = message + " , ";
                        ...
                        if (isEnableRef == true)	//se � specificato l'ufficio referente
                        {
                            ragReferente = dataset.Tables[0].Rows[0]["ID_RAGIONE_REFERENTE"].ToString();
                            if (ragReferente == null || ragReferente.Equals("")) //Se la ragione REFERENTE � assente
                            {
                                if (message != "")
                                    message = message + " , ";
    ...
    {
        //si verifica se la parola chiave � gi� presente
        /*string selectString =
    ...
    public string insertTrasmissione(/*DocsPaWS.Utils.Database db,*/ DocsPaVO.trasmissione.Trasmissione objTrasm)
    {
        ...
        //inizio 3.11.5 3/6/2010: settando la data invio durante l'insert non scatta il trigger che � solo sull'update.
        //in questo modo ottengo trasmissioni senza notifiche in TDL. naturalmente � necessario modificare anche executeTrasm affinch� non faccia 
                                        ...
                                        String idAmministrazione)
    {
        ...
        ////CONDIZIONE per non trovare documenti con visibilit� rimossa:
    ...
    /// La query � costruita dinamicamente in base al filtro selezionato
                                        ...
                                        out int recordCount)
    {
        ...
        // Reperimento array contenente tutti gli IDTrasmissione della pagina richiesta
        string[] idTrasmArray = this.GetSystemIDTrasmEffettuateLite(queryWhere, queryJoin, string.Empty, pageNumber, excel, pageSize, out totalPageNumber, out recordCount, objListaFiltri, objUtente != null ? objUtente.idAmministrazione : "0");
                                        ...
                                        out int recordCount)
    {
        ...
        string[] idTrasmArray;
        idTrasmArray = this.GetSystemIDTrasmRicevute(queryWhere, queryFrom, string.Empty, pageNumber, out totalPageNumber, out recordCount, objListaFiltri, utente != null ?  utente.idAmministrazione : "0");
        if (idTrasmArray.Length > 0)
        {
            string trasmFilter = "";
                                ...
                                string registri, DocsPaVO.filtri.FiltroRicerca[] filter)
    {
        ...
        try
        {
            ...
            // non � necessario usare i filtri se non vengono popolati
    ...
    public ArrayList GetListTrasmissioni(DataTable dataTable, string tipoTrasmissione, string currentIdPeople, string currentIdGruppo, string filterOrder)
    {
        ...
        foreach (DataRowView drv in view)
        {
            ...
            if (!includedList.Contains(key))
            {
                ...
                if (canAdd)
                {
                    // La trasmissione � sicuramente visualizzata
                    if (tipoTrasmissione == "D")
    ...
    /// <summary>
    /// Rimozione del documento per cui si sono perduti i diritti di visibilit� dalla todolist
    /// </summary>
    /// <param name="idTrasmUtente"></param>
    /// <param name="idTrasmSingola"></param>
    /// <param name="idPeople"></param>
    ...
    public bool RimuoviToDoListACL(string idTrasmUtente, string idTrasmSingola, string idPeople)
    {
        ...
        try
        {
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                //Si deve capire se � stato rimosso il diritto di visibilit� o il documento
    ...
    private DocsPaVO.trasmissione.infoToDoList CreateInfoTodoList(DataRow row, string currentIdGruppo, string currentIdPeople)
    {
        ...
        // dati presenti per obbligatoriet�.
        ...
        if (row["ID_PROFILE"] != DBNull.Value)
        {
            ...
            if (row["OGGETTO_MITTENTE"] != DBNull.Value)
            {
                if (row["OGGETTO_MITTENTE"].ToString() != "")
                {
                    ...
                    {
                        myTodo.oggetto = oggMitt[0].ToString();
                        // se uscita o interno il mittente pu� essere anche vuoto, decideremo poi se mettere destinatari
        ...
        // Se l'utente ha i diritti necessari di visibilit� sul 
        // documento
        if (resACL != 2)
        {
    ...
    private DocsPaVO.trasmissione.infoToDoList CreateInfoFascicolo(DataRow row, string currentIdGruppo, string currentIdPeople)
    {
        ...
        // dati presenti per obligatoriet�.
        this.FetchCommonFieldInfoToDoList(row, myTodo);
    ...
    //quando il "TIPO_OGGETTO" � un fascicolo.        
    ...
    public string getNewOrderFilter(DocsPaVO.filtri.FiltroRicerca[] filter)
    {
        ...
        //POI VERIFICO SE FASC "F" O DOC "D" e aggiungo order necessario:
        //Nuova gestione unificata non ho pi� doc o fasc, ma insieme
        //switch (type)
        //{
        //    case "D":
        //        filterOrder += ",ID_PROFILE DESC";
                ...
                if (f.valore != null && !f.valore.Equals(""))
                {
                    switch (f.argomento)
                    {
                        case "MY_RECEIVED_TRANSMISSIONS":
                            if (orCondition)
                                filtersInOr += " OR ";
                            filtersInOr += " ((ts.id_corr_globale IN (";
                            filtersInOr += " SELECT system_id FROM dpa_corr_globali WHERE CHA_TIPO_URP='R' AND CHA_TIPO_IE='I' START WITH SYSTEM_ID = '" + f.valore.Split('|')[0] + "' CONNECT BY PRIOR id_old = system_id )";                        
                            filtersInOr += " AND tu.id_people = " + f.valore.Split('|')[1] + ") OR (ts.id_corr_globale = " + f.valore.Split('|')[2] + ")) ";
    ...
    private DocsPaVO.trasmissione.InfoTrasmissione CreateInfoTrasmissione(IDataReader reader)
    {
        ...
        if (!string.IsNullOrEmpty(utDelegato))
        {
            ...
            //Uso il metodo GetUtenteNoFiltroDisabled e non GetUtente perch� nel caso in cui il delegato � disabilitato non ritorna nulla
    ...
    /// <summary>
    /// Ricava la lista delle query in base al tipo destinatario per estendere la visibilit� gerarchica
    /// </summary>
    /// <param name="currentTrasmissione"></param>
    /// <param name="objTrasm"></param>
    /// <param name="idRegistro"></param>
    ...
    public ArrayList GetQueryTrasmEstVisibSup(DocsPaVO.trasmissione.TrasmissioneSingola currentTrasmissione, DocsPaVO.trasmissione.Trasmissione objTrasm, string idRegistro, string idNodoTitolario, string idObj, string idPeopleGroup, string codiceACL, string tipoDiritto, ref ArrayList ruoliSuperiori)
    {
        ...
        if (currentTrasmissione.corrispondenteInterno.GetType() == typeof(DocsPaVO.utente.Ruolo))
        {
            ...
            else
            {
                ...
                {
                    if (ruoliSuperiori == null)
                        ruoliSuperiori = new ArrayList();
    ...
    /// <summary>
    /// Elimina la visibilit� dell'oggetto all'utente o al ruolo
    /// </summary>
    ...
    /// <summary>
    /// Elimina la visibilit� degli oggetti in stato: attesa di accettazione
    /// </summary>
    /// <param name="thing">ID oggetto (doc o fasc)</param>
    /// <param name="personorgroup">ID Utente/Gruppo</param>     
    /// <param name="isFascicolo">True se l'oggetto � un fascicolo</param>
    /// <returns>true o false</returns>
    ...
    public bool EliminaAttesaAccettazione_Fascicolo(string thing, string personorgroup)
    {
        ...
        List<string> idsFolderC = GetIdFolderC(thing);

        if (idsFolderC != null && idsFolderC.Count > 0)
        {
            string idFolderC = idsFolderC[0];
    ...
    public List<string> GetIdFolderC(string idF)
    {
        DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider();
        ...
        if (this.ExecuteQuery(out ds, "folder", commandText))
        {
            if (ds.Tables["folder"] != null && ds.Tables["folder"].Rows.Count > 0)
            {
        ...
        //Estrae solo il primo sottofascicolo e non tutti
    ...
    /// <summary>  
    /// Verifica se una trasmissione di tipo UNO � gi� stata accettata o rifiutata
    /// da un utente del ruolo
    /// </summary>
    /// <param name="idTrasmSingola"></param>
    /// <returns>False se � gi� stata accettata o rifiutata</returns>
    ...
    /// <summary>
    /// Verifica se una trasmissione di tipo TUTTI � gi� stata accettata o rifiutata da tutti
    /// </summary>
    /// <param name="idTrasmSingola"></param>
    /// <returns>True se � gi� stata accettata o rifiutata da tutti</returns>
    ...
    public bool aggiornaAccessRights_Waiting(string thing, string accessRightsToSet, string condizione)
    {
        ...
        //controllo se � in stato finale, devo lasciare security a lettura in ogni caso.
    ...
    /// <summary>
    /// Update di ACCESSRIGHTS nella tabella SECURITY quando l'oggetto � di tipo FASCICOLO
    /// </summary>
    ...
    public bool aggiornaAccessRights_DocsInFascicolo(string thing, string idGroup, string idPeople, string accessRightsAttuale, string accessRightsLettura, string accessRightsToSet)
    {
        ...
        if (idsFolderC != null && idsFolderC.Count > 0)
        {
            foreach (string idFolderC in idsFolderC)
            {
    ...
    /// <summary>
    /// Se nelle ragioni delle trasmissioni c'� almeno una ragione con il valore 1 nel campo eredita,
    ...
    /// <summary>
    /// Reperimento dell'ultima data in cui � stato trasmesso dall'utente un documento o un fascicolo
    /// </summary>
    /// <param name="infoUtente"></param>
                                        ...
                                        out int recordCount)
    {
        ...
        string[] idTrasmArray;
        idTrasmArray = this.GetSystemIDTrasmRicevuteLite(queryWhere, queryFrom, string.Empty, pageNumber, excel, pageSize, out totalPageNumber, out recordCount, objListaFiltri, utente != null ? utente.idAmministrazione : "0");
        if (!excel)
        {
            if (idTrasmArray.Length > 0)
            {
    ...
    /// <returns>T se � di tipo tutti; S se � singola</returns>
    ...
    public string GetIdTrasmissioneByIdTrasmSingola(string idTrasmissioneSingola)
    {
        ...
        logger.Information("Fine Metodo GetIdTrasmissioneByIdTrasmSingola in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    public List<string> GetDestTrasmissioneInteroperabilitaSemplificata(string idProfile)
    {
        List<string> destinatari = new List<string>();
        try
        {
            Query query = InitQuery.getInstance().getQuery("S_DPA_TRASMISSIONE_IS_DEST");
            ...
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                using (IDataReader reader = dbProvider.ExecuteReader(commandText))
                {
                    while (reader.Read())
                    {
                        destinatari.Add(reader.GetValue(reader.GetOrdinal("ID_CORR_GLOBALE")).ToString());
                    }
                }
            }
        }
        ...
        {
            logger.Error("Errore in GetIdTrasmissioneInteroperabilitaSemplificata " + e.Message);
            destinatari = null;
        }
    ...
    public string GetTipoRagioneByIdTrasmSingola(string idTrasmSingola)
    {
        ...
        {
            logger.Error(exc, "errore in GetTipoRagioneByIdTrasmSingola");
            return null;
        }

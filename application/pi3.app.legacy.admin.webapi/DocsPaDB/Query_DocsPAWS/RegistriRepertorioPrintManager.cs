...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(RegistriRepertorioPrintManager));
            ...
            using (IDataReader dataReader = dbProvider.ExecuteReader(query.getSQL()))
            {
                if (dataReader != null && dataReader.Read())
                {
                    returnValue = new RegistroRepertorioSingleSettings
                    {
                        ...
                        RoleAndUserDescription = dataReader["RoleAndUserDescription"].ToString(),
                        CounterState = Enum.Parse<RegistroRepertorioSingleSettings.RepertorioState>(dataReader["CounterState"].ToString()),
                        PrintFrequency = Enum.Parse<RegistroRepertorioSingleSettings.Frequency>(dataReader["PrintFrequency"].ToString()),
                        LastPrintedNumber = int.TryParse(dataReader["LastPrintedNumber"].ToString(), out var num) ? num : 0,
                        RoleRespRight = Enum.Parse<RegistroRepertorioSingleSettings.ResponsableRight>(dataReader["RespRights"].ToString())
                    };

                  DateTime dateTime = DateTime.Now;
                    var culture = CultureInfo.GetCultureInfo("it-IT");

                    returnValue.DateAutomaticPrintFinish = DateTime.TryParse(dataReader["DateAutomaticPrintFinish"].ToString(), culture, DateTimeStyles.None, out dateTime) ? dateTime : DateTime.Now;
                    returnValue.DateAutomaticPrintStart = DateTime.TryParse(dataReader["DateAutomaticPrintStart"].ToString(), culture, DateTimeStyles.None, out dateTime) ? dateTime : DateTime.Now;
                    returnValue.DateLastPrint = DateTime.TryParse(dataReader["DateLastPrint"].ToString(), culture, DateTimeStyles.None, out dateTime) ? dateTime : DateTime.MinValue;
                    returnValue.DateNextAutomaticPrint = DateTime.TryParse(dataReader["DateNextAutomaticPrint"].ToString(), culture, DateTimeStyles.None, out dateTime) ? dateTime : DateTime.MinValue;
                }
            }
        }
    ...
    public bool SaveRegisterSettings(String counterId, RegistroRepertorio.SettingsType settingsType, RegistroRepertorio.TipologyKind tipologyKind, RegistroRepertorioSingleSettings settings)
    {
        ...
        var customCulture = new System.Globalization.CultureInfo("it-IT");
        System.Threading.Thread.CurrentThread.CurrentCulture = customCulture;      
        string DateAutomaticPrintStartString = settings.DateAutomaticPrintStart.ToString("dd/MM/yyyy HH:mm:ss");
        string DateAutomaticPrintFinishstring = settings.DateAutomaticPrintFinish.ToString("dd/MM/yyyy HH:mm:ss");
        string DateNextAutomaticPrintString = settings.DateNextAutomaticPrint.ToString("dd/MM/yyyy HH:mm:ss");
        DateTime DateAutomaticPrintStart = DateTime.ParseExact(DateAutomaticPrintStartString, "dd/MM/yyyy HH:mm:ss", System.Globalization.CultureInfo.GetCultureInfo("it-IT"), System.Globalization.DateTimeStyles.None);

        DateTime DateAutomaticPrintFinish = DateTime.ParseExact(DateAutomaticPrintFinishstring, "dd/MM/yyyy HH:mm:ss", System.Globalization.CultureInfo.GetCultureInfo("it-IT"), System.Globalization.DateTimeStyles.None);

        DateTime DateNextAutomaticPrint = DateTime.ParseExact(DateNextAutomaticPrintString, "dd/MM/yyyy HH:mm:ss", System.Globalization.CultureInfo.GetCultureInfo("it-IT"), System.Globalization.DateTimeStyles.None);
        ...
        if (dbType.ToUpper().Equals("SQL"))
        {
            ...
            if (DateAutomaticPrintStartString.ToString().StartsWith("01/01/0001"))
                parameters.Add(new ParameterSP("dateAutomaticPrintStart", DBNull.Value, DirectionParameter.ParamInput));
            else
                parameters.Add(new ParameterSP("dateAutomaticPrintStart", DateAutomaticPrintStart, DirectionParameter.ParamInput));

            // DateAutomaticPrintFinish
            if (DateAutomaticPrintFinishstring.ToString().StartsWith("01/01/0001"))
                parameters.Add(new ParameterSP("dateAutomaticPrintFinish", DBNull.Value, DirectionParameter.ParamInput));
            else
                parameters.Add(new ParameterSP("dateAutomaticPrintFinish", DateAutomaticPrintFinish, DirectionParameter.ParamInput));

            // DateNextAutomaticPrint
            if (DateNextAutomaticPrintString.ToString().StartsWith("01/01/0001"))
                parameters.Add(new ParameterSP("dateNextAutomaticPrint", DBNull.Value, DirectionParameter.ParamInput));
            else
                parameters.Add(new ParameterSP("dateNextAutomaticPrint", DateNextAutomaticPrint, DirectionParameter.ParamInput));
        }
        else
        {
            parameters.Add(new ParameterSP("dateAutomaticPrintStart", DateAutomaticPrintStart, DirectionParameter.ParamInput));
    ...
    public void SaveNextAutomaticPrintDate(String counterId, String registryId, String rfId, DateTime date)
    {
        ...
        dbType = DocsPaVO.Settings.AppSettings.Instance.DBType;
    ...
    public bool SaveRuoloRespConservazioneAoo(string idGroup, string idUtente, string idAmm, string idAoo)
    {
        bool result = false;
        ...
        try
        {
            Query countq = DocsPaUtils.InitQuery.getInstance().getQuery("S_CHK_CONS_AOO_CONF_EXIST");
            countq.setParam("idReg", idAoo);
            countq.setParam("idAmm", idAmm);

            string chkCommand = countq.getSQL();
            logger.Debug(chkCommand);


            using (DBProvider dbProvider = new DBProvider())
            {
                if (!dbProvider.ExecuteScalar(out value, chkCommand))
                    throw new Exception(dbProvider.LastExceptionMessage);

                if(Convert.ToInt32(value)>1)
                    throw new Exception("errore, più di una configurazione rilevata");
        ...
        try
        {
            if (result)
            {
                ...
                Query q = DocsPaUtils.InitQuery.getInstance().getQuery("U_CONS_AOO_CONF");
                q.setParam("idGroup", idGroup);
                q.setParam("idUtente", idUtente);
                q.setParam("idAmm", idAmm);
                q.setParam("idReg", idAoo);
            ...
            else
            {
                ...
                Query q = DocsPaUtils.InitQuery.getInstance().getQuery("I_CONS_AOO_CONF");
                q.setParam("idGroup", idGroup);
                q.setParam("idUtente", idUtente);
                q.setParam("idAmm", idAmm);
                q.setParam("idReg", idAoo);
    ...
    public bool SaveRuoloRespConservazioneAOO(string idGruppo, string idUtente, string idAmm, string idAOO)
    {
        ...
        try
        {
            Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_GET_ID_ROLE_RESP_CONS_AOO");
            q.setParam("idAmm", idAmm);
            ...
            string idRespCons = string.Empty;

            using (DBProvider dbProvider = new DBProvider())
            {
                if (!dbProvider.ExecuteScalar(out idRespCons, command))
                    throw new Exception(dbProvider.LastExceptionMessage);

                if(!string.IsNullOrEmpty(idRespCons))
                {
                    logger.Debug("Id ruolo responsabile={0}, aggiorno", idRespCons);
                    // Update
                    q = DocsPaUtils.InitQuery.getInstance().getQuery("U_RESP_CONS_AOO");
                    q.setParam("idAmm", idAmm);
                    q.setParam("idAOO", idAOO);
                    q.setParam("idUtente", idUtente);
                ...
                {
                    // Insert
                    logger.Debug("Responsabile non presente, lo inserisco");
                    q = DocsPaUtils.InitQuery.getInstance().getQuery("I_RESP_CONS_AOO");
                    q.setParam("idAmm", idAmm);
                    q.setParam("idAOO", idAOO);
                    q.setParam("idUtente", idUtente);
                    q.setParam("idRuolo", idGruppo);

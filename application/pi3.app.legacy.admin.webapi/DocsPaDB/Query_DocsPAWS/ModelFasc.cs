...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(ModelFasc));
        ...
        try
        {
            ...
            for (int i = 0; i < template.ELENCO_OGGETTI.Count; i++)
            {
                ...
                if (oggettoCustom.TIPO_OPERAZIONE.Equals("DaAggiornare"))
                {
                    ...
                    queryMng = DocsPaUtils.InitQuery.getInstance().getQuery("PD_U_ASS_TEMPLATES_FASC_DEFAULT");
                    queryMng.setParam("idtemplate", template.SYSTEM_ID.ToString());
    ...
    public DocsPaVO.ProfilazioneDinamica.Templates impostaCampiComuniFasc(DocsPaVO.ProfilazioneDinamica.Templates modello, ArrayList campiComuni)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < campiComuni.Count; i++)
            {
                ...
                //Verifico se il campo comune � gi� presente nella DPA_OGG_CUSTOM_COMP_FASC per il template interessato,
    ...
    //questa funzione controlla se � associato un record nella DPA_CONT_CUSTOM_FASC
    ...
    public ArrayList getListaStoricoFascicolo(string id_tipo_fasc, string idProject)
    {
        ...
        using (DBProvider dbProvider = new DBProvider())
        {
            ...
            //impostazione dell'id del fascicolo al quale � associato il modello
    ...
    public ArrayList getIdPrjByAssTemplates(string idOggFasc, string valoreDB, string ordine)
    {
        ...
        {
            retVal = null;
            logger.Error(ex, "errore in getIdPrjByAssTemplates");
        }
    ...
    public ArrayList GetTipologiaFascicoloByRuoloAndPianoConservazione(string idAmministrazione, string idRuolo, string diritti, string idPianoConservazione)
    {
        ArrayList listaTipiFasc = new ArrayList();
        DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider();
        ...
        try
        {
            DocsPaUtils.Query queryMng = DocsPaUtils.InitQuery.getInstance().getQuery("S_DPA_TIPO_FASC_FROM_RUOLO_ID_PIANO_CONSERVAZIONE");
            queryMng.setParam("idAmm", idAmministrazione);
            queryMng.setParam("idRuolo", idRuolo);
            queryMng.setParam("idPianoConservazione", idPianoConservazione);
            ...
            string commandText = queryMng.getSQL();
            System.Diagnostics.Debug.WriteLine("SQL - getTipologiaFascicoloByRuoloAndPianoConservazione - ProfilazioneDinamica/Database/modelFasc.cs - QUERY : " + commandText);
            logger.Debug("SQL - getTipologiaFascicoloByRuoloAndPianoConservazione - ProfilazioneDinamica/Database/modelFasc.cs - QUERY : " + commandText);

...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Deleghe));
    public Deleghe()
    { }
    ...
    private InfoDelega buildDelega(DataRow dataRow)
    {
        ...
        if (dataRow["DATA_DECORRENZA"] != null && dataRow["DATA_DECORRENZA"] != DBNull.Value) { 
            DateTime dataDecorrenza = Convert.ToDateTime(dataRow["DATA_DECORRENZA"]);
            string strDataDecorrenza = dataDecorrenza.ToString("dd/MM/yyyy HH:mm:ss", CultureInfo.GetCultureInfo("it-IT"));
            if (infoDelega.dataDecorrenza != strDataDecorrenza)
                infoDelega.dataDecorrenza = strDataDecorrenza;
        }

        if (dataRow["DATA_SCADENZA"] != null && dataRow["DATA_SCADENZA"] != DBNull.Value)
        {
            DateTime dataScadenza = Convert.ToDateTime(dataRow["DATA_SCADENZA"]);
            string strDataScadenza = dataScadenza.ToString("dd/MM/yyyy HH:mm:ss", CultureInfo.GetCultureInfo("it-IT"));
            if (infoDelega.dataScadenza != strDataScadenza)
                infoDelega.dataScadenza = strDataScadenza;
        }
        ...
        try
        {
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                ...
                else
                {
                    ...
                    if (((!string.IsNullOrEmpty(dataScadenzaOld) && toDate(dataScadenzaOld) < DateTime.Now)
                        && (string.IsNullOrEmpty(delegaNew.dataScadenza) || toDate(delegaNew.dataScadenza) >= DateTime.Now)
                        && (toDate(dataDecorrenzaOld).ToShortDateString() != toDate(delegaNew.dataDecorrenza).ToShortDateString())))
                    {

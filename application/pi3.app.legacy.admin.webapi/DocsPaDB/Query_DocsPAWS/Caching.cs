...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Caching));
    ...
    public string GET_ID_AMM_BY_DOC_NUMER(string docnumber)
    {
        ...
        using (DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider())
        {
            using (System.Data.IDataReader reader = dbProvider.ExecuteReader(commandText))
            {
                if (reader?.Read() == true && !reader.IsDBNull(reader.GetOrdinal("idamm")))
                {
    ...
    public string GET_DOCNUMER_BY_SEGNATURA(string segnatura)
    {
        ...
        using (DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider())
        {
            using (System.Data.IDataReader reader = dbProvider.ExecuteReader(commandText))
            {
                if (reader?.Read() == true && !reader.IsDBNull(reader.GetOrdinal("docnumber")))
                {

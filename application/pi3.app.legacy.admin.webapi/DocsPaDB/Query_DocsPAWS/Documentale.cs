...
public class Documentale : DBProvider
	{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Documentale));
    public long GetRND()
		{
			long retValue;
		...
		public string CreateDocument(DocsPaVO.documento.SchedaDocumento schedaDocumento)
		{
			...
			{			
				...
				long randomized;
					...
					string documentType= DocsPaVO.Settings.AppSettings.Instance.documentale;
    ...
    public string CreazioneNuovaVersioneSP(string docNumber, string subVersion,  string idPeople, string descrizione, string cartaceo)
    {
        ...
        logger.Information("BEGIN");
            ...
            {
                #region old (ora � gestita con una store procedure) 
                /*
    ...
    /// con possibilit� di inserire l'informazione se il doc � cartaceo o meno
    /// </summary>
    /// <param name="versionId"></param>
    ...
    public bool InsertIntoVersions(string versionId, string subVersion, string versionLabel, string author, string typist, string docNumber, string descrizione, bool cartaceo)
    {
		...
		/// <summary>
		/// Esegue una insert nella tabella VERSIONS, 
    /// con possibilit� di inserire la descrizione
		/// </summary>
		/// <param name="versionId"></param>
		/// <param name="subVersion"></param>
		/// <param name="versionLabel"></param>
...
//					return false;
    ...
    /// <summary>
    /// Esegue la stored procedure per il calcolo della atipicit� di un documento / fascicolo
    /// </summary>
    /// <param name="infoUtente"></param>
    ...
    public void CalcolaAtipicitaRuoliSottoposti(OrgRuolo role)
    {
        ...
        {
            // Esecuzione della store procedure per il calcolo delle atipicit� nei documenti
    ...
    public DocsPaVO.amministrazione.EsitoOperazione CopyVisibility(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.Security.CopyVisibility copyVisibility)
    {
        ...
        try
        {
            sl.Log(System.DateTime.Now + " - Inizio Copia Visibilit�");
            int rows=0;
            ...
            if(rows==0)
                logger.Debug("Attenzione, non � stato possibile cancellare i log della DPA_COPY_LOG");

            //Documenti
            logger.Debug("Inizio copia visibilit� documenti");
            sl.Log(System.DateTime.Now + " - Inizio copia visibilit� documenti");
            ...
            logger.Debug("Fine copia visibilit� documenti");
            sl.Log(System.DateTime.Now + " - Fine copia visibilit� documenti");

            //Fascicoli
            logger.Debug("Inizio copia visibilit� fascicoli");
            sl.Log(System.DateTime.Now + " - Inizio copia visibilit� fascicoli");
            ...
            if (queryFascicoli != null && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloDestinazione) && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloOrigine))
            {
                ...
                if (!this.ExecuteNonQuery(queryFascicoli.getSQL(), out rowsAffected))
                {
                    esitoOperazione.Codice = -1;
                };
                esitoOperazione.numeroFascCopiati = rowsAffected / 2;
            }
            logger.Debug("Fine copia visibilit� fascicoli");

            logger.Debug("Inizio copia visibilit� documenti in fascicolo");
            sl.Log(System.DateTime.Now + " - Inizio copia visibilit� documenti in fascicolo");
            ...
            if (queryDocInFasc != null && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloDestinazione) && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloOrigine))
            {
                ...
                if (!this.ExecuteNonQuery(queryDocInFasc.getSQL(), out rowsAffected))
                {
                    esitoOperazione.Codice = -1;
                };
                esitoOperazione.numeroDocinFascCopiati = rowsAffected / 2;
            }
            logger.Debug("Fine copia visibilit� documenti in fascicolo");

            sl.Log(System.DateTime.Now + " - Fine copia visibilit� fascicoli");

            //Estendi visibilit� ai ruoli superiori dei documenti / fascicoli
            if (copyVisibility.estendiVisibilita.ToUpper().Equals("SI") || copyVisibility.estendiVisibilita.ToUpper().Equals("NO_ATIPICI"))
            {
                ...
                //Documenti
                logger.Debug("Inizio estensione visibilit� documenti per copia visibilit�");
                sl.Log(System.DateTime.Now + " - Inizio estensione visibilit� documenti per copia visibilit�");
                for (int i = 0; i < dsIdGruppoRuoliSuperiori.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (queryDocumenti != null && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloDestinazione) && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloOrigine))
                    {
                        ...
                        if (!this.ExecuteNonQuery(queryDocumenti.getSQL(), out rowsAffected))
                        {
                            esitoOperazione.Codice = -1;
                        };
                    }
                ...
                logger.Debug("Fine estensione visibilit� documenti per copia visibilit�");
                sl.Log(System.DateTime.Now + " - Fine estensione visibilit� documenti per copia visibilit�");
                reportSuperiori += "/";

                //Fascicoli
                logger.Debug("Inizio estensione visiblit� fascicoli per copia visibilit�");
                sl.Log(System.DateTime.Now + " - Inizio estensione visiblit� fascicoli per copia visibilit�");
                for (int i = 0; i < dsIdGruppoRuoliSuperiori.Tables[0].Rows.Count; i++)
                {
                    string idGruppoSup = dsIdGruppoRuoliSuperiori.Tables[0].Rows[i]["ID_GRUPPO"].ToString();
                    ...
                    if (queryFascicoli != null && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloDestinazione) && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloOrigine))
                    {
                        //devo inserire anche un controllo sul l'id gruppo destinatario, perch� potrebbe capitare che il gruppo destinatario non vede il titolario
                        //e quindi non acquisisce in copia la visibilit� dei fascicoli, di conseguenza neppure i suoi superiori devono acquisire in copia gli stessi fasccioli
                ...
                logger.Debug("Fine estensione visibilit� fascicoli per copia visibilit�");
                sl.Log(System.DateTime.Now + " - Fine estensione visibilit� fascicoli per copia visiblit�");
                reportSuperiori += "/";
                ...
                logger.Debug("Fine estensione visiblit� doc in fasc per copia visibilit�");
                sl.Log(System.DateTime.Now + " - Fine estensione visiblit� doc in fasc per copia visiblit�");
            }

            //Esecuzione Stored per atipicita, viene eseguita solo se non si � richiesto l'estensione della visibilit�
            string valoreChiaveAtipicita = DocsPaUtils.Configuration.InitConfigurationKeys.GetValue(infoUtente.idAmministrazione, "ATIPICITA_DOC_FASC");
            if (string.IsNullOrEmpty(valoreChiaveAtipicita))
                valoreChiaveAtipicita = DocsPaUtils.Configuration.InitConfigurationKeys.GetValue("0", "ATIPICITA_DOC_FASC");
            ...
            {
                //Documenti
                logger.Debug("Inizio calcolo atipicit� documenti per copia visiblit�");
                sl.Log(System.DateTime.Now + " - Inizio calcolo atipicit� documenti per copia visiblit�");
                ...
                logger.Debug("Fine calcolo atipicit� documenti per copia visiblit�");
                sl.Log(System.DateTime.Now + " - Fine calcolo atipicit� documenti per copia visiblit�");

                //Fascicoli
                logger.Debug("Inizio calcolo atipicit� fascicoli per copia visiblit�");
                sl.Log(System.DateTime.Now + " - Inizio calcolo atipicit� fascicoli per copia visiblit�");
                ...
                logger.Debug("Fine calcolo atipicit� fascicoli per copia visiblit�");
                sl.Log(System.DateTime.Now + " - Fine calcolo atipicit� fascicoli per copia visiblit�");

                // Documenti nel fascicolo
                logger.Debug("Inizio calcolo atipicit� documenti nel fascicolo per copia visiblit�");
                sl.Log(System.DateTime.Now + " - Inizio calcolo atipicit� documenti nel fascicolo per copia visiblit�");
                ...
                logger.Debug("Fine calcolo atipicit� documenti nel fascicolo per copia visiblit�");
                sl.Log(System.DateTime.Now + " - Fine calcolo atipicit� documenti nel fascicolo per copia visiblit�");
            }

            sl.Log(System.DateTime.Now + " - Fine Copia Visibilit�");
            sl.Log(System.DateTime.Now + " - Operazione avvenuta con successo");

            esitoOperazione.Codice = !string.IsNullOrEmpty(esitoOperazione.Codice.ToString())? esitoOperazione.Codice :0;
        ...
        catch (Exception e)
        {
            ...
            esitoOperazione.Codice = -1;
            esitoOperazione.Descrizione = "Problemi durante la copia della visibilit�. Cosultare il file di log.";
            return esitoOperazione;
        }
    }
    ...
    public DocsPaUtils.Query getQueryCopiaVis(DocsPaVO.Security.CopyVisibility copyVisibility, objTypeCopyVisiblity type, queryNameCopyVisibility queryName, bool extendVisibility)
    {
        try
        {
            ...
            if (type == objTypeCopyVisiblity.DOCUMENTI && (copyVisibility.docNonProtocollati || copyVisibility.docProtocollati))
            {
                ...
                //Condizione atipicita (presa in considerazione solo se si sta effettuando estensione della visibilit�)
            ...
            if (type == objTypeCopyVisiblity.FASCICOLI && copyVisibility.fascicoliProcedimentali)
            {
                ...
                // Se si sta eseguendo il calcolo di atipicit� bisogna considerare la condizione preesistente altrimenti no
                ...
                //Condizione atipicita ( presa in considerazione solo se si sta facendo estensione visibilit�)
                if (copyVisibility.estendiVisibilita.ToUpper().Equals("NO_ATIPICI") && extendVisibility)
            ...
            //Condizioni comuni
            //Verifichiamo se la variabile insCondComuni � a true, altrimenti � inutile processare le condizioni comuni,
            //in quanto non abbiamo impostato nessuna condizione n� per i documenti n� per i fascicoli 
            //ed in questo caso ritorniamo la query = null
            if (insCondComuni)
            {
                //Condizione visibilit� attiva e precedente copia di visibilit�
                if (copyVisibility.visibilitaAttiva && copyVisibility.precedenteCopiaVisibilita)
                {
                    ...
                    // Se si sta facendo copia visibilit� dei fascicoli bisogna selezionare anche i diritti di tipo F
                    // ottenuti quando si copia la visibilit� di un fascicolo ricevuto per trasmissione
                ...
                //Condizione visibilit� attiva senza precedente copia di visiblit�
                if (copyVisibility.visibilitaAttiva && !copyVisibility.precedenteCopiaVisibilita)
                {
                    string cond = " AND " +
                                    "( " +
                ...
                //Condizione visibilit� non attiva e precedente copia di visiblit�
                if (!copyVisibility.visibilitaAttiva && copyVisibility.precedenteCopiaVisibilita)
                {
                ...
                //Condizione visibilit� non attiva senza precedente copia di visiblit�
                if (!copyVisibility.visibilitaAttiva && !copyVisibility.precedenteCopiaVisibilita)
                {
    ...
    private string getCondizioneRegistri(DocsPaVO.Security.CopyVisibility copyVisibility, objTypeCopyVisiblity type)
    {
        ...
        try
        {
            if (copyVisibility != null && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloOrigine) && !string.IsNullOrEmpty(copyVisibility.idGruppoRuoloDestinazione))
            {
                ...
                ArrayList registriOrigine = ut.GetRegistriRuolo(copyVisibility.idCorrGlobRuoloOrigine);
                ArrayList registriDestinazione = ut.GetRegistriRuolo(copyVisibility.idCorrGlobRuoloDestinazione);
                ...
                if (registriOrigine != null && registriDestinazione != null)
                {
                    foreach (DocsPaVO.utente.Registro registroOrigine in registriOrigine)
                    {
                        foreach (DocsPaVO.utente.Registro registroDestinazione in registriDestinazione)
                        {
    ...
    /// <returns>True se l'operazione � andata a buon fine</returns>
    ...
    public DocsPaUtils.Query getQueryCopiaVisFasc(DocsPaVO.Security.CopyVisibility copyVisibility, objTypeCopyVisiblity type, queryNameCopyVisibility queryName, bool extendVisibility)
    {
        try
        {
            ...
            if (type == objTypeCopyVisiblity.FASCICOLI && copyVisibility.fascicoliProcedimentali)
            {
                ...
                //Condizione atipicita ( presa in considerazione solo se si sta facendo estensione visibilit�)
                if (copyVisibility.estendiVisibilita.ToUpper().Equals("NO_ATIPICI") && extendVisibility)
                    // Se si stanno copiando i documenti nel fascicolo, la condizione � diversa
    ...
    /// <summary>
    /// Metodo per l'estensione di visibilit� di un documento ai ruoli superiori per
    /// un dato ruolo
    /// </summary>

...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Reminder));
    ...
    public bool InsertReminder(DocsPaVO.Task.Task task,string subject, DocsPaVO.utente.InfoUtente infoUtente)
    {
        ...
        logger.Debug("{0} - START ", function);
        if (task == null)
        {
            logger.Debug("{0} - Parameter task is null", function);
            logger.Debug("{0} - END ");
            return false;
        }
        ...
        try
        {
            ...
            query = q.getSQL();
            logger.Debug("{0} - {1} ", function, query);

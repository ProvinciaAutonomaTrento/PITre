...
{
		private static ILogger logger = Serilog.Log.ForContext(typeof(Amministrazione));
	...
	/// <summary>
	/// Verifica se � presente almeno un documento in amministrazione
	/// </summary>
	...
	public bool UpdateAmministrazione(InfoAmministrazione info, out string errorMessage)
	{
		...
		queryDef.setParam("indirizzoDigitaleRif", info.indirizzoDigitaleRiferimento.ToString().Replace("'", "''"));
		queryDef.setParam("ProviderId", info.Provider.ToString().Replace("'", "''"));
		queryDef.setParam("TenantId", info.TenantId.ToString().Replace("'", "''"));
		queryDef.setParam("ClientId", info.ClientId.ToString().Replace("'", "''"));
		queryDef.setParam("ClientSecret", info.ClientSecret.ToString().Replace("'", "''"));
		queryDef.setParam("codiceIpa", info.codiceIpa.ToString().Replace("'", "''"));
		//++++++++++++++++ FINE QUERY
		...
		using (DBProvider dbProvider = new DBProvider())
		{
			...
			if (retValue)
			{
				if (updateDominio)
				{
					...
					{
						retValue = false;
						errorMessage = "si � verificato un errore: modifica autenticazione su dominio";
					}
				...
				if (deleteDominio)
				{
					...
					{
						retValue = false;
						errorMessage = "si � verificato un errore: eliminazione autenticazione su dominio";
					}
				}
			}
			else
			{
				errorMessage = "si � verificato un errore: modifica amministrazione";
				retValue = false;
			}
	...
	public bool InsertAmministrazione(InfoAmministrazione info, out string errorMessage)
	{
		...
		param2 += "'" + info.LibreriaDB + "'," +
			...
			//DI ZANNO - Gestione banner
				"'" + (info.Banner).Replace("'", "''") + "'," +
				"'" + info.indirizzoDigitaleRiferimento + "'," +
				"'" + info.codiceIpa + "'," +
				"'" + info.Provider + "'," +
				"'" + info.TenantId + "'," +
				"'" + info.ClientId + "'," +
		...
		using (DBProvider dbProvider = new DBProvider())
		{
			...
			{
				retValue = false;
				errorMessage = "si � verificato un errore: inserimento nuova amministrazione";
			}
			else
			{
				...
				if (insertRagioniTrasm(idNewAmm, info.IDRagioneTO, info.IDRagioneCC, info.IDRagioneCompetenza, info.IDRagioneConoscenza))
				{
					...
					{
						retValue = false;
						errorMessage = "si � verificato un errore: inserimento tipo funzioni";
					}
					else
					{
						...
						{
							retValue = false;
							errorMessage = "si � verificato un errore: inserimento chiavi configurazione";
						}
						else
							retValue = true;
					}
				...
				{
					retValue = false;
					errorMessage = "si � verificato un errore: inserimento ragioni di trasmissione";
				}
			}
	...
	public ArrayList GetDatiAmministrazione()
	{
		...
		DataSet dataSet;
		//luluciani se il db � shutdown, senza questo comando solo con un iisreset, docspa si accorge se il db � tornato operativo.
		this.OpenConnection();
	...
	private bool CheckTipologiaAtto(DocsPaVO.documento.TipologiaAtto tipoAtto)
	{
		...
		DataSet dataSet;
		bool ris = true;
		//si verifica se la descrizione � presente in rubrica
		/*
		string selectString =
	...
	public bool AmmInsUOContenitore(string idAmm, out string systemID_inserito)
	{
		...
		try
		{
			// verifica se esiste gi� una uo contenitore
			...
			if (id == null)
			{
				...
				// prima per� prende il codice e la descrizione dell'amministrazione...
	...
	public DocsPaVO.utente.Amministrazione AmmModificaUoTIBCO(string oldCodiceUO, DocsPaVO.amministrazione.OrgUO theUO, out bool result)
	{
		...
		{
			result = true;
			//Estraggo l'email da dpa_amministra per informare il referente che � stata effettuata una modifica o non inserimento del codice della UO
			ammin = getInfoAmministrazione(theUO.IDAmministrazione);
		}
		else
		{
			// ritorna false per indicare che non � stato effettuato nessun aggiornamento (e quindi non serve notificare)
			// oppure, in caso di inserimento della UO in PITRE, � stato trovato il record corrsipondente nella tabella TIBCO
			// e quindi non � necessario notificare
			result = false;
			ammin = null;
		}
		return ammin;
	...
	public bool AmmInsNuovoRuolo(DocsPaVO.amministrazione.OrgRuolo newRuolo)
	{
		...
		using (DBProvider dbProvider = new DBProvider())
		{
			...
			string documentale = DocsPaVO.Settings.AppSettings.Instance.documentale.ToString();
	...
	public bool AmmModRuolo(DocsPaVO.amministrazione.OrgRuolo ruolo)
	{
		...
		//    case 1:
		//        this.CommitTransaction();
		//        logger.Debug("Eseguita Commit alla Stored Procedure: SP_MODIFY_CORR_ESTERNO - il ruolo � stato modificato correttamente");
		//        result = true;
		//        // message = "OK";
	...
	public string AmmInsNuovoUtente(DocsPaVO.amministrazione.OrgUtente utente)
	{
		...
		{
			// possibili valori di ritorno:
			// 1 - userid gi� presente
			// 2 - codice rubrica gi� presente
			// 9 - errore generico
			// 0 - tutto ok!
...
// 						if (this.ExecuteScalar(out utente.IDPeople, queryString))
	...
	public string AmmModUtente(DocsPaVO.amministrazione.OrgUtente utente)
	{
		...
		try
		{
			...
			// 2 - userid gi� presente
			// 3 - codice rubrica gi� presente
			// 9 - errore generico
			// 0 - tutto ok!
			...
			if (valore != null && valore.Equals("0"))
			{
				...
				if (valore != null && valore.Equals("0"))
				{
					...
					myParam += " VAR_SEDE = q'[" + utente.Sede + "]',";
					...
					if (utente.DispositivoStampa == null|| utente.DispositivoStampa==0)
		...
		try
		{
			foreach (DocsPaVO.amministrazione.OrgRegistro registro in listaRegistri)
			{
				...
				if (valore != null && valore.Equals("0"))
				{
                    ...
                    q.setParam("param6", registro.ProtocolloAbilitato ? "1" : "0");
				...
				{
                    q = DocsPaUtils.InitQuery.getInstance().getQuery("U_AMM_REGISTRO_RUOLO");

                    q.setParam("param1", registro.IDRegistro);
                    q.setParam("param2", idCorrGlobRuolo);
                    q.setParam("param3", registro.ProtocolloAbilitato ? "1" : "0");
	...
	public bool AmmVerificaDisabilitazioneUtente(string idPeople)
	{
		...
		*/
		...
		if (this.ExecuteScalar(out valore, queryString))
		{
			...
			{
				// disabilita l'utente
				// perch� non ha altri ruoli associati...
	...
	public ArrayList AmmVerificaUtenteRespStampaRep(string userId, string roleId, string idAmm)
	{
		...
		{
			logger.Debug(ex, "errore in AmmVerificaUtenteRespStampaRep");
		}
		return result;
	}
	...
	public bool AmmRifiutaTrasmConWF(string idCorrGlobRuolo, string idGruppo)
	{
		...
		*/
		...
		//Vado a rimuovere le visibilit�
		if (result)
		{
			//Inserisco le visibilit� nella DELETED_SECURITY
			...
			//Rimuovo le visibilit� in attesa di accetazione
			if (result)
			{
	...
	public DataSet AmmRicercaInOrg(string tipo, string codice, string descrizione, string idAmm, bool searchHistoricized, bool searchByCodeExact)
	{
		...
		*/
		...
		// Se bisogna cercare ruoli, e se � stato richiesto di ricercare anche fra i ruoli
	...
	public DocsPaVO.utente.Corrispondente setCodRubrica(DocsPaVO.utente.Corrispondente corr)
	{
		...
		{
			string insertPrefix = (string)DocsPaVO.Settings.AppSettings.Instance.prefissoInsertCorr;
			/*
	...
	private DocsPaVO.utente.Corrispondente insertDatiCorr(DocsPaVO.utente.Corrispondente corr, DocsPaVO.utente.UserLogin userLogin)
	{
		...
		if (!checkPref.Equals(""))
		{
			throw new Exception("Per il codice Rubrica non pu� essere utilizzato il prefisso: " + checkPref);
	...
	public void updRuoliUO(/*DocsPaWS.Utils.Database db,*/ DocsPaVO.utente.Corrispondente uo)
	{
		...
		foreach (DataRow row in ds.Tables[0].Rows)
		{
			...
			// setto la data di fine validit� del corrispondente
	...
	public bool updateUserPassword(DocsPaVO.utente.Utente utente, string newPWD, string oldPWD)
	{
		...
		try
		{
			...
			{
				//password non valida
				throw new Exception("La vecchia password non � valida");
			}
	...
	/// la visibilit� su un dato titolario esclusi quelli non pi� attivi
	/// </summary>
	/// <param name="idAmministrazione"></param>
	...
	/// <summary>
	/// Ricava i ruoli a cui estendere la visibilit� del nodo correntemente creato, considerando la transazione
	/// </summary>
	/// <param name="idNodoTitolario"></param>
	/// <param name="idRegistro"></param>
		...
		bool ruoloAssociato, DocsPaDB.DBProvider dbProvider)
	{
		...
		finally
		{
			...
			else
			{
				if (idRuolo != null)
				{
					logger.Debug("Fallito tentativo estensione visibilit� per il ruolo: " + idRuolo);
				}
			}
		}
	...
	/// <summary>
	/// Verifica se il ruolo � gi� associato al nodo di titolario
	/// </summary>
		...
		try
		{
			...
			if (this.RetreiveIDFascicoloFolderFromTitolario(idTitolario, out idFascicolo, out idFolder))
			{
				...
				foreach (string idRecordTitolario in idToUpdate)
				{
					...
					// Se associazione ruolo, verifica che non sia gi� associato (per evitare duplicazioni)
					// Se dissociazione ruolo, verifica che sia gi� associato
	...
	public DocsPaVO.amministrazione.EsitoOperazione InsertNodoTitolario(ref DocsPaVO.amministrazione.OrgNodoTitolario nodoTitolario)
	{
		...
		try
		{
			...
			{
				// Inserimento visibilit� ruoli del titolario
					...
					nodoTitolario.IDParentNodoTitolario, dbProvider))
				{
					...
					if (errorDescription != string.Empty)
						errorDescription += "\\n";
					errorDescription += " - Errore in inserimento dei ruoli che hanno visibilit� sul titolario";
				}
			}
			else
			{
	...
	private bool InsertTitolarioInProject(string idAmministrazione, DocsPaVO.amministrazione.OrgNodoTitolario nodoTitolario)
	{
		bool retValue = true;
		string documentale = DocsPaVO.Settings.AppSettings.Instance.documentale.ToString();
		string dbms = DocsPaVO.Settings.AppSettings.Instance.DBType.ToString();
		...
		//attenzione: modifica provvisoria per la configurazione HM/SQL
		documentale = DocsPaVO.Settings.AppSettings.Instance.documentale.ToString();
		dbms = DocsPaVO.Settings.AppSettings.Instance.DBType.ToString();
		if ((documentale.ToLower() == "hummingbird"))
		...
		if (retValue)
		{
			...
			var id = nodoTitolario.ID;
			retValue = this.ExecuteScalar(out id, commandText);
			nodoTitolario.ID = id;
	...
	/// <summary>
	/// Verifica dell'univocit� del codice titolario
	...
	/// inserimento della visibilit� dei ruoli associati al nodo padre (se presente).
	/// La visibilit� dei ruoli � subordinata al registro del titolario padre:
		...
		else
		{
			...
			if (this.RetreiveIDFascicoloFolderFromTitolario(idTitolario, out idFascicolo, out idFolder, dbProvider))
			{
				foreach (DataRow rowRuoloAssociato in dtRuoliAssociati.Rows)
				{
					...
					// se il ruolo pu� avere la visibilit� del titolario
					// appena inserito
					if (htRuoliVisibiliInTitolario.ContainsKey(idRuolo))
					{
						rowToInsert++;
	...
	/// <summary>
	/// Verifica se il ruolo richiesto ha la visibilit� sul nodo
	...
	public bool ExistRuoloInTitolarioPadre(string idTitolario, string idRuolo)
	{
		...
		if (ds.Tables[0].Rows.Count != 0)
		{
			...
			if (!retValue)
				// Verifica se il ruolo ha la visibilit� sul titolario padre
		...
		// se idTitolarioPadre==0, il titolario � il nodo radice
	...
	/// <summary>
	/// Verifica se un determinato ruolo ha la visibilit�
	...
	/// <summary>
	/// Verifica se, per un titolario, � presente
	...
	/// <summary>
	/// Verifica se un determinato ruolo � il creatore
	...
	/// � il creatore di almeno un fascicolo procedimentale
	...
	public bool IsRuoloOwnerFascicoloProcedimentale(string idTitolario, string idRuolo)
	{
		...
		foreach (string idFascicoloProc in listIDFascicoliProc)
		{
			...
			{
				// Il ruolo � creatore di almeno un fascicolo
	...
	/// </summary>
	/// <param name="idTitolario"></param>
	/// <returns>Se -1, il titolario fornito � root</returns>
	...
	public bool DeleteRegistro(DocsPaVO.amministrazione.OrgRegistro registro)
	{
		...
		try
		{
			...
			// Se � un RF cancellazione riferimento in tabella "DPA_CORR_GLOBALI"
	...
	public bool UpdateRegistro(DocsPaVO.amministrazione.OrgRegistro registro)
	{
		...
		try
		{
			...
			queryDef.setParam("codiceIpa", this.GetStringParameterValue(registro.codiceIpa));


            //Gestione nuova auth Exchange SE SERVE
            queryDef.setParam("providerId", ",PROVIDER_ID='" + registro.Mail.Provider + "'");
            queryDef.setParam("tenantId", ",MS_TENANT_ID='" + registro.Mail.TenantId + "'");
            queryDef.setParam("clientId", ",MS_CLIENT_ID='" + registro.Mail.ClientId + "'");
            queryDef.setParam("clientSecret", ",MS_CLIENT_SEC='" + registro.Mail.ClientSecret + "'");
	...
	public bool InsertRegistro(DocsPaVO.amministrazione.OrgRegistro registro)
	{
		...
		try
		{
			...
			if (registro.chaRF != null && registro.chaRF == "0")
			{
				...
				queryMng.setParam("dataUltimoProtocollo", DocsPaDbManagement.Functions.Functions.ToDate(DateTime.Now.ToString("dd/MM/yy"), false));
			...
			//se � un RF popolo IdAooCollegata se � specificata
			if (registro.chaRF == "1")
			...
			queryMng.setParam("codiceIpa", this.GetStringParameterValue(registro.codiceIpa));


            //Gestione nuova auth Exchange SE SERVE
            //queryMng.setParam("providerId", ",PROVIDER_ID='" + registro.Mail.Provider + "'");
            queryMng.setParam("providerId",  !string.IsNullOrEmpty(registro.Mail.Provider)? registro.Mail.Provider: "NULL" );
            queryMng.setParam("tenantId", !string.IsNullOrEmpty(registro.Mail.TenantId) ? registro.Mail.TenantId : "NULL");
            queryMng.setParam("clientId", !string.IsNullOrEmpty(registro.Mail.ClientId) ? registro.Mail.ClientId : "NULL");
            queryMng.setParam("clientSecret", !string.IsNullOrEmpty(registro.Mail.ClientSecret) ? registro.Mail.ClientSecret : "NULL")
;
			string commandText = queryMng.getSQL();
			logger.Debug(commandText);
	...
	/// <summary>
	/// Verifica univocit� codice registro in amministrazione
	/// </summary>
	...
	/// <summary>
	/// Verifica univocit� codice tipo ruolo nell'ambito dell'amministrazione
	/// </summary>
	...
	/// <summary>
	/// Verifica univocit� descrizione mezzo di spedizione nell'ambito dell'amministrazione
	/// </summary>
	/// <param name="descrizione"></param>
	/// <returns></returns>
	...
	public ArrayList getSistemiEsterni(string idAmm)
	{
		...
		if (ds.Tables["SISTEMI_ESTERNI"] != null && ds.Tables["SISTEMI_ESTERNI"].Rows.Count > 0)
		{
			foreach (DataRow r in ds.Tables["SISTEMI_ESTERNI"].Rows)
			{
				...
				var token = sysExt.TokenPeriod;

				Int32.TryParse(r["VAR_TKN_TIME"].ToString(), out token);
				sysExt.TokenPeriod = token;
				retval.Add(sysExt);
			}
		}
	...
	// Metto qui per mettere vicino a Sistemi esterni. Si pu� spostare
	...
	public ArrayList APSSgetDelDetDaRiPubbl(string ogg_custom, string statiDiagramma, string templates, string dataUltimaEsecuzione, string tipo_evento)
	{
		...
		{
			logger.Error(ex, "errore in APSSgetDelDetDalRiPubbl");
			retval = null;
		}
	...
	public bool APSSCtrlAttachExt(string idDoc)
	{
		...
		{
			logger.Error(ex, "errore in APSSctrlAttachExt");
			retVal = false;
		}
	...
	public bool APSSUpdateEsitoPubb(string idDoc, string dataesito)
	{
		...
		{
			logger.Error(e, "errore in APSSUpdateEsitoPubb");
		}
	...
	public bool FattElEsitoNotifica(string idDoc, string esito)
	{
		...
		{
			logger.Error(e, "errore in FattElEsitoNotifica");
		}
	...
	public string FattElCtrlDupl(string idTibco, string idsdi, out string passo, out string esito)
	{
		...
		{
		   logger.Error(e, "errore in FattElCtrlDupl");
		}
	...
	public bool FattElInsTabellaLog(string idDoc, string idTibco, string idsdi)
	{
		...
		{
			logger.Error(e, "errore in FattElInsTabellaLog");
		}
	...
	public bool FattElUpdTabellaLog(string idTibco, string idsdi, string passo, string esito)
	{
		...
		{
			logger.Error(e, "errore in FattElUpdTabellaLog");
		}
	...
	public ArrayList FattElAttive_getCodFornitore(string idAmm)
	{
		...
		{
			logger.Error(ex, "errore in FattElAttive_getCodFornitore");
			retval = null;
	...
	public bool FattElAttive_InsertIntoLogPIS(string idProfile, string idUO, string codUo)
	{
		...
		using (DocsPaDB.DBProvider dbProvider = new DBProvider())
		{
			...
			{
				logger.Error(e, "errore in FattElAttive_InsertIntoLogPIS");
			}
		}
		return retval;
	}
	...
	public bool FattElAttive_UpSecProprietario(string idProfile)
	{
		...
		using (DocsPaDB.DBProvider dbProvider = new DBProvider())
		{
			...
			{
				logger.Error(e, "errore in FattElAttive_UpSecProprietario");
			}
		}
		return retval;
	}
	...
	public List<string> Albo_GetFileDaPubblicare(string idDocPrincipale, string option)
	{
		...
		{
			logger.Error(ex, "errore in Albo_GetFileDaPubblicare");
			retVal = null;
		}
		return retVal;
	}
	...
	public bool Albo_InsFileDaPubblicare(string idDocPrincipale, string docNumber, string daPubb)
	{
		...
		{
			logger.Error(ex, "errore in Albo_InsFileDaPubblicare");
			retVal = false;
		}
		return retVal;
	}
	...
	public List<string> Albo_GetStatiSelezioneFiles(string idDiagramma)
	{
		...
		{
			logger.Error(ex, "errore in Albo_GetStatiSelezioneFiles");
			retVal = null;
		}
		return retVal;
	}
	...
	public bool IsVerticalePubbAlbo(string codeAppVerticale)
	{
		...
		try
		{
			...
			if (dataset.Tables["ALBOPUBS"] != null && dataset.Tables["ALBOPUBS"].Rows.Count > 0)
			{
				logger.Debug("Verticale {0} censito per pubblicazione documenti.");
				retVal = true;
			}
			else
			{
		...
		{
			logger.Error(ex, "errore in IVerticalePubbAlbo");
			retVal = false;
		}
		return retVal;
	}
	...
	public ArrayList Ctrl_stmp_rep_errori()
	{
		...
		{
			logger.Error(ex, "errore in Ctrl_stmp_rep_errori");
			retVal = null;
		}
		return retVal;
	}
	...
	public ArrayList Ctrl_stampa_rep_file()
	{
		...
		{
			logger.Error(ex, "errore in Ctrl_stampa_rep_file");
			retVal = null;
		}
		return retVal;
	}
	...
	public ArrayList MIGR_FS_GetListMIGRFileInfo(string minVersionId, string maxVersionId)
	{
		...
		{
			logger.Error(ex, "errore in MIGR_FS_GetListMIGRFileInfo");
			retVal = null;
		}
		return retVal;
	}
	...
	public ArrayList CAPgetOpportunitiesPending(string typePrefix, string idPeople, string pendApprove,string idOpp)
	{
		...
		{
			logger.Error(ex, "errore in CAPgetOpportunitiesPending");
			retVal = null;
		}
		return retVal;
	}
	...
	public ArrayList CAPGetOppApprovals(string idOpp)
	{
		...
		{
			logger.Error(ex, "errore in CAPGetOppApprovals");
			retVal = null;
		}
		return retVal;
	}
	...
	public DataTable C3GetDocs(string fromTime, string toTime, string optionTime)
	{
		...
		{
			logger.Error(ex, "errore in C3GetDocs");
			retVal = null;
		}
		return retVal;
	}
	...
	public DataTable C3GetAllegatiByDocID(string idDoc)
	{
		...
		{
			logger.Error(ex, "errore in C3GetAllegatiByDocID");
			retVal = null;
		}
		return retVal;
	}
	...
	public DataTable C3GetDocsMod(string fromTime, string toTime, string optionTime)
	{
		...
		{
			logger.Error(ex, "errore in C3GetDocsMod");
			retVal = null;
		}
		return retVal;
	}
	...
	public DataTable C3GetDocsModAll(string fromTime, string toTime, string optionTime)
	{
		...
		{
			logger.Error(ex, "errore in C3GetDocsModAll");
			retVal = null;
		}
		return retVal;
	}
	...
	public DataTable C3GetDocsModLastEditDate(string fromTime, string toTime, string optionTime)
	{
		...
		{
			logger.Error(ex, "errore in C3GetDocsModLastEditDate");
			retVal = null;
		}
		return retVal;
	}
	...
	#endregion
	#region SearchDocEvents REST
	public DataSet SearchDocEvents(string fromDate, string ToDate, string eventi, string idTipologie, string idList, string idGruppo, string idUtente)
	{
		DataSet retval = null;
		try
		{
			DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_SRC_DOCEVENTS_REST");
			q.setParam("fromDate", fromDate.Replace("'","''"));
			if (!string.IsNullOrEmpty(ToDate))
			{
				string toDate2 = "";
				if(DBType.ToUpper().Equals("ORACLE"))
					toDate2 = string.Format(" AND P.DTA_AZIONE < TO_DATE ('{0} 23:59:59','dd/mm/yyyy hh24:mi:ss') ", ToDate.Replace("'","''"));
				else if (DBType.ToUpper().Equals("SQL"))
				{
					//TODO SQL
				}
				q.setParam("toDate", toDate2);
			}
			else
			{
				q.setParam("toDate", "");
			...
			q.setParam("eventi", eventi);
			if (!string.IsNullOrWhiteSpace(idTipologie))
			{
				q.setParam("tipi", string.Format("AND P3.ID_TIPO_ATTO in ({0})", idTipologie));
			}
			else
			{
				q.setParam("tipi", "");
			...
			{
				q.setParam("idList", string.Format("AND P.ID_OGGETTO in ({0})", idList));
			}
			else
			{
				q.setParam("idList", "");
			...
			if (eventi.ToUpper().Contains("ANNULLATA_LIQ"))
			{
				...
				union += string.Format(" ( SELECT P.SYSTEM_ID AS IDLOG, P.USERID_OPERATORE AS USER_ID_OP, P.ID_PEOPLE_OPERATORE AS ID_PEOPLE_OP, P.ID_GRUPPO_OPERATORE AS ID_RUOLO_OP, " +
					...
					" AND dat.DTA_INS >= TO_DATE('{3} 00:00:00', 'dd/mm/yyyy HH24:mi:ss') " +
					" {4}  " +
					...
					"Annullata liquidazione", "ANNULLATA_LIQ", "LIQUIDAZIONE", fromDate, toDateLiq, "Rifiutato Firma (annullo definitivo in contabilit�)", "SI", idListLiq, idGruppo, idUtente);
			}
			if (eventi.ToUpper().Contains("ANNULLATA_DET"))
			{
				...
				union += string.Format(" ( SELECT P.SYSTEM_ID AS IDLOG, P.USERID_OPERATORE AS USER_ID_OP, P.ID_PEOPLE_OPERATORE AS ID_PEOPLE_OP, P.ID_GRUPPO_OPERATORE AS ID_RUOLO_OP, " +
					...
					" AND dat.DOC_NUMBER = P.ID_OGGETTO " +
					" AND dat.DTA_INS >= TO_DATE('{4} 00:00:00', 'dd/mm/yyyy HH24:mi:ss') " +
					" {5}  " +
					" AND P.System_id = (select system_id from dpa_log pp where pp.id_oggetto = dat.doc_number and " +
					" pp.DTA_AZIONE > (dat.DTA_INS - INTERVAL '1' Minute) and pp.DTA_AZIONE < (dat.DTA_INS + INTERVAL '1' minute) and rownum =1)  " +
					...
					"Rifiutato Firma (annullo definitivo in DDG)", "ANNULLATA_DET", "Motivo del rifiuto Firma", "DETERMINAZIONE", fromDate, toDateDet, "Rifiutato Firma (annullo definitivo in DDG)", "SI", idListDet, idGruppo, idUtente);
			}
			if (eventi.ToUpper().Contains("ANNULLATA_DEL"))
			{
				...
				union += string.Format(" ( SELECT P.SYSTEM_ID AS IDLOG, P.USERID_OPERATORE AS USER_ID_OP, P.ID_PEOPLE_OPERATORE AS ID_PEOPLE_OP, P.ID_GRUPPO_OPERATORE AS ID_RUOLO_OP, " +
					...
					" AND dat.ID_OGGETTO IN (SELECT SYSTEM_ID FROM DPA_OGGETTI_CUSTOM WHERE SYSTEM_ID = DAT.ID_OGGETTO AND  UPPER(DESCRIZIONE) = UPPER('{6}')) " +
					" AND dat.VALORE_OGGETTO_DB = '{7}' {8} AND EXISTS(SELECT 'X' FROM SECURITY E WHERE E.THING = P.ID_OGGETTO AND E.ACCESSRIGHTS > 20 AND E.PERSONORGROUP IN ({9}, {10})) ) ",
					"Rifiutato Firma (annullo definitivo in DDG)", "ANNULLATA_DEL", "Motivo del rifiuto Firma", "PARERE DELIBERAZIONE", fromDate, toDateDel, "Rifiutato Firma (annullo definitivo in DDG)", "SI", idListDel, idGruppo, idUtente);
			}
			q.setParam("union", union);


			q.setParam("idRuolo", idGruppo);
			q.setParam("idUtente", idUtente);

			string queryString = q.getSQL();
			logger.Debug(queryString);
			DataSet dataset = new DataSet();
			this.ExecuteQuery(out dataset, "DOCEVENTS", queryString);
			retval = dataset;
		}
		catch (Exception ex)
		{
			logger.Error(ex, "errore in SearchDocEvents");
			throw ex;
		}
		return retval;
	}
	...
	public bool LoginAmministratore(InfoUtenteAmministratore infoUtente, string ipAddress, string webSessionId, bool forceLogin, out bool userAlreadyConnected)
	{
		...
		using (DocsPaDB.DBProvider dbProvider = new DBProvider())
		{
			...
			if (!string.IsNullOrEmpty(dataConn))
			{
				if (!forceLogin)
				{
					logger.Debug(string.Format("Amministratore gi� collegato dal giorno {0}", dataConn));

					// utente amministratore gi� collegato
	...
	public DocsPaVO.amministrazione.InfoAmministrazione AmmGetInfoAmmCorrente(string idAmm)
	{
		...
		string fields = "A.SYSTEM_ID AS ID," +
			...
			//NEW CODE
                            ...
                            "A.VAR_INDIRIZZO_DIGITALE_RIF," +
                            "A.VAR_CODICE_AMM_IPA," +
                            "A.PROVIDER_ID," +
                            "A.MS_TENANT_ID," +
                            "A.MS_CLIENT_ID," +
                            "A.MS_CLIENT_SEC";
		...
		using (DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider())
		{
			using (System.Data.IDataReader reader = dbProvider.ExecuteReader(commandText))
			{
				while (reader.Read())
				{
					...
					amm.codiceIpa = reader.GetValue(reader.GetOrdinal("VAR_CODICE_AMM_IPA")).ToString();
					amm.indirizzoDigitaleRiferimento = reader.GetValue(reader.GetOrdinal("VAR_INDIRIZZO_DIGITALE_RIF")).ToString();
                    ...
                    //Nuova gestione per parametri casella di posta MS Exchange
                    amm.Provider = reader.GetValue(reader.GetOrdinal("PROVIDER_ID")).ToString();
                    amm.ClientId = reader.GetValue(reader.GetOrdinal("MS_CLIENT_ID")).ToString();
                    amm.ClientSecret = reader.GetValue(reader.GetOrdinal("MS_CLIENT_SEC")).ToString();
                    amm.TenantId = reader.GetValue(reader.GetOrdinal("MS_TENANT_ID")).ToString();
	...
	/// <returns>True se il ruolo � stato modificato</returns>
	...
	/// <summary>
	/// Metodo per verificare se un codice ruolo � gi� utulizzato
	/// </summary>
	...
	/// <param name="storicizzato">Verr� valorizzato con true se il codice � utilizzato da un ruolo storicizzato</param>
	/// <param name="roleDesc">Descrizione del ruolo storicizzato che utilizza il codice</param>
	/// <returns>True se il codice ruolo gi� � utilizzato, false altrimenti</returns>
	...
	/// <summary>
	/// Metodo per il calcolo dell'atipicit� sugli oggetti visti dal ruolo eliminato e dai ruoi sottoposti
	/// </summary>
	...
	public DocsPaVO.utente.Registro[] GetRfByIdAmm(int idAmministrazione, string tipo)
	{
		...
		using (DBProvider dbProvider = new DBProvider())
		{
			...
			{
				result = new DocsPaVO.utente.Registro[dataSet.Tables[0].Rows.Count];
	...
	private void SetRf(ref DocsPaVO.utente.Registro registro, DataSet dataSet, int rowNumber)
	{
		try
		{
		...
		try
		{
			...
			foreach (CasellaRegistro c in caselle)
			{
				if (!string.IsNullOrEmpty(c.EmailRegistro))
				{
                    ...
                    //Gestione nuova auth Exchange SE SERVE
                    queryDef.setParam("idProvider", String.Format(",PROVIDER_ID = {0}", (!string.IsNullOrEmpty(c.Provider)) ? c.Provider : "NULL"));
                    queryDef.setParam("idTenant", !string.IsNullOrEmpty(c.TenantId)? ",MS_TENANT_ID='" + c.TenantId + "'" : string.Empty);
                    queryDef.setParam("clientId", !string.IsNullOrEmpty(c.ClientId) ? ",MS_CLIENT_ID='" + c.ClientId + "'" : string.Empty);
					queryDef.setParam("clientSecret", !string.IsNullOrEmpty(c.ClientSecret) ? ",MS_CLIENT_SEC='" + c.ClientSecret + "'": string.Empty);
                    string commandText = queryDef.getSQL();
					this.ExecuteNonQuery(commandText, out rowsAffected);
	...
	/// <summary>
	/// Inserisce una o pi� caselle di posta associate al registro/RF
	/// </summary>
	/// <param name="idRegistro"></param>
	/// <param name="caselle"></param>
	/// <returns></returns>
	public bool InsertMailRegistro(string idRegistro, CasellaRegistro[] caselle, bool insertInteropInt)
	{
		...
		//nel caso di nuovo rf/registro per il quale non � stata definita una mail principale, configuro la prima come preferita
	...
	public bool DeleteMailRegistro(string idRegistro, string casella)
	{
		...
		try
		{
			...
			//se non ci sono pi� caselle, allora svuoto anche i campi mail in DPA_EL_REGISTRI
			DataSet ds = GetDsMailRegistro(idRegistro);
			if (ds.Tables.Count == 0 || ds.Tables["MAIL_REGISTRI"].Rows.Count == 0)
			{
	...
	public bool DeleteRigthRuoloMailRegistro(string idRegistro, string idRuoloInUO, string indirizzoEmail)
	{
		...
		try
		{
			...
			if (!string.IsNullOrEmpty(idRegistro))
			{
				...
				// S. Furnari. 30/05/2013 - Interoperabilita interna diretta a UO e non ad AOO
	...
	public DataSet GetRigthRuoloMailRegistro(string idRegistro, string idRuoloInUO)
	{
		...
		try
		{
			...
			queryDef.setParam("idRegistro", this.GetNumberParameterValue(Convert.ToInt32(idRegistro)));
			if (!string.IsNullOrEmpty(DocsPaVO.Settings.AppSettings.Instance.INTEROP_INT_NO_MAIL) && DocsPaVO.Settings.AppSettings.Instance.INTEROP_INT_NO_MAIL.Equals("1"))
			{
				if (DBType.ToUpper().Equals("ORACLE"))
	...
	/// Solo per doc ricevuti per Interoperabilita
	/// </summary>
			...
			using (DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider())
			{
				...
				switch (print_freq)
				{
					case 10:
						...
						// quindi il FE pu� rimanere in pagina bianca.
	...
	public bool updateStampaRegistroValues(string idAmm, string disabled, string printFreq, string printHour)
	{
		...
		try
		{
			using (DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider())
			{
				...
				{
					//
					// Imposto la data di ultima stampa alla data odierna + l'intervallo della frequenza di stampa, poich� essa non � ancora stata definita.
					switch (print_freq)
					{
						case 10:
							double day = 1;
				...
				else
				{
					...
					switch (print_freq)
					{
						case 10:
							...
							{
								//Gabriele Melini 20-09-2013
								//se l'ora di stampa impostata � minore dell'ora attuale, la stampa viene schedulata al giorno successivo
	...
	/// <summary>
	/// Verifica se il log specificato � attivo per l'amministrazione corrente
	/// </summary>
	/// <param name="idAmm"></param>
	/// <param name="codice"></param>
	/// <returns></returns>
	...
	public DocsPaVO.amministrazione.VisualizzaStatoDoc GetInfoDocument(string docnumber, string numProto, string anno, string idAmm, string codiceRegistro)
	{
		...
		if (this.ExecuteQuery(out ds, "S_PROFILE_INFO_DOC", query))
		{
			if (ds.Tables["S_PROFILE_INFO_DOC"] != null && ds.Tables["S_PROFILE_INFO_DOC"].Rows.Count > 0)
			{
				...
				#region RICERCA PROCESSI FIRMA AVVIATI

				DocsPaDB.Query_DocsPAWS.LibroFirma libroFirma = new LibroFirma();
				info.istanzaProcessiFirmaAvviati = libroFirma.GetIstanzaProcessoDiFirmaByDocnumber(info.idDocumento, null);
    ...
    public DataSet GetDsProviderList()
    {
        DocsPaUtils.Query queryMng = DocsPaUtils.InitQuery.getInstance().getQuery("AMM_S_GET_PROVIDERS");

        string commandText = queryMng.getSQL();
        logger.Debug(commandText);

        DataSet ds = new DataSet();
        this.ExecuteQuery(ds, "MAIL_PROVIDERS", commandText);
        return ds;
    }

    public string GetLibByProviderId(string providerId)
    {
        string returnValue = string.Empty;
        DocsPaUtils.Query queryDef = DocsPaUtils.InitQuery.getInstance().getQuery("AMM_S_SELECT_LIB_BY_PROVIDER_ID");
        queryDef.setParam("providerId", providerId);
        string commandText = queryDef.getSQL();
        logger.Debug("QUERY - " + commandText);
        ExecuteScalar(out returnValue, commandText);
        logger.Debug("LIB - " + returnValue);
        return returnValue;
    }

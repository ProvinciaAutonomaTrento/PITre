...
public class LibroFirma : DBProvider
{
    ...
    private static ILogger logger = Serilog.Log.ForContext(typeof(LibroFirma));
    ...
    public List<DocsPaVO.utente.TipoRuolo> GetTypeRoleByIdAmm(string idAmm)
    {
        ...
        logger.Information("Fine Metodo GetTypeRoleByIdAmm in DocsPaDb.Query_DocsPAWS.LibroFirma");
        ...
        try
        {
            ...
            if (this.ExecuteQuery(out ds, "IstanzaPassoFirmaInAttesa", query))
            {
                if (ds.Tables["IstanzaPassoFirmaInAttesa"] != null && ds.Tables["IstanzaPassoFirmaInAttesa"].Rows.Count > 0)
                {
                    ...
                    istanza.ApplicaSegnaturaPermanente = string.IsNullOrEmpty(row["CHA_POS_SEGNATURA"].ToString()) || row["CHA_POS_SEGNATURA"].ToString().Equals("0") ? "0" : "1";
                    istanza.PosizioneSegnaturaPermanente = row["VAR_POS_SEGNATURA"].ToString();
                    istanza.IsAutomatico = string.IsNullOrEmpty(row["CHA_AUTOMATICO"].ToString()) || !row["CHA_AUTOMATICO"].ToString().Equals("1") ? false : true;
                }
            }
        }
        catch (Exception e)
    ...
    public IstanzaPassoDiFirma GetIstanzaPassoFirmaWithSegnaturaPermanente(string docnumber)
    {
        IstanzaPassoDiFirma istanza = null;
        ...
        {
            DataSet ds = new DataSet();
            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_ISTANZA_PASSO_LIBRO_FIRMA_BY_DOCNUMBER_SEGNATURA_PERMANENTE");
            q.setParam("docnumber", docnumber);

            string query = q.getSQL();
            logger.Debug("GetIstanzaPassoFirmaWithSegnaturaPermanente: " + query);

            if (this.ExecuteQuery(out ds, "IstanzaPassoFirma", query))
            {
                if (ds.Tables["IstanzaPassoFirma"] != null && ds.Tables["IstanzaPassoFirma"].Rows.Count > 0)
                {
                    DataRow row = ds.Tables["IstanzaPassoFirma"].Rows[0];
                    istanza = new IstanzaPassoDiFirma();
                    istanza.RuoloCoinvolto = new DocsPaVO.utente.Ruolo() { idGruppo = row["ID_RUOLO_COINVOLTO"].ToString() };
                    istanza.UtenteCoinvolto = new DocsPaVO.utente.Utente() { idPeople = row["Id_UTENTE_COINVOLTO"].ToString() };
                    istanza.UtenteLocker = row["ID_UTENTE_LOCKER"].ToString();
                    istanza.TipoFirma = row["TIPO_FIRMA"].ToString();
                    istanza.ApplicaSegnaturaPermanente = row["CHA_POS_SEGNATURA"].ToString();
                    istanza.PosizioneSegnaturaPermanente = row["VAR_POS_SEGNATURA"].ToString();
                    istanza.IsAutomatico = string.IsNullOrEmpty(row["CHA_AUTOMATICO"].ToString()) || !row["CHA_AUTOMATICO"].ToString().Equals("1") ? false : true;
                }
            }
        ...
        {
            logger.Error("Errore in GetIstanzaPassoFirmaWithSegnaturaPermanente: " + e.Message);
        }
    ...
    {
        logger.Information("Inizio Metodo GetEventNotification in DocsPaDb.Query_DocsPAWS.LibroFirma");
        ...
        logger.Information("Fine Metodo GetEventNotification in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetEventTypes in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<AnagraficaEventi> eventTypes = new List<AnagraficaEventi>();
        AnagraficaEventi evento = null;
        try
        {
        ...
        logger.Information("Fine Metodo GetEventTypes in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetProcessesSignatureVisibleRole in DocsPaDb.Query_DocsPAWS.LibroFirma");
        ...
        logger.Information("Fine Metodo GetProcessiDiFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetProcessiDiFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<ProcessoFirma> listProcessiDiFirma = new List<ProcessoFirma>();
        ProcessoFirma processoDiFirma = null;
            ...
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                using (IDataReader reader = dbProvider.ExecuteReader(query))
                {
                    while (reader.Read())
                    {
                        ...
                        if (processoDiFirma != null && processoDiFirma.passi != null && !reader.IsDBNull(reader.GetOrdinal("ID_PASSO")))
    ...
    {
        logger.Information("Inizio Metodo GetProcessiDiFirmaByFilter in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<ProcessoFirma> listProcessiDiFirma = new List<ProcessoFirma>();
        ProcessoFirma processoDiFirma = null;
        try
        {
        ...
        logger.Information("Fine Metodo GetProcessiDiFirmaByFilter in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetProcessoDiFirmaById in DocsPaDb.Query_DocsPAWS.LibroFirma");
        ProcessoFirma processoDiFirma = null;
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetProcessoDiFirmaById in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetProcessiDiFirmaByRuoloTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<ProcessoFirma> listProcessiDiFirma = new List<ProcessoFirma>();
        ProcessoFirma processoDiFirma = null;
        try
        {
        ...
        logger.Information("Fine Metodo GetProcessiDiFirmaByRuoloTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetCountProcessiDiFirmaByRuoloTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        int countProcessi = 0;
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetCountProcessiDiFirmaByRuoloTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetProcessiDiFirmaByUtenteTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<ProcessoFirma> listProcessiDiFirma = new List<ProcessoFirma>();
        ProcessoFirma processoDiFirma = null;
        try
        {
        ...
        logger.Information("Fine Metodo GetProcessiDiFirmaByUtenteTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetCountProcessiDiFirmaByUtenteTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        int countProcessi = 0;
        try
        {
            string query;
    ...
    {
        logger.Information("Inizio Metodo GetIstanzaProcessoDiFirmaByDocnumber in DocsPaDb.Query_DocsPAWS.LibroFirma");
        ...
        logger.Information("Fine Metodo GetIstanzaProcessoDiFirmaByDocnumber in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetCountIstanzaProcessiDiFirmaByRuoloTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        ...
        logger.Information("Fine Metodo GetCountIstanzaProcessiDiFirmaByRuoloTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetIstanzaProcessiDiFirmaByUtenteTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<IstanzaProcessoDiFirma> istanzeProcessoDiFirmaList = new List<IstanzaProcessoDiFirma>();
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetIstanzaProcessiDiFirmaByUtenteTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetCountIstanzaProcessiDiFirmaByUtenteTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        int countIstanzaProcessi = 0;
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetCountIstanzaProcessiDiFirmaByUtenteTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    public PassoFirma GetPassiProcessoDiFirma(IDataReader reader)
    {
        ...
        try
        {
            ...
            passo.ApplicaSegnaturaPermanente = DocsPaUtils.Data.DataReaderHelper.GetValue<object>(reader, "CHA_POS_SEGNATURA", true, string.Empty).ToString();
            passo.PosizioneSegnaturaPermanente = DocsPaUtils.Data.DataReaderHelper.GetValue<object>(reader, "VAR_POS_SEGNATURA", true, string.Empty).ToString();
        }
        catch (Exception exc)
        {
    ...
    {
        logger.Information("Inizio Metodo GetPassiProcessoDiFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
            ...
            if (this.ExecuteQuery(out ds, "passiProcessoDiFirma", query))
            {
                if (ds.Tables["passiProcessoDiFirma"] != null && ds.Tables["passiProcessoDiFirma"].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables["passiProcessoDiFirma"].Rows)
                    {
                        ...
                        passo.ApplicaSegnaturaPermanente = row["CHA_POS_SEGNATURA"].ToString();
                        passo.PosizioneSegnaturaPermanente = row["VAR_POS_SEGNATURA"].ToString();
                        listPassi.Add(passo);
        ...
        logger.Information("Fine Metodo GetPassiProcessoDiFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetIstanzaPassoDiFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
                ...
                if (ds.Tables["istanzaPassoDiFirma"] != null && ds.Tables["istanzaPassoDiFirma"].Rows.Count > 0)
                {
                    foreach (DataRow row in ds.Tables["istanzaPassoDiFirma"].Rows)
                    {
                        istanzaPassoDiFirma = new IstanzaPassoDiFirma()
                        {
                            ...
                            ApplicaSegnaturaPermanente = row["CHA_POS_SEGNATURA"].ToString(),
                            PosizioneSegnaturaPermanente = row["VAR_POS_SEGNATURA"].ToString()
                        };
                    }
                }
            }
    ...
    public IstanzaPassoDiFirma GetNextIstanzaPasso(string idIstanzaProcesso, int numOrdinePrecedente, string versionId)
    {
        ...
        logger.Information("Inizio Metodo GetNextIstanzaPasso in DocsPaDb.Query_DocsPAWS.LibroFirma");
        IstanzaPassoDiFirma istanzaPassoDiFirma = null;
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetNextIstanzaPasso in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    public List<ElementoInLibroFirma> GetElementiInLibroFirmaIntoPage(DocsPaVO.utente.InfoUtente infoUtente, int pageSize, int requestedPage, string testoRicerca, DocsPaVO.Mobile.RicercaType tipoRicerca, out int totalRecordCount, List<DocsPaVO.filtri.FiltroRicerca> filtriDoc)
    //string oggetto, string proponente, string note, string iddocumento, string numproto, string dataDa, string dataa,string numAnnoProto, string dataProtoDa, string dataProtoA)
    {
        logger.Debug("Inizio Metodo GetElementiInLibroFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
        totalRecordCount = 0;
        string[] tempsplit;
        ...
        try
        {
            ...
            //            " OR  LOWER(getpeoplename(UTENTE_PROPONENTE)) LIKE LOWER('%" + testoRicerca + "%') OR  LOWER(getpeopleuserid(UTENTE_PROPONENTE)) LIKE LOWER('%" + testoRicerca + "%'))";
            //    }
            //    if (tipoRicerca.Equals(DocsPaVO.Mobile.RicercaType.RIC_IDDOC_LF))
            //    {
            //        filtroRicerca = " AND P.SYSTEM_ID = "+testoRicerca+ " ";
            //    }
            //    if (tipoRicerca.Equals(DocsPaVO.Mobile.RicercaType.RIC_NUMPROTO_LF))
            //    {
            //        filtroRicerca = " AND P.NUM_PROTO = " + testoRicerca + " ";
            //    }
            //    if (tipoRicerca.Equals(DocsPaVO.Mobile.RicercaType.RIC_NUMANNOPROTO_LF))
            //    {
            //        tempsplit = testoRicerca.Split('-');
            //        if(tempsplit!= null && tempsplit.Length>0)
            //            filtroRicerca = " AND P.NUM_PROTO = " + tempsplit[0].Trim() + " AND P.NUM_ANNO_PROTO = "+ tempsplit[1].Trim() +" ";
            //    }
            //    if (tipoRicerca.Equals(DocsPaVO.Mobile.RicercaType.RIC_DATA_LF))
            //    {
            //        if (dbType.ToUpper() == "ORACLE")
            //        {
            //            filtroRicerca = " AND (P.CREATION_DATE BETWEEN TO_DATE('" + testoRicerca + " 00:00:00','dd/mm/yyyy hh24:mi:ss')  AND TO_DATE('" + testoRicerca + " 23:59:59','dd/mm/yyyy hh24:mi:ss')  OR P.DTA_PROTO  BETWEEN TO_DATE('" + testoRicerca + " 00:00:00','dd/mm/yyyy hh24:mi:ss')   AND TO_DATE('" + testoRicerca + " 23:59:59','dd/mm/yyyy hh24:mi:ss') )";
            //        }
            //        else if (dbType.ToUpper() == "SQL")
            //        {
            //            filtroRicerca = " AND (P.CREATION_DATE BETWEEN CONVERT(DATETIME,'" + testoRicerca + "',103)+' 00:00:00' AND CONVERT(DATETIME,'" + testoRicerca + "',103)+' 23:59:59'  OR P.DTA_PROTO  BETWEEN CONVERT(DATETIME,'" + testoRicerca + "',103)+' 00:00:00' AND CONVERT(DATETIME,'" + testoRicerca + "',103)+' 23:59:59' )";
            //        }
            //    }

            //    if (tipoRicerca.Equals(DocsPaVO.Mobile.RicercaType.RIC_INTER_DATE_LF))
            //    {
            //        tempsplit = testoRicerca.Split('-');
            //        if (tempsplit != null && tempsplit.Length > 0)
            //        {
            //            if (dbType.ToUpper() == "ORACLE")
            //            {
            //                filtroRicerca = " AND (P.CREATION_DATE BETWEEN TO_DATE('" + tempsplit[0] + " 00:00:00','dd/mm/yyyy hh24:mi:ss')  AND TO_DATE('" + tempsplit[1] + " 23:59:59','dd/mm/yyyy hh24:mi:ss')  OR P.DTA_PROTO  BETWEEN TO_DATE('" + tempsplit[0] + " 00:00:00','dd/mm/yyyy hh24:mi:ss')   AND TO_DATE('" + tempsplit[1] + " 23:59:59','dd/mm/yyyy hh24:mi:ss') )";
            //            }
            //            else if (dbType.ToUpper() == "SQL")
            //            {
            //                filtroRicerca = " AND (P.CREATION_DATE BETWEEN CONVERT(DATETIME,'" + tempsplit[0] + "',103)+' 00:00:00' AND CONVERT(DATETIME,'" + tempsplit[1] + "',103)+' 23:59:59'  OR P.DTA_PROTO  BETWEEN CONVERT(DATETIME,'" + tempsplit[0] + "',103)+' 00:00:00' AND CONVERT(DATETIME,'" + tempsplit[1] + "',103)+' 23:59:59' )";
            //            }
            //        }
            //    }
            //}
            if (filtriDoc != null && filtriDoc.Count > 0)
            {
                foreach (DocsPaVO.filtri.FiltroRicerca filtroX in filtriDoc)
                {
                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.OGGETTO.ToString())
                    {
                        //if (!string.IsNullOrWhiteSpace(oggetto))

                        filtroRicerca += " AND lower(P.VAR_PROF_OGGETTO) like lower('%" + filtroX.valore + "%') ";
                    }
                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.NOTE.ToString())
                    {
                        filtroRicerca += " AND lower(e.note) like lower('%" + filtroX.valore + "%') ";
                    }
                    if (filtroX.argomento == "PROPONENTE")
                    {
                        filtroRicerca += " AND (LOWER(GETDESCRUOLO(RUOLO_PROPONENTE)) LIKE LOWER('%" + filtroX.valore + "%') OR LOWER(getcodRuolo(RUOLO_PROPONENTE)) LIKE LOWER('%" + filtroX.valore + "%') " +
                            " OR  LOWER(getpeoplename(UTENTE_PROPONENTE)) LIKE LOWER('%" + filtroX.valore + "%') OR  LOWER(getpeopleuserid(UTENTE_PROPONENTE)) LIKE LOWER('%" + filtroX.valore + "%'))";
                    }
                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.DOCNUMBER.ToString())
                    {
                        filtroRicerca += " AND P.SYSTEM_ID = " + filtroX.valore + " ";
                    }
                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.NUM_PROTOCOLLO.ToString())
                    {
                        filtroRicerca += " AND P.NUM_PROTO = " + filtroX.valore + " ";
                    }
                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.ANNO_PROTOCOLLO.ToString())
                    {
                        tempsplit = filtroX.valore.Split('-');
                        if (tempsplit != null && tempsplit.Length > 0)
                            filtroRicerca += " AND P.NUM_PROTO = " + tempsplit[0].Trim() + " AND P.NUM_ANNO_PROTO = " + tempsplit[1].Trim() + " ";
                    }
                    //if (tipoRicerca.Equals(DocsPaVO.Mobile.RicercaType.RIC_DATA_LF))
                    ...
                    //    }
                    //}
                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.DATA_CREAZIONE_SUCCESSIVA_AL.ToString())
                    {
                        if (dbType.ToUpper() == "ORACLE")
                        {
                            filtroRicerca += " AND P.CREATION_TIME > TO_DATE('" + filtroX.valore + " 00:00:00','dd/mm/yyyy hh24:mi:ss') ";
                        }
                        else if (dbType.ToUpper() == "SQL")
                        {
                            filtroRicerca += " AND P.CREATION_TIME > CONVERT(DATETIME,'" + filtroX.valore + "',103)+' 00:00:00' ";
                        }
                    }

                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.DATA_CREAZIONE_PRECEDENTE_IL.ToString())
                    {
                        if (dbType.ToUpper() == "ORACLE")
                        {
                            filtroRicerca += " AND P.CREATION_TIME < TO_DATE('" + filtroX.valore + " 23:59:59','dd/mm/yyyy hh24:mi:ss') ";
                        }
                        else if (dbType.ToUpper() == "SQL")
                        {
                            filtroRicerca += " AND P.CREATION_TIME < CONVERT(DATETIME,'" + filtroX.valore + "',103)+' 23:59:59' ";
                        }
                    }

                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.DATA_PROT_SUCCESSIVA_AL.ToString())
                    {
                        if (dbType.ToUpper() == "ORACLE")
                        {
                            filtroRicerca += " AND P.DTA_PROTO > TO_DATE('" + filtroX.valore + " 00:00:00','dd/mm/yyyy hh24:mi:ss') ";
                        }
                        else if (dbType.ToUpper() == "SQL")
                        {
                            filtroRicerca += " AND P.DTA_PROTO > CONVERT(DATETIME,'" + filtroX.valore + "',103)+' 00:00:00' ";
                        }
                    }

                    if (filtroX.argomento == DocsPaVO.filtri.ricerca.listaArgomenti.DATA_PROT_PRECEDENTE_IL.ToString())
                    {
                        if (dbType.ToUpper() == "ORACLE")
                        {
                            filtroRicerca += " AND P.DTA_PROTO < TO_DATE('" + filtroX.valore + " 23:59:59','dd/mm/yyyy hh24:mi:ss') ";
                        }
                        else if (dbType.ToUpper() == "SQL")
                        {
                            filtroRicerca += " AND P.DTA_PROTO < CONVERT(DATETIME,'" + filtroX.valore + "',103)+' 23:59:59' ";
                        }
                    }

                    //if (!string.IsNullOrWhiteSpace(dataDa) && !string.IsNullOrWhiteSpace(dataa))
                    //{
                    //    if (dbType.ToUpper() == "ORACLE")
                    //        {
                    //            filtroRicerca += " AND (P.CREATION_DATE BETWEEN TO_DATE('" + dataDa + " 00:00:00','dd/mm/yyyy hh24:mi:ss')  AND TO_DATE('" + dataa + " 23:59:59','dd/mm/yyyy hh24:mi:ss')  OR P.DTA_PROTO  BETWEEN TO_DATE('" + dataDa + " 00:00:00','dd/mm/yyyy hh24:mi:ss')   AND TO_DATE('" + dataa + " 23:59:59','dd/mm/yyyy hh24:mi:ss') )";
                    //        }
                    //        else if (dbType.ToUpper() == "SQL")
                    //        {
                    //            filtroRicerca += " AND (P.CREATION_DATE BETWEEN CONVERT(DATETIME,'" + dataDa + "',103)+' 00:00:00' AND CONVERT(DATETIME,'" + dataa + "',103)+' 23:59:59'  OR P.DTA_PROTO  BETWEEN CONVERT(DATETIME,'" + dataDa + "',103)+' 00:00:00' AND CONVERT(DATETIME,'" + dataa + "',103)+' 23:59:59' )";
                    //        }
    ...
    public DocsPaVO.LibroFirma.TipoStatoElemento GetStatoElementoInLibroFirma(string idIstanzaPasso)
    {
        logger.Debug("INIZIO Metodo GetStatoElementoInLibroFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
        DocsPaVO.LibroFirma.TipoStatoElemento statoFirma = TipoStatoElemento.PROPOSTO;

        try
        {
            DataSet ds = new DataSet();
            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_DPA_ELEMENTI_LIBRO_FIRMA_STATO_FIRMA");
            q.setParam("idIstanzaPasso", idIstanzaPasso);

            string query = q.getSQL();
            logger.Debug("GetStatoElementoInLibroFirma: " + query);

            if (this.ExecuteQuery(out ds, "STATO_FIRMA", query))
            {
                if (ds.Tables["STATO_FIRMA"] != null && ds.Tables["STATO_FIRMA"].Rows.Count > 0)
                {
                    DataRow row = ds.Tables["STATO_FIRMA"].Rows[0];
                    statoFirma = (DocsPaVO.LibroFirma.TipoStatoElemento)Enum.Parse(typeof(TipoStatoElemento), row["STATO_FIRMA"].ToString());
                }
            }
        }
        ...
        {
            logger.Error("Errore in GetStatoElementoInLibroFirma: " + e.Message);
        }

        logger.Debug("FINE Metodo GetStatoElementoInLibroFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
        return statoFirma;
    }
    ...
    private InfoDocLibroFirma BuildInfoDocumento(DataRow row)
    {
        ...
        //Bind Fascicoli
        if (row.Table.Columns.Contains("COD_FASCICOLI") && !string.IsNullOrEmpty(row["COD_FASCICOLI"].ToString()))
        {
            string[] _spitFascicoli = row["COD_FASCICOLI"].ToString().Split(';');
            if(_spitFascicoli != null && _spitFascicoli.Length > 0 )
            {
                infoDocumento.Fascicoli = new List<InfoFascicoloAppartenenza>();
                foreach(string fasc in _spitFascicoli)
                {

                    string[] _splitFasc = fasc.Split('#');
                    infoDocumento.Fascicoli.Add(new InfoFascicoloAppartenenza() { CodiceFascicolo = _splitFasc[0], TipoFascicolo = _splitFasc[1], IdProject = _splitFasc[2], SicurezzaUtente = _splitFasc[3] }) ;
                }
            }
        }
    ...
    {
        logger.Information("Inizio Metodo GetIstanzaPassoDiFirmaInAttesa in DocsPaDb.Query_DocsPAWS.LibroFirma");
        IstanzaPassoDiFirma istanzaPassoDiFirma = null;
    ...
    {
        logger.Information("Inizio Metodo GetIstanzaProcessoDiFirmaByIdIstanzaProcesso in DocsPaDb.Query_DocsPAWS.LibroFirma");
        IstanzaProcessoDiFirma istanzaProcessoDiFirma = new IstanzaProcessoDiFirma();
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetIstanzaProcessoDiFirmaByIdIstanzaProcesso in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetIstanzaProcessoDiFirmaByIdIstanzaProcessoLite in DocsPaDb.Query_DocsPAWS.LibroFirma");
        IstanzaProcessoDiFirma istanzaProcessoDiFirma = new IstanzaProcessoDiFirma();
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetIstanzaProcessoDiFirmaByIdIstanzaProcessoLite in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetIstanzaProcessoDiFirmaByIdProcesso in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<IstanzaProcessoDiFirma> istanzeProcessoDiFirmaList = new List<IstanzaProcessoDiFirma>();
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetIstanzaProcessoDiFirmaByIdProcesso in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetTypeSignatureToBeEntered in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetIstanzePassoDiFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
        ...
        logger.Information("Fine Metodo GetIstanzePassoDiFirma in DocsPaDb.Query_DocsPAWS.LibroFirma");
                    ...
                    if (ds.Tables["istanzaProcessiDiFirma"] != null && ds.Tables["istanzaProcessiDiFirma"].Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in ds.Tables["istanzaProcessiDiFirma"].Rows)
                        {
                            istanzaProcessoDiFirma = new IstanzaProcessoDiFirma()
                            {
                                ...
                                DataAnnullamentoProtocollazione = !string.IsNullOrEmpty(row["DTA_ANNULLA"].ToString()) ? row["DTA_ANNULLA"].ToString() : string.Empty,
            ...
            switch (f.Argomento)
            {
                ...
                case "RUOLO_COINVOLTO":
                    if (!string.IsNullOrEmpty(f.Valore))
                    {
                        ...
                        condition += string.Format(" AND IP.ID_RUOLO_COINVOLTO = (SELECT ID_GRUPPO FROM DPA_CORR_GLOBALI WHERE SYSTEM_ID = {0}) AND I.ID_ISTANZA = IP.ID_ISTANZA_PROCESSO ", f.Valore);
                    }
                    break;
                case "UTENTE_COINVOLTO":
                    if (!string.IsNullOrEmpty(f.Valore))
                    {
                        ...
                        condition += string.Format(" AND IP.ID_UTENTE_COINVOLTO = (SELECT ID_PEOPLE FROM DPA_CORR_GLOBALI WHERE SYSTEM_ID = {0}) AND I.ID_ISTANZA = IP.ID_ISTANZA_PROCESSO ", f.Valore);
                    }
                    break;
                /*
                ...
                case "PROPONENTE_AVVIATI_DA_ME":
                    if (Convert.ToBoolean(f.Valore))
                    {
                        ...
                        if (string.IsNullOrEmpty(tipoVisibilita))
                        {
                            ...
                            tipoVisibilita += " AND ( ( ";
                            //tipoVisibilita += " V.CHA_TIPO_VISIBILITA IN ('P', 'T') AND ";
                            tipoVisibilita += " I.ID_RUOLO_PROPONENTE = " + IdRuoloProponente + "  AND I.ID_UTENTE_PROPONENTE = " + idUtenteProponente + "  ) ";
                        }
                        else
                        {
                            tipoVisibilita += " OR ( ";
                ...
                case "PROPONENTE_AVVIATI_DAL_RUOLO":
                    if (Convert.ToBoolean(f.Valore))
                    {
                        ...
                        if (string.IsNullOrEmpty(tipoVisibilita))
                        {
                            ...
                            tipoVisibilita += " AND ( ( ";
                            //tipoVisibilita += " V.CHA_TIPO_VISIBILITA IN ('P', 'T') AND ";
                            tipoVisibilita += " I.ID_RUOLO_PROPONENTE = " + IdRuoloProponente + "  ) ";
                        }
                        else
                        {
                            tipoVisibilita += " OR ( ";
    ...
    {
        logger.Information("Inizio Metodo GetProcessiDiFirmaByTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<ProcessoFirma> listProcessiDiFirma = new List<ProcessoFirma>();
        ProcessoFirma processoDiFirma = null;
        numTotPage = 0;
        nRec = 0;
        ...
        logger.Information("Fine Metodo GetProcessiDiFirmaByTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetCountProcessiDiFirmaByTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        int countProcessi = 0;
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetCountProcessiDiFirmaByTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetCountIstanzaProcessiDiFirmaByTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
        int countIstanzaProcessi = 0;
        try
        {
            string query;
        ...
        logger.Information("Fine Metodo GetCountIstanzaProcessiDiFirmaByTitolare in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    {
        logger.Information("Inizio Metodo GetProcessiFirmaByIdAmm in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<ProcessoFirma> listProcessiDiFirma = new List<ProcessoFirma>();
        ProcessoFirma processoDiFirma = null;
        try
        {
        ...
        logger.Information("Fine Metodo GetProcessiFirmaByIdAmm in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    public PassoFirma InsertPassoDiFirma(PassoFirma passo, DocsPaVO.utente.InfoUtente infoutente)
    {
        ...
        if (passo != null)
        {
            try
            {
                ...
                q.setParam("posizioneSegnatura", string.IsNullOrEmpty(passo.PosizioneSegnaturaPermanente) ? string.Empty : passo.PosizioneSegnaturaPermanente);
                q.setParam("applicaSegnatura", string.IsNullOrEmpty(passo.ApplicaSegnaturaPermanente) ? string.Empty : passo.ApplicaSegnaturaPermanente);
                    ...
                    if (!string.IsNullOrEmpty(idIstanza))
                    {
                        ...
                        if (ExecuteNonQuery(query))
                        {
                            ...
                            */
                            ...
                            List<PassoFirma> listPassiFirmaToUpdate = (from p in processoDiFirma.passi where p.DaAggiornare && p.IncludiInIstanzaPasso != IncludiInIstanzaPasso.NO select p).ToList();
                            foreach (PassoFirma passo in listPassiFirmaToUpdate)
                            {
                                ...
                                q.setParam("idTipologia", string.IsNullOrEmpty(passo.IdTipologia) ? "null" : passo.IdTipologia);
                                q.setParam("idStatoDiagramma", string.IsNullOrEmpty(passo.IdStatoDiagramma) ? "null" : passo.IdStatoDiagramma);
                                q.setParam("applicaSegnatura", string.IsNullOrEmpty(passo.ApplicaSegnaturaPermanente) ? "0" : passo.ApplicaSegnaturaPermanente);

                                if (string.IsNullOrEmpty(passo.ApplicaSegnaturaPermanente) || passo.ApplicaSegnaturaPermanente.Equals("0"))
                    ...
                    if (this.ExecuteQuery(out ds, "xmlFirma", query))
                    {
                        if (ds.Tables["xmlFirma"] != null && ds.Tables["xmlFirma"].Rows.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                var ruoloSystemId = ruolo != null ? ruolo.systemId : string.Empty;
                                var utenteSystemId = ruolo != null ? utente.systemId : string.Empty;

                                firma.GeneraXML(nominativo, descRuolo, delegato, ruoloSystemId, utenteSystemId);
                                q.setParam("xml", firma.Xml);
                            }
    ...
    public bool IsUniqueProcessName(string nomeProcesso, string idProcesso, DocsPaVO.utente.InfoUtente infoUtente)
    {
        ...
        try
        {
            ...
            q.setParam("idProcesso", !string.IsNullOrEmpty(idProcesso) ? " AND ID_PROCESSO <> " + idProcesso : string.Empty);
    ...
    public bool AggiornaStatoElementiInLibroFirma(List<ElementoInLibroFirma> elementi, string nuovoStato, DocsPaVO.utente.InfoUtente infoUtente, out string message)
    {
        ...
        string motivoRespingimento = string.Empty;
        string idUtenteLocker = nuovoStato.Equals(TipoStatoElemento.PROPOSTO.ToString()) || nuovoStato.Equals(TipoStatoElemento.IN_SOSPESO.ToString()) ? "null" : infoUtente.idPeople;
        ...
        try
        {
            ...
            if (elementi != null && rowsAffected != elementi.Count)
            {
                string idElemento;
                string oggetto;
    ...
    {
        logger.Information("Inizio Metodo GetIstanzaProcessiDiFirmaRegistriCoinvolti in DocsPaDb.Query_DocsPAWS.LibroFirma");
        List<IstanzaProcessoDiFirma> istanzeProcessoDiFirmaList = new List<IstanzaProcessoDiFirma>();
        string idRegistriSelezionati = string.Empty;
        try
        {
        ...
        logger.Information("Fine Metodo GetIstanzaProcessiDiFirmaRegistriCoinvolti in DocsPaDb.Query_DocsPAWS.LibroFirma");
    ...
    public List<IstanzaProcessoDiFirma> GetIstanzaProcessiDiFirmaByIdRegistroAndEmailRegistro(string idRegistro, string emailRegistro, DocsPaVO.utente.InfoUtente infoUtente)
    {
        ...
        logger.Information("Fine Metodo GetIstanzaProcessiDiFirmaByIdRegistroAndEmailRegistro in DocsPaDb.Query_DocsPAWS.LibroFirma");

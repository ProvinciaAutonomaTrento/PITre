...
public class Model
{
    ...
    public ArrayList DO_GetRegistriAoo(int id_amm)
    {
        ArrayList result = new ArrayList();
        ...
        {
            DocsPaDB.DBProvider dbProvider = new DocsPaDB.DBProvider();
            DocsPaUtils.Query queryMng = DocsPaUtils.InitQuery.getInstance().getQuery("PR_DO_GET_REGISTRI_BY_AMM_AOO");
            queryMng.setParam("IDAMM", id_amm.ToString());
            string commandText = queryMng.getSQL();
            logger.Debug("Prospetti Riepilogativi - PR_DO_GET_REGISTRI_BY_AMM_AOO: " + commandText);
            DataSet ds = new DataSet();
            dbProvider.ExecuteQuery(ds, commandText);
            if (ds.Tables[0].Rows.Count != 0)
            {
        ...
        {
            logger.Debug("Errore Prospetti Riepilogativi - PR_DO_GET_REGISTRI_BY_AMM_AOO: ", ex);
        }
        return result;
    }
    #endregion
    ...
    public DataSet DO_StampaExportLogExcel(string data_inizio_iop, string data_fine_iop, string periodo_ipa, bool sottoposti, string ampiezza_ipa, string numero_ampiezza_ipa, string iagg, string ruolo, string uo, string rf, string aoo, string evento, string evento_secondario, string idAmministrazione, string descrizione_evento_secondario, string tipo_ruolo)
    {
        ...
        using (DBProvider dbProvider = new DBProvider())
        {
            ...
            string iop = string.Empty;
            string dbType = dbProvider.DBType.ToUpper();
            if (dbType.Equals("SQL"))
            {
                iop = " a.DTA_AZIONE < Convert (datetime, '" + data_fine_iop + "', 103) AND a.DTA_AZIONE > Convert (datetime, '" + data_inizio_iop + "', 103)";
                q.setParam("iop", iop);
            }
            else
            {
                iop = " a.DTA_AZIONE < TO_DATE ('" + data_fine_iop + "', 'dd/mm/yyyy') AND a.DTA_AZIONE > TO_DATE ('" + data_inizio_iop + "', 'dd/mm/yyyy')";
            ...
            if (periodo_ipa.Equals("6"))
            {
                if (dbType.Equals("SQL"))
                {
                    query_periodo_iap = " and convert(VARCHAR(4), a.dta_azione,112) >= " + annoInizioIOP + " and convert(VARCHAR(4),a.dta_azione,112) <= " + annoFineIOP + "";
                }
                else
                {
            ...
            if (periodo_ipa.Equals("5"))
            {
                if (dbType.Equals("SQL"))
                {
                    query_periodo_iap = " and (SELECT DATEPART(QUARTER, a.dta_azione)) >= 1 and (SELECT DATEPART(QUARTER, a.dta_azione)) <= 4";
                }
                else
                {
            ...
            if (periodo_ipa.Equals("4"))
            {
                if (dbType.Equals("SQL"))
                {
                    query_periodo_iap = " and convert(varchar(2), a.dta_azione,110) >= 1 and convert(varchar(2), a.dta_azione,110) <= 12";
                }
                else
                {
            ...
            if (periodo_ipa.Equals("3"))
            {
                if (dbType.Equals("SQL"))
                {
                    query_periodo_iap = " and (DATEDIFF(week,0,a.dta_azione) - (DATEDIFF(week,0,DATEADD(dd, -DAY(a.dta_azione)+1, a.dta_azione))-1)) >= 1 and (DATEDIFF(week,0,a.dta_azione) - (DATEDIFF(week,0,DATEADD(dd, -DAY(a.dta_azione)+1, a.dta_azione))-1)) <= 4";
                }
                else
                {
            ...
            if (periodo_ipa.Equals("2"))
            {
                if (dbType.Equals("SQL"))
                {
                    query_periodo_iap = " and DATEPART(day, a.dta_azione) >= 1 and DATEPART(day, a.dta_azione) <= 31";
                }
                else
                {
            ...
            if (periodo_ipa.Equals("1"))
            {
                if (dbType.Equals("SQL"))
                {
                    query_periodo_iap = " and DATEPART(HOUR, a.dta_azione) >= 1 and DATEPART(HOUR, a.dta_azione) <= 24";
                }
                else
                {

...
public abstract class TransactionContext : ITransactionContext
{
    ...
    /// <summary>
    /// Se true, indica che l'instanza � proprietaria della transazione aperta
    /// </summary>
    ...
    public TransactionContext(string dbKey, IsolationLevel iso)
    {
        ...
        if (string.IsNullOrEmpty(dbKey))
            dbKey = HeaderValue.Instance.ConnectionName.Value;

        // Verifica che non sia gi� presente un contesto transazionale
        if (!HasTransactionalContext)
        {
            if (!string.IsNullOrEmpty(AppSettings.Instance.ConnectionStrings[dbKey]))
            {
                // Avvio nuova transazione
                this._transaction = this.BeginTransaction(AppSettings.Instance.ConnectionStrings[dbKey], iso);
    ...
    public TransactionContext(string dbKey, IsolationLevel iso, bool openConnection)
    {
        this._dbKey = dbKey;
        ...
        // Verifica che non sia gi� presente un contesto transazionale
        if (!HasTransactionalContext || openConnection)
        {
            if (!string.IsNullOrEmpty(AppSettings.Instance.ConnectionStrings[dbKey]))
            {
                // Avvio nuova transazione
    ...
    /// <summary>
    /// Verifica se � presente un contesto transazionale
    /// </summary>
    ...
    public void Dispose()
    {
        if (!this._disposed)
        {
            if (this._isTransactionOwner && this._transaction != null)
            {
                ...
                {
                    // Non � stato richiesto il completamento della transazione,

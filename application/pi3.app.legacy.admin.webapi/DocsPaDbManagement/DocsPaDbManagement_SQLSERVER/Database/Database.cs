...
{
        private static ILogger logger = Serilog.Log.ForContext(typeof(Database));
	...
	public bool ExecuteQuery(out DataSet dataSet, string tableName, string command)
	{
		...
		try
		{
			...
			docsPaDataAdapter.SelectCommand = docsPaCommand;
                ...
                if (DocsPaVO.Settings.AppSettings.Instance.sqlCommandTimeOut != null)
                {
                    docsPaCommand.CommandTimeout = Convert.ToInt32(DocsPaVO.Settings.AppSettings.Instance.sqlCommandTimeOut);
                }
#if TRACE_DB
			DocsPaUtils.LogsManagement.PerformaceTracer pt = new DocsPaUtils.LogsManagement.PerformaceTracer("TRACE_DB");
#endif
        ...
        public bool ExecuteQuery(out DataSet dataSet, string tableName, string command, ArrayList parameters)
        {
            ...
            try
            {
                ...
                for (int j = 0; j < parameters.Count; j++)
                {
                    ...
                    docsPaCommand.Parameters.Add(new SqlParameter() { Value = param.Valore, ParameterName = param.Nome });
                }
	...
	public int ExecuteStoreProcedure(string namestoreproc, ArrayList parametri)
	{
		...
		SqlCommand cmd=new SqlCommand();
            ...
            if (DocsPaVO.Settings.AppSettings.Instance.sqlCommandTimeOut != null)
            {
                cmd.CommandTimeout = Convert.ToInt32(DocsPaVO.Settings.AppSettings.Instance.sqlCommandTimeOut);
            }
	...
	public int ExecuteStoredProcedure(string namestoreproc, ArrayList parametri, DataSet ds)
	{
		...
		foreach(ParameterSP par in parametri )
		{
			...
			p.Value = par.Valore;
                ...
                // Se par.Size � valorizzato e maggiore di 0,
                // p.Size sar� impostato a par.Size
                if (par.Size > 0)
                    p.Size = par.Size;
                else
	...
	public IDataReader ExecuteReader (string qry)
	{
        ...
        public IDataReader ExecuteReader(string qry, ArrayList parameters)
        {
            try
            {
                ...
                {
                    param = (parameters[j] as CommandParameter);
                    cmd.Parameters.Add(new SqlParameter() { Value = param.Valore, ParameterName = param.Nome });
                }
                cmd.Transaction = transaction;

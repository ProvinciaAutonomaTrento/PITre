...
{
    private ILogger logger = Serilog.Log.ForContext(typeof(Database));
	...
	/// <summary>
	/// Questo flag � 'true' se la connessione � stata creata in questa istanza,
	/// 'false' se � stata acquisita da un'altra istanza (passata come parametro al costruttore).
	/// </summary>
	private bool isOwnConn = true; // Il valore default � 'true'

	/// <summary>
	/// Questa propriet� ritorna il messaggio dell'ultima eccezione avvenuta nell'istanza.
	/// </summary>
	...
	#region Properties
	/// <summary>
	/// Questa propriet� ritorna un flag (isOwnConn) che indica se la connessione � stata
	/// creata in questa instanza o � stata acquisita.
	/// </summary>
	...
	/// <summary>
	/// Questa propriet� ritorna 'true' se la connessione � instanziata ed aperta,
	...
	public bool ConnectionExists
	{
		get
		{
			 ...
			 * la necessit� di controllare ulteriori stati della connessione.
			 */
	...
	/// <summary>
	/// Questa propriet� ritorna 'true' se esiste una transazione attiva,
	/// altrimenti ritorna 'false'
	/// </summary>
	...
	/// <summary>
	/// Questa propriet� ritorna la connessione.
	/// </summary>
	...
	/// <summary>
	/// Questa propriet� ritorna la transazione.
	/// </summary>
	...
	public Database()
	{
		string connectionString = HeaderValue.Instance.ConnectionString.Value;
	...
	/// <summary>
	/// Inizia una transazione sulla connessione. Ci� � possibile solo se la connessione
	/// � stata creata in questa istanza (vedi propriet� IsOwnConnection).
	/// </summary>
	...
	/// <summary>
	/// Chiudi la transazione sulla connessione. Ci� � possibile solo se la connessione
	/// � stata creata in questa istanza (vedi propriet� IsOwnConnection).
	/// </summary>
	...
	/// <summary>
	/// Annulla una transazione sulla connessione. Ci� � possibile solo se la connessione
	/// � stata creata in questa istanza (vedi propriet� IsOwnConnection).
	/// </summary>
...
#endif
		var minilogger = new Logger("DB", "ExecuteQuery", command);
        minilogger.Start();
	...
	public bool ExecuteQuery(out DataSet dataSet, string tableName, string command, ArrayList parameters)
	{
		bool result = true; // Presume successo
        ...
        try
        {
			...
			{
				param = (parameters[j] as CommandParameter);
				docsPaCommand.Parameters.Add(new OracleParameter() { Value = param.Valore, ParameterName = param.Nome });
			}
			docsPaCommand.Transaction = transaction;
			docsPaDataAdapter = new DocsPaDataAdapter();
			docsPaDataAdapter.SelectCommand = docsPaCommand;
	...
	public bool ExecutePaging(out DataSet dataSet, int startRecord, int recordsReturned, string command)
	{
        ...
        var minilogger = new Logger("DB", "ExecutePaging", command);
        //Esegui il comando
        try
        {
			docsPaCommand = new DocsPaCommand(command, conn);
	...
	/// </param>
	/// <returns>
	/// Numero di record della tabella (>= 0) oppure -1 se si � verificata un'eccezione.
	/// </returns>
	public int Count(string sqlClauses)
	{
        ...
        var minilogger = new Logger("DB", "Count", command);
        // Esegui il comando
        try
        {
			OpenConnection();
		...
		finally
		{
			...
			// Non chiudere la connessione se si � all'interno di una transazione
			if(! TransactionExists)
			{
				CloseConnection();
			}
	...
	public bool ExecutePaging(out DataSet dataSet, out int totalPages, out int totalRecords, int pageId, int recordsReturned, string command, string tableName)
	{
        ...
        try
        {
			...
			//N.B: il codice seguente non � testato in diverse condizioni
			if(totalRecords > 0)
			{
	...
	{
		int rowsAffected; // Questo parametro verr� ignorato
#if TRACE_DB
		DocsPaUtils.LogsManagement.PerformaceTracer pt = new DocsPaUtils.LogsManagement.PerformaceTracer("TRACE_DB");
#endif
	...
	/// <param name="command">Comando da eseguire sul database</param>
	/// <returns>true = OK; false = Errore durnte l'esecuzione del comando</returns>
	public bool ExecuteNonQuery(string command, ArrayList parameters)
	{
		int rowsAffected; // Questo parametro verr� ignorato
#if TRACE_DB
		DocsPaUtils.LogsManagement.PerformaceTracer pt = new DocsPaUtils.LogsManagement.PerformaceTracer("TRACE_DB");
#endif
		bool result = this.ExecuteNonQuery(command, parameters, out rowsAffected);
#if TRACE_DB
		pt.WriteLogTracer(command);
#endif
		return result;
	...
	public bool ExecuteNonQuery(string command, out int rowsAffected)
	{
        ...
        var minilogger = new Logger("DB", "ExecuteNonQuery", command);
        //Esegui il comando
        try
        {
			OpenConnection();
	...
	/// <param name="rowsAffected">Numero di record aggiunti/modificati/cancellati</param>
	/// <returns>true = OK; false = Errore durnte l'esecuzione del comando</returns>
	public bool ExecuteNonQuery(string command, ArrayList parameters, out int rowsAffected)
	{
		bool result = true; // Presume successo
		rowsAffected = 0;
		DocsPaCommand docsPaCommand = null;
        ...
        try
        {
			...
			{
				param = (parameters[j] as CommandParameter);
				docsPaCommand.Parameters.Add(new OracleParameter() { Value = param.Valore, ParameterName = param.Nome, DbType = param.Tipo });
			}
	...
	{
		bool result = true; // Presume successo
		bool ownTransaction = false; // Presume che la transazione � gi� esistente
	...
	{
		// TODO: Questo metodo � relativo ad una sola tabella.
	...
	public int ExecuteStoreProcedure(string namestoreproc, ArrayList parametri)
	{
        ...
        OracleParameter parRetValue = new OracleParameter();
		parRetValue.ParameterName = "returnvalue";
        parRetValue.DbType = DbType.Int32;
        parRetValue.Direction=ParameterDirection.Output;
		cmd.Parameters.Add(parRetValue);

        var minilogger = new Logger("DB", "ExecuteStoreProcedure", namestoreproc);
        try
        {
			this.OpenConnection();
			cmd.Connection=conn;
	...
	public int ExecuteStoredProcedure(string namestoreproc, ArrayList parametri, DataSet ds)
	{
		...
		// Visto che Oracle � molto pignolo nella definizione dei parametri
		// lasciamo che sia lui a scegliere quelli che gli piacciono di pi�...
		...
		// Se e solo se il dataset passato � diverso da NULL,
		...
		// Se il dataset passato � nullo non ci sono dati da restituire
	...
	public string GetLargeText(string tableName, string systemId, string columnName)
	{
		string val = null;
		bool connOpen = false;
        var minilogger = new Logger("DB", "GetLargeText", tableName + " " + columnName);
        try
        {
			...
			var sqlCommandOracle =
				new DocsPaCommand(qry,conn);
			...
			rstOracle.Read();
			var clob = rstOracle.GetOracleBlob(0);
            minilogger.End();
	...
	{
        var minilogger = new Logger("DB", "setLargeText", tableName + " " + columnName);
        try
        {
			string qry = "select " + columnName + " " +
				"from " + tableName + " " +
	...
	public string GetSysdate()
	{
		string val = null;
		bool connOpen = false;
        var minilogger = new Logger("DB", "GetSysdate", "");
        try
        {
	...
	public IDataReader ExecuteReader (string qry)
	{
        ...
        var minilogger = new Logger("DB", "ExecuteReader", qry);
        try
        {
			this.OpenConnection();
	...
	public IDataReader ExecuteReader(string qry, ArrayList parameters)
	{
		DocsPaCommand cmd = null;
        var minilogger = new Logger("DB", "ExecuteReader", qry);
        try
        {
			...
			{
				param = (parameters[j] as CommandParameter);
				cmd.Parameters.Add(new OracleParameter() { Value = param.Valore, ParameterName = param.Nome });
			}
	...
	/// <summary>
	/// Chiude la connessione se non siamo all'interno di una transazione. Ci� � possibile solo se la
	/// connessione � stata creata in questa istanza (vedi propriet� IsOwnConnection).
	/// </summary>
	/// <returns>true = OK; false = Connessione gi� chiusa o errore durante la chiusura della connessione</returns>
	...
	/// <summary>
	/// Acquisisce una transazione/connessione gi� instanziata. In questa istanza viene inibita la possibilit�
	/// di chiudere la transazione/connessione (da utilizzare quando la transazione � gi� stata
	/// aperta dall'oggetto che passa la connessione).
	/// </summary>
	/// <param name="oracleTransaction">Transazione istanziata</param>
	/// <returns>true = OK; false = Una transazione/connessione � gi� stata acquisita o errore durante
	/// l'acquisizione della connessione</returns>
	protected bool AcquireTransaction(DocsPaTransaction oracleTransaction)
	{
		bool result = true; // Presume successo

		/* Non si sta gi� usando una connessione acquisit� quindi si pu�
	...
	/// <summary>
	/// Questo metodo � utilizzato per deallocare oggetti.
	/// </summary>
	public void Dispose()
	{
		if (!TransactionContext.HasTransactionalContext)

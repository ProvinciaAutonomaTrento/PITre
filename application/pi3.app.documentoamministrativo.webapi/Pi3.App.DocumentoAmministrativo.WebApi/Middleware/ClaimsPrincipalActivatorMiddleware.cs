...
namespace Pi3.App.DocumentoAmministrativo.WebApi.Middleware
{
    public class ClaimsPrincipalActivatorMiddleware
    {
            ...
            ILogger<ClaimsPrincipalActivatorMiddleware> logger)
        {
            ...
            var codiceAmministrazione = context.Request.Headers["tenant"].ToString();
            var userId = context.Request.Headers["userId"].ToString();
            var groupCode = context.Request.Headers["groupCode"].ToString();

            var claimsPrincipalService = context.RequestServices.GetService<IClaimsPrincipalService>();

            ((ClaimsPrincipalService)claimsPrincipalService!).Current = context.User;

            context.User.AddIdentity(new ClaimsIdentity(
                authenticationType: "Pi3Authentication"));

            var claimsIdentity = context.User.Identities.First(i => i.AuthenticationType == "Pi3Authentication");

            var instance = context.Request.RouteValues["instance"];
            ...
            var userContext = await
                         (from p in dbContext.PeopleEntities.AsNoTracking()
                          ...
                          where
                             ...
                             && p.USER_ID.ToUpper() == userId.ToUpper()
            ...
            claimsIdentity.AddClaim(new Claim(Pi3ClaimTypes.Instance, context.GetRouteData().Values["instance"].ToString()));

            claimsIdentity.AddClaims(await (from tfr in dbContext.TipoFRuoloEntities.AsNoTracking()

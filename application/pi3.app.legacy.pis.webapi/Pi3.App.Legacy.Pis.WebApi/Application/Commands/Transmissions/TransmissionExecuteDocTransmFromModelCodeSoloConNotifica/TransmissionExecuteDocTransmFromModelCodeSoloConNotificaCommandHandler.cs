...
{
    public class TransmissionExecuteDocTransmFromModelCodeSoloConNotificaCommandHandler : IRequestHandler<TransmissionExecuteDocTransmFromModelCodeSoloConNotificaCommand, TransmissionExecuteDocTransmFromModelCodeSoloConNotificaCommandResponse>
    {
        public TransmissionExecuteDocTransmFromModelCodeSoloConNotificaCommandHandler(
            IMediator mediator,
            ILogger<TransmissionExecuteDocTransmFromModelCodeSoloConNotificaCommandHandler> logger,
        ...
        public async Task<TransmissionExecuteDocTransmFromModelCodeSoloConNotificaCommandResponse> Handle(TransmissionExecuteDocTransmFromModelCodeSoloConNotificaCommand request, CancellationToken cancellationToken)
        {
            bool output = false;

            string modelId = this.GetModelId(request.ModelCode);
            var modelTrasm = (await this._mediator.Send(new GetModelloByIDSoloConNotificaCommand()
            {
                IdAmm = request.Role.idAmministrazione,
                IdModello = modelId
            })).Output;

            if (modelTrasm != null && modelTrasm.CHA_TIPO_OGGETTO == "D")
            {
                output = await this.EseguiTrasmissioneDaModello(modelTrasm, request.Documento.docNumber, request.InfoUtente);
            }
        ...
        protected readonly ILogger<TransmissionExecuteDocTransmFromModelCodeSoloConNotificaCommandHandler> _logger;
        ...
        protected async Task<bool> EseguiTrasmissioneDaModello(ModelloTrasmissione modello, string docnumber, InfoUtente infoUtente)
        {
            bool output = false;
            try
            {

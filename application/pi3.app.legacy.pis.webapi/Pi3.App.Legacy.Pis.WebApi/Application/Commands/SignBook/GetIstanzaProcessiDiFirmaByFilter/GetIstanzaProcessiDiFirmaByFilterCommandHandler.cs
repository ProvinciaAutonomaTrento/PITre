...
using Pi3.App.Legacy.Pis.WebApi.Application.Commands.Utils.Date.GetFirstDayOfMonth;
using Pi3.App.Legacy.Pis.WebApi.Application.Commands.Utils.Date.GetFirstDayOfWeek;
using Pi3.App.Legacy.Pis.WebApi.Application.Commands.Utils.Date.GetLastDayOfMonth;
using Pi3.App.Legacy.Pis.WebApi.Application.Commands.Utils.Date.GetLastDayOfWeek;
using Pi3.App.Legacy.Pis.WebApi.Application.Commands.Utils.Date.GetLastSevenDay;
using Pi3.App.Legacy.Pis.WebApi.Application.Commands.Utils.Date.GetLastThirtyOneDay;
using Pi3.App.Legacy.Pis.WebApi.Application.Commands.Utils.Date.GetYesterday;
...
{
    public class GetIstanzaProcessiDiFirmaByFilterCommandHandler : IRequestHandler<GetIstanzaProcessiDiFirmaByFilterCommand, GetIstanzaProcessiDiFirmaByFilterCommandResponse>
    {
        public GetIstanzaProcessiDiFirmaByFilterCommandHandler(ILogger<GetIstanzaProcessiDiFirmaByFilterCommandHandler> logger,
        ...
        public async Task<GetIstanzaProcessiDiFirmaByFilterCommandResponse> Handle(GetIstanzaProcessiDiFirmaByFilterCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var firstDayOfWeek = (await this._mediator.Send(new GetFirstDayOfWeekCommand())).output.AsDateTime().Date;
                var lastDayOfWeek = (await this._mediator.Send(new GetLastDayOfWeekCommand())).output.AsDateTime().Date;
                var firstDayOfMonth = (await this._mediator.Send(new GetFirstDayOfMonthCommand())).output.AsDateTime().Date;
                var lastDayOfMonth = (await this._mediator.Send(new GetLastDayOfMonthCommand())).output.AsDateTime().Date;
                var yesterday = (await this._mediator.Send(new GetYesterdayCommand())).output.AsDateTime().Date;
                var lastSevenDay = (await this._mediator.Send(new GetLastSevenDayCommand())).output.AsDateTime().Date;
                var lastThirtyOneDay = (await this._mediator.Send(new GetLastThirtyOneDayCommand())).output.AsDateTime().Date;
            ...
            return new()
            {
                ...
                nRec = nRec,
                istanzeProcessi = istanzeProcessi
        ...
        protected readonly ILogger<GetIstanzaProcessiDiFirmaByFilterCommandHandler> _logger;
        ...
        protected virtual void InitializeMapper()
        {
            var configuration = new MapperConfiguration(cfg =>
            {
                ...
                cfg.CreateMap<IstanzaPassoFirmaEventoEntity, IstanzaPassoDiFirma>()
                    ...
                    .ForMember(dest => dest.RuoloCoinvolto, opt => opt.MapFrom(src => 
                    new DocsPaVO.utente.Ruolo()
                    {
                        idGruppo = src.IstanzaPassoFirma.ID_RUOLO_COINVOLTO != null ? src.IstanzaPassoFirma.ID_RUOLO_COINVOLTO.ToString() : string.Empty
                    })).ForMember(dest => dest.UtenteCoinvolto, opt => opt.MapFrom(src =>
                    new DocsPaVO.utente.Utente()
                    {
                        idPeople = src.IstanzaPassoFirma.ID_UTENTE_COINVOLTO != null ? src.IstanzaPassoFirma.ID_UTENTE_COINVOLTO.ToString() : string.Empty
                    }));

...
{
    public class EseguiPassoAutomaticoCommandHandler : IRequestHandler<EseguiPassoAutomaticoCommand, EseguiPassoAutomaticoCommandResponse>
    {
        #region Public Members

        public EseguiPassoAutomaticoCommandHandler(ILogger<EseguiPassoAutomaticoCommandHandler> logger,
        ...
        public async Task<EseguiPassoAutomaticoCommandResponse> Handle(EseguiPassoAutomaticoCommand request, CancellationToken cancellationToken)
        {
            var esito = new Esito();
            ...
            try
            {
                ...
                var idIstanzaProcesso = request.idIstanzaProcessoFirma;
            ...
            return new EseguiPassoAutomaticoCommandResponse()
            {
        ...
        protected readonly ILogger<EseguiPassoAutomaticoCommandHandler> _logger;
        ...
        protected async Task<Esito> Protocolla(string docnumber, bool applicaSegnaturaPermanente, string posizioneSegnaturaPermanente, InfoUtente infoUtente)
        {
            var esito = new Esito()
            {
                result = true,
            ...
            try
            {
                ...
                if (esito.result)
                {
                    ...
                    {
                        DatiRegistro = new DatiRegistro() { IdRegistro = idRegistro.ToString() }
                    });
                    ...
                    if (documentoAmministrativoAggregate.TipologiaFlusso == TipologiaFlussoEnum.U)
                    {
                        ...
                        {
                            SchedaDocumento? schedaDocumento = (await this._mediator.Send(new DocumentoGetDettaglioDocumentoNoSecurityCommand()
                            {
                            ...
                            if (!(schedaDocumento.documenti[0] as FileRequest).conSegnaturaPermanente)
                            {
                                ...
                                var fileDocumento = (await _mediator.Send(new DocumentoGetFileConSegnaturaCommand()
                                {
                                ...
                                await _mediator.Send(new RemotePdfSignStampCommand()
                                {
                                   labelPdf = fileDocumento.LabelPdf,
                                   infoUtente = infoUtente,
                                   fr = (FileRequest)schedaDocumento.documenti[0],
        ...
        protected async Task<Esito> CambioStato(string docnumber, string idStatoDiagramma, string idTipologia, InfoUtente infoUtente)
        {
            ...
            try
            {
                ...
                var diagrammaEntity = await _dbContext.DiagrammiEntities.Where(d => d.ID_STATO == idStatoDiagramma.AsLong()).FirstOrDefaultAsync();
                ...
                if (esito.result)
                {
                    DiagrammaStato diagramma = (await _mediator.Send(new GetDiagrammaByIdCommand()
                    {
                        IdDiagramma = statoEntity.ID_DIAGRAMMA.ToString()
                    })).Output;

                    await SalvaModificaStato(docnumber.AsLong(), idStatoDiagramma.AsLong(), diagramma, infoUtente);
                    ...
                    var modelli = await IsStatoTrasmAuto(statoEntity.SYSTEM_ID, idTipologia.AsLong(), idTenant.AsLong());
                    ...
                    if (statoEntity.STATO_FINALE == 1)
                    {
                        ...
                        List<SecurityEntity> entitiesToUpdate = await _dbContext.SecurityEntities.Where(x => x.THING == docnumber.AsLong() &&
                            x.ACCESSRIGHTS != 0 &&
                            !_dbContext.SecurityEntities.Any(s => s.THING == x.THING && s.ACCESSRIGHTS == accessRight && s.PERSONORGROUP == x.PERSONORGROUP))
                         .ToListAsync();
                        ...
                        entitiesToUpdate.ForEach(e =>
                        {
                            ...
                            _dbContext.SecurityEntities.Remove(e);
                            _dbContext.SecurityEntities.Add(entityToAdd);
                        });

                        await ((DbContext)_dbContext).SaveChangesAsync();

                        List<int> rights = new List<int> { 0, accessRight };
                        SecurityEntity entityToDelete = await _dbContext.SecurityEntities.Where(x => x.THING == docnumber.AsLong() &&
                            (x.ACCESSRIGHTS != 0 || x.ACCESSRIGHTS != accessRight))
                        .FirstOrDefaultAsync();

                        if (entityToDelete != null)
                        {
                            _dbContext.SecurityEntities.Remove(entityToDelete);
            ...
            long idAmm)
        {
            ...
            try
            {
                var assDiagrammiEntities = await _dbContext.AssDiagrammiEntities.Where(a => a.ID_STATO == idStato && a.ID_TIPO_DOC == idTemplate).Select(a => new { a.ID_MOD_TRASM, a.TRASM_AUT }).ToListAsync();
                if (assDiagrammiEntities != null && assDiagrammiEntities.Count > 0)
                {
                    ...
                    foreach (var a in assDiagrammiEntities)
                    {
                        if (a.TRASM_AUT == 1)
                        {
                            var modelloTrasmEntities = await _dbContext.ModelloTrasmEntities
                            ...
                            {
                                var modelloMittEntities_OLD = await _dbContext.ModelloMittDestEntities
                                    .Join(_dbContext.CorrGlobaliEntities, modelloMittDest => modelloMittDest.ID_CORR_GLOBALI, corrGlobali => corrGlobali.SYSTEM_ID, (modelloMittDest, corrGlobali) => new { modelloMittDest, corrGlobali.VAR_COD_RUBRICA, corrGlobali.VAR_DESC_CORR })
                                    .Where(m => m.modelloMittDest.ID_MODELLO == a.ID_MOD_TRASM && m.modelloMittDest.CHA_TIPO_MITT_DEST == "M")
                                    .ToListAsync();

                                var modelloMittEntities = await _dbContext.ModelloMittDestEntities
                                ...
                                foreach (var m in modelloMittEntities)
                                {
                                    ...
                                    mittDest.VAR_COD_RUBRICA = await _dbContext.CorrGlobaliEntities.Where(c => c.SYSTEM_ID == m.SYSTEM_ID).Select(c => c.VAR_COD_RUBRICA).FirstOrDefaultAsync();
                                    mittDest.DESCRIZIONE = await _dbContext.CorrGlobaliEntities.Where(c => c.SYSTEM_ID == m.SYSTEM_ID).Select(c => c.VAR_DESC_CORR).FirstOrDefaultAsync();
                            ...
                            var modelloDestEntities = await _dbContext.ModelloMittDestEntities
                                .Join(_dbContext.RagioneTrasmissioneEntities, dest => dest.ID_RAGIONE, ragione => ragione.SYSTEM_ID, (dest, ragione) => new { dest, ragione })
                                .Join(_dbContext.CorrGlobaliEntities, dest => dest.dest.ID_CORR_GLOBALI, corr => corr.SYSTEM_ID, (dest, corr) => new
                                {
                                    dest.dest,
                            ...
                            foreach (var d in modelloDestEntities)
                            {
                                ...
                                {
                                    var utentiEntities = await _dbContext.PeopleEntities
                                        .Join(_dbContext.PeopleGroupEntities, people => people.SYSTEM_ID, peopleGroup => peopleGroup.PEOPLE_SYSTEM_ID, (people, peoplegroups) => new { people, peoplegroups.DTA_FINE, peoplegroups.GROUPS_SYSTEM_ID })
                                        .Join(_dbContext.CorrGlobaliEntities, people => people.GROUPS_SYSTEM_ID, corr => corr.ID_GRUPPO, (people, corr) => new { people, corr.VAR_DESC_CORR, ID_CORR_GLOBALI = corr.SYSTEM_ID })
                                    ...
                                    utentiEntities.ForEach(u =>
                                        utentiNotificaList.Add(new DocsPaVO.Modelli_Trasmissioni.UtentiConNotificaTrasm()
                                        {
                                            ...
                                            FLAG_NOTIFICA = _dbContext.ModelloDestConNotificaEntities.Any(n => n.ID_MODELLO == dest.ID_MODELLO && n.ID_PEOPLE == u.people.people.SYSTEM_ID) ? "1" : "0"
                                        }));
        ...
        protected async Task SalvaModificaStato(long docnumber,
           ...
           InfoUtente user)
        {
            var idPeople = _claimsPrincipalService.Current.GetPi3ClaimValue<long>(Pi3ClaimTypes.IdUser, true);
            var idGroup = _claimsPrincipalService.Current.GetPi3ClaimValue<long>(Pi3ClaimTypes.IdGroup, true);
            var idCorrGlobali = await _dbContext.CorrGlobaliEntities.AsNoTracking().Where(x => x.ID_GRUPPO == idGroup).Select(x => x.SYSTEM_ID).FirstAsync();

            StatoEntity? statoEntity = null;

            try
            {
                var diagrammiEntity = await _dbContext.DiagrammiEntities
                    .FirstOrDefaultAsync(x => x.DOC_NUMBER == docnumber);

                // statoOld
                string? currentState = default;

                statoEntity = await _dbContext.StatoEntities.AsNoTracking().FirstOrDefaultAsync(x => x.SYSTEM_ID == idStato);

                if (diagrammiEntity != null)
                {
                    currentState = await _dbContext.StatoEntities
                        .Where(x => x.SYSTEM_ID == diagrammiEntity.ID_STATO)
                        .Select(x => x.VAR_DESCRIZIONE)
                        .FirstAsync();

                    var trasmDiagrEntity = await _dbContext.TrasmDiagrEntities.FirstOrDefaultAsync(x => x.ID_STATO == diagrammiEntity.ID_STATO && x.DOC_NUMBER == diagrammiEntity.DOC_NUMBER);

                    if (trasmDiagrEntity != null)
                    {
                        _dbContext.TrasmDiagrEntities.Remove(trasmDiagrEntity);
                    }
                ...
                else
                {
                    ...
                    await _dbContext.DiagrammiEntities.AddAsync(diagrammiEntity);

                    //Se stato iniziale serve impostare la data di scadenza
                    if (statoEntity.STATO_INIZIALE == 1)
                    {
                        var idTipoDoc = await _dbContext.ProfileEntities.AsNoTracking()
                        ...
                        var aggregate = await _documentoAmministrativoRepository.Get(idTenant, docnumber.ToString(), new ILoadBehavior[1]
                        {
                            new GetDocumentoAmministrativoLoadBehavior()
                            {
                                LoadProfiles = false,
                        ...
                        var scadenza = await _dbContext.TipoAttoEntities.AsNoTracking().Where(t => t.SYSTEM_ID == idTipoDoc).Select(t => t.GG_SCADENZA).FirstOrDefaultAsync();
                        if (scadenza != null && scadenza != 0)
                            dtaScadenza = (await _dbContext.GetSystemDateTime()).AddDays((double)scadenza).Date;
                ...
                await _dbContext.DiagrammiStoEntities.AddAsync(diagrammiStoEntity);

                // Libro firma
                var processoEntity = await _dbContext.SchemaProcessoFirmaEntities.AsNoTracking()
                ...
                {
                    var processo = await SignBookUtils.GetProcessoFirma(statoEntity.ID_PROCESSO_FIRMA.Value, processoEntity, _dbContext);
                    var fileRequest = await DBUtils.GetVersionsMainDocument(user,
                       ...
                       docnumber,
                       _dbContext);
                    var avvioProcesso = await _mediator.Send(new AvvioProcessoDiFirmaCommand()
                    {
                ...
                // Consolidamento
                var key = await _configurationService.GetValue<string>("BE_CONSOLIDAMENTO");
                if (key == "1")
                {
                    if (!string.IsNullOrWhiteSpace(statoEntity.STATO_CONSOLIDAMENTO) && statoEntity.STATO_CONSOLIDAMENTO != "0")
                    {
                        var profileEntity = await _dbContext.ProfileEntities.FirstAsync(x => x.SYSTEM_ID == docnumber);
                        ...
                        {
                            await _mediator.Send(new Application.Requests.ConsolidateDocumentById(
                                request.user,
                                request.docNumber,
                ...
                await _webMethodLoggerService.LogOK("DOC_CAMBIO_STATO", docnumber.ToString(),
            ...
            catch (Pi3Exception pi3Ex)
            {
                ...
                await _webMethodLoggerService.LogKO("DOC_CAMBIO_STATO", docnumber.ToString(), string.Format(Resources.LogCambioStato, statoEntity.VAR_DESCRIZIONE));
            }
            catch (Exception ex)
            {

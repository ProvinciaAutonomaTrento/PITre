...
namespace Pi3.App.Legacy.Pis.WebApi.Application.Commands.Projects.FollowProject
{
    ...
    public class FollowProjectCommandHandler : IRequestHandler<FollowProjectCommand, FollowProjectCommandResponse>
    {
        #region Public Members

        public FollowProjectCommandHandler(ILogger<FollowProjectCommandHandler> logger, IClaimsPrincipalService claimsPrincipalService, IMediator mediator, IHttpContextAccessor httpContextAccessor, IPi3DbContext pi3DbContext)
        {
            this._logger = logger;
        ...
        public async Task<FollowProjectCommandResponse> Handle(FollowProjectCommand request, CancellationToken cancellationToken)
        {

            _logger.LogInformation("FollowProject - START");

            FollowProjectCommandResponse response = new FollowProjectCommandResponse();
            try
            {
                ...
                _logger.LogInformation("end FollowProject");


            }
            catch (RestException pisEx)
            {
                _logger.LogError("Eccezione FollowProject: {errorCode}, {errorDescription}", pisEx.ErrorCode, pisEx.Description);
                response = new FollowProjectCommandResponse();
                response.Code = Documents.MessageResponseCode.SYSTEM_ERROR;
                response.ErrorMessage = pisEx.Description;
            }
            catch (Exception e)
            {
                _logger.LogCritical(e, "eccezione FollowProject");
                response = new FollowProjectCommandResponse();
                response.Code = Documents.MessageResponseCode.SYSTEM_ERROR;
        ...
        protected readonly ILogger<FollowProjectCommandHandler> _logger;

...
{
    public class FascicolazioneGetListaFascicoliDaCodiceCommandHandler : IRequestHandler<FascicolazioneGetListaFascicoliDaCodiceCommand, FascicolazioneGetListaFascicoliDaCodiceCommandResponse>
    {
        public FascicolazioneGetListaFascicoliDaCodiceCommandHandler(
            ILogger<FascicolazioneGetListaFascicoliDaCodiceCommandHandler> logger, IClaimsPrincipalService claimsPrincipalService, IMediator mediator, IPi3DbContext dbContext
        )
        {
            this._logger = logger;
        ...
        public async Task<FascicolazioneGetListaFascicoliDaCodiceCommandResponse> Handle(FascicolazioneGetListaFascicoliDaCodiceCommand request, CancellationToken cancellationToken)
        {
            List<Fascicolo> result = new List<Fascicolo>();
            ...
            try
            {
                List<long> idTitolarioListD = this._dbContext.ProjectEntities.Where(x => x.ID_TITOLARIO == 0 && !x.CHA_STATO.Equals("D")).Select(x => x.SYSTEM_ID).ToList();
                List<long> idTitolarioListA = this._dbContext.ProjectEntities.Where(x => x.ID_TITOLARIO == 0 && x.CHA_STATO.Equals("A")).Select(x => x.SYSTEM_ID).ToList();
                ...
                if (string.IsNullOrEmpty(insRic) || insRic.Equals("R"))
                    projectEntities = projectEntities.Where(x => x.CHA_TIPO_PROJ.Equals("F"));
                else if (insRic.Equals("I"))
                    projectEntities = projectEntities
                        .Where(x => (x.CHA_TIPO_FASCICOLO.Equals("P") && x.CHA_STATO.Equals("A")) ||
                        (x.CHA_TIPO_FASCICOLO.Equals("G") && idTitolarioListA.Contains(x.ID_TITOLARIO ?? 0)));

                if (!string.IsNullOrEmpty(codiceFascicolo))
                {
                    projectEntities = projectEntities.Where(x => idTitolarioListD.Contains(x.ID_TITOLARIO ?? 0) && x.VAR_CODICE.ToUpper().Equals(codiceFascicolo.ToUpper()));
                }

                if (registro != null)
                    projectEntities = projectEntities.Where(x => x.ID_REGISTRO == null || x.ID_REGISTRO == registro.systemId.AsLong());

                var fList = await projectEntities.Select(x => new
                {
                ...
                {
                    var rights = _dbContext.GetSecurityRights(f.SYSTEM_ID.ToString(), idUser, idGroup).Result;
                    ...
                    fascicolo = new DocsPaVO.fascicolazione.Fascicolo()
                    {
                        ...
                        dtaScadenza = f.DTA_SCADENZA.ToString(),
                        numFascicolo = f.NUM_FASCICOLO?.ToString()
                    };

                    fascicolo.codiceRegistroNodoTit = await GetCodReg(f.ID_REGISTRO);
                    fascicolo.isFascConsentita = await GetChaConsentiClass(f.ID_PARENT, f.CHA_TIPO_PROJ, f.ID_FASCICOLO);
                    fascicolo.isFascicolazioneConsentita = await GetChaConsentiFasc(f.ID_PARENT, f.CHA_TIPO_PROJ, f.CHA_TIPO_FASCICOLO);
                    fascicolo.InAreaLavoro = await GetInAdl(f.SYSTEM_ID, idGroupAsLong, idUserAsLong);

                    if (enableProfilazione)
                    {
                        var template = await this._mediator.Send(new GetTemplateFascDettagliCommand()
                        {
                            IdProject = f.SYSTEM_ID.ToString()
                        });
                        fascicolo.template = template.Output ?? null;
                    }
                    ...
                    fascicolo.noteFascicolo = FetchNoteFascicolo(idUserAsLong, idGroupAsLong, idCorrGlobaliRuolo, fascicolo);
                    ...
                    {
                        fascicolo.pianoConservazione = await GetPianoConservazioneById(f.ID_PIANO_CONSERVAZIONE);
                    }

                    result.Add(fascicolo);
                });
            ...
            return new FascicolazioneGetListaFascicoliDaCodiceCommandResponse()
            {
        ...
        protected readonly ILogger<FascicolazioneGetListaFascicoliDaCodiceCommandHandler> _logger;

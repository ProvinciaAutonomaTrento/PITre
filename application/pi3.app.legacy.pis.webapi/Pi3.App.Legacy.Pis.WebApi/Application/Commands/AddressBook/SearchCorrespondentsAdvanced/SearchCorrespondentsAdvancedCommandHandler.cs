...
namespace Pi3.App.Legacy.Pis.WebApi.Application.Commands.AddressBook.SearchCorrespondentsAdvanced
{
    ...
    public class SearchCorrespondentsAdvancedCommandHandler : IRequestHandler<SearchCorrespondentsAdvancedCommand, SearchCorrespondentsAdvancedCommandResponse>
    {
        #region Public Members

        public SearchCorrespondentsAdvancedCommandHandler(ILogger<SearchCorrespondentsAdvancedCommandHandler> logger, IClaimsPrincipalService claimsPrincipalService, IMediator mediator, IHttpContextAccessor httpContextAccessor, IPi3DbContext pi3DbContext)
        {
            this._logger = logger;
        ...
        public async Task<SearchCorrespondentsAdvancedCommandResponse> Handle(SearchCorrespondentsAdvancedCommand request, CancellationToken cancellationToken)
        {

            _logger.LogInformation("SearchCorrespondentsAdvanced - START");

            SearchCorrespondentsAdvancedCommandResponse response = new SearchCorrespondentsAdvancedCommandResponse();
            try
            {
                ...
                if (corrs != null && corrs.Count() > 0)
                {
                    ...
                    response.Correspondents = new CorrespondentAdvanced[corrs.Count()];
                    for (int i = 0; i < corrs.Count(); i++)
                    {
                        corr = corrs[i];
                        var corrTemp = new CorrespondentAdvanced()
                        {
                            Id = corr.systemId,
                        ...
                        if (idFromRC && string.IsNullOrEmpty(corr.systemId))
                        {
                            ...
                            if (extractDetails && !string.IsNullOrWhiteSpace(corrTemp.Id))
                            {
                                ...
                                {
                                    corrTemp = RestUtils.GetCorrAdvanced(this._pi3DbContext, dettResp.output, infoUtente);
                                }
                            }
                        }
                ...
                {
                    response.Correspondents = new CorrespondentAdvanced[0];
                }

                #endregion

                response.Code = SearchCorrAdvancedResponseCode.OK;

                _logger.LogInformation("end SearchCorrespondentsAdvanced");


            }
            catch (RestException pisEx)
            {
                _logger.LogError("Eccezione SearchCorrespondentsAdvanced: {errorCode}, {errorDescription}", pisEx.ErrorCode, pisEx.Description);
                response = new SearchCorrespondentsAdvancedCommandResponse();
                response.Code = SearchCorrAdvancedResponseCode.SYSTEM_ERROR;
                response.ErrorMessage = pisEx.Description;
            }
            catch (Exception e)
            {
                _logger.LogCritical(e, "eccezione SearchCorrespondentsAdvanced");
                response = new SearchCorrespondentsAdvancedCommandResponse();
                response.Code = SearchCorrAdvancedResponseCode.SYSTEM_ERROR;
        ...
        protected readonly ILogger<SearchCorrespondentsAdvancedCommandHandler> _logger;

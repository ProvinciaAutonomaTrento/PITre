...
{
    public class CorrispondentiDeleteModifyCorrispondenteEsternoCommandHandler : IRequestHandler<CorrispondentiDeleteModifyCorrispondenteEsternoCommand, CorrispondentiDeleteModifyCorrispondenteEsternoCommandResponse>
    {
        public CorrispondentiDeleteModifyCorrispondenteEsternoCommandHandler(
            ILogger<CorrispondentiDeleteModifyCorrispondenteEsternoCommandHandler> logger,
        ...
        public async Task<CorrispondentiDeleteModifyCorrispondenteEsternoCommandResponse> Handle(CorrispondentiDeleteModifyCorrispondenteEsternoCommand request, CancellationToken cancellationToken)
        {
            bool result = false;
            ...
            {
                this._logger.LogError(ex,Resources.LgErrorMessage);
                await this._webMethodLoggerService.LogKO("CORRISPONDENTIDELETECORRISPONDENTEESTERNO", request.DatiModificaCorr.idCorrGlobali, descAzione);
            }
            return new()
            {
        ...
        protected readonly ILogger<CorrispondentiDeleteModifyCorrispondenteEsternoCommandHandler> _logger;
        ...
        private async Task<long> SpDeleteCorrEsterno(string idCorrglobali, int flagListe, DocsPaVO.utente.InfoUtente user)
        {
            ...
            if (countDoc == 0 && countProfileSto == 0)
            {
                ...
                if (countProfilato > 0)
                {
                    ...
                    {
                        // cursor doc
                        await (from at in this._dbContext.AssociazioneTemplatesEntities
                               from oc in this._dbContext.OggettiCustomEntities
                               from dto in this._dbContext.TipoOggettoEntities
                               where (at.VALORE_OGGETTO_DB != null && at.VALORE_OGGETTO_DB.Equals(idCorrglobali)) &&
                               (at.ID_OGGETTO == oc.SYSTEM_ID) &&
                               (oc.ID_TIPO_OGGETTO == dto.SYSTEM_ID) &&
                               (dto.DESCRIZIONE != null && dto.DESCRIZIONE.ToUpper().Equals("CORRISPONDENTE"))
                               select new
                               {
                                   at.DOC_NUMBER,
                                   at.ID_TEMPLATE,
                    ...
                    try
                    {
                        if (countProfFasc > 0)
                        {
                            await (from atf in this._dbContext.AssTemplatesFascEntities
                                   from ocf in this._dbContext.OggettiCustomFascEntities
                                   from tof in this._dbContext.TipoOggettoFascEntities
                                   where (atf.VALORE_OGGETTO_DB != null && atf.VALORE_OGGETTO_DB.Equals(idCorrglobali)) &&
                                   (atf.ID_OGGETTO == ocf.SYSTEM_ID) &&
                                   (ocf.ID_TIPO_OGGETTO == tof.SYSTEM_ID) &&
                                   (tof.DESCRIZIONE != null && tof.DESCRIZIONE.ToUpper().Equals("CORRISPONDENTE"))
                                   select new
                                   {
                                       atf.ID_PROJECT,
                                       atf.ID_TEMPLATE,
                                       atf.ID_OGGETTO
                                   }).ForEachAsync(async fasc =>
                                   {
            ...
            )
        {
            ...
            try
            {
                    ...
                    */
                    ...
                    if (IdRegistro != null)
                    {
                        corrToInsert = new()
                        {
                            ...
                            DTA_INIZIO = (await this._dbContext.GetSystemDateTime()).AsDateFormat().AsDateTime(),
                            ID_PARENT = idParent,
                    ...
                    if (countProfDoc > 0)
                    {
                        await (from at in this._dbContext.AssociazioneTemplatesEntities.AsNoTracking()
                               from oc in this._dbContext.OggettiCustomEntities.AsNoTracking()
                               from dto in this._dbContext.TipoOggettoEntities.AsNoTracking()
                               where (at.VALORE_OGGETTO_DB != null && at.VALORE_OGGETTO_DB.Equals(idcorrglobale.ToString())) &&
                               (at.ID_OGGETTO == oc.SYSTEM_ID) &&
                               (oc.ID_TIPO_OGGETTO == dto.SYSTEM_ID) &&
                               (dto.DESCRIZIONE != null && dto.DESCRIZIONE.ToUpper().Equals("CORRISPONDENTE"))
                               select new
                               ...
                               }).ForEachAsync(async c =>
                               {
                                   ProfilStoEntity proSto = new ProfilStoEntity()
                                   {
                                       ...
                                       DTA_MODIFICA = (await this._dbContext.GetSystemDateTime()).AsDateFormat().AsDateTime(),
                    ...
                    if (countProfFasc > 0)
                    {
                        await (from atf in this._dbContext.AssTemplatesFascEntities.AsNoTracking()
                               from ocf in this._dbContext.OggettiCustomFascEntities.AsNoTracking()
                               from tof in this._dbContext.TipoOggettoFascEntities.AsNoTracking()
                               where (atf.VALORE_OGGETTO_DB != null && atf.VALORE_OGGETTO_DB.Equals(idcorrglobale.ToString())) &&
                               (atf.ID_OGGETTO == ocf.SYSTEM_ID) &&
                               (ocf.ID_TIPO_OGGETTO == tof.SYSTEM_ID) &&
                               (tof.DESCRIZIONE != null && tof.DESCRIZIONE.ToUpper().Equals("CORRISPONDENTE"))
                               select new
                ...
                if (newid != 0)
                {
                    var listEntToUp = await this._dbContext.ListeDistrEntities.Where(d => d.ID_DPA_CORR == idcorrglobale).FirstOrDefaultAsync();

                    if (listEntToUp != null)
                    {
                        listEntToUp.ID_DPA_CORR = newid;
                    }
                }

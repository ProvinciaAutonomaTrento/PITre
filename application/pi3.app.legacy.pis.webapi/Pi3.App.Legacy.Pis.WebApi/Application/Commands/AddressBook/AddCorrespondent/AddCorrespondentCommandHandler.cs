...
namespace Pi3.App.Legacy.Pis.WebApi.Application.Commands.AddressBook.AddCorrespondent
{
    ...
    public class AddCorrespondentCommandHandler : IRequestHandler<AddCorrespondentCommand, AddCorrespondentCommandResponse>
    {
        #region Public Members

        public AddCorrespondentCommandHandler(ILogger<AddCorrespondentCommandHandler> logger, IClaimsPrincipalService claimsPrincipalService, IMediator mediator, IHttpContextAccessor httpContextAccessor, IPi3DbContext pi3DbContext, IWebMethodLoggerService loggerService)
        {
            this._logger = logger;
        ...
        public async Task<AddCorrespondentCommandResponse> Handle(AddCorrespondentCommand request, CancellationToken cancellationToken)
        {

            _logger.LogInformation("AddCorrespondent - START");

            AddCorrespondentCommandResponse response = new AddCorrespondentCommandResponse();
            try
            {
                ...
                if (request.Correspondent.CorrespondentType.ToUpper() == "O")
                {
                    ...
                    if (idCorrOcc != null)
                    {
                        response.Correspondent = DBUtils.GetCorrespondentFromDB(idCorrOcc, _pi3DbContext);
                    }
                }
                else
                {
                    ...
                    response.Correspondent = DBUtils.GetCorrespondentFromDB(corr.SYSTEM_ID.ToString(), _pi3DbContext);
                    // prelievo mail:
                ...
                _logger.LogInformation("end AddCorrespondent");


            }
            catch (RestException pisEx)
            {
                _logger.LogError("Eccezione AddCorrespondent: {errorCode}, {errorDescription}", pisEx.ErrorCode, pisEx.Description);
                response = new AddCorrespondentCommandResponse();
                response.Code = GetCorrespondentResponseCode.SYSTEM_ERROR;
                response.ErrorMessage = pisEx.Description;
            }
            catch (Exception e)
            {
                _logger.LogCritical(e, "eccezione AddCorrespondent");
                response = new AddCorrespondentCommandResponse();
                response.Code = GetCorrespondentResponseCode.SYSTEM_ERROR;
        ...
        protected readonly ILogger<AddCorrespondentCommandHandler> _logger;

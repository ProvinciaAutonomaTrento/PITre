...
{
    public class GetModelloByIDSoloConNotificaCommandHandler : IRequestHandler<GetModelloByIDSoloConNotificaCommand, GetModelloByIDSoloConNotificaCommandResponse>
    {
        public GetModelloByIDSoloConNotificaCommandHandler(
            ILogger<GetModelloByIDSoloConNotificaCommandHandler> logger,
        ...
        public async Task<GetModelloByIDSoloConNotificaCommandResponse> Handle(GetModelloByIDSoloConNotificaCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                output = await this.UtentiConNotificaTrasmSoloConNotifica(output,null,null,"GET");
        ...
        protected readonly ILogger<GetModelloByIDSoloConNotificaCommandHandler> _logger;
        ...
        private async Task<DocsPaVO.Modelli_Trasmissioni.ModelloTrasmissione> UtentiConNotificaTrasmSoloConNotifica(DocsPaVO.Modelli_Trasmissioni.ModelloTrasmissione objModTrasm, Object[] utentiDaInserire, Object[] utentiDaCancellare, string operazione)
        {
            DocsPaVO.Modelli_Trasmissioni.UtentiConNotificaTrasm? utNotifica = null;
                                ...
                                case "GET":
                                    var rows = (from p in this._dbContext.PeopleEntities.AsNoTracking()
                                                ...
                                                select new
                                                {
                                                    ...
                                                    p.VAR_COGNOME
                                                }).AsEnumerable().Where(row => GetFlagNotifica(mittDest.SYSTEM_ID, row.P_SYSTEM_ID) == "1").Select(row => new
                                                {
                                                    row.ID,
                                                    row.CODICE,
                                                    row.UTENTE,

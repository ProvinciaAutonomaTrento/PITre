...
using System.Text;
using System.Xml;
using Firmatario = Pi3.Core.Services.File.FirmaDigitale2.Firmatario;

namespace Pi3.App.Legacy.Pis.WebApi.Application.Commands.Documents.DocumentoGetFileConSegnatura
{
    public class DocumentoGetFileConSegnaturaCommandHandler : IRequestHandler<DocumentoGetFileConSegnaturaCommand, DocumentoGetFileConSegnaturaCommandResponse>
    {
        #region Public Members

        public DocumentoGetFileConSegnaturaCommandHandler(ILogger<DocumentoGetFileConSegnaturaCommandHandler> logger,
        ...
        public async Task<DocumentoGetFileConSegnaturaCommandResponse> Handle(DocumentoGetFileConSegnaturaCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                if (!request.Forced)
                {
                    fileDoc = await GetFileConSegnatura(request.fileRequest, request.sch, request.infoUtente, request.position);
                }
                else
                {
            ...
            return new DocumentoGetFileConSegnaturaCommandResponse()
            {
        ...
        protected readonly ILogger<DocumentoGetFileConSegnaturaCommandHandler> _logger;
        ...
        protected virtual void InitializeMapper()
        {
            var configuration = new MapperConfiguration(cfg =>
            {
                cfg.CreateMap<AmministrazioneEntity, Amministrazione>()
                    ...
                    .ForMember(dest => dest.orientamento, src => src.MapFrom(opt => opt.ORIENTAMENTO ?? string.Empty))
                    .ForMember(dest => dest.posizione, src => src.MapFrom(opt => opt.ID_POS_DF))
                    .ForMember(dest => dest.rotazione, src => src.MapFrom(opt => opt.TIPO_ROTAZ))
                ...
                cfg.CreateMap<FirmaElettronicaEntity, FirmaElettronica>()
                     ...
                     .AfterMap((src, dest) =>
                     {
                         if (!string.IsNullOrEmpty(src.XML))
                         {
                             ...
                             dest.Firmatario = string.IsNullOrEmpty(delegato) ? $"{utenteFirmatario} ({ruoloFirmatario})" : $"{delegato} ({ruoloFirmatario}) {Resources.SostitutoDi} {utenteFirmatario}";
                         }
                     });
            });
        ...
        protected async Task<FileDocumento> GetFileConSegnatura(FileRequest fileRequest, SchedaDocumento schedaDocumento, InfoUtente infoUtente, labelPdf labelPdf)
        {
            ...
            _logger.LogInformation("INIZIO Lettura file dal _documentBlobRepository");
            ...
            _logger.LogInformation("FINE Lettura file dal _documentBlobRepository");
            ...
            if (!fileRequest.conSegnaturaPermanente && fileDocumento.estensioneFile.ToLowerInvariant() == "pdf")
            {
                ...
                if (labelPdf.notimbro == false && !labelPdf.repertorioTrasversale)
                {
                    ...
                    {
                        var allegato = (await DocumentoGetAllegati(schedaDocumento.documentoPrincipale.docNumber.AsLong(), string.Empty, string.Empty))
                            .Where(a => a.docNumber == schedaDocumento.docNumber).First();

                        var orientamento = labelPdf.orientamento.ToLower() == "verticale" ? "\n" : " - ";
                        dati += $"{orientamento}Allegato {DecodeTypeAttachment(allegato.TypeAttachment)} {allegato.position} ({allegato.versionLabel})";
                    }
                }
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(datiFirma))
                    {
                        ...
                        if (labelPdf.digitalSignInfo.printOnFirstPage)
                        {
                            ...
                            timbro += String.Join("", datiFirmaAsArray);
                            infoPageEtichetta.Add(new KeyValuePair<int, string>(1, timbro));
                        }
                        ...
                        if (labelPdf.digitalSignInfo.printOnLastPage)
                        {
                            ...
                            {
                                infoPageEtichetta.Add(new KeyValuePair<int, string>(1, timbro));
                                infoPageEtichetta.Add(new KeyValuePair<int, string>(Int32.Parse(numberOfPages), String.Join("", datiFirmaAsArray)));
                ...
                {
                    _logger.LogError(exception: pi3Ex, message: $"Id documento: {fileRequest.docNumber} {pi3Ex.Message}");
                }
                catch (Exception ex)
                {
                            ...
                            )
        {
            ...
            if (labelInfo.tipoLabel)
            {
                ...
                if (datiTimbro.Contains("CLASSIFICA"))
                {
                    ...
                    classifica = await FascicolazioneGetFascicoliDaDoc(sch.systemId.AsLong());
                    ...
                    {
                        var foldersResult = await FascicolazioneGetFoldersDocument(sch.docNumber.AsLong());
                    }
        ...
        protected virtual async Task<string> GetDatiFirmaElettronica(string idDocumento, string versionId, labelPdf lblPdf, DocsPaVO.utente.InfoUtente infoUtente)
        {
            ...
            List<DocsPaVO.LibroFirma.FirmaElettronica> listSign = await GetElettronicSignatureDocument(idDocumento.AsLong(), versionId.AsLong());
                                              ...
                                              string idAmm)
        {
            ...
            if (verificaResponse != null && verificaResponse.Esito.DatiFirmatari != null)
            {
                System.Text.StringBuilder sb = new System.Text.StringBuilder();
                ...
                bool isFirstSigner = true;
                foreach (DatiFirmatari datiFirmatario in verificaResponse.Esito.DatiFirmatari)
                {
        ...
        /// </summary>
        protected string GetDatiFirmaPerEtichettaInfoFirmaCompleta(Firmatario firmatario, int maxCharsPerRow)
        {
        ...
        protected async Task<List<FirmaElettronica>> GetElettronicSignatureDocument(long docnumber, long versionId)
        {
        ...
        protected async Task<List<Folder>> FascicolazioneGetFoldersDocument(long systemIdDocumento)
        {
            ...
            try
            {
                this._dbContext.ProjectEntities
                    ...
                    .Where(x => x.pc.LINK == systemIdDocumento && x.pc.TYPE.Equals("D"))
                    .ToList()
                    .ForEach(f =>
                    {
        ...
        protected async Task<List<Allegato>> DocumentoGetAllegati(long docnumber, string filterAllegatiPec, string simplifiedInteroperabilityId)
        {
            List<Allegato> output = new List<Allegato>();

            try
            {
                var filterAllegati = filterAllegatiPec;
                if (filterAllegatiPec.Equals(simplifiedInteroperabilityId))
                    filterAllegati = "pitre";

                var allegatoQuery = this._dbContext.ProfileEntities.Where(p => (p.CHA_IN_CESTINO ?? "0") != "1" && p.ID_DOCUMENTO_PRINCIPALE == docnumber)
                ...
                switch (filterAllegatiPec)
                {
                    case "pec":
                        allegatoQuery = allegatoQuery.Where(a => this._dbContext.NotificaEntities.Any(n => n.DOCNUMBER == docnumber && a.VAR_PROF_OGGETTO.StartsWith("Ricevuta di ritorno delle Mail")));
                        break;
                    case "user":
                        allegatoQuery = allegatoQuery.Where(a => !this._dbContext.NotificaEntities.Any(n => n.DOCNUMBER == docnumber && (a.VAR_PROF_OGGETTO.StartsWith("Ricevuta di ritorno delle Mail")
                        || a.VAR_PROF_OGGETTO.StartsWith("Ricevuta di mancata consegna") || a.VAR_PROF_OGGETTO.StartsWith("Ricevuta di avvenuta"))
                        && !this._dbContext.VersionEntities.Any(v => v.DOCNUMBER == a.DOCNUMBER && (v.CHA_ALLEGATI_ESTERNO == "1" || v.CHA_ALLEGATI_ESTERNO == "D"))));
                        break;
                    case "pitre":
                        allegatoQuery = allegatoQuery.Where(a => this._dbContext.NotificaEntities.Any(n => n.DOCNUMBER == docnumber && (a.VAR_PROF_OGGETTO.StartsWith("Ricevuta di avvenuta") || a.VAR_PROF_OGGETTO.StartsWith("Ricevuta di mancata consegna"))));
                        break;
                    case "esterni":
                        allegatoQuery = allegatoQuery.Where(a => this._dbContext.VersionEntities.Any(v => v.DOCNUMBER == a.DOCNUMBER && v.CHA_ALLEGATI_ESTERNO == "1"));
                        break;
                    case "derivati":
                        allegatoQuery = allegatoQuery.Where(a => this._dbContext.VersionEntities.Any(v => v.DOCNUMBER == a.DOCNUMBER && v.CHA_ALLEGATI_ESTERNO == "D"));
                        break;
                    case "albopubb":
                        allegatoQuery = allegatoQuery.Where(a => this._dbContext.VersionEntities.Any(v => v.DOCNUMBER == a.DOCNUMBER && v.CHA_ALLEGATI_ESTERNO == "0")
                        && this._dbContext.AlboDocPubbEntities.Any(p => p.DOCNUMBER == a.DOCNUMBER && p.DA_PUBB == "S"));
                        break;
                }
                ...
                foreach (ProfileEntity allegatoEntity in allegatoEntities)
                {
                    ...
                    var lastVersionEntity = await this._dbContext.VersionEntities.Where(v => v.DOCNUMBER == allegatoEntity.DOCNUMBER).OrderByDescending(v => v.VERSION_ID).FirstAsync();
                    var lastComponentsEntity = await this._dbContext.ComponentEntities.Where(c => c.DOCNUMBER == allegatoEntity.DOCNUMBER).OrderByDescending(c => c.VERSION_ID).FirstAsync();
                    ...
                    allegato.autore = await this._dbContext.PeopleEntities.Where(p => p.SYSTEM_ID == lastVersionEntity.AUTHOR).Select(p => p.FULL_NAME).FirstAsync();
                    ...
                    if (allegato.idPeopleDelegato != null)
                        allegato.autore = await this._dbContext.PeopleEntities.Where(p => p.SYSTEM_ID == allegatoEntity.ID_PEOPLE_DELEGATO).Select(p => p.FULL_NAME).FirstAsync()
                    ...
                    allegato.fileName = lastComponentsEntity.PATH ?? string.Empty;
                    ...
                    if (lastComponentsEntity.ID_PEOPLE_DELEGATO_PUTFILE != null && lastComponentsEntity.ID_PEOPLE_DELEGATO_PUTFILE != 0)
                        allegato.autore = await this._dbContext.PeopleEntities.Where(p => p.SYSTEM_ID == lastComponentsEntity.ID_PEOPLE_DELEGATO_PUTFILE).Select(p => p.FULL_NAME).FirstAsync()
        ...
        protected async Task<List<DocsPaVO.fascicolazione.Fascicolo>> FascicolazioneGetFascicoliDaDoc(long idProfile)
        {
            ...
            try
            {
                ...
                var idFascicoloList = this._dbContext.
                        ProjectComponentEntities.Where(x => x.LINK == idProfile)
                ...
                if (fascicoloList != null && fascicoloList.Any())
                {
                    foreach (var f in fascicoloList.Where(x => x.SICUREZZA.Equals("1")))
                    {
                        DocsPaVO.fascicolazione.Fascicolo fasc = new DocsPaVO.fascicolazione.Fascicolo()
                        {
                            ...
                            isFascicolazioneConsentita = f.CHA_CONSENTI_FASC != null ? f.CHA_CONSENTI_FASC.Equals("1") : false,

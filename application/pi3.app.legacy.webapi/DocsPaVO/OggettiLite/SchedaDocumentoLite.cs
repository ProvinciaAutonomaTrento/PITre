...
namespace DocsPaVO.OggettiLite
{
    /// <summary>
    /// </summary>
    [XmlInclude(typeof(ProtocolloEntrataLite))]
    [XmlInclude(typeof(ProtocolloUscitaLite))]
    [XmlInclude(typeof(ProtocolloInternoLite))]
    ...
    [XmlInclude(typeof(SpedizioneDocumentoLite))]
    [Serializable()]
    public class SchedaDocumentoLite
    {
        /// <summary>
        ...
        public ProtocolloLite protocollo;
        ...
        public DocsPaVO.OggettiLite.SpedizioneDocumentoLite spedizioneDocumento;

        public DocsPaVO.fascicolazione.Folder[] folder;

        public DocsPaVO.documento.Documento[] documenti;

        public DocsPaVO.documento.Allegato[] allegati;

        public DocsPaVO.documento.ParolaChiave[] paroleChiave;
        ...
        public InfoDocumento InfoDocumento
        {
            get
            {
                ...
                if (this.protocollo != null)
                {
                    ...
                    if (this.protocollo.GetType().Equals(typeof(ProtocolloEntrataLite)))
                    {
                        infoDoc.mittDest = new string[1];
                        ProtocolloEntrataLite pe = (ProtocolloEntrataLite)this.protocollo;
                    ...
                    else if (this.protocollo.GetType().Equals(typeof(ProtocolloUscitaLite)))
                    {
                        ProtocolloUscitaLite pu = (ProtocolloUscitaLite)this.protocollo;
                        if (pu.destinatari != null)
                        {
                            infoDoc.mittDest = new string[pu.destinatari.Length];
                            for (int i = 0; i < pu.destinatari.Length; i++)
                                infoDoc.mittDest[i] = ((CorrispondenteLite)pu.destinatari[i]).descrizione;
                        }
                    }

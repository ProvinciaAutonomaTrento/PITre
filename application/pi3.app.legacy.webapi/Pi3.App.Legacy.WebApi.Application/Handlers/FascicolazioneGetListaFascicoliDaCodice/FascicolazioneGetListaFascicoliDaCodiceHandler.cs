...
namespace Pi3.App.Legacy.WebApi.Application.Handlers.FascicolazioneGetListaFascicoliDaCodice
{
    ...
    public class FascicolazioneGetListaFascicoliDaCodiceHandler : IRequestHandler<Application.Requests.FascicolazioneGetListaFascicoliDaCodice, FascicolazioneGetListaFascicoliDaCodiceResult>
    {
        #region Public Members

        public FascicolazioneGetListaFascicoliDaCodiceHandler(ILogger<FascicolazioneGetListaFascicoliDaCodiceHandler> logger, IClaimsPrincipalService claimsPrincipalService, IMediator mediator, IPi3DbContext dbContext)
        {
            this._logger = logger;
        ...
        public async Task<FascicolazioneGetListaFascicoliDaCodiceResult> Handle(Application.Requests.FascicolazioneGetListaFascicoliDaCodice request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                List<long> idTitolarioListD = this._dbContext.ProjectEntities.Where(x => x.ID_TITOLARIO == 0 && !x.CHA_STATO.Equals("D") && x.ID_AMM == idTenantAsLong).Select(x => x.SYSTEM_ID).ToList();
                List<long> idTitolarioListA = this._dbContext.ProjectEntities.Where(x => x.ID_TITOLARIO == 0 && x.CHA_STATO.Equals("A") && x.ID_AMM == idTenantAsLong).Select(x => x.SYSTEM_ID).ToList();
            ...
            return new FascicolazioneGetListaFascicoliDaCodiceResult(result.ToArray());
        }
        ...
        protected readonly ILogger<FascicolazioneGetListaFascicoliDaCodiceHandler> _logger;

...
using TrasmettiProtocolloInternoRequest = Pi3.App.Legacy.WebApi.Application.Requests.TrasmettiProtocolloInterno;
...
namespace Pi3.App.Legacy.WebApi.Application.Handlers.TrasmettiProtocolloInterno
{
    public class TrasmettiProtocolloInternoHandler : IRequestHandler<TrasmettiProtocolloInternoRequest, TrasmettiProtocolloInternoResult>
    {
        #region Public Members

        public TrasmettiProtocolloInternoHandler(ILogger<TrasmettiProtocolloInternoHandler> logger, 
        ...
        public async Task<TrasmettiProtocolloInternoResult> Handle(TrasmettiProtocolloInternoRequest request, CancellationToken cancellationToken)
        {
            var output = true;
            var ragioniVerificate = true;
            ...
            try
            {
                ...
                var idRegistro = request.schedaDoc.registro.systemId.AsLong();
                ...
                if (amministraEntity.SPEDIZIONE_AUTO_DOC == "1")
                {
                    ...
                    if (ragioneTo == null)
                    {
                        message = Resources.DestinatariInTo;
                        ragioniVerificate = false;
                    }
                    if (ragioneCC == null)
                    {
                        message = !string.IsNullOrEmpty(message) ? message + " , " + Resources.DestinatariInCC : Resources.DestinatariInCC;
                        ragioniVerificate = false;
                    }
                    if (ragioniVerificate)
                        throw new RagioneTrasmissioneNotFoundPi3Exception();

                    Corrispondente[] destinatariTo = null;
                    Corrispondente[] destinatariCC = null;
                    Corrispondente mittente = null;

                    if (request.schedaDoc.tipoProto == "I")
                    {
                        mittente = ((ProtocolloInterno)request.schedaDoc.protocollo).mittente;
                        destinatariTo = ((ProtocolloInterno)request.schedaDoc.protocollo).destinatari.Cast<Corrispondente>().Where(c => c.tipoIE == "I").ToArray();
                        destinatariCC = ((ProtocolloInterno)request.schedaDoc.protocollo).destinatariConoscenza.Cast<Corrispondente>().Where(c => c.tipoIE == "I").ToArray();
                    }
                    if (request.schedaDoc.tipoProto == "P")
                    {
                        mittente = ((ProtocolloUscita)request.schedaDoc.protocollo).mittente;
                        destinatariTo = ((ProtocolloUscita)request.schedaDoc.protocollo).destinatari.Cast<Corrispondente>().Where(c => c.tipoIE == "I").ToArray();
                        destinatariCC = ((ProtocolloUscita)request.schedaDoc.protocollo).destinatariConoscenza.Cast<Corrispondente>().Where(c => c.tipoIE == "I").ToArray();
                    }
                    var aggregate = new Trasmissione(idTenant.ToString(), DateTime.Now, request.schedaDoc.docNumber, TipiOggettiTrasmessiEnum.DocumentoAmministrativo);
                    foreach (var destTO in destinatariTo)
                    {
                        ...
                        if (mittente == null || !mittente.systemId.Equals(destTO.systemId))
                        {
                            if (destTO.GetType() == typeof(UnitaOrganizzativa))
                            {
                                ...
                                QueryCorrispondenteAutorizzato qa = new QueryCorrispondenteAutorizzato()
                                {
                                    ...
                                    idRegistro = request.schedaDoc.registro.systemId,
                                ...
                                {
                                    var noTrasm = string.Format(Resources.UOPrivaDiRuoliRiferimento, destTO.descrizione, destTO.codiceRubrica);
                                    message = !string.IsNullOrEmpty(message) ? message + " - " + noTrasm + "\\n" : noTrasm;
                                }
                            }
                        ...
                        {
                            var noTrasm = string.Format(Resources.DestinatarioMittenteCoincidono, destTO.descrizione, destTO.codiceRubrica);
                            message = !string.IsNullOrEmpty(message) ? message + " - " + noTrasm + "\\n": noTrasm;
                        }
                    }
                    ...
                    foreach (var destCC in destinatariCC)
                    {
                        ...
                        if (mittente == null || !mittente.systemId.Equals(destCC.systemId))
                        {
                            if (destCC.GetType() == typeof(UnitaOrganizzativa))
                            {
                                ...
                                {
                                    var noTrasm = string.Format(Resources.UOPrivaDiRuoliRiferimento, destCC.descrizione, destCC.codiceRubrica);
                                    message = !string.IsNullOrEmpty(message) ? message + " - " + noTrasm + "\\n" : noTrasm;
                                }
                            }
                        ...
                        {
                            var noTrasm = string.Format(Resources.DestinatarioMittenteCoincidono, destCC.descrizione, destCC.codiceRubrica);
                            message = !string.IsNullOrEmpty(message) ? message + " - " + noTrasm + "\\n" : noTrasm;
                        }
                    }
            ...
            if(!string.IsNullOrEmpty(message))
                message = String.Format(Resources.AvvisoTrasmissioniNonEffettuate, message);

            return new TrasmettiProtocolloInternoResult(output, ragioniVerificate, message);
        }
        ...
        protected readonly ILogger<TrasmettiProtocolloInternoHandler> _logger;

...
using GetProcessesSignatureVisibleRoleRequest = Pi3.App.Legacy.WebApi.Application.Requests.GetProcessesSignatureVisibleRole;

namespace Pi3.App.Legacy.WebApi.Application.Handlers.GetProcessesSignatureVisibleRole
{
    public class GetProcessesSignatureVisibleRoleHandler : IRequestHandler<GetProcessesSignatureVisibleRoleRequest, GetProcessesSignatureVisibleRoleResult>
    {
        #region Public Members

        public GetProcessesSignatureVisibleRoleHandler(ILogger<GetProcessesSignatureVisibleRoleHandler> logger,
        ...
        public async Task<GetProcessesSignatureVisibleRoleResult> Handle(GetProcessesSignatureVisibleRoleRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                List<string> tipoVisibilita = new List<string>() { "T"};
                if(request.asProponente)
                    tipoVisibilita.Add("P");
                if (request.asMonitoratore)
                    tipoVisibilita.Add("M");

                var queryable = this._dbContext.SchemaProcessoFirmaEntities
                    .Join(this._dbContext.ProcessoFirmaVisibilitaEntities, processo => processo.ID_PROCESSO, vis => vis.ID_PROCESSO, (processo, vis) => new { processo, vis })
                    .Where(j => j.vis.ID_GROUPS == idGroup && j.processo.ELIMINATO == null && tipoVisibilita.Contains(j.vis.CHA_TIPO_VISIBILITA));

                if (!request.includiVisibilitaRimosse)
                    queryable = queryable.Where(j => j.vis.DTA_FINE == null);

                var processiFirma = await queryable
                    .OrderBy(j => j.processo.NOME.ToUpper())
                    .Select(j => new ProcessoFirmaEntity()
                    {
                        SCHEMA_PROCESSO_FIRMA = j.processo
                    })
                    .AsNoTracking()
                    .ToListAsync();

                if(processiFirma != null && processiFirma.Any())
                {
                    processiFirma.ForEach(p =>
                    {
                         p.passi = this._dbContext.PassoDiFirmaEntities
                            ...
                            .Where(j => j.passo.ID_PROCESSO == p.SCHEMA_PROCESSO_FIRMA.ID_PROCESSO)
                ...
                output = this._mapper.Map<ProcessoFirma[]>(processiFirma);
            }
            catch (Exception ex)
            {
                this._logger.LogError(exception: ex, message: ex.Message);
            }
            return new GetProcessesSignatureVisibleRoleResult(output);
        }
        ...
        protected readonly ILogger<GetProcessesSignatureVisibleRoleHandler> _logger;
        ...
        protected virtual void InitializeMapper()
        {
            ...
            {
                cfg.CreateMap<ProcessoFirmaEntity, ProcessoFirma>()
                    .ForMember(dest => dest.idProcesso, opt => opt.MapFrom(src => src.SCHEMA_PROCESSO_FIRMA.ID_PROCESSO))
                    .ForMember(dest => dest.nome, opt => opt.MapFrom(src => src.SCHEMA_PROCESSO_FIRMA.NOME))
                    .ForMember(dest => dest.idRuoloAutore, opt => opt.MapFrom(src => src.SCHEMA_PROCESSO_FIRMA.RUOLO_AUTORE != null ? src.SCHEMA_PROCESSO_FIRMA.RUOLO_AUTORE.ToString() : string.Empty))
                    .ForMember(dest => dest.idPeopleAutore, opt => opt.MapFrom(src => src.SCHEMA_PROCESSO_FIRMA.UTENTE_AUTORE != null ? src.SCHEMA_PROCESSO_FIRMA.UTENTE_AUTORE.ToString() : string.Empty))
                    .ForMember(dest => dest.isInvalidated, opt => opt.MapFrom(src => src.SCHEMA_PROCESSO_FIRMA.TICK == "1"))
                    .ForMember(dest => dest.IsProcessModel, opt => opt.MapFrom(src => src.SCHEMA_PROCESSO_FIRMA.CHA_MODELLO == "1"));
        ...
        protected class ProcessoFirmaEntity
        {
            public SchemaProcessoFirmaEntity SCHEMA_PROCESSO_FIRMA { get; set; }

            public List<PassoFirmaDiProcessoEntity> passi { get; set; }
        }

...
namespace Pi3.App.Legacy.WebApi.Application.Handlers.DocumentoGetDettaglioDocumentoNoSecurity
{
    // Richiede libreria MediatR
    public class DocumentoGetDettaglioDocumentoNoSecurityHandler : IRequestHandler<DocumentoGetDettaglioDocumentoNoSecurityRequest, DocumentoGetDettaglioDocumentoNoSecurityResult>
    {
        #region Public Members

        public DocumentoGetDettaglioDocumentoNoSecurityHandler(
            ILogger<DocumentoGetDettaglioDocumentoNoSecurityHandler> logger,
        ...
        public async Task<DocumentoGetDettaglioDocumentoNoSecurityResult> Handle(DocumentoGetDettaglioDocumentoNoSecurityRequest request, CancellationToken cancellationToken)
        {
            ...
            var allegati = new System.Collections.ArrayList();
            ...
            try
            {
                ...
                var profileEntitiesAllegati = await this._dbContext.ProfileEntities.AsNoTracking()
                    .Where(p => p.ID_DOCUMENTO_PRINCIPALE == profileEntity.SYSTEM_ID)
                    .Select(p => new { SYSTEM_ID = p.SYSTEM_ID, IN_LIBROFIRMA = p.IN_LIBROFIRMA, p.FORWARDING_SOURCE, p.VAR_PROF_OGGETTO, p.DOCNUMBER })
                    .OrderBy(p => p.DOCNUMBER)
                    .ToListAsync();

                foreach (var a in profileEntitiesAllegati)
                {
                    var lastVersionAllegatoEntity = await (from versions in this._dbContext.VersionEntities
                                                           ...
                                                           {
                                                               Version = versions,
                                                               AuthorUserId = people.USER_ID,
                                                               DelegatoUserId = peopleDelegato.USER_ID
                                                           })
                                                    .AsNoTracking()
                    ...
                    var lastComponentAllegatoEntity = await (from components in this._dbContext.ComponentEntities
                                                             ...
                                                             select new
                                                             {
                                                                 ...
                                                                 AuthorUserId = components.ID_PEOPLE_PUTFILE.HasValue ? IPi3DbContextMappedFunctions.GetPeopleUserId(components.ID_PEOPLE_PUTFILE.Value) : null,
                                                                 DelegatoUserId = components.ID_PEOPLE_DELEGATO_PUTFILE.HasValue ? IPi3DbContextMappedFunctions.GetPeopleUserId(components.ID_PEOPLE_DELEGATO_PUTFILE.Value) : null
                                                             })
                                                    .AsNoTracking()
                                                    .FirstAsync();
                ...
                foreach (var versionEntity in versionEntities.OrderByDescending(v => v.Version.VERSION_ID).ToList())
                {
                    ...
                    {
                        documento.firmatari = new Firmatario[1];
                ...
                if (profileEntity.ID_DOCUMENTO_PRINCIPALE.HasValue)
                {
                    var tempDocPrinc = await
                            (from docPrincipale in this._dbContext.ProfileEntities
                             ...
                             select new
                             {
                                 ...
                                 dataApertura = docPrincipale.DTA_PROTO.HasValue ? docPrincipale.DTA_PROTO.AsDateFormat() : docPrincipale.CREATION_DATE.AsDateFormat(),
                    ...
                    schedaDocumento.documentoPrincipale = new InfoDocumento()
                    {
                        ...
                        codRegistro = !string.IsNullOrEmpty(tempDocPrinc.idRegistro) ? await _dbContext.RegistroEntities.Where(r => r.SYSTEM_ID == tempDocPrinc.idRegistro.AsLong()).Select(r => r.VAR_CODICE).FirstOrDefaultAsync() : string.Empty,
                ...
                schedaDocumento.oggetto = new Oggetto((profileEntity.ID_OGGETTO.HasValue ? profileEntity.ID_OGGETTO.ToString() : null), profileEntity.VAR_PROF_OGGETTO ?? string.Empty);
                ...
                var soggettiProtocollo = await (from dap in this._dbContext.DocArrivoParEntities
                                                join p in this._dbContext.ProfileEntities on dap.ID_PROFILE equals p.SYSTEM_ID
                                                ...
                                                join si in this._dbContext.StatoInvioEntities on new
                                                {
                                                    key1 = cg.SYSTEM_ID,
                                                    key2 = p.SYSTEM_ID
                                                }
                                                equals new
                                                {
                                                    key1 = si.ID_CORR_GLOBALE.Value,
                                                    key2 = si.ID_PROFILE.Value
                                                }
                                                into StatoInvio
                                                from si in StatoInvio.DefaultIfEmpty()
                                                where p.SYSTEM_ID == profileEntity.SYSTEM_ID && dap.ID_PROFILE == profileEntity.SYSTEM_ID
                ...
                {
                    mittente.canalePref = await _dbContext.CanaleCorrEntities.AsNoTracking()
                        .Join(_dbContext.DocumentTypesEntities.AsNoTracking(),
                            c => c.ID_DOCUMENTTYPE,
                            d => d.SYSTEM_ID,
                            (c, d) => new { c, d })
                        .Where(j => j.c.ID_CORR_GLOBALE == mittente.systemId.AsLong())
                        .Select(j => new Canale
                        {
                            systemId = j.d.SYSTEM_ID.ToString(),
                            descrizione = j.d.DESCRIPTION,
                            typeId = j.d.TYPE_ID,
                            tipoCanale = j.d.TYPE_ID
                        })
                        .FirstOrDefaultAsync();
                }
                ...
                if (protocollo != null)
                {
                    ...
                    protocollo.dataProtocollazione = profileEntity.DTA_PROTO?.AsDateFormat();
                    ...
                    protocollo.invioConferma = profileEntity.CHA_INVIO_CONFERMA;

                    if (!string.IsNullOrEmpty(protocollo.segnatura))
                    {
                        var corrStoEntities = await _dbContext.CorrStoEntities.AsNoTracking()
                            .Where(c => c.ID_PROFILE == profileEntity.SYSTEM_ID && c.DTA_MODIFICA > profileEntity.DTA_PROTO)
                            .ToListAsync();

                        if (corrStoEntities.Any())
                        {
                            protocollo.isMittModificato = corrStoEntities.Any(c => (new string[] { "M", "I" }).Contains(c.CHA_TIPO_MITT_DES));
                            protocollo.isMittMultiModificato = profileEntity.CHA_TIPO_PROTO == "A" ? corrStoEntities.Any(c => c.CHA_TIPO_MITT_DES == "MD") : false;
                            protocollo.isDestModificato = profileEntity.CHA_TIPO_PROTO != "A" ? corrStoEntities.Any(c => (new string[] { "C", "F", "D" }).Contains(c.CHA_TIPO_MITT_DES)) : false;
                        }
                        ...
                        opt => opt.AfterMap((src, dest) =>
                        {
                            var elencoOggetti = (from dat in this._dbContext.AssociazioneTemplatesEntities
                                                 ...
                                                 select new
                                                 {
                                                     ...
                                                     TIPO_CONTATORE = doc.CHA_TIPO_TAR,
                                                     CAMPO_XML_ASSOC = docc.CAMPO_XML_ASSOC
                                                 })
                                                 .AsNoTracking();
                            ...
                            foreach (var o in elencoOggetti)
                            {
                                ...
                                if (o.TIPO.DESCRIZIONE_TIPO.Equals("CasellaDiSelezione"))
                                {
                                    var oggCustomValues = _dbContext.AssociazioneTemplatesEntities.AsNoTracking()
                                        .Where(x => x.ID_OGGETTO == o.SYSTEM_ID && x.DOC_NUMBER == request.docNumber)
                                        .Select(o => new
                                        {
                                            o.VALORE_OGGETTO_DB
                                        })
                                            ...
                                            case "CasellaDiSelezione":
                                                if (k < valoriSelezionati.Count)
                                                {
                                                    valOg = new DocsPaVO.ProfilazioneDinamica.ValoreOggetto
                                                    {
                                                        ...
                                                        VALORE_DI_DEFAULT = oggCustomValues[k].VALORE_DI_DEFAULT ?? string.Empty,
                                                        COLOR_BG = oggCustomValues[k].COLOR_BG,
                                                        ABILITATO = oggCustomValues[k].ABILITATO.HasValue ? Convert.ToInt32(oggCustomValues[k].ABILITATO) : 1
                                                    };
                                ...
                                no = new OggettoCustom()
                                {
                                    ...
                                    REPERTORIO = (o.REPERTORIO.HasValue ? o.REPERTORIO.ToString() : string.Empty),
                                    ...
                                    TIPO_CONTATORE = o.TIPO_CONTATORE ?? string.Empty,
                                    CAMPO_XML_ASSOC = o.CAMPO_XML_ASSOC
                                };
                                lo.Add(no);
                ...
                {
                    schedaDocumento.noteDocumento = await _dbContext.NoteEntities
                        .Join(_dbContext.PeopleEntities, n => n.IDUTENTECREATORE, p => p.SYSTEM_ID, (n, p) => new { n, p })
                        .Join(_dbContext.GroupEntities, j => j.n.IDRUOLOCREATORE, r => r.SYSTEM_ID, (j, r) => new { j.n, j.p, r })
                        .GroupJoin(_dbContext.PeopleEntities, j => j.n.IDPEOPLEDELEGATO, delegato => delegato.SYSTEM_ID, (j, delegato) => new { j.n, j.p, j.r, delegato })
                        .SelectMany(j => j.delegato.DefaultIfEmpty(), (j, delegato) => new { j.n, j.p, j.r, delegato })
                        .Where(w => w.n.IDOGGETTOASSOCIATO == schedaDocumento.systemId.AsLong()
                         ...
                         .OrderByDescending(s => s.n.DATACREAZIONE)
                         .Select(s => new InfoNota()
                         {
                             ...
                             DescrPeopleDelegato = s.delegato.FULL_NAME,
                             ...
                             DaInserire = false,
                             DaRimuovere = false,
                             SolaLettura = s.n.IDUTENTECREATORE != idUserAsNumber
                         })
                         .AsNoTracking()
                         .ToListAsync();
                }

                if (_dbContext.ProfParoleEntities.AsNoTracking().Where(w => w.ID_PROFILE == schedaDocumento.systemId.AsLong()).Any())
                {
                    schedaDocumento.paroleChiave =
                         await _dbContext.ProfParoleEntities
                         .Join(_dbContext.ParolaEntities,
                             pp => pp.ID_PAROLA, p => p.SYSTEM_ID,
                             (pp, p) => new { pp, p }).
            ...
            return new DocumentoGetDettaglioDocumentoNoSecurityResult(schedaDocumento);
        }
        ...
        protected readonly ILogger<DocumentoGetDettaglioDocumentoNoSecurityHandler> _logger;
        ...
        protected IMapper InitializeMapper()
        {
            var configuration = new MapperConfiguration(cfg =>
            {
                ...
                cfg.CreateMap<SoggettoProtocolloEntity, DocsPaVO.utente.Corrispondente>()
                    ...
                    .ForMember(dest => dest.descrizione, opt => opt.MapFrom(src => src.CorrGlobali.VAR_DESC_CORR ?? string.Empty))
                    ...
                    .AfterMap((src, dest) =>
                    {
                        ...
                        if (src.StatoInvio != null)
                        {
                            dest.protocolloDestinatario = new ProtocolloDestinatario()
                            {
                                systemId = src.StatoInvio.SYSTEM_ID.ToString(),
                                codiceAOO = src.StatoInvio.VAR_CODICE_AOO,
                                codiceAmm = src.StatoInvio.VAR_CODICE_AMM,
                                dta_spedizione = src.StatoInvio.DTA_SPEDIZIONE.AsDateTimeFormat(),
                                protocolloDestinatario = src.StatoInvio.VAR_PROTO_DEST,
                                dataProtocolloDestinatario = src.StatoInvio.DTA_PROTO_DEST.AsDateTimeFormat(),
                                annullato = src.StatoInvio.CHA_ANNULLATO,
                                motivo = src.StatoInvio.VAR_MOTIVO_ANNULLA,
                                provvedimento = src.StatoInvio.VAR_PROVVEDIMENTO,
                                descrizioneCorr = src.CorrGlobali.VAR_DESC_CORR
                            };
                        }
                    });
        ...
        protected class SoggettoProtocolloEntity
        {
            ...
            public StatoInvioEntity StatoInvio { get; set; }
        }

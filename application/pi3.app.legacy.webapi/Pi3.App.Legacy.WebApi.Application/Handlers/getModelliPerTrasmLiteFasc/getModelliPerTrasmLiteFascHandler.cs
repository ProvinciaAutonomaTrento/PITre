...
using getModelliPerTrasmLiteFascRequest = Pi3.App.Legacy.WebApi.Application.Requests.getModelliPerTrasmLiteFasc;

namespace Pi3.App.Legacy.WebApi.Application.Handlers.getModelliPerTrasmLiteFasc
{
    public class getModelliPerTrasmLiteFascHandler : IRequestHandler<getModelliPerTrasmLiteFascRequest, getModelliPerTrasmLiteFascResult>
    {
        #region Public Members

        public getModelliPerTrasmLiteFascHandler(ILogger<getModelliPerTrasmLiteFascHandler> logger,
        ...
        public async Task<getModelliPerTrasmLiteFascResult> Handle(getModelliPerTrasmLiteFascRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(request.idTipoFasc))
                {
                    ...
                    var idTipoFasc = request.idTipoFasc.AsLong(); 
                    ...
                    if (idDiagramma != null && idStato != null)
                    {
                        ...
                        .Any(d => d.ID_MOD_TRASM == m.SYSTEM_ID && d.ID_TIPO_FASC == idTipoFasc && d.ID_DIAGRAMMA == idDiagramma && d.ID_STATO == idStato) &&
                            !this._dbContext.ModelloMittDestEntities
                                .Join(this._dbContext.CorrGlobaliEntities, m2 => m2.ID_CORR_GLOBALI, c => c.SYSTEM_ID, (m2, c) => new {m2, c})
                                .Where(x => x.m2.ID_MODELLO == m.SYSTEM_ID && x.m2.CHA_TIPO_URP == "R" && x.m2.CHA_TIPO_MITT_DEST == "D" && x.c.CHA_DISABLED_TRASM == "1").Any());

                        modelliTrasmQueryable_4 = modelliTrasmQueryable_4.Where(j => this._dbContext.AssDiagrammiEntities.AsNoTracking()
                        .Any(d => d.ID_MOD_TRASM == j.m.SYSTEM_ID && d.ID_TIPO_FASC == idTipoFasc && d.ID_DIAGRAMMA == idDiagramma && d.ID_STATO == idStato) &&
                            !this._dbContext.ModelloMittDestEntities
                                .Join(this._dbContext.CorrGlobaliEntities, m2 => m2.ID_CORR_GLOBALI, c => c.SYSTEM_ID, (m2, c) => new { m2, c })
                                .Where(x => x.m2.ID_MODELLO == j.m.SYSTEM_ID && x.m2.CHA_TIPO_URP == "R" && x.m2.CHA_TIPO_MITT_DEST == "D" && x.c.CHA_DISABLED_TRASM == "1").Any());
                    }
                    else
                    {
                        if (idDiagramma != null)
                        {
                            modelliTrasmQueryable_3 = modelliTrasmQueryable_3.Where(m => this._dbContext.AssDiagrammiEntities.AsNoTracking()
                                .Any(d => d.ID_MOD_TRASM == m.SYSTEM_ID && d.ID_TIPO_FASC == idTipoFasc && d.ID_DIAGRAMMA == idDiagramma) &&
                            !this._dbContext.ModelloMittDestEntities
                                .Join(this._dbContext.CorrGlobaliEntities, m2 => m2.ID_CORR_GLOBALI, c => c.SYSTEM_ID, (m2, c) => new { m2, c })
                                .Where(x => x.m2.ID_MODELLO == m.SYSTEM_ID && x.m2.CHA_TIPO_URP == "R" && x.m2.CHA_TIPO_MITT_DEST == "D" && x.c.CHA_DISABLED_TRASM == "1").Any());

                            modelliTrasmQueryable_4 = modelliTrasmQueryable_4.Where(j => this._dbContext.AssDiagrammiEntities.AsNoTracking()
                                .Any(d => d.ID_MOD_TRASM == j.m.SYSTEM_ID && d.ID_TIPO_FASC == idTipoFasc && d.ID_DIAGRAMMA == idDiagramma) &&
                            !this._dbContext.ModelloMittDestEntities
                                .Join(this._dbContext.CorrGlobaliEntities, m2 => m2.ID_CORR_GLOBALI, c => c.SYSTEM_ID, (m2, c) => new { m2, c })
                                .Where(x => x.m2.ID_MODELLO == j.m.SYSTEM_ID && x.m2.CHA_TIPO_URP == "R" && x.m2.CHA_TIPO_MITT_DEST == "D" && x.c.CHA_DISABLED_TRASM == "1").Any());
                        }
                        ...
                        {
                            modelliTrasmQueryable_3 = modelliTrasmQueryable_3.Where(m => this._dbContext.AssDiagrammiEntities.AsNoTracking()
                                .Any(d => d.ID_MOD_TRASM == m.SYSTEM_ID && d.ID_TIPO_FASC == idTipoFasc && d.ID_STATO == idStato) &&
                            !this._dbContext.ModelloMittDestEntities
                                .Join(this._dbContext.CorrGlobaliEntities, m2 => m2.ID_CORR_GLOBALI, c => c.SYSTEM_ID, (m2, c) => new { m2, c })
                                .Where(x => x.m2.ID_MODELLO == m.SYSTEM_ID && x.m2.CHA_TIPO_URP == "R" && x.m2.CHA_TIPO_MITT_DEST == "D" && x.c.CHA_DISABLED_TRASM == "1").Any());

                            modelliTrasmQueryable_4 = modelliTrasmQueryable_4.Where(j => this._dbContext.AssDiagrammiEntities.AsNoTracking()
                                .Any(d => d.ID_MOD_TRASM == j.m.SYSTEM_ID && d.ID_TIPO_FASC == idTipoFasc && d.ID_STATO == idStato) &&
                            !this._dbContext.ModelloMittDestEntities
                                .Join(this._dbContext.CorrGlobaliEntities, m2 => m2.ID_CORR_GLOBALI, c => c.SYSTEM_ID, (m2, c) => new { m2, c })
                                .Where(x => x.m2.ID_MODELLO == j.m.SYSTEM_ID && x.m2.CHA_TIPO_URP == "R" && x.m2.CHA_TIPO_MITT_DEST == "D" && x.c.CHA_DISABLED_TRASM == "1").Any());
                        }
                        ...
                        {
                            modelliTrasmQueryable_3 = modelliTrasmQueryable_3.Where(m => this._dbContext.AssDiagrammiEntities.AsNoTracking()
                                .Any(d => d.ID_MOD_TRASM == m.SYSTEM_ID && d.ID_TIPO_FASC == idTipoFasc) &&
                            !this._dbContext.ModelloMittDestEntities
                                .Join(this._dbContext.CorrGlobaliEntities, m2 => m2.ID_CORR_GLOBALI, c => c.SYSTEM_ID, (m2, c) => new { m2, c })
                                .Where(x => x.m2.ID_MODELLO == m.SYSTEM_ID && x.m2.CHA_TIPO_URP == "R" && x.m2.CHA_TIPO_MITT_DEST == "D" && x.c.CHA_DISABLED_TRASM == "1").Any());

                            modelliTrasmQueryable_4 = modelliTrasmQueryable_4.Where(j => this._dbContext.AssDiagrammiEntities.AsNoTracking()
                                .Any(d => d.ID_MOD_TRASM == j.m.SYSTEM_ID && d.ID_TIPO_FASC == idTipoFasc) &&
                            !this._dbContext.ModelloMittDestEntities
                                .Join(this._dbContext.CorrGlobaliEntities, m2 => m2.ID_CORR_GLOBALI, c => c.SYSTEM_ID, (m2, c) => new { m2, c })
                                .Where(x => x.m2.ID_MODELLO == j.m.SYSTEM_ID && x.m2.CHA_TIPO_URP == "R" && x.m2.CHA_TIPO_MITT_DEST == "D" && x.c.CHA_DISABLED_TRASM == "1").Any());
                        }
                ...
                if (modelloTrasmEntities != null && modelloTrasmEntities.Count > 0)
            ...
            return new getModelliPerTrasmLiteFascResult(output);
        }
        ...
        protected readonly ILogger<getModelliPerTrasmLiteFascHandler> _logger;

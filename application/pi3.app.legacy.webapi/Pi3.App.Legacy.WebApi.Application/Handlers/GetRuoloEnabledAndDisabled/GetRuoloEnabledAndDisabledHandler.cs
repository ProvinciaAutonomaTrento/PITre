...
using GetRuoloEnabledAndDisabledRequest = Pi3.App.Legacy.WebApi.Application.Requests.GetRuoloEnabledAndDisabled;

namespace Pi3.App.Legacy.WebApi.Application.Handlers.GetRuoloEnabledAndDisabled
{
    public class GetRuoloEnabledAndDisabledHandler : IRequestHandler<GetRuoloEnabledAndDisabledRequest, GetRuoloEnabledAndDisabledResult>
    {
        protected readonly ILogger<GetRuoloEnabledAndDisabledHandler> _logger;
        ...
        private async Task<List<CorrGlobaliEntity>> GetRuoliUtente(long? idUo)
        {
            ...
            var baseQuery = await (this._dbContext.CorrGlobaliEntities.AsNoTracking().
                Where(a => (a.CHA_TIPO_IE != null ? a.CHA_TIPO_IE.Equals("I") : false) &&
                (a.CHA_TIPO_URP != null ? a.CHA_TIPO_URP.Equals("U") : false) &&
                !a.DTA_FINE.HasValue && a.SYSTEM_ID != idUo)).ToListAsync();
        ...
        private async Task<DocsPaVO.utente.Registro[]> GetRegistriRuolo(string idRuolo)
        {
            ...
            var rolesInfo = await (from b in this._dbContext.RuoloRegistroEntities.AsNoTracking()
                                   ...
                                   where a.SYSTEM_ID == b.ID_REGISTRO && a.CHA_RF.Equals("0") && b.ID_RUOLO_IN_UO == idRuolo.AsLong()
        ...
        private async Task<DocsPaVO.utente.Funzione[]> GetFunzioni(string idRuolo)
        {
            var tempFunc = await (from a in this._dbContext.FunzioneEntities.AsNoTracking()
                                  from b in this._dbContext.TipoFunzioneEntities.AsNoTracking()
                                  from c in this._dbContext.TipoFRuoloEntities.AsNoTracking()
                                  where a.ID_TIPO_FUNZIONE == b.SYSTEM_ID && b.SYSTEM_ID == c.ID_TIPO_FUNZ && c.ID_RUOLO_IN_UO == idRuolo.AsLong()
        ...
        public GetRuoloEnabledAndDisabledHandler(
            ILogger<GetRuoloEnabledAndDisabledHandler> logger,
        ...
        public async Task<GetRuoloEnabledAndDisabledResult> Handle(GetRuoloEnabledAndDisabledRequest request, CancellationToken cancellationToken)
        {
            DocsPaVO.utente.Ruolo objRuolo = null;

            try
            {
                var ruoli = await (from a in this._dbContext.PeopleGroupEntities.AsNoTracking()
                             from b in this._dbContext.CorrGlobaliEntities.AsNoTracking()
                             from c in this._dbContext.TipoRuoloEntities.AsNoTracking()
                             from d in this._dbContext.CorrGlobaliEntities.AsNoTracking()
                             let groupSysId = this._dbContext.CorrGlobaliEntities.AsNoTracking().Where(corr => corr.SYSTEM_ID == b.ORIGINAL_ID).Select(corr => corr.ID_GRUPPO).FirstOrDefault()
                             where (b.ID_TIPO_RUOLO == c.SYSTEM_ID) &&
                             (d.SYSTEM_ID == b.ID_UO) &&
                             (groupSysId == a.GROUPS_SYSTEM_ID) &&
                             (b.SYSTEM_ID == request.idRuolo.AsLong())
                ...
                if (ruoli.Count > 0)
                {
                    string maxLivello = ruoli.OrderByDescending(r => r.NUM_LIVELLO).FirstOrDefault()?.NUM_LIVELLO.ToString();

                    string usaConByPrior = await this._configurationService.GetValue<string>("0","USA_CONNECTBYPRIOR_OR_WITH");
                    List<CorrGlobaliEntity>? uoEntSet = null;
                    var tempQueryLivello = ruoli.FirstOrDefault();

                    if (tempQueryLivello != null)
                    {
                        if (usaConByPrior.Equals("1"))
                        {

...
namespace Pi3.App.Legacy.WebApi.Application.Handlers.GeneraMetadatiAGID
{
    ...
    public class GeneraMetadatiAGIDHandler : MessageQueueBaseCommandHandler<GeneraMetadatiAGIDRequest>
    {
        #region Public Members
        public GeneraMetadatiAGIDHandler(ILogger<GeneraMetadatiAGIDHandler> logger, IServiceProvider serviceProvider) : base(logger, serviceProvider)
        { }


        protected override async Task InternalHandle(IServiceProvider serviceProvider, GeneraMetadatiAGIDRequest message)
        {
            ...
            try
            {
                ...
                if (anagraficaLogEntity == null)
                    throw new CodiceEventoNotFoundPi3Exception(message.Method);
        ...
        protected async Task CreateDocumentoAmministrativoInformatico(AnagraficaLogEntity anagraficaLogEntity,
            ...
            IMediator mediator,
            GeneraMetadatiAGIDRequest message)
        {
            var idTenant = claimsPrincipalService.Current.GetPi3ClaimValue<long>(Pi3ClaimTypes.IdTenant);
            ...
            try
            {
                ...
                DocsPaVO.documento.SchedaDocumento schedaDoc = (await mediator.Send(new Requests.DocumentoGetDettaglioDocumentoNoSecurity(infoUtente,
                    message.IdOggetto.ToString(),
                    message.IdOggetto.ToString())))
                    .output;
            ...
            {
                this._logger.LogError(exception: pi3Ex, message: pi3Ex.Message, "Errore in CreateDocumentoAmministrativoInformatico, event: " + message.IdOggetto);
            }
            catch (Exception ex)
            {
                this._logger.LogCritical(exception: ex, message: ex.Message, "Errore in CreateDocumentoAmministrativoInformatico, event: " + message.IdOggetto);
            }
        }
            ...
            GeneraMetadatiAGIDRequest message)
        {
            ...
            try
            {
                ...
                {
                    schedaDoc = (await mediator.Send(new Requests.DocumentoGetDettaglioDocumentoNoSecurity(infoUtente,
                            message.IdOggetto.ToString(),
                            message.IdOggetto.ToString())))
                            .output;
                ...
                if (documentoAmministrativoInformatico != null)
                {
                    if (aggiornaMetadati)
                    {
                        metadatiDocumentoEntity.METADATI_XML = documentoAmministrativoInformatico.ToXmlString(true, false, false, Encoding.UTF8);
                        metadatiDocumentoEntity.DTA_AZIONE = message.DataAzione;
                        metadatiDocumentoEntity.VAR_COD_AZIONE = anagraficaLogEntity.VAR_CODICE;
                        metadatiDocumentoEntity.VAR_DESC_AZIONE = anagraficaLogEntity.VAR_DESCRIZIONE;
                        metadatiDocumentoEntity.VAR_OGGETTO = anagraficaLogEntity.VAR_OGGETTO;
                        metadatiDocumentoEntity.DTA_INSERIMENTO = await dbContext.GetSystemDateTime();

                        await ((DbContext)dbContext).SaveChangesAsync();
                    }
                }
            }
            catch (Pi3Exception piEx)
            {
                this._logger.LogError(exception: piEx, message: piEx.Message);
            }
            catch (Exception ex)
            {
                this._logger.LogCritical(exception: ex, message: ex.Message);
            ...
            IMediator mediator)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(schedaDoc.documenti[0].fileSize) && Convert.ToInt32(schedaDoc.documenti[0].fileSize) > 0)
                    fileDocumento = (await mediator.Send(new Requests.DocumentoGetInfoFile(schedaDoc.documenti[0], infoUtente))).output;
                ...
                if (schedaDoc.registro != null && !string.IsNullOrEmpty(schedaDoc.registro.systemId))
                    ...
                    registro = (await mediator.Send(new Requests.GetListaRegistriByRuolo(creatore.idCorrGlob_Ruolo))).output[0];
                ...
                if (schedaDoc.protocollo != null)
                {
                    ...
                    switch (schedaDoc.tipoProto)
                    {
                        ...
                        case "I":
                            ...
                            {
                                foreach (DocsPaVO.utente.Corrispondente dest in protocolloInterno.destinatari)
                                {
                                    documentoAmmInformatico.Soggetti = AddSoggetto(documentoAmmInformatico, AddDestinatario(dest));
                                }
                            }
                            ...
                            {
                                foreach (DocsPaVO.utente.Corrispondente dest in protocolloInterno.destinatariConoscenza)
                                {
                                    documentoAmmInformatico.Soggetti = AddSoggetto(documentoAmmInformatico, AddDestinatario(dest));
                                }
                            }
            ...
            {
                this._logger.LogCritical(exception: ex, message: ex.Message, "Errore in AddDocumentoAmministrativoInformatico");
            }
            return documentoAmmInformatico;
        }

        protected DocumentoAmministrativoInformaticoType AddSoggettoOperatore(DocumentoAmministrativoInformaticoType documentoAmmInformatico, DocsPaVO.utente.Utente utente)
        {
            bool presente = false;
        ...
        protected DocumentoAmministrativoInformaticoType UpdateMittDest(DocumentoAmministrativoInformaticoType documentoAmmInformatico, DocsPaVO.documento.SchedaDocumento schedaDoc, DocsPaVO.utente.InfoUtente infoUtente)
        {
            ...
            DocsPaVO.utente.InfoUtente infoUtente)
        {
            ...
            DocsPaVO.documento.Documento[] docs = (await mediator.Send(new Requests.GetVersionsMainDocument(infoUtente, docnumber.ToString()))).output;
            ...
            if (!isAllegato)
            {
                DocsPaVO.documento.FileDocumento fileDocumento = !string.IsNullOrEmpty(doc.fileSize) && Convert.ToInt32(doc.fileSize) > 0 ?
                    (await mediator.Send(new Requests.DocumentoGetInfoFile(doc, infoUtente))).output : null;
        ...
        protected async Task<DocumentoAmministrativoInformaticoType> UpdateAllegati(DocumentoAmministrativoInformaticoType documentoAmmInformatico,
            ...
            IMediator mediator)
        {
            var allegati = (await mediator.Send(new Requests.DocumentoGetAllegati(docnumber.ToString(), string.Empty, string.Empty))).output;
        ...
        protected async Task<DocumentoAmministrativoInformaticoType> ScambiaDocumento(DocumentoAmministrativoInformaticoType documentoAmmInformatico,
            ...
            IMediator mediator)
        {
            DocsPaVO.documento.Documento doc = (await mediator.Send(new Requests.GetVersionsMainDocument(infoUtente, docnumber.ToString()))).output[0];
        ...
        protected async Task<Utente> GetUtente(long idPeople, IPi3DbContext dbContext)
        {
            ...
            var utente = new DocsPaVO.utente.Utente()
            {
        ...
        private static RuoloType AddDestinatario(DocsPaVO.utente.Corrispondente dest)
        {
                ...
                if (dest.tipoCorrispondente == "P")
                {
                    destinatario = new RuoloType()
                    {
                        Item = new TipoSoggetto31Type()
                        {
                            Item = new PFType()
                            {
                                Nome = (dest as DocsPaVO.utente.Utente).nome,
                                Cognome = (dest as DocsPaVO.utente.Utente).cognome
                            }
                        }
                    };
                }
        ...
        private static RuoloType AddMittente(DocsPaVO.utente.Corrispondente mitt)
        {
            ...
            IMediator mediator)
        {
            ...
            try
            {
                ...
                DocsPaVO.utente.Registro registro = (await mediator.Send(new Requests.GetListaRegistriByRuolo(infoUtente.idCorrGlobali))).output[0];
        ...
        {
            bool result = true;
            Registro registro = (await mediator.Send(new Requests.GetListaRegistriByRuolo(infoUtente.idCorrGlobali))).output[0];

...
using GetSpedizioneDocumentoRequest = Pi3.App.Legacy.WebApi.Application.Requests.GetSpedizioneDocumento;

namespace Pi3.App.Legacy.WebApi.Application.Handlers.GetSpedizioneDocumento
{
    public class GetSpedizioneDocumentoHandler : IRequestHandler<GetSpedizioneDocumentoRequest, GetSpedizioneDocumentoResult>
    {
        #region Public Members

        public GetSpedizioneDocumentoHandler(
            ILogger<GetSpedizioneDocumentoHandler> logger, 
        ...
        public async Task<GetSpedizioneDocumentoResult> Handle(GetSpedizioneDocumentoRequest request, CancellationToken cancellationToken)
        {
            ...
            return new GetSpedizioneDocumentoResult(output);
        }
        ...
        protected readonly ILogger<GetSpedizioneDocumentoHandler> _logger;
            ...
            DocsPaVO.documento.SchedaDocumento documento)
        {
            ...
            foreach (DictionaryEntry entry in destinatariDivisi)
            {
                ...
                foreach (DocsPaVO.utente.Corrispondente destinatario in listDestinatari)
                {
                    ...
                    //se il corrispondente � interoperabilit� passo l'url come indirizzo
                    ...
                    // Determina se il documento � stato spedito almeno una volta al destinatario
                    ...
                    if (spedito)
                    {
                        // Se � stato spedito ma � presente mancata consegna devo rispedire di nuovo
                    ...
                    .Concat(protocolloUscita.destinatariConoscenza ?? new Corrispondente[0]))
            {
                ...
                {
                    // Determina se il corrispondente esterno � gi� stato inserito tra gli interoperanti
        ...
        protected virtual async Task FetchDestinatariInterni(DocsPaVO.Spedizione.SpedizioneDocumento output, DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.documento.SchedaDocumento documento)
        {
            ...
            foreach (DocsPaVO.utente.Corrispondente destinatario in destinatari)
            {
                ...
                {
                    // Bench� interno, determina se il corrispondente non appartenga ad un'altra AOO
                    // verificando se � considerato tra i destinatari esterni
                    if (output.DestinatariEsterni.Count(e => e.DatiDestinatari.Any(d => d.systemId == destinatario.systemId)) == 0)
                    {
                        ...
                        //In funzione dell'abilitazione alle trasmissioni vengono impostate le propriet�
        ...
        {
            // Se il documento � stato spedito per interoperabilit�,
            // l'informazione relativa alla data di spedizione � nella DPA_STATO_INVIO,
        ...
        private async Task<string> GetDataUltimaTrasmissione(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.documento.SchedaDocumento documento, DocsPaVO.utente.Corrispondente destinatario)
        {
            ...
            {
                // Quando il destinatario di una trasmissione � una UO,
                // la trasmissione in realt� viene effettuata ai ruoli di riferimento dello stesso.
                // Pertanto nella ricerca della data ultima trasmissione, 
                // sar� reperita quella relativa alla trasmissione
                ...
                // Il destinatario � un utente o un ruolo
        ...
        private DocsPaVO.Spedizione.StatoSpedizioneDocumento GetStatoDestinatarioEsternoNonInteroperante(DocsPaVO.utente.Corrispondente destinatario)
        {
                         ...
                         destinatario.canalePref.descrizione != "INTEROPERABILITA"))
            {
                ...
                statoSpedizione.Descrizione = "La mail non � canale preferenziale della UO, destinatario non interoperante";
            }
        ...
        private async Task AssociaCaselleDestinatariEsterni(List<DocsPaVO.Spedizione.DestinatarioEsterno> destinatari)
        {
            foreach (var dest in destinatari)
            {
                            ...
                            || c.canalePref.descrizione.ToUpper().Equals("PORTALE"))))
                {
                    ...
                    //                         Email = m.VAR_EMAIL,
                    //                         Principale = m.VAR_PRINCIPALE ?? string.Empty,
                    //                         Note = m.VAR_NOTE
                    //                     })
                    //        .ToListAsync();
                    corr.Emails = (await _mediator.Send(new Requests.GetMailCorrEsterno(corr.systemId))).output;
                }
            }
        }
            ...
            { 
                ...
                try
                {
                    if (destinatario.GetType() != typeof(DocsPaVO.utente.Corrispondente))
                    {
                        ...
                        // Booleano utilizzato per indicare se bisogna spedire per interoperabilit� semplificata
                        ...
                        if (destinatario.GetType() == typeof(DocsPaVO.utente.Utente))
                        {
                            ...
                            if (tipoIE == "I")
                            {
                                ...
                                var ruoliUtente = (await this._mediator.Send(
                                        new Requests.GetListaRuoliUtente(destinatarioUtente.idPeople))).output;
                                ...
                                if (ruoliUtente != null && ruoliUtente.Any())
                                {
                                        ...
                                        (!string.IsNullOrEmpty(uo.email) && uo.email != emailReg))
                                    {
                                        ...
                                        {
                                            var hasRegistro = ruolo.registri.Any(reg => 
                                            ...
                                            if (hasRegistro)
                                            {
                                                uo = (DocsPaVO.utente.UnitaOrganizzativa)
                                                    ...
                                                    new Requests.AddressbookGetCorrispondenteBySystemId(ruolo.uo.systemId))).output;
                        ...
                        else if (destinatario.GetType() == typeof(DocsPaVO.utente.UnitaOrganizzativa))
                        {
                            ...
                            // Verifica se utilizzare l'interoperabilit� semplificata
                        ...
                        else if (destinatario.GetType() == typeof(DocsPaVO.utente.RaggruppamentoFunzionale))
                        {
                            ...
                            // Verifica se utilizzare l'interoperabilit� semplificata per l'RF
                        ...
                        // Se il canale preferito � interoperabilit� semplificata, questa si pu� utilizzare solo se attiva
                        if (isSimpInterop)
                        ...
                        {
                            // per i corrisp interni nel caso di interoperabilit� si suppone che il canale
                            ...
                            isSimpInterop)
                        {
                            ...
                            // Se si � nel caso di spedizione per interoperabilit� semplificata, la chiave � l'url
                            if (isSimpInterop)
                                email = varUrl;
                        ...
                        else //da Scartare
                        {
                            ...
                            if (varCodiceAOO == codRegistroMitt)
                                r.SendErrorMessage = "Destinatario Non Interoperante, poich� appartenente alla stessa AOO";
        ...
        {
            if (corr == null) //Necessario nel caso di spedizione a ruolo esterno con mail
                return false;

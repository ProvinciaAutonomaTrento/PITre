...
using AddressbookInsertCorrispondenteRequest = Pi3.App.Legacy.WebApi.Application.Requests.AddressbookInsertCorrispondente;


namespace Pi3.App.Legacy.WebApi.Application.Handlers.AddressbookInsertCorrispondente
{
    public class AddressbookInsertCorrispondenteHandler : IRequestHandler<AddressbookInsertCorrispondenteRequest, AddressbookInsertCorrispondenteResult>
    {
        #region Public Members

        public AddressbookInsertCorrispondenteHandler(
            IWebMethodLoggerService webMethodLoggerService,
            ILogger<AddressbookInsertCorrispondenteHandler> logger,
        ...
        public async Task<AddressbookInsertCorrispondenteResult> Handle(AddressbookInsertCorrispondenteRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if(string.IsNullOrEmpty(output.errore) && !string.IsNullOrEmpty(output.systemId))
                    await this._webMethodLoggerService.LogOK("RUBRICAINSCORR", output.systemId,string.Format(Resources.InseritoCorrispondente, output.codiceCorrispondente));
            }
            catch (Exception ex)
            {
        ...
        private async Task<(bool, DocsPaVO.utente.Corrispondente)> Insert(DocsPaVO.utente.Corrispondente corr, DocsPaVO.utente.Corrispondente parent, string idAmm)
        {
            ...
            try
            {
                ...
                try
                {
                    bool isCodRubPresente = (await _mediator.Send(new Requests.CheckCodRubricaPresente(corr.codiceRubrica, corr.tipoCorrispondente, ammId, corr.idRegistro, corr.inRubricaComune))).output;
        ...
        protected readonly ILogger<AddressbookInsertCorrispondenteHandler> _logger;

...
{
    public class InviaEmailTrasmissioneCommandHandler : MessageQueueBaseCommandHandler<InviaEmailTrasmissioneRequest>
    {
        ...
        protected override async Task InternalHandle(IServiceProvider serviceProvider, InviaEmailTrasmissioneRequest message)
        {
            ...
            if (trasmissioneEntity.ID_PROFILE.HasValue)
            {
                ...
                subject = string.Format(
                    Resources.Resources.EmailSubject,
                    string.Empty,
                    Resources.Resources.DocumentLabel,
            ...
            else
            {
                ...
                subject = string.Format(
                    Resources.Resources.EmailSubject,
                    string.Empty,
                    Resources.Resources.FolderLabel,
            ...
            try
            {
                ...
                if (!creation.Success)
                    throw new ProviderNotFoundPi3Exception(String.Format(Resources.Resources.ProviderNotFound, provider));
                        ...
                        (configurations) =>
                        {
                            switch (configurations)
                            {
                                ...
                                case GraphSendEmailConfiguration graphSendEmailConfigurations:
                                    ...
                                    break;
                                default:
                                    throw new SendMailPi3Exception(Resources.Resources.ProviderNonGestito);
                            }
                        },
                        instructions
                        );
        ...
        protected async Task<string> GetMessageBody(IPi3DbContext dbContext, object entity, TrasmSingolaEntity trasmissioneSingolaEntity, TrasmissioneEntity trasmissioneEntity)
        {
            ...
            if (entity is ProfileEntity)
            {
                ...
                message = message.Replace(Resources.Resources.BodyTagDocumentDescription, profileEntity.VAR_PROF_OGGETTO.AsHtmlBold())
                    .Replace(Resources.Resources.BodyTagIdentifier, !string.IsNullOrWhiteSpace(profileEntity.VAR_SEGNATURA) ? profileEntity.VAR_SEGNATURA.AsHtmlBold() : profileEntity.DOCNUMBER.ToString().AsHtmlBold())
                    .Replace(Resources.Resources.BodyTagDocumentSender, mittProto.AsHtmlBoldOrStrikeThrough());
            }
            else if (entity is ProjectEntity)
            {
                ...
                message = message.Replace(Resources.Resources.BodyTagFolderCode, projectEntity.VAR_CODICE)
                    .Replace(Resources.Resources.BodyTagFolderDescription, projectEntity.DESCRIPTION);
            }
            else
            {
                return string.Empty;
            ...
            message = message.Replace("\n", "<BR>")
                .Replace(Resources.Resources.BodyTagTransmissionReason, ragioneEntity.VAR_DESC_RAGIONE.AsHtmlBold())
                .Replace(Resources.Resources.BodyTagTransmissionRecipient, (await dbContext.CorrGlobaliEntities.FirstAsync(x => x.SYSTEM_ID == trasmissioneSingolaEntity.ID_CORR_GLOBALE)).VAR_DESC_CORR.AsHtmlBold())
                .Replace(Resources.Resources.BodyTagGeneralNotes, trasmissioneEntity.VAR_NOTE_GENERALI.AsHtmlBoldOrStrikeThrough())
                .Replace(Resources.Resources.BodyTagRecipientNotes, trasmissioneSingolaEntity.VAR_NOTE_SING.AsHtmlBoldOrStrikeThrough());
            ...
            switch (objType)
            {
                case "D":
                    ...
                    if (recipientGroupId.HasValue)
                    {
                        if (toFile)
                        {
                            link = string.Format(Resources.Resources.LinkToFileWithRole, path, idObject, idObject, recipientGroupId.Value);
                            message = string.Format(Resources.Resources.BodyLinkToFile, link);
                        }

                        link = string.Format(Resources.Resources.LinkToDocumentWithRole, path, idTenant, idObject, recordType ?? string.Empty, recipientGroupId.Value);
                        message += string.Format(Resources.Resources.BodyLinkDocument, link);
                    ...
                    else
                    {
                        if (toFile)
                        {
                            link = string.Format(Resources.Resources.LinkToFileNoRole, path, idTenant, idObject, recordType ?? string.Empty);
                            message = string.Format(Resources.Resources.BodyLinkToFile, link);
                        }

                        link = string.Format(Resources.Resources.LinkToDocumentNoRole, path, idTenant, idObject, recordType ?? string.Empty);
                        message += string.Format(Resources.Resources.BodyLinkDocument, link);
                    }
                    break;
                case "F":
                    message = string.Format(
                        Resources.Resources.BodyLinkToFolder,
                        recipientGroupId.HasValue ?
                            string.Format(Resources.Resources.LinkToFolderWithRole, path, idTenant, idObject, recipientGroupId.Value) :
                            string.Format(Resources.Resources.LinkToFolderNoRole, path, idTenant, idObject));
                    break;
            }
        ...
        protected async Task<List<EmailContentAttachment>> GetMessageAttachments(IServiceProvider serviceProvider, long docnumber, long idTenant)
        {
            ...
            var profileAttachmentsEntities = await dbContext.ProfileEntities
                .Where(x => x.ID_DOCUMENTO_PRINCIPALE == docnumber)
                .ToListAsync();

            foreach (var x in profileAttachmentsEntities)
            {

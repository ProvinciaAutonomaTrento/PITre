...
public class VistoTrasmissioneCommandHandler : IRequestHandler<VistoTrasmissioneCommand, VistoTrasmissioneCommandResponse>
{
    ...
    public VistoTrasmissioneCommandHandler(
    ...
    public async Task<VistoTrasmissioneCommandResponse> Handle(VistoTrasmissioneCommand request, CancellationToken cancellationToken)
    {
        if (!request.Id.IsValidAggregateId())
            throw new IdAggregateNotFoundPi3Exception(request.Id);

        //this._claimsPrincipalService.Current.GetPi3Identity();
        ...
        aggregate.Visto(idGroup.ToString(), idUtente.ToString(), new Core.AggregateModels.TrasmissioneAggregate.ValueObjects.Visto { 
           IdDelegato = idUtente.ToString(),
           UserIdDelegato = userId
        });
        await this._trasmissioneRepository.Update(aggregate);

        return new VistoTrasmissioneCommandResponse();
    }
}

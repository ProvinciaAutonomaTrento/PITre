...
namespace Pi3.App.AggregazioneDocumentale.WebApi.Application.Commands.Trasmissioni.AddModello
{
    public class AggiungiTrasmissioneModelloCommandHandler: IRequestHandler<AggiungiTrasmissioneModelloCommand,AggiungiTrasmissioneModelloCommandResponse>
    {
        ...
        public async Task<AggiungiTrasmissioneModelloCommandResponse> Handle(AggiungiTrasmissioneModelloCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!isValid)
                throw new ModelloFormatoInvalidoPi3Exception(request.Modello);
            ...
            if (modello.TipoOggettoTrasmesso != Core.AggregateModels.ModelloTrasmissioneAggregate.ValueObjects.TipiOggettiTrasmessiEnum.Fascicolo) {
                throw new ModelloTrasmissioneDocumentoPi3Exception(request.IdAggregato, request.Modello);
            }


            var aggregate = new Core.AggregateModels.TrasmissioneAggregate.Trasmissione(idTenant, DateTime.Now,
                request.IdAggregato, 
                Core.AggregateModels.TrasmissioneAggregate.ValueObjects.TipiOggettiTrasmessiEnum.Fascicolo);
            ...
            foreach (var destinatario in modello.Destinatari ) {
                if (destinatario is GruppoDestinatarioModelloTrasmissione) {
                    ...
                    {
                        IdGruppoDestinatario = gruppoEntity.SYSTEM_ID.ToString(), //132003064.ToString(),
                        ...
                        IdRagioneTrasmissione = item.RagioneTrasmissione.Id.ToString(),// 131976805.ToString(),

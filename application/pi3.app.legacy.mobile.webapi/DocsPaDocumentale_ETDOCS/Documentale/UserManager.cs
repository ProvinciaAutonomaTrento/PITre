...
namespace DocsPaDocumentale_ETDOCS.Documentale
{
    ...
    {
        private static Serilog.ILogger logger = Serilog.Log.ForContext(typeof(UserManager));
        ...
        public bool LoginUser(DocsPaVO.utente.UserLogin userLogin, out DocsPaVO.utente.Utente utente, out DocsPaVO.utente.UserLogin.LoginResult loginResult)
        {
            ...
            try
            {
                ...
                // se facciamo login con codice fiscale, setto un flag e imposto il codiceFiscale come userId
                // per non incasinare troppi metodi
                bool loginCodiceFiscale = !string.IsNullOrWhiteSpace(userLogin.CodiceFiscale);
                if (loginCodiceFiscale)
                    userLogin.UserName = userLogin.CodiceFiscale;
                ...
                if (utenti.IsUtenteDisabled(userLogin.UserName, userLogin.Modulo, userLogin.IdAmministrazione, loginCodiceFiscale))
                {
                ...
                if (result && !utenti.UserLoginNoPwd(out peopleId, name, idAmm.ToString(), userLogin.Modulo, loginCodiceFiscale))
                {
                    loginResult = DocsPaVO.utente.UserLogin.LoginResult.UNKNOWN_USER;
                    result = false;
                ...
                if (result && !string.IsNullOrEmpty(peopleId))
                {
                    ...
                    {
                        if(!string.IsNullOrWhiteSpace(userLogin.Password) && DocsPaUtils.Security.SSOAuthTokenHelper.IsAuthToken(userLogin.Password))
                        {
                            try
                            {
                        ...
                        else
                        {
                            ...
                            if (DocsPaLdapServices.LdapUserConfigurations.UserCanConnectToLdap(userLogin.UserName, loginCodiceFiscale))
                            {
                                ...
                                string userNameLdap = DocsPaLdapServices.LdapUserConfigurations.GetLdapUserConfigByName(userLogin.UserName, loginCodiceFiscale).LdapIdSync;
                            ...
                            else
                            {
                                ...
                                {
                                    AdminPasswordConfig pwdConfig = new AdminPasswordConfig();
                                    if (!loginCodiceFiscale && pwdConfig.IsSupportedPasswordConfig())
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        if (!utenti.UserLogin(out peopleId, name, password, idAmm.ToString(), loginCodiceFiscale))
                                        {
                                            result = false;
                                            loginResult = DocsPaVO.utente.UserLogin.LoginResult.UNKNOWN_USER;
                                            logger.Debug("Utente non valido o non trovato");
                    ...
                    if (result)
                    {
                        ...
                        utente = utenti.GetUtente(name, userLogin.IdAmministrazione, userLogin.Modulo, loginCodiceFiscale);

...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(DocumentManager));
    ...
    public bool CreateDocumentoStampaRegistro(DocsPaVO.documento.SchedaDocumento schedaDocumento, DocsPaVO.utente.Ruolo ruolo, out DocsPaVO.utente.Ruolo[] ruoliSuperiori)
    {
        ...
        try
        {
            ...
            if (retValue)
            {
                ...
                //Aggiungo la riga per le informazioni sulla DB_FILE_INFO
                using (DocsPaDB.Query_DocsPAWS.Documenti dbDocumenti = new DocsPaDB.Query_DocsPAWS.Documenti())
                    dbDocumenti.InsertInfoFile(new DocsPaVO.documento.InfoFile() { IdProfile = schedaDocumento.docNumber, VersionId = (schedaDocumento.documenti[0] as DocsPaVO.documento.FileRequest).versionId, IdDocumentoPrincipale = null });
    ...
    {
        bool retValue = false;
        string idDocumentoPrincipale = allegato.docNumber;
        try
        {
            ...
            else
            {
                ...
                using (DocsPaDB.TransactionContext transactionContext = new DocsPaDB.TransactionContext())
                {
                    ...
                    allegato.versionLabel = DocsPaDB.Query_DocsPAWS.Documenti.FormatCodiceAllegato(allegato.position);
                    ...
                    if (allegato.TypeAttachment == 6 && schedaDocumento.documentoPrincipale == null)
                    {
                        //scrivo il log se qualcosa è null
                        DocsPaDB.Query_DocsPAWS.Documenti db = new DocsPaDB.Query_DocsPAWS.Documenti();
                        db.InsertLogErrorSegnaturaProtocollo(allegato.docNumber, "schedaDocumento.documentoPrincipale è null");
                    }
                    ...
                    if (retValue)
                    {
                        ...
                        //Aggiungo la riga in DPA_INFO_FILE
                        if (allegato.TypeAttachment != 2 && allegato.TypeAttachment != 3)
                            documentiDb.InsertInfoFile(new DocsPaVO.documento.InfoFile() { IdProfile = allegato.docNumber, VersionId = allegato.versionId, IdDocumentoPrincipale = idDocumentoPrincipale });
                    }
        ...
        catch (Exception ex)
        {
            ...
            logger.Write(Serilog.Events.LogEventLevel.Debug, ex, "error om AddAttachment");
            if (allegato.TypeAttachment == 6)
            {
                //Inserisco nella Tabella di log di errori di creazione segnatura
                DocsPaDB.Query_DocsPAWS.Documenti db = new DocsPaDB.Query_DocsPAWS.Documenti();
                db.InsertLogErrorSegnaturaProtocollo(allegato.docNumber, ex.StackTrace.Replace(System.Environment.NewLine, " "));
            }
        }
    ...
    public bool CreateDocumentoGrigio(DocsPaVO.documento.SchedaDocumento schedaDocumento, DocsPaVO.utente.Ruolo ruolo, out DocsPaVO.utente.Ruolo[] ruoliSuperiori, string conCopia = null)
    {
        ...
        try
        {
            ...
            //Aggiungo la riga per le informazioni sulla DB_FILE_INFO
            documenti.InsertInfoFile(new DocsPaVO.documento.InfoFile() { IdProfile = schedaDocumento.docNumber, VersionId = (schedaDocumento.documenti[0] as DocsPaVO.documento.FileRequest).versionId, IdDocumentoPrincipale = null });
        }
        catch (Exception exception)
        {
            //logger.Debug(exception);
            logger.Write(Serilog.Events.LogEventLevel.Debug, exception, "error om CreateDocumentoGrigio");
    ...
    public static string PathAsUnixPath(string pathAsString)
    {
    ...
    public bool GetFile(ref DocsPaVO.documento.FileDocumento fileDocumento, ref DocsPaVO.documento.FileRequest fileRequest)
    {
        ...
        try
        {
            ...
            filePath = PathAsUnixPath(filePath);
    ...
    /// <returns></returns>
    protected virtual string GetDocPathAdvanced(DocsPaVO.documento.FileRequest fileRequest, DocsPaVO.utente.InfoUtente objSicurezza, string repositoryRootPath = "")
    {
        string result = null;
        ...
        try
        {
            if (fileRequest != null)
            {
                ...
                //legge l'id della uo di appartenenza del gruppo
                string id = documentale.DOC_GetIdUoBySystemId(objSicurezza.idGruppo);
                //GM 24-7-2013: per generazione stampe del registro di conservazione
                //le stampe non hanno ruolo definito: gestisco id==null
                ...
                //verifica se la directory esiste
                string rootPath = string.IsNullOrEmpty(repositoryRootPath) ? DocsPaVO.Settings.AppSettings.Instance.DOC_ROOT : repositoryRootPath;
                string appo = @rootPath + "\\" + filePath;
                appo = PathAsUnixPath(appo);
                DirectoryInfo docFullPath = new DirectoryInfo(appo);

                logger.Debug($"GetPathDocAdvanced - rootPath: {rootPath},\n appo: {appo},\n docFullPath: {docFullPath},\n filerequest.path: {fileRequest.path}");
                if (!docFullPath.Exists)
                {
                    logger.Debug($"Il path {docFullPath} non esiste, lo creo");
                    //crea la directory
                    docFullPath.Create();
                }
    ...
    {
        logger.Information("PutFileMetadata - BEGIN");
        bool retValue = false;

        // Test finale del nome file originale (per checkin/checkout) prima della modifica definitiva
        if (String.IsNullOrEmpty(fileDocumento.nomeOriginale))
            fileDocumento.nomeOriginale = fileDocumento.name;
        try
        {
            ...
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                ...
                parameters.Add(new DocsPaUtils.Data.ParameterSP("tipoFirma", fileRequest.tipoFirma, 2, DocsPaUtils.Data.DirectionParameter.ParamInput));
                if (!String.IsNullOrEmpty(fileDocumento.nomeOriginale))
                ...
                string idPeoplePutfile = this.userInfo.idPeople;

                parameters.Add(new DocsPaUtils.Data.ParameterSP("idPeoplePutfile", idPeoplePutfile, 0, DocsPaUtils.Data.DirectionParameter.ParamInput));
                retValue = (dbProvider.ExecuteStoredProcedure("putFile", parameters, null) > 0);

                if (retValue)
                {
                    DocsPaDB.Query_DocsPAWS.Documenti documenti = new DocsPaDB.Query_DocsPAWS.Documenti();
                    documenti.UpdateLastEditDateProfile(fileRequest.docNumber);

                    if (this.userInfo.delegato != null && !string.IsNullOrEmpty(this.userInfo.delegato.idPeople))
                    {
                        documenti.UpdateComponentIdPeopleDelegato(fileRequest.versionId, this.userInfo.delegato.idPeople);
                    }
                }
            }
        }
        ...
        logger.Information("PutFileMetadata - END");
        return retValue;
    }
    ...
    {
        logger.Information("PutFile - BEGIN");
        logger.Debug($"estensione: {estensione}");
        bool retValue = false;
        ...
        try
        {
            ...
            string relativeDocPath = GetDocPathAdvanced(fileRequest, this.userInfo, repositoryRootPath);
            ...
            {
                logger.Debug($"fileRequest.fileName not null: {fileRequest.fileName}");
                fileName = fileRequest.fileName;

                string extensions = string.Empty;
                logger.Debug($"Path.GetExtension(fileName): {Path.GetExtension(fileName)}");
                ...
                while (!string.IsNullOrEmpty(Path.GetExtension(fileName)))
                {
                    ...
                    logger.Debug($"fileName nel while: {fileName}");
            ...
            logger.Debug($"rootPath: {rootPath},\n relativeRootPath: {relativeDocPath},\n fileName: {fileName},\n filerequest.path: {fileRequest.path}");

            // Scrittura dei soli metadati del file da inserire nel sistema documentale
            logger.Debug($"Inizio PutFileMetadata");
            retValue = this.PutFileMetadata(fileRequest, fileDocumento, estensione);

            if (retValue)
            {
                logger.Debug($"Scrivo il file");

                try
                {
                    // Scrittura file, solamente se l'operazione di aggiornamento sui dati è andata a buon fine
                    var pathAsUnixPath = PathAsUnixPath(fileRequest.path);
                    using (FileStream fileStream = new FileStream(pathAsUnixPath, FileMode.Create, FileAccess.Write, FileShare.Write))
                    {
                        logger.Debug($"Scrivo il file nel path {fileRequest.path}");
                        logger.Debug($"Content.length {fileDocumento.content.Length}");

                        fileStream.Write(fileDocumento.content, 0, fileDocumento.content.Length);
                        // Alessandro Aiello
                        // Correzione errore che impediva ai documenti di essere visualizzati prima di essere salvati
                        // perche il file temporaneo veniva cancellato
                        // La cencellazione del file è stata spostata qui, in modo tale da essere effettuata esclusivamente dopo il salvataggio effettivo del file

                        // Verifico se istanza di BigFile
                        DocsPaVO.documento.BigFileDocumento bigFileDocumento = fileDocumento as DocsPaVO.documento.BigFileDocumento;
                        if (bigFileDocumento != null && !String.IsNullOrEmpty(bigFileDocumento.BigFilePath))
                        {
                            try
                            {
                                System.IO.DirectoryInfo tempDirectoryBigFile = System.IO.Directory.GetParent(bigFileDocumento.BigFilePath);
                                tempDirectoryBigFile.Delete(true);
                            }
                            catch (Exception ex)
                            {
                                logger.Error(string.Format("DocumentManager->PutFile() >> Errore durante la rimozione del file temporaneo (bigfile): {0}", ex.Message));
                            }
                        }
                    }
                }
                ...
                {
                    logger.Error($"Errore nella scrittura del file {fileRequest.path} >\n {ex.Message}");
                }

                // Aggiornamento oggetto FileRequest
                fileRequest.docServerLoc = rootPath; // System.Configuration.ConfigurationManager.AppSettings["DOC_ROOT"];
        ...
        {
            logger.Debug($"Eccezione PathTooLongException in PutFile");
        ...
        logger.Information("PutFile - END");
        return retValue;
    }
        ...
        try
        {
            ...
            if (risultatoProtocollazione == DocsPaVO.documento.ResultProtocollazione.OK)
            {
                ...
                if (schedaDocumento.docNumber != null && schedaDocumento.docNumber != "")
                {
                    ...
                    //Aggiungo la riga per le informazioni sulla DB_FILE_INFO
                    doc.InsertInfoFile(new DocsPaVO.documento.InfoFile() { IdProfile = schedaDocumento.docNumber, VersionId = (schedaDocumento.documenti[0] as DocsPaVO.documento.FileRequest).versionId, IdDocumentoPrincipale = null });
    ...
    public bool AddVersion(DocsPaVO.documento.FileRequest fileRequest, bool daInviare)
    {
        ...
        try
        {
            ...
            fileRequest.idPeopleDelegato = string.Empty;
            if (this.userInfo.delegato != null && !string.IsNullOrEmpty(this.userInfo.delegato.idPeople))
            {
                //Vado ad aggiorna il db con l'id_people_delegato
                fileRequest.idPeopleDelegato = this.userInfo.delegato.idPeople;
            }
            ...
            //Aggiorno last_edit_date in profile
            documenti2.UpdateLastEditDateProfile(fileRequest.docNumber);

            // INC000000598064
            // se si sta aggiungendo una nuova versione di un allegato va inserito anche il numero pagine
            var allegato = fileRequest as DocsPaVO.documento.Allegato;
            if (allegato != null && allegato.numeroPagine != null)
            {
                documenti2.UpdateNumPageDescription(allegato.numeroPagine, allegato.versionId, allegato.descrizione);
            }

            //PULISCO I CAMPI NELLA DPA_INFO_FILE POPOLATI CON LA VERSIONE PRECEDENTE
            documenti2.UpdateInfoFile(new DocsPaVO.documento.InfoFile() { IdProfile = fileRequest.docNumber, VersionId = fileRequest.versionId });
    ...
    public bool RestoreDocumentoDaCestino(DocsPaVO.documento.InfoDocumento infoDocumento)
    {
            ...
            !infoDocumento.allegato)
        {
            ...
            List<string> idAllegati = doc.GetDocnumberAllegati(infoDocumento.docNumber);
            foreach (string docnumberAllegato in idAllegati)
            {
                DocsPaVO.documento.InfoDocumento infoDocAllegato = doc.GetInfoDocumento(this.userInfo.idGruppo, this.userInfo.idPeople, docnumberAllegato, false);

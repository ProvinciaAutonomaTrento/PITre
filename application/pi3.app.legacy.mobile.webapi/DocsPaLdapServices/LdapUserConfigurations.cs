...
public sealed class LdapUserConfigurations
{
    ...
    /// <returns></returns>
    public static LdapUserConfig GetLdapUserConfig(string idUser, bool loginCodiceFiscale)
    {
        ...
        return GetLdapUserConfigByName(user.userId, loginCodiceFiscale);
    }
    ...
    /// <returns></returns>
    public static LdapUserConfig GetLdapUserConfigByName(string userName, bool loginCodiceFiscale)
    {
        ...
        if (ldapIntegrationActive)
        {
            ...
            config = ldapDbServices.GetLdapUserConfigByName(userName, loginCodiceFiscale);
        }
    ...
    /// <param name="userName"></param>
    /// <returns></returns>
    public static bool UserCanConnectToLdap(string userName, bool loginCodiceFiscale)
    {
        string idAmministrazioneAsString;
        using (DocsPaDB.Query_DocsPAWS.Utenti utentiDb = new DocsPaDB.Query_DocsPAWS.Utenti())
            utentiDb.GetIdAmmUtente(out idAmministrazioneAsString, userName, loginCodiceFiscale);
        ...
        if (idAmministrazione > 0)
        {
            ...
            DocsPaVO.Ldap.LdapUserConfig userConfigInfo = GetLdapUserConfigByName(userName, loginCodiceFiscale);
    ...
    private static void CheckProperty<T>(object obj, string propertyName, bool required, int maxLenght)
    {
        ...
        if (p != null)
        {
            ...
            if (p.FieldType.Equals(typeof(string)) && maxLenght > 0 &&
                value is string s && s.Length > maxLenght)

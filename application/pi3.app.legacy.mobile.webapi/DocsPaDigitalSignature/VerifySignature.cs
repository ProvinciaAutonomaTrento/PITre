...
public class VerifySignature
{
    ...
    public string GetPKCS7InputDirectory(string basePath = "")
    {
        string PKCS7InputDirectory = basePath;
        if (string.IsNullOrEmpty(PKCS7InputDirectory))
            PKCS7InputDirectory = DocsPaUtils.Configuration.InitConfigurationKeys.GetValue("0", "BE_TEMP_PATH");

        PKCS7InputDirectory = System.IO.Path.Combine(PKCS7InputDirectory, @"Sign\Input\");
        return PKCS7InputDirectory.PathAsUnixPath();
        //return this.GetDirectoryFromSettings("PKCS7InputDirectory");
    }

    public DocsPaVO.documento.VerifySignatureResult Verify(string pathName, string basePath)
    {
        bool crlOnlineCheck = (this.CRLOnlineCheckEnabled());
        return Verify(pathName, crlOnlineCheck, basePath);
    }
    ...
    public DocsPaVO.documento.VerifySignatureResult Verify(string pathName, bool crlOnlineCheck, string basePath)
    {
        int rc = 0;
        ...
        try
        {
            ...
            mainP7Doc = new SignedDocument(Path.Combine(this.GetPKCS7InputDirectory(basePath), pathName));
            logger.Debug("loaded");
            ...
            if (mainP7Doc.Buffer != null)
            {
                ...
                if (retval) // verify signature
                {
                    ...
                    logger.Debug($"verify p7doc  write to output folder {Path.Combine(this.GetPKCS7OutputDirectory(basePath), previousP7Doc.DocumentFileName)}");
                    logger.Debug($"previousP7Doc.DocumentFileName: {previousP7Doc.DocumentFileName}");
                    previousP7Doc.SaveContentToFile(Path.Combine(this.GetPKCS7OutputDirectory(basePath), previousP7Doc.DocumentFileName));
    ...
    public DocsPaVO.documento.VerifySignatureResult VerifySignatureXADES_External(DocsPaVO.documento.FileDocumento fileDoc, DateTime dataDiRiferimento, string basePath)
    {
        ...
        try
        {
            ...
            string file = Path.Combine(this.GetPKCS7InputDirectory(basePath), pathName);
    ...
    public DocsPaVO.documento.VerifySignatureResult Verify_External(DocsPaVO.documento.FileDocumento fileDoc, DateTime dataVerifica, string basePath)
    {
        string pathName = fileDoc.name;
        string backName = fileDoc.name;
        bool isPadesAndCades = false;
        logger.Debug("Leggo il file input per backup");
        byte[] backup = File.ReadAllBytes(Path.Combine(this.GetPKCS7InputDirectory(basePath), pathName));
            ...
            || (type == DocsPaVO.documento.TsType.UNKNOWN && !Path.GetExtension(fileDoc.name).ToUpper().Equals(".P7M")))
        {
            ...
            string fileName_notsd = Path.Combine(this.GetPKCS7InputDirectory(basePath), Path.GetFileNameWithoutExtension(fileDoc.name));
            ...
            vsr = Verify(fileName_notsd, false, basePath);
            try
            {
        ...
        else
        {
            ...
            vsr = Verify(pathName, false, basePath);
        ...
        if (Helpers.getFileType(backup) != DocsPaVO.documento.TsType.PKCS)
        {
            ...
            File.WriteAllBytes(Path.Combine(this.GetPKCS7InputDirectory(basePath), pathName), Helpers.sbustaFileTimstamped(backup));
        }
    ...
    public string GetPKCS7OutputDirectory(string basePath = "")
    {
        string PKCS7OutputDirectory = basePath;
        if (string.IsNullOrEmpty(PKCS7OutputDirectory))
            PKCS7OutputDirectory = DocsPaUtils.Configuration.InitConfigurationKeys.GetValue("0", "BE_TEMP_PATH");

        PKCS7OutputDirectory = System.IO.Path.Combine(PKCS7OutputDirectory, @"Sign\Output\");
        return PKCS7OutputDirectory.PathAsUnixPath();
    ...
    /// <returns></returns>
    public DocsPaVO.documento.VerifySignatureResult VerifyM7M(string pathName, string basePath)
    {
        DocsPaVO.documento.VerifySignatureResult vsr = new DocsPaVO.documento.VerifySignatureResult();
        try
        {

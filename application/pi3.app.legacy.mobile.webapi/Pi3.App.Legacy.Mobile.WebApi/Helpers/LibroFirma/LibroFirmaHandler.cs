...
namespace Pi3.App.Legacy.Mobile.WebApi.Helpers.LibroFirma
{
    public class LibroFirmaHandler : MessageQueueBaseCommandHandler<LibroFirmaRequest>
    {
        ...
        protected override async Task InternalHandle(IServiceProvider serviceProvider, LibroFirmaRequest message)
        {
            ...
            var descUserLocker = delegatedIdUser != 0 ? $"{delegatedUserSurname} {DelegatedUserName} {LibroFirmaHandlerResources.SostitutoDi} {userSurname} {userName}" : $"{userSurname} {userName}";
            ...
            if (anagraficaEventoEntity.CHA_TIPO_EVENTO != "W")
            {
                ...
                if (anagraficaEventoEntity.CHA_TIPO_EVENTO == "E" || existTransmission == null)
                {
                    ...
                    if (anagraficaEventoEntity.CHA_TIPO_EVENTO == "E")
                    {
                        ...
                        noteGenerali = string.Format(LibroFirmaHandlerResources.NoteGeneraliEvento, anagraficaEventoEntity.DESCRIZIONE, noteGenerali);
                    }
                    ...
                    var aggregate = new Pi3.Core.AggregateModels.TrasmissioneAggregate.Trasmissione(idTenant.ToString(), DateTime.Now,
                    ...
                    await webMethodLoggerService.LogOK(
                             ...
                             idDocumentoPrincipale.ToString(),
                             string.Format(LibroFirmaHandlerResources.LogTrasmessoDocumento, (string.IsNullOrEmpty(profileEntity.VAR_SEGNATURA) ? idDocumentoPrincipale : profileEntity.VAR_SEGNATURA)),
                ...
                if (istanzaPassoFirmaEntity.CHA_AUTOMATICO == "1")
                {
                    DocsPaVO.LibroFirma.IstanzaProcessoDiFirma istanzaProcesso = new DocsPaVO.LibroFirma.IstanzaProcessoDiFirma() { docNumber = idDocumentoPrincipale.ToString(), statoProcesso = DocsPaVO.LibroFirma.TipoStatoProcesso.IN_EXEC };
                    var istanzaPassoFirma = new IstanzaPassoDiFirma()
                    {
                        RuoloCoinvolto = new DocsPaVO.utente.Ruolo() { idGruppo = istanzaPassoFirmaEntity.ID_RUOLO_COINVOLTO.ToString() },
                        UtenteCoinvolto = new DocsPaVO.utente.Utente() { idPeople = istanzaPassoFirmaEntity.ID_UTENTE_COINVOLTO.ToString() },
                        UtenteLocker = istanzaPassoFirmaEntity.ID_UTENTE_LOCKER.ToString(),
                        TipoFirma = istanzaPassoFirmaEntity.TIPO_FIRMA,
                        ApplicaSegnaturaPermanente = istanzaPassoFirmaEntity.CHA_POS_SEGNATURA ?? "0",
                        PosizioneSegnaturaPermanente = istanzaPassoFirmaEntity.VAR_POS_SEGNATURA,
                        IsAutomatico = istanzaPassoFirmaEntity.CHA_AUTOMATICO == "1"
                    };
                    //BusinessLogic.LibroFirma.LibroFirmaManager.EseguiPassoAutomaticoAsync(istanzaPassoFirma, istanzaProcesso);
                    //var eseguiPassoAutomaticoRequest = new EseguiPassoAutomatico(istanzaProcessoFirmaEntity.ID_ISTANZA.ToString());
                    
                    await mediator.Send(new EseguiPassoAutomatico(istanzaProcessoFirmaEntity.ID_ISTANZA.ToString()));
                }
            }
        }
            ...
            IMediator mediator)
        {
            ...
            var description = LibroFirmaHandlerResources.ConclusioneProcessoDocumento;
            if (await dbContext.IstanzaPassoFirmaEntities.AnyAsync(p => p.ID_ISTANZA_PROCESSO == istanzaProcessoFirmaEntity.ID_ISTANZA && p.CHA_AUTOMATICO == "1"))
            {
                ...
                description = LibroFirmaHandlerResources.ConclusioneProcessoAutomaticoDocumento;
            }
            ...
            #region Forse non serve - è già nella BusinessLogic
            /*
                ...
                && !await dbContext.IstanzaPassoFirmaEntities.AnyAsync(p => p.ID_ISTANZA_PROCESSO == istanzaProcessoFirmaEntity.ID_ISTANZA && p.STATO_PASSO == "CUT"))
            {
                ...
                if (idStatoAutomaticoSuccessivoLF != null)
                {
                    ...
                    DiagrammaStato diagramma = BusinessLogic.DiagrammiStato.DiagrammiStato.getDiagrammaById(statoEntity.ID_DIAGRAMMA.ToString());
                    BusinessLogic.DiagrammiStato.DiagrammiStato.salvaModificaStato(docnumber, idStatoAutomaticoSuccessivoLF.ToString(), diagramma, infoUtente.userId, infoUtente, string.Empty);
           ...
           IMediator mediator)
        {
            ...
            await webMethodLoggerService.LogOK("CONCLUSIONE_PROCESSO_LF_ALLEGATO", istanzaProcessoFirmaEntity.ID_DOCUMENTO.ToString(), LibroFirmaHandlerResources.ConclusioneProcessoAllegato,
                null, null, null, null, null, null, null, null, istanzaProcessoFirmaEntity.CONCLUSO_IL);
            ...
            try
            {
                ...
                foreach (var ts in aggregate.TrasmissioniSingole)
                {
                    await webMethodLoggerService.LogOK("TRASM_DOC_" + ts.RagioneTrasmissione.Nome.ToUpper().Replace(" ", "_"),
                        ...
                        string.Format(LibroFirmaHandlerResources.LogTrasmessoDocumento, docname),
                        ts.Id,
                        null,
                        modello.NO_NOTIFY == "1");
                }

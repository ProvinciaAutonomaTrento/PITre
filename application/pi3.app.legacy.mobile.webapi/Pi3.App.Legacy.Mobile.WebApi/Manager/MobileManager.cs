...
using Pi3.Infrastructure.Tibco.Services.File.FirmaRemota.ValueObjects;
using Pi3.Infrastructure.Tibco.Services.File.FirmaRemota2;
using Serilog;
using System.Collections;

namespace Pi3.App.Legacy.Mobile.WebApi.Manager;

public class MobileManager
{
    private static Serilog.ILogger logger = Serilog.Log.ForContext(typeof(MobileManager));
    ...
    public static async Task<DelegheResponse> listaDelegheAssegnate(string token, string statoDel, string tipoDel)
    {
        logger.Information("listaDeleghe - begin");
        try
        {
            ...
            if (authInfoArray[0].Equals("0"))
                ruolo = (Ruolo)BusinessLogic.Utenti.UserManager.getRuoliUtente(utente.idPeople)[0];
            else
            ...
            //dan  
            if (authInfoArray.Length > 8) // lo faccio solo se ho delegato , per rimanere conservativo
                infoUtente = Utils.getInfoUtenteFromToken(token.Substring(4));
            //dan
            DelegheResponse resp = new DelegheResponse();
            string tipoDelega = tipoDel;// "assegnate";
            ...
            {
                logger.Debug("Delega: decorr: {0} -> scad :{1} codDel {2} - IdRuoloDel: {3}", temp.dataDecorrenza, temp.dataScadenza, temp.codiceDelegante, temp.id_ruolo_delegante);
                logger.Debug("Delegato {0} - Ruolo {1} - Ruolo id {2}", temp.cod_utente_delegato, temp.cod_ruolo_delegato, temp.id_ruolo_delegato);
            ...
            logger.Information("listaDeleghe - end");

            return resp;
        }
        catch (Exception e)
        {
            logger.Error(e, "listaDeleghe - eccezione: ");
    ...
    {
        logger.Information("creaDelega - begin");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            */
            ...
            logger.Information("creaDelega - end");
    ...
    {
        logger.Information("revocaDelega - begin");
        try
        {
            bool res = false;
            DocsPaVO.utente.Utente utente = null;
            ...
            logger.Information("revocaDelega - end");
    ...
    {
        logger.Information("getListaModelliDelega - begin");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            logger.Information("getListaModelliDelega - end");
            return resp;
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("getListaRuoli - begin");
        try
        {
            string authInfoString = Utils.Decrypt(token.Substring(4));
            string[] authInfoArray = authInfoString.Split('|');
            ...
            logger.Information("getListaRuoli - end");
    ...
    {
        logger.Information("getListaUtenti - begin");
        try
        {
            string authInfoString = Utils.Decrypt(token.Substring(4));
            string[] authInfoArray = authInfoString.Split('|');
            ...
            logger.Information("getListaUtenti - end");
            return resp;
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("creaDelegaDaModello - begin");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            logger.Information("CreaDelegaDaModello - end");
            return resp;
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("ricercaUtenti - begin");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            logger.Information("ricercaUtenti - end");
            return res;
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("getDocInfo - BEGIN");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            logger.Information("getDocInfo - end ");
            return resp;
        }
        catch (Exception e)
        {
    ...
    public static async Task<GetFileResponse> getFile(string token, string idDoc, string getSignedFile, string basePath)
    {
        logger.Information("getFile - BEGIN");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            if (sbusta)
                fileDocumento = BusinessLogic.Documenti.FileManager.getFile(fileRequest, infoUtente, basePath);
            else
                fileDocumento = BusinessLogic.Documenti.FileManager.getFileFirmato(fileRequest, infoUtente, false, basePath);
            ...
            logger.Information("getFile - END");
    ...
    {
        logger.Information("getFascInfo - START");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            logger.Information("getFascInfo - end");
            return response;
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("rimuoviNotifica - begin");
        try
        {
            DocsPaDB.Query_DocsPAWS.NotificationDB notification = new NotificationDB();
            logger.Information("rimuoviNotifica - end");
    ...
    public static async Task<ADLActionResponse> ADLAction(string token, AdlActionRequest request)
    {
        ...
        {
            logger.Error("Errore in addToAdl, {0} {1}", e.Message, e.StackTrace);
            response.Code = AddToADLResponseCode.SYSTEM_ERROR;
        }
        logger.Information("ADLAction - END");
        return response;
    ...
    {
        logger.Information("getTokenCondivisioneDocumento - BEGIN");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            logger.Information("getTokenCondivisioneDocumento - END");
            return retval;
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("ctrlTokenCondivisioneDoc - BEGIN");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
    ...
    public static async Task<HSMSignResponse> hsmSign(string token, HSMSignRequest request, IFirmaRemota2Service firmaRemotaService)
    {
        try
        {
            logger.Information("hsmSign - begin");
            ...
            res.Code = ((await BusinessLogic.Documenti.FileManager.HSM_Sign(infoUtente,
                                                                     fr,
                                                                     request.cofirma,
                                                                     request.timestamp,
                                                                     request.TipoFirma,
            ...
            logger.Information("hsmSign - end: " + res.Code.ToString());
            return res;
        }
        catch (Exception e)
        {
    ...
    public static async Task<HSMSignResponse> hsmRequestOTP(string token, HSMSignRequest request, IFirmaRemota2Service firmaRemotaService)
    {
        try
        {
            logger.Information("hsmRequestOTP - begin");
            HSMSignResponse res = new HSMSignResponse();
            ...
            res.Code = (await BusinessLogic.Documenti.FileManager.HSM_RequestOTP(request.AliasCertificato, request.DominioCertificato, firmaRemotaService)) ?
                                                                    HSMSignResponseCode.OK :
                                                                    HSMSignResponseCode.SYSTEM_ERROR;

            logger.Information("hsmRequestOTP - end: " + res.Code.ToString());
            return res;
    ...
    public static async Task<HSMSignResponse> hsmVerifySign(string token, HSMSignRequest request)
    {
        try
        {
            logger.Information("hsmVerifySign - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            logger.Information("hsmVerifySign - end: " + res.Code.ToString());
            return res;
        }
        catch (Exception e)
        {
    ...
    public static async Task<HSMSignResponse> hsmInfoSign(string token, HSMSignRequest request)
    {
        try
        {
            logger.Information("hsmInfoSign - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            logger.Information("hsmInfoSign - end: " + res.Code.ToString());
            return res;
        }
        catch (Exception e)
        {
    ...
    public static async Task<HSMSignResponse> hsmGetMementoForUser(string token)
    {
        try
        {
            logger.Information("hsmGetMementoForUser - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            res.Code = HSMSignResponseCode.OK;
            logger.Information("hsmGetMementoForUser - end: " + res.Code.ToString());
            return res;
        }
        catch (Exception e)
        {
    ...
    {
        bool result = false;
        logger.Information("isAllowedOTP - begin");
        DocsPaVO.utente.Utente utente = null;
        DocsPaVO.utente.InfoUtente infoUtente = null;
        DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
        ...
        logger.Information("isAllowedOTP - end");
        return result;

    }

    public static async Task<LibroFirmaResponse> GetLibroFirmaElements(string token, Model.LibroFirmaRequest request)
    {
        try
        {
            logger.Information("GetLibroFirmaElements - start");
            ...
            //dan  
            if (authInfoArray.Length > 8) // lo faccio solo se ho delegato , per rimanere conservativo
                infoUser = Utils.getInfoUtenteFromToken(token.Substring(4));
            //dan
            ...
            logger.Information("GetLibroFirmaElements - end");
            return res;
        }
        catch (Exception e)
        {
    ...
    private static LibroFirmaResponse getElemLibroFirma(InfoUtente infoUser, Model.LibroFirmaRequest request)
    {
        int totalRecordCount = 0;
        LibroFirmaResponse res = new LibroFirmaResponse();
        res.Elements = new List<LibroFirmaElement>();
    ...
    public static async Task<LibroFirmaResponse> CambiaStatoElementoLibroFirma(string token, LibroFirmaCambiaStatoRequest request)
    {
        ...
        try
        {
            logger.Information("CambiaStatoElementoLibroFirma - begin");
        ...
        logger.Information("CambiaStatoElementoLibroFirma - end");

        return res;
    }

    public static async Task<LibroFirmaResponse> RespingiSelezionatiElementiLf(string token, Model.LibroFirmaRequest request)
    {
        logger.Information("RespingiSelezionatiElementiLf - begin");
        LibroFirmaResponse res = new LibroFirmaResponse();
        string message = string.Empty;
        bool result = false;
        try
    ...
    public static async Task<LFCambiaStatiResponse> LFCambiaStati(string token, LFCambiaStatiRequest request)
    {
        ...
        try
        {
            ...
            if (request != null && request.Elementi != null && request.Elementi.Length > 0)
            {
                ...
                foreach (ElementToChange elementX in request.Elementi)
                {
                    ...
                    elemento.IdTrasmSingola = elementX.Elemento.IdTrasmSingola;
        ...
        logger.Information("LFCambiaStati - end");

        return res;
    }

    public static async Task<LibroFirmaResponse> FirmaSelezionatiElementiLf(string token, Model.LibroFirmaRequest request, MediatR.IMediator mediator, IClaimsPrincipalService claimsPrincipalService, IWebMethodLoggerService webMethodLoggerService)
    {
        LibroFirmaResponse res = new LibroFirmaResponse();
        string message = string.Empty;
        try
        {
            logger.Information("FirmaSelezionatiElementiLf - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUser = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            if (elements != null && elements.Count > 0)
            {
                ...
                if (listElementAdvancementProcess != null && listElementAdvancementProcess.Count > 0)
                {
                    ...
                    await PutElectronicSignature(fileReqToSign, infoUser, isAdvancementProcess, mediator, claimsPrincipalService, webMethodLoggerService);
                }
                #endregion
        ...
        logger.Information("FirmaSelezionatiElementiLf - end");
        return res;
    }

    private static async System.Threading.Tasks.Task PutElectronicSignature(List<DocsPaVO.documento.FileRequest> approvingFiles, DocsPaVO.utente.InfoUtente infoUtente, bool isAdvancementProcess, MediatR.IMediator mediator, IClaimsPrincipalService claimsPrincipalService, IWebMethodLoggerService webMethodLoggerService)
    {
        string message;
        foreach (DocsPaVO.documento.FileRequest fileReq in approvingFiles)
        {
            ...
            if (BusinessLogic.LibroFirma.LibroFirmaManager.PutElectronicSignature(fileReq, infoUtente, isAdvancementProcess, out message))
            {
                ...
                if (fileReq.inLibroFirma)
                {
                    ...
                    await mediator.Send(
                        new MessageQueueCommandWrapper(new Pi3.App.Legacy.Mobile.WebApi.Requests.LibroFirmaRequest(claimsPrincipalService.Current)
                        {
                            IdProfile = fileReq.docNumber,
                            Evento = method2,
                        }));
                }
                ...
                await webMethodLoggerService.LogOK(method2, fileReq.docNumber, description2, null, "PITRE");
            }
    ...
    {
        logger.Information("ExistsElementWithSignCades - begin");
        bool result = false;
        try
        {
            DocsPaVO.utente.Utente utente = null;
        ...
        logger.Information("ExistsElementWithSignCades - end");
        return result;
    ...
    {
        logger.Information("ExistsElementWithSignPades - begin");
        ...
        logger.Information("ExistsElementWithSignPades - end");
        return result;

    }

    public static async Task<HSMMultiSignResponse> HsmMultiSign(string token, HSMSignRequest request, IFirmaRemota2Service firmaRemotaService, Core.Services.Configuration.IConfigurationService configurationService, string temporaryRootPath, MediatR.IMediator mediator, IClaimsPrincipalService claimsPrincipalService, IWebMethodLoggerService webMethodLoggerService)
    {
        try
        {
            logger.Information("HsmMultiSign - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            string repositoryRootPath = await configurationService.GetValue<string>("REPOSITORY_ROOT_PATH", true);
            ...
            if (idDocuments != null && idDocuments.Count > 0)
            {
                ...
                string tokenHSM = BusinessLogic.Documenti.FileManager.HSM_OpenMultiSignSession(infoUtente, frs, request.cofirma, request.timestamp, request.TipoFirma, configurationService, temporaryRootPath);
                DocsPaVO.documento.FirmaResult[] firme = await BusinessLogic.Documenti.FileManager.HSM_SignMultiSignSession(infoUtente, frs, tokenHSM, request.AliasCertificato, request.DominioCertificato, request.OtpFirma, request.PinCertificato, request.cofirma, firmaRemotaService, temporaryRootPath, repositoryRootPath);
               ...
               if (firme != null && firme.Length > 0)
                {
                    ...
                    res.infoFirma = new List<InfoDocFirmaHSM>();
                    int irax = 0;
                    foreach (FirmaResult firmaX in firme)
                    {
                        if (!string.IsNullOrWhiteSpace(firmaX.errore) && firmaX.errore.ToUpper().StartsWith("TRUE"))
                        {
                            ...
                            if (firmaX.fileRequest != null)
                            {
                                //Inserisco nella coda del motore di Libro firma
                                await mediator.Send(
                                    new MessageQueueCommandWrapper(new Pi3.App.Legacy.Mobile.WebApi.Requests.LibroFirmaRequest(claimsPrincipalService.Current)
                                    {
                                        IdProfile = firmaX.fileRequest.docNumber,
                                        Evento = method,
                                    }));

                                await webMethodLoggerService.LogOK(method, firmaX.fileRequest.docNumber, description, null, "PITRE");
                            }
                        }
                        else
                        {
                        ...
                        res.infoFirma.Add(rispX);
                        irax++;
                        resFirmaHSMMulti.Add(rispX);
                    }
                    //res.infoFirma = (InfoDocFirmaHSM[])resFirmaHSMMulti.ToArray();
                    //res.infoFirma = resultArray;
                    //res.infoFirma = new InfoDocFirmaHSM[resFirmaHSMMulti.Count];
                    //for (int i = 0; i <resFirmaHSMMulti.Count; i++)
                    //{
                    //    res.infoFirma[i] = (InfoDocFirmaHSM)resFirmaHSMMulti[i];
                    //}
            ...
            logger.Information("HsmMultiSign - end");
            return res;
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("LibroFirmaIsAutorized - begin");
        bool result = false;
        try
        {
            string authInfoString = Utils.Decrypt(token.Substring(4));
        ...
        logger.Information("LibroFirmaIsAutorized - end");
        return result;
    ...
    {
        logger.Information("resetPassword - begin");
        ...
        try
        {
            ...
            else
            {
                ...
                logger.Information("cambioPassword - END");
                return retval;
            }
    ...
    public static async Task<DocsPaVO.Mobile.Responses.LoginResponse> login(Model.AuthenticateRequest loginRequest)
    {
        logger.Information("LoginResponse - begin");
        DocsPaVO.Mobile.Responses.LoginResponse retval = null;
        DocsPaVO.utente.UserLogin.LoginResult loginResult = DocsPaVO.utente.UserLogin.LoginResult.OK;
        string ipAddress = "";
        try
        {
            ...
            usrLog.Password = loginRequest.Password;
            usrLog.UserName = loginRequest.Username;
            usrLog.CodiceFiscale = loginRequest.CodiceFiscale;
            ...
            if (utente != null)
            {
                ...
                string set_data_vista_grd = DocsPaVO.Settings.AppSettings.Instance.SET_DATA_VISTA_GRD;
            ...
            logger.Information("LoginResponse - END");
    ...
    {
        logger.Information("cambioPassword - begin");
        DocsPaVO.Mobile.Responses.LoginResponse retval = null;
        DocsPaVO.utente.UserLogin.LoginResult loginResult = DocsPaVO.utente.UserLogin.LoginResult.OK;
        string ipAddress = "";
        try
    ...
    {
        logger.Information("resetPassword get OTP- begin");
        DocsPaVO.Mobile.Responses.LoginResponse retval = null;
        DocsPaVO.utente.UserLogin.LoginResult loginResult = DocsPaVO.utente.UserLogin.LoginResult.OK;
        string ipAddress = "";
        string email = "";
        try
        {
            ...
            else
            {
                ...
                /*    if (retval.Code == LoginResponseCode.OK)
                  ...
                  */
                logger.Information("cambioPassword get OTP - END");
                return retval;
            }
    ...
    {
        logger.Information("verifica disponibilita aggironaemnto mobile- begin");
    ...
    public static async Task<DocsPaVO.Mobile.Responses.LoginResponse> esercitaDelega(string token, string idDelega, string idRuoloDElegante, string idDlegante)
    {
        ...
        try
        {
            ...
            string[] authInfoArray = authInfoString.Split('|');
            //dan test
            //    utente = BusinessLogic.Utenti.UserManager.getUtente("LULUCIANI", "361"); //BusinessLogic.Utenti.UserManager.getUtente(authInfoArray[5], authInfoArray[4]);
            //ori  utente = BusinessLogic.Utenti.UserManager.getUtente(usernamenDlegante, authInfoArray[4]); //BusinessLogic.Utenti.UserManager.getUtente(authInfoArray[5], authInfoArray[4]);
            utente = BusinessLogic.Utenti.UserManager.getUtenteById(idDlegante); //BusinessLogic.Utenti.UserManager.getUtente(authInfoArray[5], authInfoArray[4]);
            if (idRuoloDElegante.Equals("0"))
                ruolo = (Ruolo)BusinessLogic.Utenti.UserManager.getRuoliUtente(utente.idPeople)[0];
            else
                ruolo = BusinessLogic.Utenti.UserManager.getRuoloById(idRuoloDElegante);  // BusinessLogic.Utenti.UserManager.getRuoloById(authInfoArray[0]);
            infoUtente = new DocsPaVO.utente.InfoUtente(utente, ruolo);
            //  infoUtente.delegato = getdelegatoFromToken();
            //delegato 


            //string authInfoDelegatoString = Utils.Decrypt(token.Substring(4));
            //string[] authInfoDelegatoArray = authInfoString.Split('|');
            DocsPaVO.utente.Utente utenteDelegato = null;
            DocsPaVO.utente.Ruolo ruoloDelegato = new DocsPaVO.utente.Ruolo();
            DocsPaVO.utente.InfoUtente infoUtenteDelegato = null;
            utenteDelegato = BusinessLogic.Utenti.UserManager.getUtente(authInfoArray[5], authInfoArray[4]);
            ruoloDelegato = BusinessLogic.Utenti.UserManager.getRuoloById(authInfoArray[0]);
            infoUtenteDelegato = new DocsPaVO.utente.InfoUtente(utenteDelegato, ruoloDelegato);

            UserInfo uDelegato = UserInfo.buildInstance(utenteDelegato);
            uDelegato.Token = token;
            infoUtente.delegato = infoUtenteDelegato;

            //
            UserLogin userlogin = new UserLogin();
            userlogin.UserName = utente.userId;
            userlogin.IdAmministrazione = infoUtente.idAmministrazione;
            //utente mi finisce direttamente in sessione 
            utente = BusinessLogic.Deleghe.DelegheManager.esercitaDelega(infoUtente, userlogin, null, idDelega, idRuoloDElegante, out loginResult);
            retval = new DocsPaVO.Mobile.Responses.LoginResponse(utente, loginResult);
            ...
            //   retval.UserInfo.Token = Utils.CreateAuthToken(utente, ruolo);
            retval.UserInfo.Token = Utils.CreateAuthToken(infoUtente);

            retval.UserInfo.DelegatoInfo = uDelegato;

            return retval;
        }
        catch (Exception e)
        {
            logger.Error("Login - eccezione: " + e);
            return DocsPaVO.Mobile.Responses.LoginResponse.ErrorResponse;
        }
    }

    public static async Task<bool> terminaDelega(string token)
    {
        logger.Information("terminaDelega - begin");
        ...
        try
        {
            result = BusinessLogic.Deleghe.DelegheManager.dismettiDelega(infoUtente, infoUtente.userId);
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("getListaAmmByUser - begin");
    ...
    {
        logger.Information("Logout - BEGIN");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            string authInfoString = Utils.Decrypt(token.Substring(4));
    ...
    public static async Task<int> DelegaCheckAttiva(string token)
    {
        DocsPaVO.utente.InfoUtente infoUtente = null;
        infoUtente = Utils.getInfoUtenteFromToken(token.Substring(4));
        return BusinessLogic.Deleghe.DelegheManager.checkDelegaAttiva(infoUtente);
    }

    public static async Task<ToDoListResponse> getTodoList(string token, int requestedPage, int pageSize)
    {
        logger.Information("getTodoList - Begin");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            logger.Information("getTodoList - end");
            return res;
        }
        catch (Exception e)
        {
    ...
    {
        logger.Information("cambiaRuolo - BEGIN");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            if (authInfoArray.Length > 8) // lo faccio solo se ho delegato , per rimanere conservativo
            {
                DocsPaVO.utente.InfoUtente infoUtented = Utils.getInfoUtenteFromToken(token.Substring(4));
                infoUtente.delegato = infoUtented.delegato;
            }
            //dan
            DocsPaVO.Mobile.Responses.LoginResponse retval = null;
            DocsPaVO.utente.UserLogin.LoginResult loginResult = DocsPaVO.utente.UserLogin.LoginResult.OK;
            ...
            logger.Information("cambiaRuolo - END");
            return retval;
    ...
    public static async Task<ArrayList> ListaIstanze()
    {
        ...
        try
        {
            //PER BUILD PITRE ESTRAGGO IL FILE DAL REPOSITORY
            //string pathListaIstanze = HttpContext.Current.Server.MapPath("xml/listaIstanze.xml");
            logger.Debug("Recupero lista istanze");
            string pathListaIstanze = Path.Combine(DocsPaVO.Settings.AppSettings.Instance.LISTA_ISTANZE_PATH, @"listaIstanze.xml");
    ...
    {
        logger.Information("getRicSalvate - START");
        try
        {
            ...
            logger.Information("getRicSalvate - end");
            return resp;
        }
        catch (Exception e)
        {
    ...
    public static async Task<RicercaResponse> ricerca(string token, Model.RicercaRequest request)
    {
        try
        {
            logger.Information("ricerca - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            if (true)
            {
                System.Xml.Serialization.XmlSerializer seri = new System.Xml.Serialization.XmlSerializer(typeof(Model.RicercaRequest));
                using (var sww = new System.IO.StringWriter())
                {
                    using (System.Xml.XmlWriter writer = System.Xml.XmlWriter.Create(sww))
                    {
            ...
            Model.RicercaStrategies.RicercaStrategy.GetStrategy(request).buildResponse(infoUtente, request, resp);
            resp.Code = RicercaResponseCode.OK;
            logger.Information("ricerca - end");
            return resp;
        }
        catch (Exception e)
        {
    ...
    public static async Task<RicercaFascResponse> getDocsInFasc(string token, string idFasc, int pagesize, int numpage, string text)
    {
        try
        {
            ...
            Model.RicercaRequest request = new Model.RicercaRequest();
    ...
    {
        logger.Information("begin MobileManager.setDataVistaSP_TV");
        try
        {
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            ...
            logger.Information("end MobileManager.setDataVistaSP_TV");
            return resp;
        }
        catch (Exception e)
        {
    ...
    public static async Task<AccettaRifiutaTrasmResponse> accettaRifiutaTrasm(string token, Model.AccettaRifiutaTrasmRequest request)
    {
        logger.Information("begin MobileManager.accettaRifiutaTrasm");
        try
        {
            ...
            logger.Information("end MobileManager.accettaRifiutaTrasm");
            return resp;
        }
        catch (Exception e)
        {
    ...
    public static async Task<AccettaRifiutaTrasmResponse> smista(string token, SmistaRequest request)
    {
        try
        {
            logger.Information("smista - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            if (result)
            {
                if (!string.IsNullOrEmpty(request.IdModelloTrasm))
                {
                    ...
                    if (DocsPaVO.Settings.AppSettings.Instance.URL_PATH_IS != null)
                        path = DocsPaVO.Settings.AppSettings.Instance.URL_PATH_IS.ToString();
                    else
                        path = request.Path;
    ...
    {
        logger.Information("getListaModelliTrasm - begin");
        try
        {
            ...
            //dan  
            if (authInfoArray.Length > 8) // lo faccio solo se ho delegato , per rimanere conservativo
                iu = Utils.getInfoUtenteFromToken(token.Substring(4));
            //dan
            ...
            logger.Information("getListaModelliTrasm - end");
            //PassTrough
            return resp;
        }
        catch (Exception e)
    ...
    public static async Task<EseguiTrasmResponse> eseguiTrasm(string token, EseguiTrasmRequest request)
    {
        try
        {
            logger.Information("eseguiTrasm - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            logger.Information("eseguiTrasm - end");
            return response;
        }
        catch (Exception e)
        {
    ...
    public static async Task<EseguiTrasmResponse> eseguiTrasmDiretta(string token, EseguiTrasmDirettaRequest request)
    {
        try
        {
            logger.Information("eseguiTrasmDiretta - begin");
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
            ...
            logger.Information("eseguiTrasmDiretta - end");
            return response;
        }
        catch (Exception e)
        {
    ...
    public static async Task<RicercaSmistamentoResponse> GetListaCorrispondentiVeloce(string token, GetListaCorrVeloceRequest request)
    {
        try
        {
            logger.Information("GetListaCorrispondentiVeloce - begin");
            logger.Information("descrizione: " + request.Descrizione);
            logger.Information("ragione: " + request.Ragione);
            DocsPaVO.utente.Utente utente = null;
            DocsPaVO.utente.InfoUtente infoUtente = null;
            DocsPaVO.utente.Ruolo ruolo = new DocsPaVO.utente.Ruolo();
    ...
    public static async Task<ListaRuoliByUserResponse> getListaRuoliUser(string token, string idUtenteDaCercare)
    {
        try
        {
            ...
            logger.Information("getListaRuoliUser - begin");
            ...
            logger.Information("getListaRuoliUser - end");
            return resp;
        }
        catch (Exception e)
        {
    ...
    public static async Task<ListaRagioniResponse> getListaRagioni(string token, string idObject, string DoF)
    {
        ...
        try
        {
            logger.Information("getListaRagioni - begin");

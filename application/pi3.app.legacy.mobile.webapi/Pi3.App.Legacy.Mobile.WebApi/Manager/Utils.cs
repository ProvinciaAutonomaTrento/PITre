    ...
    public class Utils
    {
        ...
        public static DocsPaVO.utente.InfoUtente getInfoUtenteFromToken(string tokenDiAutenticazione)
        {
            try
            {
                ...
                if (arrayInfoUtente.Length > 8)
                {
                    DocsPaVO.utente.InfoUtente infoUtenteDelegato = new DocsPaVO.utente.InfoUtente();
                    infoUtenteDelegato.idCorrGlobali = arrayInfoUtente[8];
                    infoUtenteDelegato.idPeople = arrayInfoUtente[9];
                    infoUtenteDelegato.idGruppo = arrayInfoUtente[10];
                    infoUtenteDelegato.dst = arrayInfoUtente[11];
                    infoUtenteDelegato.idAmministrazione = arrayInfoUtente[12];
                    infoUtenteDelegato.userId = arrayInfoUtente[13];
                    infoUtenteDelegato.sede = arrayInfoUtente[14];
                    infoUtenteDelegato.urlWA = arrayInfoUtente[15];
        ...
        public static string CreateAuthToken(InfoUtente infoUtente)
        {
            //string token = DocsPaUtils.Security.SSOAuthTokenHelper.Generate(utente.userId);

            //  string ss_token = GetToken(GetUtente(infoutente),getRuoloUtente(infoutente));
            //  return ss_token;

            Utente utente = GetUtente(infoUtente);
            Ruolo ruolo = getRuoloUtente(infoUtente);
            bool okRuolo = false;
            if (utente != null && utente.ruoli != null && utente.ruoli.Count > 0)
            {
                foreach (DocsPaVO.utente.Ruolo rl in utente.ruoli)
            ...
            if (okRuolo)
            {
                ...
                try
                {
                    string clearToken = getClearToken(utente, ruolo);
                    if (infoUtente.delegato != null)
                    {
                        clearToken += "|" + getClearToken(GetUtente(infoUtente.delegato), getRuoloUtente(infoUtente.delegato));
                    }
        ...
        private static string getClearToken(Utente utente, Ruolo ruolo)
        {
            string clearToken = string.Empty;
            clearToken += ruolo.systemId + "|";
            clearToken += utente.idPeople + "|";
        ...
        public static Utente GetUtente(InfoUtente ut)
        {
            Utente utente = null;
            ...
            if (ut != null)
            {
                ...
                utente.userId = ut.userId;
                utente.dst = ut.dst;
                utente.idAmministrazione = ut.idAmministrazione;
                utente.sede = ut.sede;
                utente.urlWA = ut.urlWA;
                utente.matricola = ut.matricola;
                utente.extApplications = ut.extApplications;
                utente.codWorkingApplication = ut.codWorkingApplication;
            }
        ...
        public static Ruolo getRuoloUtente(InfoUtente ut)
        {
            Ruolo ruo = null;
            if (ut != null)
            {
                ruo = new Ruolo();
                ruo.systemId = ut.idCorrGlobali;
                ruo.idGruppo = ut.idGruppo;
                ruo.idAmministrazione = ut.idAmministrazione;
            }
            return ruo;
        }

...
{
        private static ILogger logger = Serilog.Log.ForContext(typeof(Utils));
...
//			}
	...
	public void getRuoAut(out DataSet dataSet, DocsPaVO.utente.Ruolo ruolo, string idRegistro, string idNodoTitolario, DocsPaVO.trasmissione.TipoOggetto tipoOggetto)
	{
		...
		{
                //Emanuela 07/04/2014: nel caso di trasmissione massiva di fascicoli, idNodoTitolario � vuoto e la query non fornisce alcun
                //risulato.
...
//			}
	...
	private string getQueryAut(DocsPaVO.trasmissione.TipoOggetto tipoOggetto,string idRegistro,string idNodoTitolario,DocsPaVO.utente.Ruolo ruolo,System.Collections.ArrayList listaIdUO, string tipoGerarchia)
	{
		...
		//se la chiave sul web.config � assente oppure � impostata a "0"
		if (DocsPaVO.Settings.AppSettings.Instance.EST_VIS_SUP_PARI_LIV == null || DocsPaVO.Settings.AppSettings.Instance.EST_VIS_SUP_PARI_LIV.Equals("0"))
		{
			if(listaIdUO != null && listaIdUO.Count>0)
			{
				queryString=queryString+" A.ID_UO IN (";
                    ...
                    int i = 0;
                    foreach (string id in listaIdUO)
                    {
                        queryString += id;
                        if (i < listaIdUO.Count - 1)
                        {
		...
		{
			/* caso nuovo per ANAS -- si estende la visibilit� a tutti i ruoli dello stesso livello 
		...
		{
			//nel caso di una sottocartella THING contiene la system_id del fascicolo a cui � associata,
			//mentre nel caso di un fascicolo contiene la system_id del nodo di titolario a cui � associata
			queryString=queryString+" AND C.PERSONORGROUP=A.ID_GRUPPO AND C.THING="+idNodoTitolario+" AND C.ACCESSRIGHTS > 0";
			if(idRegistro!=null && idRegistro != "")
			{
				queryString=queryString+" AND D.ID_RUOLO_IN_UO=A.SYSTEM_ID"; 
...
//		}
	...
	public string SelectDBTime()
	{
        ...
        public DateTime SelectDBDateTime()
        {
            ...
            string dbmms = DocsPaVO.Settings.AppSettings.Instance.DBType.ToUpper();

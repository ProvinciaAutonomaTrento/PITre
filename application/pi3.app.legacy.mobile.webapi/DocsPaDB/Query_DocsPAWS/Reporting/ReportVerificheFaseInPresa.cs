...
class ReportVerificheFaseInPresa
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(ReportVerificheFaseInPresa));

    [ReportDataExtractorMethod(ContextName = "ReportVerificheFaseInPresa")]
    public DataSet GetReportConservazione(InfoUtente userInfo, List<FiltroRicerca> searchFilters)
    {
        ...
        using (DBProvider dbProvider = new DBProvider())
        {
            ...
            Query query = InitQuery.getInstance().getQuery("S_REPORT_VERIFICHE_IN_FASE");
        ...
        if (dataSet.Tables[0].Rows.Count > 0)
        {
            ...
            {
                if (r["VER_LEGG_ESITO"] != null)
                {
                    if (r["VER_LEGG_ESITO"].Equals("0"))
                    {
                        r["VER_LEGG_ESITO"] = "Negativo";
                    }

                    else if (r["VER_LEGG_ESITO"].Equals("1"))
                    {
                        r["VER_LEGG_ESITO"] = "Positivo";
                    }

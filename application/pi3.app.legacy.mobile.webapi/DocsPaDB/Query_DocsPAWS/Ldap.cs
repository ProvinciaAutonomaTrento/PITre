...
public class Ldap
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Ldap));
    /// <summary>
    /// 
    /// </summary>
    public Ldap()
    ...
    /// <param name="userName"></param>
    /// <returns></returns>
    public DocsPaVO.Ldap.LdapUserConfig GetLdapUserConfigByName(string userName, bool loginCodiceFiscale)
    {
        try
        {
            ...
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                ...
                if (loginCodiceFiscale)
                    queryDef = DocsPaUtils.InitQuery.getInstance().getQuery("S_GET_LDAP_USER_CONFIG_BY_CF");

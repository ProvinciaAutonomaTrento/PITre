...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Utenti));
    ...
    /// <summary>
    /// controlla se i dettagli di un corrispondente occassionale sono gi� esistenti.
    /// </summary>
    /// <returns></returns>
    ...
    public void IsCodRubricaPresente(string codRubrica, string tipoCorr, string idAmm, string idReg, bool inRubricaComune, out DataSet dataSet)
    {
        ...
        // Il pezzo di codice che controlla l'id registro era stato commentato. Ora � stato decommentato e modificato
        // in modo che renda possibile inserire un corrispondente su TUTTI e su un registro / RF (l'univoci� quindi �
        ...
        /* Emanuela 26/06/2014: commentato perch� nel verificarsi di ciascuna condizione quello che fa � aggiungere sempre AND A.DTA_FINE IS NULL
        ...
        {
            logger.Debug("ERRORE in IsCodRubricaPresente il tipoCorr � null");
        ...
        if (dataSet.Tables["CODICE"].Rows.Count > 0)
        {
            if (inRubricaComune)
            {
                foreach (DataRow row in dataSet.Tables["CODICE"].Rows)
                {
                    ...
                    {
                        logger.Debug("Codice gi� presente in rubrica (Query - IsCodRubricaPresente)");
                        throw new Exception("Codice gi� presente in rubrica");
                    }
                    else
                    {
            ...
            else
            {
                foreach (DataRow row in dataSet.Tables["CODICE"].Rows)
                {
                    ...
                    {
                        logger.Debug("Codice gi� presente in Rubrica Comune (Query - IsCodRubricaPresente)");
                        throw new Exception("Corrispondente gi� presente in Rubrica Comune");
    ...
    {
        bool result = true;
        string prefix = DocsPaVO.Settings.AppSettings.Instance.prefissoCorrOccasionale;
    ...
    private void IsCodeCorrect(string codRubrica)
    {
        ...
        char[] separator = { ';' };
        String[] prefissi = DocsPaVO.Settings.AppSettings.Instance.prefissiRubricaRiservati.Split(separator);
        for (int i = 0; i < prefissi.Length; i++)
        {
            if (codRubrica.ToUpper().StartsWith(prefissi[i]))
            {
                ...
                throw new Exception("Il codice rubrica inserito non pu� iniziare per " + prefissi[i]);
            }
        }
    }
    ...
    public string InsertCorr(DocsPaVO.utente.Corrispondente corr, DocsPaVO.utente.Corrispondente parent)
    {
        ...
        {
            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("I_DPACorrglobali");
            q.setParam("param1", DocsPaDbManagement.Functions.Functions.GetSystemIdColName() + " NUM_LIVELLO, CHA_TIPO_IE, ID_REGISTRO, ID_AMM, VAR_DESC_CORR, ID_OLD, DTA_INIZIO, ID_PARENT, VAR_CODICE, CHA_TIPO_CORR, CHA_TIPO_URP, CHA_PA, VAR_CODICE_AOO, VAR_COD_RUBRICA, CHA_DETTAGLI, VAR_EMAIL, VAR_CODICE_AMM, VAR_CODICE_ISTAT,VAR_DESC_CORR_OLD, RUBRICA_ESTERNA, InteropUrl");
            ...
            // Il tipo sar� "F" se il corrispondente, di rubrica comune, � un RF altrimenti sar� "U"
            ...
            if (!string.IsNullOrEmpty(corr.rubricaEsterna))
                param += ",'" + corr.rubricaEsterna + "' ";
            else
                param += ", null ";
        ...
        if (!string.IsNullOrEmpty(DocsPaUtils.Configuration.InitConfigurationKeys.GetValue("0", "USA_CONNECTBYPRIOR_OR_WITH")) && (DocsPaUtils.Configuration.InitConfigurationKeys.GetValue("0", "USA_CONNECTBYPRIOR_OR_WITH")).Equals("1"))
        {
            ...
            if (corrRow["CHA_TIPO_URP"].ToString().Equals("R"))
            {
                ...
                else if (dbType.ToUpper().Equals("SQL"))
                {
                    ...
                    //per adesso � come la precedente dopo l'ottimizzaremo con la clausola WITH
        ...
        try
        {
            ...
            if (objQueryCorrispondente.codiceRubrica != null)
            {
                ...
                else
                {
                    ...
                    //se il parent � un utente si restituisce la lista vuota
            ...
            else
            {
                ...
                if (objQueryCorrispondente.descrizioneUO != null)
                {
                    ...
                    char[] separator = DocsPaVO.Settings.AppSettings.Instance.separator.ToCharArray();
            ...
            foreach (DataRow corrRow in dataSet.Tables["CORRISPONDENTI"].Rows)
            {
                ...
                if (dataSet != null)
                {
                    if (corrRow["CHA_TIPO_URP"].ToString().Equals("U"))
                    {
                        //si verifica se � replicata nella lista
                        int replies = 0;
                    ...
                    if (corrRow["CHA_TIPO_URP"].ToString().Equals("R"))
                    {
                        ...
                        // non imposta il parametro tipoIE della UO parent del ruolo. L'ho aggiunto cos� almeno riesco
                        // a spedire.
            ...
            if (objQueryCorrispondente.codiceRubrica != null)
            {
                ...
                if (objQueryCorrispondente.getChildren == false)
                {
                    ...
                    else if (dataSet.Tables["PARENT"].Rows[0]["CHA_TIPO_URP"].Equals("U"))
                    {
                        ...
                        //Commentato il 17/07/2006: perch� altrimenti la ricerca per codice corrisp da dei risultati diversi dalla
    ...
    public ArrayList ListaCorrispondentiIntInterop(DocsPaVO.addressbook.QueryCorrispondente qco)
    {
        ...
        DocsPaUtils.Query q;
        ArrayList listaCorr = new ArrayList();
        //ricavo il ruolo mittente del protocollo ricevuto per interoperabilit�
        if (objQueryCorrispondente.isRuoloDefined())
        {
            ...
            foreach (DataRow corrRow in dataSet.Tables["CORRISPONDENTI"].Rows)
            {
                ...
                if (dataSet != null)
                {
                    if (corrRow["CHA_TIPO_URP"].ToString().Equals("R"))
                    {
                        ...
                        //viene calcolata la parentela del mittente del protocollo ricevuto per interoperabilit�
                        if (objQueryCorrispondente.isUODefined())
                        {
                            if ((objQueryCorrispondente.descrizioneUO != null && HasDefinedUo(objQueryCorrispondente.descrizioneUO, 1, corrRow["ID_UO"].ToString(), dataSet.Tables["UO"])) || (objQueryCorrispondente.codiceUO != null && HasDefinedUo(objQueryCorrispondente.codiceUO, 2, corrRow["ID_UO"].ToString(), dataSet.Tables["UO"])))
                            {
    ...
    private DocsPaVO.utente.UnitaOrganizzativa GetUO(DataRow corrRow)
    {
        ...
        // Indica se il corrispondente � proveniente da rubrica comune
        ...
        // Impostazione dell'url per l'interoperabilit� semplificata
        String url = corrRow["InteropUrl"].ToString();
        corrispondenteUO.Url = new List<Corrispondente.UrlInfo>();
        if (!String.IsNullOrEmpty(url))
            corrispondenteUO.Url.Add(new Corrispondente.UrlInfo() { Url = url });

        if (corrRow.Table.Columns.Contains("RUBRICA_ESTERNA") && corrRow["RUBRICA_ESTERNA"] != DBNull.Value)
            corrispondenteUO.rubricaEsterna = corrRow["RUBRICA_ESTERNA"].ToString();
    ...
    {
        char[] separator_or = DocsPaVO.Settings.AppSettings.Instance.separator.ToCharArray();
        char[] separator_and = { ' ' };
    ...
    /// <returns></returns>
    public bool UserLogin(out string peopleId, string userName, string password, string idAmm, bool loginCodiceFiscale)
    {
        bool result = false;
        ...
        try
        {
            ...
            {
                DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_Login_CF");
                q.setParam("codiceFiscale", userName);
    ...
    /// <returns></returns>
    public bool UserLoginNoPwd(out string peopleId, string userName, string idAmm, string modulo, bool loginCodiceFiscale)
    {
        bool result = true;
        peopleId = System.String.Empty;
        ...
        try
        {
            ...
            if (!loginCodiceFiscale)
            {
                ...
                if (modulo != null)
                {
                    ...
                    {
                        // login al modulo centro servizi della conservazione
                        queryName = "S_LOGIN_CENTRO_SERVIZI_CONSERVAZIONE_CF";
                    }
                    else
                    {
                        // login al sistema di gestione documentale
                        queryName = "S_Login_verificaUser_CF";
                    }
                }
                else
                {
    ...
    public void GetRegistro(string idRegistro, ref DocsPaVO.utente.Registro reg)
    {
        ...
        q.setParam("param1", "SYSTEM_ID, VAR_CODICE, NUM_RIF, VAR_DESC_REGISTRO, " +
            ...
            DocsPaDbManagement.Functions.Functions.ToChar("DTA_ULTIMO_PROTO", false) + " , ID_RUOLO_AOO,ID_RUOLO_RESP,ID_PEOPLE_AOO, CHA_AUTO_INTEROP, CHA_RF, CHA_DISABILITATO, ID_AOO_COLLEGATA, INVIO_RICEVUTA_MANUALE, FLAG_WSPIA, VAR_PREG, ANNO_PREG, DIRITTO_RUOLO_AOO, VAR_CODICE_IPA"); //DocsPaWS.Utils.dbControl.toChar("DTA_ULTIMO_PROTO",false));
        ...
        foreach (DataRow dataRow in dataSet.Tables[0].Rows)
        {
            ...
            reg.codiceIpa = dataRow[22].ToString();
    ...
    public ArrayList GetRegistriRuolo(string idRuolo)
    {
        ...
        q.setParam("param1", DocsPaDbManagement.Functions.Functions.ToChar("A.DTA_OPEN", false) + " AS DTA_OPEN, " +
            ...
            DocsPaDbManagement.Functions.Functions.ToChar("A.DTA_ULTIMO_PROTO", false) + " AS DTA_ULTIMO_PROTO " + ", A.ID_RUOLO_AOO,A.ID_RUOLO_RESP,A.ID_PEOPLE_AOO, A.CHA_AUTO_INTEROP, A.VAR_PREG, A.ANNO_PREG, A.VAR_CODICE_IPA");
        ...
        if (dataSet != null)
        {
            foreach (DataRow row in dataSet.Tables[0].Rows)
            {
                ...
                reg.codiceIpa = row["VAR_CODICE_IPA"].ToString();
                registri.Add(reg);
            }
        }

        // leggo il numero di protocollo solo se lo stato del registro � chiuso
    ...
    public void RegistroModifica(ref DocsPaVO.utente.Registro registro, DocsPaVO.utente.InfoUtente infoUtente)
    {
        ...
        if (registro.stato.Equals("A"))
        {
            logger.Debug("Errore nella gestione delle trasmissioni (Query - RegistroModifica). Il registro non � chiuso!");
            //throw new Exception("Il registro non � chiuso");
            throw new Exception("Il registro non � chiuso");
        }
        try
        {
        ...
        try
        {
            if (this.ExecuteQuery(out ds, "corrispondenti", commandText))
            {
                if (ds.Tables["corrispondenti"].Rows.Count >= 1)
                {
                    ...
                    //Se il risultato � maggiore di uno vado a dare la priorit� a quello di RC con codiceAmm e CodiceAoo uguale a quello in input(quello della segnatura),
                    //se non ne trovo nessuno vado a vedere se ce ne � uno che non � di RC ma con CodiceAmm e CodiceAoo in Input
        ...
        try
        {
            if (this.ExecuteQuery(out ds, "corrispondenti", commandText))
            {
                ...
                if (ds.Tables["corrispondenti"].Rows.Count >= 1)
                {
                    ...
                    {
                        //se uno dei corrispondenti � di rubrica comune, restituisco o quest'ultimo
    ...
    /// <summary>
    /// Questo metodo � analogo a GetCorrispondenteBySystemId ma estrare piu informazioni, ad esempio
    ...
    public DocsPaVO.utente.Corrispondente GetCorrispondenteBySystemID(string systemID)
    {
        ...
        {
            logger.Debug(string.Format("Si � verificato un errore nella ricerca del corrispondente con ID '{0}'", systemID), ex);
        }
        finally
        {
    ...
    public void GetIdAmmUtente(out string idAmm, string userName, bool loginCodiceFiscale)
    {
        ...
        if (!loginCodiceFiscale)
            ...
            q.setParam("param2", "UPPER(VAR_CODICE_FISCALE) = UPPER('" + userName + "')");
    ...
    public void GetIdAmmUtente(out ArrayList List_idAmm, string userName)
    {
        ...
        try
        {
            this.OpenConnection(); //alcune volte la connessione � chiusa inspegabilmente..
    ...
    public void GetIdAmmUtenteCf(out ArrayList List_idAmm, string codiceFiscale)
    {
        logger.Debug("START : DocsPaDB > Query_DocsPAWS > Utenti > GetIdAmmUtente");
        DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_People");
        q.setParam("param1", "ID_AMM");
        q.setParam("param2", "UPPER(VAR_CODICE_FISCALE) = UPPER('" + codiceFiscale + "')");
        string sql = q.getSQL();
        logger.Debug(sql);
        List_idAmm = null;
        DataSet ds = new DataSet();
    ...
    public String[] GetMementoUtente(string idPeople, string IdAmministrazione)
    {
        ...
        if (dataSet.Tables["PEOPLE"].Rows.Count > 0)
        {
            ...
            {
                logger.Debug("Errore Recuperando il memento {0} {1}", e.Message, e.StackTrace);
            }
        }
        dataSet.Dispose();
    ...
    public bool SetMementoUtente(string idPeople, string IdAmministrazione, string dominio, string alias)
    {
        ...
        {
            logger.Debug("Errore Impostanto il memento {0} {1}", e.Message, e.StackTrace);
        }
        return result;
    }
    ...
    /// <returns></returns>
    public DocsPaVO.utente.Utente GetUtenteNoFiltroDisabled_old(string idPeople)
    {
        //string sql ="SELECT SYSTEM_ID, USER_ID, ID_AMM, VAR_COGNOME, VAR_NOME, VAR_TELEFONO, EMAIL_ADDRESS, CHA_NOTIFICA, CHA_AMMINISTRATORE,CHA_NOTIFICA_CON_ALLEGATO  FROM PEOPLE WHERE SYSTEM_ID="+idPeople;
        DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_People_all");
        q.setParam("param1", "SYSTEM_ID, USER_ID, ID_AMM, VAR_COGNOME, VAR_NOME, VAR_TELEFONO, EMAIL_ADDRESS, CHA_NOTIFICA, CHA_AMMINISTRATORE,CHA_NOTIFICA_CON_ALLEGATO,VAR_SEDE,MATRICOLA");
    ...
    {
        logger.Debug("START : DocsPaDB > Query_DocsPAWS > Utenti > GetUtenteNoFiltroDisabled");

        DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_People_all_1");
        ArrayList parameters = new ArrayList();
        parameters.Add(new DocsPaUtils.Data.CommandParameter("idPeople", idPeople));
        string sql = q.getSQL();
        logger.Debug("ID_PEOPLE: " + idPeople + " QUERY: " + sql);

        DocsPaVO.utente.Utente utente = null;
        //ricerca delle caratteristiche utente nella tabella people
        DataSet dataSet;
        this.ExecuteQuery(out dataSet, "PEOPLE", sql, parameters);
        ...
        dataSet.Dispose();
        logger.Debug("END : DocsPaDB > Query_DocsPAWS > Utenti > GetUtenteNoFiltroDisabled");
    ...
    public DocsPaVO.utente.Utente GetUtente(string userName, string idAmministrazione, string modulo, bool loginCodiceFiscale)
    {
        string queryName = string.Empty;
        ...
        string param = "";
        if (!loginCodiceFiscale)  
            param = "UPPER(P.USER_ID)='" + userName.ToUpper() + "'";
        else
            param = "UPPER(P.VAR_CODICE_FISCALE)='" + userName.ToUpper() + "'";
    ...
    {
        return GetUtente(userName, idAmministrazione, string.Empty, false);
    }
    ...
    public DocsPaVO.utente.Ruolo GetRuolo(string idRuolo, bool loadFunzioni)
    {
        ...
        {
            // cerco il valore di livello pi� alto per le UO associate all'utente in modo
    ...
    /// <param name="idRuolo"></param>
    /// <returns></returns>
    public DocsPaVO.utente.Ruolo GetRuoloEnabledAndDisabledByIdGruppo(string idGruppo)
    {
        logger.Debug("getRuolo");
        DocsPaVO.utente.Ruolo objRuolo = null;
        DataSet dataSet = new DataSet();
        string sql = "";
        DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_J_DPA_CORR_GLOBALI__PEOPLEGROUPS__TIPO_RUOLO__CORR_GLOBALI_ID_GRUPPO_E_D");
        q.setParam("param1", "B.ID_GRUPPO = " + idGruppo);
        sql = q.getSQL();
        logger.Debug(sql);
        this.ExecuteQuery(dataSet, "RUOLI", sql);
    ...
    /// <summary>
    /// Questo metodo ricerca il ruolo e lo ritorna anche se � disabilitato
    /// </summary>
    /// <param name="idRuolo"></param>
    /// <returns></returns>
    ...
    //query che estrae i ruoli che hanno la visibilit� sul documento selezionato dal portale
    ...
    public ArrayList getListaStoriciMittenti(string idProfile, string tipo)
    {
        ...
        DocsPaUtils.Query q;
        if(tipo.ToUpper().Equals("D"))
            q = DocsPaUtils.InitQuery.getInstance().getQuery("S_STORICO_DEST");
        else
            q = DocsPaUtils.InitQuery.getInstance().getQuery("S_STORICO_MITT_1");
    ...
    protected DocsPaVO.documento.StoricoMittente getStoricoMittenti(DataRow dataRow)
    {
        ...
        if (dataRow["VAR_MOTIVO"] != null)
            storico.motivo = dataRow["VAR_MOTIVO"].ToString();
    ...
    public bool UnlockUserLogin(string userId, string idAmm)
    {
        ...
        try
        {
            ...
            q.setParam("param1", userId);
            if (idAmm != null && idAmm != "") //tanto la userId � univoca in Docspa, quindi non serve anche l'idAmm.
    ...
    public bool UnlockUsersLogin(string idAmm)
    {
        bool result = false;
        try
        {
            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("UNLOCK_USERS_LOGIN");
            q.setParam("param2", idAmm);
            string sql = q.getSQL();
            logger.Debug(sql);
            if (this.ExecuteNonQuery(sql))
    ...
    public bool IsUtenteDisabled(string userName, string idAmm, bool loginCodiceFiscale)
    {
        string disabled = "";
        ...
        try
        {
            if (!loginCodiceFiscale)
            {
                ...
                DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_People_Disabled_CF");
                q.setParam("codiceFiscale", userName);
                if (!string.IsNullOrEmpty(idAmm))
                    q.setParam("param2", " and ID_AMM = " + idAmm);
                else
    ...
    /// � autorizzato sul registro sul quale si sta protocollando un doc.
    /// </summary>
    /// <param name="idRuolo"></param>
    ...
    /// attenzione restituisce ruolo.uo = null
    ///
    /// Se la ID_UO non � null, restituisce solo ed esclusivamente ruolo.uo.system_id
    /// </summary>
    /// <param name="idCorrGlobali"></param>
    /// <returns></returns>
        ...
        try
        {
            ...
            if (idCorrglobali != null && idCorrglobali != "")
            {
                ...
                /*
                    ...
                    7: DISABILITAZIONE EFFETTUATA - il corrispondente non � stato usato come mittente/destinatario ma solo in campi profilati di documenti/fascicoli
                ...
                */
                ...
                switch (retProc)
                {
                    case 0:
                        this.CommitTransaction();
                        logger.Debug("Eseguita Commit alla Stored Procedure: SP_DELETE_CORR_ESTERNO - il corrispondente � stato cancellato");
                        retValue = true;
                        message = "OK";
                        break;
                    case 1:
                        this.CommitTransaction();
                        logger.Debug("Eseguita Commit alla Stored Procedure: SP_DELETE_CORR_ESTERNO - il corrispondente � stato disabilitato poich� � mitt o dest di documenti!");
                        retValue = true;
                        message = "OK";
                        break;
                    case 2:
                        this.RollbackTransaction();
                        logger.Debug("Eseguita Rollback alla Stored Procedure: SP_DELETE_CORR_ESTERNO - il corrispondente non viene rimosso poich� � presente in una lista di distribuzione");
                        message = "NOTOK";
                        break;
                    case 3:
                        this.RollbackTransaction();
                        logger.Debug("ERRORE - Eseguita Rollback alla Stored Procedure:  SP_DELETE_CORR_ESTERNO  - CAUSA : Delete sulla DPA_CORR_GLOBALI NON � andata a buon fine");
                        message = "ERROR3";
                        break;
                    case 4:
                        this.RollbackTransaction();
                        logger.Debug("ERRORE - Eseguita Rollback alla Stored Procedure:  SP_DELETE_CORR_ESTERNO  - CAUSA :  Delete sulla DPA_DETT_GLOBALI NON � andata a buon fine");
                        message = "ERROR4";
                        break;
                    case 5:
                        this.RollbackTransaction();
                        logger.Debug("ERRORE - Eseguita Rollback alla Stored Procedure:  SP_DELETE_CORR_ESTERNO - CAUSA : UPDATE sulla dpa_corr_globali NON � andata a buon fine");
                        message = "ERROR5";
                        break;
                    case 6:
                        this.RollbackTransaction();
                        logger.Debug("ERRORE - Eseguita Rollback alla Stored Procedure:  SP_DELETE_CORR_ESTERNO - CAUSA :  DELETE sulla dpa_liste_distr NON � andata a buon fine");
                        message = "ERROR6";
                        break;
                    case 7:
                        this.CommitTransaction();
                        logger.Debug("Eseguita Commit alla Stored Procedure: SP_DELETE_CORR_ESTERNO - il corrispondente � stato disabilitato poich� � stato usato in campi profilati!");
                        retValue = true;
                        message = "OK";
                        break;
        ...
        try
        {
            ...
            if (datiModifica != null)
            {
                ...
                */
                ...
                // Url per l'interoperabilit� semplificata (attualmente uno solo)
                ...
                /*
                    ...
                    0: ERRORE - L'operazione di modifica NON � andata a buon fine
                    1: MODIFICA EFFETTUATA CON SUCCESSO
                        - il corrispondente � stato modificato con successo
                ...
                */
                ...
                case 1:
                    this.CommitTransaction();
                    logger.Debug("Eseguita Commit alla Stored Procedure: SP_MODIFY_CORR_ESTERNO_IS - il corrispondente � stato modificato correttamente");
                    retValue = true;
                    message = "OK";
    ...
    public string GetSystemIDCorr(string codRubrica, string idamm, string idRegistro)
    {
        try
        {
            ...
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                ...
                if (!dbProvider.ExecuteQuery(out ds, "CORR", commandText))
                    throw new Exception("Errore in GetSystemIDCorr");

                if (ds.Tables["CORR"] != null && ds.Tables["CORR"].Rows.Count > 0)
                {
                    DataRow row = ds.Tables["CORR"].Rows[0];
    ...
    public string GetSystemIDCanale()
    {
        try
        {
            ...
            using (DocsPaDB.DBProvider dbProvider = new DBProvider())
            {
                if (!dbProvider.ExecuteQuery(out ds, "CANALE", commandText))
                    throw new Exception("Errore in GetSystemIDCanale");

                if (ds.Tables["CANALE"] != null && ds.Tables["CANALE"].Rows.Count > 0)
                {
                    DataRow row = ds.Tables["CANALE"].Rows[0];
                    system_id = !string.IsNullOrEmpty(row["SYSTEM_ID"].ToString()) ? row["SYSTEM_ID"].ToString() : string.Empty;
                }
            }
            return system_id;
    ...
    public string GetRegistroDaCodice(string codice)
    {
        try
        {
            ...
            {
                //dbProvider.ExecuteScalar(out system_id, commandText);
                if (!dbProvider.ExecuteQuery(out ds, "REGISTRO", commandText))
                    throw new Exception("Errore in GetRegistroDaCodice");

                if (ds.Tables["REGISTRO"] != null && ds.Tables["REGISTRO"].Rows.Count > 0)
                {
                    DataRow row = ds.Tables["REGISTRO"].Rows[0];
                    system_id = !string.IsNullOrEmpty(row["SYSTEM_ID"].ToString()) ? row["SYSTEM_ID"].ToString() : string.Empty;
                }
            }
            return system_id;
    ...
    /// <summary>
    /// Verifica se, in docspa, � presente il corrispondente richiesto da rubrica comune
    /// </summary>
    /// <param name="codiceRubrica"></param>
    /// <param name="idAmministrazione"></param>
    /// <param name="id"></param>
    ...
    /// <summary>
    /// Metodo per verificare se un dato ruolo � disabilitato
    /// </summary>
    /// <param name="authorCorrGlob">Id group del ruolo da verificare</param>
    /// <returns>True se il ruolo � disabilitato</returns>
    ...
    //verifico univocit� del codice qualifica per l'inserimento
    ...
    //verifico univocit� della tupla PeopleGroups che voglio inserire
    ...
    public void GetIdAmmUtenteCentroServiziConservazione(out ArrayList List_idAmm, string userName)
    {
        ...
        using (DocsPaDB.DBProvider dbProvider = new DBProvider())
        {
            ...
            string commandText = queryDef.getSQL();
            logger.Information("S_PEOPLE_CENTRO_SERVIZI: {0}", commandText);
    ...
    /// <summary>
    /// Verifica se l'utente � abilitato all'accesso al modulo Centro Servizi di conservazione
    /// </summary>
    /// <param name="userName"></param>
    /// <returns></returns>
    public bool IsUtenteDisabledCentroServizi(string userName, bool loginCodiceFiscale)
    {
        bool disabled = false;
        ...
        using (DocsPaDB.DBProvider dbProvider = new DBProvider())
        {
            if (!loginCodiceFiscale)
            {
                ...
                string commandText = queryDef.getSQL();
                logger.Information("S_PEOPLE_DISABLED_CENTRO_SERVIZI: {0}", commandText);
                ...
                DocsPaUtils.Query queryDef = DocsPaUtils.InitQuery.getInstance().getQuery("S_PEOPLE_DISABLED_CENTRO_SERVIZI_CF");

                queryDef.setParam("codiceFiscale", userName);

                string commandText = queryDef.getSQL();
                logger.Information("S_PEOPLE_DISABLED_CENTRO_SERVIZI_CF: {0}", commandText);
    ...
    /// <param name="modulo"></param>
    /// <returns></returns>
    public bool IsUtenteDisabled(string userName, string modulo, string idAmm, bool loginCodiceFiscale)
    {
        if (!string.IsNullOrEmpty(modulo))
        {
            if (DocsPaUtils.Moduli.ModuliAuthManager.IsModuloCentroServizi(modulo))
                return IsUtenteDisabledCentroServizi(userName, loginCodiceFiscale);
            else
                return IsUtenteDisabled(userName, idAmm, loginCodiceFiscale);
        }
        else
            return IsUtenteDisabled(userName, idAmm, loginCodiceFiscale);
    }
    ...
    public bool InsertMailCorr(List<MailCorrispondente> listCaselleCorr, string idCorrispondente)
    {
        ...
        {
            logger.Error(e, "errore in InsertMailCorr");
            RollbackTransaction();
            return false;
        }
    }
    ...
    public DocsPaVO.utente.Corrispondente GetDettaglioCorrispondente(string idCorrispondente)
    {
                ...
                using (DBProvider dbProvider = new DBProvider())
                {
                    using (System.Data.IDataReader reader = dbProvider.ExecuteReader(commandText))
                    {
                        if (reader.Read())
                        {
                            ...
                            if (!reader.IsDBNull(reader.GetOrdinal("VAR_TELEFONO2")))
                                corrispondente.telefono2 = reader.GetValue(reader.GetOrdinal("VAR_TELEFONO2")).ToString();
        ...
        {
            logger.Debug(string.Format("Si � verificato un errore nella ricerca del corrispondente con ID '{0}'", idCorrispondente), ex);
        }
        finally
        {
    ...
    public List<DocsPaVO.utente.Corrispondente> GetPrefChannelAllDest(string idProfile, string typeDest)
    {
        ...
        {
            logger.Debug(string.Format("Si � verificato un errore nel metodo GetPrefChannelAllDest.\nDettaglio errore: {0}"), e);
            logger.Debug("END - GetPrefChannelAllDest");
            return corrispondenti;
        }
    }
    ...
    public bool AddBrowserInfo(BrowserInfo browser, string idPeople, string userName, string ipAddress)
    {
        ...
        try
        {
            ...
            if (browser != null)
            {
                logger.Debug("BROWSERINFO browserVersion  " + browser.browserVersion);
                logger.Debug("BROWSERINFO ip  " + browser.ip);
            }
            else logger.Debug("BROWSERINFO � vuoto!");
            ...
            int res = 0;
            logger.Debug("BROWSERINFO INSERT value " + insert.ToString());
            if (insert)
            {
                string query;
                query = q.getSQL();
                logger.Debug("browserinfo insert " + query);
    ...
    public bool isUtArchivistaDeposito(string idPeople, string idGruppo)
    {
        ...
        //per ora l'unico controllo che faccio � sull'attivazione dell'ambiente di deposito

        //se idPeople null vuol dire che la security � associata al ruolo (tipo nodi di titolario, tipologia documento, ...)
    ...
    public bool SetOTPResetPasswordUtente(Utente utente, string otp)
    {
        bool result = true;
        ...
        try
        {
            using (DBProvider dbProvider = new DBProvider())
            {
                //RIMUOVO OTP PRECEDENTEMENTE SALVATO
                DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("D_DPA_PEOPLE_OTP_RESET_PASSWORD");
                q.setParam("userId", utente.userId.ToUpper());
                string query = q.getSQL();
                logger.Debug("SetOTPResetPasswordUtente: " + query);
                if (!dbProvider.ExecuteNonQuery(query))
                {
                    throw new Exception("Errore durante la rimozione dell'OTP: " + query);
                }

                //INSERISCO IL NUOVO OTP
                q = DocsPaUtils.InitQuery.getInstance().getQuery("I_DPA_PEOPLE_OTP_RESET_PASSWORD");
                if (DBType.ToUpper().Equals("ORACLE"))
                    q.setParam("systemId", DocsPaDbManagement.Functions.Functions.GetSystemIdNextVal("DPA_PEOPLE_OTP_RESET_PASSWORD"));
                q.setParam("userId", utente.userId.ToUpper());
                q.setParam("email", utente.email);
                q.setParam("otp", otp);
                ...
                if (!dbProvider.ExecuteNonQuery(query))
                {
                    throw new Exception("Errore durante l'inserimento di un nuovo OTP: " + query);
                }
            }
        ...
        {
            logger.Error("Errore in SetOTPResetPasswordUtente: " + e.Message);
            result = false;
        }
    ...
    public string GetOTPResetPasswordUtente(string userId)
    {
        ...
        try
        {
            using (DBProvider dbProvider = new DBProvider())
            {
                DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_DPA_PEOPLE_OTP_RESET_PASSWORD");
                q.setParam("userId", userId.ToUpper());
                string query = q.getSQL();
                logger.Debug("GetOTPResetPasswordUtente: " + query);

                if (this.ExecuteQuery(out ds, "OTP", query))
                {
                    if (ds.Tables["OTP"] != null && ds.Tables["OTP"].Rows.Count > 0)
                    {
                        otp = ds.Tables["OTP"].Rows[0]["VAR_OTP"].ToString();
                    }
                }
            }
        ...
        {
            logger.Error("Errore in GetOTPResetPasswordUtente: " + e.Message);
            otp = string.Empty;
        }
    ...
    public bool IsUtenteVerticale(string idPeople)
    {
        bool result = false;
        DataSet ds = new DataSet();
        try
        {
            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("S_PEOPLE_SISTEMA");
            q.setParam("idPeople", idPeople);

            string query = q.getSQL();
            logger.Debug(query);

            if (this.ExecuteQuery(out ds, "sistema", query))
            {
                if (ds.Tables["sistema"] != null && ds.Tables["sistema"].Rows.Count > 0)
                {
                    result = true;
                }
            }
        ...
        {
            logger.Error("Errore in IsUtenteVerticale " + e.Message);
            result = false;
        }
    ...
    public bool SetUtenteVerticale(string userId, string idAmm, bool isVerticale)
    {
        bool result = false;
        DataSet ds = new DataSet();
        try
        {
            DocsPaUtils.Query q = DocsPaUtils.InitQuery.getInstance().getQuery("U_PEOPLE_SISTEMA");
            q.setParam("userId", userId);
            q.setParam("idAmm", idAmm);
            q.setParam("diSistema", isVerticale ? "1" : "0");
            ...
            {
                throw new Exception("Errore durante l'aggiornamento dell'utente verticale: " + query);
            }
        }
        catch (Exception e)
        {
            logger.Error("Errore in SetUtenteVerticale " + e.Message);
            result = false;
        }

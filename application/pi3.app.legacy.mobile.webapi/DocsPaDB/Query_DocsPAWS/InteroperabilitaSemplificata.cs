...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(InteroperabilitaSemplificata));
    ...
    public bool AggiornaStatusMask(string statusmask, string codAOO, string codAmm, string idDocument)
    {
        ...
        try
        {
            ...
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                if (RowTornate > 0)
                {
                    ...
                    logger.Warning("Aggiornamento status mask. Righe modificate: " + RowTornate);
                }
                else
                {
                    logger.Error("Aggiornamento status mask. Nessuna riga modificata per il destinatario con codice AMM {0} e codice AOO {1} per il documento {2}. FIX.", codAmm, codAOO, idDocument);
        ...
        {
            logger.Error("Errore nell'aggiornamento della status mask: Messaggio {0} - StackTrace {1}", ex.Message, ex.StackTrace);
    ...
    public bool IS_statusMaskUpdater(string idDoc)
    {
        ...
        try
        {
            using (DBProvider dbProvider = new DBProvider())
            {
                ...
                if (fixRowTorn > 0)
                {
                    logger.Warning("Fix eseguito avvenuta consegna , righe aggiornate:" + fixRowTorn);
                }
                else
                {
                    logger.Warning("Errore nell'esecuzione del FIX, o nessuna ricevuta di avvenuta consegna");
                }
                ...
                if (fixRowTorn2 > 0)
                {
                    logger.Warning("Fix eseguito avvenuta , righe aggiornate:" + fixRowTorn2);
                }
                else
                {
                    logger.Warning("Errore nell'esecuzione del FIX, o nessuna ricevuta mancata consegna");
                }
            }
        ...
        {
            logger.Error("Errore nell'aggiornamento della status mask per le spedizioni IS: Messaggio {0} - StackTrace {1}", ex.Message, ex.StackTrace);

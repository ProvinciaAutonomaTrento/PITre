        ...
        using (DBProvider dbProvider = new DBProvider())
        {
            using (IDataReader dataReader = dbProvider.ExecuteReader(query.getSQL()))
            {
                if (dataReader != null && dataReader.Read())
                {
                    ...
                    returnValue.DateAutomaticPrintFinish = DateTime.TryParse(dataReader["DateAutomaticPrintFinish"].ToString(), out var finish) ? finish : DateTime.Now;
                    returnValue.DateAutomaticPrintStart = DateTime.TryParse(dataReader["DateAutomaticPrintStart"].ToString(), out var start) ? start : DateTime.Now;
                    returnValue.DateLastPrint = DateTime.TryParse(dataReader["DateLastPrint"].ToString(), out var last) ? last : DateTime.MinValue;
                    returnValue.DateNextAutomaticPrint = DateTime.TryParse(dataReader["DateNextAutomaticPrint"].ToString(), out var next) ? next : DateTime.MinValue;
                }
            }

...
public class NotificationDB : DBProvider
{
    ...
    private static ILogger logger = Serilog.Log.ForContext(typeof(NotificationDB));
    #endregion
        ...
        //{
            if (ds.Tables["notifications"] != null && ds.Tables["notifications"].Rows.Count > 0)
            {
                foreach (DataRow row in ds.Tables["notifications"].Rows)
                {
                    ...
                    DateTime dtNotify = DateTime.MinValue;

                    try
                    {
                        dtEvent = Convert.ToDateTime(row["DTA_EVENT"].ToString(), new System.Globalization.CultureInfo("it-IT"));
                    }
                    catch {
                        dtEvent = Convert.ToDateTime(row["DTA_EVENT"].ToString());
                    }

                    try
                    {
                        dtNotify = Convert.ToDateTime(row["DTA_NOTIFY"].ToString(), new System.Globalization.CultureInfo("it-IT"));
                    }
                    catch
                    {
                        dtNotify = Convert.ToDateTime(row["DTA_NOTIFY"].ToString());
                    }
                ...
                Notification notification = new Notification()
                    {
                        ...
                        ACCESSRIGHTS = row.Table.Columns.Contains("ACCESSRIGHTS") ? (!string.IsNullOrEmpty(row["ACCESSRIGHTS"].ToString()) ? row["ACCESSRIGHTS"].ToString() : string.Empty) : string.Empty
        ...
        try
        {
            ...
            foreach (Notification notification in listNotification)
            {
                ...
                if (result && IsTrasm(notification.TYPE_EVENT))
                {
                    ...
                    {
                        logger.Information("Errore in DocsPaDb.Query_DocsPAWS.NotificationDB - Metodo RemoveNotifications : errore query U_DATE_REMOVE_FROM_NC con ID TRASMISSIONE SINGOLA " + notification.ID_SPECIALIZED_OBJECT);
                    }
                    //this.ExecuteNonQuery(query, out  rowAffected);
                    //if (rowAffected > 0)  
                    //  result = true;
                ...
                if (ds.Tables["notifications"] != null && ds.Tables["notifications"].Rows.Count > 0)
                {
                    ...
                    if (result)
                    {
                        foreach (DataRow row in ds.Tables["notifications"].Rows)
                        {
                            ...
                            if (!this.ExecuteNonQuery(query, out  rowAffected))
                            {
                                logger.Information("Errore in DocsPaDb.Query_DocsPAWS.NotificationDB - Metodo RemoveNotifications : errore query U_DATE_REMOVE_FROM_NC con ID TRASMISSIONE SINGOLA " + row["ID_SPECIALIZED_OBJECT"].ToString());
                            }
                        //        result = true;
                        //    else
                        //    {
    ...
    {
        logger.Information("Inizio Metodo ChangeStateReadNotification in DocsPaDb.Query_DocsPAWS.NotificationDB");
        bool result = false;
        string outParam = string.Empty;
        try
        {
        ...
        {
            logger.Information("Errore in DocsPaDb.Query_DocsPAWS.NotificationDB - Metodo ChangeStateReadNotification", exc);
            throw exc;
        }
        logger.Information("Fine Metodo ChangeStateReadNotification in DocsPaDb.Query_DocsPAWS.NotificationDB");
        return result;
    }
    ...
    {
        logger.Information("Inizio Metodo SetNotificationAsRead in DocsPaDb.Query_DocsPAWS.NotificationDB");
        bool result = false;
        string outParam = string.Empty;
        int rowAffected = 0;
        try
        ...
        {
            logger.Information("Errore in DocsPaDb.Query_DocsPAWS.NotificationDB - Metodo SetNotificationAsRead", exc);
            throw exc;
        }
        logger.Information("Fine Metodo SetNotificationAsRead in DocsPaDb.Query_DocsPAWS.NotificationDB");
        return result;
    }

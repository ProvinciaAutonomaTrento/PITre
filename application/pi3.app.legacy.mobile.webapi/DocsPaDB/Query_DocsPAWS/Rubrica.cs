...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(Rubrica));
    ...
    static Rubrica()
    {
        ...
        //				logger.Debug (created ? "Create SP per Rubrica" : "SP per Rubrica gi� esistenti");
        //			}
    }
    ...
    private string ricercaFullText(string campo, string andStr, string ricerca)
    {
        ...
        else if (Cfg_USE_TEXT_INDEX == "1")
        {
            ...
            // il codice sotto dovrebbe essere giusto, ma nel caso si decidesse di utilizzare questa modalit�
    ...
    public ArrayList GetElementiRubrica(DocsPaVO.rubrica.ParametriRicercaRubrica qc)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(qc.noteEmailPISREST))
                {
                    qry += String.Format("(exists (select system_id from dpa_mail_corr_esterni m where (upper(m.var_note) LIKE upper('%{0}%')) and m.id_corr = a.system_id )) AND ", qc.noteEmailPISREST.Replace("'", "''"));
                }

                if (!string.IsNullOrEmpty(qc.emailsPISREST))
                {
                    qry += String.Format("(exists (select system_id from dpa_mail_corr_esterni m where (upper(m.var_email) LIKE upper('%{0}%')) and m.id_corr = a.system_id )) AND ", qc.emailsPISREST.Replace("'", "''"));
                }
                ...
                //   ////dato un registro, ricerco anche tra gli rf ad esso associati che il ruolo pu� vedere
                ...
                ////CASO in cui si � selezionata la voce Esterni AOO.
                ...
                {
                    //se � un solo registro verifico se � un RF
                    bool cha_rf = false;
                ...
                // se:		� stata chiamata da modelli di trasmissione o dalla funzionalit� trova e sostituisci
    ...
    public ArrayList GetElementiRubricaPaging(DocsPaVO.rubrica.ParametriRicercaRubrica qc, int firstRowNum, int maxRowForPage, out int totale)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    qry += String.Format(" (exists (select system_id from dpa_mail_corr_esterni m where (upper(m.var_note) LIKE upper('%{0}%')) and m.id_corr = a.system_id  ", qc.noteEmail.Replace("'", "''"));
                    if (!string.IsNullOrEmpty(qc.email))
                        qry += String.Format(" AND (upper(m.VAR_EMAIL) LIKE upper('%{0}%')) ", qc.email.Replace("'", "''"));
                    qry += ")) AND ";
                }
                ...
                if (qc.doListe)
                {
                    ...
                    else
                    {
                        ...
                        {
                            qry += String.Format(" AND (exists (select system_id from dpa_mail_corr_esterni m where (upper(m.var_note) LIKE upper('%{0}%')) and m.id_corr = a.system_id  ", qc.noteEmail.Replace("'", "''"));
                            if(!string.IsNullOrEmpty(qc.email))
                                qry += String.Format(" AND (upper(m.VAR_EMAIL) LIKE upper('%{0}%')) ", qc.email.Replace("'", "''"));
                            qry += "))";
                        }
    ...
    public ArrayList GetGerarchiaElemento(string codice, DocsPaVO.addressbook.TipoUtente tipoIE)
    {
        ...
        //Controllo se il codice � di un utente
        DocsPaUtils.Query q;
        string queryString;
    ...
    //Il primo parametro nella firma del metodo � sempre un codice di una UO,
    ...
    /// <returns>Attributo richiesto o il codice passato per parametro se non � stato individuato il corrispondente</returns>

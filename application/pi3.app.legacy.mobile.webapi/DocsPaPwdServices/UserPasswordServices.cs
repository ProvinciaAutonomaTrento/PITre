...
namespace DocsPaPwdServices
{
    ...
    /// 
    /// Gestisce la nuova funzionalit� supportata in amministrazione relativa
    /// alla validit� temporale delle password.
    /// Se la gestione non � supportata dall'amministrazione
    /// la classe, nei servizi richiesti, effettuer� le validazioni
    ...
    public sealed class UserPasswordServices
    {
        ...
        /// <summary>
        /// Verifica se la gestione scadenza password � abilitata per l'utente
        ...
        {
            // Verifica se per l'utente l'autenticazione impostata � quella di dominio
            bool retValue = (!DomainAuthenticationEnabled(idAmministrazione, userId));

            if (retValue)
            {
                // Verifica se, da amministrazione, � abilitata o meno la gestione della scadenza password
        ...
        /// <summary>
        /// Verifica se la password per l'utente � scaduta
        /// </summary>
        /// <remarks>
        /// La password � considerata automaticamente scaduta nel
        /// caso in cui la password � stata impostata dall'amministratore
        /// </remarks>
        /// <param name="idAmministrazione"></param>
        /// <param name="userId"></param>
        /// <returns></returns>
        ...
        //MEV utenti multi-amministrazione
        /// <summary>
        /// Verifica se la password per l'utente di un'amministrazione � scaduta 
        /// </summary>
        /// <remarks>
        /// La password � considerata automaticamente scaduta nel
        /// caso in cui la password � stata impostata dall'amministratore
        ...
        /// <summary>
        /// Reperimento del numero di giorni rimanenti di validit� della password
        /// </summary>
        /// <param name="userId"></param>
        /// <returns></returns>
        ...
        /// <param name="force">
        /// Se true, sono disabilitati i controlli sulla validit� formale della password
        /// </param>
        /// <returns></returns>
        public static ValidationResultInfo SetPassword(int idAmministrazione, string userId, string password, bool isAdminPassword, bool force, string modulo="")
        {
            ...
            {
                // La password non pu� essere modificata dall'utente qualora 
                // per esso sia stata attivata l'autenticazione di dominio
                result.BrokenRules.Add(CreateBrokenRule("DOMAIN_AUTH_ENABLED", "La password non pu� essere modificata in quanto per l'utente � attivata l'autenticazione di dominio", BrokenRule.BrokenRuleLevelEnum.Error));
            }
            else
            {
                ...
                if (!force)
                {
                    ...
                    {
                        // Vengono effettuati i controlli di validit� sulla password,
                        // solo se l'utente � legato ad un'amministrazione (se non lo �, � un superadmin)
        ...
        /// <summary>
        /// Verifica se l'opzione "Nessuna scadenza password" � abilitata per l'utente
        /// </summary>
        /// <param name="idAmministrazione"></param>
        /// <param name="userId"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Verifica se la password dell'utente � stata definita dall'utente stesso
        ...
        /// <summary>
        /// Verifica della validit� formale della password
        /// </summary>
        /// <param name="password"></param>
        /// <param name="idAmministrazione"></param>
        /// <returns></returns>
        private static ValidationResultInfo CheckPasswordString(string password, int idAmministrazione)
        {
            ...
            if (!password.Any(char.IsUpper))
                result.BrokenRules.Add(CreateBrokenRule("PASSWORD_REQUIRED_UPPER_CHARS", "La password deve contenere almeno una lettera maiuscola", BrokenRule.BrokenRuleLevelEnum.Error));

            if (!password.Any(char.IsLower))
                result.BrokenRules.Add(CreateBrokenRule("PASSWORD_REQUIRED_LOWER_CHARS", "La password deve contenere almeno una lettera minuscola", BrokenRule.BrokenRuleLevelEnum.Error));
        ...
        /// <summary>
        /// Verifica se per l'utente (o, pi� in generale, per l'amministrazione)
        /// � attiva l'autenticazione di dominio. 
        /// In tal caso, l'intera gestione scadenza password verr� ignorata.
        /// </summary>
        /// <param name="userId"></param>
        /// <returns></returns>
        ...
        private static int GetIdAmministrazione(string userId)
        {
            ...
            // Se non � stato immesso alcun valore per l'amministrazione, 
            // � necessario reperire automaticamente l'id in base alla userid fornita
            DocsPaDB.Query_DocsPAWS.Utenti utenti = new DocsPaDB.Query_DocsPAWS.Utenti();
            ArrayList ammList;

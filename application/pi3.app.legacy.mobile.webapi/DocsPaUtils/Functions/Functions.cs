...
namespace DocsPaUtils.Functions
{
	...
	{
        private static ILogger logger = Log.ForContext(typeof(Functions));
        /// <summary>
		...
		{
			string basePathFiles = DocsPaVO.Settings.AppSettings.Instance.ARCHIVIO_LOG_PATH;			
		...
		public static DocsPaVO.documento.FileDocumento read(string fullName) 
		{
			...
			var read = file.Read(fileDoc.content, 0, fileDoc.length);
			file.Close();
			return fileDoc;
		}
        ...
        /// <param name="t1">Type pi� esterno</param>

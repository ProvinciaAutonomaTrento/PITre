...
public class Functions : DocsPaUtils.Interfaces.DbManagement.IFunctions
{
	...
	public string ToDate(string date) 
	{
		...
		if (date.EndsWith("PM") || date.EndsWith("AM"))
		{
            ...
            try {
				...
				try
				{
					dateTime = Convert.ToDateTime(date, new System.Globalization.CultureInfo("it-IT"));
				}
				catch {
                    dateTime = Convert.ToDateTime(date, new System.Globalization.CultureInfo("en-US"));
                }
            }
            //date = dateTime.ToString("dd/MM/yyyy hh:mm");			
	...
	public string GetSystemIdNextVal(string tableName) 
	{
		...
		string defaultSequenceName= DocsPaVO.Settings.AppSettings.Instance.oracleSequenceName;
	...
	{
		string numRighe = DocsPaVO.Settings.AppSettings.Instance.numeroMaxRisultatiQuery;
		string disableSelectTop = DocsPaVO.Settings.AppSettings.Instance.DisableSelectTop; //0=no top;
	...
	{
		return "SELECT " + DocsPaVO.Settings.AppSettings.Instance.oracleSequenceName + ".currval FROM dual";
	}
	...
	public string GetQueryLastSystemIdInserted(string tableName)
	{
		...
		//TODO: verranno gestite le nuove sequence inserendole sia nel db e nel xml (luciani) 
			...
			{
				// Se non � stata fornita una tableName,
	...
	{						
		...
		switch (DocsPaVO.Settings.AppSettings.Instance.documentale.ToString())
		{
			case "ETNOTEAM":
			case "PITRE":
			case "DOCUMENTUM":

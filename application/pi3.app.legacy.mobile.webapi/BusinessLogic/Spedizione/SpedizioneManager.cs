...
public class SpedizioneManager
{
    private static ILogger logger = Log.ForContext(typeof(SpedizioneManager));
    ...
    public static DocsPaVO.Spedizione.SpedizioneDocumento GetSpedizioneDocumento(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.documento.SchedaDocumento documento)
    {
        ...
        if (documento != null && !string.IsNullOrEmpty(documento.systemId))
        {
            ...
            {
                infoSpedizioni = null;
                logger.Debug(new ApplicationException("Errore nel reperimento delle informazioni sulle spedizioni del documento", ex), "errore GetSpedizioneDocumento");
            }
        }
        else
        {
    ...
    public static void CheckProtocolloUscita(DocsPaVO.documento.SchedaDocumento documento)
    {
    ...
    /// </remarks>
    public static void SetStatoSpedito(DocsPaVO.Spedizione.SpedizioneDocumento infoSpedizioni)
    {
    ...
    private static void FetchDestinatariEsterni(DocsPaVO.Spedizione.SpedizioneDocumento infoSpedizioni, DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.documento.SchedaDocumento documento)
    {
        ...
        Hashtable table = BusinessLogic.Interoperabilita.InteroperabilitaInvioSegnatura.dividiDestinatari(destinatariProtocollo, documento.registro.codRegistro, documento.registro.email, response, infoUtente.idAmministrazione);
        ...
        foreach (DictionaryEntry entry in table)
        {
            ...
            for (int i = 0; i < listDestinatari.Count; i++)
            {
                ...
                //se il corrispondente è Interoperabilita passo l'url come indirizzo
       ...
       ISigilloElettronicoService sigilloElettronicoService)
    {
        ...
        try
        {
            ...
            // Registro o RF mittente della spedizione per Interoperabilita
            ...
            bool existsIterop = infoSpedizioni.DestinatariEsterni.Where(d => d.IncludiInSpedizione &&
                 d.Interoperante &&
                 d.DatiDestinatari.Where(
                     d1 => d1.canalePref != null && d1.canalePref.descrizione.Equals("INTEROPERABILITA")).Count() > 0).Count() > 0;
            ...
            if (!string.IsNullOrEmpty(creaAllegatoSegnaturaXml) && creaAllegatoSegnaturaXml.Equals("1") && existsIterop)
            {
                ...
                if (allegatoSegnatura != null)
                {
                    ...
                    allegatoSegnatura = CreaSegnaturaProtocollo(allegatoSegnatura, documento, registroRfMittente, infoUtente, out resultSigillo, sigilloElettronicoService) as DocsPaVO.documento.Allegato;
                    if (resultSigillo != DocsPaVO.documento.ResultSigilloElettronico.OK)
                    {
                        ...
                        string errorMessage = "Errore durante l'applicazione del sigillo sul file segnatura.xml: non è stato possibile procedere con la spedizione";
                        ...
                        foreach (DocsPaVO.Spedizione.DestinatarioEsterno destinatario in infoSpedizioni.DestinatariEsterni.Where(e => e.IncludiInSpedizione))
                        {
                            ...
                            destinatario.StatoSpedizione = GetStatoErroreInSpedizione(errorMessage);
        ...
        catch (Exception ex)
        {
            ...
            logger.Debug(new ApplicationException(string.Format("Errore nella spedizione del documento con id {0}", documento.systemId), ex), "Errore SpedisciDocumento");
        }
        ...
        catch (Exception ex1)
        {
            ...
            logger.Debug(new ApplicationException("Errore nell'ordinamento dei destinatari", ex1), "Errore SpedisciDocumento");
        }
        // PEC 4 - requisito 5 - storico spedizioni
    ...
    /// <summary>
    /// Reperimento della data e ora dell'ultima spedizione del documento per Interoperabilita con destinatario il corrispondente fornito
    /// </summary>
    /// <param name="infoUtente"></param>
    /// <param name="documento"></param>
    /// <param name="email"></param>
    /// <returns></returns>
    public static string GetDataUltimaSpedizione(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.documento.SchedaDocumento documento, DocsPaVO.utente.Corrispondente dest)
    {
        // Se il documento è stato spedito per Interoperabilita,
        ...
        // Documento spedito al corrispondente per Interoperabilita
    ...
    public static DocsPaVO.documento.FileRequest CreaSegnaturaProtocollo(DocsPaVO.documento.Allegato allegatoSegnatura, DocsPaVO.documento.SchedaDocumento schedaDocumento, DocsPaVO.utente.Registro registroRfMittente, DocsPaVO.utente.InfoUtente infoUtente, out DocsPaVO.documento.ResultSigilloElettronico resultSigillo,
       ISigilloElettronicoService sigilloElettronicoService)
    {
        resultSigillo = DocsPaVO.documento.ResultSigilloElettronico.OK;
        //Creo la nuova versione dell'allegato con la segnatura aggiornata
        DocsPaVO.documento.Allegato a = new DocsPaVO.documento.Allegato();
        a.descrizione = allegatoSegnatura.descrizione;
        a.docNumber = allegatoSegnatura.docNumber;
        DocsPaVO.documento.FileRequest fileReq = a;
        ...
        if (fileReq != null)
        {
            ...
            byte[] segnaturaProtocollo = BusinessLogic.Interoperabilita.InteroperabilitaInvioSegnatura.CreaSegnaturaProtocollo(mittente, infoUtente, schedaDocumento.registro, registroRfMittente, schedaDocumento.docNumber, true, out resultSigillo, sigilloElettronicoService);
            if (resultSigillo != DocsPaVO.documento.ResultSigilloElettronico.OK)
            {
                return null;
            }
            ...
            fileReq.fileName = "segnatura.xml";
            if (!BusinessLogic.Documenti.FileManager.putFile(ref fileReq, fdAll, infoUtente, out err))
    ...
    /// <param name="errore"></param>
    /// <returns></returns>
    public static DocsPaVO.Spedizione.StatoSpedizioneDocumento GetStatoErroreInSpedizione(string errore)
    {
        DocsPaVO.Spedizione.StatoSpedizioneDocumento statoSpedizione = GetStatoSpedizione(DocsPaVO.Spedizione.StatiSpedizioneDocumentoEnum.ErroreInSpedizione);
        statoSpedizione.Descrizione = errore;
        return statoSpedizione;
    ...
    /// <summary>
    /// Metodo per la gestione della spedizione di un documento per Interoperabilita semplificata
    /// </summary>
            ...
            IEnumerable<DocsPaVO.Spedizione.DestinatarioEsterno> destinatario)
    {
        try
        {
            // Spedizione per Interoperabilita
            ...
            foreach (DocsPaVO.Spedizione.DestinatarioEsterno de in destinatario)
            {
                foreach (DocsPaVO.utente.Corrispondente c in de.DatiDestinatari)
                {
                    ...
                    // Mapping del risultato della spedizione dagli oggetti dell'Interoperabilita
                    ...
                    {
                        // Reperimento della data dell'ultima spedizione del documento per Interoperabilita
    ...
    /// <summary>
    /// Spedizione del documento ad un destinatario esterno per Interoperabilita
    /// </summary>
    /// <param name="infoUtente"></param>
    /// <param name="documento"></param>
    /// <param name="registroRfMittente">
    /// Indica il registro o l'RF mittente della spedizione per Interoperabilita
    /// </param>
    /// <param name="destinatario"></param>
    public static void SpedisciPerInteroperabilita(
            DocsPaVO.utente.InfoUtente infoUtente,
            DocsPaVO.documento.SchedaDocumento documento,
    ...
    public static void SpedisciPerTrasmissione(
                    DocsPaVO.utente.InfoUtente infoUtente,
                    DocsPaVO.documento.SchedaDocumento documento,

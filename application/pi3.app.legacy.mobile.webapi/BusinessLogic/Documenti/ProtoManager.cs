...
public class ProtoManager
{
    ...
    public delegate void AddFileSegnaturaProtocolloDelegate(DocsPaVO.documento.Allegato allegatoSegnatura, DocsPaVO.documento.SchedaDocumento schedaDocumento, DocsPaVO.utente.InfoUtente infoUtente,
       ISigilloElettronicoService sigilloElettronicoService, string temporaryRootPath, string repositoryRootPath);
    ...
    public static DocsPaVO.documento.SchedaDocumento protocolla(DocsPaVO.documento.SchedaDocumento schedaDoc, DocsPaVO.utente.Ruolo objRuolo, DocsPaVO.utente.InfoUtente objSicurezza, out DocsPaVO.documento.ResultProtocollazione risultatoProtocollazione,
       ISigilloElettronicoService sigilloElettronicoService, string temporaryRootPath, string repositoryRootPath, bool protocollazioneAutomatica = false)
    {
        ...
        if (risultatoProtocollazione == DocsPaVO.documento.ResultProtocollazione.OK && schedaDoc.tipoProto.Equals("P"))
        {
            ...
            if (!string.IsNullOrEmpty(creaAllegatoSegnaturaXml) && creaAllegatoSegnaturaXml.Equals("1"))
            {
                ...
                if (!protocollazioneAutomatica)
                {
                    ...
                    addFileSegnaturaProtocollo.BeginInvoke(allegatoSegnatura, schedaDoc, objSicurezza, sigilloElettronicoService, temporaryRootPath, repositoryRootPath, callback, addFileSegnaturaProtocollo);
                }
                else
                {
                    AddFileSegnaturaProtocollo(allegatoSegnatura, schedaDoc, objSicurezza, sigilloElettronicoService, temporaryRootPath, repositoryRootPath);
                }
            }
        }
       ...
       ISigilloElettronicoService sigilloElettronicoService, string temporaryRootPath, string repositoryRootPath)
    {
        ...
        try
        {
            ...
            byte[] segnaturaProtocollo = BusinessLogic.Interoperabilita.InteroperabilitaInvioSegnatura.CreaSegnaturaProtocollo(mittente, infoUtente, schedaDocumento.registro, schedaDocumento.registro, schedaDocumento.docNumber, true, out resultSigillo, sigilloElettronicoService);
            ...
            if (!BusinessLogic.Documenti.FileManager.putFile(ref fRAll, fdAll, infoUtente, out err, true, repositoryRootPath, temporaryRootPath))

    ...
    public class PianoConservazioneManager
    {
            ...
            ISpreadsheetService spreadsheetService)
        {
            ...
            try
            {
                ...
                //DocsPaDB.Utils.SimpleLog sl = new DocsPaDB.Utils.SimpleLog(tempPath + "\\Modelli\\Import\\logImportazioneTabellaConfine");

                //FileStream fs1 = new FileStream(tempPath + "\\Modelli\\Import\\importTabellaConfine.xls", FileMode.OpenOrCreate, FileAccess.ReadWrite);
                //fs1.Write(dati, 0, dati.Length);
                //fs1.Close();
                //fs1.Dispose();

                //sl.Log("**** Inizio importazione tabella di confine - " + System.DateTime.Now.ToString());
                //xlsConn = ImportUtils.ConnectToFile(provider, extendedProperty, tempPath + "\\Modelli\\Import\\importTabellaConfine.xls");
                                ...
                                try
                                {
                                    ...
                                    if (!string.IsNullOrWhiteSpace(rowCells.Where(cell => cell.Column == 11)?.FirstOrDefault()?.ValueAsString))
                                    {
                                        ...
                                        if (!string.IsNullOrWhiteSpace(rowCells.Where(cell => cell.Column == 12)?.FirstOrDefault()?.ValueAsString))
                                        {
                                            ...
                                            try
                                            {
                                                idTipoFasc = templates.Where(e => e.DESCRIZIONE.ToUpper() == rowCells.Where(cell => cell.Column == 12)?.First().ValueAsString.ToUpper()).FirstOrDefault().SYSTEM_ID.ToString();
                                            }
                                            catch (Exception e)
                                            {
                                                throw new Exception(String.Format(

...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(SimplifiedInteroperabilityProtoManager));
    ...
    {
        logger.Debug("Invio ricevuta di conferma di ricezione per interoperabilità semplificata del documento con id {0}", idProfile);
        ...
        using (DocsPaDB.Query_DocsPAWS.InteroperabilitaSemplificata dbManager = new DocsPaDB.Query_DocsPAWS.InteroperabilitaSemplificata())
        {
            ...
            else
            {
                ...
                logger.Error("Errore nel recupero dei dati per la creazione della ricevuta di conferma ricezione per il documento con id {0}", idProfile);
            }
    ...
    {
#if false  // usa Interoperatibility

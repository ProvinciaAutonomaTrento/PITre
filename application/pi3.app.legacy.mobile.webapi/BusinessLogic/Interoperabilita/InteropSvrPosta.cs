...
public class SvrPosta : IMailConnector
{
    ...
    private static ILogger logger = Log.ForContext(typeof(SvrPosta));
    ...
    {
        initaliazeProvider();
#if false // MailsystemNet non supportato
        ...
        if (provider == MailProvider.Chilkat)
            mail = new BusinessLogic.Interoperabilita.SvrPosta_CK();

    }

    public SvrPosta(string sServer, string sUser, string sPassword, string sPort, string sWorkingDir, CMClientType clientType, string SmtpSsl, string smtpSTA, string providerId, string tenantId, string clientId, string clientSecret, string msMailBox, string folderToRead)
    {
        initaliazeProvider(providerId);
        //if (provider == MailProvider.MailsystemNet)
        //    mail = new BusinessLogic.Interoperabilita.SvrPosta_MSNet(sServer, sUser, sPassword, sPort, sWorkingDir, clientType, SmtpSsl, smtpSTA);
        if (provider == MailProvider.Chilkat)
            mail = new BusinessLogic.Interoperabilita.SvrPosta_CK(sServer, sUser, sPassword, sPort, sWorkingDir, clientType, SmtpSsl, smtpSTA);

        //Nuova gestione parametri di connessione con MS Exchange 
        //if (provider == MailProvider.MsGraph)
        //    mail = new BusinessLogic.Interoperabilita.InteropSvrPosta_MSGraph(tenantId, clientId, clientSecret, msMailBox, folderToRead);

    }

    private void initaliazeProvider(string providerId)
    {
        //string providerString = BusinessLogic.Amministrazione.RegistroManager.GetLibByProviderDesc(providerDesc);
        string providerString = BusinessLogic.Amministrazione.RegistroManager.GetLibByProviderId(providerId);
    ...
    public SvrPosta(string providerId)
    {
        initaliazeProvider(providerId);
        //if (provider == MailProvider.MailsystemNet)
        //    mail = new BusinessLogic.Interoperabilita.SvrPosta_MSNet();
        ...
        //Nuova gestione parametri di connessione con MS Exchange 
        //if (provider == MailProvider.MsGraph)
        //    mail = new BusinessLogic.Interoperabilita.InteropSvrPosta_MSGraph();
...
#endif
        if (provider == MailProvider.Chilkat)
            mail = new BusinessLogic.Interoperabilita.SvrPosta_CK(sServer, sUser, sPassword, sPort, sWorkingDir, clientType, SmtpSsl, PopSsl, smtpSTA, Mailbox, Mailelaborate, MailNonElaborate);
    }
...
#endif
        if (provider == MailProvider.Chilkat)
            mail = new BusinessLogic.Interoperabilita.SvrPosta_CK(sServer, sUser, sPassword, sPort, sWorkingDir, clientType, SmtpSsl, PopSsl, smtpSTA);
    }
...
public class SvrMailbox : IMailConnector
{
    ...
    private static ILogger logger = Serilog.Log.ForContext(typeof(SvrMailbox));
                        ...
                        string Mailbox = "", string Mailelaborate = "", string MailNonElaborate = "")
    {
        ...
        if (provider == MailProvider.Chilkat)
        {
            if (clientType == CMClientType.POP)
                mail = new BusinessLogic.Interoperabilita.ChilkatEmailComponent(sServer, sUser, sPassword, sPort, sWorkingDir, clientType, SmtpSsl, PopSsl, smtpSTA, regId);
            else if (clientType == CMClientType.IMAP)
            {
                mail = new BusinessLogic.Interoperabilita.ChilkatEmailComponent(sServer, sUser, sPassword, sPort, sWorkingDir, clientType, SmtpSsl, PopSsl, smtpSTA, Mailbox, Mailelaborate, MailNonElaborate, regId);
            }
        }
    }
...
public class CMAttachment
{
    ...
    private byte[] get_file_data(string path)
    {
        ...
        var read = fs.Read(res, 0, (int)fs.Length);
        fs.Close();
        return res;
    }
...
public class CMMsg
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(CMMsg));
    #region Dichiarazioni

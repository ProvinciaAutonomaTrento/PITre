...
{
    private static ILogger logger = Serilog.Log.ForContext(typeof(InteroperabilitaUtils));
    ...
    public static bool InteropIntNoMail
    {
        get
        {
            string eme = DocsPaVO.Settings.AppSettings.Instance.INTEROP_INT_NO_MAIL;
    ...
    public static byte[] GetTSRForDocument(DocsPaVO.utente.InfoUtente infoUtente, DocsPaVO.documento.FileRequest fileRequest)
    {
        ...
        try
        {
            ...
            if (tsrAL != null && tsrAL.Count > 0 && tsrAL[0] is DocsPaVO.documento.TimestampDoc doc && !string.IsNullOrEmpty(doc.TSR_FILE))
            {
                retval = Convert.FromBase64String(doc.TSR_FILE);
            }
        }
        catch (Exception e)
        {
            logger.Error("Errore Ricavando il TS {0} {1}", e.Message, e.StackTrace);
        }
        return retval;
    }
    ...
    public static bool MatchTSR(byte[] tsrFile, byte[] content)
    {
        ...
        {
            logger.Error("Errore verificando il TSR {0} {1}", e.Message, e.StackTrace);
        }
        return false;
    }
    ...
    public static string GetLivelloAnomaliaSegnatura(string email, string anomalia)
    {
        // 0: nessun'anomalia
        ...
        {
            DocsPaDB.Query_DocsPAWS.Interoperabilita obj = new DocsPaDB.Query_DocsPAWS.Interoperabilita();
            livello = obj.GetLivelloAnomaliaSegnatura(email, anomalia);
        }
        catch (Exception e)
        {
            logger.Error("Errore in GetLivelloAnomaliaSegnatura: " + e.Message);
        }
    ...
    public static bool InteropInternaEnabled
    {
        get
        {
            ...
            if (DocsPaVO.Settings.AppSettings.Instance.INTEROP_INT_NO_MAIL != null && DocsPaVO.Settings.AppSettings.Instance.INTEROP_INT_NO_MAIL.Equals("1"))
            {
                supported = true;
            }
            return supported;
    ...
    public static int Cfg_NumeroTentativiLogServerPosta
    {
        get
        {
            string eme = DocsPaVO.Settings.AppSettings.Instance.NUM_TENTATIVI_CONN_SVR_POSTA;

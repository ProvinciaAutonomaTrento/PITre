...
namespace Pi3.App.Legacy.WebApi.Application.Handlers.FascicolazioneGetFascicoloById
{

    // Richiede libreria MediatR
    public class FascicolazioneGetFascicoloByIdHandler : IRequestHandler<Application.Requests.FascicolazioneGetFascicoloById, FascicolazioneGetFascicoloByIdResult>
    {
        #region Public Members

        public FascicolazioneGetFascicoloByIdHandler(ILogger<FascicolazioneGetFascicoloByIdHandler> logger, IClaimsPrincipalService claimsPrincipalService, IMediator mediator, IPi3DbContext dbContext)
        {
            this._logger = logger;
        ...
        public async Task<FascicolazioneGetFascicoloByIdResult> Handle(Application.Requests.FascicolazioneGetFascicoloById request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if(f != null)
                {
                    ...
                    var setDataVistaOutput = await this._mediator.Send(new Application.Requests.SetDataVistaSP(request.infoUtente, result.systemID, "F"));
                }
            }
            catch (Exception ex)
            {
                this._logger.LogCritical(exception: ex, message: "Id Project: " + request.idFascicolo + " " + ex.Message);
            }

            return new FascicolazioneGetFascicoloByIdResult(result);
        }
        ...
        protected readonly ILogger<FascicolazioneGetFascicoloByIdHandler> _logger;

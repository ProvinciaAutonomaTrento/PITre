...
{
    public class GetProcessoDiFirmaHandler : IRequestHandler<GetProcessoDiFirmaRequest, GetProcessoDiFirmaResult>
    {
        #region Public Members

        public GetProcessoDiFirmaHandler(ILogger<GetProcessoDiFirmaHandler> logger,
        ...
        public async Task<GetProcessoDiFirmaResult> Handle(GetProcessoDiFirmaRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if(passiFirmaEntities != null && passiFirmaEntities.Count > 0)
                {
                    ...
                    foreach (var p in output.passi)
                    {
                        ...
                        {
                            p.ruoloCoinvolto.funzioni = (await this._mediator.Send(new GetFunzioniRuoloRequest(p.ruoloCoinvolto.systemId))).funzioni;
                        }
                    }
                }
            }
            ...
            return new GetProcessoDiFirmaResult(output);
        }
        ...
        protected readonly ILogger<GetProcessoDiFirmaHandler> _logger;

                ...
                if (passo.numeroSequenza != numeroSequenzaOld)
                {
                    if(passo.numeroSequenza > numeroSequenzaOld)
                    {
                        var passoDiFirmaEntities = await this._dbContext.PassoDiFirmaEntities
                            .Where(p => p.ID_PROCESSO == idProcessoAsLong && p.ID_PASSO != passoFirmaEntity.ID_PASSO && p.NUMERO_SEQUENZA > numeroSequenzaOld && p.NUMERO_SEQUENZA <= passo.numeroSequenza)
                            .Select(p => p)
                            .ToListAsync();
                    ...
                    {
                        var passoDiFirmaEntities = await this._dbContext.PassoDiFirmaEntities
                            .Where(p => p.ID_PROCESSO == idProcessoAsLong && p.ID_PASSO != passoFirmaEntity.ID_PASSO && p.NUMERO_SEQUENZA >= passo.numeroSequenza && p.NUMERO_SEQUENZA <= numeroSequenzaOld)
                            .Select(p => p)
                            .ToListAsync();

...
namespace Pi3.App.Legacy.WebApi.Application.Handlers.AvvioProcessoDiFirma
{
    public class AvvioProcessoDiFirmaHandler : IRequestHandler<AvvioProcessoDiFirmaRequest, AvvioProcessoDiFirmaResult>
    {
        #region Public Members

        public AvvioProcessoDiFirmaHandler(ILogger<AvvioProcessoDiFirmaHandler> logger,
        ...
        public async Task<AvvioProcessoDiFirmaResult> Handle(AvvioProcessoDiFirmaRequest request, CancellationToken cancellationToken)
        {
            var output = false;
            ...
            try
            {
                ...
                if (file.versionId == "1")
                    throw new AvvioProcessoFirmaVersionIdNonValido();
                ...
                //Inserimento delle opzioni di notifica per l'istanza del processo
                //List<PassoEventoEntity> passoEventoEntitiesToInsert = new List<PassoEventoEntity>();
                //istanzaPassoFirmaEntitiesToInsert.ForEach(i =>
                //{
                //    passoEventoEntitiesToInsert.AddRange(this._dbContext.PassoEventoEntities.AsNoTracking().Where(e => e.ID_PASSO == i.ID_PASSO)
                //        .Select(e => new PassoEventoEntity()
                //        {
                //            ID_EVENTO = e.ID_EVENTO,
                //            ID_PASSO = null,
                //            ID_ISTANZA_PASSO = i.ID_ISTANZA_PASSO
                //        })
                //        .ToList()
                //    );
                //});
                //await this._dbContext.PassoEventoEntities.AddRangeAsync(passoEventoEntitiesToInsert);
                ...
                if (tipoEvento.CHA_TIPO_EVENTO == "E" || existTransmission == null)
                {
                    ...
                    if (noteGenerali.Length > 2000)
                        noteGenerali = noteGenerali.Substring(0, 1997) + "...";
                ...
                if (istanzaPassoFirmaInAttesaEsecuzione.CHA_AUTOMATICO == "1")
                {
                    await this._mediator.Send(new EseguiPassoAutomaticoRequest(istanzaProcessoFirmaEntity.ID_ISTANZA.ToString()));
                }
            ...
            return new AvvioProcessoDiFirmaResult(output, resultAvvioProcesso);
        }
        ...
        protected readonly ILogger<AvvioProcessoDiFirmaHandler> _logger;

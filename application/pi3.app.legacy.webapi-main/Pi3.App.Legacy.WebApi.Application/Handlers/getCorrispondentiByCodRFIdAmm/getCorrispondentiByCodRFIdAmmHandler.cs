...
using getCorrispondentiByCodRFIdAmmRequest = Pi3.App.Legacy.WebApi.Application.Requests.getCorrispondentiByCodRFIdAmm;

namespace Pi3.App.Legacy.WebApi.Application.Handlers.getCorrispondentiByCodRFIdAmm
{
    public class getCorrispondentiByCodRFIdAmmHandler : IRequestHandler<getCorrispondentiByCodRFIdAmmRequest, getCorrispondentiByCodRFIdAmmResult>
    {
        #region Public Members

        public getCorrispondentiByCodRFIdAmmHandler(ILogger<getCorrispondentiByCodRFIdAmmHandler> logger, 
        ...
        public async Task<getCorrispondentiByCodRFIdAmmResult> Handle(getCorrispondentiByCodRFIdAmmRequest request, CancellationToken cancellationToken)
        {
            List<DocsPaVO.utente.Corrispondente> output = new();
            try
            {
                output = await this.GetCorrispondentiByCodRf(request.codiceRF,request.idAmm);
            }
            catch(Exception ex)
            {
        ...
        protected readonly ILogger<getCorrispondentiByCodRFIdAmmHandler> _logger;
        protected readonly IClaimsPrincipalService _claimsPrincipalService;
        protected readonly IMediator _mediator;
        protected readonly IPi3DbContext _dbContext;
        private async Task<List<DocsPaVO.utente.Corrispondente>> GetCorrispondentiByCodRf(string codiceRF, string idAmm)
        {
            List<DocsPaVO.utente.Corrispondente> output = new();

            var regOfCorrWithCod = await _dbContext.RegistroEntities.AsNoTracking()
                .Join(_dbContext.CorrGlobaliEntities.AsNoTracking(),
                    r => r.SYSTEM_ID,
                    c => c.ID_RF,
                    (r, c) => new { r, c })
                .Where(j => j.r.VAR_CODICE.ToUpper().Equals(codiceRF.ToUpper()))
                .Select(j => j.r.SYSTEM_ID)
                .ToListAsync();


            var ruo = await _dbContext.RuoloRegistroEntities.AsNoTracking()
                .Join(_dbContext.RegistroEntities.AsNoTracking(),
                      rr => rr.ID_REGISTRO,
                      r => r.SYSTEM_ID,
                      (rr, r) => new { rr, r })
                .Join(_dbContext.CorrGlobaliEntities.AsNoTracking(),
                    j => j.rr.ID_RUOLO_IN_UO,
                    c => c.SYSTEM_ID,
                    (j, c) => new { j.rr, j.r, c })
                .Where(j => regOfCorrWithCod.Contains(j.rr.ID_REGISTRO.GetValueOrDefault()) && j.r.ID_AMM == idAmm.AsLong())
                .Select(j => new
                {
                    j.rr.ID_RUOLO_IN_UO,
                    j.r.ID_AMM,
            ...
            foreach ( var r in ruo )
            {
                string? sysId = r.ID_RUOLO_IN_UO != null ? r.ID_RUOLO_IN_UO.ToString() : null;
                DocsPaVO.utente.Corrispondente c = (await this._mediator.Send(new Application.Requests.AddressbookGetCorrispondenteCompletoBySystemId(sysId, GetIeType(r.CHA_TIPO_IE), null))).output;

                if (c.tipoIE == "I")
                {
                    output.Add(await this.GetCorrByCodRubrica(idAmm, c.codiceRubrica, DocsPaVO.addressbook.TipoUtente.INTERNO,"",false));
                }
                if (c.tipoIE == "E")
                {
                    output.Add(await this.GetCorrByCodRubrica(idAmm, c.codiceRubrica, DocsPaVO.addressbook.TipoUtente.ESTERNO,"",false));
                }
        ...
        private DocsPaVO.addressbook.TipoUtente GetIeType(string tipoIE)
        {
            DocsPaVO.addressbook.TipoUtente tipo = TipoUtente.GLOBALE;

            switch (tipoIE)
            {
                case "I":
                    tipo = TipoUtente.INTERNO;
                    break;
                case "E":
                    tipo = DocsPaVO.addressbook.TipoUtente.ESTERNO;
                    break;
                default:
                    tipo = TipoUtente.GLOBALE;
                    break;
            }
            return tipo;
        }

        private async Task<DocsPaVO.utente.Corrispondente> GetCorrByCodRubrica(string idAmm, string codRubrica, DocsPaVO.addressbook.TipoUtente tipoIe, string condRegistri, bool storicizzato)
        {
            ...
            {
                List<string> regs = condRegistri.Split(',').ToList();
            ...
            if (corr != null)
            {
                ...
                else
                {
                    ...
                    {
                        return await this.GetCorrInterno(sysId, idAmm, tipo);
                    }
                    else
                    {
                        return await this.GetCorrEsterno(sysId, idAmm, tipo);
                    }
                }
        ...
        {
            var datiChan = await _dbContext.DocumentTypesEntities.AsNoTracking()
                .Join(_dbContext.CanaleCorrEntities.AsNoTracking(),
                      a => a.SYSTEM_ID,
                      b => b.ID_DOCUMENTTYPE,
                      (a, b) => new { a, b})
                .Where(j => j.b.ID_CORR_GLOBALE == corr.systemId.AsLong() && j.b.CHA_PREFERITO == "1")
                .Select(j => new
                {
                    j.a.SYSTEM_ID,
                    j.a.DESCRIPTION,

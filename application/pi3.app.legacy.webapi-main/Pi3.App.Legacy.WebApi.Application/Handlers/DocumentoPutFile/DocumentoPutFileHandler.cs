...
namespace Pi3.App.Legacy.WebApi.Application.Handlers.DocumentoPutFile
{
    public class DocumentoPutFileHandler : IRequestHandler<DocumentoPutFileRequest, DocumentoPutFileResult>
    {
        ...
        public async Task<DocumentoPutFileResult> Handle(DocumentoPutFileRequest request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                // Se il nome supera la lunghezza massima, lo tronchiamo
        ...
        protected string TruncateFileName(string fileName, int maxLength)
        {
            string extention = Path.GetExtension(GetExtentionIntoSignedFile(fileName));

            // Trova la posizione dell'ultima occorrenza dell'estensione
            int extensionStartIndex = fileName.LastIndexOf(extention, StringComparison.OrdinalIgnoreCase);

            if (extensionStartIndex >= 0)
            {
                // Divide il nome del file in due parti
                string baseFileName = fileName.Substring(0, extensionStartIndex);
                string fullExtension = fileName.Substring(extensionStartIndex);

                // Calcola quanto spazio è disponibile per la parte base
                int maxBaseLength = maxLength - fullExtension.Length;
                if (maxBaseLength < 1)
                    maxBaseLength = 1;

                // Tronca il nome base se necessario
                if (baseFileName.Length > maxBaseLength)
                {
                    baseFileName = baseFileName.Substring(0, maxBaseLength);

                    // Ricompone il nome finale
                    fileName = baseFileName + fullExtension;
                }
            }

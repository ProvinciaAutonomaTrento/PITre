...
using executeAndSaveTSRRequest = Pi3.App.Legacy.WebApi.Application.Requests.executeAndSaveTSR;

namespace Pi3.App.Legacy.WebApi.Application.Handlers.executeAndSaveTSR
{
    public class executeAndSaveTSRHandler : IRequestHandler<executeAndSaveTSRRequest, executeAndSaveTSRResult>
    {
        #region Public Members

        public executeAndSaveTSRHandler(ILogger<executeAndSaveTSRHandler> logger, IClaimsPrincipalService claimsPrincipalService, IMediator mediator,
        ...
        public async Task<executeAndSaveTSRResult> Handle(executeAndSaveTSRRequest request, CancellationToken cancellationToken)
        {
            ...
            return new executeAndSaveTSRResult(ret);
        }
        ...
        private async Task<OutputResponseMarca> ExecuteAndSaveTSR(InputMarca richiesta, DocsPaVO.documento.FileRequest fileRequest, long idPeople, DocsPaVO.utente.InfoUtente infoUtente, long idTenant)
        {
            ...
            if (string.IsNullOrEmpty(richiesta.file_p7m))
            {
                var fileDocumento = (await this._mediator.Send(new Requests.DocumentoGetFileFirmato(fileRequest, infoUtente))).output;
                string stringFile = BitConverter.ToString(fileDocumento.content);
                stringFile = stringFile.Replace("-", "");
                richiesta.file_p7m = stringFile;
            }
        ...
        protected readonly ILogger<executeAndSaveTSRHandler> _logger;

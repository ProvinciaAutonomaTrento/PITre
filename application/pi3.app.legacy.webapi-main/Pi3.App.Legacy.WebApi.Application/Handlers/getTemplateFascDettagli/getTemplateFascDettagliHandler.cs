...
namespace Pi3.App.Legacy.WebApi.Application.Handlers.getTemplateFascDettagli
{

    // Richiede libreria MediatR
    public class getTemplateFascDettagliHandler : IRequestHandler<Application.Requests.getTemplateFascDettagli, getTemplateFascDettagliResult>
    {
        #region Public Members

        public getTemplateFascDettagliHandler(ILogger<getTemplateFascDettagliHandler> logger, IClaimsPrincipalService claimsPrincipalService, IMediator mediator, IPi3DbContext dbContext)
        {
            this._logger = logger;
        ...
        public async Task<getTemplateFascDettagliResult> Handle(Application.Requests.getTemplateFascDettagli request, CancellationToken cancellationToken)
        {
                ...
                if (idTemplate != null)
                {
                    ...
                    var elencoOggettiEntity = await this._dbContext.AssTemplatesFascEntities.AsNoTracking()
                            ...
                            .Select(j => new
                            {
                                ...
                                MULTILINEA = j.ogg.MULTILINEA ?? string.Empty,
                    ...
                    {
                        //Controllo contatore
                        if (!string.IsNullOrEmpty(o.FORMATO_CONTATORE))
                        {
                            var contCustomDoc = _dbContext.ContCustomFascEntities
                                .Where(x => x.ID_OGG_FASC == o.SYSTEM_ID)
                                .ToList()
                                .FirstOrDefault();

                            if (contCustomDoc != null)
                            {
                                o.DATA_INIZIO = contCustomDoc.DATA_INIZIO.ToString();
                                o.DATA_FINE = contCustomDoc.DATA_FINE.ToString();
                            }
                        }
                                        ...
                                        case "CasellaDiSelezione":
                                            if (k < o.VALORI_SELEZIONATI.Length)
                                            {
                                                elencoValoriList.Add(new ValoreOggetto()
                                                {
                                                    ...
                                                    VALORE_DI_DEFAULT = associazioneValori[k].VALORE_DI_DEFAULT ?? string.Empty,
                ...
                else
                {
                    ...
                    if (idTemplateFromProject != null)
                    {
                        ...
                        if (templateFromProject != null)
                        {
                            ...
                            return new getTemplateFascDettagliResult(template);
                        }
        ...
        protected readonly ILogger<getTemplateFascDettagliHandler> _logger;

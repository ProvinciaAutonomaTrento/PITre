...
namespace Pi3.App.Legacy.WebApi.Application.Services.WebMethodLogger
{
    public class WebMethodLoggerEFAgidService : Pi3.Infrastructure.Legacy.EF.Services.WebMethodLogger.WebMethodLoggerEFService
    {
        public WebMethodLoggerEFAgidService(ILogger<WebMethodLoggerEFService> logger, IClaimsPrincipalService claimsPrincipalService, IPi3DbContext dbContext, IMediator mediator) : base(logger, claimsPrincipalService, dbContext)
        {
        ...
        protected override async Task Log(bool result, string webMethodName, string? idObject = null, string? objectDescription = null, string? idSingleTransmission = null, string? application = null, bool? bypassNotifications = null, string? idTenant = null, string? idUser = null, string? userId = null, string? idGroup = null, string? delegatedIdUser = null, DateTime? executionDateTime = null)
        {
            ...
            if(result && !string.IsNullOrEmpty(idObject))
            {
                ...
                {
                    //Inserisco nella coda per la generazione/ modifica dei metadati AGID
                    await this._mediator.Send(
                        new MessageQueueCommandWrapper(new GeneraMetadatiAGIDRequest(this._claimsPrincipalService.Current)
                        {
